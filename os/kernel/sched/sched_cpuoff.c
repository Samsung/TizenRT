 ...
 *   4. If task migration is not requested, then CPU to turn OFF must be
 *      running IDLE task.
 *
 ****************************************************************************/
...
int sched_cpuoff(int cpu, bool migrate)
{
	...
	if ((tcb->pid != cpu) && !migrate) {
		slldbg("Task migration is disabled, but CPU%d not running Idle thread\n", cpu);
		return -ENOTSUP;
	}
