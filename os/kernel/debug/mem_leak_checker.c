...
static bool g_leak_checker_running;
...
static int leak_worklist_init(void)
{
	...
	if (!g_leak_worklist) {
		LEAK_CHECKER_DEBUG("Worklist malloc failed\n");
		return ERROR;
	}
...
static int leak_worklist_push(int block_index)
{
	if (leak_worklist_is_full()) {
		LEAK_CHECKER_DEBUG("Worklist full, cannot push block %d\n", block_index);
		return ERROR;
	}
...
/* Callback for sched_foreach to scan a single thread's stack */
static void leak_scan_stack_callback(FAR struct tcb_s *tcb, FAR void *arg)
{
	int checker_pid = *(int *)arg;

	/* Skip the checker's own stack */
	if (tcb->pid == checker_pid) {
		return;
	}
...
{
	sched_foreach(leak_scan_stack_callback, &checker_pid);
}
...
#define region 0
...
#endif
	{
		...
		for (node = kheap->mm_heapstart[region]; node < kheap->mm_heapend[region]; node = (struct mm_allocnode_s *)((char *)node + node->size)) {
			...
			if ((node->preceding & MM_ALLOC_BIT) != 0) {
				/* Skip our own allocations to prevent false marking */
				if (leak_is_own_node((void *)node)) {
					LEAK_CHECKER_DEBUG("Skipping own node in kernel heap scan: %p (user data: %p)\n",
						(void *)node, (void *)((char *)node + SIZEOF_MM_ALLOCNODE));
					continue;
				}
...
#endif
			{
				...
				for (node = app_heap->mm_heapstart[region]; node < app_heap->mm_heapend[region]; node = (struct mm_allocnode_s *)((char *)node + node->size)) {
					...
					if ((node->preceding & MM_ALLOC_BIT) != 0) {
						/* Skip our own allocations to prevent false marking */
						if (leak_is_own_node((void *)node)) {
							LEAK_CHECKER_DEBUG("Skipping own node in app heap scan (%s): %p (user data: %p)\n",
								BIN_NAME(bin_idx), (void *)node, (void *)((char *)node + SIZEOF_MM_ALLOCNODE));
							continue;
						}
						void *start = (void *)((char *)node + SIZEOF_MM_ALLOCNODE);
						void *end = (void *)((char *)node + node->size);
...
static int run_mem_leak_checker_leak_single(int checker_pid, char *bin_name, int filter_pid)
{
	...
	if (g_leak_checker_running) {
		printf("mem_leak_checker is already running.\n");
		return ERROR;
	}
	g_leak_checker_running = true;
...
cleanup:
	...
	printf("Memory leak checker execution time: %ld ms\n", elapsed_ms);
