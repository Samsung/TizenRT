...
static void sysdbg_print(void)
{
...
#endif
				  cpu,
				  (uint64_t)sysdbg_struct->sched[cpu][idx].time,
#if CONFIG_TASK_NAME_SIZE > 0
	...
	for (cpu = 0; cpu < CONFIG_SMP_NCPUS; cpu++) {
		...
		lldbg("******************************* CPU%d History *******************************\n", cpu);
		idx = sysdbg_struct->irq_lastindex[cpu];
		do {
			lldbg("  %5d%16lld%12d%12X\n", cpu, (uint64_t)sysdbg_struct->irq[cpu][idx].time, sysdbg_struct->irq[cpu][idx].irq, sysdbg_struct->irq[cpu][idx].fn);
			idx--;
			/* Keeping it circular buffer */
			idx = idx & (max_irq_count - 1);
		} while (idx != sysdbg_struct->irq_lastindex[cpu]);
	}
#else /* CONFIG_SMP */
...
void save_semaphore_history(FAR sem_t *sem, void *addr, sem_status_t status)
{
	...
	saved_state = spin_lock_irqsave(&g_sysdbg_lock);
	if (!sysdbg_monitor) {
		lldbg("sysdbg monitoring is not enabled\n");
		spin_unlock_irqrestore(&g_sysdbg_lock, saved_state);
		return;
	}
	if (!sysdbg_struct) {
...
static void sysdbg_monitor_disable(void)
{
	...
	if (sysdbg_struct) {
		...
		for (cpu = 0; cpu < CONFIG_SMP_NCPUS; cpu++) {
			if (sysdbg_struct->irq[cpu]) {
				kmm_free(sysdbg_struct->irq[cpu]);
	...
	if (sysdbg_struct == NULL) {
		...
		for (cpu = 0; cpu < CONFIG_SMP_NCPUS; cpu++) {
			...
			if (sysdbg_struct->sched[cpu] == NULL) {
				...
				/* Free previously allocated CPU buffers */
				while (--cpu >= 0) {
					kmm_free(sysdbg_struct->sched[cpu]);
					sysdbg_struct->sched[cpu] = NULL;
				}
...
#ifdef CONFIG_SMP
		for (cpu = 0; cpu < CONFIG_SMP_NCPUS; cpu++) {
			sysdbg_struct->irq[cpu] = (FAR irq_history_t *)kmm_zalloc(size);
			if (sysdbg_struct->irq[cpu] == NULL) {
