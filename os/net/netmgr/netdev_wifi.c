	...
	do {													\
		if (sem_wait(&queue.lock) != 0) {					\
			...
			if (derr_no == EINTR) {							\
				...
				NET_LOGKE(NL_MOD_NET_MANAGER, "lock fail %d\n", derr_no);	\
				assert(0);									\
			}												\
		}													\
	} while (0)
...
#define UNLOCK_EVTQUEUE(queue)							\
	do {												\
		if (sem_post(&queue.lock) != 0) {				\
			NET_LOGKE(NL_MOD_NET_MANAGER, "unlock fail %d\n", errno);	\
			assert(0);									\
		}												\
	} while (0)
	...
	do {															\
		if (sem_wait(&queue.signal) != 0) {							\
			...
			if (derr_no == EINTR) {									\
				...
				NET_LOGKE(NL_MOD_NET_MANAGER, "wait signal error %d\n", derr_no);	\
				assert(0);											\
			}														\
		}															\
	} while (0)
...
#define SIGNAL_EVTQUEUE(queue)								\
	do {													\
		if (sem_post(&queue.signal) != 0) {					\
			NET_LOGKE(NL_MOD_NET_MANAGER, "send signal fail %d\n", errno);	\
			assert(0);										\
		}													\
	} while (0)
...
int netdev_handle_wifi(struct netdev *dev, lwnl_req cmd, void *data, uint32_t data_len)
{
	...
	NET_LOGKV(NL_MOD_NET_MANAGER, "T%d cmd(%d) (%p) (%d)\n", getpid(), cmd.type, data, data_len);
	switch (cmd.type) {
	case LWNL_REQ_WIFI_INIT:
	{
	...
	if (res == TRWIFI_SUCCESS) {
		if (_trwifi_handle_command(dev, cmd) < 0) {
			...
			NET_LOGKE(NL_MOD_NET_MANAGER, "critical error network stack is not enabled\n");
			assert(0);
		}
	}
	return res;
...
int trwifi_serialize_scaninfo(uint8_t **buffer, trwifi_scan_list_s *scan_list)
{
	...
	NET_LOGKV(NL_MOD_NET_MANAGER, "total size(%d) (%d) \n", sizeof(trwifi_ap_scan_info_s), total);

	*buffer = (uint8_t *)kmm_malloc(total);
	if (!(*buffer)) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "malloc fail %d\n", total);
		return -1;
	}
...
void *_trwifi_event_handler(void *arg)
{
	...
	NET_LOGKE(NL_MOD_NET_MANAGER, "critical error\n", errno);
	return NULL;
}
...
int trwifi_run_handler(void)
{
	if (sem_init(&g_queue.lock, 0, 1) != 0) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "init lock fail %d\n", errno);
		return -1;
	}
	if (sem_init(&g_queue.signal, 0, 0) != 0) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "init signal fail %d\n", errno);
		return -1;
	}
	...
	if (tid == ERROR) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "critical error %d\n", errno);
		return -2;
	}
	return 0;
}
