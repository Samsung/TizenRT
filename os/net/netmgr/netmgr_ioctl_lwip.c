...
static struct netif *_netdev_dhcp_dev(FAR const char *intf)
{
	struct netif *cnif;
	if (intf == NULL) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "Input NULL\n");
		return NULL;
	}

	cnif = GET_NETIF_FROM_NETDEV(nm_get_netdev((uint8_t *)intf));
	if (cnif == NULL) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "No netif which has the name of %s\n", intf);
		return NULL;
	}
	return cnif;
}
...
static int _netdev_set_dnsserver(struct sockaddr *addr, int index)
{
	...
	if (index > 0) {
		if (index >= CONFIG_NET_DNS_MAX_SERVERS) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "index go beyond the limits index(%d) max(%d)\n",
	...
	NET_LOGKE(NL_MOD_NET_MANAGER, "set dnsserver fail\n");
	return -3;
}
...
{
	NET_LOGKV(NL_MOD_NET_MANAGER, "LWIP DHCPC started (%s)\n", intf);
	struct netif *cnif;
	cnif = _netdev_dhcp_dev(intf);
	if (cnif == NULL) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "No network interface for dhcpc %s\n", intf);
		return ERROR;
	}
	...
	if (res) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "DHCP client start failed %d\n", res);
		return ERROR;
	}

	NET_LOGKV(NL_MOD_NET_MANAGER, "DHCP client started successfully, waiting IP address (timeout %d msecs)\n", timeleft);
	while (netifapi_dhcp_address_valid(cnif) != 0) {
		usleep(100000);
		timeleft -= 100;
		if (timeleft <= 0) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "DHCP client timeout\n");
			netifapi_dhcp_stop(cnif);
			return ERROR;
		}
	}
	...
	NET_LOGKV(NL_MOD_NET_MANAGER, "Got IP address %s\n", inet_ntoa(ip_check));
...
int _netdev_dhcpc_stop(const char *intf)
{
	...
	netifapi_dhcp_stop(cnif);
	NET_LOGKV(NL_MOD_NET_MANAGER, "Release IP address (lwip)\n");
...
{
	NET_LOGKI(NL_MOD_NET_MANAGER, "dhcpd joined\n");
	...
	if (!md) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "mq open fail (errno %d)\n", errno);
		mq_close(md);
		return;
	}
	...
	if (mq_ret < 0) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "send mq fail (errno %d)\n", errno);
	}

	if (mq_close(md) != 0) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "fail to close mq\n");
	}
...
static struct addrinfo *_netdev_copy_addrinfo(struct addrinfo *src)
{
	...
	while (tmp) {
		...
		if (!dst) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "kumm_malloc failed\n");
			break;
		}
...
static int lwip_func_ioctl(int s, int cmd, void *arg)
{
	...
	if (!sock) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "get socket fail\n");
		ret = -EBADF;
		return ret;
	}
	struct req_lwip_data *req = (struct req_lwip_data *)arg;
	if (!req) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "invalid parameter req\n");
		goto errout;
	}
	...
	case GETADDRINFO:
		...
		if (req->req_res != 0) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "lwip_getaddrinfo() returned with the error code: %d\n", req->req_res);
	...
	case GETHOSTBYNAME:
		...
		if (!host_ent) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "lwip_gethostbyname() returned with the error code: %d\n", HOST_NOT_FOUND);
			ret = -EINVAL;
		} else {
	...
	case GETNAMEINFO:
		req->req_res = lwip_getnameinfo(req->msg.netdb.sa,
										...
										req->msg.netdb.flags);
		if (req->req_res != 0) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "lwip_getnameinfo() returned with the error code: %d\n", req->req_res);
			ret = -EINVAL;
		} else {
			ret = OK;
		}
	...
	case DHCPCSTART:
		...
		if (req->req_res != OK) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "Start DHCP clent failed %d\n", req->req_res);
			goto errout;
		}
		ret = OK;
		break;
	case DHCPCSTOP:
		...
		if (req->req_res != OK) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "Stop DHCP client failed %d\n", req->req_res);
			goto errout;
		}
		ret = OK;
		break;
	...
	case DHCPDSTART: {
		...
		if (cnif == NULL) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "No network interface for dhcpd start\n");
			req->req_res = ERROR;
			goto errout;
		}
		if (dhcps_register_cb(_netdev_dhcp_join_cb) != OK) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "Register link callback failed\n");
			req->req_res = ERROR;
			goto errout;
		}
		if ((req->req_res = netifapi_dhcps_start(cnif)) != OK) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "DHCP server start failed (LWIP)\n");
			goto errout;
		}
		NET_LOGKV(NL_MOD_NET_MANAGER, "DHCP server started successfully (LWIP)\n");
		req->req_res = OK;
		ret = OK;
		break;
	}
	case DHCPDSTOP: {
		struct netif *cnif = _netdev_dhcp_dev((const char *)req->msg.dhcp.intf);
		if (cnif == NULL) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "No network interface for dhcpd stop\n");
			req->req_res = ERROR;
			goto errout;
		}
		if (cnif->dhcps_pcb == NULL) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "stop dhcpd fail: no pcb\n");
			req->req_res = ERROR;
			goto errout;
		}
		netifapi_dhcps_stop(cnif);
		NET_LOGKV(NL_MOD_NET_MANAGER, "DHCP server stopped successfully (LWIP)\n");
		req->req_res = OK;
		ret = OK;
		break;
	}
	case DHCPDSTATUS: {
		struct netif *cnif = _netdev_dhcp_dev((const char *)req->msg.dhcp.intf);
		if (cnif == NULL) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "No network interface for dhcpd status\n");
			req->req_res = ERROR;
			goto errout;
		}
		if (cnif->dhcps_pcb == NULL) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "No running DHCP server\n");
			req->req_res = ERROR;
			goto errout;
		}
		NET_LOGKV(NL_MOD_NET_MANAGER, "DHCP server opened\n");
		req->req_res = OK;
		ret = OK;
		break;
	}
	...
	default:
		NET_LOGKE(NL_MOD_NET_MANAGER, "Wrong request type: %d\n", req->type);
		ret = -ENOTTY;
		break;
	}
	return ret;
