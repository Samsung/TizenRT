...
#ifdef CONFIG_NET_IPv6
static void ioctl_getip6addr(FAR struct sockaddr_storage *outaddr, ip6_addr_t * inaddr)
{
...
static void _ioctl_setip6addr(ip6_addr_t * outaddr, FAR const struct sockaddr_storage *inaddr)
{
...
static void _netif_setip6addr(struct netif *dev, FAR const struct sockaddr_storage *inaddr)
{
	...
	_ioctl_setip6addr(ip_2_ip6(&dev->ip_addr), inaddr);
...
static int lwip_get_ip4addr(struct netdev *dev, struct sockaddr *addr, int type)
{
	...
	in_addr_t inaddr;
	if (type == NETDEV_IP) {
		inaddr = ip4_addr_get_u32(ip_2_ip4(&ni->ip_addr));
	} else if (type == NETDEV_GW) {
		inaddr = ip4_addr_get_u32(ip_2_ip4(&ni->gw));
	} else if (type == NETDEV_NETMASK) {
		inaddr = ip4_addr_get_u32(ip_2_ip4(&ni->netmask));
	} else {
		return -1;
	}

	FAR struct sockaddr_in *dest = (FAR struct sockaddr_in *)addr;
...
static int lwip_set_ip4addr(struct netdev *dev, struct sockaddr *addr, int type)
{
	...
	if (type == NETDEV_IP) {
		ip4_addr_get_u32(ip_2_ip4(&ipaddr)) = src->sin_addr.s_addr;
	} else if (type == NETDEV_GW) {
		ip4_addr_get_u32(ip_2_ip4(&gw)) = src->sin_addr.s_addr;
	} else if (type == NETDEV_NETMASK) {
		ip4_addr_get_u32(ip_2_ip4(&netmask)) = src->sin_addr.s_addr;
	}
...
static int lwip_get_ip6addr(struct netdev *dev, struct sockaddr_storage *addr, int type)
{
	struct netif *ni = GET_NETIF_FROM_NETDEV(dev);
	if (type == NETDEV_IP) {
		ioctl_getip6addr(addr, ip_2_ip6(&ni->ip_addr));
	} else if (type == NETDEV_GW) {
		ioctl_getip6addr(addr, ip_2_ip6(&ni->gw));
	} else if (type == NETDEV_NETMASK) {
		ioctl_getip6addr(addr, ip_2_ip6(&ni->netmask));
	} else {
		ndbg("unknown type\n");
		return -1;
	}
...
static int lwip_set_ip6addr(struct netdev *dev, struct sockaddr_storage *addr, int type)
{
	...
	if (type == NETDEV_IP) {
		...
		_ioctl_setip6addr(ip_2_ip6(&ni->gw), addr);
	} else if (type == NETDEV_NETMASK) {
		_ioctl_setip6addr(ip_2_ip6(&ni->netmask), addr);
	} else {
		ndbg("unknown type\n");
		return -1;
	}
...
struct netdev_ops *get_netdev_ops_lwip(void)
{
	...
	netdev_ops->get_ip6addr = lwip_get_ip6addr;
