...
{
	if (!callback) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "invalid callback\n");
		return -1;
	}
...
struct netdev *nm_register(struct netdev_config *config)
{
	...
	if (g_netdev_idx == CONFIG_NETDEV_NUM) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "No available netdev slot %d %d\n",
	...
	if (config->type == NM_LOOPBACK) {
		...
		if (res < 0) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "initialize loopback fail\n");
			return NULL;
		}
		return dev;
	...
	if (!dev->tx_buf) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "create txbuf fail(%d)\n", config->mtu + 12);
		return NULL;
	}
	struct nic_config nconfig;
...
int nm_ifup(struct netdev *dev)
{
	...
	if (dev->type == NM_WIFI) {
		...
		if (ret < 0) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "fail to up wi-fi driver interface\n");
			return -2;
		}
	} else if (dev->type == NM_ETHERNET) {
		ret = dev->t_ops.eth->init(dev);
		if (ret < 0) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "fail to intialize ethernet driver interface\n");
			return -1;
		}
		ret = dev->t_ops.eth->enable(dev);
		if (ret < 0) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "fail to up ethernet driver interface\n");
			return -1;
		}
	}
	...
	if (ret < 0) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "fail to link up network interface\n");
		return -1;
	}
	return 0;
}
...
int nm_ifdown(struct netdev *dev)
{
	...
	if (ret < 0) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "fail to link down down network interface\n");
		return -1;
	}
	...
	if (dev->type == NM_WIFI) {
		...
		if (ret < 0) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "fail to deinit wi-fi driver interface\n");
			return -2;
		}
	} else if (dev->type == NM_ETHERNET) {
		ret = dev->t_ops.eth->deinit(dev);
		if (ret < 0) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "fail to deinit etherent driver\n");
			return -1;
		}
	}
