...
void net_setup(void)
{
	...
	if (!g_netmgr.dev) {
		...
		if (!g_netmgr.dev) {
			NET_LOGKE(NL_MOD_NET_MANAGER, "!!!alloc dev fail!!!\n");
		}
	}

	res = lwnl_register((struct lwnl_lowerhalf_s *)g_netmgr.dev);
	if (res < 0) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "!!!register device fail!!!\n");
	}
#endif
	...
	if (res < 0) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "!!!initialize stack fail!!!\n");
	}
	netdev_mgr_start();
}
...
void net_initialize(void)
{
	...
	if (res < 0) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "!!!start stack fail!!!\n");
		assert(0);
	}
	if (trwifi_run_handler() != 0) {
		NET_LOGKE(NL_MOD_NET_MANAGER, "!!!start event handler fail!!!\n");
		assert(0);
	}
