 ...
 */
#ifdef PEDANTIC_ARGS
static void *Cmalloc(int n, void *heap)
{
	return malloc(n);
}

static void Cfree(void *p, void *heap)
{
	free(p);
}
...
_TYPE(void) cstr_set_allocator(cstr_allocator *alloc)
{
	default_alloc = alloc;
}
...
_TYPE(void) cstr_clear_free(cstr *str)
{
	if (--str->ref == 0) {
		if (str->cap > 0) {
...
_TYPE(void) cstr_empty(cstr *str)
{
	if (str->cap > 0) {
		(*str->allocator->free)(str->data, str->allocator->heap);
	}
...
static int cstr_alloc(cstr *str, int len)
{
	char *t;
	...
	if (len > str->cap) {
		...
		t = (char *)(*str->allocator->alloc)(len * sizeof(char), str->allocator->heap);
		if (t) {
			if (str->data) {
				t[str->length] = 0;
				if (str->cap > 0) {
