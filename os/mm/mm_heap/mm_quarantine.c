...
bool mm_quarantine_add(FAR struct mm_heap_s *heap, FAR struct mm_allocnode_s *node)
{
	...
	/* Calculate the quarantine area early to avoid evicting useful chunks
	 * for chunks that cannot be verified. The poison starts after
	 * SIZEOF_MM_FREENODE, so for small chunks (e.g., 32 bytes or less),
	 * nbytes will be 0 - nothing to poison or verify.
	 */
