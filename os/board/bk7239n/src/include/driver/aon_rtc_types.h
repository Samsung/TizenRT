...
#define AON_RTC_EXTERN_32K_CLOCK_FREQ (32768)
#define AON_RTC_DEFAULT_CLOCK_FREQ    (32000)

#define AON_RTC_MS_TICK_CNT           (bk_rtc_get_ms_tick_count())
...
#ifdef CONFIG_AON_RTC_64BIT
#define AON_RTC_ROUND_TICK (0XFFFFFFFFFFFFFFFF)	//we can set to other value, but maybe easy to cause a new round.
typedef uint64_t rtc_tick_t;
#elif defined(CONFIG_ANA_RTC)
#define AON_RTC_ROUND_TICK (0XFFFFFFFFF)	//we can set to other value, but maybe easy to cause a new round.
typedef uint64_t rtc_tick_t;
#else
...
typedef struct _alarm_node_t
{
	...
	uint8_t name[ALARM_NAME_MAX_LEN+1];
	rtc_tick_t period_tick;	//tick:AON RTC use it
	uint32_t period_cnt;	//total period count == 0Xffffffff means forever period;else period how many times
	aon_rtc_isr_t callback;
	void *cb_param_p;
