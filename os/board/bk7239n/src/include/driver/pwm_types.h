...
#define PWM_PHASE_SHIFT_CHAN_NUM    6
...
typedef struct {
	...
	uint32_t psc;           /*!< Specifies the prescaler value used to divide the TIM clock.
	                             This parameter can be a number between Min Data=0x00 and Max Data= 0xFF */
...
typedef struct {
	uint32_t psc;              /**< PWM prescaler value used to divide the TIM clock */
	uint32_t period_cycle;     /**< PWM period cycle */
	uint32_t chan_num;         /**< PWM phase shift channel number */
