...
//SW combines many payloads to a bigger packet
typedef struct sdio_sw_pbd {
	uint16_t len;	//payload len:valid data len, include sdio_sw_pbd_t, it will be pack in the packet.
	uint8_t type;	//chan
	uint8_t sub_type;
} sdio_sw_pbd_t, *sdio_sw_pbd_ptr_t;
typedef struct _sdio_node_t
{
	struct _sdio_node_t *next;
	uint32_t len;	//payload len:valid data len, include sdio_sw_pbd_t, it will be pack in the packet.
	uint32_t priv;
	uint32_t rsvd;
