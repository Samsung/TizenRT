...
static uint32_t rx_wait_ms = 1000;
#endif
static bk_err_t sdio_test_tx_cb(uint32_t chan_id, sdio_node_ptr_t head_p, sdio_node_ptr_t tail_p, uint32_t count)
{
	bk_err_t ret = BK_OK;
...
static void sdio_test_tx_tp_thread(void *arg)
{
	...
	while(1)
	{
		...
		if((ret == BK_OK) && buf_p)
		{
			...
			len = size - 4;
			uint32_t *ptmp = (uint32_t *)((uint8_t *)buf_p + sizeof(sdio_node_t) + sizeof(sdio_sw_pbd_t));
			for (int i = 0; i < len/4; i++) {
				if(i == 0) {
					*ptmp = SDIO_TEST_TX_TP_MSG_ID;   //TX_TP_MSG_ID
				} else if(i == 1) {
					*ptmp = (len << 16);   //MSG_LEN
				} else {
			...
			val_test[chan_id]++;
			SDIO_FILL_PAYLOAD_CHAN_ID(buf_p, chan_id);
			SDIO_FILL_PAYLOAD_LEN(buf_p, len);
...
static bk_err_t sdio_test_tx_tp_init(uint32_t chan_id, uint32_t buf_cnt, uint32_t buf_size)
{
	...
	if(s_tx_thread_handle[chan_id] == NULL)
	{
		...
		//snprintf(str_name, sizeof(str_name), "%s_ch%u_dir%d", SDIO_TEST_TP_THREAD_NAME, chan_id, SDIO_CHAN_TX);
		ret = rtos_create_thread(
				&s_tx_thread_handle[chan_id],
				SDIO_TEST_TX_TP_PRIORITY,
				"dbg_tx_tp_thd",
	...
	{
		//memset(str_name, 0, sizeof(str_name));
		//snprintf(str_name, sizeof(str_name), "%s_ch%u", SDIO_TEST_TX_TP_MSG_QUEUE_NAME, chan_id);
...
static bk_err_t sdio_test_tx_sub_case(SDIO_TEST_MSG_T *msg_p)
{
	...
	switch(msg_p->sub_case)
	{
		...
		case SDIO_TEST_TX_SINGLE_PACKET:
		{
			...
			uint32_t len = msg_p->param2;
			uint32_t value = (uint32_t)msg_p->param3;
			uint32_t size;
			ret = bk_sdio_chan_pop_free_node(chan_id, SDIO_CHAN_TX, &buf_p, &size);
			if((ret == BK_OK) && buf_p)
			{
				...
				{
					for (int i = 0; i < len; i++) {
						*(uint8_t *)(((uint8_t *)buf_p) + sizeof(sdio_node_t) + sizeof(sdio_sw_pbd_t) + i) = i & 0xff;
					}
				}
				else
				{
					memset((uint8_t *)buf_p + sizeof(sdio_node_t) + sizeof(sdio_sw_pbd_t), value, len);
				}
				if (SDIO_CHAN_LOOPCHECK == chan_id) {
					static uint16_t sdio_loopcheck_ack_cnt = 0;
					*(uint32_t *)(((uint8_t *)buf_p) + sizeof(sdio_node_t) + sizeof(sdio_sw_pbd_t)) = (SDIO_TEST_TX_LOOPCHECK_MSG_ID  + ((sdio_loopcheck_ack_cnt++) << 16) ) ;
					*(uint32_t *)(((uint8_t *)buf_p) + sizeof(sdio_node_t) + sizeof(sdio_sw_pbd_t) + sizeof(uint32_t)) = (len << 16);
				}
				SDIO_FILL_PAYLOAD_LEN(buf_p, len);
				ret = bk_sdio_slave_sync_write(chan_id, buf_p, buf_p, 1);
			}
...
static bk_err_t sdio_test_rx_cb(uint32_t chan_id, sdio_node_ptr_t head_p, sdio_node_ptr_t tail_p, uint32_t count)
{
	bk_err_t ret = BK_OK;
...
//not print received data by default
#if 1
			os_printf("chan_id=0x%x, payload cur_p=0x%08x \r\n", chan_id, cur_p);
			for(j = 0; j < 20; j += sizeof(uint32_t))
			{
...
#else
	print_len = (*(uint32_t *)((uint8_t *)cur_p + sizeof(sdio_node_t)) ) & 0xffff;
	os_printf("chan_id=0x%x, print_len=0x%x \r\n", chan_id, print_len);
	...
	else	//sync:
	{
		...
		while(cur_p && (i > 0))
		{
			SDIO_LOG_DEBUG("sync: chan_id=0x%x, payload cur_p=0x%08x \r\n", chan_id, cur_p);
			for(j = 0; j < print_len; j += sizeof(uint32_t) * 4)
			{
				SDIO_LOG_DEBUG("0x%08x 0x%08x 0x%08x 0x%08x", *(uint32_t *)((uint8_t *)cur_p + (sizeof(sdio_node_t)) + j),
											*(uint32_t *)((uint8_t *)cur_p + (sizeof(sdio_node_t)) + j+4),
											*(uint32_t *)((uint8_t *)cur_p + (sizeof(sdio_node_t)) + j+8),
											*(uint32_t *)((uint8_t *)cur_p + (sizeof(sdio_node_t)) + j+12));
			}
			cur_p = (sdio_node_ptr_t)cur_p->next;
			i--;
		}
		...
		//use loopcheck channel to ACK host
		if (SDIO_CHAN_LOOPCHECK == chan_id) {
			BK_DUMP_OUT("=== sdio loopcheck chn%d msg recived. ===\r\n", chan_id);
			bk_sdio_set_test_case(SDIO_TEST_RX, SDIO_TEST_RX_SINGLE_PACKET, chan_id, SDIO_LOOPCHECK_BUF_SIZE, 0, 1); //rx, single_packet, chan_id, len, value, packet_cnt
			bk_sdio_set_test_case(SDIO_TEST_TX, SDIO_TEST_TX_INIT, chan_id, SDIO_LOOPCHECK_BUF_SIZE, 1, 1);
			bk_sdio_set_test_case(SDIO_TEST_TX, SDIO_TEST_TX_SINGLE_PACKET, chan_id, (SDIO_LOOPCHECK_BUF_SIZE - 4), 12345678, 1);
			//Use loopcheck channel specail data type to enable throughput auto test, thus auto test case is triggered by HOST
			if(0x01314520 == (*(uint32_t *)((uint8_t *)head_p + (sizeof(sdio_node_t)) + 4)) ) {
				bk_sdio_auto_test();
			}
		}
	}
...
static void sdio_test_rx_tp_thread(void *arg)
{
	...
	while(1)
	{
		...
		ret = rtos_pop_from_queue(&s_sdio_ch_x_rx_msg_que[chan_id], &msg, SDIO_TEST_RX_TP_MSG_WAIT_TIME);
		if (kNoErr == ret)
		{
			sdio_list_t list;
			//deal APP data
...
static bk_err_t sdio_test_rx_tp_init(uint32_t chan_id, uint32_t buf_cnt, uint32_t buf_size)
{
	...
	{
		//memset(str_name, 0, sizeof(str_name));
		//snprintf(str_name, sizeof(str_name), "%s_ch%u_dir%d", SDIO_TEST_TP_THREAD_NAME, chan_id, SDIO_CHAN_RX);
		ret = rtos_create_thread(
				&s_rx_thread_handle[chan_id],
				SDIO_TEST_RX_TP_PRIORITY,
				"dbg_rx_tp_thd",
	...
	{
		//memset(str_name, 0, sizeof(str_name));
		//snprintf(str_name, sizeof(str_name), "%s_ch%u", SDIO_TEST_RX_TP_MSG_QUEUE_NAME, chan_id);
...
static bk_err_t sdio_test_rx_sub_case(SDIO_TEST_MSG_T *msg_p)
{
	...
	switch(msg_p->sub_case)
	{
		...
		case SDIO_TEST_RX_SINGLE_PACKET:
		{
			...
			sdio_chan_id_t chan_id  = msg_p->param1;
			uint32_t len = msg_p->param2;
			uint8_t value = (uint8_t)msg_p->param3;
			uint32_t size = 0;
			ret = bk_sdio_chan_pop_free_node(chan_id, SDIO_CHAN_RX, &buf_p, &size);
			if((ret == BK_OK) && buf_p)
			{
		...
		if (kNoErr == ret)
		{
			switch(msg.main_case)
			{
				...
				case SDIO_TEST_CFG:
				{
					switch(msg.sub_case)
					{
						case SDIO_TEST_CFG_LOG_LEVEL:
						{
							sdio_set_log_level(msg.param1);
							break;
						}
						default:
							break;
...
bk_err_t bk_sdio_auto_test(void)
{
	bk_err_t ret = BK_OK;
	SDIO_LOG_DEBUG_FUNCTION_ENTRY();
	sdio_test_rx_tp_init(SDIO_CHAN_TP_TEST, 5, SDIO_BUFFER_SIZE);  //chan buf_cnt, buf_size
	s_rx_tp_sub_case[SDIO_CHAN_TP_TEST] = 1;
	sdio_test_tx_tp_init(SDIO_CHAN_TP_TEST, 5, SDIO_BUFFER_SIZE);  //chan buf_cnt, buf_size

#if CONFIG_TRNG_SUPPORT
	wait_ms = 100;
#endif

	s_tx_tp_sub_case[SDIO_CHAN_TP_TEST] = 1;
	SDIO_LOG_DEBUG_FUNCTION_EXIT();
...
bk_err_t bk_sdio_loopcheck_init(void)
{
	sdio_node_ptr_t buf_p = NULL;
	uint32_t size = 0;
	sdio_chan_id_t chan_id = SDIO_CHAN_LOOPCHECK;
	uint32_t buf_cnt = 2;
	uint32_t buf_size = SDIO_LOOPCHECK_BUF_SIZE;
	bk_err_t ret = BK_OK;
	sdio_test_rx_init(chan_id, buf_cnt, buf_size);
	ret = bk_sdio_chan_pop_free_node(chan_id, SDIO_CHAN_RX, &buf_p, &size);
	if((ret == BK_OK) && buf_p)
	{
		//request read
