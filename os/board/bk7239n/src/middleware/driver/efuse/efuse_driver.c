...
#define EFUSE_OPERT_ADDRE                (0x0)//byte1
#define EFUSE_SECURITY_BOOT_ENABLE_POS  (0x0)
#define EFUSE_BOOT_MODE_POS             (0x3)
...
bk_err_t bk_efuse_driver_init(void)
{
	...
	bk_efuse_read_byte(EFUSE_OPERT_ADDRE, &efuse_data);
	if(efuse_data&(0x1<< EFUSE_SECURITY_BOOT_ENABLE_POS)&&( efuse_data&(0x1<< EFUSE_BOOT_MODE_POS)))
	{
		os_printf("\r\nHW SECURE ENABLE\r\n");
	}
        else
	{
		os_printf("\r\nHW SECURE NOT ENABLE\r\n");
	}
