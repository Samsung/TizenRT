...
int mdio_register(struct mii_dev *bus)
{
	...
	if (miiphy_get_dev_by_name(bus->name)) {
		ETH_LOGI("mdio_register: non unique device name '%s'\n",
			bus->name);
		return -1;
	}
...
void mdio_list_devices(void)
{
	...
	list_for_each(entry, &mii_devs) {
		...
		for (i = 0; i < PHY_MAX_ADDR; i++) {
			...
			if (phydev) {
				ETH_LOGI("%x - %s", i, phydev->drv->name);

				if (phydev->dev)
					ETH_LOGI(" <--> %s\n", netif_name(phydev->dev));
				else
					ETH_LOGI("\n");
			}
		}
...
struct phy_device *mdio_phydev_for_ethname(const char *ethname)
{
	...
	list_for_each(entry, &mii_devs) {
		...
		for (i = 0; i < PHY_MAX_ADDR; i++) {
			...
			if (strncmp(netif_name(bus->phymap[i]->dev), ethname, 2) == 0)
				return bus->phymap[i];
		}
	}

	ETH_LOGI("%s is not a known ethernet\n", ethname);
	return NULL;
}
		 ...
		 unsigned char *model, unsigned char *rev)
{
	...
	if (miiphy_read(devname, addr, MII_PHYSID2, &tmp) != 0) {
		ETH_LOGI("PHY ID register 2 read failed\n");
		return -1;
	}
	reg = tmp;

	ETH_LOGI("MII_PHYSID2 @ 0x%x = 0x%04x\n", addr, reg);
	...
	if (miiphy_read(devname, addr, MII_PHYSID1, &tmp) != 0) {
		ETH_LOGI("PHY ID register 1 read failed\n");
		return -1;
	}
	reg |= tmp << 16;
	ETH_LOGI("PHY_PHYIDR[1,2] @ 0x%x = 0x%08x\n", addr, reg);
...
int miiphy_speed(const char *devname, unsigned char addr)
{
	...
	if (bmcr & BMCR_ANENABLE) {
		...
		if (miiphy_read(devname, addr, MII_LPA, &anlpar)) {
			ETH_LOGI("PHY AN speed");
			goto miiphy_read_failed;
		}

		if (miiphy_read(devname, addr, MII_ADVERTISE, &adv)) {
			ETH_LOGI("PHY AN adv speed");
			goto miiphy_read_failed;
		}
...
miiphy_read_failed:
	ETH_LOGI(" read failed, assuming 10BASE-T\n");
	return _10BASET;
}
...
int miiphy_duplex(const char *devname, unsigned char addr)
{
	...
	if (miiphy_is_1000base_x(devname, addr)) {
		/* 1000BASE-X */
		if (miiphy_read(devname, addr, MII_LPA, &anlpar)) {
			ETH_LOGI("1000BASE-X PHY AN duplex");
			goto miiphy_read_failed;
		}
	}
	/*
	...
	if (bmcr & BMCR_ANENABLE) {
		/* Get auto-negotiation results. */
		if (miiphy_read(devname, addr, MII_LPA, &anlpar)) {
			ETH_LOGI("PHY AN duplex");
			goto miiphy_read_failed;
		}

		if (miiphy_read(devname, addr, MII_ADVERTISE, &adv)) {
			ETH_LOGI("PHY AN adv duplex");
			goto miiphy_read_failed;
		}
...
miiphy_read_failed:
	ETH_LOGI(" read failed, assuming half duplex\n");
	return HALF;
}
...
int miiphy_is_1000base_x(const char *devname, unsigned char addr)
{
	...
	if (miiphy_read(devname, addr, MII_ESTATUS, &exsr)) {
		ETH_LOGI("PHY extended status read failed, assuming no "
			"1000BASE-X\n");
		return 0;
	}
...
int miiphy_link(const char *devname, unsigned char addr)
{
	...
	if (miiphy_read(devname, addr, MII_BMSR, &reg)) {
		ETH_LOGI("MII_BMSR read failed, assuming no link\n");
		return 0;
	}
