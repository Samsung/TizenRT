...
extern volatile uint8_t g_sdio_log_level;
#define SDIO_LOG_OUTPUT_LEVEL (g_sdio_log_level)
...
#else
#define SDIO_LOG_ERR(...) do {{ \
	BK_LOG_RAW("SDIO ERR(%s:%d) ", __FUNCTION__, __LINE__); \
	BK_LOG_RAW(__VA_ARGS__); \
	BK_LOG_RAW("\r\n"); \
}}while(0)
#define SDIO_LOG_WARNING(...)
#define SDIO_LOG_INFO(...)
#define SDIO_LOG_DEBUG(...)
#define SDIO_LOG_DUMP(...)
#define SDIO_LOG_DEBUG_FUNCTION_ENTRY(...)
#define SDIO_LOG_DEBUG_FUNCTION_EXIT(...)
#endif
#ifdef CONFIG_SDIO_DEBUG_EN
...
#endif
/*
 If SDIO transfer data block by block,and two naibor blocks data hasn't any clock cycles
 delay in SDIO wires,host side will get error data.
 1. It can't drive to transfer data by DMA, though DMA transfers data with 508,512,512...,516
    because DMA speed is faster then SDIO wires, maybe after many transactions, DMA catches
    up SDIO wires transactions,then two blocks in SDIO wires hasn't delay.
 2. Drive transfers data by SDIO block end int,and SDIO block end int
    minimum length is one block size which is 512 bytes,maybe many front payloads
    sum size is less then 512 bytes,then it can't be drived by SDIO block end int yet.
 3. Add memcpy step to pack all of payloads to a bigger packet.
*/
#define CONFIG_SDIO_SLAVE_TX_MEMCPY_EN (1)
#define SDIO_DMA_SPLIT_FIRST_SIZE ((2*SDIO_BLOCK_SIZE)-4)
#define CONFIG_SDIO_SLAVE_RX_MEMCPY_EN (1)	//DMA copy all RX data to a temp buffer and then parse it to payloads buffer
#if CONFIG_SDIO_SLAVE_RX_MEMCPY_EN
#define SDIO_RX_MEMCPY_PACK_CNT (2)
#define SDIO_RX_MEMCPY_PACK_SIZE (16<<10)
#endif
#define SDIO_THREAD_PRIORITY (2)
#define SDIO_THREAD_NAME "sdio_thread"
#define SDIO_THREAD_STACK_SIZE (0x4<<10)	//TODO:have to check realy size
#if CONFIG_SDIO_SLAVE_TX_MEMCPY_EN
#define SDIO_TX_PACK_FULL_TIMEOUT (1000)
#define SDIO_MEMCPY_THREAD_PRIORITY (2)
#define SDIO_MEMCPY_THREAD_NAME "sdio_memcpy_thread"	//tx only until now:2024-05-27
#define SDIO_MEMCPY_THREAD_STACK_SIZE (0x4<<10)	//TODO:have to check realy size
#define SDIO_MEMCPY_PACK_CNT (3)
#define SDIO_MEMCPY_PACK_SIZE (16<<10)
#define SDIO_MEMCPY_PACK_USED_LEN(index) (((uint32_t)s_sdio_driver.pack.end_addr[index] - (uint32_t)s_sdio_driver.pack.start_addr[index]))
#endif
...
#define SDIO_CMD_INDEX_0	(0)
#define SDIO_CMD_INDEX_5	(5)
#define SDIO_CMD_INDEX_52	(52)
#define SDIO_CMD_INDEX_53	(53)
#define SDIO_CMD_FIFO_NUM	(5)
#define SDIO_BLOCK_SIZE (0x200)		//512 bytes per round
#define SDIO_BUFF_SIZE  (0x800)		//2048 bytes per round
#define CONFIG_SDIO_ALIGN_BLOCKS (1)
#define SDIO_BYTES_TO_BLOCKS_CNT(x) ((x+(SDIO_BLOCK_SIZE-1))/SDIO_BLOCK_SIZE)	//bytes length to block counts
#define SDIO_ALIGN_BYTES (4)
#if SDIO_ALIGN_BYTES
#define SDIO_ALIGN_LENGTH(x) ((x+(SDIO_ALIGN_BYTES-1))&(~(SDIO_ALIGN_BYTES-1)))
#else
#define SDIO_ALIGN_LENGTH(x) (x)
#endif
#define CONFIG_SDIO_PACK_END_FLAG (1)
#define SDIO_PACK_END_MAGIC (0)
#define SDIO_DATA_PTR_TO_NODE_PTR(x) (void *)(((uint32_t)x) - sizeof(sdio_node_t))
#define SDIO_NODE_PTR_TO_DATA_PTR(x) (void *)(((uint32_t)x) + sizeof(sdio_node_t))
#define SDIO_SET_PAYLOAD_VALID_DATA_LEN(sdio_node_p, len) (((sdio_sw_pbd_ptr_t)((uint8_t *)sdio_node_p + sizeof(sdio_node_t)))->len = len&0xffff)
#define SDIO_SET_NODE_VALID_DATA_LEN(sdio_node_p,len) ((sdio_node_ptr_t)sdio_node_p->len = len&0xffff)
#define SDIO_FILL_PAYLOAD_LEN(sdio_node_p, len) { \
	((sdio_node_ptr_t)sdio_node_p)->len = (len&0xffff) + sizeof(sdio_sw_pbd_t); \
	((sdio_sw_pbd_ptr_t)((uint8_t *)sdio_node_p + sizeof(sdio_node_t)))->len = ((sdio_node_ptr_t)sdio_node_p)->len; \
}
#define SDIO_GET_PAYLOAD_LEN(sdio_node_p) (((sdio_sw_pbd_ptr_t)((uint8_t *)sdio_node_p + sizeof(sdio_node_t)))->len)
#define SDIO_FILL_PAYLOAD_CHAN_ID(sdio_node_p, chan_id) { \
	((sdio_sw_pbd_ptr_t)((uint8_t *)sdio_node_p + sizeof(sdio_node_t)))->type = chan_id; \
}
#define SDIO_NODE_PAYLOAD_CHAN_ID(sdio_node_p) ((sdio_sw_pbd_ptr_t)((uint8_t *)sdio_node_p + sizeof(sdio_node_t)))->type
...
typedef enum
{
	...
	SDIO_READ_PACK_FINISH,			//read all data from SDIO FIFO to RAM
	SDIO_READ_BLK_FINISH,           //read each block data finish
	SDIO_MSG_SYNC_READ,
	SDIO_MSG_ASYNC_READ,

	//WRITE
	SDIO_APP_REQUEST_WRITE,			//APP requests SDIO driver to write data
	...
	SDIO_WRITE_TO_FIFO_FINISH,		//Current round DMA finish,from RAM to SDIO FIFO

	SDIO_WRITE_NODE_FINISH,
	SDIO_WRITE_PACK_FINISH,
	SDIO_WRITE_BLK_FINISH,
	SDIO_MSG_SYNC_WRITE,
	SDIO_MSG_ASYNC_WRITE
}sdio_msg_id_t;
typedef struct {
	uint32_t magic;
	uint32_t pack_len;	//all of the packets length(bytes),contains the packet info.
	uint32_t pack_cnt;
} pack_tail_t;
typedef union
{
...
typedef union
{
	struct
	{
		uint32_t write_stuff : 8;	//Write Data or Stuff Bits
		uint32_t stuff_1  : 1;
		uint32_t reg_addr : 17;
		uint32_t stuff_2  : 1;
		uint32_t raw_flag : 1;	//read after write
		uint32_t func_num : 3;
		uint32_t rw_flag  : 1;	//read or write
	};
	uint32_t v;
}sdio_cmd52_arg_t;
typedef union
{
	struct
	{
	   volatile uint32_t  cmd_s_rec_index_buf0			 : 6;  //0x40[5:0],缓存接收到的第1个CMD index,0x0,R
	   volatile uint32_t  cmd_s_rec_index_buf1			 : 6;  //0x40[11:6],缓存接收到的第2个CMD index,0x0,R
	   volatile uint32_t  cmd_s_rec_index_buf2			 : 6;  //0x40[17:12],缓存接收到的第3个CMD index,0x0,R
	   volatile uint32_t  cmd_s_rec_index_buf3			 : 6;  //0x40[23:18],缓存接收到的第4个CMD index,0x0,R
	   volatile uint32_t  cmd_s_rec_index_buf4			 : 6;  //0x40[29:24],缓存接收到的第5个CMD index,0x0,R
	   volatile uint32_t  reserved						 : 2;  //0x40[31:30], ,0x0,R
	};
	uint32_t v;
}sdio_cmd_fifo_t;
...
typedef bk_err_t (*sdio_chan_cb_t)(uint8_t chan_id, sdio_node_ptr_t head_p, sdio_node_ptr_t tail_p, uint32_t count);
...
typedef struct
{
	uint32_t *start_addr[SDIO_MEMCPY_PACK_CNT];	//points to packet buffer start addr
	uint32_t *end_addr[SDIO_MEMCPY_PACK_CNT];
	volatile uint32_t rd_index;	//SDIO hardware will transfer this pack now
	volatile uint32_t wr_index;	//SDIO DRIVER SW memcpy data to this pack
}tx_memcopy_t;
#endif
#if (CONFIG_SDIO_SLAVE_RX_MEMCPY_EN)
typedef struct
{
	uint32_t *start_addr[SDIO_RX_MEMCPY_PACK_CNT];	//points to packet buffer start addr
	uint32_t *end_addr[SDIO_RX_MEMCPY_PACK_CNT];
	volatile uint32_t rd_index;	//SDIO hardware will transfer this pack now
	volatile uint32_t wr_index;	//SDIO DRIVER SW memcpy data to this pack
}rx_memcopy_t;
#endif
...
#define GPIO_SDIO_HOST_WAKEUP_SLAVE (GPIO_27)	// == gpio_wk_receive_num
	...
	#ifndef GPIO_SDIO_NOTIFY_HOST
	#define GPIO_SDIO_NOTIFY_HOST (GPIO_0)	   // == gpio_wk_send_num
	#endif
#else
	#ifndef GPIO_SDIO_NOTIFY_HOST
	#define GPIO_SDIO_NOTIFY_HOST (GPIO_2)
	#endif
#endif
...
bk_err_t bk_sdio_loopcheck_init(void);
bk_err_t bk_sdio_auto_test(void);
#endif // _SDIO_H_
