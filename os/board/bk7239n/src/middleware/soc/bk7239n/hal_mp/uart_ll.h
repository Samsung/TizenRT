...
#define UART_PM_BACKUP_REG_NUM    (8)
...
static inline void uart_ll_backup(uart_hw_t *hw, uint32_t *pm_backup)
{
	...
	gpio_id_t uart_tx_pin = uart_ll_get_tx_pin(0);
	gpio_id_t uart_rx_pin = uart_ll_get_rx_pin(0);

	pm_backup[6] = REG_READ(SOC_AON_GPIO_REG_BASE + (uart_tx_pin << 2));
	pm_backup[7] = REG_READ(SOC_AON_GPIO_REG_BASE + (uart_rx_pin << 2));
}
...
static inline void uart_ll_restore(uart_hw_t *hw, uint32_t *pm_backup)
{
	...
	REG_WRITE(SOC_AON_GPIO_REG_BASE + (uart_tx_pin << 2), pm_backup[6]);
	REG_WRITE(SOC_AON_GPIO_REG_BASE + (uart_rx_pin << 2), pm_backup[7]);
}
#endif
...
#if (4 == SOC_UART_ID_NUM_PER_UNIT)
	uint32_t baudrate3;
	uart_hw_t *hw3 = (uart_hw_t *)UART_LL_REG_BASE(3);

	baudrate3 = UART_CLOCK / (hw3->config.clk_div + 1);

	uart_wait_us = 1000000 * hw3->fifo_status.tx_fifo_count * 10 / baudrate3
				 + 1000000 * hw2->fifo_status.tx_fifo_count * 10 / baudrate2
				 + 1000000 * hw1->fifo_status.tx_fifo_count * 10 / baudrate1
				 + 1000000 * hw0->fifo_status.tx_fifo_count * 10 / baudrate0;
	while (!hw3->fifo_status.tx_fifo_empty){
                ;
        }
#endif
