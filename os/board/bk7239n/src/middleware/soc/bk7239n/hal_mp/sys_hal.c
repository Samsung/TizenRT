...
bk_err_t sys_hal_switch_cpu_bus_freq_low_to_high(pm_cpu_freq_e cpu_bus_freq)
{
	...
	switch(cpu_bus_freq)
	{
		...
		case PM_CPU_FRQ_XTAL://cpu0:XTAL;bus:XTAL(40M)
			#if CONFIG_PM_CPU_FIXED_FREQ_ENABLE
				sys_hal_ctrl_vdddig_h_vol(CONFIG_PM_VDDDIG_FIXED_VOLTAGE);
			#else
				sys_hal_ctrl_vdddig_h_vol(0xB);//0.875V
			#endif
			ret = sys_hal_core_bus_clock_ctrl(0x0,0x0,0x0,0x1,0x1);
...
__FLASH_BOOT_CODE void sys_hal_early_init(void)
{
	...
	sys_hal_analog_set_flash(ANALOG_REG4, 0x40000000);
#else
	sys_hal_analog_set_flash(ANALOG_REG4, val & 0xF0000000);
#endif
	...
	//sys_hal_analog_set(ANALOG_REG7, 0x57E657C6);//xinyuan20250611:<9:6>=F for better EVM;tenglong20251010<18>=1
	sys_hal_analog_set_flash(ANALOG_REG7, 0x55A657C6);//NOTICE TODO: modify bg_cal to improve VDD as current mp chip is not ATE calibrated. PSRAM 160M config
	//sys_hal_analog_set(ANALOG_REG8, 0xF87C72E4);//ronghui20250519:<31:30>=3(0.9V), avoid write analog failure
	sys_hal_analog_set_flash(ANALOG_REG8, 0xF87A72E4);//NOTICE TODO: modify init vdddig as 0.85V for asic psram issue. PSRAM 160M config
