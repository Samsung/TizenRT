...
{
	hal->hw = (aon_rtc_hw_t *)AON_RTC_LL_REG_BASE(hal->id);
	aon_rtc_ll_set_tick_val_hi(hal->hw, val >> 32);
	aon_rtc_ll_set_tick_val(hal->hw, (uint32_t)val);
}

uint64_t aon_rtc_hal_get_tick_val(aon_rtc_hal_t *hal)
{
	hal->hw = (aon_rtc_hw_t *)AON_RTC_LL_REG_BASE(hal->id);
	return (((uint64_t)aon_rtc_ll_get_tick_val_hi(hal->hw) << 32) + aon_rtc_ll_get_tick_val(hal->hw));
}
...
{
	hal->hw = (aon_rtc_hw_t *)AON_RTC_LL_REG_BASE(hal->id);
	aon_rtc_ll_set_upper_val_hi(hal->hw, up_val >> 32);
	aon_rtc_ll_set_upper_val(hal->hw, (uint32_t)up_val);
}

uint64_t aon_rtc_hal_get_upper_val(aon_rtc_hal_t *hal)
{
	hal->hw = (aon_rtc_hw_t *)AON_RTC_LL_REG_BASE(hal->id);
	return (((uint64_t)(aon_rtc_ll_get_upper_val_hi(hal->hw)) << 32) + aon_rtc_ll_get_upper_val(hal->hw));
}
...
{
	hal->hw = (aon_rtc_hw_t *)AON_RTC_LL_REG_BASE(hal->id);
	return (((uint64_t)(aon_rtc_ll_get_upper_val_lpo_hi(hal->hw)) << 32) + aon_rtc_ll_get_upper_val_lpo(hal->hw));
}

uint64_t aon_rtc_hal_get_tick_val_lpo(aon_rtc_hal_t *hal)
{
	hal->hw = (aon_rtc_hw_t *)AON_RTC_LL_REG_BASE(hal->id);
	return (((uint64_t)(aon_rtc_ll_get_tick_val_lpo_hi(hal->hw)) << 32) + aon_rtc_ll_get_tick_val_lpo(hal->hw));
}
...
bk_err_t aon_rtc_hal_init(aon_rtc_hal_t *hal)
{
	...
	aon_rtc_hal_set_tick_val(hal, 0);
	aon_rtc_hal_set_upper_val(hal, 0);
	aon_rtc_ll_enable(hal->hw);
