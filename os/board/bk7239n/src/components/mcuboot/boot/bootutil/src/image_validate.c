                  ...
                  uint8_t *seed, int seed_len)
{
    ...
    size = hdr_size = hdr->ih_hdr_size - ADAPT_SIGN_IMAGE_HEAD_OFFSET;
...
#endif
        addr += ADAPT_SIGN_IMAGE_HEAD_OFFSET;  
        bootutil_sha256_update(&sha256_ctx,
...
bootutil_hash_hash(uint8_t *digest, uint32_t digest_sz, uint8_t *hash_result)
{
    bootutil_sha256_context sha256_ctx;
    bootutil_sha256_init(&sha256_ctx);
    bootutil_sha256_update(&sha256_ctx, digest, digest_sz);
    bootutil_sha256_finish(&sha256_ctx, hash_result);
    bootutil_sha256_drop(&sha256_ctx);
    return 0;
}
                      ...
                      int seed_len, uint8_t *out_hash)
{
    ...
    rc = bootutil_hash_hash(hash, 32, hash_hash);
    if (rc) {
        goto out;
    }
...
#if CONFIG_BL2_SKIP_VALIDATE
    if(reset_reason != 0x18){
        /*reboot, only validate hash*/
        return 0;
    }
#endif
