...
#include "../src/common/hw_features_common.h"
...
void wpa_supplicant_auto_reconnect_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	const u8 *bssid __maybe_unused = wpa_s->bssid;
	...
	// if already in DISCONNECTED state, post event, else
	// wpa_state notify will post event
	if (wpa_s->wpa_state == WPA_DISCONNECTED) {
		// scan count reached, post event
...
static void wpa_supplicant_cleanup(struct wpa_supplicant *wpa_s)
{
	...
	eloop_cancel_timeout(wpa_supplicant_external_auth_timeout, wpa_s, NULL);
#endif
			      ...
			      enum wpa_states state)
{
	...
	if (state == WPA_COMPLETED && wpa_s->new_connection) {
		...
		__maybe_unused_var(fils_hlp_sent);
#ifdef CONFIG_SME
...
#if defined(CONFIG_SAE) && !BK_SUPPLICANT
			  ...
			  struct hostapd_freq_params *freq)
{
	...
	int bw160[] __maybe_unused = { 5955, 6115, 6275, 6435, 6595, 6755, 6915 };
	...
	u32 vht_caps = 0;
	bool is_24ghz __maybe_unused, is_6ghz;
...
#if BK_SUPPLICANT
			wpa_sm_set_ft_params(wpa_s->wpa, mdie, mdie ? 2 + mdie[1] : 0);
#endif
			size_t len = 0;
			const u8 *md = mdie + 2;
...
static void wpas_start_assoc_cb(struct wpa_radio_work *work, int deinit)
{
	...
	params.beacon_int = bss->beacon_int;
	params.cap_info = bss->caps;
	params.rssi = bss->level;
#endif
...
#if BK_SUPPLICANT
		int timeout = CONFIG_WPA_AUTH_TIMEOUT;
#else
		int timeout = 10;
#endif
...
next_driver:
	...
	if (wpa_s->drv_priv == NULL) {
		const char *pos;
		int level __maybe_unused = MSG_ERROR;
...
static int * get_bss_freqs_in_ess(struct wpa_supplicant *wpa_s)
{
	...
	dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) {
		...
		if (bss->ssid_len == cbss->ssid_len &&
		    ...
		    wpa_blacklist_get(wpa_s, bss->bssid) == NULL*/) {  // FIXME: tiancj
...
void wpas_connection_failed(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
...
#else
	wpa_s->consecutive_conn_failures = 1;
#endif
	...
	if (wpa_s->scan_interval)
		timeout = wpa_s->scan_interval * 1000;
#endif
#endif
...
void wpas_auth_failed(struct wpa_supplicant *wpa_s, char *reason)
{
	...
	if (dur) {
#ifdef CONFIG_WPA_FIXED_SCAN_INTERVAL
		struct os_reltime elapse;

		/* calculate the elapsed time from last select network */
		os_get_reltime(&now);
		os_reltime_sub(&now, &wpa_s->select_network_time, &elapse);
		if (elapse.sec > 0) {
			dur -= elapse.sec;
			if (dur < 1)
				dur = 1;
