...
#include "driver_i.h"
#include "../src/utils/pcsc_funcs.h"
#include "../src/rsn_supp/preauth.h"
#include "../src/rsn_supp/pmksa_cache.h"
#include "../src/common/wpa_ctrl.h"
#include "../src/eap_peer/eap.h"
...
#include "../src/common/ieee802_11_common.h"
#include "../src/common/gas_server.h"
#include "../src/common/dpp.h"
#include "../src/common/ptksa_cache.h"
...
int get_security_type_from_ie(u8 *ie_start, int len, u16 caps)
{
	...
	int key_mgmt = 0, pairwise_cipher = 0;
...
#if BK_SUPPLICANT && !CONFIG_QUICK_TRACK
		if (wpa_config_set_wpa(bss, ssid, &ie)) {
			if (debug_print)
				wpa_dbg(wpa_s, MSG_DEBUG,
					"   skip NO WPA - PSK failed");
			break;
		}
#endif
			    ...
			    bool debug_print)
{
	...
	if (bk_feature_bssid_connect_enable() &&
		ssid->ssid_len == 0 && ssid->bssid_set &&
		os_memcmp(bss->bssid, ssid->bssid, ETH_ALEN) == 0) {

		/* bssid set but ssid->ssid is null, set ssid from wpa_bss */
		/* set wpa_ssid's ssid if not set */
		if (!bss->ssid_len) {
			if (debug_print)
				wpa_dbg(wpa_s, MSG_DEBUG, "   set ssid");
			return false;
		}

		if (ssid->ssid)
			os_free(ssid->ssid);
		ssid->ssid = (u8 *)dup_binstr(bss->ssid, bss->ssid_len);
		if (!ssid->ssid) {
			if (debug_print)
				wpa_dbg(wpa_s, MSG_DEBUG, "   set ssid oom");
			return false;
		}
		ssid->ssid_len = bss->ssid_len;

		wpa_dbg(wpa_s, MSG_DEBUG, "bssid " MACSTR " set ssid to %s",
		...
		if (!wpa) {
			...
			/* trigger wpa_psk_cal thread to calcuate PSK */
			wpa_psk_request(bss->ssid, bss->ssid_len, ssid->passphrase, psk, psk_len);
		}
	}
#endif
...
{
	u8 wpa_ie_len __maybe_unused, rsn_ie_len __maybe_unused;
	const u8 *ie;
	struct wpa_ssid *ssid;
	int osen __maybe_unused;
	const u8 *match_ssid;
	size_t match_ssid_len;
					      ...
					      int new_scan, int own_request)
{
...
#ifdef CONFIG_WPA_FIXED_SCAN_INTERVAL
	os_get_reltime(&wpa_s->select_network_time);
#endif
	...
	if (selected) {
...
#if defined(BK_SUPPLICANT) && defined(CONFIG_SELECT_NETWORK_AFTER_PSK_SET)
		if (wpa_key_mgmt_wpa_psk(ssid->key_mgmt)) {
			...
			/* If AP supports sae, we will use sae in prefer, don't wait for psk */
			if (wpa_key_mgmt_sae(ssid->key_mgmt) &&
				(ssid->sae_password || ssid->passphrase))
				psk_set = 1;

			if (!psk_set) {
				wpa_printf(MSG_INFO, "Postpone select network because no PSK set");
				return 0;
			}
		}
#endif
		if (bk_feature_network_found_event()) {
			wifi_event_network_found_t network_found = {0};
			os_memset(&network_found, 0, sizeof(network_found));
			os_memcpy(&network_found.ssid, selected->ssid, sizeof(selected->ssid));
			os_memcpy(&network_found.bssid,selected->bssid, ETH_ALEN);
			bk_event_post(EVENT_MOD_WIFI, EVENT_WIFI_NETWORK_FOUND,
						&network_found, sizeof(network_found), BEKEN_NEVER_TIMEOUT);
		}
		...
		if (bk_feature_bssid_connect_enable()) {
			...
			if (!is_zero_ether_addr(g_sta_param_ptr->fast_connect.bssid)) {
				...
				struct wpa_ssid *conf_ssid = wpa_config_get_network(wpa_s->conf, 0);

				/* if ssid or ssid_len */
				if (conf_ssid && conf_ssid->ssid && conf_ssid->ssid_len) {
					os_memset(g_sta_param_ptr->fast_connect.bssid, 0, ETH_ALEN);
					if (conf_ssid) {
						conf_ssid->bssid_set = 0;
					}
				}
			}
		}
					  ...
					  union wpa_event_data *data)
{
	...
	const u8 *p;
	u8 bssid[ETH_ALEN];
	bool bssid_known __maybe_unused;
...
#if defined(CONFIG_SME) || defined(BK_SUPPLICANT)
...
#endif /* CONFIG_SME || BK_SUPPLICANT */
...
#if BK_SUPPLICANT
	bk_event_post(EVENT_MOD_WIFI, EVENT_WIFI_STA_ASSOCIATED, NULL,
		0, BEKEN_NEVER_TIMEOUT);
#endif
...
#if BK_SUPPLICANT
	sta_tick.sta_assoc_tick = rtos_get_time();
#endif
...
#ifdef BK_SUPPLICANT_OPTIMIZE_CONNECTION
...
/**
 * check supplicant auto reconnect is disabled.
 */
static bool wpa_supplicant_auto_reconnect_disabled(struct wpa_supplicant *wpa_s,
		enum wpa_states old_state)
{
	...
	/* if auto reconnect is disable after deauth */
	if (wpa_s->disable_auto_reconnect_after_disconnect &&
		old_state == WPA_COMPLETED)
		return true;

	/*
	 * if disconnect after completed, don't check reconnect time, because
	 * reconnect time is not set.
	 */
	os_get_reltime(&now);
	if (old_state != WPA_COMPLETED &&
		wpa_s->auto_reconnect_timeout != 0 &&
		os_reltime_expired(&now, &wpa_s->auto_reconnect_start_time,
			wpa_s->auto_reconnect_timeout)) {
		return true;
	}
						 ...
						 int locally_generated)
{
...
#if BK_SUPPLICANT
		wpas_notify_psk_mismatch(wpa_s, reason_code, locally_generated);
#endif
	...
	if (!wpa_s->disconnected &&
	    (
#if defined(BK_SUPPLICANT) && defined(CONFIG_AUTO_RECONNECT)
		 !wpa_supplicant_auto_reconnect_disabled(wpa_s, old_state) ||
#else
...
#ifdef BK_SUPPLICANT_OPTIMIZE_CONNECTION
		/* some AP (f**k TP_LINK) don't response us with EAPol until we
		 * restart AUTH so drop pmksa here let supplicant restart auth
		 * instead of Open System */
		if (wpa_s->wpa_state >= WPA_ASSOCIATED &&
...
#if BK_SUPPLICANT
		if (wpa_s->drv_flags2 & WPA_DRIVER_FLAGS2_SM_FAST_CONNECT){
			/*
			 * If there is no JOIN procedure, AP's beacon may changed,
			 * rescan for this case.
			 */
			int *freqs = (int *)os_zalloc(sizeof(int) * 3); // zero terminated
			if (freqs) {
				os_free(wpa_s->select_network_scan_freqs);
				freqs[0] = freqs[1] = fast_reconnect->freq;
				wpa_s->select_network_scan_freqs = freqs;
			}
			os_memcpy(wpa_s->next_scan_bssid, fast_reconnect->bssid, ETH_ALEN);
			wpa_supplicant_req_scan(wpa_s, 0, 0);
		} else
#endif
...
#if defined(BK_SUPPLICANT) && defined(CONFIG_AUTO_RECONNECT)
	/* start auto reconnect timer if reconnect is enabled */
				    ...
				    union wpa_event_data *data)
{
	...
	wpa_s->disconnect_reason = WIFI_REASON_MAX;
...
#ifdef BK_SUPPLICANT
	/*
	 * If akm is not SAE and 1x, flush pmksa cache
	 * some AP will disassoc us if it changes it akm from WPA2->WPA3->WPA2
	 */
	if (wpa_s->current_ssid &&
		!wpa_key_mgmt_sae(wpa_s->current_ssid->key_mgmt) &&
		!wpa_key_mgmt_wpa_ieee8021x(wpa_s->current_ssid->key_mgmt) &&
		!data->assoc_reject.timed_out) {
		wpa_dbg(wpa_s, MSG_DEBUG, "WPA: Drop PMKSA cache entry");
		wpa_sm_aborted_cached(wpa_s->wpa);
		wpa_sm_pmksa_cache_flush(wpa_s->wpa, wpa_s->current_ssid);
	}
#endif
			  ...
			  union wpa_event_data *data)
{
	...
	int resched;
#endif
	struct os_reltime age __maybe_unused, clear_at __maybe_unused;
	...
	case EVENT_ASSOC:
		...
		if (wpa_s->wpa_state == WPA_DISCONNECTED) {
			wpa_printf(MSG_INFO, "Rx EVENT_ASSOC when WPAS is in disconnected state, ignore it!\n");
			return;
		}
#endif
	...
	case EVENT_SCAN_RESULTS:
		...
		if (!(data && data->scan_info.external_scan) &&
			...
			wpa_s->scan_use_time.sec = diff.sec;
			wpa_s->scan_use_time.usec = diff.usec;
