 ...
 * return: 0 success, 1 psk calculation in progress
 */
int wpa_config_update_psk(struct wpa_ssid *ssid)
{
#ifndef CONFIG_NO_PBKDF2
#ifdef CONFIG_WPA_PSK_CACHE
#ifdef CONFIG_SELECT_NETWORK_AFTER_PSK_SET
	int ret;
	...
	return 1; // psk still calc in progress, return without wait and sleep
#else
	...
	ssid->psk_set = 1;
#endif /* CONFIG_SELECT_NETWORK_AFTER_PSK_SET */
#else
	pbkdf2_sha1(ssid->passphrase, ssid->ssid, ssid->ssid_len, 4096,
		    ssid->psk, PMK_LEN);
