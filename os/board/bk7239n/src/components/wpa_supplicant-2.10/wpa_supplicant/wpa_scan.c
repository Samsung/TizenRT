...
static void wpas_trigger_scan_cb(struct wpa_radio_work *work, int deinit)
{
	...
	WPA_LOGD("scan work start, deinit=%d\n", deinit);
	if (deinit) {
		if (!work->started) {
	...
	if (ret) {
		...
		wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_SCAN_FAILED "ret=%d%s",
			ret, retry ? " retry=1" : "");
		WPA_LOGI("req driver scan failed, ret=-0x%x, %s\n",
			-ret, retry ? "retry" : "");
		radio_work_done(work);
	...
	if (res == 1) {
		...
		if (wpa_s->scan_res_handler != scan_only_handler &&
			...
			if (wpa_s->auto_reconnect_count > 0) {
				...
				WPA_LOGI("%s: max scan count reached, wpa_s reconnect %d, no_suitable_network %d, suitable_network %d\n", __func__,
				...
				if (g_sta_param_ptr->fast_connect_set && g_sta_param_ptr->fast_connect.chann !=0)
				{
					...
					WPA_LOGI("change scan delay timeout to 0");
					eloop_register_timeout(0, 0, wpa_supplicant_scan, wpa_s, NULL);
				} else
#endif
...
{
	WPA_LOGD("cancell scan request timer\r\n");
