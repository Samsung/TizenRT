...
#ifdef BK_SUPPLICANT
	/* used by wlan fast connect without scan */
	s8 rssi;
	u16 cap_info;
	u8 *bcn_ie;
	int bcn_len;
...
#if BK_SUPPLICANT
/** Driver expects user space implementation of SME in AP mode */
#define WPA_DRIVER_FLAGS2_SM_FAST_CONNECT 0x8000000000000000ULL
#endif
	u64 flags2;
...
enum tdls_oper {
	...
	TDLS_ENABLE_LINK,
	TDLS_DISABLE_LINK,
	TDLS_ENABLE0,	/* renamed from TDLS_ENABLE, conflict with rw */
	TDLS_DISABLE
};
...
#if defined(BK_SUPPLICANT) && defined(CONFIG_IEEE80211R)
	/**
	 * handle_ft_response - Notify private driver about FT AUTH Response
	 * @priv: Private driver interface data
	 * @data: union wpa_event_data data
	 * Returns: 0 on success, -1 on failure
	 */
	int (*handle_ft_response)(void *priv, void *data);
#endif
};
...
extern const struct wpa_driver_ops wpa_driver_hostap_ops; /* driver_beken.c */
