...
#ifdef BK_SUPPLICANT
int wpa_parse_wpa_ie_key_mgmt_and_pairwise_cipher(const u8 *wpa_ie, size_t wpa_ie_len, int *key_mgmt, int *cipher)
{
	if (wpa_ie_len >= 1 && wpa_ie[0] == WLAN_EID_RSN)
		return wpa_parse_wpa_ie_rsn_key_mgmt_and_pairwise_cipher(wpa_ie, wpa_ie_len, key_mgmt, cipher);
	if (wpa_ie_len >= 6 && wpa_ie[0] == WLAN_EID_VENDOR_SPECIFIC &&
	    wpa_ie[1] >= 4 && WPA_GET_BE32(&wpa_ie[2]) == OSEN_IE_VENDOR_TYPE)
		return wpa_parse_wpa_ie_rsn_key_mgmt_and_pairwise_cipher(wpa_ie, wpa_ie_len, key_mgmt, cipher);
	else
		return wpa_parse_wpa_ie_wpa_key_mgmt_and_pairwise_cipher(wpa_ie, wpa_ie_len, key_mgmt, cipher);
}
#endif
