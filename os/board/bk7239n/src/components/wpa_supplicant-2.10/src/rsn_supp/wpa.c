...
static int wpa_supplicant_get_pmk(struct wpa_sm *sm,
				  ...
				  const u8 *pmkid)
{
	int abort_cached __maybe_unused = 0;
...
#if BK_SUPPLICANT
	/* send EAPOL-Key with retries */
	send_success = false;
	for (i = 0; i  < RESEND_KEY_MAX; i++) {
		if (wpa_supplicant_send_2_of_4(sm, sm->bssid, key, ver, sm->snonce,
				       kde, kde_len, ptk) >= 0) {
			send_success = true;
			break;
		}
		wpa_printf(MSG_WARNING, "Sending EAPOL-Key 2/4 fail times:%d", i + 1);
...
#if BK_SUPPLICANT
	if (keyidx & 0xFF00) {
		/* Some AP has not initialize upper 8 bits of keyidx,
		 * and makes keyidx larger than 256 which is reserved in spec,
		 * so here we mask the upper 8 bits */

		wpa_dbg(sm->ctx->msg_ctx, MSG_DEBUG,
			"WPA: IGTK keyid upper 8 bits is not reset to zero");
...
#if BK_SUPPLICANT
bool wpa_tkip_key_set_flag = false;
#endif
					  ...
					  size_t key_data_len)
{
...
#if BK_SUPPLICANT
	if (sm->pairwise_cipher == WPA_CIPHER_TKIP) {
		wpa_tkip_key_set_flag = true;
		tkip_key_set_flag = true;
	...
	for (i = 0; i  < RESEND_KEY_MAX; i++) {
		...
		wpa_printf(MSG_WARNING, "Sending EAPOL-Key 4/4 fail times: %d", i + 1);
		rtos_delay_milliseconds(50);

		if(tkip_key_set_flag)
			wpa_tkip_key_set_flag = true;
	}
					  ...
					  size_t key_data_len, u16 ver)
{
	...
	for (i = 0; i  < RESEND_KEY_MAX; i++) {
		...
		wpa_printf(MSG_WARNING, "Send EAPOL-Key 2/2 fail times:%d", i + 1);
		rtos_delay_milliseconds(50);
	}
