...
#if BK_SUPPLICANT
static int rsn_key_mgmt_to_bitfield_all(const u8 *s)
{
	if (RSN_SELECTOR_GET(s) == RSN_AUTH_KEY_MGMT_UNSPEC_802_1X)
		return WPA_KEY_MGMT_IEEE8021X;
	if (RSN_SELECTOR_GET(s) == RSN_AUTH_KEY_MGMT_PSK_OVER_802_1X)
 ...
 * Returns: 0 on success, <0 on failure
 */
int wpa_parse_wpa_ie_rsn_key_mgmt_and_pairwise_cipher(const u8 *rsn_ie, size_t rsn_ie_len, int *key_mgmt, int *pairwise_cipher)
{
	const u8 *pos;
	int left;
	int i, count;
	...
	*pairwise_cipher = 0;
	...
	if (left >= 2) {
		...
		for (i = 0; i < count; i++) {
			*pairwise_cipher |= rsn_selector_to_bitfield(pos);
			pos += RSN_SELECTOR_LEN;
			left -= RSN_SELECTOR_LEN;
		}
	} else if (left == 1) {
	...
	if (left >= 2) {
		*key_mgmt = 0;
		...
		for (i = 0; i < count; i++) {
			*key_mgmt |= rsn_key_mgmt_to_bitfield_all(pos);
			pos += RSN_SELECTOR_LEN;
			left -= RSN_SELECTOR_LEN;
		}
	} else if (left == 1) {
...
#ifdef BK_SUPPLICANT
int wpa_parse_wpa_ie_wpa_key_mgmt_and_pairwise_cipher(const u8 *wpa_ie, size_t wpa_ie_len, int *key_mgmt, int *pairwise_cipher)
{
	const struct wpa_ie_hdr *hdr;
	const u8 *pos;
	int left;
	...
	*pairwise_cipher = 0;
	...
	if (left >= 2) {
		...
		for (i = 0; i < count; i++) {
			*pairwise_cipher |= wpa_selector_to_bitfield(pos);
			pos += WPA_SELECTOR_LEN;
			left -= WPA_SELECTOR_LEN;
		}
	} else if (left == 1) {
	...
	if (left >= 2) {
		*key_mgmt = 0;
		...
		for (i = 0; i < count; i++) {
			*key_mgmt |= wpa_key_mgmt_to_bitfield(pos);
			pos += WPA_SELECTOR_LEN;
			left -= WPA_SELECTOR_LEN;
		}
	} else if (left == 1) {
...
const char * wpa_cipher_txt(int cipher)
{
	...
	case WPA_CIPHER_NONE:
		return "NONE";
#if defined(CONFIG_WEP_AP) || defined(CONFIG_WEP)
