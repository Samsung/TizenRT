...
#if BK_SUPPLICANT
/* change CPU/Flash Clock to speedup ECC/DH computation */
void sae_overclock_set(bool enable)
{
#ifdef CONFIG_SAE_OVERCLOCK
	if (enable) {
			  ...
			  u8 *confirm)
{
	...
	addr[4] = element2;
	len[4] = element2_len;
	ret = hkdf_extract(sae->tmp->kck_len, sae->tmp->kck, sae->tmp->kck_len,
			    5, addr, len, confirm);
			      ...
			      u8 *confirm)
{
	...
	if (crypto_bignum_to_bin(element1, element_b1, SAE_MAX_PRIME_LEN,
				 sae->tmp->prime_len) < 0 ||
...
{
	u8 verifier[SAE_MAX_HASH_LEN] = {0};
	size_t hash_len;
