...
#if BK_SUPPLICANT
u8 *target_p2p_device = (u8 *)"\xee\xaa\x25\x4a\xf6\x9f";
int p2p_debug_flag = 0;
#endif
...
/**
 * p2p_listen_in_find - Start p2p listen during find
 * @p2p: Pointer to p2p data
 * @dev_disc: whether need to discover devices(called from find process)
 *
 */
static void p2p_listen_in_find(struct p2p_data *p2p, int dev_disc)
{
	...
	if (p2p->state != P2P_CONNECT_LISTEN)
		...
		p2p->pending_listen_usec = 500 * 1000;

	if (p2p->cfg->start_listen(p2p->cfg->cb_ctx, freq, p2p->pending_listen_usec / 1000,
		    ies) < 0) {
				       ...
				       const u8 *ie, size_t ie_len)
{
	...
	if (os_memcmp(target_p2p_device, addr, ETH_ALEN) == 0)
		p2p_debug_flag = 1;
#endif
...
#ifdef BK_SUPPLICANT
	//set P2P intended address using P2P device address to fix GO send deauth after association
	p2p_set_intended_addr(p2p, cfg->dev_addr);
#endif
...
#ifdef BK_SUPPLICANT
	/*
	 * If we have responsed peer GO NEG request frame with status P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE,
	 * once CONNECT timeout, active connectiong process could be started if number of connection timers
	 * is less than threshhold(120)
	 */
#endif
	if (p2p->go_neg_peer &&
	    (p2p->go_neg_peer->flags & P2P_DEV_PEER_WAITING_RESPONSE) &&
