...
/**
 * WPAH_FLAG_FREE: its the wpa_s' responsibility to free msg->argu
 * WPAH_FLAG_DEFER: queue this command instead of call immedately
 * WPAH_FLAG_NOWAIT: no wait if command queue if full
 */
enum wpah_flags {
	WPAH_FLAG_FREE   = 0x01,
	WPAH_FLAG_DEFER  = 0x02,
	WPAH_FLAG_NOWAIT = 0x04,
};
