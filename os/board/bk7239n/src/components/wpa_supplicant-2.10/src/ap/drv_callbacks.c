			...
			const u8 *req_ies, size_t req_ies_len, int reassoc)
{
	...
	const u8 *ie;
	size_t ielen;
#if defined(CONFIG_IEEE80211R_AP) || defined(CONFIG_IEEE80211W_AP) || defined(CONFIG_FILS_AP) || defined(CONFIG_OWE_AP)
	u8 buf[sizeof(struct ieee80211_mgmt) + 1024];
	u8 *p = buf;
#endif /* CONFIG_IEEE80211R_AP || CONFIG_IEEE80211W_AP || CONFIG_FILS_AP || CONFIG_OWE_AP */
...
skip_wpa_check:
...
#if defined(CONFIG_IEEE80211R_AP) || defined(CONFIG_FILS_AP) || defined(CONFIG_OWE_AP)
	...
	if (sta->auth_alg == WLAN_AUTH_FT ||
	    ...
	    sta->auth_alg == WLAN_AUTH_FILS_PK)
		ap_sta_set_authorized(hapd, sta, 1);
#else /* CONFIG_IEEE80211R_AP || CONFIG_FILS_AP */
	/* Keep compiler silent about unused variables */
	if (status) {
	}
#endif /* CONFIG_IEEE80211R_AP || CONFIG_FILS_AP */
...
{
#ifdef NEED_AP_MLME
	int channel, chwidth, is_dfs __maybe_unused = 0;
	u8 seg0_idx = 0, seg1_idx = 0;
	size_t i __maybe_unused;
...
#if BK_SUPPLICANT
	if (!hapd->csa_in_progress && hapd->csa_settings_pending) {
		wpa_printf(MSG_INFO, "schedule pending csa: freq=%d",
				hapd->csa_settings_pending->freq_params.freq);
		hostapd_switch_channel(hapd, hapd->csa_settings_pending);
		os_free(hapd->csa_settings_pending);
		hapd->csa_settings_pending = 0;
	}
#endif
