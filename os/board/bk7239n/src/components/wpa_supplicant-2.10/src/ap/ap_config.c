...
#ifdef CONFIG_NO_TKIP_AP
	bss->wpa_pairwise = WPA_CIPHER_CCMP;
	bss->wpa_group = WPA_CIPHER_CCMP;
#else /* CONFIG_NO_TKIP_AP */
	bss->wpa_pairwise = WPA_CIPHER_TKIP;
	bss->wpa_group = WPA_CIPHER_TKIP;
#endif /* CONFIG_NO_TKIP_AP */
	bss->rsn_pairwise = 0;

	bss->max_num_sta = g_ap_param_ptr->max_con;
...
#ifdef CONFIG_MACSEC_AP
	bss->mka_priority = DEFAULT_PRIO_NOT_KEY_SERVER;
	bss->macsec_port = 1;
#endif /* CONFIG_MACSEC_AP */
...
#if BK_SUPPLICANT && CFG_SUPPORT_80211G
...
#if BK_SUPPLICANT && CONFIG_AP_HT_IE
...
#if BK_SUPPLICANT
	conf->track_sta_max_age = 0;  // FIXME bk7236 ???
#else
...
#ifdef CONFIG_AIRTIME_POLICY_AP
...
static int hostapd_derive_psk(struct hostapd_ssid *ssid)
{
...
#if 0//def CONFIG_WPA_PSK_CACHE
	int count = 0, found = 0, ret;
	while (1) {
		if (!(ret = hostapd_get_psk_from_cache(ssid))) {
			found = 1;
			break;
		}
	...
	if (!found) {
		wpa_printf(MSG_WARNING, "PSK caculation in hostapd thread");
...
{
#if defined(CONFIG_WPS_AP) || defined(CONFIG_HS20_AP)
	size_t i;
#endif
