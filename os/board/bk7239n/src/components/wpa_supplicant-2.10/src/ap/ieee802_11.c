...
prepare_auth_resp_fils(struct hostapd_data *hapd,
		       ...
		       int *is_pub);
#endif /* CONFIG_FILS_AP */

#ifdef CONFIG_PASN_AP
...
#ifdef CONFIG_HS20_AP
	if (hapd->conf->osen)
		privacy = 1;
#endif /* CONFIG_HS20_AP */
...
#if defined(CONFIG_SAE_AP) || defined(CONFIG_PASN_AP)
...
#endif /* defined(CONFIG_SAE_AP) || defined(CONFIG_PASN_AP) */
...
static void handle_auth(struct hostapd_data *hapd,
			...
			int rssi, int from_queue)
{
	u16 auth_alg, auth_transaction, status_code __maybe_unused;
	u16 resp = WLAN_STATUS_SUCCESS;
	struct sta_info *sta = NULL;
	int res __maybe_unused, reply_res;
	u16 fc;
	const u8 *challenge __maybe_unused = NULL;
...
#ifdef BK_SUPPLICANT
	if (len >= IEEE80211_HDRLEN + sizeof(mgmt->u.auth) + 5) {
		if (mgmt->u.auth.variable[0] == WLAN_EID_VENDOR_SPECIFIC &&
			mgmt->u.auth.variable[1] == 6 &&
			!os_memcmp(&mgmt->u.auth.variable[2], "\x66\x69\x6c", 3)) {
			bk_printf("bk debug: ignore vendor specific auth\r\n");
			//wpa_printf(MSG_DEBUG, "ignore vendor specific auth\n");
			return;
		}
	}
#endif
...
{
#ifdef CONFIG_INTERWORKING_AP
	/* check for QoS Map support */
	if (ext_capab_ie_len >= 5) {
		if (ext_capab_ie[4] & 0x01)
			sta->qos_map_enabled = 1;
	}
#endif /* CONFIG_INTERWORKING_AP */
...
#if defined(CONFIG_FILS_AP) && defined(CONFIG_OCV_AP)
...
#endif /* CONFIG_FILS_AP && CONFIG_OCV_AP */
			...
			u16 reason_code)
{
	...
	if (hostapd_drv_send_mlme(hapd, &reply, send_len, 0, NULL, 0, 0) < 0)
		wpa_printf(MSG_INFO, "Failed to send deauth");
}
...
rsnxe_done:
	...
	if (hostapd_drv_send_mlme(hapd, reply, send_len, 0, NULL, 0, 0) < 0) {
		wpa_printf(MSG_INFO, "Failed to send assoc resp");
		res = WLAN_STATUS_UNSPECIFIED_FAILURE;
	}
...
#ifdef CONFIG_TAXONOMY_AP
	taxonomy_sta_info_assoc_req(hapd, sta, pos, left);
#endif /* CONFIG_TAXONOMY_AP */
