...
static int db_table_create_milenage(sqlite3 *db)
{
	...
	WPA_LOGD("Adding database table for milenage information\n");
	if (sqlite3_exec(db, sql, NULL, NULL, &err) != SQLITE_OK) {
		WPA_LOGE("SQLite error: %s\n", err);
		sqlite3_free(err);
		return -1;
	}
...
static sqlite3 * db_open(const char *db_file)
{
	...
	if (sqlite3_open(db_file, &db)) {
		WPA_LOGE("Failed to open database %s: %s\n",
...
static int db_update_milenage_sqn(struct milenage_parameters *m)
{
	...
	*pos = '\0';
	...
	if (sqlite3_exec(sqlite_db, cmd, NULL, NULL, NULL) != SQLITE_OK) {
		WPA_LOGE("Failed to update SQN in database for IMSI %s\n",
		       m->imsi);
		return -1;
	}
	return 0;
...
static int read_gsm_triplets(const char *fname)
{
	...
	f = fopen(fname, "r");
	if (f == NULL) {
		WPA_LOGE("Could not open GSM triplet data file '%s'\n", fname);
		return -1;
	}
...
static int read_milenage(const char *fname)
{
	...
	f = fopen(fname, "r");
	if (f == NULL) {
		WPA_LOGE("Could not open Milenage data file '%s'\n", fname);
		return -1;
	}
...
static void update_milenage_file(const char *fname)
{
	...
	f2 = fopen(name, "w");
	if (f2 == NULL) {
		WPA_LOGE("Could not write Milenage data file '%s'\n", name);
		fclose(f);
		return;
	}
...
static int sim_req_auth(char *imsi, char *resp, size_t resp_len)
{
	...
	if (count == 0) {
		WPA_LOGE("No GSM triplets found for %s\n", imsi);
...
static int aka_req_auth(char *imsi, char *resp, size_t resp_len)
{
	...
	if (m) {
		...
		sqn_changes = 1;
		if (stdout_debug) {
			WPA_LOGD("AKA: Milenage with SQN=%02x%02x%02x%02x%02x%02x\n",
		...
		WPA_LOGE("Unknown IMSI: %s\n", imsi);
#ifdef AKA_USE_FIXED_TEST_VALUES
		WPA_LOGD("Using fixed test values for AKA\n");
...
static int aka_auts(char *imsi, char *resp, size_t resp_len)
{
	...
	*__rand++ = '\0';
	...
	if (stdout_debug) {
		WPA_LOGD("AKA-AUTS: IMSI=%s AUTS=%s RAND=%s\n",
		       imsi, auts, __rand);
	}
	if (hexstr2bin(auts, _auts, EAP_AKA_AUTS_LEN) ||
	    hexstr2bin(__rand, _rand, EAP_AKA_RAND_LEN)) {
		WPA_LOGE("Could not parse AUTS/RAND\n");
		return -1;
	}
	...
	if (milenage_auts(m->opc, m->ki, _rand, _auts, sqn)) {
		WPA_LOGE("AKA-AUTS: Incorrect MAC-S\n");
	} else {
		memcpy(m->sqn, sqn, 6);
		if (stdout_debug) {
			WPA_LOGD("AKA-AUTS: Re-synchronized: "
...
static int process(int s)
{
	...
	if (process_cmd(buf, resp, sizeof(resp)) < 0) {
		WPA_LOGE("Failed to process request\n");
		return -1;
	}

	if (resp[0] == '\0') {
		WPA_LOGE("No response\n");
		return 0;
	}
...
{
	WPA_LOGD("HLR/AuC testing gateway for hostapd EAP-SIM/AKA "
...
int main(int argc, char *argv[])
{
	...
	for (;;) {
		...
		case 'D':
			...
			break;
#else /* CONFIG_SQLITE */
			WPA_LOGE("No SQLite support included in the build\n");
			return -1;
#endif /* CONFIG_SQLITE */
		case 'g':
	...
	if (optind == argc) {
		...
		WPA_LOGD("Listening for requests on %s\n", socket_path);
