...
int at_wlan_station_start(int sync, int argc, char **argv)
{
	...
	if ((argc < 1) || (argc > 4)) {
		BK_LOGE(TAG,"invalid argc number\n");
		err = kParamErr;
		goto error;
	}
...
static void at_wlan_station_static_ip(int sync,int argc, char **argv)
{
	...
	if(dns2 != NULL){
		if(at_wlan_judge_the_string_is_ipv4_string(dns2) != 0){
			atsvr_cmd_rsp_error();
			BK_LOGD(TAG,"[at_wlan]dns2:%s error\r\n",dns2);
			return;
		}
	}
	...
	if (argc == 0) 
	{
		...
		if (os_strcmp(argv[0], "STA") == 0) {
			...
			if (uap_ip_is_start()) {
				...
				if(err != kNoErr) {
					BK_LOGE(TAG,"get ap link status fail!\n");
					err = kGeneralErr;
					goto error;
				}
...
static int at_wlan_wifi_ping_start_cmd(int sync,int argc, char **argv)
{
	...
	uint32_t size = 0;
	if (argc == 0) {
		BK_LOGE(TAG,"Please input: ping <host address>\n");
		goto error;
	}
...
static int at_wlan_wifi_close_coex_csa_cmd(int sync,int argc, char **argv)
{
	...
	if (argc != 1) {
		BK_LOGE(TAG,"invalid close_coex_csa command\n");
		goto error;
	}
...
int at_wlan_bridge_start(char *bridge_ssid, char *ext_ssid, char *key) {
	...
	if (bridge_is_enabled) {
		BK_LOGE(TAG,"bridge already open\r\n");
		return kGeneralErr;
	}
	...
	while(1) {
		...
		if (info.state != WIFI_LINKSTATE_STA_GOT_IP) {
			...
			break;
		if (i++ > 60) {
			BK_LOGE("br", "bridge open timeout\r\n");
			return kGeneralErr;
		}
	}
...
int at_wlan_bridge_stop() {
	bridge_ip_stop();
	if(bk_wifi_sta_stop()){
		BK_LOGE(TAG,"bridge stop sta fail\r\n");
		return kGeneralErr;
	}
	if(bk_wifi_ap_stop()){
		BK_LOGE(TAG,"bridge stop ap fail\r\n");
		return kGeneralErr;
	}
	bridge_is_enabled = 0;
	return kNoErr;
