...
#if (0 == WEENY_AES_ROM_TABLES)
#include "os/mem.h"
#include "os/os.h"
#endif

#define TAG "WIFI"
char xor_key[XOR_KEY_COUNT] = {0x55, 0xaa, 0x5a, 0x55, 0xa5, 77, 99, 127};
int xor_key_id = 0;

void xor_key_init(void)
{
	xor_key_id = 0;
}

char xor_get_key(void)
{
	return xor_key[(xor_key_id ++) & (XOR_KEY_COUNT - 1)];
}
...
static unsigned long FT0[256] = { FT };

#undef V

#define V(a,b,c,d) 0x##b##c##d##a
static unsigned long FT1[256] = { FT };

#undef V

#define V(a,b,c,d) 0x##c##d##a##b
static unsigned long FT2[256] = { FT };

#undef V

#define V(a,b,c,d) 0x##d##a##b##c
static unsigned long FT3[256] = { FT };
...
#define V(a,b,c,d) 0x##a##b##c##d
static unsigned long RT0[256] = { RT };

#undef V

#define V(a,b,c,d) 0x##b##c##d##a
static unsigned long RT1[256] = { RT };

#undef V

#define V(a,b,c,d) 0x##c##d##a##b
static unsigned long RT2[256] = { RT };

#undef V

#define V(a,b,c,d) 0x##d##a##b##c
static unsigned long RT3[256] = { RT };
...
static void aes_gen_tables(void)
{
	...
	if (NULL == tmp) {
		BK_WIFI_LOGE(TAG, "aes_gen_tables failed\r\n");
		return;
	}
...
void weeny_aes_setkey_dec(weeny_aes_context *ctx, unsigned char *key, int keysize)
{
	...
	weeny_aes_setkey_enc(&cty, key, keysize);
	SK = cty.rk + cty.nr * 4;
	...
	*RK++ = *SK++;
	...
	memset(&cty, 0, sizeof(weeny_aes_context));
}
						 ...
						 unsigned char *input, unsigned char *output)
{
	...
	if (mode == AES_DECRYPT) {
		while (length > 0) {
			...
			weeny_aes_crypt_ecb(ctx, mode, input, output);
		...
		while (length > 0) {
			...
			weeny_aes_crypt_ecb(ctx, mode, output, output);
			memcpy(iv, output, 16);
							...
							unsigned char *input, unsigned char *output)
{
	...
	if (mode == AES_DECRYPT) {
		while (length--) {
			if (n == 0)
				weeny_aes_crypt_ecb(ctx, AES_ENCRYPT, iv, iv);
