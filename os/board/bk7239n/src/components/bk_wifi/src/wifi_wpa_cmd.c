...
static int cmd_wpas_parse_int(const char *value, int min, int max, int *dst)
{
	...
	if (*end) {
		BK_WIFI_LOGE(TAG, "Invalid number '%s'", value);
		return -1;
	}

	if (val < min || val > max) {
		BK_WIFI_LOGE(TAG, "out of range value %d (%s), range is [%d, %d]\n",
				 val, value, min, max);
		return -1;
	}
...
static int cmd_wpas_parse_key_mgmt(const char *value)
{
	...
	while (*start != '\0') {
		...
		*end = '\0';
		if (os_strcmp(start, "WPA-PSK") == 0)
			...
			BK_WIFI_LOGE(TAG, "Invalid key_mgmt '%s'", start);
			errors++;
		}
	...
	if (val == 0) {
		BK_WIFI_LOGE(TAG, "No key_mgmt values configured\n");
		errors++;
	}

	BK_WIFI_LOGI(TAG, "key_mgmt: 0x%x\n", val);
	return errors ? -1 : val;
}
	...
	while (*start != '\0') {
		...
		*end = '\0';
		...
		if (os_strcmp(start, "WPA") == 0)
			...
			BK_WIFI_LOGE(TAG, "Invalid proto '%s'\n", start);
			errors++;
		}
...
#if 0
	if (val == 0) {
		BK_WIFI_LOGI(TAG, "No proto values configured\n");
		errors++;
	}
#endif

	BK_WIFI_LOGI(TAG, "proto: 0x%x\n", val);
	return errors ? -1 : val;
}
...
static int cmd_wpas_parse_auth_alg(const char *value)
{
	...
	while (*start != '\0') {
		...
		*end = '\0';
		if (os_strcmp(start, "OPEN") == 0)
			...
			BK_WIFI_LOGE(TAG, "Invalid auth_alg '%s'\n", start);
			errors++;
		}
	...
	if (val == 0) {
		BK_WIFI_LOGE(TAG, "No auth_alg values configured\n");
		errors++;
	}

	BK_WIFI_LOGI(TAG, "auth_alg: 0x%x\n", val);
	return errors ? -1 : val;
}

static void cmd_wlan_sta_print_ap(struct ApListStruct *ap)
{
	BK_WIFI_LOGI(TAG, "" BK_MAC_FORMAT " ssid=%-32.32s\n",
		...
		if ((num = cmd_parse_array_int(value, groups, ARRAY_SIZE(groups))) > 0) {
			...
			for (i = 0; i < num; i++) {
				if (dragonfly_suitable_group(groups[i], 0) == 0) {
					BK_WIFI_LOGE(TAG, "Invalid sae group %d\n", groups[i]);
					valid = 0;
				}
			}
			if (!valid)
...
out:
	...
	BK_WIFI_LOGE(TAG, "invalid arg '%s %s'\n", cmd, value);
	return -2;
}
...
int cmd_wlan_sta_exec(char *cmd)
{
	...
	if (os_strncmp(cmd, "config ", 7) == 0) {
		...
		if (ret == 0) {
			BK_WIFI_LOGI(TAG, "psk: ");
			for (i = 0; i < sizeof(param.psk); ++i)
				BK_WIFI_LOGD(TAG, "%02x", param.psk[i]);
			BK_WIFI_LOGI(TAG, "\n");
		}
...
out:
	if (ret == 0) {
		...
		BK_WIFI_LOGE(TAG, "cmd '%s' invalid arg\n", cmd);
		return -1;
	} else if (ret == -1) {
		BK_WIFI_LOGE(TAG, "cmd '%s' exec failed\n", cmd);
		return -1;
	}
...
static int cmd_wlan_ap_get(char *cmd)
{
	...
	if (config.field == WLAN_AP_FIELD_SSID)
		...
		BK_WIFI_LOGI(TAG, "wpa_cipher: %#06x\n", config.u.wpa_cipher);
	else if (config.field == WLAN_AP_FIELD_RSN_CIPHER)
		BK_WIFI_LOGI(TAG, "rsn_cipher: %#06x\n", config.u.rsn_cipher);
...
int cmd_wlan_ap_exec(char *cmd)
{
	...
	if (os_strncmp(cmd, "config ", 7) == 0) {
		...
		if(ap_ssid) {
			...
			//{
				//BK_WIFI_LOGE(TAG, "monitor (ie.airkiss) is not finish yet, stop it or waiting it finish!\r\n");
				//return ret;
			//}
		...
		if (!g_ap_param_ptr) {
			BK_WIFI_LOGE(TAG, "enable AP first\n");
			goto out;
		}
