...
__IRAM2 uint32_t sys_drv_modem_bus_clk_ctrl_ptr(bool clk_en)
{
	return sys_drv_modem_bus_clk_ctrl(clk_en);
}

__IRAM2 uint32_t sys_drv_modem_clk_ctrl_ptr(bool clk_en)
{
	return sys_drv_modem_clk_ctrl(clk_en);
}

__IRAM2 void phy_exit_dsss_only_ptr(void)
{
...
__IRAM2 void phy_enter_dsss_only_ptr(void)
{
#if (CONFIG_SOC_BK7236XX || CONFIG_SOC_BK7239XX || CONFIG_SOC_BK7286XX)
	phy_enter_dsss_only();
#else
...
#endif // CONFIG_FREERTOS_SMP
__IRAM2 static uint32_t rtos_disable_int_ptr(void)
{
...
__IRAM2 static void rtos_enable_int_ptr(uint32_t int_level)
{
	#ifdef CONFIG_FREERTOS_SMP
