                                            ...
                                            psa_algorithm_t alg )
{
    ...
    if( psa_key_algorithm_permits( key_type, policy->alg, alg ) ||
        ...
        return( PSA_SUCCESS );  // do not check the key policy of ecdsa and ecdh
}
#endif
    ...
    size_t *data_length )
{
    ...
    if( PSA_KEY_TYPE_IS_RSA( type ) || PSA_KEY_TYPE_IS_ECC( type ) || PSA_KEY_TYPE_IS_DH( type ))
    {
...
#endif /* MBEDTLS_PSA_BUILTIN_ALG_ECDH */
           case PSA_ALG_FFDH:
                if( ! PSA_KEY_TYPE_IS_DH_KEY_PAIR( private_key->attr.type ) )
                    return( PSA_ERROR_INVALID_ARGUMENT );
#if defined(MBEDTLS_PSA_BUILTIN_ALG_FFDH)
            return mbedtls_psa_ffdh_key_agreement(&private_key->attr,
                                                  peer_key,
                                                  peer_key_length,
                                                  private_key->key.data,
                                                  private_key->key.bytes,
