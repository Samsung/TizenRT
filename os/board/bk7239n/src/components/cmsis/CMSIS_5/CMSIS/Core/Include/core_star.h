...
__STATIC_INLINE void __NVIC_TriggerIRQ(uint32_t IRQn)
{
  if ((int32_t)(IRQn) >= 0)
  {
    NVIC->STIR = IRQn;
...
/**
  ...
  \note    IRQn must not be negative.
 */
__STATIC_INLINE uint32_t NVIC_GetTargetState(uint32_t IRQn)
{
  if ((int32_t)(IRQn) >= 0)
  {
...
/**
  ...
  \note    IRQn must not be negative.
 */
__STATIC_INLINE uint32_t NVIC_SetTargetState(uint32_t IRQn)
{
  if ((int32_t)(IRQn) >= 0)
  {
...
/**
  ...
  \note    IRQn must not be negative.
 */
__STATIC_INLINE uint32_t NVIC_ClearTargetState(uint32_t IRQn)
{
  if ((int32_t)(IRQn) >= 0)
  {
    NVIC->ITNS[(((uint32_t)IRQn) >> 5UL)] &= ~((uint32_t)(1UL << (((uint32_t)IRQn) & 0x1FUL)));
 ...
 */
__STATIC_INLINE void __NVIC_SetPriority(uint32_t IRQn, uint32_t priority)
{
  if ((int32_t)(IRQn) >= 0)
  {
 ...
 */
__STATIC_INLINE uint32_t __NVIC_GetPriority(uint32_t IRQn)
{
...
/**
  ...
  \note    IRQn must not be negative.
 */
__STATIC_INLINE void TZ_NVIC_EnableIRQ_NS(uint32_t IRQn)
{
  if ((int32_t)(IRQn) >= 0)
  {
...
/**
  ...
  \note    IRQn must not be negative.
 */
__STATIC_INLINE uint32_t TZ_NVIC_GetEnableIRQ_NS(uint32_t IRQn)
{
  if ((int32_t)(IRQn) >= 0)
  {
...
/**
  ...
  \note    IRQn must not be negative.
 */
__STATIC_INLINE void TZ_NVIC_DisableIRQ_NS(uint32_t IRQn)
{
  if ((int32_t)(IRQn) >= 0)
  {
...
/**
  ...
  \note    IRQn must not be negative.
 */
__STATIC_INLINE uint32_t TZ_NVIC_GetPendingIRQ_NS(uint32_t IRQn)
{
  if ((int32_t)(IRQn) >= 0)
  {
...
/**
  ...
  \note    IRQn must not be negative.
 */
__STATIC_INLINE void TZ_NVIC_SetPendingIRQ_NS(uint32_t IRQn)
{
  if ((int32_t)(IRQn) >= 0)
  {
...
/**
  ...
  \note    IRQn must not be negative.
 */
__STATIC_INLINE void TZ_NVIC_ClearPendingIRQ_NS(uint32_t IRQn)
{
  if ((int32_t)(IRQn) >= 0)
  {
...
/**
  ...
  \note    IRQn must not be negative.
 */
__STATIC_INLINE uint32_t TZ_NVIC_GetActive_NS(uint32_t IRQn)
{
  if ((int32_t)(IRQn) >= 0)
  {
 ...
 */
__STATIC_INLINE void TZ_NVIC_SetPriority_NS(uint32_t IRQn, uint32_t priority)
{
  if ((int32_t)(IRQn) >= 0)
  {
 ...
 */
__STATIC_INLINE uint32_t TZ_NVIC_GetPriority_NS(uint32_t IRQn)
{
...
/**
  ...
  \param[in]   dsize   size of memory block (in number of bytes)
*/
__attribute__((section(".iram"))) __STATIC_FORCEINLINE void SCB_CleanInvalidateDCache_by_Addr (uint32_t *addr, int32_t dsize)
{
  #if defined (__DCACHE_PRESENT) && (__DCACHE_PRESENT == 1U)
    if ( dsize > 0 ) { 
       int32_t op_size = dsize + (((uint32_t)addr) & (__SCB_DCACHE_LINE_SIZE - 1U));
...
__STATIC_INLINE uint32_t SysTick_Config(uint32_t ticks)
{
  ...
  NVIC_SetPriority (SYS_TICK_IRQ_ID, (1UL << __NVIC_PRIO_BITS) - 1UL); /* set Priority for Systick Interrupt */
...
__STATIC_INLINE uint32_t TZ_SysTick_Config_NS(uint32_t ticks)
{
  ...
  TZ_NVIC_SetPriority_NS (SYS_TICK_IRQ_ID, (1UL << __NVIC_PRIO_BITS) - 1UL); /* set Priority for Systick Interrupt */
