...
int tz_aes_cbc_test_main(void)
{
	...
	//uint32_t  olen          = 0;
	//psa_cipher_operation_t operation = PSA_CIPHER_OPERATION_INIT;
	unsigned char input[CRYPTO_EXAMPLE_AES_MAX_TEXT_SIZE] = {
	           "Example string to demonstrate basic usage of AES CBC mode."};
	unsigned char output[CRYPTO_EXAMPLE_AES_MAX_TEXT_SIZE];
	unsigned char decrypted_text[CRYPTO_EXAMPLE_AES_MAX_TEXT_SIZE];

	length = sizeof(input);

	tz_key_mgt_init();
	tz_mbedtls_aes_init(&ctx);

	tz_mbedtls_aes_setkey_enc(&ctx, key,keybits);

	mode  = MBEDTLS_AES_ENCRYPT;
	/* Generate an IV */
	#if 0
	status = psa_cipher_generate_iv(&operation, iv, sizeof(iv), (size_t *)&olen);
	if (status != PSA_SUCCESS) {
		BK_LOGI(TAG, "psa_cipher_generate_iv failed! (Error: %d)\r\n", status);
		return APP_ERROR;
	}
	#endif
	tz_mbedtls_aes_crypt_cbc(&ctx,mode,length,&iv[0],input,output );

	tz_mbedtls_aes_setkey_dec(&ctx, key,keybits);
