 ...
 * Parse a private key
 */
#if defined(CONFIG_PSA_MBEDTLS_FORWARD_COMPATIBILITY)
int mbedtls_pk_parse_key_ex(mbedtls_pk_context *pk,
                         const unsigned char *key, size_t keylen,
                         const unsigned char *pwd, size_t pwdlen,
                         int (*f_rng)(void *, unsigned char *, size_t), void *p_rng)
{
...
/*
    ...
    //fflush(stdout);
    ret = mbedtls_entropy_add_source(&entropy, mbedtls_bk_entropy_poll, NULL,
    ...
    ret = mbedtls_pk_parse_key_ex(pk,key,keylen,pwd,pwdlen,mbedtls_ctr_drbg_random,&ctr_drbg);
