 ...
 */
#if 0//defined(CONFIG_TFM_ECP_NSC)
...
static int ecp_mod_p256(mbedtls_mpi *N)
{
	...
	mpi_b.olen = sizeof(mpi_b.buf);
	MBEDTLS_MPI_CHK(mbedtls_mpi_write_binary((const mbedtls_mpi *)N, mpi_b.buf, mpi_b.olen));

	tfm_ns_interface_lock();
	GLOBAL_INT_DECLARATION();
	GLOBAL_INT_DISABLE();

	ret = psa_ecp_mod_nsc(MBEDTLS_ECP_DP_SECP256R1, &mpi_b);
	...
	if(ret != 0)
	{
		os_printf(" Err: ecp_mod_p256 ret = %d \r\n", ret);
		goto cleanup;
	}

	MBEDTLS_MPI_CHK( mbedtls_mpi_read_binary(N, mpi_b.buf, mpi_b.olen ));
...
static int ecp_mod_p384(mbedtls_mpi *N)
{
	...
	ret = psa_ecp_mod_nsc(MBEDTLS_ECP_DP_SECP384R1, &mpi_b);
	...
	if(ret != 0)
	{
		os_printf(" Err: ecp_mod_p384 ret = %d \r\n", ret);
		goto cleanup;
	}
