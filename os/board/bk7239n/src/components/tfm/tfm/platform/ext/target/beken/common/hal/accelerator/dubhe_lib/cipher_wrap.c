...
#define CIPHER_AES_SUPPORT_LEN_192 (1)
...
#if defined(ARM_CE_DUBHE_SCA)
static int aes_set_derived_key_wrap( void *ctx, mbedtls_derived_key_type_t key_type, int mode,
                                     const unsigned char *key1, const unsigned char *key2,
                                     const unsigned char *key3, unsigned int key1_bitlen)
{
    return mbedtls_aes_set_derived_key( (mbedtls_aes_context *) ctx, key_type, mode, key1,
                                key2, key3, key1_bitlen);
}
#endif
...
static int sm4_set_derived_key_wrap( void *ctx, mbedtls_derived_key_type_t key_type, int mode,
                                     const unsigned char *key1, const unsigned char *key2,
                                     const unsigned char *key3, unsigned int key1_bitlen)
{
    return mbedtls_sm4_set_derived_key( (mbedtls_sm4_context *) ctx, key_type, mode, key1,
                                key2, key3, key1_bitlen);
}
