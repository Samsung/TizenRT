...
#if defined(ARM_CE_DUBHE_SM4)
int mbedtls_cipher_set_derived_key( mbedtls_cipher_context_t *ctx,
                           mbedtls_derived_key_type_t key_type,
                           const unsigned char *key1, const unsigned char *key2,
                           const unsigned char *key3, int key1_bitlen,
                           const mbedtls_operation_t operation )
{
    ...
    if( ( ctx->cipher_info->flags & MBEDTLS_CIPHER_VARIABLE_KEY_LEN ) == 0 &&
        (int) ctx->cipher_info->key_bitlen != key1_bitlen )
    {
        return( MBEDTLS_ERR_CIPHER_BAD_INPUT_DATA );
    }

    ctx->key_bitlen = key1_bitlen;
    ctx->operation = operation;

    return ctx->cipher_info->base->set_derived_key_func( ctx->cipher_ctx, key_type,
            key1, key2, key3, key1_bitlen );
}
#endif
