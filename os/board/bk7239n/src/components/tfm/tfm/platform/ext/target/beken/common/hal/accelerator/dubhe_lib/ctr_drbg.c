...
int block_cipher_df_ex( unsigned char *output,
                            const unsigned char *data, size_t data_len )
{
	return block_cipher_df( output, data, data_len );
}
...
#if ( MBEDTLS_WITH_DUBHE && defined(MBEDTLS_AES_ALT))
...
#define ROUND_DOWN(x, align) ((unsigned long)(x) & ~((unsigned long)align - 1))
    ...
    int32_t offset;
    size_t output_len_up, output_len_down;

    output_len_up = ROUND_UP(output_len, MBEDTLS_CTR_DRBG_BLOCKSIZE);
    output_len_down = ROUND_DOWN(output_len, MBEDTLS_CTR_DRBG_BLOCKSIZE);

    (void)use_len;
    counter = mbedtls_calloc(1, output_len_up);
    if (NULL == counter) {
        return -1;
    }
    memcpy(tmp, ctx->counter, MBEDTLS_CTR_DRBG_BLOCKSIZE);
    offset = 0;

    while(offset < output_len_up)
    {
        /*
         * Increase counter
         */
        memcpy(counter + offset, tmp, MBEDTLS_CTR_DRBG_BLOCKSIZE);
        for( i = MBEDTLS_CTR_DRBG_BLOCKSIZE; i > 0; i-- )
            if( ++counter[offset + i - 1] != 0 )
                break;
        memcpy(tmp, counter + offset, MBEDTLS_CTR_DRBG_BLOCKSIZE);
        offset += MBEDTLS_CTR_DRBG_BLOCKSIZE;
    }
    /*
     * Crypt counter block
     */
    ret = arm_ce_sca_crypt( &ctx->aes_ctx, MBEDTLS_AES_ENCRYPT, ARM_CE_AES_ECB, 0,
                          output_len_down, NULL, NULL, NULL, counter, p);

    if(ret != 0 )
    {
        mbedtls_free(counter);
        return( ret );
    }

    if (output_len_up > output_len_down) {
        ret = arm_ce_sca_crypt( &ctx->aes_ctx, MBEDTLS_AES_ENCRYPT, ARM_CE_AES_ECB, 0,
                              MBEDTLS_CTR_DRBG_BLOCKSIZE, NULL, NULL, NULL,
                              counter + output_len_down, tmp);
        if(ret != 0 )
        {
            mbedtls_free(counter);
            return( ret );
        }
        memcpy(p + output_len_down, tmp, output_len - output_len_down);
    }
    if (0 == (output_len % MBEDTLS_CTR_DRBG_BLOCKSIZE)) {
        memcpy(ctx->counter, counter + output_len - MBEDTLS_CTR_DRBG_BLOCKSIZE, MBEDTLS_CTR_DRBG_BLOCKSIZE);
    } else {
        memcpy(ctx->counter, counter + output_len_down, MBEDTLS_CTR_DRBG_BLOCKSIZE);
    }
    mbedtls_free(counter);
#else
    while( output_len > 0 )
