...
static gpio_isr_t s_iomx_gpio_isr[SOC_GPIO_NUM] = {NULL};
...
static void iomx_gpio_isr(void);
...
static void iomx_gpio_isr(void)
{
	...
	for (gpio_id = 0; gpio_id < SOC_GPIO_NUM; gpio_id++) {
		if (iomx_hal_is_interrupt_triggered(gpio_id, &gpio_status)) {
			if (s_iomx_gpio_isr[gpio_id]) {
				IOMX_LOGD("gpio int: index:%d \r\n",gpio_id);
				s_iomx_gpio_isr[gpio_id](gpio_id);
			}
