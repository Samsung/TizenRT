...
{
	prro_ll_set_gbl_control_soft_reset(1);
	return BK_OK;
}
...
bk_err_t prro_hal_set_privilege(prro_dev_t dev, prro_privilege_type_t privilege_type)
{
	...
	if (dev != ap_map[dev].dev) {
		PRRO_HAL_LOGE(TAG, "priv device=%d not match map index\n", dev);
	...
	if (ap_map[dev].reg == PRRO_INVALID_REG) {
		PRRO_HAL_LOGD(TAG, "skip the invalid dev=%d\r\n", dev);
		return BK_OK;
	}
...
{
	uint32_t gpio_bit = dev - PRRO_DEV_GPIO0;
...
bk_err_t prro_hal_set_gpios_secure(uint32_t group0_bits, uint32_t group1_bits)
{
	...
	//prro_ll_set_aon_gpio_nonsec0(group0_bits); //origin 
	prro_ll_set_aonp_gpio_nsec0_aon_gpio_nsec0(group0_bits);
	//prro_ll_set_aon_gpio_nonsec1(group1_bits);//origin 
	prro_ll_set_aonp_gpio_nsec1_aon_gpio_nsec1(group1_bits);
	return BK_OK;
}
