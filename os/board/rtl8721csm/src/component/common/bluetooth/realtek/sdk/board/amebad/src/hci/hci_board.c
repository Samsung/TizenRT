...
extern uint32_t bt_flatk_8721d(uint16_t txgain_flatk);
    ...
    if (payload_len != config_len - BT_CONFIG_HEADER_LEN)
    {
        ...
        LE_UINT16_TO_STREAM(p_len, config_len - BT_CONFIG_HEADER_LEN);  /* just avoid the length is not correct */
        /* FIX the len */
    }
...
#define HCI_CONFIG_HEAD 6
    ...
    LE_UINT16_TO_STREAM(p_len, config_length);  /* just avoid the length is not correct */
    if(!CHECK_SW(EFUSE_SW_DRIVER_DEBUG_LOG))
    {
