...
#define VAD_DEV ((VADBUF_TypeDef *)VAD_BUF_REG_BASE)
...
void get_vad_data_interleave(u32 time_period_ms, u8 *buf)
{
	...
	if (first_time_flag) {
		...
		sram_sel = VADBUF_VAD_W_SRAM_ADDRESS_SEL(VAD_DEV -> VADBUF_VAD_BUF_CTRL0);
		...
		switch (hit_channel) {
		case 0:
			hit_block = VADBUF_GET_CODEC0_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0);
			printf("here 0\n");
			break;
		case 1:
			hit_block = VADBUF_GET_CODEC1_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0);
			printf("here 1\n");
			break;
		case 2:
			hit_block = VADBUF_GET_CODEC2_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0);
			printf("here 2\n");
			break;
		case 3:
			hit_block = VADBUF_GET_CODEC3_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0);
			printf("here 3\n");
			break;
		default:
			break;
		...
		*/
		...
		if (VAD_DEV -> VADBUF_VAD_BUF_CTRL0 & VADBUF_BIT_CH0_IN_VADBUF_EN) {
			if (block_low_prio < VADBUF_GET_CODEC0_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0)) {
				block_low_prio = VADBUF_GET_CODEC0_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0);
			}
			if (block_high_prio > VADBUF_GET_CODEC0_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0)) {
				block_high_prio = VADBUF_GET_CODEC0_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0);
			}
			used_channels++;
		}

		if (VAD_DEV -> VADBUF_VAD_BUF_CTRL0 & VADBUF_BIT_CH1_IN_VADBUF_EN) {
			if (block_low_prio < VADBUF_GET_CODEC1_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0)) {
				block_low_prio = VADBUF_GET_CODEC1_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0);
			}

			if (block_high_prio > VADBUF_GET_CODEC1_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0)) {
				block_high_prio = VADBUF_GET_CODEC1_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0);
			}
			used_channels++;
		}

		if (VAD_DEV -> VADBUF_VAD_BUF_CTRL0 & VADBUF_BIT_CH2_IN_VADBUF_EN) {
			if (block_low_prio < VADBUF_GET_CODEC2_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0)) {
				block_low_prio = VADBUF_GET_CODEC2_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0);
			}
			if (block_high_prio > VADBUF_GET_CODEC2_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0)) {
				block_high_prio = VADBUF_GET_CODEC2_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0);
			}
			used_channels++;
		}

		if (VAD_DEV -> VADBUF_VAD_BUF_CTRL0 & VADBUF_BIT_CH3_IN_VADBUF_EN) {
			if (block_low_prio < VADBUF_GET_CODEC3_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0)) {
				block_low_prio = VADBUF_GET_CODEC3_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0);
			}
			if (block_high_prio > VADBUF_GET_CODEC3_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0)) {
				block_high_prio = VADBUF_GET_CODEC3_IN_BUF_ADD_BLOCK(VAD_DEV -> VADBUF_VAD_BUF_CTRL0);
			}
			used_channels++;
		}
...
void get_vad_data(u32 time_period_ms, u8 *buf)
{
	...
	if (TrustZone_IsSecure()) {
		VAD = ((VADBUF_TypeDef *)VAD_BUF_REG_BASE_S);
	} else {
		VAD = ((VADBUF_TypeDef *)VAD_BUF_REG_BASE);
	}

	if ((VAD -> VADBUF_VAD_BUF_CTRL0) && VAD_BIT_W_MODE_SEL) {
