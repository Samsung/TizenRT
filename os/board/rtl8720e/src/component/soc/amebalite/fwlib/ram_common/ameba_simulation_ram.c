...
u32 simulation_stage_set(u32 cpuid, u32 stage_bit)
{
	...
	if ((cpuid == SIMULATION_KM4_CPUID) && (stage_bit == BIT_KM4_RUN_INTO_FLASH)) {
		HAL_WRITE32(SYSTEM_CTRL_BASE, KM0_SIMULATION_STAGE_REG, 0);
		HAL_WRITE32(SYSTEM_CTRL_BASE, KM4_SIMULATION_STAGE_REG, 0);
	}

	if (cpuid == SIMULATION_KM0_CPUID) {
		temp = HAL_READ32(SYSTEM_CTRL_BASE, KM0_SIMULATION_STAGE_REG);
		temp |= stage_bit;
		HAL_WRITE32(SYSTEM_CTRL_BASE, KM0_SIMULATION_STAGE_REG, temp);

		//DBG_8195A("KM0 STAGE: %x BIT(%d)\n", temp, bit_index);
	} else {
		temp = HAL_READ32(SYSTEM_CTRL_BASE, KM4_SIMULATION_STAGE_REG);
		temp |= stage_bit;
		HAL_WRITE32(SYSTEM_CTRL_BASE, KM4_SIMULATION_STAGE_REG, temp);
