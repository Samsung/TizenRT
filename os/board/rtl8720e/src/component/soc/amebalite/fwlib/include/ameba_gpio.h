...
/** @defgroup GPIO_DR
 * @brief Portx/B data register
 * @{
 **/
#define GPIO_MASK_PORT_DATA            ((u32)0xFFFFFFFF << 0)          /*!<R/W 0  Values written to this register are output on the I/O signals for Portx if the corresponding data direction bits for Portx are set to Output mode and the corresponding control bit for Portx is set to software mode. The value read back is equal to the last value written to this register. AmebaLite valid bits is: PortA[31:0] and PortB[19:0]. */
#define GPIO_PORT_DATA(x)              ((u32)(((x) & 0xFFFFFFFF) << 0))
#define GPIO_GET_PORT_DATA(x)          ((u32)(((x >> 0) & 0xFFFFFFFF)))
/** @} */

/** @defgroup GPIO_DDR
 * @brief Portx/B data direction register
 * @{
 **/
#define GPIO_MASK_PORT_DATA_DIRECTION  ((u32)0xFFFFFFFF << 0)          /*!<R/W 0  Values written to this register independently control the direction of the corresponding data bit in Portx. The default direction can be configured as input or output after system reset through the GPIO_DFLT_DIR_x parameter. 0: Input (default) 1: Output AmebaLite valid bits is: PortA[31:0] and PortB[19:0]. */
