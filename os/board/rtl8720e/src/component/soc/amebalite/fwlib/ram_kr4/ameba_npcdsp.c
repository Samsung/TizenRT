...
void dsp_power_off(void)
{
	...
	DBG_PRINTF(MODULE_KR4, LEVEL_INFO, "DSPOFF\n");
...
void dsp_power_on(void)
{
	...
	DBG_PRINTF(MODULE_KR4, LEVEL_INFO, "DSPON\n");
...
void dsp_clock_gate(void)
{
	...
	DBG_PRINTF(MODULE_KR4, LEVEL_INFO, "DSPCG\n");
...
void dsp_clock_on(void)
{
	...
	DBG_PRINTF(MODULE_KR4, LEVEL_INFO, "DSPCW\n");
...
u32 dsp_suspend(u32 type)
{
	...
	DBG_8195A("dsp_suspend %x %x\n", sleep_wevent_config_val[0], sleep_wevent_config_val[1]);
	if ((sleep_wevent_config_val[0] | sleep_wevent_config_val[1])) {
		InterruptRegister(NPWDSP_INTHandler, DSP_WAKE_IRQ_KR4, (u32)PMC_BASE, INT_PRI3);
		InterruptEn(DSP_WAKE_IRQ_KR4, INT_PRI3);
	}
...
void dsp_tickless_ipc_int(UNUSED_WARN_DIS VOID *Data, UNUSED_WARN_DIS u32 IrqStatus, UNUSED_WARN_DIS u32 ChanNum)
{
	...
	DBG_PRINTF(MODULE_KR4, LEVEL_INFO, "dsp_tickless_ipc_int\n");
...
const IPC_INIT_TABLE   ipc_kr4cdsp_table[] = {
	{IPC_USER_DATA, 	dsp_tickless_ipc_int,	(VOID *) NULL, IPC_DSP_TO_KR4, IPC_D2R_TICKLESS_INDICATION, IPC_RX_FULL},
};
