...
_sema g_ipc_master_wake_sema[IMQ_QUEUE_MAX];

imq_error_t IPC_Message_Queue_HeapRegion_Init(int ID, int size, int type, uint32_t *address, uint32_t *asize)
{
	uint32_t baseAddr;
	uint32_t maxLen = 0;
	uint32_t xAddress;
	imq_share_heap_t *pxHeapHeader;
	uint32_t xAlignSize = 0;

	switch (type) {
	case IMQ_MASTER:
		baseAddr = IMQ_QUEUE_MASTER_BASE_ADDR;
		maxLen = IMQ_QUEUE_MASTER_MAX_LEN;
		break;
	case IMQ_SLAVE:
		baseAddr = IMQ_QUEUE_SLAVE_BASE_ADDR;
		maxLen = IMQ_QUEUE_SLAVE_MAX_LEN;
		break;
	}

	pxHeapHeader = (imq_share_heap_t *)baseAddr;

	if (pxHeapHeader->init != 1) {
		pxHeapHeader->remainFreeBytes = maxLen - xShareHeapHeaderSize;
		pxHeapHeader->startBlock = baseAddr + xShareHeapHeaderSize;
		pxHeapHeader->totalLen = maxLen;
		pxHeapHeader->init = 1;
		pxHeapHeader->lock = 0;
	}

	if (pxHeapHeader->remainFreeBytes < size) {
...
imq_error_t IPC_Message_Queue_Master_Init(u8 ID, int sendbuf_size, int recvbuf_size)
{
	...
	uint32_t queue_addr;
	uint32_t s_address, m_address;
	uint32_t s_size, m_size;
	...
	/* Initialize for master queue ring buffer */

	ret = IPC_Message_Queue_HeapRegion_Init(ID, sendbuf_size, IMQ_MASTER, &m_address, &m_size);

	if (ret != IMQ_OK) {
		return ret;
	}

	/* Initialize for slave queue ring buffer */

	ret = IPC_Message_Queue_HeapRegion_Init(ID, recvbuf_size, IMQ_SLAVE, &s_address, &s_size);
	...
	queue_info->m_addr = m_address;
	queue_info->m_len = m_size;
	queue_info->m_wptr = m_address;
	queue_info->m_rptr = s_address;

	queue_info->s_addr = s_address;
	queue_info->s_len = s_size;
	queue_info->s_wptr = s_address;
	queue_info->s_rptr = m_address;
	queue_info->m_init = 1;


	printf("%s, queue id: %x, master addr: 0x%08x, size: %x\n", __func__, ID, m_address, m_size);
	printf("%s, queue id: %x, slave  addr: 0x%08x, size: %x\n", __func__, ID, s_address, s_size);

	/* Initialize the semaphore */

	g_ipc_master_wake_sema[ID] = xSemaphoreCreateCounting(0xFFFFFFFF, 0);

	DCache_Clean(queue_addr, sizeof(imq_queue_info_t));

	/* Wait until slave initialized */
	while (queue_info->s_init != 1) {
		vTaskDelay(10);
		DCache_Invalidate((u32)queue_info, sizeof(imq_queue_info_t));
	}

	printf("%s, init done, s_init: %d\n", __func__, queue_info->s_init);
...
uint32_t IPC_checkRemainFreeBytes(imq_queue_info_t *queue_info)
{
	...
	if (queue_info->s_rptr > queue_info->m_wptr) {
		remainFreeBytes = queue_info->s_rptr - queue_info->m_wptr;
	} else {
		remainFreeBytes = (queue_info->s_rptr - startAddr) + (endAddr - queue_info->m_wptr);
	}
...
imq_error_t IPC_Message_Queue_Send(u8 ID, u8 *buf, int size)
{
	...
	if (alignSize > queue_info->m_len) {
		return IMQ_NO_MEMORY;
	}

	/* Block incase the slave haven't recevied the message */
	...
	if (segmentNeed) {
		...
		memcpy(queue_info->m_wptr, &tx_message, hdrLen);
		offset = segmentLen - hdrLen;
		/* Copy buf into queue ring buffer */
		memcpy(queue_info->m_wptr + hdrLen, buf, offset);
		DCache_Clean(queue_info->m_wptr, segmentLen);

		/* Write pointer switch to the start of queue ring buffer */
		queue_info->m_wptr = queue_info->m_addr;
		memcpy(queue_info->m_wptr, buf + offset, size - offset);
		DCache_Clean(queue_info->m_wptr, alignSize - segmentLen);

		queue_info->m_wptr += (alignSize - segmentLen);
	} else {
		/* Copy tx_message into queue ring buffer */
		memcpy(queue_info->m_wptr, &tx_message, hdrLen);
		...
		memcpy(queue_info->m_wptr + hdrLen, buf, size);
		DCache_Clean(queue_info->m_wptr, alignSize);

		/* Update master write pointer and slave read pointer */
		queue_info->m_wptr += alignSize;
	}

	DCache_Clean((u32)queue_info, sizeof(imq_queue_info_t));
	//printf("%s, master, id: %d, size: %d,w_ptr: 0x%08x, s_rptr: 0x%08x\n",
	//					__func__, ID, size, queue_info->m_wptr, queue_info->s_rptr);
	...
	ipc_send_message(IPC_DIR_MSG_TX, IPC_M2S_IMQ_TRX_TRAN, &ipc_message);
...
imq_error_t IPC_Message_Queue_Send_Timeout(u8 ID, u8 *buf, int size, int timeout)
{
	...
	/* Wait timeout incase the slave haven't recevied the message */
	while (remainFreeBytes < alignSize) {
		if (cnt >= timeout) {
...
imq_error_t IPC_Message_Queue_Master_Recv(u8 ID, u8 *buf, int *size)
{
	imq_queue_info_t *queue_info;
	uint32_t queue_addr;
	...
	/* Recv message from slave, get rx_message */

	if (endAddr - queue_info->m_rptr < hdrLen) {
		switchNeed = 1;
		queue_info->m_rptr = queue_info->s_addr;
	}

	rx_message = (struct imq_message *)queue_info->m_rptr;
	DCache_Invalidate((u32)rx_message, sizeof(struct imq_message));

	if (rx_message->msg_len < 0 || rx_message->msg_len > queue_info->s_len) {
		printf("%s %d recv error, msg_len: %d\n", __func__, ID, rx_message->msg_len);
		return IMQ_RX_ERROR;
	}
	...
	if (queue_info->m_rptr + alignSize > endAddr && !switchNeed) {
		segmentNeed = 1;
		segmentLen = endAddr - queue_info->m_rptr;
	}

	/* Get data buffer */

	if (segmentNeed) {
		DCache_Invalidate(queue_info->m_rptr, segmentLen);
		offset = segmentLen - hdrLen;
		/* Get buf from queue ring buffer */
		memcpy(buf, queue_info->m_rptr + hdrLen, offset);

		/* Read pointer switch to the start of queue ring buffer */
		queue_info->m_rptr = queue_info->s_addr;

		DCache_Invalidate(queue_info->m_rptr, alignSize - segmentLen);
		memcpy(buf + offset, queue_info->m_rptr, rx_message->msg_len - offset);
		/* Update master read pointer */
		queue_info->m_rptr += (alignSize - segmentLen);
	} else {
		DCache_Invalidate(queue_info->m_rptr, alignSize);

		/* Get buf from queue ring buffer */
		memcpy(buf, queue_info->m_rptr + hdrLen, rx_message->msg_len);
		/* Update master read pointer */
		queue_info->m_rptr += alignSize;
	}
	...
	*size = rx_message->msg_len;
	...
	//			ID, rx_message->msg_len, queue_info->m_rptr, queue_info->s_wptr);
	...
	if (chksum != rx_message->chksum) {
		printf("Master recv checksum error, id: %d, checksum0: %d, checksum1: %d\n", ID, chksum, rx_message->chksum);
	}
#endif
...
{
	/* Block receive */
	xSemaphoreTake(g_ipc_master_wake_sema[ID], 0xFFFFFFFF);
	return IPC_Message_Queue_Master_Recv(ID, buf, size);
}

imq_error_t IPC_Message_Queue_Recv_Timeout(u8 ID, u8 *buf, int *size, int timeout)
{
	if (xSemaphoreTake(g_ipc_master_wake_sema[ID], timeout) != pdTRUE) {
...
#endif
void share_memory_ipc_master_int_hdl(VOID *Data, u32 IrqStatus, u32 ChanNum)
{
	...
	PIPC_MSG_STRUCT p_ipc_recv_msg = ipc_get_message(IPC_DIR_MSG_RX, IPC_S2M_IMQ_TRX_TRAN);
	...
	xSemaphoreGiveFromISR(g_ipc_master_wake_sema[ID], &taskWoken);
...
const IPC_INIT_TABLE   ipc_message_master_table[] = {
	{IPC_USER_POINT,	share_memory_ipc_master_int_hdl,	(VOID *) NULL, IPC_DIR_MSG_RX, IPC_S2M_IMQ_TRX_TRAN, IPC_RX_FULL},
};
#endif
