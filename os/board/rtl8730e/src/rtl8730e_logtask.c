...
void rtl8730e_km4_logtask(void)
{
	static km4log_msg_t event = { 0 };

	/* initialize the queue that will hold the messages */
	if (!osif_msg_queue_create(&g_km4_log_queue, CONFIG_KM4_MAX_QUEUE_SIZE, sizeof(km4log_msg_t))) {
	...
	while(true) {
		...
		if (osif_msg_recv(g_km4_log_queue, &event, 0xFFFFFFFF)) {
			...
			((uint8_t *)event.buffer)[CONFIG_KM4_MAX_BUFFER_SIZE - 1] = 0;
...
void rtl8730e_km4_logtask_initialize(void)
{
	...
	if (!initialized) {
		...
		if (false == rtw_create_task(&km4_log_task, (const char *)"km4_log_task", CONFIG_RTL8730E_KM4_LOGTASK_STACK, CONFIG_RTL8730E_KM4_LOGTASK_PRIO, (void *)rtl8730e_km4_logtask, NULL)) {
			DBG_8195A("Fail to create init km4/np logtask\n");
			return;
		}

		lldbg("Logtask init ok!\n");
		initialized = true;
	}
