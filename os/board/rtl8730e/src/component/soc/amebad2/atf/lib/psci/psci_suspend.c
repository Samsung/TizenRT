			    ...
			    unsigned int is_power_down_state)
{
	 ...
	 */
	/* mask here because the lock may not release when lp system shutdown AP */
	//psci_acquire_pwr_domain_locks(end_pwrlvl, parent_nodes);
	...
	/*
	 * Don't check isr here because of unexpected interrupt by cpu hotplug.
	 * CPU hotplug need some time so timer interrupt may occurs.
	 */
	//if (read_isr_el1() != 0U) {
	if (0) {
		skip_wfi = 1;
