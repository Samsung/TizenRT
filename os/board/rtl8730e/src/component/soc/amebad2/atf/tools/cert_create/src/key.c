...
static int key_create_rsa(key_s *key, int key_bits)
{
	BIGNUM *e;
	RSA *rsa = NULL;
...
static int key_create_ecdsa(key_s *key, int key_bits)
{
	EC_KEY *ec;
...
typedef int (*key_create_fn_t)(key_s *key, int key_bits);
...
int key_create(key_s *key, int type, int key_bits)
{
...
int key_load(key_s *key, unsigned int *err_code)
{
	FILE *fp;
	EVP_PKEY *k;
...
key_s *key_get_by_opt(const char *opt)
{
	key_s *key;
	unsigned int i;
