...
static void draw_div_lines(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx)
{
	...
	if (chart->hdiv_cnt != 0) {
		...
		if (border_opa > LV_OPA_MIN && border_w > 0) {
			if ((border_side & LV_BORDER_SIDE_TOP) && (lv_obj_get_style_pad_top(obj, LV_PART_MAIN) == 0)) {
				i_start++;
			}
			if ((border_side & LV_BORDER_SIDE_BOTTOM) && (lv_obj_get_style_pad_bottom(obj, LV_PART_MAIN) == 0)) {
				i_end--;
			}
		}
	...
	if (chart->vdiv_cnt != 0) {
		...
		if (border_opa > LV_OPA_MIN && border_w > 0) {
			if ((border_side & LV_BORDER_SIDE_LEFT) && (lv_obj_get_style_pad_left(obj, LV_PART_MAIN) == 0)) {
				i_start++;
			}
			if ((border_side & LV_BORDER_SIDE_RIGHT) && (lv_obj_get_style_pad_right(obj, LV_PART_MAIN) == 0)) {
				i_end--;
			}
		}
...
static void draw_x_ticks(lv_obj_t *obj, lv_draw_ctx_t *draw_ctx, lv_chart_axis_t axis)
{
	...
	if (axis == LV_CHART_AXIS_PRIMARY_X) {
		...
		if (y_ofs + label_gap  + label_dsc.font->line_height + t->major_len < draw_ctx->clip_area->y1) {
			return;
		}
	}
