...
msg:
PChar;           // last error message, NULL if no error
internal:
Pointer;    // not visible by applications

zalloc:
TAlloc;       // used to allocate the internal state
zfree:
TFree;         // used to free the internal state
AppData:
...

{
	...
	you can seek forward in the stream, but not
...
In:
	InBuf = ptr to compressed data
			InBytes = number of bytes in InBuf
				  Out:
					  OutBuf = ptr to newly allocated buffer containing decompressed data
...
In:
	InBuf = ptr to compressed data
			InBytes = number of bytes in InBuf
				  Out:
					  OutBuf = ptr to user - allocated buffer to contain decompressed data
...
procedure crc32;
external;
procedure deflateInit2_;
external;
procedure deflateParams;
external;
...
strm.zfree :
	= zlibFreeMem;
OutBytes :
	= ((InBytes + (InBytes div 10) + 12) + 255) and not 255;
...
{
Result :
		= (1.0 - (FZRec.total_out / FZRec.total_in)) * 100.0;
	}
end;
...
while (FZRec.avail_out > 0) do
			...
			if FZRec.avail_in = 0 then
								begin
					FZRec.avail_in :
								= FStrm.Read(FBuffer, sizeof(FBuffer));
if FZRec.avail_in = 0 then
					begin
					Result :
					= Count - FZRec.avail_out;
Exit;
end;
FZRec.next_in :
	= FBuffer;
