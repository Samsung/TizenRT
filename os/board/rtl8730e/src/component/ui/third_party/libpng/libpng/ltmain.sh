									...
									_G_PATH = $ {3 - "$PATH"}
											...
											_G_path_prog_max = 0
													_G_path_prog_found = false
															_G_save_IFS = $IFS;
	IFS = $ {PATH_SEPARATOR - : }
		  for _G_dir in $_G_PATH;
do
	IFS = $_G_save_IFS
		  test - z "$_G_dir" && _G_dir = .
										 for _G_prog_name in $_G_progs_list;
do
	for _exeext in '' .EXE;
do
...
while :
		do
			...
			echo '' >> conftest.nl
					...
					if test "$_G_count" - gt "$_G_path_prog_max";
	then
...
func_path_progs "sed gsed" func_check_prog_sed $PATH:
/ usr / xpg4 / bin
rm - f conftest.sed
...
func_path_progs "grep ggrep" func_check_prog_grep $PATH:
/ usr / xpg4 / bin
...
# whether color sequences are supported.
			test - n "${COLORTERM+set}"&& :
			${USE_ANSI_COLORS = "1"}

			if test 1 = "$USE_ANSI_COLORS";
			then
...
# being used:
if test set = "${BASH_VERSION+set}${ZSH_VERSION+set}";
then
:
...
_G_directory_path = $1
					_G_dir_list =
						...
						if test - n "$_G_directory_path" && test :
							!= "$opt_dry_run";
then
...
# While some portion of DIR does not yet exist...
	while test ! -d "$_G_directory_path";
		do
# ...make a list in topmost first order.  Use a colon delimited
# list incase some portion of path contains whitespace.
			_G_dir_list = $_G_directory_path:
			$_G_dir_list
...
$LD(gnu ? $with_gnu_ld)
	version:
	$progname(GNU libtool) 2.4.6
	automake:
	`($AUTOMAKE --version) 2 > / dev / null | $SED 1q`
	autoconf:
	`($AUTOCONF --version) 2 > / dev / null | $SED 1q`
...
{
echo "host: $host"
if test yes = "$build_libtool_libs";
then
echo "enable shared libraries"
else
	echo "disable shared libraries"
	fi
	if test yes = "$build_old_libs";
then
...
cat > & 2 << _LT_EOF
		  $progname:
Version mismatch error.  This is $PACKAGE $VERSION, but the
$progname:
definition of this LT_INIT comes from an older release.
$progname:
You should recreate aclocal.m4 with macros from $PACKAGE $VERSION
$progname:
...
_LT_EOF
	...
	cat > & 2 << _LT_EOF
		  $progname:
			  Version mismatch error.  This is $PACKAGE $VERSION, but the
		  $progname:
			  definition of this LT_INIT comes from $PACKAGE $macro_version.
		  $progname:
			  You should recreate aclocal.m4 with macros from $PACKAGE $VERSION
		  $progname:
			  and run autoconf again.
			  _LT_EOF
				  ...
				  cat > & 2 << _LT_EOF
		  $progname:
				  Version mismatch error.  This is $PACKAGE $VERSION, revision $package_revision,
				  $progname:
						  but the definition of this LT_INIT comes from revision $macro_revision.
				  $progname:
						  You should recreate aclocal.m4 with macros from revision $package_revision
				  $progname:
						  of $PACKAGE $VERSION and run autoconf again.
						  _LT_EOF
						  fi
...
# preserve --debug
		 test :
		 = "$debug_cmd" || func_append preserve_args " --debug"
...
# Only execute mode is allowed to have -dlopen flags.
	if test - n "$opt_dlopen" && test execute != "$opt_mode";
	then
...
for lalib_p_l in 1 2 3 4
	...
	case $lalib_p_line in
			\#\ Generated\ by\ *$PACKAGE *) lalib_p = yes;
			break;;
			esac
			done
			exec 0 < & 5 5 < & -
...
# Place PIC objects in $objdir
																		...
																		if test warn = "$need_locks" &&
																						   test "X`cat $lockfile 2>/dev/null`" != "X$srcfile";
																						   then
																						   $ECHO "\
...
# If the previous option needed an argument, then skip it.
											if test - n "$prev";
												then
												if test X - m = "X$prev" && test - n "$install_override_mode";
													then
																																																										...
																																																										libfile = $libdir / `$ECHO "$lib" | $SED 's%^.*/%%g'`
																																																											...
																																																											finalize = false
																																																													   ...
																																																													   relink_command =
																																																														   ...
																																																														   outputname =
																																																															   if test no = "$fast_install" && test - n "$relink_command";
																																																												then
																																																												$opt_dry_run || {
																																																												if $finalize;
																																																												then
								...
								my_pic_p = $ {3 - false}
										my_prefix = `$ECHO "$my_originator" | $SED 's%[^a-zA-Z0-9]%_%g'`
												my_dlsyms =
														...
														if test - n "$dlfiles$dlprefiles" || test no != "$dlself";
then
...
func_cygming_gnu_implib_p()
{
	...
	func_cygming_gnu_implib_tmp = `$NM "$func_to_tool_file_result" | eval "$global_symbol_pipe" | $EGREP
...
shift
f_ex_an_ar_oldlib = $1
					if test yes = "$lock_old_archive_extraction";
then
...
if ($AR t "$f_ex_an_ar_oldlib" | sort | sort - uc > / dev / null 2 > & 1);
	then
:
	else
...
# Export our shlibpath_var if we have one.
																			if test yes = "$shlibpath_overrides_runpath" && test - n "$shlibpath_var" && test - n "$temp_rpath";
																			then
																			$ECHO "\
																						...
																						dlself = needless
																								 export_dynamic = yes
																												  ...
																												  prev =
																													  ...
																													  ;;
																						self)
																						if test dlprefiles = "$prev";
																							then
																							dlself = yes
																									 elif test dlfiles = "$prev" && test yes != "$dlopen_self";
																							then
																							dlself = yes
																																				 else
																																					 dlself = needless
...
# Prepend the subdirectory the object is found in.
																																													pic_object = $xdir$pic_object
																																																 ...
																																																 if test dlfiles = "$prev";
																																														then
																																														if test yes = "$build_libtool_libs" && test yes = "$dlopen_support";
																																															then
...
# A standard non-PIC object
																																																						 ...
																																																						 if test - z "$pic_object" || test none = "$pic_object";
																																																			then
																																																			arg = $non_pic_object
																																																				  fi
																																																				  else
...
# FIXME: remove this flag sometime in the future.
																																																															...
																																																															-export - symbols | -export - symbols - regex)
																																																																if test - n "$export_symbols" || test - n "$export_symbols_regex";
																																																																	...
																																																																	if test X - export - symbols = "X$arg";
																																																																		then
																																																																	prev = expsyms
																																																																		   else
																																																																			   prev = expsyms_regex
...
# so, if we see these flags be careful not to treat them like -L
	...
	*-*-cygwin* | *-*-mingw* | *-*-pw32* | *-*-os2* | *-cegcc*)
	...
	$dllsearchpath:
																																																																					...
																																																																					-l *)
																																																																						if test X - lc = "X$arg" || test X - lm = "X$arg";
																																																																							then
																																																																						case $host in
																																																																																																																		...
																																																																																																																		do
																																																																																																																			tmp_deplibs = "$deplib $tmp_deplibs"
																																																																																																																				...
																																																																																																																				case $pass in
																																																																																																																						...
																																																																																																																						if test lib, dlpreopen = "$linkmode,$pass";
																																																																																																																							then
																																																																																																																																		...
																																																																																																																																		compile_deplibs = "$deplib $compile_deplibs"
																																																																																																																																			...
																																																																																																																																			case "$new_inherited_linker_flags " in
																																																																																																																																					...
																																																																																																																																					*) func_append new_inherited_linker_flags " $deplib" ;;
																																																																																																																																					...
																																																																																																																																					;;
																																																																																																																																					-l *)
																																																																																																																																						if test lib != "$linkmode" && test prog != "$linkmode";
																																																																																																																																							then
...
# We need to do some special things here, and not later.
																																																																																																																																																		if test yes = "$allow_libtool_libs_with_static_runtimes";
																																																																																																																																																			then
																																																																																																																																																			...
																																																																																																																																																			library_names =
																																																																																																																																																				old_library =
																																																																																																																																																					...
																																																																																																																																																					ll = $l
																																																																																																																																																						 ...
																																																																																																																																																						 if test "X$ll" = "X$old_library";
																																																																																																																																																							 then # only static version available
																																																																																																																																																						found = false
																																																																																																																																																								func_dirname "$lib" "" "."
...
# *.$libext
																																																																																																																																																																																							*.lo | *.$objext)
																																																																																																																																																																																							if test conv = "$pass";
																																																																																																																																																																																								then
																																																																																																																																																																															deplibs = "$deplib $deplibs"
																																																																																																																																																																																	  elif test prog = "$linkmode";
																																																																																																																																																																																	  then
																																																																																																																																																																																	  if test dlpreopen = "$pass" || test yes != "$dlopen_support" || test no = "$build_libtool_libs";
																																																																																																																																																																																		  then
...
# Convert "-framework foo" to "foo.ltframework"
																																																																																																																																																																																			...
																																																																																																																																																																																			tmp_inherited_linker_flags = `$ECHO "$inherited_linker_flags" | $SED 's/-framework \([^ $]*\)/\1.ltframework/g'`
																																																																																																																																																																																						...
																																																																																																																																																																																						dependency_libs = `$ECHO " $dependency_libs" | $SED 's% \([^ $]*\).ltframework% -framework \1%g'`
																																																																																																																																																																																										  if test lib, link = "$linkmode,$pass" ||
																																																																																																																																																																																																  test prog, scan = "$linkmode,$pass" ||
																																																																																																																																																																																						{ test prog != "$linkmode" &&test lib != "$linkmode"; };
																				then
...
# It is a libtool convenience library, so add in its objects.
																							...
																							elif test prog != "$linkmode" &&test lib != "$linkmode";
																							then
...
# Get the name of the library we link against.
																											...
																											if test - n "$old_library" && {
																												...
																												test built, no = "$prefer_static_libs,$installed";
																											};
																				then
																				linklib = $old_library
																													  else
																																													...
																																													dir = $lt_sysroot$libdir
																																														  absdir = $lt_sysroot$libdir
																																																   ...
																																																   test yes = "$hardcode_automatic" && avoidtemprpath = yes
																																																		   else if test ! -f "$ladir/$objdir/$linklib" && test - f "$abs_ladir/$linklib";
																																								then
																																								dir = $ladir
																																									  absdir = $abs_ladir
...
# This library was specified with -dlpreopen.
																																																				  if test dlpreopen = "$pass";
																																										then
																																										if test - z "$libdir" && test prog = "$linkmode";
																																											then
...
# Link the convenience library
																																																	if test lib = "$linkmode";
																																																		...
																																																		deplibs = "$dir/$old_library $deplibs"
																																																				  elif test prog, link = "$linkmode,$pass";
																																																		then
																																																		compile_deplibs = "$dir/$old_library $compile_deplibs"
																																																						  finalize_deplibs = "$dir/$old_library $finalize_deplibs"
																																																												 ...
																																																												 deplibs = "$lib $deplibs" # used for prog, scan pass
																																																															 ...
																																																															 if test prog = "$linkmode" && test link != "$pass";
																																																					then
...
# We only need to search for static libraries
																				...
																				link_static = no # Whether the deplib will be linked statically
																							  use_static_libs = $prefer_static_libs
																												if test built = "$use_static_libs" && test yes = "$installed";
																					then
																					use_static_libs = no
																									  fi
																									  if test - n "$library_names" &&
																					{ test no = "$use_static_libs" || test - z "$old_library"; };
																				then
																			case $host in
																							...
																							do
																								if test "X$dlpremoduletest" = "X$lib";
																									...
																									dlopenmodule = $dlpremoduletest
																												   ...
																												   if test - z "$dlopenmodule" && test yes = "$shouldnotlink" && test link = "$pass";
																										then
																										echo
																										if test prog = "$linkmode";
																											then
...
# Create $newlib
																																								if test - f "$output_objdir/$newlib";
then :
																																									;
																																									else
...
# make sure the library variables are pointing to the new library
																																										dir = $output_objdir
																																											  linklib = $newlib
																																														...
																																														if test prog = "$linkmode" || test relink != "$opt_mode";
																																											then
																																											add_shlibpath =
																																												add_dir =
																																													add =
																																																...
																																																if test - z "$old_library";
																																																	...
																																																	case $host in
																																																			* - * -sunos *) add_shlibpath = $dir ;;
																																																			...
																																																			add_dir = -L$dir
																																																					  add = -l$name
																																																							elif test no = "$hardcode_shlibpath_var";
																																																			then
																																																			add_shlibpath = $dir
																																																							add = -l$name
																																																								  else
																																																				...
																																																				if test yes = "$hardcode_direct" &&
																																																								  test no = "$hardcode_direct_absolute";
																																																					then
																																																					add = $dir / $linklib
																																																						  elif test yes = "$hardcode_minus_L";
																																																					then
																																																					add_dir = -L$absdir
# Try looking first in the location we're being installed to.
																																																					...
																																																					case $libdir in
																																																							...
																																																							add = -l$name
																																																								  elif test yes = "$hardcode_shlibpath_var";
																																																							then
																																																							add_shlibpath = $dir
																																																											add = -l$name
																																																												  else
																																																																									...
																																																																									test - n "$add_dir" && deplibs = "$add_dir $deplibs"
																																																																											test - n "$add" && deplibs = "$add $deplibs"
																																																																													if test yes != "$hardcode_direct" &&
																																																																														...
																																																																														test yes = "$hardcode_shlibpath_var";
																																																														then
																																																													case :
$finalize_shlibpath:
																																																															in
...
$finalize_shlibpath:
																																																																		...
																																																																		*) func_append finalize_shlibpath "$libdir:" ;;
																																																																		...
																																																																		add = -l$name
																																																																			  elif test yes = "$hardcode_automatic";
																																																																		then
																																																																		if test - n "$inst_prefix_dir" &&
...
# shared platforms.
																																																																									if test unsupported != "$hardcode_direct";
																																																																										then
																																																																																					...
																																																																																					finalize_deplibs = "$dir/$linklib $finalize_deplibs"
																																																																																								...
																																																																																								compile_deplibs = "-l$name -L$dir $compile_deplibs"
																																																																																										finalize_deplibs = "-l$name -L$dir $finalize_deplibs"
																																																																																												...
																																																																																												elif test yes = "$build_libtool_libs";
																																																																											then
# Not a shared library
																																																																											if test pass_all != "$deplibs_check_method";
																																																																												then
																							...
																							if test - n "$i";
																								...
																								dependency_libs = $tmp_libs
																										...
																										dlfiles = $newdlfiles
																												  ...
																												  if test prog = "$linkmode" || test lib = "$linkmode";
																											then
...
# Add in all the interfaces that we are compatible with.
																																																																																																																																																loop = $revision
																																																																																																																																																													   while test 0 - ne "$loop";
																																																																																																																																																										do
																																																																																																																																																											func_arith $revision - $loop
																																																																																																																																																											iface = $func_arith_result
																																																																																																																																																													func_arith $loop - 1
																																																																																																																																																													loop = $func_arith_result
																																																																																																																																																														   verstring = $verstring_prefix$major.$iface:
																																																																																																																																																											$verstring
																																																																																																																																																											done
...
# Check to see if the archive will have undefined symbols.
																																																																																																																																																																					if test yes = "$allow_undefined";
																																																																																																																																																																			then
																																																																																																																																																																			if test unsupported = "$allow_undefined_flag";
																																																																																																																																																																				then
																																																																																																																																																																				if test yes = "$build_old_libs";
																																																																																																																																																																					then
																																																																																																																																																																								...
																																																																																																																																																																								removelist =
																																																																																																																																																																									...
																																																																																																																																																																									case $p in
																																																																																																																																																																											*.$objext | *.gcno)
																																																																																																																																																																											...
																																																																																																																																																																											if test - n "$precious_files_regex";
																																																																																																																																																																												then
...
# Now set the variables for building old libraries.
																																																																																																																																																																													if test yes = "$build_old_libs" && test convenience != "$build_libtool_libs";
																																																																																																																																																																														then
...
# but so what?
																																																																																																																																																																																																																															potlib = $potent_lib
																																																																																																																																																																																																																																	 while test - h "$potlib" 2 > / dev / null;
																																																																																																																																																																																																																																do
...
# can't use Xsed below, because $i might contain '/'
																																																																																																																																																																																																																																																							tmp_deplibs=`$ECHO " $tmp_deplibs" | $SED "s|$i||"`
																																																																																																																																																																																																																																																									...
																																																																																																																																																																																																																																																									case $tmp_deplibs in
																																																																																																																																																																																																																																																											*[!\	\ ] *)
																																																																																																																																																																																																																																																								echo
																																																																																																																																																																																																																																																								if test none = "$deplibs_check_method";
																																																																																																																																																																																																																																																									then
...
# FIXME: should test the right _cmds variable.
																																																																																																																																																																																																																																																																													case $archive_cmds in
																																																																																																																																																																																																																																																																															*\$LD\ *) wl = ;;
																																																																																																																																																																																																																																																																															...
																																																																																																																																																																																																																																																																															if test yes = "$hardcode_into_libs";
																																																																																																																																																																																																																																																																																then
...
# Prepare the list of exported symbols
																				if test - z "$export_symbols";
																					then
																					if test yes = "$always_export_symbols" || test - n "$export_symbols_regex";
																						then
...
# set to false by a later but shorter cmd.
																																																																																																																																																																																																																																																																																																														...
																																																																																																																																																																																																																																																																																																														IFS = $save_ifs
	  if test - n "$export_symbols_regex" && test :
																																																																																																																																																																																																																																																																																																																  != "$skipped_export";
																																																																																																																																																																																																																																																																																																															then
							 ...
							 if test :
																																																																																																																																																																																																																																																																																																																								 != "$skipped_export" && test - n "$orig_export_symbols";
																																																																																																																																																																																																																																																																																																																	then
																																																																																																																																																																																																																																																																																																																														  ...
																																																																																																																																																																																																																																																																																																																														  test "X$libobjs" = "X " && libobjs =
																																																																																																																																																																																																																																																																																																																																	  ...
																																																																																																																																																																																																																																																																																																																																	  gentop = $output_objdir / $ {outputname} x
																																																																																																																																																																																																																																																																																																																																			  ...
																																																																																																																																																																																																																																																																																																																																			  test "X$libobjs" = "X " && libobjs =
																																																																																																																																																																																																																																																																																																																																					  ...
																																																																																																																																																																																																																																																																																																																																					  if test yes = "$thread_safe" && test - n "$thread_safe_flag_spec";
																																																																																																																																																																																																																																																																																																																									then
...
# Do each of the archive commands.
																																																																																																																																																																																																																																																																																																																										if test yes = "$module" && test - n "$module_cmds";
																																																																																																																																																																																																																																																																																																																											then
																																																																																																																																																																																																																																																																																																																																 ...
																																																																																																																																																																																																																																																																																																																																 cmds = $module_expsym_cmds
																																																																																																																																																																																																																																																																																																																																			...
																																																																																																																																																																																																																																																																																																																																			eval test_cmds = \"$module_cmds\"
																																																																																																																																																																																																																																																																																																																																					cmds = $module_cmds
																																																																																																																																																																																																																																																																																																																																						   ...
																																																																																																																																																																																																																																																																																																																																						   else if test - n "$export_symbols" && test - n "$archive_expsym_cmds";
																																																																																																																																																																																																																																																																																																																														then
...
# initialize k to one.
																																																																																																																																																																																																																																																																																																																																															 ...
																																																																																																																																																																																																																																																																																																																																															 objlist =
																																																																																																																																																																																																																																																																																																																																																	 last_robj =
																																																																																																																																																																																																																																																																																																																																																			 k = 1
																																																																																																																																																																																																																																																																																																																																																					 ...
																																																																																																																																																																																																																																																																																																																																																					 if test - n "$save_libobjs" && test :
																																																																																																																																																																																																																																																																																																																																																						 != "$skipped_export" && test yes = "$with_gnu_ld";
																																																																																																																																																																																																																																																																																																																																			then
																																																																																																																																																																																																																																																																																																																																			output = $output_objdir / $output_la.lnkscript
																																																																																																																																																																																																																																																																																																																																					 ...
																																																																																																																																																																																																																																																																																																																																					 for obj in $save_libobjs
																																																																																																																																																																																																																																																																																																																																						 do
																																																																																																																																																																																																																																																																																																																																							 ...
																																																																																																																																																																																																																																																																																																																																							 output = $func_to_tool_file_result
																																																																																																																																																																																																																																																																																																																																									  ...
																																																																																																																																																																																																																																																																																																																																									  != "$skipped_export" && test - n "$file_list_spec";
																																																																																																																																																																																																																																																																																																																																					then
																																																																																																																																																																																																																																																																																																																																					output = $output_objdir / $output_la.lnk
																																																																																																																																																																																																																																																																																																																																							 ...
																																																																																																																																																																																																																																																																																																																																							 shift
																																																																																																																																																																																																																																																																																																																																							 firstobj =
																																																																																																																																																																																																																																																																																																																																								 if test yes = "$compiler_needs_object";
																																																																																																																																																																																																																																																																																																																																						then
																																																																																																																																																																																																																																																																																																																																						firstobj = "$1 "
																																																																																																																																																																																																																																																																																																																																								   shift
																																																																																																																																																																																																																																																																																																																																								   fi
...
# If -module or -export-dynamic was specified, set the dlname.
																										if test yes = "$module" || test yes = "$export_dynamic";
																											then
...
# $opt_dry_run || eval "echo timestamp > $libobj" || exit $?
																				...
																				if test - n "$pic_flag" || test default != "$pic_mode";
																					then
																																																																													...
																																																																													libdir = $hardcode_libdirs
																																																																															 eval rpath = \" $hardcode_libdir_flag_spec\"
																																																																																		  ...
																																																																																		  finalize_rpath = $rpath
																																																																																						   ...
																																																																																						   if test - n "$libobjs" && test yes = "$build_old_libs";
																																																																														then
...
# Disable wrappers for cegcc and mingw32ce hosts, we are cross compiling anyway.
																																																																																...
																																																																																*cygwin* | *mingw *)
																																																																																test yes = "$build_libtool_libs" || wrappers_required = false
																																																																																		   ;;
																																																																																*)
																																																																																if test no = "$need_relink" || test yes != "$build_libtool_libs";
																																																																																	then
																																																																																	wrappers_required = false
																																																																																						fi
																																																																																						;;
...
# Do each command in the archive commands.
																										 if test - n "$old_archive_from_new_cmds" && test yes = "$build_libtool_libs";
																									then
...
# not supported by libtool).
																																																																																																								 if (for obj in $oldobjs
																																																																																																											 do
																																																																																																												 ...
																																																																																																												 done | sort | sort - uc > / dev / null 2 > & 1);
																																																																																																									then
																																																																																																									:
																																																																																																									else
...
# Quote the link command for shipping.
																																																																																																																																											  relink_command = "(cd `pwd`; $SHELL \"$progpath\" $preserve_args --mode=relink $libtool_args @inst_prefix_dir@)"
																																																																																																																																													  relink_command = `$ECHO "$relink_command" | $SED "$sed_quote_subst"`
																																																																																																																																															  if test yes = "$hardcode_automatic";
																																																																																																																																				then
																																																																																																																																				relink_command =
																																																																																																																																					fi
...
# LD_LIBRARY_PATH before the program is installed.
																			...
																			if test link = "$opt_mode" || test relink = "$opt_mode";
																			then
																			func_mode_link ${1 + "$@"}
																			fi
...
# Add PIC object to the list of files to remove.
																													if test - n "$pic_object" && test none != "$pic_object";
																														then
																														func_append rmfiles " $dir/$pic_object"
																														fi

# Add non-PIC object to the list of files to remove.
																														if test - n "$non_pic_object" && test none != "$non_pic_object";
																															then
...
# as does the version of $file that was added into $rmfiles
																																						...
																																						if test yes = "$fast_install" && test - n "$relink_command";
																																					then
...
# Try to remove the $objdir's in the directories where we deleted files
																			...
																			if test uninstall = "$opt_mode" || test clean = "$opt_mode";
																			then
...
# ### BEGIN LIBTOOL TAG CONFIG: disable-static
																		build_old_libs = `case $build_libtool_libs in yes) echo no;;
																				*) echo yes;;
																				esac`
