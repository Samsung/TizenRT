...
#include "../libpng/contrib/tools/sRGB.h"
				   ...
				   png_const_colorp background)
{
	...
	if (stridea < 0) {
		rowa += (height - 1) * (-stridea);
	}
	if (strideb < 0) {
		rowb += (height - 1) * (-strideb);
	}
			...
			if (bchannels) {
				...
				if (formatb & PNG_FORMAT_FLAG_LINEAR) { /* 16-bit checks */
					...
					case 1:
						...
						if (alpha_added != 4 && pub[alpha_added] != 65535) {
							break;
						}
						continue; /* x loop */
					default:
					...
					case 1:
						...
						if (alpha_added != 4 && psb[alpha_added] != 255) {
							break;
						}
						continue; /* x loop */
					default:
