...
/ ^.*PNG_DEFN_MAGIC - .* -PNG_DEFN_END.*$$ / {
	...
	print
		...
		$(COPY) dfn2.out $@
		@type << > pngconfig.inf
		This is a locally configurable build of libpng.lib;
for configuration
instructions consult and edit projects / openwatcom / pngconfig.dfa
		...
		pnglibconf.dfn :
			..\..\scripts\pnglibconf.dfa ..\..\scripts\options.awk pngconfig.dfa ..\..\pngconf.h
...
#
						pnglibconf.h:
								..\..\scripts\pnglibconf.h.prebuilt .existsonly
								@$(ECHO) .
								@$(ECHO) .
						@$(ECHO) $$(AWK) NOT AVAILABLE:
								COPYING scripts\pnglibconf.h.prebuilt
								@$(ECHO) .
								@$(ECHO) .
...
# Make the default files
			...
			$#   3) Delete this file(config.inf).
					...
					@$(COPY) << pngconfig.dfa
		$# pngconfig.dfa:
					this file contains configuration options for libpng.
