...
#  include <cexcept.h>
#else
#  include "../libpng/contrib/visupng/cexcept.h"
#endif
			  ...
			  png_byte colour_type, png_byte bit_depth, png_uint_32 y)
{
	...
	case 1:
		while (i < 128 / 8) {
			buffer[i] = (png_byte)(v & 0xff), v += 17, ++i;
		}
		return;

	case 2:
		while (i < 128 / 4) {
			buffer[i] = (png_byte)(v & 0xff), v += 33, ++i;
		}
		return;

	case 4:
		while (i < 128 / 2) {
			buffer[i] = (png_byte)(v & 0xff), v += 65, ++i;
		}
		return;
									 ...
									 const transform_display *display)
{
	if (that->bit_depth == 16) {
		...
		if (that->green_sBIT > 8) {
			that->green_sBIT = 8;
		}
		if (that->blue_sBIT > 8) {
			that->blue_sBIT = 8;
		}
		if (that->alpha_sBIT > 8) {
			that->alpha_sBIT = 8;
		}
	}
...
#        endif /* before 1.7 */
				if (data.gamma == 1) {
					...
					r = pow(r, g1) * (1 - DBL_EPSILON);
					rhi = pow(rhi, g1) * (1 + DBL_EPSILON);
					g = pow(g, g1) * (1 - DBL_EPSILON);
					ghi = pow(ghi, g1) * (1 + DBL_EPSILON);
					b = pow(b, g1) * (1 - DBL_EPSILON);
					bhi = pow(bhi, g1) * (1 + DBL_EPSILON);
...
png_pass_row_shift(int pass)
{
	...
	if (base == (-1)) {
		return 0xfe;    /* error - no row in pass! */
	}
