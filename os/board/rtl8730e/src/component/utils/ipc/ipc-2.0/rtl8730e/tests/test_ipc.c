...
void RPCServerTask(void *param)
{
	...
	while (count < IMQ_TEST_COUNT1) {
		...
		size = IMQ_BUF_SIZE1;
		int32_t ret = IPC2_Message_Queue_Recv(QUEUE_AP_TO_NP, buf, &size);
...
void RPCClientTask(void *param)
{
	...
	while (count < IMQ_TEST_COUNT2) {
		...
		int32_t ret = IPC2_Message_Queue_Send(QUEUE_AP_TO_NP, buf, IMQ_BUF_SIZE2, WAIT_FOREVER);
		//if (count % 11 == 0)
		{
