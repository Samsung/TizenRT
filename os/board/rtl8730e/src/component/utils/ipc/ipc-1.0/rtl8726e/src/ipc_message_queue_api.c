...
imq_error_t IPC_Message_Queue_Send(u32 ID, u8 *buf, u32 size, int timeout)
{
	...
	if (queue_info->type == MESSAGE_QUEUE) {
		...
		ipc_send_message(queue_info->dir, IPC_IMQ_RX_TRAN_1, &ipc_message);
	}
	return IMQ_OK;
}
...
void IPC_Message_Queue_ipc_int_hdl_1(VOID *Data, u32 IrqStatus, u32 ChanNum)
{
	/* To avoid gcc warnings */
	(void) Data;
	(void) IrqStatus;
	...
	/* up the semaphore according to the id */
	PIPC_MSG_STRUCT p_ipc_recv_msg = ipc_get_message(IPC_DIR_IMQ_MSG_RX_1, IPC_IMQ_RX_TRAN_1);
	DCache_Invalidate((u32)p_ipc_recv_msg, sizeof(IPC_MSG_STRUCT));
...
const IPC_INIT_TABLE   ipc_message_queue_table[] = {
	...
	{IPC_USER_POINT,	IPC_Message_Queue_ipc_int_hdl_1,	(VOID *) NULL, IPC_DIR_IMQ_MSG_RX_1, IPC_IMQ_RX_TRAN_1, IPC_RX_FULL},
};
