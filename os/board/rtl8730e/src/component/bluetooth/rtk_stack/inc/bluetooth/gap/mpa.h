 ...
 * mpa.h
 *
 * \brief    Unregister a callback function to handle L2CAP related messages for a specific PSM.
 *
 * \param[in]  index          Protocol index assigned by MPA.
 *
 * \return    The result of callback function unregister.
 * \retval    true     The function was successfully unregistered.
 * \retval    false    The function was failed to unregister.
 *
 * <b>Example usage</b>
   \code{.c}
   ...
   int test(void)
   {
       mpa_l2c_proto_unregister(index);
 ...
 * \ingroup  MPA
 */
bool mpa_l2c_proto_unregister(uint8_t index);
