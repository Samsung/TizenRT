...
void general_client_discover_res_hdl(void *data)
{
	...
	if (RTK_BT_STATUS_CONTINUE == disc_status) {
		...
		case RTK_BT_GATT_DISCOVER_PRIMARY_BY_UUID:
			debug_print("[APP] GATTC discover primary by uuid result: start_handle: 0x%04x, end_handle: 0x%04x\r\n",
		...
		case RTK_BT_GATT_DISCOVER_INCLUDE:
			debug_print("[APP] GATTC discover include result: handle: 0x%04x, start_handle: 0x%04x, end_handle: 0x%04x, UUID: ",
			...
			if (BT_UUID_TYPE_16 == disc_res->disc_inc_per.uuid_type) {
				...
				debug_print(UUID128_STR, UUID128_VAL(disc_res->disc_inc_per.uuid));
				debug_print("\r\n");
			}
			break;
		...
		case RTK_BT_GATT_DISCOVER_CHARACTERISTIC_BY_UUID:
			...
			if (BT_UUID_TYPE_16 == disc_res->disc_char_all_per.uuid_type) {
				...
				debug_print(UUID128_STR, UUID128_VAL(disc_res->disc_char_all_per.uuid));
				debug_print("\r\n");
			}
			break;

		case RTK_BT_GATT_DISCOVER_DESCRIPTORS_ALL:
			debug_print("[APP] GATTC discover discriptor all result: handle: 0x%04x, UUID: ",
			...
			if (BT_UUID_TYPE_16 == disc_res->disc_descriptor_per.uuid_type) {
				...
				debug_print(UUID128_STR, UUID128_VAL(disc_res->disc_descriptor_per.uuid));
				debug_print("\r\n");
			}
			break;

		default:
			debug_print("[APP] GATTC discover result event, wrong event type (%d)!\r\n", disc_res->type);
			break;
		}
	} else if (RTK_BT_STATUS_DONE == disc_status) {
		debug_print("[APP] GATTC discover completed. "
					"profile_id: %d, conn_handle: %d, type: %d\r\n",
					disc_res->profile_id, disc_res->conn_handle, disc_res->type);
		APP_PRINT_SEPARATOR();
		return;
	} else {
		dbg("[APP] GATTC discover failed! "
					"profile_id: %d, conn_handle: %d, type: %d\r\n",
					disc_res->profile_id, disc_res->conn_handle, disc_res->type);
		APP_PRINT_SEPARATOR();
		return;
...
void general_client_read_res_hdl(void *data)
{
	...
	if (RTK_BT_STATUS_FAIL == read_status) {
		...
		case RTK_BT_GATT_CHAR_READ_BY_HANDLE:
			...
			dbg("[APP] GATT client read result, profile_id: %d, conn_handle: %d, "
...
//			handle = read_res->multiple_variable_per.handle;
	...
	debug_print("[APP] GATTC read completed, "
				"profile_id: %d, conn_handle: %d, type: %d\r\n",
...
//	rtk_bt_gattc_cfm_param_t cfm_param = {0};
	...
	dbg("[APP] GATTC indicate received, profile_id: %d, conn_handle: %d, handle: 0x%x\r\n",
