...
uint16_t rtk_bt_device_copy_act_ret_param(uint16_t act, void *dst, void *src)
{
	if (act == RTK_BT_DEVICE_IPC_ACT_BT_ENABLE) {
		...
		if (&le_audio_app_conf->pacs_param) {
			...
			if (le_audio_app_conf->pacs_param.p_pac_sink_codec) {
			...
			if (le_audio_app_conf->pacs_param.p_pac_source_codec) {
				*le_audio_app_conf->pacs_param.p_pac_source_codec = *((int8_t *)src + 3);
			}
		}
		if (&le_audio_app_conf->mcs_param) {
			if (le_audio_app_conf->mcs_param.p_mcs_id) {
...
static int le_ipc_audio_pacs_get_pac_record_push(uint8_t *buf, rtk_bt_le_audio_pacs_get_pac_record_param_t *param,
		uint32_t size_limit, uint32_t *actual_size)
{
	uint8_t *end = buf;
	uint32_t size = 0;
...
#endif
#if (defined RTK_BLE_AUDIO_UNICAST_CLIENT_SUPPORT) && (RTK_BLE_AUDIO_UNICAST_CLIENT_SUPPORT)
static int le_ipc_audio_unicast_config_push(uint8_t *buf, rtk_bt_le_audio_unicast_config_param_t *param,
		uint32_t size_limit, uint32_t *actual_size)
{
	uint8_t *end = buf;
	uint32_t size = 0;
...
exit:
	printf("le_ipc_audio_unicast_config_push size is over deep copy buf\r\n");
	return -1;
}
static int le_ipc_audio_unicast_get_cfg_mask_push(uint8_t *buf, rtk_bt_le_audio_unicast_get_cfg_mask_param_t *param,
		uint32_t size_limit, uint32_t *actual_size)
{
	uint8_t *end = buf;
	uint32_t size = 0;
...
exit:
	printf("le_ipc_audio_unicast_get_cfg_mask_push size is over deep copy buf\r\n");
	return -1;
}
#if (defined RTK_BLE_AUDIO_CSIP_SET_COORDINATOR_SUPPORT) && (RTK_BLE_AUDIO_CSIP_SET_COORDINATOR_SUPPORT)
static int le_ipc_audio_csis_set_coordinator_check_adv_rsi_push(uint8_t *buf, rtk_bt_le_audio_csis_set_coordinator_check_adv_rsi_info_t *param,
		uint32_t size_limit, uint32_t *actual_size)
{
	uint8_t *end = buf;
	uint32_t size = 0;
...
exit:
	printf("le_ipc_audio_csis_set_coordinator_check_adv_rsi_push size is over deep copy buf\r\n");
	return -1;
}
#endif
...
#if 0
static int le_ipc_audio_unicast_cfg_ase_metadata_push(uint8_t *buf, rtk_bt_le_audio_unicast_cfg_ase_metadata_param_t *param,
		uint32_t size_limit, uint32_t *actual_size)
{
	uint8_t *end = buf;
	uint32_t size = 0;
...
void *bt_le_audio_push_cmd_ipc_buf(uint16_t act, void *data, uint32_t buf_len, uint32_t *pactual_size)
{
	...
	case RTK_BT_LE_AUDIO_ACT_PACS_GET_PAC_RECORD:
		if (le_ipc_audio_pacs_get_pac_record_push((uint8_t *)host_msg->param_buf, (rtk_bt_le_audio_pacs_get_pac_record_param_t *)data,
				IPC_HOST_API_DATA_MAX, pactual_size)) {
			osif_mem_free(host_msg);
			host_msg = NULL;
		}
	...
	case RTK_BT_LE_AUDIO_ACT_UNICAST_CONFIG:
		if (le_ipc_audio_unicast_config_push((uint8_t *)host_msg->param_buf, (rtk_bt_le_audio_unicast_config_param_t *)data,
											 IPC_HOST_API_DATA_MAX, pactual_size)) {
			osif_mem_free(host_msg);
			host_msg = NULL;
		}
	...
	case RTK_BT_LE_AUDIO_ACT_UNICAST_GET_CFG_MASK:
		if (le_ipc_audio_unicast_get_cfg_mask_push((uint8_t *)host_msg->param_buf, (rtk_bt_le_audio_unicast_get_cfg_mask_param_t *)data,
				IPC_HOST_API_DATA_MAX, pactual_size)) {
			osif_mem_free(host_msg);
			host_msg = NULL;
		}
	...
	case RTK_BT_LE_AUDIO_ACT_UNICAST_CFG_ASE_METADATA:
		if (le_ipc_audio_unicast_cfg_ase_metadata_push((uint8_t *)host_msg->param_buf, (rtk_bt_le_audio_unicast_cfg_ase_metadata_param_t *)data,
				IPC_HOST_API_DATA_MAX, pactual_size)) {
			osif_mem_free(host_msg);
			host_msg = NULL;
		}
	...
	case RTK_BT_LE_AUDIO_ACT_CSIS_SET_COORDINATOR_CHECK_ADV_RSI:
		if (le_ipc_audio_csis_set_coordinator_check_adv_rsi_push((uint8_t *)host_msg->param_buf, (rtk_bt_le_audio_csis_set_coordinator_check_adv_rsi_info_t *)data,
				IPC_HOST_API_DATA_MAX, pactual_size)) {
			osif_mem_free(host_msg);
			host_msg = NULL;
		}
...
void bt_le_audio_push_event_ipc_ret(uint16_t event, void *dst, void *src)
{
	...
	case RTK_BT_LE_AUDIO_EVT_BASS_GET_PA_SYNC_PARAM_IND: {
		...
		memcpy((void *)ret, (void *)&param->pa_sync_param, sizeof(rtk_bt_le_audio_bass_pa_sync_param_t));
		break;
	}
	case RTK_BT_LE_AUDIO_EVT_BASS_GET_BIG_SYNC_PARAM_IND: {
		...
		memcpy((void *)ret, (void *)&param->big_sync_param, sizeof(rtk_bt_le_audio_bass_big_sync_param_t));
		break;
	}
	case RTK_BT_LE_AUDIO_EVT_BASS_GET_BROADCAST_CODE_IND: {
		...
		memcpy((void *)ret, (void *)param->broadcast_code, RTK_BT_LE_AUDIO_BROADCAST_CODE_LEN);
		break;
	}
	case RTK_BT_LE_AUDIO_EVT_BASS_GET_PREFER_BIS_SYNC_IND: {
		...
		memcpy((void *)ret, (void *)&param->bis_sync, sizeof(uint32_t));
		break;
	}
#endif
#if (defined RTK_BLE_AUDIO_UNICAST_CLIENT_SUPPORT) && (RTK_BLE_AUDIO_UNICAST_CLIENT_SUPPORT)
	case RTK_BT_LE_AUDIO_EVT_BAP_START_QOS_CFG_IND: {
		...
		memcpy((void *)ret, (void *)&param->session_qos, sizeof(rtk_bt_le_audio_unicast_session_qos_t));
		break;
	}
	case RTK_BT_LE_AUDIO_EVT_BAP_START_METADATA_CFG_IND: {
		rtk_bt_le_audio_bap_start_metadata_cfg_ind_t *param = (rtk_bt_le_audio_bap_start_metadata_cfg_ind_t *)src;
		memcpy((void *)ret, (void *)&param->metadata_len, sizeof(uint8_t));
		memcpy((void *)(ret + 1), (void *)param->p_metadata, param->metadata_len);
		break;
	}
#endif
	default:
...
static struct evt_mem_option rtk_bt_mesh_stack_evt_mem_op_tab[] = {
	{RTK_BT_MESH_STACK_EVT_DEVICE_INFO_UDB_DISPLAY, sizeof(rtk_bt_mesh_stack_evt_dev_info_udb_t)},
	{RTK_BT_MESH_STACK_EVT_DEVICE_INFO_PROV_DISPLAY, sizeof(rtk_bt_mesh_stack_evt_dev_info_provision_adv_t)},
	{RTK_BT_MESH_STACK_EVT_DEVICE_INFO_PROXY_DISPLAY, sizeof(rtk_bt_mesh_stack_evt_dev_info_proxy_adv_t)},
	{RTK_BT_MESH_STACK_EVT_PB_ADV_LINK_STATE, sizeof(rtk_bt_mesh_prov_generic_cb_type_t)},
	{RTK_BT_MESH_STACK_EVT_PROV_COMPLETE, sizeof(rtk_bt_mesh_stack_evt_prov_complete_t)},
	{RTK_BT_MESH_STACK_EVT_UNPROVISIONED_DEVICE, 0},
	{RTK_BT_MESH_STACK_EVT_PROVISIONED_DEVICE, sizeof(rtk_bt_mesh_stack_evt_provisioned_device_t)},
	{RTK_BT_MESH_STACK_EVT_DEVICE_START_PROVED, 0},
	{RTK_BT_MESH_STACK_EVT_LIST_INFO, 0},
	{RTK_BT_MESH_STACK_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_config_client_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_CONFIG_MODEL_APP_KEY_STAT, sizeof(rtk_bt_mesh_cfg_app_key_stat_t)},
	{RTK_BT_MESH_CONFIG_MODEL_MODEL_APP_STAT, sizeof(rtk_bt_mesh_cfg_model_app_stat_t)},
	{RTK_BT_MESH_CONFIG_MODEL_MODEL_SUB_STAT, sizeof(rtk_bt_mesh_cfg_model_sub_stat_t)},
	{RTK_BT_MESH_CONFIG_MODEL_DEFAULT_TTL_STAT, sizeof(rtk_bt_mesh_cfg_default_ttl_stat_t)},
	{RTK_BT_MESH_CONFIG_MODEL_RELAY_STAT, sizeof(rtk_bt_mesh_cfg_relay_stat_t)},
	{RTK_BT_MESH_CONFIG_MODEL_NET_TRANSMIT_STAT, sizeof(rtk_bt_mesh_cfg_net_transmit_stat_t)},
	{RTK_BT_MESH_CONFIG_MODEL_NODE_RESET_STAT, sizeof(rtk_bt_mesh_config_client_model_node_reset_stat_t)},
	{RTK_BT_MESH_CONFIG_MODEL_BEACON_STAT, sizeof(rtk_bt_mesh_cfg_beacon_stat_t)},
	{RTK_BT_MESH_CONFIG_MODEL_PROXY_STAT, sizeof(rtk_bt_mesh_cfg_proxy_stat_t)},
	{RTK_BT_MESH_CONFIG_MODEL_MODEL_PUB_STAT, sizeof(rtk_bt_mesh_cfg_model_pub_stat_t)},
	{RTK_BT_MESH_CONFIG_MODEL_NET_KEY_STAT, sizeof(rtk_bt_mesh_cfg_net_key_stat_t)},
	{RTK_BT_MESH_CONFIG_MODEL_NODE_IDENTITY_STAT, sizeof(rtk_bt_mesh_cfg_node_identity_stat_t)},
	{RTK_BT_MESH_CONFIG_MODEL_FRND_STAT, sizeof(rtk_bt_mesh_cfg_frnd_stat_t)},
	{RTK_BT_MESH_CONFIG_MODEL_LPN_POLL_TO_STAT, sizeof(rtk_bt_mesh_cfg_lpn_poll_timeout_stat_t)},
	{RTK_BT_MESH_CONFIG_MODEL_KEY_REFRESH_PHASE_STAT, sizeof(rtk_bt_mesh_cfg_key_refresh_phase_stat_call_t)},
	{RTK_BT_MESH_CONFIG_MODEL_HB_PUB_STAT, sizeof(rtk_bt_mesh_cfg_hb_pub_stat_t)},
	{RTK_BT_MESH_CONFIG_MODEL_HB_SUB_STAT, sizeof(rtk_bt_mesh_cfg_hb_sub_stat_t)},
	{RTK_BT_MESH_CONFIG_MODEL_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_generic_onoff_client_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_GENERIC_ONOFF_CLIENT_MODEL_EVT_STATUS, sizeof(rtk_bt_mesh_generic_onoff_client_evt_status_t)},
	{RTK_BT_MESH_GENERIC_ONOFF_CLIENT_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_generic_onoff_server_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_GENERIC_ONOFF_SERVER_MODEL_EVT_SET, sizeof(rtk_bt_mesh_generic_onoff_server_evt_set_t)},
	{RTK_BT_MESH_GENERIC_ONOFF_SERVER_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_datatrans_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_DATATRANS_MODEL_EVT_SERVER_WRITE, sizeof(rtk_bt_mesh_datatrans_server_write_event_t)},
	{RTK_BT_MESH_DATATRANS_MODEL_EVT_SERVER_READ, sizeof(rtk_bt_mesh_datatrans_server_read_event_t)},
	{RTK_BT_MESH_DATATRANS_MODEL_EVT_CLIENT_WRITE_STATUS, sizeof(rtk_bt_mesh_datatrans_client_write_event_t)},
	{RTK_BT_MESH_DATATRANS_MODEL_EVT_CLIENT_READ_RESULT, sizeof(rtk_bt_mesh_datatrans_client_read_event_t)},
	{RTK_BT_MESH_DATATRANS_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_light_lightness_client_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_LIGHT_LIGHTNESS_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_lightness_client_status_t)},
	{RTK_BT_MESH_LIGHT_LIGHTNESS_LINEAR_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_lightness_client_status_t)},
	{RTK_BT_MESH_LIGHT_LIGHTNESS_LAST_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_lightness_client_status_last_t)},
	{RTK_BT_MESH_LIGHT_LIGHTNESS_DEFAULT_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_lightness_client_status_default_t)},
	{RTK_BT_MESH_LIGHT_LIGHTNESS_RANGE_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_lightness_client_status_range_t)},
	{RTK_BT_MESH_LIGHT_LIGHTNESS_CLIENT_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_light_lightness_server_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_LIGHT_LIGHTNESS_SERVER_MODEL_GET, sizeof(rtk_bt_mesh_light_lightness_server_get_t)},
	{RTK_BT_MESH_LIGHT_LIGHTNESS_LINEAR_SERVER_MODEL_GET, sizeof(rtk_bt_mesh_light_lightness_server_get_t)},
	{RTK_BT_MESH_LIGHT_LIGHTNESS_DEFAULT_SERVER_MODEL_GET, sizeof(rtk_bt_mesh_light_lightness_server_get_t)},
	{RTK_BT_MESH_LIGHT_LIGHTNESS_LAST_SERVER_MODEL_GET, sizeof(rtk_bt_mesh_light_lightness_server_get_t)},
	{RTK_BT_MESH_LIGHT_LIGHTNESS_RANGE_SERVER_MODEL_GET, sizeof(rtk_bt_mesh_light_lightness_server_get_range_t)},
	{RTK_BT_MESH_LIGHT_LIGHTNESS_SERVER_MODEL_SET, sizeof(rtk_bt_mesh_light_lightness_server_set_t)},
	{RTK_BT_MESH_LIGHT_LIGHTNESS_LINEAR_SERVER_MODEL_SET, sizeof(rtk_bt_mesh_light_lightness_server_set_t)},
	{RTK_BT_MESH_LIGHT_LIGHTNESS_LAST_SERVER_MODEL_SET, sizeof(rtk_bt_mesh_light_lightness_server_set_last_t)},
	{RTK_BT_MESH_LIGHT_LIGHTNESS_SERVER_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_light_lightness_setup_server_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_LIGHT_LIGHTNESS_DEFAULT_SERVER_MODEL_SET, sizeof(rtk_bt_mesh_light_lightness_server_set_default_t)},
	{RTK_BT_MESH_LIGHT_LIGHTNESS_RANGE_SERVER_MODEL_SET, sizeof(rtk_bt_mesh_light_lightness_server_set_range_t)},
	{RTK_BT_MESH_LIGHT_LIGHTNESS_SETUP_SERVER_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_light_ctl_client_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_LIGHT_CTL_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_ctl_client_status_t)},
	{RTK_BT_MESH_LIGHT_CTL_CLIENT_MODEL_TEMPERATURE_STATUS, sizeof(rtk_bt_mesh_light_ctl_client_status_temperature_t)},
	{RTK_BT_MESH_LIGHT_CTL_CLIENT_MODEL_TEMPERATURE_RANGE_STATUS, sizeof(rtk_bt_mesh_light_ctl_client_status_temperature_range_t)},
	{RTK_BT_MESH_LIGHT_CTL_CLIENT_MODEL_DEFAULT_STATUS, sizeof(rtk_bt_mesh_light_ctl_client_status_default_t)},
	{RTK_BT_MESH_LIGHT_CTL_CLIENT_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_light_ctl_server_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_LIGHT_CTL_SERVER_MODEL_GET, sizeof(rtk_bt_mesh_light_ctl_server_get_t)},
	{RTK_BT_MESH_LIGHT_CTL_SERVER_MODEL_DEFAULT_GET, sizeof(rtk_bt_mesh_light_ctl_server_get_default_t)},
	{RTK_BT_MESH_LIGHT_CTL_SERVER_MODEL_TEMPERATURE_RANGE_GET, sizeof(rtk_bt_mesh_light_ctl_server_get_temperature_range_t)},
	{RTK_BT_MESH_LIGHT_CTL_SERVER_MODEL_SET, sizeof(rtk_bt_mesh_light_ctl_server_set_t)},
	{RTK_BT_MESH_LIGHT_CTL_SERVER_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_light_ctl_setup_server_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_LIGHT_CTL_SERVER_MODEL_TEMPERATURE_RANGE_SET, sizeof(rtk_bt_mesh_light_ctl_server_set_temperature_range_t)},
	{RTK_BT_MESH_LIGHT_CTL_SERVER_MODEL_DEFAULT_SET, sizeof(rtk_bt_mesh_light_ctl_server_set_default_t)},
	{RTK_BT_MESH_LIGHT_CTL_SETUP_SERVER_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_light_ctl_temperature_server_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_LIGHT_CTL_SERVER_MODEL_TEMPERATURE_GET, sizeof(rtk_bt_mesh_light_ctl_server_get_temperature_t)},
	{RTK_BT_MESH_LIGHT_CTL_SERVER_MODEL_TEMPERATURE_SET, sizeof(rtk_bt_mesh_light_ctl_server_set_temperature_t)},
	{RTK_BT_MESH_LIGHT_CTL_SERVER_MODEL_TEMPERATURE_RANGE_GET_T, sizeof(rtk_bt_mesh_light_ctl_server_get_temperature_range_t)},
	{RTK_BT_MESH_LIGHT_CTL_SERVER_TEMPERATURE_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_light_hsl_client_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_LIGHT_HSL_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_hsl_client_status_t)},
	{RTK_BT_MESH_LIGHT_HSL_TARGET_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_hsl_client_status_t)},
	{RTK_BT_MESH_LIGHT_HSL_HUE_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_hsl_client_status_hue_t)},
	{RTK_BT_MESH_LIGHT_HSL_SATURATION_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_hsl_client_status_saturation_t)},
	{RTK_BT_MESH_LIGHT_HSL_DEFAULT_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_hsl_client_status_default_t)},
	{RTK_BT_MESH_LIGHT_HSL_RANGE_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_hsl_client_status_range_t)},
	{RTK_BT_MESH_LIGHT_HSL_CLIENT_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_light_hsl_server_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_LIGHT_HSL_SERVER_MODEL_GET, sizeof(rtk_bt_mesh_light_hsl_server_get_t)},
	{RTK_BT_MESH_LIGHT_HSL_DEFAULT_SERVER_MODEL_GET, sizeof(rtk_bt_mesh_light_hsl_server_get_default_t)},
	{RTK_BT_MESH_LIGHT_HSL_RANGE_SERVER_MODEL_GET, sizeof(rtk_bt_mesh_light_hsl_server_get_range_t)},
	{RTK_BT_MESH_LIGHT_HSL_SERVER_MODEL_SET, sizeof(rtk_bt_mesh_light_hsl_server_set_t)},
	{RTK_BT_MESH_LIGHT_HSL_SERVER_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_light_hsl_hue_server_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_LIGHT_HSL_HUE_SERVER_MODEL_GET, sizeof(rtk_bt_mesh_light_hsl_server_get_hue_t)},
	{RTK_BT_MESH_LIGHT_HSL_HUE_SERVER_MODEL_SET, sizeof(rtk_bt_mesh_light_hsl_server_set_hue_t)},
	{RTK_BT_MESH_LIGHT_HSL_RANGE_SERVER_MODEL_GET_H, sizeof(rtk_bt_mesh_light_hsl_server_get_range_t)},
	{RTK_BT_MESH_LIGHT_HSL_HUE_SERVER_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_light_hsl_saturation_server_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_LIGHT_HSL_SATURATION_SERVER_MODEL_GET, sizeof(rtk_bt_mesh_light_hsl_server_get_saturation_t)},
	{RTK_BT_MESH_LIGHT_HSL_SATURATION_SERVER_MODEL_SET, sizeof(rtk_bt_mesh_light_hsl_server_set_saturation_t)},
	{RTK_BT_MESH_LIGHT_HSL_RANGE_SERVER_MODEL_GET_S, sizeof(rtk_bt_mesh_light_hsl_server_get_range_t)},
	{RTK_BT_MESH_LIGHT_HSL_SATURATION_SERVER_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_light_hsl_setup_server_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_LIGHT_HSL_DEFAULT_SERVER_MODEL_SET, sizeof(rtk_bt_mesh_light_hsl_server_set_default_t)},
	{RTK_BT_MESH_LIGHT_HSL_RANGE_SERVER_MODEL_SET, sizeof(rtk_bt_mesh_light_hsl_server_set_range_t)},
	{RTK_BT_MESH_LIGHT_HSL_SETUP_SERVER_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_light_xyl_client_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_LIGHT_XYL_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_xyl_client_status_t)},
	{RTK_BT_MESH_LIGHT_XYL_TARGET_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_xyl_client_status_t)},
	{RTK_BT_MESH_LIGHT_XYL_DEFAULT_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_xyl_client_status_default_t)},
	{RTK_BT_MESH_LIGHT_XYL_RANGE_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_xyl_client_status_range_t)},
	{RTK_BT_MESH_LIGHT_XYL_CLIENT_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_light_xyl_server_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_LIGHT_XYL_SERVER_MODEL_GET, sizeof(rtk_bt_mesh_light_xyl_server_get_t)},
	{RTK_BT_MESH_LIGHT_XYL_SERVER_MODEL_SET, sizeof(rtk_bt_mesh_light_xyl_server_set_t)},
	{RTK_BT_MESH_LIGHT_XYL_DEFAULT_SERVER_MODEL_GET, sizeof(rtk_bt_mesh_light_xyl_server_get_default_t)},
	{RTK_BT_MESH_LIGHT_XYL_RANGE_SERVER_MODEL_GET, sizeof(rtk_bt_mesh_light_xyl_server_get_range_t)},
	{RTK_BT_MESH_LIGHT_XYL_SERVER_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_light_xyl_setup_server_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_LIGHT_XYL_DEFAULT_SERVER_MODEL_SET, sizeof(rtk_bt_mesh_light_xyl_server_set_default_t)},
	{RTK_BT_MESH_LIGHT_XYL_RANGE_SERVER_MODEL_SET, sizeof(rtk_bt_mesh_light_xyl_server_set_range_t)},
	{RTK_BT_MESH_LIGHT_XYL_SETUP_SERVER_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_light_lc_client_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_LIGHT_LC_MODE_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_lc_client_mode_status_t)},
	{RTK_BT_MESH_LIGHT_LC_OM_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_lc_client_om_status_t)},
	{RTK_BT_MESH_LIGHT_LC_LIGHTONOFF_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_lc_client_light_on_off_status_t)},
	{RTK_BT_MESH_LIGHT_LC_PROPERTY_CLIENT_MODEL_STATUS, sizeof(rtk_bt_mesh_light_lc_client_property_status_t)},
	{RTK_BT_MESH_LIGHT_LC_CLIENT_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_light_lc_server_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_LIGHT_LC_SERVER_MODEL_MODE_GET, sizeof(rtk_bt_mesh_light_lc_server_get_mode_t)},
	{RTK_BT_MESH_LIGHT_LC_SERVER_MODEL_MODE_SET, sizeof(rtk_bt_mesh_light_lc_server_set_mode_t)},
	{RTK_BT_MESH_LIGHT_LC_SERVER_MODEL_OM_GET, sizeof(rtk_bt_mesh_light_lc_server_get_om_t)},
	{RTK_BT_MESH_LIGHT_LC_SERVER_MODEL_OM_SET, sizeof(rtk_bt_mesh_light_lc_server_set_om_t)},
	{RTK_BT_MESH_LIGHT_LC_SERVER_MODEL_LIGHT_ON_OFF_GET, sizeof(rtk_bt_mesh_light_lc_server_get_light_on_off_t)},
	{RTK_BT_MESH_LIGHT_LC_SERVER_MODEL_LIGHT_ON_OFF_SET, sizeof(rtk_bt_mesh_light_lc_server_set_light_on_off_t)},
	{RTK_BT_MESH_LIGHT_LC_SERVER_MODEL_SM_TRANSITION_TIME_GET, sizeof(rtk_bt_mesh_light_lc_server_get_sm_transition_time_t)},
	{RTK_BT_MESH_LIGHT_LC_SERVER_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

static struct evt_mem_option rtk_bt_mesh_light_lc_setup_server_model_evt_mem_op_tab[] = {
	{RTK_BT_MESH_LIGHT_LC_SERVER_MODEL_PROPERTY_GET, sizeof(rtk_bt_mesh_light_lc_server_get_property_t)},
	{RTK_BT_MESH_LIGHT_LC_SERVER_MODEL_PROPERTY_SET, sizeof(rtk_bt_mesh_light_lc_server_set_property_t)},
	{RTK_BT_MESH_LIGHT_LC_SETUP_SERVER_MODEL_EVT_MAX, RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE}
};

uint32_t get_bt_mesh_evt_mem_size(uint32_t group, uint8_t evt_code)
{
	uint8_t i = 0;
	struct evt_mem_option *evt_op_tab = NULL;
	switch (group) {
	case RTK_BT_LE_GP_MESH_STACK:
		evt_op_tab = rtk_bt_mesh_stack_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_CONFIG_CLIENT_MODEL:
		evt_op_tab = rtk_bt_mesh_config_client_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_GENERIC_ONOFF_CLIENT_MODEL:
		evt_op_tab = rtk_bt_mesh_generic_onoff_client_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_GENERIC_ONOFF_SERVER_MODEL:
		evt_op_tab = rtk_bt_mesh_generic_onoff_server_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_DATATRANS_MODEL:
		evt_op_tab = rtk_bt_mesh_datatrans_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_LIGHT_LIGHTNESS_CLIENT_MODEL:
		evt_op_tab = rtk_bt_mesh_light_lightness_client_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_LIGHT_LIGHTNESS_SERVER_MODEL:
		evt_op_tab = rtk_bt_mesh_light_lightness_server_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_LIGHT_LIGHTNESS_SETUP_SERVER_MODEL:
		evt_op_tab = rtk_bt_mesh_light_lightness_setup_server_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_LIGHT_CTL_CLIENT_MODEL:
		evt_op_tab = rtk_bt_mesh_light_ctl_client_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_LIGHT_CTL_SERVER_MODEL:
		evt_op_tab = rtk_bt_mesh_light_ctl_server_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_LIGHT_CTL_SETUP_SERVER_MODEL:
		evt_op_tab = rtk_bt_mesh_light_ctl_setup_server_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_LIGHT_CTL_TEMPERATURE_SERVER_MODEL:
		evt_op_tab = rtk_bt_mesh_light_ctl_temperature_server_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_LIGHT_HSL_CLIENT_MODEL:
		evt_op_tab = rtk_bt_mesh_light_hsl_client_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_LIGHT_HSL_SERVER_MODEL:
		evt_op_tab = rtk_bt_mesh_light_hsl_server_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_LIGHT_HSL_HUE_SERVER_MODEL:
		evt_op_tab = rtk_bt_mesh_light_hsl_hue_server_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_LIGHT_HSL_SATURATION_SERVER_MODEL:
		evt_op_tab = rtk_bt_mesh_light_hsl_saturation_server_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_LIGHT_HSL_SETUP_SERVER_MODEL:
		evt_op_tab = rtk_bt_mesh_light_hsl_setup_server_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_LIGHT_XYL_CLIENT_MODEL:
		evt_op_tab = rtk_bt_mesh_light_xyl_client_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_LIGHT_XYL_SERVER_MODEL:
		evt_op_tab = rtk_bt_mesh_light_xyl_server_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_LIGHT_XYL_SETUP_SERVER_MODEL:
		evt_op_tab = rtk_bt_mesh_light_xyl_setup_server_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_LIGHT_LC_CLIENT_MODEL:
		evt_op_tab = rtk_bt_mesh_light_lc_client_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_LIGHT_LC_SERVER_MODEL:
		evt_op_tab = rtk_bt_mesh_light_lc_server_model_evt_mem_op_tab;
		break;
	case RTK_BT_LE_GP_MESH_LIGHT_LC_SETUP_SERVER_MODEL:
		evt_op_tab = rtk_bt_mesh_light_lc_setup_server_model_evt_mem_op_tab;
		break;
	default:
		printf("[%s] Unknown group:%d\r\n", __func__, (int)group);
		break;
	...
	if (evt_op_tab) {
		while (1) {
			if (evt_op_tab[i].evt == evt_code) {
				return evt_op_tab[i].size;
			}
			if (evt_op_tab[i].size == RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE) {
				printf("[%s]: Cannot find matched group:%d, event %d \r\n", __func__, (int)group, evt_code);
				return RTK_BT_MESH_EVT_MEM_OP_MAX_SIZE;
			}
			i++;
		}
	}
...
void *bt_mesh_push_event_ipc_buf(uint32_t group, uint16_t event, void *data, uint32_t buf_len, uint32_t *pactual_size)
{
	...
	uint8_t *start = NULL, *end = NULL, *border = NULL;
	bt_ipc_dev_request_message *dev_msg = osif_mem_alloc(RAM_TYPE_DATA_ON, sizeof(*dev_msg));
	...
	case RTK_BT_LE_GP_MESH_STACK: {
		switch (event) {
		// For the event with unknown data struct,get the actual len from first 4 bytes for data memcpy
		case RTK_BT_MESH_STACK_EVT_LIST_INFO:
			buf_len = *(uint32_t *)data;
			break;
		default:
			break;
		}
...
// 	{RTK_BT_LE_AUDIO_EVT_BAP_DISCOVERY_ALL_DONE_IND, sizeof(rtk_bt_le_audio_bap_discovery_all_done_ind_t)},
// #if (defined RTK_BLE_AUDIO_UNICAST_CLIENT_SUPPORT) && (RTK_BLE_AUDIO_UNICAST_CLIENT_SUPPORT)
...
void bt_le_audio_pop_event_ipc_ret(uint16_t event, void *dst, void *src)
{
	...
	case RTK_BT_LE_AUDIO_EVT_BASS_GET_PA_SYNC_PARAM_IND: {
		...
		memcpy((void *)&param->pa_sync_param, (void *)ret, rtk_le_audio_get_evt_ret_param_size(event));
		break;
	}
	case RTK_BT_LE_AUDIO_EVT_BASS_GET_BIG_SYNC_PARAM_IND: {
		rtk_bt_le_audio_bass_get_big_sync_param_ind_t *param = (rtk_bt_le_audio_bass_get_big_sync_param_ind_t *)dst;
		memcpy((void *)&param->big_sync_param, (void *)ret, rtk_le_audio_get_evt_ret_param_size(event));
		break;
	}
	case RTK_BT_LE_AUDIO_EVT_BASS_GET_BROADCAST_CODE_IND: {
		rtk_bt_le_audio_bass_get_broadcast_code_ind_t *param = (rtk_bt_le_audio_bass_get_broadcast_code_ind_t *)dst;
		memcpy((void *)param->broadcast_code, (void *)ret, rtk_le_audio_get_evt_ret_param_size(event));
		break;
	}
	case RTK_BT_LE_AUDIO_EVT_BASS_GET_PREFER_BIS_SYNC_IND: {
		rtk_bt_le_audio_bass_get_prefer_bis_sync_ind_t *param = (rtk_bt_le_audio_bass_get_prefer_bis_sync_ind_t *)dst;
		memcpy((void *)&param->bis_sync, (void *)ret, rtk_le_audio_get_evt_ret_param_size(event));
		break;
	}
#endif
#if (defined RTK_BLE_AUDIO_UNICAST_CLIENT_SUPPORT) && (RTK_BLE_AUDIO_UNICAST_CLIENT_SUPPORT)
	case RTK_BT_LE_AUDIO_EVT_BAP_START_QOS_CFG_IND: {
		...
		memcpy((void *)&param->session_qos, (void *)ret, rtk_le_audio_get_evt_ret_param_size(event));
		break;
	}
	case RTK_BT_LE_AUDIO_EVT_BAP_START_METADATA_CFG_IND: {
		rtk_bt_le_audio_bap_start_metadata_cfg_ind_t *param = (rtk_bt_le_audio_bap_start_metadata_cfg_ind_t *)dst;
		memcpy((void *)&param->metadata_len, (void *)ret, sizeof(uint8_t));
		memcpy((void *)param->p_metadata, (void *)(ret + 1), param->metadata_len);
		break;
	}
#endif
	default:
...
exit:
	printf("le_audio_ipc_bap_start_qos_cfg_ind_evt_push size is over deep copy buf\r\n");
	return -1;
}
#endif
...
static void le_ipc_audio_pacs_get_pac_record_pop(rtk_bt_le_audio_pacs_get_pac_record_param_t *param)
{
...
#endif
#if (defined RTK_BLE_AUDIO_UNICAST_CLIENT_SUPPORT) && (RTK_BLE_AUDIO_UNICAST_CLIENT_SUPPORT)
static void le_ipc_audio_unicast_config_pop(rtk_bt_le_audio_unicast_config_param_t *param)
{
	param->p_device_handle_tbl = (rtk_bt_le_audio_device_handle_t *)((uint8_t *)param + (uint32_t)param->p_device_handle_tbl);
}
static void le_ipc_audio_unicast_get_cfg_mask_pop(rtk_bt_le_audio_unicast_get_cfg_mask_param_t *param)
{
	param->p_device_handle_tbl = (rtk_bt_le_audio_device_handle_t *)((uint8_t *)param + (uint32_t)param->p_device_handle_tbl);
}
#if (defined RTK_BLE_AUDIO_CSIP_SET_COORDINATOR_SUPPORT) && (RTK_BLE_AUDIO_CSIP_SET_COORDINATOR_SUPPORT)
static void le_ipc_audio_csis_set_coordinator_check_adv_rsi_pop(rtk_bt_le_audio_csis_set_coordinator_check_adv_rsi_info_t *param)
{
...
#if 0
static void le_ipc_audio_unicast_cfg_ase_metadata_pop(rtk_bt_le_audio_unicast_cfg_ase_metadata_param_t *param)
{
	param->p_metadata = (uint8_t *)((uint8_t *)param + (uint32_t)param->p_metadata);
}
#endif
...
void bt_le_audio_ipc_pop_cmd_param(uint8_t act, void *param)
{
	...
	case RTK_BT_LE_AUDIO_ACT_ISO_DATA_SEND:
		...
		break;
	case RTK_BT_LE_AUDIO_ACT_PACS_GET_PAC_RECORD:
		le_ipc_audio_pacs_get_pac_record_pop((rtk_bt_le_audio_pacs_get_pac_record_param_t *)param);
		break;
#if (defined RTK_BLE_AUDIO_BROADCAST_SINK_SUPPORT) && (RTK_BLE_AUDIO_BROADCAST_SINK_SUPPORT)
	case RTK_BT_LE_AUDIO_ACT_SYNC_SETUP_DATA_PATH:
...
#if (defined RTK_BLE_AUDIO_UNICAST_CLIENT_SUPPORT) && (RTK_BLE_AUDIO_UNICAST_CLIENT_SUPPORT)
	case RTK_BT_LE_AUDIO_ACT_UNICAST_CONFIG:
		le_ipc_audio_unicast_config_pop((rtk_bt_le_audio_unicast_config_param_t *)param);
		break;
	case RTK_BT_LE_AUDIO_ACT_UNICAST_GET_CFG_MASK:
		le_ipc_audio_unicast_get_cfg_mask_pop((rtk_bt_le_audio_unicast_get_cfg_mask_param_t *)param);
		break;
#if 0
	case RTK_BT_LE_AUDIO_ACT_UNICAST_CFG_ASE_METADATA:
		le_ipc_audio_unicast_cfg_ase_metadata_pop((rtk_bt_le_audio_unicast_cfg_ase_metadata_param_t *)param);
		break;
#endif
#endif
#if (defined RTK_BLE_AUDIO_CSIP_SET_COORDINATOR_SUPPORT) && (RTK_BLE_AUDIO_CSIP_SET_COORDINATOR_SUPPORT)
	case RTK_BT_LE_AUDIO_ACT_CSIS_SET_COORDINATOR_CHECK_ADV_RSI:
		le_ipc_audio_csis_set_coordinator_check_adv_rsi_pop((rtk_bt_le_audio_csis_set_coordinator_check_adv_rsi_info_t *)param);
		break;
#endif
#if (defined RTK_BLE_AUDIO_MCP_MEDIA_CONTROL_CLIENT_SUPPORT) && (RTK_BLE_AUDIO_MCP_MEDIA_CONTROL_CLIENT_SUPPORT) ||\
     (defined RTK_BLE_AUDIO_MCP_MEDIA_CONTROL_SERVER_SUPPORT) && (RTK_BLE_AUDIO_MCP_MEDIA_CONTROL_SERVER_SUPPORT) 
