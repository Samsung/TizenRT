...
#if RTK_BLE_5_0_AE_ADV_SUPPORT && F_BT_LE_5_0_AE_ADV_SUPPORT
typedef struct {
	bool used;
	uint8_t adv_handle;
...
#endif
#if RTK_BLE_5_0_PA_ADV_SUPPORT && F_BT_LE_5_0_PA_ADV_SUPPORT
...
#if !RTK_BLE_MGR_LIB
extern uint16_t g_rtk_bt_stack_credits;
static uint16_t g_rtk_bt_stack_init_credits = 0;
#endif
...
static bt_stack_le_link_info_t bt_stack_le_link_tbl[GAP_MAX_LINKS] = {0};
static uint16_t bt_stack_le_conn_handle[GAP_MAX_LINKS] = {0};
...
#endif
#if RTK_BLE_5_0_PA_SYNC_SUPPORT && F_BT_LE_5_0_PA_SYNC_SUPPORT
static T_GAP_PA_SYNC_DEV_STATE le_gap_pa_sync_dev_state = {0};
#endif

#if RTK_BLE_PRIVACY_SUPPORT && F_BT_LE_PRIVACY_SUPPORT
typedef enum {
...
static void privacy_handle_resolv_list(void);
...
#if RTK_BLE_5_0_AE_ADV_SUPPORT && F_BT_LE_5_0_AE_ADV_SUPPORT && !RTK_BLE_MGR_LIB_EADV
static void bt_stack_le_gap_handle_ext_adv_state_evt(uint8_t adv_handle, T_GAP_EXT_ADV_STATE new_state, uint16_t cause)
{
	...
	if (new_state == EXT_ADV_STATE_IDLE) {
		p_cmd = bt_stack_gap_find_pending_cmd(GAP_MSG_LE_EXT_ADV_STATE_CHANGE_INFO);
		if (p_cmd && (adv_handle == *((uint8_t *)p_cmd->param))) {
			API_PRINT("app_handle_ext_adv_state_evt: async_cmd complete.\r\n");
			bt_stack_gap_delete_pending_cmd(p_cmd);
			p_cmd->ret = 0;
...
static T_APP_RESULT bt_stack_le_gap_callback(uint8_t type, void *data)
{
	...
	case GAP_MSG_LE_MODIFY_WHITE_LIST: {
		...
		p_cmd = bt_stack_gap_find_pending_cmd(type);
		if (p_cmd) {
...
#if RTK_BLE_5_0_AE_SCAN_SUPPORT && F_BT_LE_5_0_AE_SCAN_SUPPORT
...
#if RTK_BLE_5_0_AE_SCAN_SUPPORT && F_BT_LE_5_0_AE_SCAN_SUPPORT && F_BT_LE_GAP_MSG_INFO_WAY
...
#if RTK_BLE_5_1_PAST_RECIPIENT_SUPPORT && F_BT_LE_5_1_PAST_RECIPIENT_SUPPORT
	...
	case GAP_MSG_LE_PAST_RECIPIENT_PERIODIC_ADV_SYNC_TRANSFER_RECEIVED_INFO: {
		...
		p_evt = rtk_bt_event_create(RTK_BT_LE_GP_GAP,
									RTK_BT_LE_GAP_PAST_RECEIVED_INFO_IND,
...
#if RTK_BLE_5_1_PAST_SENDER_SUPPORT && F_BT_LE_5_1_PAST_SENDER_SUPPORT
	...
	case GAP_MSG_LE_EXT_ADV_START_SETTING: {
		...
		p_cmd = bt_stack_gap_find_pending_cmd(RTK_BT_LE_GAP_ACT_START_EXT_ADV << 8);
		if (p_cmd) {
			bt_stack_gap_delete_pending_cmd(p_cmd);
			if (p_data->p_le_ext_adv_start_setting_rsp->cause == GAP_SUCCESS) {
				p_cmd->user_data = GAP_MSG_LE_EXT_ADV_ENABLE;
				bt_stack_gap_insert_pending_cmd(p_cmd);
	...
	case GAP_MSG_LE_PA_ADV_START_SETTING: {
		...
		p_cmd = bt_stack_gap_find_pending_cmd(RTK_BT_LE_GAP_ACT_START_PA << 8);
		if (p_cmd) {
			bt_stack_gap_delete_pending_cmd(p_cmd);
		...
		p_cmd = bt_stack_gap_find_pending_cmd(RTK_BT_LE_GAP_ACT_UPDATE_PA << 8);
		if (p_cmd) {
			bt_stack_gap_delete_pending_cmd(p_cmd);
			p_cmd->ret = p_data->p_le_pa_adv_start_setting_rsp->cause;
			osif_sem_give(p_cmd->psem);
	...
	case GAP_MSG_LE_RF_INFO: {
...
#if RTK_BLE_5_2_POWER_CONTROL_SUPPORT && F_BT_LE_5_2_POWER_CONTROL_SUPPORT
		case GAP_LE_RF_ENHANCED_READ_TRANSMIT_POWER_LEVEL: {
			...
			p_cmd = bt_stack_gap_find_pending_cmd(p_le_rf_cb->opcode | (type << 8));
			if (p_cmd) {
...
void bt_stack_vendor_callback(uint8_t cb_type, void *p_cb_data)
{
	...
	case GAP_MSG_VENDOR_CMD_RSP:
		...
		case HCI_LE_VENDOR_READ_BT_REGISTER: {
			uint16_t bt_reg = 0;
			uint16_t buf = 0;
...
#if defined(RKT_BLE_MESH_SUPPORT) && RKT_BLE_MESH_SUPPORT
			if (rtk_bt_mesh_is_enable()) {
				// APP_PRINT("One shot adv resp: cause 0x%x", cb_data.p_gap_vendor_cmd_rsp->cause);
...
{
#if (RTK_BLE_5_0_AE_ADV_SUPPORT || RTK_BLE_5_0_AE_SCAN_SUPPORT) && (!RTK_BLE_MGR_LIB_EADV)
	{
...
{
	if (false == le_gap_init(GAP_MAX_LINKS)) {
...
static void bt_stack_le_gap_handle_auth_passkey_input_evt(T_LE_GAP_MSG *p_gap_msg)
{
	...
	key_input_ind->conn_handle = conn_id;
...
uint16_t bt_stack_le_gap_deinit(void)
{
	...
	memset((void *)bt_stack_le_link_tbl, 0, sizeof(bt_stack_le_link_info_t)*GAP_MAX_LINKS);
	memset((void *)bt_stack_le_conn_handle, 0, sizeof(uint16_t)*GAP_MAX_LINKS);
...
static uint16_t bt_stack_le_gap_start_ext_adv(void *param)
{
	...
	cause = le_ext_adv_set_adv_enable_param(adv_handle, duration, 0);
...
#if RTK_BLE_5_0_AE_ADV_SUPPORT && F_BT_LE_5_0_AE_ADV_SUPPORT
static bool privacy_check_is_ext_adv_idle(void)
{
#if RTK_BLE_MGR_LIB_EADV
...
#if RTK_BLE_5_0_AE_ADV_SUPPORT && F_BT_LE_5_0_AE_ADV_SUPPORT
	adv_idle = privacy_check_is_ext_adv_idle();
#else
...
static uint16_t bt_stack_le_privacy_modify_resolving_list(void *param)
{
	rtk_bt_le_modify_resolv_list_t *p_modify_param = (rtk_bt_le_modify_resolv_list_t *)param;

	privacy_modify_resolving_list((T_GAP_RESOLV_LIST_OP)p_modify_param->op,
								  (T_GAP_IDENT_ADDR_TYPE)p_modify_param->entry.addr_type,
								  p_modify_param->entry.addr, p_modify_param->entry.device_mode);
...
static uint16_t bt_stack_le_sm_get_bond_info(void *param)
{
	...
	uint8_t bond_num = le_get_bond_dev_num();
...
uint16_t bt_stack_le_gap_get_init_tx_credits(void)
{
#if !RTK_BLE_MGR_LIB
	//get init TX credits after le  stack ready
	le_get_gap_param(GAP_PARAM_LE_REMAIN_CREDITS, &g_rtk_bt_stack_init_credits);
	API_PRINT("%s g_rtk_bt_stack_init_credits=%d\r\n", __func__, g_rtk_bt_stack_init_credits);
	g_rtk_bt_stack_credits = g_rtk_bt_stack_init_credits;
#endif
...
static uint16_t bt_stack_le_gap_get_tx_pending_num(void *param)
{
	...
	if (!le_get_conn_id_by_handle(get_tx_pending_num_param->conn_handle, &conn_id)) {
		return RTK_BT_ERR_PARAM_INVALID;
	}


	//get gatts queue number
	bt_stack_le_gatts_get_tx_pending_num(conn_id, &gatts_tx_pending_num);

	//get gattc queue number
	bt_stack_le_gattc_get_tx_pending_num(conn_id, &gattc_tx_pending_num);
...
static uint16_t bt_stack_le_gap_vendor_cmd_req(void *param)
{
	T_GAP_CAUSE cause;
...
uint16_t bt_stack_le_gap_act_handle(rtk_bt_cmd_t *p_cmd)
{
	...
	case RTK_BT_LE_GAP_ACT_PRIVACY_MODIFY_LIST:
		ret = bt_stack_le_privacy_modify_resolving_list(p_cmd->param);
		break;
#endif
	...
	case RTK_BT_LE_GAP_ACT_VENDOR_CMD_REQ:
		ret = bt_stack_le_gap_vendor_cmd_req(p_cmd->param);
		break;
