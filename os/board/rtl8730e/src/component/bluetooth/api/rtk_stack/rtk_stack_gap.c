...
static struct list_head g_gap_cmd_pending_list;

rtk_bt_cmd_t *bt_stack_gap_find_pending_cmd(uint32_t msg_type)
{
    rtk_bt_cmd_t * p_cmd = NULL;
    struct list_head *plist = NULL;

    if(list_empty(&g_gap_cmd_pending_list)){
        API_PRINT("pending command list is empty \r\n");
        goto end;
    }

    plist = g_gap_cmd_pending_list.next;
    while(&g_gap_cmd_pending_list != plist) {
        if(((rtk_bt_cmd_t *)plist)->user_data == msg_type){
            p_cmd = (rtk_bt_cmd_t *)plist;
            API_PRINT("found cmd: msg_type = 0x%x \r\n", (unsigned int)msg_type);
            break;
        }
        else{
            plist = plist->next;
...
void bt_stack_gap_insert_pending_cmd(rtk_bt_cmd_t *p_cmd)
{
    API_PRINT("insert cmd: msg_type = 0x%x\r\n", (unsigned int)p_cmd->user_data);
    list_add_tail(&p_cmd->list, &g_gap_cmd_pending_list);
}

void bt_stack_gap_delete_pending_cmd(rtk_bt_cmd_t *p_cmd)
{
...
{
    INIT_LIST_HEAD(&g_gap_cmd_pending_list);
...
uint16_t app_ecfc_callback(void *p_buf, T_GAP_ECFC_MSG msg)
{
    ...
    case GAP_ECFC_PROTO_REG_RSP: {
        ...
        p_cmd = bt_stack_gap_find_pending_cmd(msg);
        if (p_cmd) {
