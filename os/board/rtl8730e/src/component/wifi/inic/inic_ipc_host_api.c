...
static u8 g_inic_ipc_logging_buf[CONFIG_KM4_MAX_QUEUE_SIZE][CONFIG_KM4_MAX_BUFFER_SIZE] = { 0 };
...
static void inic_ipc_print_int_hdl(VOID *Data, u32 IrqStatus, u32 ChanNum)
{
	...
	/* pick a buffer to fill. for rapid processing, assume that the higher priority task has already consumed the buffer, otherwise overwrite it */
	...
	message_event.buffer_len = CONFIG_KM4_MAX_BUFFER_SIZE;
	...
	g_inic_ipc_logging_buf_ctr = (g_inic_ipc_logging_buf_ctr + 1) % CONFIG_KM4_MAX_QUEUE_SIZE;
