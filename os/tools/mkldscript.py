...
# Define boards that use RTL mode (dual OTA support)
RTL_BOARDS = ["rtl8730e"]
is_rtl_mode = board_name in RTL_BOARDS
...
# For RTL mode, initialize offset with loadable_start_offset
...
if is_rtl_mode:
    # RTL mode: use 2D array to store scripts for dual OTA partitions
...
def reset_offset_if_needed():
    ...
    if not is_rtl_mode and board_name == "bk7239n" and not first_loadable_encountered:
        offset = loadable_start_offset
...
for name in NAME_LIST :
    ...
    if not is_rtl_mode and name in ("app1", "app2", "common") and not first_loadable_encountered:
    ...
    if name == "kernel" :
        ...
        if is_rtl_mode:
            # RTL mode: generate dual OTA files
