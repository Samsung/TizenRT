...
Added = [item for sublist in list(Required.values()) for item in sublist]
...
def PLO(size, libObj, currSym, subSym):
    ...
    if library in list(library_to_object_map.keys()):
        library_to_object_map[library].add(dotO)
    else:
        library_to_object_map[library] = Set([dotO])
    currSymParent = findParent(currSym)
    if dotO in list(dotO_to_section_to_size_map.keys()):
        if currSymParent in list(dotO_to_section_to_size_map[dotO].keys()):
...
def prntLibrarySizes():
    ...
    for r in list(Required.keys()):
        print("\t"+r, end=' ')
    print("\t Total")
    for l in list(library_to_object_map.keys()):
        sizearr = [0] * nfields
        for o in library_to_object_map[l]:
            if o in list(dotO_to_section_to_size_map.keys()):
                for s in list(dotO_to_section_to_size_map[o].keys()):
                    sizearr[list(Required.keys()).index(
                        s)] += dotO_to_section_to_size_map[o][s]
        for r in list(Required.keys()):
    ...
    for l in list(library_to_object_map.keys()):
        ...
        for o in library_to_object_map[l]:
            ...
            for s in list(Required.keys()):
                if s in list(dotO_to_section_to_size_map[o].keys()):
                    sizearr[list(Required.keys()).index(
                        s)] = dotO_to_section_to_size_map[o][s]
