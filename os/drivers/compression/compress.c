 ...
 * Data
 ****************************************************************************/
static bool g_compression_registered = false;
...
static int comp_close(FAR struct file *filep)
{
	struct comp_ctx *data = filep->f_priv;
	
	if(data) {
		compress_uninit(data->fd);
		kmm_free(data->compression_header);
		kmm_free(data);
		close(data->fd);
		filep->f_priv = NULL;
	}
...
{
	if (g_compression_registered) {
		return -EBUSY;
	}

	initialize_comp_ctx_list();
	int ret = register_driver(COMP_DRVPATH, &compress_fops, 0666, NULL);
	if (ret == OK) {
		g_compression_registered = true;
	}
	return ret;
}
