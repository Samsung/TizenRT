...
/****************************************************************************
 * drivers/mipidsi/mipi_dsi_host.c
 *
...
int mipi_dsi_host_register(FAR struct mipi_dsi_host *host)
{
	...
	if (g_hosts == NULL) {
		g_hosts = (struct mipi_dsi_hosts_s *)kmm_zalloc(sizeof(struct mipi_dsi_hosts_s));
	} else {
		g_hosts = (struct mipi_dsi_hosts_s *)kmm_realloc(g_hosts, sizeof(struct mipi_dsi_hosts_s) + sizeof(FAR struct mipi_dsi_host *) * g_hosts->count);
	}
	if (g_hosts == NULL) {
		return -ENOMEM;
	}
