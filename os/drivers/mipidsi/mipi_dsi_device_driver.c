...
/****************************************************************************
 * drivers/mipidsi/mipi_dsi_device_driver.c
 *
...
static ssize_t dsi_dev_read(FAR struct file *filep, FAR char *buffer, size_t buflen);
static ssize_t dsi_dev_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
int mipi_dsi_device_driver_register(FAR struct mipi_dsi_device *device)
{
	...
	priv = (struct mipi_dsi_device_driver_s*)kmm_zalloc(sizeof(struct mipi_dsi_device_driver_s));
	if (priv == NULL) {
		mipidbg("ERROR: mipi dsi device driver register failed, no memory.\n");
		return -ENOMEM;
	}
