...
ssize_t mipi_dsi_transfer(FAR struct mipi_dsi_device *device, FAR struct mipi_dsi_msg *msg)
{
...
int mipi_dsi_generic_write(FAR struct mipi_dsi_device *device, FAR const void *payload, size_t size)
{
	int ret;
	struct mipi_dsi_msg msg;
...
ssize_t mipi_dsi_generic_read(FAR struct mipi_dsi_device *device, FAR const void *params, size_t num_params, FAR void *data, size_t size)
{
	struct mipi_dsi_msg msg;
...
ssize_t mipi_dsi_dcs_write_buffer(FAR struct mipi_dsi_device *device, FAR const void *data, size_t len)
{
	struct mipi_dsi_msg msg;
...
ssize_t mipi_dsi_dcs_write(FAR struct mipi_dsi_device *device, uint8_t cmd, FAR const void *data, size_t len)
{
	ssize_t ret;
	uint8_t stack_tx[8];
...
ssize_t mipi_dsi_dcs_read(FAR struct mipi_dsi_device *device, uint8_t cmd, FAR void *data, size_t len)
{
	struct mipi_dsi_msg msg;
...
int mipi_dsi_dcs_get_power_mode(FAR struct mipi_dsi_device *device, FAR uint8_t *mode)
{
	int ret;

	ret = mipi_dsi_dcs_read(device, MIPI_DCS_GET_POWER_MODE, mode, sizeof(*mode));
	if (ret <= 0) {
		if (ret == 0) {
			ret = -ENODATA;
		}
...
int mipi_dsi_dcs_get_pixel_format(FAR struct mipi_dsi_device *device, FAR uint8_t *format)
{
	int ret;
...
int mipi_dsi_dcs_set_column_address(FAR struct mipi_dsi_device *device, uint16_t start, uint16_t end)
{
	uint8_t payload[4] = {
		start >> 8,
			  start & 0xff,
	...
	ret = mipi_dsi_dcs_write(device, MIPI_DCS_SET_COLUMN_ADDRESS, payload, sizeof(payload));
	return ret < 0 ? ret : OK;
}
...
int mipi_dsi_dcs_set_page_address(FAR struct mipi_dsi_device *device, uint16_t start, uint16_t end)
{
	uint8_t payload[4] = {
		start >> 8,
			  start & 0xff,
	...
	ret = mipi_dsi_dcs_write(device, MIPI_DCS_SET_PAGE_ADDRESS, payload, sizeof(payload));
	return ret < 0 ? ret : OK;
}
...
int mipi_dsi_dcs_set_tear_on(FAR struct mipi_dsi_device *device, uint8_t mode)
{
	int ret;

	ret = mipi_dsi_dcs_write(device, MIPI_DCS_SET_TEAR_ON, &mode, sizeof(mode));
	return ret < 0 ? ret : OK;
}
...
int mipi_dsi_dcs_set_pixel_format(FAR struct mipi_dsi_device *device, uint8_t format)
{
	int ret;

	ret = mipi_dsi_dcs_write(device, MIPI_DCS_SET_PIXEL_FORMAT, &format, sizeof(format));
	return ret < 0 ? ret : OK;
}
...
int mipi_dsi_dcs_set_tear_scanline(FAR struct mipi_dsi_device *device, uint16_t scanline)
{
	uint8_t payload[2] = {
		scanline >> 8,
				 scanline & 0xff
	...
	ret = mipi_dsi_dcs_write(device, MIPI_DCS_SET_TEAR_SCANLINE, payload, sizeof(payload));
	return ret < 0 ? ret : OK;
}
...
int mipi_dsi_dcs_set_display_brightness(FAR struct mipi_dsi_device *device, uint16_t brightness)
{
	...
	ret = mipi_dsi_dcs_write(device, MIPI_DCS_SET_DISPLAY_BRIGHTNESS, payload, sizeof(payload));
	return ret < 0 ? ret : OK;
}
...
int mipi_dsi_dcs_get_display_brightness(FAR struct mipi_dsi_device *device, FAR uint16_t *brightness)
{
	...
	ret = mipi_dsi_dcs_read(device, MIPI_DCS_GET_DISPLAY_BRIGHTNESS, brightness, sizeof(*brightness));
	if (ret <= 0) {
		if (ret == 0) {
			ret = -ENODATA;
		}
...
FAR struct mipi_dsi_device *mipi_dsi_device_register(FAR struct mipi_dsi_host *host, FAR const char *name, int channel)
{
	...
	if (channel > 3) {
		printf("invalid virtual channel: %u\n", channel);
		return NULL;
	}

	dev = (struct mipi_dsi_device *)kmm_zalloc(sizeof(struct mipi_dsi_device));
	if (dev) {
		dev->host = host;
		dev->channel = channel;
