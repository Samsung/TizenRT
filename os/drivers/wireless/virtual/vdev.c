...
trwifi_result_e vdev_init(struct netdev *dev)
{
	...
	struct vwifi_req req = {VWIFI_MSG_INIT, 0};
...
trwifi_result_e vdev_deinit(struct netdev *dev)
{
	...
	struct vwifi_req req = {VWIFI_MSG_DEINIT, 0};
	int res = _progress_message(&req);
	if (res < 0) {
		return TRWIFI_FAIL;
	}
...
trwifi_result_e vdev_scan_ap(struct netdev *dev, trwifi_ap_config_s *config)
{
	...
	struct vwifi_req req = {VWIFI_MSG_SCANAP, 0};
	int res = _progress_message(&req);
	if (res < 0) {
		return TRWIFI_FAIL;
	}
...
trwifi_result_e vdev_connect_ap(struct netdev *dev, trwifi_ap_config_s *ap_connect_config, void *arg)
{
	...
	struct vwifi_req req = {VWIFI_MSG_CONNECTAP, 0};
	int res = _progress_message(&req);
	if (res < 0) {
		return TRWIFI_FAIL;
	}
...
trwifi_result_e vdev_disconnect_ap(struct netdev *dev, void *arg)
{
	...
	struct vwifi_req req = {VWIFI_MSG_DISCONENCTAP, 0};
	int res = _progress_message(&req);
	if (res < 0) {
		return TRWIFI_FAIL;
	}
...
trwifi_result_e vdev_get_info(struct netdev *dev, trwifi_info *wifi_info)
{
	...
	struct vwifi_req req = {VWIFI_MSG_GETINFO, 0};
	int res = _progress_message(&req);
	if (res < 0) {
		return TRWIFI_FAIL;
	}
...
trwifi_result_e vdev_start_softap(struct netdev *dev, trwifi_softap_config_s *softap_config)
{
	...
	struct vwifi_req req = {VWIFI_MSG_STARTSOFTAP, 0};
	int res = _progress_message(&req);
	if (res < 0) {
		return TRWIFI_FAIL;
	}
...
trwifi_result_e vdev_start_sta(struct netdev *dev)
{
	...
	struct vwifi_req req = {VWIFI_MSG_STARTSTA, 0};
	int res = _progress_message(&req);
	if (res < 0) {
		return TRWIFI_FAIL;
	}
...
trwifi_result_e vdev_stop_softap(struct netdev *dev)
{
	...
	struct vwifi_req req = {VWIFI_MSG_STOPSOFTAP, 0};
	int res = _progress_message(&req);
	if (res < 0) {
		return TRWIFI_FAIL;
	}
...
trwifi_result_e vdev_set_autoconnect(struct netdev *dev, uint8_t check)
{
	...
	struct vwifi_req req = {VWIFI_MSG_SETAUTOCONNECT, 0};
	int res = _progress_message(&req);
	if (res < 0) {
		return TRWIFI_FAIL;
	}
...
int vdev_linkoutput(struct netdev *devy, void *buf, uint16_t dlen)
{
	VWIFI_ENTRY;
