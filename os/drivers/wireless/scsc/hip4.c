...
int hip4_init(struct slsi_hip4 *hip)
{
	...
	ret = mbulk_pool_add(MBULK_CLASS_FROM_HOST_DAT, from_host_pool_addr,
		       from_host_pool_addr + IMG_MGR_SEC_WLAN_TX_DAT_SIZE,
		       (IMG_MGR_SEC_WLAN_TX_DAT_SIZE / HIP4_DAT_SLOTS) - sizeof(struct mbulk), 0);
	if (ret)
		return ret;

	from_host_ctrl_pool_addr = hip4_get_from_host_ctrl_pool_addr(from_host_pool_addr);
	/* Configure mbulk allocator - Control QUEUES */
	ret = mbulk_pool_add(MBULK_CLASS_FROM_HOST_CTL, from_host_ctrl_pool_addr,
		       from_host_ctrl_pool_addr + IMG_MGR_SEC_WLAN_TX_CTL_SIZE,
