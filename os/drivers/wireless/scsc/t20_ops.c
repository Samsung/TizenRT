...
int slsi_change_virtual_intf(struct wiphy *wiphy,
			     ...
			     enum slsi_80211_iftype type, u32 *flags,
			     struct vif_params *params)
{
...
static u32 wpa_alg_to_cipher_suite(enum wpa_alg alg, size_t key_len)
{
	...
	default: /* SLSI_WPA_ALG_NONE/SLSI_WPA_ALG_PMK/SLSI_WPA_ALG_IGTK */
		SLSI_ERR_NODEV(" Unexpected encryption algorithm %d",
			       alg);
		return 0;
	}
}

int slsi_add_key(const char *ifname, void *priv, enum wpa_alg alg,
		 const u8 *mac_addr, int key_index, int set_tx,
		 ...
		 const u8 *key, size_t key_len)
{
	...
	if ((ndev_vif->vif_type == FAPI_VIFTYPE_STATION) &&
	    ((params.cipher == SLSI_WLAN_CIPHER_SUITE_WEP40) ||
	     (params.cipher == SLSI_WLAN_CIPHER_SUITE_WEP104)) && peer) {
...
int slsi_del_key(void *priv, struct netif *dev,
...
int slsi_hw_scan(void *priv, struct wpa_driver_scan_params *request)
{
	...
	if (r != 0) {
		...
		work_queue(SCSC_WORK, &ndev_vif->scan_timeout_work, slsi_scan_ind_timeout_handle,
			   (FAR void *)drv, MSEC2TICK(SLSI_FW_SCAN_DONE_TIMEOUT_MSEC));
#ifdef CONFIG_SCSC_ENABLE_P2P
...
int slsi_connect(void                               *priv,
		 struct wpa_driver_associate_params *request)
{
	struct slsi_t20_drv *drv = priv;
	struct slsi_dev     *sdev;
	struct netif        *dev;
...
int slsi_mlme_send_ap_eapol(void *priv, const u8 *addr, const u8 *data,
...
struct hostapd_hw_modes *slsi_get_hw_feature_data(void *priv, u16 *num_modes, u16 *flags)
{
	...
	*num_modes = 2;
	...
	for (i = 0; i < supp_band->n_bitrates; i++) {
		if (supp_band->bitrates[i].bitrate != 10 && supp_band->bitrates[i].bitrate != 20
		    && supp_band->bitrates[i].bitrate != 55 && supp_band->bitrates[i].bitrate != 110)
			continue;
...
static struct slsi_80211_sta_ht_cap slsi_ht_cap_2ghz = {
	.ht_supported       = true,
	.cap                = SLSI_80211_HT_CAP_RX_STBC |
			      SLSI_80211_HT_CAP_SGI_20,
