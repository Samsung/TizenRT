 ...
 */
static struct max_buff *slsi_mlme_tx_rx(struct slsi_dev *sdev,
					struct netif *dev,
					struct max_buff *mbuf,
					u16 cfm_id,
					struct max_buff **mib_error,
					u16 ind_id,
					bool (*validate_cfm_wait_ind)(struct slsi_dev *sdev, struct netif *dev, struct max_buff *cfm))
{
...
/* NOTE: dev can be NULL! */
static inline struct max_buff *slsi_mlme_req_cfm_ind(struct slsi_dev *sdev,
						     struct netif *dev,
						     struct max_buff *mbuf,
						     u16 cfm_id,
						     u16 ind_id,
...
int slsi_mlme_get(struct slsi_dev *sdev, struct netif *dev, u8 *mib, int mib_len, u8 *resp,
...
static inline int slsi_set_scan_params(
	...
	u16                         report_mode,
	int                         num_ssids,
	struct wpa_driver_scan_ssid *ssids,
	struct max_buff             *req)
{
	u8                          *p = NULL;
	u8                          i;
...
static int slsi_mlme_connect_rsse_prep(struct wpa_driver_associate_params *sme, struct slsi_mlme_rsse *rsse, u8 *mfp)
{
	...
	if (sme->wpa_proto & WPA_PROTO_RSN) {
		...
		ie = slsi_80211_find_vendor_ie(WLAN_OUI_MICROSOFT,
					       WLAN_OUI_TYPE_MICROSOFT_WPA, sme->wpa_ie, sme->wpa_ie_len);
						...
						bool is_copy, u8 *ie_dest, int ie_dest_len)
{
	...
	if (ie_pos != NULL) {
		...
		wpa_ie_pos = slsi_80211_find_vendor_ie(WLAN_OUI_MICROSOFT,
...
int slsi_mlme_connect(struct slsi_dev *sdev, struct netif *dev, struct wpa_driver_associate_params *sme)
{
	...
	case (WPA_AUTH_ALG_OPEN | WPA_AUTH_ALG_SHARED): /* SLSI_80211_AUTHTYPE_AUTOMATIC */
		...
		if ((sme->group_suite == WPA_CIPHER_WEP40 ||
		     sme->group_suite == WPA_CIPHER_WEP104))
			auth_type = SLSI_80211_AUTHTYPE_SHARED_KEY;
		else
			auth_type = SLSI_80211_AUTHTYPE_OPEN_SYSTEM;
		break;
	...
	if ((auth_type == SLSI_80211_AUTHTYPE_SHARED_KEY) &&
	    ((sme->group_suite == WPA_CIPHER_WEP40) ||
	     (sme->group_suite == WPA_CIPHER_WEP104)) &&
...
int slsi_mlme_get_sinfo_mib(struct slsi_dev *sdev, struct netif *dev,
...
int slsi_mlme_set_packet_filter(struct slsi_dev *sdev, struct netif *dev,
				int pkt_filter_len,
				u8 num_filters,
