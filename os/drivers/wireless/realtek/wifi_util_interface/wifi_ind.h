...
typedef void (*rtw_event_handler_t)(char *buf, int buf_len, int flags,
 ...
 * @param[in]  event: An event reported from driver to upper layer application.
 *Please refer to rtw_event_indicate_t enum.
 * @param[in]  buf: If it is not NUL, buf is a pointer to the buffer for message
 *string.
 * @param[in]  buf_len: The length of the buffer.
 * @param[in]  flags: Indicate some extra information, sometimes it is 0.
 * @retval None
 * @note  If upper layer application triggers additional operations on receiving
 *of wext_wlan_indicate,
 *			please strictly check current stack size usage (by using
 *uxTaskGetStackHighWaterMark() ),
 *			and tries not to share the same stack with wlan driver if
 *remaining stack space is not available
 *			for the following operations.
 *			ex: using semaphore to notice another thread instead of
 *handing event directly in wifi_indication().
 */
extern void wifi_indication(rtw_event_indicate_t event, char *buf, int buf_len,
                            int flags);
 ...
 */
extern void wifi_reg_event_handler(unsigned int event_cmds,
 ...
 * @return  RTW_ERROR : if an error occurred.
 */
extern void wifi_unreg_event_handler(unsigned int event_cmds,
                                     rtw_event_handler_t handler_func);
