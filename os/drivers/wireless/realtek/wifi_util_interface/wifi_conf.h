...
#define PSCAN_FAST_SURVEY                                                      \
  0x02 // set to select scan time to FAST_SURVEY_TO, otherwise SURVEY_TO
#define PSCAN_SIMPLE_CONFIG                                                    \
  0x04 // set to select scan time to FAST_SURVEY_TO and resend probe request
 ...
 *                         - RTW_SECURITY_WEP_PSK        - WEP Security with
 *open authentication
 *                         - RTW_SECURITY_WEP_SHARED     - WEP Security with
 *shared authentication
 *                         - RTW_SECURITY_WPA_TKIP_PSK   - WPA Security
 *                         - RTW_SECURITY_WPA2_AES_PSK   - WPA2 Security using
 *AES cipher
 *                         - RTW_SECURITY_WPA2_TKIP_PSK  - WPA2 Security using
 *TKIP cipher
 *                         - RTW_SECURITY_WPA2_MIXED_PSK - WPA2 Security using
 *AES and/or TKIP ciphers
 * @param[in]  password: A byte array containing either the cleartext security
 *key for WPA/WPA2
 *  						 secured networks, or a pointer to an
 *array of rtw_wep_key_t
 *  						 structures for WEP secured
 *networks.
 * @param[in]  ssid_len: The length of the SSID in bytes.
 * @param[in]  password_len: The length of the security_key in bytes.
 * @param[in]  key_id: The index of the wep key (0, 1, 2, or 3). If not using
 *it, leave it with value -1.
 * @param[in]  semaphore: A user provided semaphore that is flagged when the
 *join is complete. If not using it, leave it with NULL value.
 * @return  RTW_SUCCESS: when the system is joined and ready to send data
 *packets.
 ...
 * @note  Please make sure the Wi-Fi is enabled before invoking this function.
 *(@ref wifi_on())
 * @note  The difference between @ref wifi_connect_bssid() and @ref
 *wifi_connect() is that BSSID has higher priority as the basis of connection in
 *@ref wifi_connect_bssid.
 */
int wifi_connect_bssid(unsigned char bssid[ETH_ALEN], char *ssid,
                       rtw_security_t security_type, char *password,
                       int bssid_len, int ssid_len, int password_len,
 ...
 * @param[in]  interface: The interface can be set as RTW_STA_INTERFACE or
 * RTW_AP_INTERFACE. (@ref rtw_interface_t)
 * @return  If the function succeeds, the return value is 1. Otherwise, return
 * 0.
 */
...
/**
 * @brief  This function sets the current Media Access Control (MAC) address of
 * the 802.11 device.
 ...
 * @param[out]  client_list_buffer: The location where the client list will be
 * stored.
 ...
 * note: inidata_rate = 2 * (data rate), you need inidata_rate/2.0 to get the
 * real rate
 */
 ...
 * @param[out]  pRSSI: Points to the integer to store the RSSI value gotten from
 * driver.
 ...
 * @note  Do NOT need to call this function for STA mode wifi driver, since it
 * will determine the channel from received beacon.
 */
 ...
 * 		Once a multicast address has been registered, all packets detected
 * on the
 ...
 * 		You can replace this weak function by the same name funcation to
 * setup adaptivity mode you want.
 * @param  None
 ...
 * @param  None
 * @return  If the function succeeds, the return value is 0.
 * @note  The difference between @ref wifi_rf_on() and @ref wifi_on() is that
 * @ref wifi_rf_on() simply enable RF HAL, it does not enable the driver or
 * allocate memory.
 */
int wifi_rf_on(void);
 ...
 * @param  None
 * @return  If the function succeeds, the return value is 0.
 * @note  The difference between @ref wifi_rf_off() and @ref wifi_off() is that
 * @ref wifi_rf_off() simply disable RF HAL, the driver and used heap memory
 ...
 * @param[in] ips_mode: The desired IPS mode. It becomes effective when wlan
 *enter ips.\n
 *		@ref ips_mode is inactive power save mode. Wi-Fi automatically
 *turns RF off if it is not associated to AP. Set 1 to enable inactive power
 *save mode.
 * @param[in] lps_mode: The desired LPS mode. It becomes effective when wlan
 *enter lps.\n
 *		@ref lps_mode is leisure power save mode. Wi-Fi automatically
 *turns RF off during the association to AP is traffic is not busy while it also
 *automatically turns RF on to listen to beacon. Set 1 to enable leisure power
 *save mode.
 ...
 *                           If we separate TBTT into 2 slots, then slot_period
 * should be larger or equal to 50ms.
 *                           It means 2 slot period is
 *                               slot_period, 100-slot_period
 *                           If we separate TBTT into 3 slots, then slot_period
 * should be less or equal to 33ms.
 ...
 *                    STA leave LPS until dtim count of packages buffered at AP
 * side.
 ...
 * @param[in] mode: LPS threshold mode
 * LPS_THRESH_PKT_COUNT/LPS_THRESH_DIRECT_ENTER/LPS_THRESH_TP
 ...
 * @param[in] value:
 *				- NO_MGMT_FRAME_PROTECTION 		- not
 *support
 *				- MGMT_FRAME_PROTECTION_OPTIONAL 	-
 *capable
 *				- MGMT_FRAME_PROTECTION_REQUIRED 	-
 *required
 * @return  RTW_SUCCESS if setting Management Frame Protection Support
 *successful.
 * @return  RTW_ERROR otherwise.
 */
 ...
 * @warning If a STA interface is active when this function is called, the
 * softAP will
 *          start on the same channel as the STA. It will NOT use the channel
 * provided!
 * @param[in]  ssid: A null terminated string containing the SSID name of the
 * network.
 ...
 * @param[in]  password: A byte array containing the cleartext security key for
 * the network.
 * @param[in]  ssid_len: The length of the SSID in bytes.
 * @param[in]  password_len: The length of the security_key in bytes.
 ...
 *  			may take between 1 and 3 seconds to complete. The results
 * of
 *  			the scan will be individually provided to the callback
 *  			function. Note: The callback function will be executed
 * in
...
/**
 * @brief  Initiate a scan to search for 802.11 networks, a higher level API
 *based on wifi_scan
 *			to simplify the scan operation.
 * @param[in]  results_handler: The callback function which will receive and
 *process the result data.
 * @param[in]  user_data: User specified data that will be passed directly to
 *the callback function.
 * @return  RTW_SUCCESS or RTW_ERROR
 * @note  Callback must not use blocking functions, since it is called from the
 *context of the RTW thread.
 *			The callback, user_data variables will be referenced after
 *the function returns.
 *			Those variables must remain valid until the scan is
 *completed.
 *			The usage of this api can reference ATWS in
 *atcmd_wifi.c.
 */
 ...
 *completed.
 */
int wifi_scan_networks_with_ssid(int(results_handler)(char *, int, char *,
                                                      void *),
                                 void *user_data, int scan_buflen, char *ssid,
                                 int ssid_len);
 ...
 * @return  RTW_SUCCESS or RTW_ERROR.
 * @note  This function should be used with wifi_scan function. First, use @ref
 *wifi_set_pscan_chan to
 *			indicate which channel will be scanned, and then use @ref
 *wifi_scan to get scanned results.
 */
 ...
 * @param[out]  pSetting: Points to the rtw_wifi_setting_t structure to store
 * the WIFI setting gotten from driver.
 * @return  RTW_SUCCESS or RTW_ERROR.
 */
int wifi_get_setting(const char *ifname, rtw_wifi_setting_t *pSetting);

/**
 * @brief  Show the network information stored in a rtw_wifi_setting_t
 * structure.
 * @param[in]  ifname: the wlan interface name, can be WLAN0_NAME or WLAN1_NAME.
 * @param[in]  pSetting: Points to the rtw_wifi_setting_t structure which
 * information is gotten by @ref wifi_get_setting().
 * @return  RTW_SUCCESS or RTW_ERROR.
 */
 ...
 *			Driver works in BGN mode in default after driver
 initialization. This function is used to
 *			change wireless network mode for station mode before
 connecting to AP.
 * @param[in]  mode: Network mode to set. The value can be
 RTW_NETWORK_B/RTW_NETWORK_BG/RTW_NETWORK_BGN.
 * @return  RTW_SUCCESS or RTW_ERROR.
 */
 ...
 * @param[in]  enabled: enabled can be set 0, 1, 2, 3 and 4. if enabled is zero,
 *disable the promisc, else enable the promisc.
 *                    - 0 means disable the promisc.
 *                    - 1 means enable the promisc special for all ethernet
 *frames.
 *                    - 2 means enable the promisc special for
 *Broadcast/Multicast ethernet frames.
 *                    - 3 means enable the promisc special for all 802.11
 *frames.
 *                    - 4 means enable the promisc special for
 ...
 *
 * @return  RTW_SUCCESS or RTW_ERROR
 * @note  This function can be used to implement vendor specified simple
 *configure.
...
int wifi_restart_ap(unsigned char *ssid, rtw_security_t security_type,
                    unsigned char *password, int ssid_len, int password_len,
                    int channel);
 ...
 * @note  Defining CONFIG_AUTO_RECONNECT in "autoconf.h" needs to be done before
 *compiling,
 *			or this API won't be effective.
 * @note  The difference between @ref wifi_config_autoreconnect() and @ref
 *wifi_set_autoreconnect() is that
 *			user can specify the retry times and timeout value in @ref
 *wifi_config_autoreconnect().
 *			But in @ref wifi_set_autoreconnect() these values are set
 *with 3 retry limit and 5 seconds timeout as default.
 */
int wifi_config_autoreconnect(__u8 mode, __u8 retry_times, __u16 timeout);

/**
 * @brief  Set reconnection mode with 3 retry limit and 5 seconds timeout as
 *default.
 * @param[in]  mode: Set 1/0 to enalbe/disable the reconnection mode.
 * @return  0 if success, otherwise return -1.
 * @note  Defining CONFIG_AUTO_RECONNECT in "autoconf.h" needs to be done before
...
#define _CUS_IE_
/**
 * @brief  The structure is used to set WIFI custom ie list, and type match
 *CUSTOM_IE_TYPE.\n
 ...
 * @warning  This API can't be executed twice before deleting the previous
 *custom ie list.
 * @param[in]  cus_ie: Pointer to WIFI CUSTOM IE list.
 * @param[in]  ie_num: The number of WIFI CUSTOM IE list.
 * @return  0 if success, otherwise return -1.
 * @note  Defininig CONFIG_CUSTOM_IE in "autoconf.h" needs to be done before
 *compiling,
 *			or this API won't be effective.
 */
...
/**
 * @brief: Only receive the packets sent by the specified ap and phone in
 * promisc mode.
 * @param[in]  enable: set 1 to enable filter, set 0 to disable this filter
 * function.
 * @param[in]  ap_mac: if 'enable' equals 0, it's useless; if 'enable' equals 1,
 * this value is the ap's mac address.
 * @param[in]  phone_mac: if 'enable' equals 0, it's useless; if 'enable' equals
 * 1, this value is the phone's mac address.
 * @return  None.
 * @note  Please invoke this function as
 * "wifi_filter_by_ap_and_phone_mac(0,NULL,NULL)" before exiting promisc mode if
 * you enabled it during the promisc mode.
 */
 ...
 * @param[in]  antenna: Points to store the antenna value gotten from driver, 0:
 * main, 1: aux.
 * @return  0 if success, otherwise return -1.
 */
 ...
 * @param[out]  ability : 0x1 stand for mp driver, and 0x0 stand for normal
 * driver
 * @return  RTW_SUCCESS
 */
...
/**
 * @brief  Set channel plan into flash/efuse, must reboot after setting channel
 * plan
 * @param[in]  channel_plan : the value of channel plan, define in
 * wifi_constants.h
 * @return  RTW_SUCCESS or RTW_ERROR
 */
 ...
 * @param[out]  channel_plan : point to the value of channel plan, define in
 * wifi_constants.h
 * @return  RTW_SUCCESS or RTW_ERROR
 */
...
/**
 * @brief  Set flag for concurrent mode wlan1 issue_deauth when channel switched
 * by wlan0
 *          usage: wifi_set_ch_deauth(0) -> wlan0 wifi_connect ->
...
/**
 * @brief  interface for bt to set mailbox info to wifi, mostly for coexistence
 * usage
