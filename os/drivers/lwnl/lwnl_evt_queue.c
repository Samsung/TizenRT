...
static int _lwnl_update_event_filep(struct lwnl_event *evt)
{
	...
	//lldbg("[pkbuild] refs %d %d\n", g_connected[dtype], refs);
	LWQ_UNLOCK;
	return 0;
}
...
{
	LWNL_ENTER(TAG);
	if ((void *)evt < (void *)0x2000000 || (void *)evt > (void *)0x3000000) {
		assert(0);
	}
	evt->refs--;
...
int lwnl_remove_listener(struct file *filep)
{
	...
	lldbg("remove listener filep %p %d %d\n",
			filep, g_totalevt, LWNL_NPOLLWAITERS);
	LWQ_LOCK;
...
int lwnl_check_queue(struct file *filep)
{
	...
	struct lwnl_filep *llfp = (struct lwnl_filep *)filep->f_priv;
	if (!llfp) {
		lldbg("[T%d] llfp is null\n", getpid(), llfp);
		goto done;
	}
	if (!sq_empty(&llfp->queue)) {
		res = 1;
	}
	lldbg("[T%d] que status(%d) %d\n", getpid(), res, queue_size(llfp));
done:
	LWQ_UNLOCK;
	return res;
}
