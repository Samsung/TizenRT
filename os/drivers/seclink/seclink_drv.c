...
int seclink_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	if (cmd == SECLINK_HAL_INIT || cmd == SECLINK_HAL_DEINIT) {
		hd_handle_common_request(cmd, arg, (void *)upper->lower);
	} else if (cmd & SECLINK_AUTH) {
		hd_handle_auth_reqeust(cmd, arg, (void *)upper->lower);
	} else if (cmd & SECLINK_KEY) {
		hd_handle_key_request(cmd, arg, (void *)upper->lower);
	} else if (cmd & SECLINK_SS) {
		hd_handle_ss_request(cmd, arg, (void *)upper->lower);
	} else if (cmd & SECLINK_CRYPTO) {
		hd_handle_crypto_request(cmd, arg, (void *)upper->lower);
	}
