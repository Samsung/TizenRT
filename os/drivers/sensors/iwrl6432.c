...
{
	int ret;
	ret = sem_tickwait(&priv->acksem, clock_systimer(), MSEC2TICK(2000));
	if (ret < 0) {
		int err = get_errno();
		if (err == ETIMEDOUT) {
			...
			if (irwl6432_init(priv) == OK) {
				...
				mmwavedbg("irwl6432 init failed!!\n");
			}
			return;
		}
	...
	mmwavevdbg("Sending ACK for Cube No : %d\n", priv->req_num);
...
static void iwrl6432_request_cube_data(FAR struct iwrl6432_dev_s *priv)
{
	...
	mmwavevdbg("Requesting Cube no : %d\n", priv->req_num);
	priv->config->irq_enable(true);
	...
	if (priv->req_num == 1) {
		mmwavevdbg("Receiving Frame No : %d\n", (rxbuf->data[12] | rxbuf->data[13] << 8));
	}
	...
	if (priv->mq == NULL) {
		mmwavedbg("mq is not registered!!!\n");
		return;
	}
...
static void iwrl6432_interrupt_handler(void *arg, bool state)
{
	...
	if (work_available(&priv->irq_work)) {
		if (!state) {
			ret = work_queue(HPWORK, &priv->irq_work, iwrl6432_work_handler, &priv->datasem, 0);
		} else {
			ret = work_queue(HPWORK, &priv->irq_work, iwrl6432_work_handler, &priv->acksem, 0);
		}
		if (ret != 0) {
...
cleanup:
	...
	ret = sem_tickwait(&priv->datasem, clock_systimer(), MSEC2TICK(5000));
	if (ret < 0) {
			int err = get_errno();
			if (err == ETIMEDOUT) {
				mmwavevdbg("=========================================Timeout during init========================================\n");
				return -1;
			}
...
static int iwrl6432_mq_thread(int argc, char **agrv)
{
	...
	while (1) {
		...
		ret = sem_tickwait(&priv->datasem, clock_systimer(), MSEC2TICK(2000));
		if (ret < 0) {
			int err = get_errno();
			if (err == ETIMEDOUT) {
				iwrl6432_semtake(&priv->exclsem, false);
				mmwavevdbg("=========================================Timeout while waiting to receive req_num:%d========================================\n", priv->req_num);
				struct iwrl6432_msg_s msg;
				msg.msgId = IWRL6432_MSG_TIMEOUT;
				if (mq_send(priv->mq, (FAR const char *)&msg, sizeof(msg), CONFIG_IWRL6432_SG_DEQUEUE_PRIO) != OK) {
					mmwavedbg("mq_send failed!!\n");
...
int irwl6432_register(FAR const char *devname, FAR struct iwrl6432_config_s *config, FAR struct spi_dev_s *spi)
{
	...
	if (ret < 0) {
		...
		mmwavedbg("irwl6432 interrupt handler registration failed\n");
		return ret;
	}
	...
	if (ret < 0) {
		...
		sem_destroy(&priv->datasem);
		sem_destroy(&priv->acksem);
		mmwavedbg("irwl6432 Driver registration failed\n");
		return ret;
	}

	mmwavevdbg("irwl6432 Driver registered Successfully\n");
	return OK;
}
