 ...
 ****************************************************************************/
 struct iwrl6432_dev_s {
	struct iwrl6432_buf_s *pBuffer[IWRL6432_BUF_NUM];
...
static void iwrl6432_request_cube_data(FAR struct iwrl6432_dev_s *priv)
{
	...
	lldbg("rxbuf : %x req_num : %d size : %d\n", rxbuf->data, priv->req_num, size);
	...
	if (time_stamp > INT_MAX) { // TODO this is temp value!
		msg.msgId = IWRL6432_MSG_UNDERRUN;
	} else if (priv->enable == false) {
		msg.msgId = IWRL6432_MSG_STOP_FORCELY;
	} else {
		msg.msgId = IWRL6432_MSG_DEQUEUE;
		msg.pData = rxbuf;
	}
	lldbg("before send!! msgId : %d\n", msg.msgId);
	int ret = mq_send(priv->mq, (FAR const char *)&msg, sizeof(msg), CONFIG_IWRL6432_SG_DEQUEUE_PRIO);
	lldbg("after send!! ret : %d\n", ret);
	if (ret != OK) {
		lldbg("mq_send error...errno : %d\n", errno);
	}
	/* TODO consider enqueue buffer in iwrl6432_get_data */
...
static void iwrl6432_work_handler(void *arg)
{
	...
	lldbg("Let's handle cube data here!!\n");
...
static void iwrl6432_interrupt_handler(void *arg)
{
	...
	if (work_available(&priv->work)) {
		...
		ret = work_queue(HPWORK, &priv->work, iwrl6432_work_handler, priv, 0);
		if (ret != 0) {
...
static int iwrl6432_get_data(FAR struct iwrl6432_dev_s *priv)
{
...
static int iwrl6432_stop(FAR struct iwrl6432_dev_s *priv)
{
	...
	for (int i = 0; i < IWRL6432_BUF_NUM; i++) {
		if (priv->pBuffer[i]) {
			free(priv->pBuffer[i]);
		}
	}
...
static int iwrl6432_prepare(FAR struct iwrl6432_dev_s *priv)
{
	int ret = OK;
	for (int i = 0; i < IWRL6432_BUF_NUM; i++) {
		priv->pBuffer[i] = (struct iwrl6432_buf_s *)kmm_malloc(sizeof(FAR struct iwrl6432_buf_s));
		if (!priv->pBuffer[i]) {
			ret = -ENOMEM;
			lldbg("pBuffer index : %d malloc failed!\n", i);
			goto error_with_buf;
		}
	}
	priv->timestamp = 0;
	sq_init(&priv->pendq);
	for (int i = 0; i < IWRL6432_BUF_NUM; i++) {
		lldbg("enqueue buffer : %x\n", priv->pBuffer[i]->data);
		sq_addlast((sq_entry_t *)&priv->pBuffer[i]->entry, &priv->pendq);
	}
	return OK;
error_with_buf:
	for (int i = 0; i < IWRL6432_BUF_NUM; i++) {
...
{
	iwrl6432_semtake(&g_iwrl6432_priv.exclsem, false);
	auddbg("set Record enable : %d\n", enable);
	if (g_iwrl6432_priv.enable == enable) {
		iwrl6432_semgive(&g_iwrl6432_priv.exclsem);
		return;
	}

	g_iwrl6432_priv.enable = enable;
	g_iwrl6432_priv.config->irq_enable(enable);
	if (g_iwrl6432_priv.running) {
		g_iwrl6432_priv.running = false;
		iwrl6432_flush(&g_iwrl6432_priv);
	}
	iwrl6432_semgive(&g_iwrl6432_priv.exclsem);
}
