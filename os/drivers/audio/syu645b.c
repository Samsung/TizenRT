...
static int syu645b_writereg_2byte(FAR struct syu645b_dev_s *priv, uint8_t regaddr, uint8_t regval)
{
        int ret;
        uint8_t reg[2];
        ...
        ret = i2c_write(dev, syu645b_i2c_config, (uint8_t *)reg, 2);
        if (ret < 0) {
		auddbg("Error, cannot write reg 0x%x/0x%x\n", regaddr, regval);
        }
        return ret;
}
 ...
 *
 * Description:
 *   Executes given script through i2c to configuure SYU645B device.
 *
 ************************************************************************************/
...
static int syu645b_exec_i2c_script(FAR struct syu645b_dev_s *priv, t_codec_init_script_entry *script, uint32_t size)
{
	uint32_t i;
        ...
        for (i = 0; i < size; i++) {
		ret = syu645b_writereg_2byte(priv, script[i].addr, script[i].val);
                if (script[i].delay > 0) {
...
static void syu645b_setvolume(FAR struct syu645b_dev_s *priv)
{
        ...
        if (priv->running) {
			...
			if (priv->volume == 0) {
				...
				codec_set_master_volume_script[0].val = SYU645B_HW_VOL_MAX - (SYU645B_SPK_VOL_MAX - priv->volume) * SYU645B_HW_VOL_STEP;
			}
