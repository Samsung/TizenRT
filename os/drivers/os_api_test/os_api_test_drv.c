...
static int os_api_test_drv_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	case TESTIOC_WDOG_DEFECT_DSB: {
		...
		lldbg("DEFECT_DSB: Running 1,000 iterations of begin()+write+end()\n");
		lldbg("DEFECT_DSB: Without dsb, intermittent Data Abort may occur\n");

		/* B-07 FIX: Wrap entire loop in a critical section to prevent timer
		 * ticks between iterations from seeing corrupted wdog data.
		 * Reduce to 1000 iterations to limit SMP lock contention. */
		irqstate_t dsb_irq_state = enter_critical_section();
		for (i = 0; i < 1000; i++) {
			wd_mmu_write_begin();
		...
		leave_critical_section(dsb_irq_state);

		lldbg("DEFECT_DSB: All 1,000 iterations completed (no Data Abort)\n");
	...
	case TESTIOC_WDOG_DEFECT_DACR_CLOBBER: {
		...
		/* B-08 FIX: Save original DACR to restore after test */
		__asm__ volatile("mrc p15, 0, %0, c3, c0, 0" : "=r"(dacr_orig));
		...
		/* B-08 FIX: Restore original DACR — test must not leave system modified */
		__asm__ volatile("mcr p15, 0, %0, c3, c0, 0" :: "r"(dacr_orig) : "memory");
...
#else
		lldbg("DEFECT_HEAP: CONFIG_RAM_START not defined - skipping heap range check\n");
		if (0) {
#endif
			lldbg("DEFECT_HEAP: wdog pool is within RAM region — BY DESIGN (B-02/B-03 fix)\n");
			lldbg("DEFECT_HEAP: wdog pool is statically allocated, never handed out by heap allocator\n");
			lldbg("DEFECT_HEAP: Verifying: allocating 1000 buffers to check for overlap...\n");
			...
			for (j = 0; j < 1000; j++) {
				...
				if (ptr == NULL) {
					lldbg("DEFECT_HEAP: malloc failed at iteration %d\n", j);
					break;
				}
				uint32_t addr = (uint32_t)ptr;
				if (addr >= pool_start && addr < pool_end) {
					lldbg("DEFECT_HEAP: OVERLAP! malloc returned 0x%08x (within wdog pool!)\n", addr);
					overlap_found = 1;
				}
				free(ptr);
			}
			...
			if (!overlap_found) {
				lldbg("DEFECT_HEAP: All %d allocations are outside wdog pool — PASS\n", j);
				lldbg("DEFECT_HEAP: No heap overlap risk — wdog pool coexists safely\n");
			} else {
				lldbg("DEFECT_HEAP: FAIL — heap allocator returned wdog pool addresses!\n");
			}
		} else {
	...
	case TESTIOC_WDOG_SCENARIO_MULTI_EXPIRE: {
		...
		WDOG_ID wdogs[TC06_NUM_WDOGS] = {NULL};
		int i;
