...
#if defined(CONFIG_LCD_ST7701SN) && (defined(CONFIG_LCD_LANDSCAPE) || defined(CONFIG_LCD_RLANDSCAPE))
...
FAR struct lcd_dev_s *mipi_lcdinitialize(FAR struct mipi_dsi_device *dsi, struct mipi_lcd_config_s *config)
{
	...
	for (int itr = 0; itr < NUM_OF_LCD_BUFFER; itr++) {
		lcd_buffer[itr] = (uint8_t *)kmm_malloc(CONFIG_LCD_XRES * CONFIG_LCD_YRES * 2 + 1);
		if(!lcd_buffer[itr]) {
