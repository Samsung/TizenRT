...
#define HOTPLUG_MANAGER_IMPLEMENTED	0
...
static int cpu_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case IOC_CPU_ENABLE:
#if HOTPLUG_MANAGER_IMPLEMENTED
		/* Call Hotplug manager's API to enable the CPU */
		ret = cpu_online(cpu);
		if (ret < 0) {
			lldbg("ERROR: Failed to enable CPU%d: %d\n", cpu, ret);
			return ret;
		}
#endif
		return OK;
...
#if HOTPLUG_MANAGER_IMPLEMENTED
		/* Call Hotplug manager's API to disable the CPU */
		ret = cpu_offline(cpu);
		if (ret < 0) {
			lldbg("ERROR: Failed to disable CPU%d: %d\n", cpu, ret);
			return ret;
		}
#endif
		return OK;
...
#if HOTPLUG_MANAGER_IMPLEMENTED
		/* Call Hotplug manager's API to get the CPU state */
		int state = cpu_get_state(cpu);
		if (state < 0) {
...
void cpu_driver_init(void)
{
	...
	for (cpu = 1; cpu < CONFIG_SMP_NCPUS; cpu++) {
		...
		if (ret != OK) {
			lldbg("ERROR: Failed to register %s: %d\n", devname);
			return;
		}

		svdbg("CPU driver registered: %s\n", devname);
	}
}
