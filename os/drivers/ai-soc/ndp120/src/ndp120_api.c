...
void ndp120_show_debug(int do_check_mb)
{
	...
	/* address are specific to DSP FW build... */
	...
	uint32_t adx;
	uint32_t mcu_fw_wake_count, mb_int_count;
	...
	adx = (uint32_t) (ndp120->mcu_fw_state_addr + (uint32_t) offsetof(struct ndp120_fw_state_s, mb_int_count));
	syntiant_ndp120_read_block(ndp, 1, adx, &mb_int_count, 4);
