...
bool is_safe_write_address(size_t addr, size_t size)
{
	bool result = true;
	...
	for (int i = 0; i < len; i += sizeof(before_buf)) {
		...
		for (int r = 0; r < read_len; r += sizeof(uint32_t)) {
			...
			if ((before & write) != write) {
				spi_flash_guard_end();
				ESP_LOGW(TAG, "Write at offset 0x%x requests 0x%08x but will write 0x%08x -> 0x%08x", target + i + r, write, before, before & write);
				spi_flash_guard_start();
			}
		}
#endif
...
{
	ESP_LOGI(TAG, "%s  count=%8d  time=%8dus  bytes=%8d\n", name, counter->count, counter->time, counter->bytes);
}
