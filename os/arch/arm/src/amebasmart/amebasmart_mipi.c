...
/* Helpers */
static void MipiDsi_rtl8730_Send_DCS(MIPI_TypeDef *MIPIx, u8 cmd, u8 payload_len, u8 *para_list);
...
static void check_freq(struct lcd_data *data)
{
	...
	u32 vo_freq = totaly * totalx * MIPI_FRAME_RATE / Mhz + 4;
	lldbg("vo_freq: %d\n", vo_freq);
...
static int amebasmart_mipi_transfer(FAR struct mipi_dsi_host *dsi_host, FAR const struct mipi_dsi_msg *msg)
{
	...
	if (mipi_dsi_packet_format_is_short(msg->type)) {
		if (packet.header[1] == 0) {
			MipiDsi_rtl8730_Send_DCS(priv->MIPIx, packet.header[0], 0, NULL);
		} else {
			MipiDsi_rtl8730_Send_DCS(priv->MIPIx, packet.header[0], 1, packet.header + 1);
		}
	} else {
		MipiDsi_rtl8730_Send_DCS(priv->MIPIx, packet.header[0], packet.payload_length, packet.payload);
	}
...
struct mipi_dsi_host *mipi_dsi_host_initialize(struct lcd_data *config)
{
	...
	return (FAR struct mipi_dsi_host *)priv;
}
