...
static int ospi_memory(struct ospi_dev_s *dev, struct ospi_meminfo_s *meminfo)
{
  ...
  if (OSPIMEM_ISWRITE(meminfo->flags))
  {
    ...
    uint8_t *mem_addr;
    //OSPI_NOR_AutoPollingMemReady
    OSPI_AutoPollingMemReady(&OSPINORHandle);

    if(mode == MEMORY_MAPPED_OFF)
    {
      stm32l4_ospi_enter_memorymapped((struct ospi_dev_s *)&g_ospi0dev, NULL, 0);
      mode = MEMORY_MAPPED_ON;
    }

    mem_addr = (uint8_t *)(OCTOSPI1_BASE + WriteAddr);
    ...
    HAL_Delay(10/*MEMORY_PAGE_PROG_DELAY*/);
    return OSPI_NOR_OK;
  }
  /* octo spi read mode*/
