...
#ifndef __ARCH_ARM_SRC_AMEBALITE_SPI_H
#define __ARCH_ARM_SRC_AMEBALITE_SPI_H
...
/************************************************************************************
 * Name:  amebalite_spi1/2/...select and amebalite_spi1/2/...status
 *
 * Description:
 *   The external functions, amebalite_spi1/2/...select, amebalite_spi1/2/...status, and
 *   amebalite_spi1/2/...cmddata must be provided by board-specific logic.  These are
 *   implementations of the select, status, and cmddata methods of the SPI interface
 *   defined by struct spi_ops_s (see include/nuttx/spi/spi.h). All other methods
 *   (including amebalite_spibus_initialize()) are provided by common AMEBALITE logic.  To use this
 *   common SPI logic on your board:
 *
 *   1. Provide logic in amebalite_boardinitialize() to configure SPI chip select
 *      pins.
 *   2. Provide amebalite_spi1/2/...select() and amebalite_spi1/2/...status() functions in your
 *      board-specific logic.  These functions will perform chip selection and
 *      status operations using GPIOs in the way your board is configured.
 *   3. If CONFIG_SPI_CMDDATA is defined in your NuttX configuration file, then
 *      provide amebalite_spi1/2/...cmddata() functions in your board-specific logic.
 *      These functions will perform cmd/data selection operations using GPIOs in the
 *      way your board is configured.
 *   4. Add a calls to amebalite_spibus_initialize() in your low level application
 *      initialization logic
 *   5. The handle returned by amebalite_spibus_initialize() may then be used to bind the
...
/************************************************************************************
 * Name: amebalite_spi1/2/...register
 *
 * Description:
...
int amebalite_spi0register(FAR struct spi_dev_s *dev, spi_mediachange_t callback,
			FAR void *arg);
int amebalite_spi1register(FAR struct spi_dev_s *dev, spi_mediachange_t callback,
			FAR void *arg);
...
#endif /* __ASSEMBLY__ */
#endif /* __ARCH_ARM_SRC_AMEBALITE_SPI_H */
