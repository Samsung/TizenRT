...
extern uart_dev_t *up_get_console_dev(void);	/* Get Console device structure of serial */
...
{
	uart_dev_t *dev = up_get_console_dev();

	if (dev == NULL) {
		lldbg_noarg("ERROR: UART CONSOLE structure is not defined\n");
		return;
	}
