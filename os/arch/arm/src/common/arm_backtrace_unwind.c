...
static const struct __EIT_entry *search_index(unsigned long addr, const struct __EIT_entry *start, const struct __EIT_entry *origin, const struct __EIT_entry *stop)
{
...
static const struct __EIT_entry *unwind_find_entry(unsigned long addr)
{
	...
	entry = search_index(addr, __exidx_start, unwind_find_origin(__exidx_start, __exidx_end), __exidx_end);
	return entry;
}
...
static int unwind_pop_register(struct unwind_ctrl_s *ctrl, unsigned long **vsp, unsigned int reg)
{
...
static int unwind_exec_pop_subset_r4_to_r13(struct unwind_ctrl_s *ctrl, unsigned long mask)
{
...
static int unwind_exec_pop_r4_to_rn(struct unwind_ctrl_s *ctrl, unsigned long content)
{
	unsigned long *vsp = (unsigned long *)ctrl->vrs[SP];
	int reg;
...
static int unwind_exec_pop_subset_r0_to_r3(struct unwind_ctrl_s *ctrl, unsigned long mask)
{
	unsigned long *vsp = (unsigned long *)ctrl->vrs[SP];
	int reg = 0;
...
static int unwind_exec_content(struct unwind_ctrl_s *ctrl)
{
	...
	if ((content & 0xc0) == 0x00) {
		...
		ret = ((content & 0x0fff) == 0) ? -1 : unwind_exec_pop_subset_r4_to_r13(ctrl, content & 0x0fff);
...
int backtrace_unwind(struct unwind_frame_s *frame, void **buffer, int size, int *skip, unsigned char *methods)
{
	...
	lldbg("[BT] backtrace_unwind: initial fp=0x%x sp=0x%x lr=0x%x pc=0x%x size=%d skip=%d\n", (unsigned int)frame->fp, (unsigned int)frame->sp, (unsigned int)frame->lr, (unsigned int)frame->pc, size, *skip);
#endif
...
#ifdef CONFIG_DEBUG_BACKTRACE_UNWIND
		lldbg("[BT] unwind_frame returned %d, new fp=0x%x sp=0x%x lr=0x%x pc=0x%x\n", unwind_ret, (unsigned int)frame->fp, (unsigned int)frame->sp, (unsigned int)frame->lr, (unsigned int)frame->pc);
#endif
	...
	if (tcb == NULL || tcb == rtcb) {
		if (up_interrupt_context()) {
			...
			if (asserted_location && asserted_location < 0x60000000) {
				...
				unsigned long stack_top = (unsigned long)tcb->stack_base_ptr + tcb->adj_stack_size;
