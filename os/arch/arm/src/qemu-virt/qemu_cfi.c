...
#include <tinyara/semaphore.h> /* Add this for sem_t and semaphore operations */
#include <wctype.h>
#include <time.h>
...
/* Semaphore for protecting flash operations */

static sem_t g_cfi_flash_mutex;
                                 ...
                                 size_t nbytes, FAR const uint8_t *buffer)
{
  ...
  if ((delta = offset - wp) == 0 && nbytes >= cfi->bankwidth)  //Consider nbytes is not aligned with backwidth.
    {
      return 0;
    }
  ...
  cfi_reset(cfi); //reset for read existing bytes from flash
...
int cfi_check(FAR struct cfi_dev_s *cfi)
{
  ...
  /* Initialize the flash mutex */

  sem_init(&g_cfi_flash_mutex, 0, 1);
...
{
  blkcnt_t i;
  int ret = -EINVAL; /* Initialize with error value */

  sem_wait(&g_cfi_flash_mutex); /* Acquire the flash mutex */
  ...
  sem_post(&g_cfi_flash_mutex); /* Release the flash mutex */
  return ret;
}
...
int cfi_read(FAR struct cfi_dev_s *cfi, off_t offset, size_t nbytes,
             FAR uint8_t *buffer)
{
  int ret = 0; /* Initialize with success */
              ...
              FAR const uint8_t *buffer)
{
  ...
  //TEMPORAL WORKAROUND : Buffered Program about 1 word not work in QEMU.
  size_t tmp_size = nbytes / cfi->bankwidth;

  if (cfi->info.max_write_bytes_num != 0 && tmp_size > 1)
    {
      while (nbytes >= cfi->bankwidth)
        {
          ...
          if (size > nbytes)
            {
              size = nbytes / cfi->bankwidth * cfi->bankwidth;
            }
