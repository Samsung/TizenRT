...
#if !defined(CONFIG_DEBUG_OPTIMIZATION)

#include <arch/spinlock.h>
/* This is an out-of-line implementation of spin_is_locked() needed for
 * low-optimization builds where the function is not inlined.
 */
bool spin_is_locked(volatile FAR spinlock_t *lock)
{
    SP_DMB();
    return (*(lock) == SP_LOCKED);
}
#endif /* CONFIG_DEBUG_FEATURES && !CONFIG_DEBUG_OPTIMIZATION */
