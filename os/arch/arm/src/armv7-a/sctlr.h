...
.macro cp15_rdid, id mrc p15, 0, \id, c0, c0, 0.endm
/* Read/write the system control register (SCTLR) */
.macro cp15_rdsctlr, sctlr mrc p15, 0, \sctlr, c1, c0, 0.endm.macro cp15_wrsctlr, sctlr mcr p15, 0, \sctlr, c1, c0, 0 nop nop nop nop nop nop nop nop.endm
#endif							/* __ASSEMBLY__ */
/****************************************************************************
 * Inline Functions
 ****************************************************************************/
...
{
	unsigned int cecr;
	__asm__ __volatile__("\tmrc p15, 0, %0, c9, c12, 2\n":"=r"(cecr)
						 ::"memory");
...
{
	unsigned int ofsr;
	__asm__ __volatile__("\tmrc p15, 0, %0, c9, c12, 3\n":"=r"(ofsr)
						 ::"memory");
...
{
	unsigned int sir;
	__asm__ __volatile__("\tmrc p15, 0, %0, c9, c12, 4\n":"=r"(sir)
						 ::"memory");
...
{
	__asm__ __volatile__("\tmcr p15, 0, %0, c9, c12, 4\n"::"r"(sir)
						 :"memory");
}
...
{
	__asm__ __volatile__("\tmcr p15, 0, %0, c9, c14, 1\n"::"r"(iesr)
						 :"memory");
}
...
{
	unsigned int ecr;
	__asm__ __volatile__("\tmrc p15, 0, %0, c9, c13, 2":"=r"(ecr)
						 ::"memory");
