...
static void pm_timer_wakeup_callback(aon_rtc_id_t id, uint8_t *name_p, void *param)
{
    ...
    if(bk_pm_demo_send_msg(&msg) != BK_OK)
    {
        BK_LOGE("PM","Error: send message to demo failed\r\n");
    }
}
void up_set_pm_timer(unsigned int interval_us) 
{
	...
	alarm_info_t sleep_alarm = {
								"sleep",
								interval_us*AON_RTC_MS_TICK_CNT/1000,
								1,
								pm_timer_wakeup_callback,
								&sleep_count
								};
