...
int pdma_irq_handler(int irq, FAR void *context, FAR void *arg)
{
	...
	for (i = 0; i < dma->max_ch_num; i++) {
		if ((intstatus & (1 << i)) && (dma->dma_ch[i].task->callback)) {
			...
			dmallvdbg("CH%d sent %d\n", i, dma->dma_ch[i].task->size);
		}
	}
