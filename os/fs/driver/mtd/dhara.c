...
static ssize_t dhara_read(FAR struct inode *inode, FAR unsigned char *buffer, blkcnt_t start_sector, unsigned int nsectors);
static ssize_t dhara_write(FAR struct inode *inode, FAR const unsigned char *buffer, blkcnt_t start_sector, unsigned int nsectors);
...
static uint8_t *dhara_find_readcache(FAR dhara_dev_t *dev, dhara_page_t page)
{
	FAR dq_queue_t *q = &dev->readcache;
...
static void dhara_insert_readcache(FAR dhara_dev_t *dev, dhara_pagecache_t *cache)
{
	FAR dq_queue_t *q = &dev->readcache;
...
static void dhara_discard_readcache(FAR dhara_dev_t *dev, dhara_page_t page)
{
	FAR dhara_pagecache_t *cache;
	FAR dq_queue_t *q = &dev->readcache;
	FAR dq_entry_t *c;
...
static ssize_t dhara_read(FAR struct inode *inode, FAR unsigned char *buffer, blkcnt_t start_sector, unsigned int nsectors)
{
	...
	while (nsectors-- > 0) {
		dhara_error_t err;
		ret = dhara_map_read(&dev->map, start_sector, buffer, &err);
		if (ret < 0) {
...
int dhara_nand_erase(FAR const struct dhara_nand *n, dhara_block_t bno, FAR dhara_error_t *err)
{
	FAR dhara_dev_t *dev = (FAR dhara_dev_t *) n;
...
int dhara_nand_is_free(FAR const struct dhara_nand *n, dhara_page_t p)
{
	FAR dhara_dev_t *dev = (FAR dhara_dev_t *) n;
