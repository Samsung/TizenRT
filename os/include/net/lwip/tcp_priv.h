...
#define TCP_EVENT_POLL(pcb, ret) do { if ((pcb)->state != SYN_RCVD) {                          \
								ret = lwip_tcp_event((pcb)->callback_arg, (pcb), LWIP_EVENT_POLL, NULL, 0, ERR_OK); } else { \
								ret = ERR_ARG; } \
								} while (0)
...
#define TCP_EVENT_ACCEPT(lpcb, pcb, arg, err, ret) do { \
												if ((lpcb)->accept != NULL) \
...
#define TCP_EVENT_SENT(pcb, space, ret) do { \
											if ((pcb)->sent != NULL) \
...
#define TCP_EVENT_RECV(pcb, p, err, ret) do { \
											if ((pcb)->recv != NULL) { \
...
#define TCP_EVENT_CLOSED(pcb, ret) do { \
											if (((pcb)->recv != NULL)) { \
...
#define TCP_EVENT_CONNECTED(pcb, err, ret) do { \
...
#define TCP_EVENT_ERR(last_state, errf, arg, err) do { \
...
#define LWIP_TCP_OPT_LENGTH(flags) (flags & TF_SEG_OPTS_MSS ? LWIP_TCP_OPT_LEN_MSS : 0) + \
...
#define TCP_REG_ACTIVE(npcb)		do { \
									TCP_REG(&tcp_active_pcbs, npcb); \
									tcp_active_pcbs_changed = 1; \
									} while (0)

#define TCP_RMV_ACTIVE(npcb)		do { \
									TCP_RMV(&tcp_active_pcbs, npcb); \
									tcp_active_pcbs_changed = 1; \
									} while (0)

#define TCP_PCB_REMOVE_ACTIVE(pcb)		do { \
...
void tcp_rst(u32_t seqno, u32_t ackno, const ip_addr_t *local_ip, const ip_addr_t *remote_ip,
