...
#define ip_addr_set_loopback(is_ipv6, ipaddr) do { if (is_ipv6) { \
												...
												IP_SET_TYPE(ipaddr, IPADDR_TYPE_V6); } else { \
...
#define ip_addr_set_hton(dest, src)  do { if (IP_IS_V6(src)) { \
										...
										IP_SET_TYPE(dest, IPADDR_TYPE_V6); } else { \
...
#define ip_addr_get_network(target, host, netmask) do { if (IP_IS_V6(host)) { \
													...
													IP_SET_TYPE(target, IPADDR_TYPE_V6); } else { \
													ip4_addr_get_network(ip_2_ip4(target), ip_2_ip4(host), ip_2_ip4(netmask));\
													IP_SET_TYPE(target, IPADDR_TYPE_V4); } \
													} while (0)

/** @ingroup ipaddr */
#define ip_addr_netcmp(addr1, addr2, mask) ((IP_IS_V6(addr1) && IP_IS_V6(addr2)) ? 0 : \
...
/** @ingroup ipaddr */
#define ip_addr_isbroadcast(ipaddr, netif) ((IP_IS_V6(ipaddr)) ? 0 : \
