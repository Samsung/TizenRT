...
typedef enum {
	...
	HAL_ALLOC_FAIL,
	HAL_KEY_IN_USE,	// key slot is already in use (don't overwrite)
	HAL_CERT_IN_USE, // cert slot is already in use (don't overwrite)
	HAL_DATA_IN_USE, // data slot is already in use (don't overwrite)
	HAL_NOT_SUPPORTED,
	HAL_NOT_IMPLEMENTED,
	HAL_BUSY,
	HAL_FAIL,
 ...
 *
 * @param   params   A pointer to the parameter structure for SE initialization.
 *                   - i2c_port: The I2C port number to be used.
 *                   - gpio: The GPIO pin number to be used.
 *
 * @return  HAL_SUCCESS: Successfully initialized.
 *          HAL_NOT_INITIALIZED: Required resources for initialization are not ready.
 *          HAL_INVALID_ARGS: Input parameters are invalid.
 *          HAL_FAIL: Other internal errors occurred.
 *
 ...
 *
 * @return HAL_SUCCESS: Successfully deinitialized.
 *         HAL_FAIL: An error occurred during deinitialization.
 *
 ...
 *
 * @param   data    A pointer to the HAL data structure to be freed.
 ...
 *
 * @return  HAL_SUCCESS: Successfully freed memory.
 *          HAL_INVALID_ARGS: Input parameters are invalid.
 *          HAL_FAIL: An error occurred during memory deallocation.
 *
 ...
 *
 * @return  HAL_SUCCESS: SE is in a normal (IDLE) state.
 *          HAL_BUSY: SE is currently in use (BUSY).
 *          HAL_NOT_INITIALIZED: SE has not been initialized.
 *          HAL_FAIL: An error occurred while checking the status.
 *
 ...
 *
 * @param   mode    The type of the key to be stored (refer to hal_key_type enum).
 *                  e.g., HAL_KEY_AES_256, HAL_KEY_RSA_2048, HAL_KEY_ECC_SEC_P256R1
 * @param   key_idx The key index within the SE where the key will be saved.
 * @param   key     The public key(or the key itself for symmetric keys) data to be saved.
 *                  - data: Pointer to the key data buffer.
 *                  - data_len: Length of the key data buffer.
 * @param   prikey  The private key data to be saved (only used for asymmetric keys).
 ...
 * 
 * @return  HAL_SUCCESS: Successfully stored the key.
 *          HAL_KEY_IN_USE: The key_idx slot is already in use.
 *          HAL_INVALID_ARGS: Input parameters (mode, key_idx, key) are invalid.
 *          HAL_INVALID_SLOT_RANGE: key_idx is out of the valid slot range.
 *          HAL_INVALID_SLOT_TYPE: The requested key type cannot be stored in that slot.
 *          HAL_NOT_ENOUGH_MEMORY: Not enough memory to store the key.
 *          HAL_FAIL: Other internal errors occurred.
 *
 ...
 *
 * @param   mode    The type of the key to be retrieved (refer to hal_key_type enum).
 * @param   key_idx The key index within the SE from which the key will be extracted.
 * @param   key     [out] A pointer to the structure where the retrieved public key data will be saved.
 *                  - data: [out] Pointer to the buffer where the public key data will be stored.
 *                          The caller must allocate this buffer.
 *                  - data_len: [out] Length of the retrieved public key data.
 *                  For ECC keys, the X coordinate is stored in key->data and the Y coordinate in key->priv.
 * 
 * @return  HAL_SUCCESS: Successfully retrieved the key.
 *          HAL_EMPTY_SLOT: The key_idx slot is empty.
 *          HAL_INVALID_ARGS: Input parameters (mode, key_idx, key) are invalid.
 *          HAL_INVALID_SLOT_RANGE: key_idx is out of the valid slot range.
 *          HAL_BAD_KEY: The key in the slot does not match the requested type or is corrupted.
 *          HAL_NOT_ENOUGH_MEMORY: The output buffer is insufficient to store the key data.
 *          HAL_FAIL: Other internal errors occurred.
 *
 ...
 *
 * @param   mode    The type of the key to be removed (refer to hal_key_type enum).
 * @param   key_idx The key index within the SE where the key to be removed is stored.
 *
 * @return  HAL_SUCCESS: Successfully removed the key.
 *          HAL_EMPTY_SLOT: The key_idx slot is already empty.
 *          HAL_INVALID_ARGS: Input parameters (mode, key_idx) are invalid.
 *          HAL_INVALID_SLOT_RANGE: key_idx is out of the valid slot range.
 *          HAL_FAIL: Other internal errors occurred.
 *
 ...
 *
 * @param   mode    The type of the key to be generated (refer to hal_key_type enum).
 *                  e.g., HAL_KEY_AES_256, HAL_KEY_RSA_2048, HAL_KEY_ECC_SEC_P256R1
 * @param   key_idx The key index within the SE where the generated key will be stored.
 *
 * @return  HAL_SUCCESS: Successfully generated and stored the key.
 *          HAL_KEY_IN_USE: The key_idx slot is already in use.
 *          HAL_INVALID_ARGS: Input parameters (mode, key_idx) are invalid.
 *          HAL_INVALID_SLOT_RANGE: key_idx is out of the valid slot range.
 *          HAL_INVALID_SLOT_TYPE: The requested key type cannot be generated in that slot.
 *          HAL_NOT_SUPPORTED: Generation of that key type is not supported.
 *          HAL_FAIL: An internal error occurred during key generation.
 *
 ...
 *
 * @param   len     The length of the random data to be generated in bytes.
 * @param   random  [out] A pointer to the structure where the generated random data will be stored.
 *                  - data: [out] Pointer to the buffer where the random data will be stored.
 *                          The caller must allocate this buffer.
 *                  - data_len: [out] Length of the generated random data (should be equal to `len`).
 *
 * @return  HAL_SUCCESS: Successfully generated random data.
 *          HAL_INVALID_ARGS: Input parameters (len, random) are invalid.
 *          HAL_NOT_ENOUGH_MEMORY: The output buffer (random->data) is insufficient to store the random data.
 *          HAL_FAIL: An internal error occurred during random number generation.
 *
 ...
 *
 * @param   mode    The hash algorithm to be used (refer to hal_hash_type enum).
 *                  e.g., HAL_HASH_SHA256, HAL_HASH_SHA512
 * @param   input   A pointer to the structure containing the input data to be hashed.
 *                  - data: Pointer to the input data buffer.
 *                  - data_len: Length of the input data buffer.
 * @param   hash    [out] A pointer to the structure where the resulting hash will be stored.
 *                  - data: [out] Pointer to the buffer where the hash will be stored.
 *                          The caller must allocate this buffer. The required size depends on the hash algorithm.
 *                  - data_len: [out] Length of the generated hash.
 *
 * @return  HAL_SUCCESS: Successfully computed the hash.
 *          HAL_INVALID_ARGS: Input parameters (mode, input, hash) are invalid.
 *          HAL_NOT_SUPPORTED: The requested hash algorithm is not supported.
 *          HAL_NOT_ENOUGH_MEMORY: The output buffer (hash->data) is insufficient to store the hash.
 *          HAL_FAIL: An internal error occurred during hashing.
 *
 ...
 *
 * @param   mode    The HMAC algorithm to be used (refer to hal_hmac_type enum).
 *                  e.g., HAL_HMAC_SHA256, HAL_HMAC_SHA512
 * @param   input   A pointer to the structure containing the input data.
 *                  - data: Pointer to the input data buffer.
 *                  - data_len: Length of the input data buffer.
 * @param   key_idx The key index of the secret key to be used for HMAC calculation.
 *                  This key must have been previously stored using `hal_set_key`.
 * 					Also, secret key is basically symmetric key.
 * @param   hmac    [out] A pointer to the structure where the resulting HMAC will be stored.
 *                  - data: [out] Pointer to the buffer where the HMAC will be stored.
 *                          The caller must allocate this buffer. The required size depends on the HMAC algorithm.
 *                  - data_len: [out] Length of the generated HMAC.
 *
 * @return  HAL_SUCCESS: Successfully computed the HMAC.
 *          HAL_INVALID_ARGS: Input parameters (mode, input, key_idx, hmac) are invalid.
 *          HAL_EMPTY_SLOT: The specified key_idx slot is empty.
 *          HAL_BAD_KEY_TYPE: The key in the slot is not suitable for the requested HMAC operation.
 *          HAL_NOT_SUPPORTED: The requested HMAC algorithm is not supported.
 *          HAL_NOT_ENOUGH_MEMORY: The output buffer (hmac->data) is insufficient to store the HMAC.
 *          HAL_FAIL: An internal error occurred during HMAC calculation.
 *
 ...
 *
 * @param   mode    A pointer to the structure defining RSA signing parameters.
 ...
 * @param   hash    A pointer to the structure containing the message digest to be signed.
 *                  - data: Pointer to the message digest buffer.
 *                  - data_len: Length of the message digest buffer.
 * @param   key_idx The key index of the RSA private key to be used for signing.
 * @param   sign    [out] A pointer to the structure where the generated signature will be stored.
 *                  - data: [out] Pointer to the buffer where the signature will be stored.
 *                          The caller must allocate this buffer.
 *                  - data_len: [out] Length of the generated signature.
 *
 * @return  HAL_SUCCESS: Successfully generated the signature.
 *          HAL_INVALID_ARGS: Input parameters (mode, hash, key_idx, sign) are invalid.
 *          HAL_EMPTY_SLOT: The specified key_idx slot is empty.
 *          HAL_BAD_KEY_TYPE: The key in the slot is not an RSA private key.
 *          HAL_NOT_SUPPORTED: The requested RSA signature algorithm or parameters are not supported.
 *          HAL_NOT_ENOUGH_MEMORY: The output buffer (sign->data) is insufficient to store the signature.
 *          HAL_FAIL: An internal error occurred during signing.
 *
 ...
 *
 * @param   mode    A pointer to the structure defining RSA verification parameters.
 *                  - rsa_a: The RSA signature algorithm (e.g., HAL_RSASSA_PKCS1_V1_5).
 *                  - hash_t: The hash algorithm used to create the message digest.
 *                  - mgf: The mask generation function (for PSS).
 *                  - salt_byte_len: The length of the salt in bytes (for PSS).
 * @param   hash    A pointer to the structure containing the message digest.
 *                  - data: Pointer to the message digest buffer.
 *                  - data_len: Length of the message digest buffer.
 * @param   sign    A pointer to the structure containing the signature to be verified.
 *                  - data: Pointer to the signature buffer.
 *                  - data_len: Length of the signature buffer.
 * @param   key_idx The key index of the RSA public key to be used for verification.
 *
 * @return  HAL_SUCCESS: The signature is valid.
 *          HAL_FAIL: The signature is invalid.
 *          HAL_INVALID_ARGS: Input parameters (mode, hash, sign, key_idx) are invalid.
 *          HAL_EMPTY_SLOT: The specified key_idx slot is empty.
 *          HAL_BAD_KEY_TYPE: The key in the slot is not an RSA public key.
 *          HAL_NOT_SUPPORTED: The requested RSA signature algorithm or parameters are not supported.
 *          HAL_BAD_KEY_PAIR: The public key does not match the private key used for signing (if applicable).
 *
 ...
 *
 * @param   mode    A pointer to the structure defining ECDSA signing parameters.
 *                  - curve: The ECDSA curve to be used (e.g., HAL_ECDSA_SEC_P256R1).
 *                  - hash_t: The hash algorithm used to create the message digest.
 * @param   hash    A pointer to the structure containing the message digest to be signed.
 *                  - data: Pointer to the message digest buffer.
 *                  - data_len: Length of the message digest buffer.
 * @param   key_idx The key index of the ECDSA private key to be used for signing.
 * @param   sign    [out] A pointer to the structure where the generated signature will be stored.
 *                  - data: [out] Pointer to the buffer where the signature will be stored.
 ...
 *          HAL_INVALID_ARGS: Input parameters (mode, hash, key_idx, sign) are invalid.
 *          HAL_EMPTY_SLOT: The specified key_idx slot is empty.
 *          HAL_BAD_KEY_TYPE: The key in the slot is not an ECDSA private key or does not match the specified curve.
 *          HAL_NOT_SUPPORTED: The requested ECDSA curve or hash algorithm is not supported.
 *          HAL_NOT_ENOUGH_MEMORY: The output buffer (sign->data) is insufficient to store the signature.
 *          HAL_FAIL: An internal error occurred during signing.
 *
 * @note    The input `hash` is the message digest, not the original message.
 ...
 *
 * @param   mode    A pointer to the structure defining ECDSA verification parameters.
 *                  - curve: The ECDSA curve to be used (e.g., HAL_ECDSA_SEC_P256R1).
 *                  - hash_t: The hash algorithm used to create the message digest.
 * @param   hash    A pointer to the structure containing the message digest.
 *                  - data: Pointer to the message digest buffer.
 ...
 *                  - data_len: Length of the signature buffer.
 * @param   key_idx The key index of the ECDSA public key to be used for verification.
 *
 * @return  HAL_SUCCESS: The signature is valid.
 *          HAL_FAIL: The signature is invalid.
 *          HAL_INVALID_ARGS: Input parameters (mode, hash, sign, key_idx) are invalid.
 *          HAL_EMPTY_SLOT: The specified key_idx slot is empty.
 *          HAL_BAD_KEY_TYPE: The key in the slot is not an ECDSA public key or does not match the specified curve.
 *          HAL_NOT_SUPPORTED: The requested ECDSA curve or hash algorithm is not supported.
 *          HAL_BAD_KEY_PAIR: The public key does not match the private key used for signing.
 *
 * @note    This function verifies the signature against the provided message digest.
 *          The message digest should be computed using the hash algorithm specified in `mode->hash_t`.
 */
 ...
 *
 * @param   dh_idx  The key index where the generated private key X will be stored.
 * @param   dh_param [in/out] A pointer to the structure for DH parameters.
 ...
 *
 * @return  HAL_SUCCESS: Successfully generated DH parameters and public key.
 *          HAL_INVALID_ARGS: Input parameters (dh_idx, dh_param, G, P) are invalid.
 *          HAL_KEY_IN_USE: The dh_idx slot is already in use.
 *          HAL_INVALID_SLOT_RANGE: dh_idx is out of the valid slot range.
 *          HAL_NOT_SUPPORTED: The requested DH key type is not supported.
 *          HAL_NOT_ENOUGH_MEMORY: The output buffer (dh_param->pubkey->data) is insufficient.
 *          HAL_FAIL: An internal error occurred during parameter generation.
 *
 ...
 *
 * @param   dh_param       A pointer to the structure containing DH parameters from the peer.
 ...
 * @param   dh_idx         The key index where the local private key X is stored (generated by `hal_dh_generate_param`).
 * @param   shared_secret  [out] A pointer to the structure where the computed shared secret will be stored.
 *                        - data: [out] Pointer to the buffer where the shared secret (G^XY mod P) will be stored.
 *                                The caller must allocate this buffer.
 *                        - data_len: [out] Length of the computed shared secret.
 *
 * @return  HAL_SUCCESS: Successfully computed the shared secret.
 *          HAL_INVALID_ARGS: Input parameters (dh_param, dh_idx, shared_secret) are invalid.
 *          HAL_EMPTY_SLOT: The specified dh_idx slot is empty (local private key X not found).
 *          HAL_BAD_KEY_TYPE: The key in the slot is not a DH key or does not match dh_param->mode.
 *          HAL_NOT_SUPPORTED: The DH operation is not supported.
 *          HAL_NOT_ENOUGH_MEMORY: The output buffer (shared_secret->data) is insufficient.
 *          HAL_FAIL: An internal error occurred during shared secret computation.
 *
 ...
 *
 * @param   ecdh_mode      A pointer to the structure defining ECDH parameters and the peer's public key.
 *                        - curve: The ECDSA curve to be used (e.g., HAL_ECDSA_SEC_P256R1).
 ...
 * @param   key_idx        The key index of the local ECDH private key.
 * @param   shared_secret  [out] A pointer to the structure where the computed shared secret will be stored.
 *                        - data: [out] Pointer to the buffer where the shared secret will be stored.
 *                                The caller must allocate this buffer.
 *                        - data_len: [out] Length of the computed shared secret.
 *
 * @return  HAL_SUCCESS: Successfully computed the shared secret.
 *          HAL_INVALID_ARGS: Input parameters (ecdh_mode, key_idx, shared_secret) are invalid.
 *          HAL_EMPTY_SLOT: The specified key_idx slot is empty (local private key not found).
 *          HAL_BAD_KEY_TYPE: The key in the slot is not an ECDH private key or does not match ecdh_mode->curve.
 *          HAL_NOT_SUPPORTED: The ECDH operation for the specified curve is not supported.
 *          HAL_NOT_ENOUGH_MEMORY: The output buffer (shared_secret->data) is insufficient.
 *          HAL_FAIL: An internal error occurred during shared secret computation.
 *
 ...
 *
 * @param   cert_idx The certificate index within the SE where the certificate will be saved.
 * @param   cert_in  A pointer to the structure containing the certificate data to be stored.
 ...
 *
 * @return  HAL_SUCCESS: Successfully stored the certificate.
 *          HAL_CERT_IN_USE: The cert_idx slot is already in use.
 *          HAL_INVALID_ARGS: Input parameters (cert_idx, cert_in) are invalid.
 *          HAL_INVALID_SLOT_RANGE: cert_idx is out of the valid certificate slot range.
 *          HAL_BAD_CERT: The provided certificate data is corrupted or malformed.
 *          HAL_NOT_ENOUGH_MEMORY: Not enough memory to store the certificate.
 *          HAL_FAIL: Other internal errors occurred.
 *
 ...
 *
 * @param   cert_idx The certificate index within the SE from which the certificate will be retrieved.
 * @param   cert_out [out] A pointer to the structure where the retrieved certificate data will be saved.
 *                  - data: [out] Pointer to the buffer where the certificate data will be stored.
 *                          The caller must allocate this buffer.
 *                  - data_len: [out] Length of the retrieved certificate data.
 *
 * @return  HAL_SUCCESS: Successfully retrieved the certificate.
 *          HAL_EMPTY_SLOT: The cert_idx slot is empty.
 *          HAL_INVALID_ARGS: Input parameters (cert_idx, cert_out) are invalid.
 *          HAL_INVALID_SLOT_RANGE: cert_idx is out of the valid certificate slot range.
 *          HAL_BAD_CERT: The certificate in the slot is corrupted.
 *          HAL_NOT_ENOUGH_MEMORY: The output buffer (cert_out->data) is insufficient to store the certificate.
 *          HAL_FAIL: Other internal errors occurred.
 */
 ...
 *
 * @param   cert_idx The certificate index within the SE where the certificate to be removed is stored.
 *
 * @return  HAL_SUCCESS: Successfully removed the certificate.
 *          HAL_EMPTY_SLOT: The cert_idx slot is already empty.
 *          HAL_INVALID_ARGS: Input parameter (cert_idx) is invalid.
 *          HAL_INVALID_SLOT_RANGE: cert_idx is out of the valid certificate slot range.
 *          HAL_FAIL: Other internal errors occurred.
 *
 ...
 *
 * @param   dec_data    A pointer to the structure containing the plaintext data to be encrypted.
 ...
 * @param   aes_param   A pointer to the structure defining AES encryption parameters.
 ...
 * @param   key_idx     The key index of the AES key to be used for encryption.
 * @param   enc_data    [out] A pointer to the structure where the resulting ciphertext will be stored.
 *                      - data: [out] Pointer to the buffer where the ciphertext will be stored.
 *                              The caller must allocate this buffer.
 *                      - data_len: [out] Length of the resulting ciphertext.
 *                      The `priv` and `priv_len` fields are not used for this parameter.
 *
 * @return  HAL_SUCCESS: Successfully encrypted the data.
 *          HAL_INVALID_ARGS: Input parameters (dec_data, aes_param, key_idx, enc_data) are invalid.
 *          HAL_EMPTY_SLOT: The specified key_idx slot is empty.
 *          HAL_BAD_KEY_TYPE: The key in the slot is not an AES key or does not match the operation.
 *          HAL_NOT_SUPPORTED: The requested AES mode or parameters are not supported.
 *          HAL_NOT_ENOUGH_MEMORY: The output buffer (enc_data->data) is insufficient.
 *          HAL_FAIL: An internal error occurred during encryption.
 *
 ...
 *
 * @param    enc_data    A pointer to the structure containing the ciphertext data to be decrypted.
 *                      - data: Pointer to the ciphertext data buffer.
 *                      - data_len: Length of the ciphertext data buffer.
 *                      The `priv` and `priv_len` fields are not used for this parameter.
 * @param    aes_param   A pointer to the structure defining AES decryption parameters.
 *                      - mode: The AES algorithm mode (e.g., HAL_AES_CBC_NOPAD, HAL_AES_CTR).
 *                      - iv: Pointer to the initialization vector (IV) buffer.
 *                      - iv_len: Length of the IV buffer.
 *                      - iv_offset: Pointer to the current offset in the IV (for CFB mode).
 ...
 *                      - nonce_counter: Pointer to the 128-bit nonce and counter buffer (for CTR mode).
 *                      - stream_block: Pointer to the saved stream block for resuming (for CTR mode).
 * @param    key_idx     The key index of the AES key to be used for decryption.
 * @param    dec_data    [out] A pointer to the structure where the resulting plaintext will be stored.
 *                      - data: [out] Pointer to the buffer where the plaintext will be stored.
 *                              The caller must allocate this buffer.
 *                      - data_len: [out] Length of the resulting plaintext.
 *                      The `priv` and `priv_len` fields are not used for this parameter.
 *
 * @return   HAL_SUCCESS: Successfully decrypted the data.
 *          HAL_INVALID_ARGS: Input parameters (enc_data, aes_param, key_idx, dec_data) are invalid.
 *          HAL_EMPTY_SLOT: The specified key_idx slot is empty.
 *          HAL_BAD_KEY_TYPE: The key in the slot is not an AES key or does not match the operation.
 *          HAL_NOT_SUPPORTED: The requested AES mode or parameters are not supported.
 *          HAL_NOT_ENOUGH_MEMORY: The output buffer (dec_data->data) is insufficient.
 *          HAL_FAIL: An internal error occurred during decryption.
 *
 * @note    The `hal_aes_param` structure may be updated by this function depending on the `aes_param->mode`.
 *          - If `aes_param->mode` is HAL_AES_CBC_NOPAD, `aes_param->iv` will be updated.
 *          - If `aes_param->mode` is HAL_AES_CTR, `aes_param->nc_off`, `aes_param->nonce_counter`, and `aes_param->stream_block` will be updated.
 ...
 * @param    mode        A pointer to the structure defining RSA encryption parameters.
 *                      - rsa_a: The RSA encryption algorithm
 *                      - hash_t: The hash algorithm
 *                      - mgf: The mask generation function
 *                      - salt_byte_len: The length of the salt in bytes
 * @param    key_idx     The key index of the RSA public key to be used for encryption.
 * @param    enc_data    [out] A pointer to the structure where the resulting ciphertext will be stored.
 *                      - data: [out] Pointer to the buffer where the ciphertext will be stored.
 *                              The caller must allocate this buffer. The size should be at least the RSA key modulus size (e.g., 256 bytes for RSA-2048).
 *                      - data_len: [out] Length of the resulting ciphertext (typically the RSA key modulus size).
 *
 * @return   HAL_SUCCESS: Successfully encrypted the data.
 *          HAL_INVALID_ARGS: Input parameters (dec_data, mode, key_idx, enc_data) are invalid.
 *          HAL_EMPTY_SLOT: The specified key_idx slot is empty.
 *          HAL_BAD_KEY_TYPE: The key in the slot is not an RSA public key.
 *          HAL_NOT_SUPPORTED: The requested RSA encryption algorithm or parameters are not supported.
 *          HAL_NOT_ENOUGH_MEMORY: The output buffer (enc_data->data) is insufficient.
 *          HAL_FAIL: An internal error occurred during encryption.
 *
 ...
 * @param    mode        A pointer to the structure defining RSA decryption parameters.
 *                      - rsa_a: The RSA encryption algorithm
 *                      - hash_t: The hash algorithm
 *                      - mgf: The mask generation function
 *                      - salt_byte_len: The length of the salt in bytes
 * @param    key_idx     The key index of the RSA private key to be used for decryption.
 * @param    dec_data    [out] A pointer to the structure where the resulting plaintext will be stored.
 *                      - data: [out] Pointer to the buffer where the plaintext will be stored.
 *                              The caller must allocate this buffer.
 *                      - data_len: [out] Length of the resulting plaintext.
 *
 * @return   HAL_SUCCESS: Successfully decrypted the data.
 *          HAL_INVALID_ARGS: Input parameters (enc_data, mode, key_idx, dec_data) are invalid.
 *          HAL_EMPTY_SLOT: The specified key_idx slot is empty.
 *          HAL_BAD_KEY_TYPE: The key in the slot is not an RSA private key.
 *          HAL_NOT_SUPPORTED: The requested RSA decryption algorithm or parameters are not supported.
 *          HAL_NOT_ENOUGH_MEMORY: The output buffer (dec_data->data) is insufficient.
 *          HAL_FAIL: An internal error occurred during decryption.
 *
 ...
 *                      - data_len: Length of the plaintext data buffer.
 *                      The `priv` and `priv_len` fields are not used for this parameter.
 * @param   gcm_param   [in/out] A pointer to the structure defining GCM encryption parameters.
 ...
 * @param   key_idx     The key index of the AES key to be used for GCM encryption.
 * @param   enc_data    [out] A pointer to the structure where the resulting ciphertext will be stored.
 *                      - data: [out] Pointer to the buffer where the ciphertext will be stored.
 *                              The caller must allocate this buffer.
 *                      - data_len: [out] Length of the resulting ciphertext.
 *                      The `priv` and `priv_len` fields are not used for this parameter.
 *
 * @return  HAL_SUCCESS: Successfully encrypted the data and generated the authentication tag.
 *          HAL_INVALID_ARGS: Input parameters (dec_data, gcm_param, key_idx, enc_data) are invalid.
 *          HAL_EMPTY_SLOT: The specified key_idx slot is empty.
 *          HAL_BAD_KEY_TYPE: The key in the slot is not an AES key.
 *          HAL_NOT_SUPPORTED: The requested GCM cipher type is not supported (currently only AES is supported).
 *          HAL_NOT_ENOUGH_MEMORY: The output buffer (enc_data->data) or tag buffer (gcm_param->tag) is insufficient.
 *          HAL_FAIL: An internal error occurred during encryption.
 *
 * @note    GCM is an authenticated encryption mode that provides both confidentiality and integrity.
 *          - Before encryption: `gcm_param->cipher`, `gcm_param->iv`, `gcm_param->iv_len`, `gcm_param->aad`, `gcm_param->aad_len` must be set.
 ...
 *                      - data_len: Length of the ciphertext data buffer.
 *                      The `priv` and `priv_len` fields are not used for this parameter.
 * @param   gcm_param   [in/out] A pointer to the structure defining GCM decryption parameters.
 *                      - cipher: The cipher type (e.g., HAL_GCM_AES).
 ...
 * @param   key_idx     The key index of the AES key to be used for GCM decryption.
 * @param   dec_data    [out] A pointer to the structure where the resulting plaintext will be stored.
 *                      - data: [out] Pointer to the buffer where the plaintext will be stored.
 *                              The caller must allocate this buffer.
 *                      - data_len: [out] Length of the resulting plaintext.
 *                      The `priv` and `priv_len` fields are not used for this parameter.
 *
 * @return  HAL_SUCCESS: Successfully decrypted the data and verified the authentication tag.
 *          HAL_FAIL: Decryption failed or authentication tag verification failed.
 *          HAL_INVALID_ARGS: Input parameters (enc_data, gcm_param, key_idx, dec_data) are invalid.
 *          HAL_EMPTY_SLOT: The specified key_idx slot is empty.
 *          HAL_BAD_KEY_TYPE: The key in the slot is not an AES key.
 *          HAL_NOT_SUPPORTED: The requested GCM cipher type is not supported (currently only AES is supported).
 *          HAL_NOT_ENOUGH_MEMORY: The output buffer (dec_data->data) is insufficient.
 ...
 *          - Before decryption: `gcm_param->cipher`, `gcm_param->iv`, `gcm_param->iv_len`, `gcm_param->aad`, `gcm_param->aad_len`, `gcm_param->tag`, `gcm_param->tag_len` must be set.
 ...
 *
 * @param   ss_idx  The secure storage index within the SE where the data will be written.
 * @param   data    A pointer to the structure containing the data to be written.
 *                  - data: Pointer to the data buffer to be written.
 *                  - data_len: Length of the data buffer.
 *
 * @return   HAL_SUCCESS: Successfully wrote the data to secure storage.
 *          HAL_DATA_IN_USE: The ss_idx slot is already in use.
 *          HAL_INVALID_ARGS: Input parameters (ss_idx, data) are invalid.
 *          HAL_INVALID_SLOT_RANGE: ss_idx is out of the valid secure storage slot range.
 *          HAL_NOT_ENOUGH_MEMORY: Not enough memory in the SE's secure storage to write the data.
 *          HAL_FAIL: Other internal errors occurred.
 *
 ...
 *
 * @param   ss_idx  The secure storage index within the SE from which the data will be read.
 * @param   data    [out] A pointer to the structure where the retrieved data will be stored.
 *                  - data: [out] Pointer to the buffer where the data from secure storage will be stored.
 *                          The caller must allocate this buffer.
 *                  - data_len: [out] Length of the retrieved data.
 *
 * @return   HAL_SUCCESS: Successfully read the data from secure storage.
 *          HAL_EMPTY_SLOT: The ss_idx slot is empty.
 *          HAL_INVALID_ARGS: Input parameters (ss_idx, data) are invalid.
 *          HAL_INVALID_SLOT_RANGE: ss_idx is out of the valid secure storage slot range.
 *          HAL_NOT_ENOUGH_MEMORY: The output buffer (data->data) is insufficient to store the data.
 *          HAL_FAIL: Other internal errors occurred.
 */
 ...
 *
 * @param   ss_idx  The secure storage index within the SE where the data to be deleted is stored.
 *
 * @return   HAL_SUCCESS: Successfully deleted the data from secure storage.
 *          HAL_EMPTY_SLOT: The ss_idx slot is already empty.
 *          HAL_INVALID_ARGS: Input parameter (ss_idx) is invalid.
 *          HAL_INVALID_SLOT_RANGE: ss_idx is out of the valid secure storage slot range.
 *          HAL_FAIL: Other internal errors occurred.
 *
