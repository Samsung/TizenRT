...
typedef enum {
	...
	LWNL_EVT_BLE_SCAN_STATE = 100,
	LWNL_EVT_BLE_COMMON_WATCHDOG_RESET = 200,
} lwnl_cb_ble;
...
typedef struct {
	...
	void (*trble_device_disconnected_cb)(trble_conn_handle conn_id, uint16_t cause);
...
typedef trble_result_e (*trble_set_multi_adv_type)(struct bledev *dev, uint8_t adv_handle, uint8_t adv_event_prop, trble_addr *addr);
