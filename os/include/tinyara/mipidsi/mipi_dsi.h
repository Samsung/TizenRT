...
struct mipi_dsi_host_ops {
	...
	CODE int (*attach)(FAR struct mipi_dsi_host *host, FAR struct mipi_dsi_device *device);

	/* Detach DSI device from DSI host */

	CODE int (*detach)(FAR struct mipi_dsi_host *host, FAR struct mipi_dsi_device *device);
...
struct mipi_dsi_device_ops {
	CODE int (*prepare)(FAR struct mipi_dsi_device *device, struct mipi_dsi_msg *msg);
	CODE int (*enable)(FAR struct mipi_dsi_device *device);
	CODE int (*disable)(FAR struct mipi_dsi_device *device);
};
...
struct mipi_dsi_device {
	...
	uint32_t lp_rate;			/* Maximum lane frequency for low power
						 * mode in hertz,  this shoud be set
						 * to the real limits of the hardware. */
	FAR const struct mipi_dsi_device_ops *ops;
};
