...
struct pm_metric_domain_s {
	...
	uint32_t blocking_board_sleep_counts[CONFIG_PM_NDOMAINS];    /* It counts the suspended domain inside idle thread which prevent board sleep */
...
struct pm_metric_s {
	...
	uint32_t total_try_counts;                                  /* It counts no. of pm tries to make board sleep */
};
...
static void pm_print_metrics(double total_time, int n_domains)
{
	int index;
	pmdbg("\n");
	pmdbg("TOTAL TIME = %dms\n", TICK2MSEC((int)total_time));
	pmdbg("*TOTAL TIME represents the actual time of measurement. It is always greater than given time cause of system overhead.\n");
	pmdbg("\n");
	pmdbg("TOTAL TRY COUNTS = %d\n", g_pm_metrics->total_try_counts);
	pmdbg("*TOTAL TRY COUNTS represents the total no. of times PM tries to make board sleep.\n");
	pmdbg("\n");
	pmdbg("\n");
	pmdbg("              DOMAIN              |   TOTAL SUSPEND TIME   | BLOCKING BOARD SLEEP COUNTS \n");
	pmdbg("----------------------------------|------------------------|-----------------------------\n");
	for (index = 0; index < n_domains; index++) {
		pmdbg(" %32s | %10dms (%6.2f%%) | %17d (%6.2f%%) \n", pm_domain_map[index], TICK2MSEC(g_pm_metrics->domain_metrics.suspend_ticks[index]),\
		((double)g_pm_metrics->domain_metrics.suspend_ticks[index]) * 100.0 / total_time, g_pm_metrics->domain_metrics.blocking_board_sleep_counts[index],\
		((double)g_pm_metrics->domain_metrics.blocking_board_sleep_counts[index]) * 100.0 / ((double)g_pm_metrics->total_try_counts));
	}
	pmdbg("\n");
	pmdbg("*BLOCKING BOARD SLEEP COUNTS represents the no. of times board was not able to sleep due to suspended domain.\n");
	pmdbg("*BLOCKING BOARD SLEEP COUNTS (%) represents percentage of fail tries to make board sleep cause of suspended domain or errors.\n");
	pmdbg("*BLOCKING BOARD SLEEP COUNTS (%) = (BLOCKING BOARD SLEEP COUNTS / TOTAL TRY COUNTS) * 100.\n");
	pmdbg("*TOTAL SUSPEND TIME (%) = (TOTAL SUSPEND TIME / TOTAL TIME) * 100.\n");
	pmdbg("\n");
	pmdbg("\n");
	...
	if (g_pm_metrics_running && (g_pmglobals.recommended != PM_SLEEP)) {
		...
		for (index = 0; index < CONFIG_PM_NDOMAINS; index++) {
			if (g_pmglobals.suspend_count[index] != 0) {
				g_pm_metrics->domain_metrics.blocking_board_sleep_counts[index]++;
			}
		}
	}
}
