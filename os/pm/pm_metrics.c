...
static void pm_print_metrics(double total_time, int n_domains)
{
	...
	pmdbg("-------------|----------|------------------------\n");
	for (pm_state = PM_FOREGROUND; pm_state < PM_SLEEP; pm_state++) {
		pmdbg(" %11s | %8s | %10dms (%6.2f%%) \n", ((pm_state == PM_FOREGROUND) ? "WAKEUP" : ""), pm_state_name[pm_state], TICK2MSEC(g_pm_metrics->state_metrics.state_accum_ticks[pm_state]),
