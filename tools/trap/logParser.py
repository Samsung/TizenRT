...
class logParser:
	...
	def __init__(self, dump_file = None, elf = None, log_file = None, bin_path = None, config_path= None, xip_enabled = False, have_ram_kernel_text = False):
	...
	def print_trap_output_header(self):
		print('')
		print('')
		print('*************************************************************')
		print('dump_file                   :', self.dump_file)
		print('log_file                    :', self.log_file)
		if (self.g_app_idx > 0):
			print('Number of binaries          : %d [kernel + apps]' % (self.g_app_idx + 1))
		else:
			print('Number of binary            : 1 [kernel]')
		print('"kernel" elf_file           :', self.elf)
		for app_idx in range(self.g_app_idx):
			print('"app%d" elf_file             :' % (app_idx + 1), self.bin_path + self.app_name[app_idx] + '_dbg ')
		print('*************************************************************')
		print('')
				...
				if current_line == stack_details and 'up_stackdump' in line:
					...
					for sub_word in t:
						...
						if (self.is_kernel_text_address(hex(stack_val))):
							...
							utils.print_symbol(stack_addr, stack_val, 0)
						...
						if (is_app_symbol):
							...
							utils.print_symbol(stack_addr, stack_val, is_app_symbol)
	...
	def parse_log(self):
		...
		utils.setup_symbol_table(self.bin_path + "System.map", 0)
		for idx in range(self.g_app_idx):
			os.system("nm --defined-only -l --numeric-sort " + self.bin_path + self.app_name[idx] + "_dbg > " + self.bin_path + self.app_name[idx] + ".map")
