...
def usage():
    ...
    print('---------------------------------------------------------')
    print('python3 ramdumpParser.py -t log.txt ')
    print('')
    print('Below example if you give binary file other than default as path: ')
    print('---------------------------------------------------------')
    print('python3 ramdumpParser.py -t log.txt -b ../../os/vidisha/bin/ -c ../../os/cfile -e tinayara.axf')
    print('')
    print('')
    print('Note:')
    print('--------')
...
def main():
	...
	try:
		...
		lparser = logParser(dump_file = dump_file, elf = elf, log_file = log_file, bin_path = BIN_PATH, config_path = CONFIG_PATH, xip_enabled = xip_enabled, have_ram_kernel_text =have_ram_kernel_text)
		...
		lparser.print_trap_output_header()
		
		# Calling the Constructor with the initial set of arguments
		rParser = dumpParser(dump_file = dump_file, elf = elf, log_file = log_file, bin_path = BIN_PATH, config_path = CONFIG_PATH, have_ram_kernel_text = have_ram_kernel_text, debug=False)
		...
		if log_file is not None:
			rParser.parse_log_file(log_file,debug=True)
			return None
