...
class dumpParser:
	...
	def __init__(self, dump_file = None, elf = None, log_file = None, bin_path = None, config_path = None, have_ram_kernel_text = False, debug = False):
	...
	def parse_log_file(self,log_file,debug=False):
		k = 1
		i = 0
		got = 0
		offset = 0
			...
			for line in searchfile:
				...
				if  current_line == register_dump:
					...
					if word[1] == ' R8': # Observe the space before R8
						...
						if (not(pc >= self.g_stext_flash and pc <= self.g_etext_flash)):
							if (self.have_ram_kernel_text):
								if (not(pc >= self.g_stext_ram and pc <= self.g_etext_ram)):
