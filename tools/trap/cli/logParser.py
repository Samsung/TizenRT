...
class logParser:
	...
	def parse_assert_info(self):
		...
		with open(self.log_file) as searchfile:
			for line in searchfile:
				...
				if 'IRQ errno:' in line:
					word = line.split(' ')
					irq_errno = word[-1]
					if irq_errno in self.errno_info:
						print('\t- Interrupt errno\t\t\t:', irq_errno, '\t - ' + self.errno_info[irq_errno])
					else:
						print('\t- Interrupt errno\t\t\t:', irq_errno)
				...
				if tcb_info in current_line:
					while '==================' not in line:
						...
						if "State" in line:
							...
							if data in self.errno_info:
								line = line.replace("\n", "") + "\t - " + self.errno_info[data] + "\n"
	...
	# API to parse TCB errno to corresponding symbolic name
	def init_errno_info(self):
		self.errno_info = {
			"0": "OK",
