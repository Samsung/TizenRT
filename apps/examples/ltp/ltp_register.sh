...
ORIGS=$(find ${TEST_SUBDIR} -name "*.c" 2>/dev/null | sort -u)
...
get_os_category() {
    ...
    parent_dir=$(echo "${file_path}" | awk -F "[/]" '{print $(NF-2)}')
    ...
    # For speculative tests the immediate directory is always "speculative",
    # so categorize based on the parent directory (the actual API being tested).
    # Previously all speculative tests were wrongly categorized as "scheduler".
    if [ "${dir_name}" = "speculative" ]; then
        case "${parent_dir}" in
            pthread_cond*)      echo "condvar" ;;
            pthread_mutex*)     echo "pthreads" ;;
            pthread_rwlock*)    echo "pthreads" ;;
            pthread_spin*)      echo "pthreads" ;;
            pthread*)           echo "pthreads" ;;
            timer*|clock*)      echo "timers" ;;
            mq_*)               echo "mqueues" ;;
...
# Default thread count for stress tests requiring a thread count argument.
# Uses CONFIG_EXAMPLES_LTP_DEFAULT_THREADS if set, otherwise defaults to 5.
LTP_DEFAULT_THREADS="${CONFIG_EXAMPLES_LTP_DEFAULT_THREADS:-5}"

# Function to return default arguments for tests that require them.
# Returns "NULL" if no default args are needed.
get_default_args() {
    file_path="$1"
    case "${file_path}" in
        */multi_send_rev_1.c) echo "\"${LTP_DEFAULT_THREADS}\"" ;;
        */multi_send_rev_2.c) echo "\"${LTP_DEFAULT_THREADS}\"" ;;
        */multi_con_pro.c)    echo "\"${LTP_DEFAULT_THREADS}\"" ;;
        *)                    echo "NULL" ;;
    esac
}

# Function to detect if a test needs SIGUSR1 from the timeout mechanism.
#
# A test needs SIGUSR1 if:
# 1. It is a stress test (runs indefinitely until signaled)
# 2. It registers a SIGUSR1 or SIGALRM handler via sigaction() or signal()
#
# Conformance tests are short-lived and complete on their own, so they
# never need SIGUSR1 from the timeout mechanism - even if they register
# signal handlers for their own test logic (e.g., mq_notify uses SIGUSR1
# for message arrival notification, timer tests use SIGALRM).
#
# Returns 1 if the test should receive SIGUSR1 from the timeout mechanism,
# 0 otherwise.
needs_signal() {
    file_path="$1"

    # Only stress tests run indefinitely and need signal-based termination
    case "${file_path}" in
        */stress/*) ;;
        *) echo 0; return ;;
    esac

    # Check if stress test registers SIGUSR1/SIGALRM handler for termination.
    # Use sigaction( or signal( with opening parenthesis to match function calls,
    # not variable names like "signalset" which contain "signal".
    if grep -qE "sigaction\(.*SIGUSR1|sigaction\(.*SIGALRM|signal\(.*SIGUSR1|signal\(.*SIGALRM" "${file_path}" 2>/dev/null; then
        echo 1
    else
        echo 0
    fi
...
typedef struct {
    ...
    const char *default_args;    /* Default args for tests requiring them, NULL otherwise */
    uint8_t needs_signal;        /* 1 if test uses SIGUSR1/SIGALRM for termination */
} ltp_test_entry_t;
...
for f in ${MAINCSRCS}; do
    funcname=$(echo "${f}" | awk -F "[/]" '{if ($(NF-1) == "speculative") print "ltp_"$(NF-2)"_"$(NF-1)"_"$NF; else print "ltp_"$(NF-1)"_"$NF}' | sed 's/\.c$//' | sed 's/-/_/g')_main
...
for f in ${MAINCSRCS}; do
    ...
    # Get default args for this test (NULL if none needed)
    defargs=$(get_default_args "${f}")

    # Check if test uses SIGUSR1/SIGALRM for termination
    sig=$(needs_signal "${f}")

    echo "    {\"${cmdname}\", ${cidx}, ${funcname}, ${defargs}, ${sig}}," >> "${REGISTRY_H}"
