...
static void multi_recv_callback(msg_reply_type_t msg_type, msg_recv_buf_t *recv_data, void *cb_data)
{
	...
	printf("[MICOM] Success to recv multicast msg(nonblock) [%s].\n", (char *)recv_data->buf);
}
...
static int multi_recv_block(int argc, FAR char *argv[])
{
	...
	if (ret < 0) {
		free(msg.buf);
		multicast_fail_cnt++;
		printf("[MICOM] Fail to receive multicast message.\n");
		return ERROR;
	}
...
static int multicast_recv_test(void)
{
	...
	if (recv_pid < 0) {
		multicast_fail_cnt++;
		printf("[MICOM] Fail to create multi_recv_nonblock task. %d\n", errno);
		return ERROR;
	}
	...
	if (recv_pid < 0) {
		multicast_fail_cnt++;
		printf("[MICOM] Fail to create multi_recv_block1 task. %d\n", errno);
		return ERROR;
	}
	...
	if (recv_pid < 0) {
		multicast_fail_cnt++;
		printf("[MICOM] Fail to create multi_recv_block2 task. %d\n", errno);
		return ERROR;
	}
...
static int send_do_next(char *control_msg)
{
	...
	if (ret != OK) {
		printf("[MICOM] Fail to send control msg. : %s\n", control_msg);
		return ERROR;
	}
...
void messaging_test(void)
{
	...
	while (1) {
		...
		if (ret < 0) {
			...
			printf("[MICOM] Fail to create task.\n");
			return;
		}
		sem_wait(&test_sem);
	}
