...
static int decompress_log_data(unsigned char *comp_buf, int total_read, unsigned char **decom_buf, size_t *decomp_size);

static void print_decompressed_data(unsigned char *decom_buf, size_t decomp_size);
...
static int decompress_log_data(unsigned char *comp_buf, int total_read, unsigned char **decom_buf, size_t *decomp_size)
{
	...
	while (remaining >= 4) {
		...
		decomp_data->output_size = block_len * 10; // max 90% Compression Ratio assumtion
		...
		new_decomp = realloc(*decom_buf, *decomp_size + decomp_data->output_size);
		if (!new_decomp) {
			free(decomp_data->output_buffer);
			free(decomp_data);
			ret_val = ERROR;
		...
		*decom_buf = new_decomp;
		memcpy(*decom_buf + *decomp_size, decomp_data->output_buffer, decomp_data->output_size);
