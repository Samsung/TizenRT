...
int ess_process(void)
{
	...
	bool easysetup_complete = false;
        ...
        if (st_things_start() == ST_THINGS_ERROR_NONE) {
                printf("[%s]=====================================================\n", TAG);
                printf("[%s]                    ST_Things_SDK_TC_2 FAIL                    \n", TAG);
                printf("[%s]=====================================================\n", TAG);
        }
        else
        {
	 	printf("[%s]=====================================================\n", TAG);
                printf("[%s]                    ST_Things_SDK_TC_2 PASS                    \n", TAG);
                printf("[%s]=====================================================\n", TAG);
        }

       
        if(st_things_initialize("test_device_def.json", &easysetup_complete) == ST_THINGS_ERROR_NONE)
        {

                 printf("[%s]====ST_Things_SDK_TC_3 FAIL ======================\n", TAG);
        }
        else
        {
                printf("[%s]====ST_Things_SDK_TC_3 PASS ======================\n", TAG);
        }
       

	if(st_things_initialize("device_def.json", &easysetup_complete) == ST_THINGS_ERROR_NONE)
        {

		 printf("[%s]====ST_Things_SDK_TC_1 PASS ======================\n", TAG);
        }
        else
        {
		printf("[%s]====ST_Things_SDK_TC_1 FAIL ======================\n", TAG);
        }
