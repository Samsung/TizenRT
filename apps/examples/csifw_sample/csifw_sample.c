...
static csifw_service_handle handle = 0;
...
static int initialize_csi_service(csi_config_type_t config_type, int interval, callback_type_t cb_type)
{
    ...
    switch (cb_type) {
        ...
        case CB_TYPE_BOTH:
            ...
            break;
        default:
            printf("CSIFW_SAMPLE_APP: Invalid callback type %d\n", cb_type);
            return -1;
    }

    // Register the service with the CSI framework
    CSIFW_RES res = csifw_registerService(&handle, &service_callbacks, config_type, interval);
    if (res != CSIFW_OK) {
...
{
    CSIFW_RES res = csifw_start(handle);
    if (res != CSIFW_OK) {
...
{
    CSIFW_RES res = csifw_stop(handle);
    if (res != CSIFW_OK) {
...
{
    CSIFW_RES res = csifw_unregisterService(handle);
    if (res != CSIFW_OK) {
        printf("CSIFW_SAMPLE_APP: csifw_unregisterService FAIL (result: %d)\n", res);
        return -1;
    }
    handle = 0; // Reset handle value
                        ...
                        int *interval, int *run_duration, int *data_cb_type)
{
    ...
    *data_cb_type = atoi(args[4]);
    ...
    if (*data_cb_type < CB_TYPE_RAW_ONLY || *data_cb_type > CB_TYPE_BOTH) {
        printf("Error: Invalid callback type %d. Valid values are 0, 1, 2\n", *data_cb_type);
        return -1;
    }
