...
{
	usleep(ITC_SEC_10000);
	return 0;
}

/**
* @fn                   :itc_libc_spawn_posix_spawnattr_init_destroy_p
* @brief                :Initializes the given spawn attributes object.
* @scenario             :Initializes and destroy of spawn attr
* @API's covered        :posix_spawnattr_init
* @Preconditions        :none
* @Postconditions       :none
*/
static void itc_libc_spawn_posix_spawnattr_init_destroy_p(void)
{
	int ret_chk = ERROR;
	...
	ret_chk = posix_spawnattr_destroy(&st_attr);
	TC_ASSERT_EQ("posix_spawnattr_destroy", ret_chk, 0);
...
static void itc_libc_spawn_posix_spawnattr_setgetflags_p_all_flag(void)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		ret_chk = posix_spawnattr_setflags(&st_attr, spawn_attr[i]);
		...
		TC_ASSERT_EQ("posix_spawnattr_getflags", flags, spawn_attr[i]);
	}
	TC_SUCCESS_RESULT();
}
...
{
	itc_libc_spawn_posix_spawnattr_init_destroy_p();
