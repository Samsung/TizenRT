...
static void tc_libc_math_gamma(void)
{
	double input[13] = { 2.0, 3.0, INFINITY, 256.5,
						...
						-1.5, 8.1, 8.3, 8.8, 9.3 };
	double compare[13] = { 1, 2, INFINITY, 0x1p1023 * 256.5,
						1 / 1.0e-20, 0, -0.0, 0/0.0,
	...
	for (int i = 0; i < 13; i++) {
		if (i >= 8) {
			 ...
			 */
			TC_ASSERT_EQ("gamma", floor(gamma(input[i]) * digit), floor(compare[i] * digit));
		} else if (i == 7) {
			/**
			 * Compare to NaN
			 */
			TC_ASSERT_EQ("gamma", isnan(gamma(input[i])), isnan(compare[i]));
		} else {
			TC_ASSERT_EQ("gamma", gamma(input[i]), compare[i]);
		}
	}
	TC_SUCCESS_RESULT();
}
