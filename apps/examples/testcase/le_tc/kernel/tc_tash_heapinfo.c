...
/* group_x_size[0] means current heap alloc size of group_x
 * group_x_size[1] means peak heap size of group_x
 * group_x_size[2] means total current alloc size of grou_x
 * group_x_size[3] means total current alloc size of grou_x
 */
int group_a_info[4];
int group_b_info[4];
int group_c_info[4];
...
static void calc_group_size(int *mem, int info[4], int type)
{
	...
	} else if (type == HEAPINFO_FREE) {
		info[0] -= node->size;
		info[2] -= node->size;
	} else if (type == HEAPINFO_STACK_ALLOC) {
		info[2] += node->size;
		if (info[2] > info[3]) {
			info[3] = info[2];
		}
...
static void heapinfo_calc_stack(pid_t pid, int info[4], int type)
{
	...
	if (type == HEAPINFO_STACK_ALLOC) {
		calc_group_size(tcb->stack_alloc_ptr, info, HEAPINFO_STACK_ALLOC);
	} else {
		calc_group_size(tcb->stack_alloc_ptr, info, HEAPINFO_STACK_FREE);
	}
}
...
static int tc_tash_heapinfo(int argc, char **args)
{
	...
	if (argc < 2) {
		...
		task_create("heapinfo_task", SCHED_PRIORITY_MAX, 1024, heapinfo_task, (char * const *)NULL);
		task_create("heapinfo_task2", SCHED_PRIORITY_MAX, 1024, heapinfo_task2, (char * const *)NULL);
		task_create("heapinfo_task3", SCHED_PRIORITY_MAX, 1024, heapinfo_task3, (char * const *)NULL);
		pthread_create(&pid, NULL, heapinfo_pthread, (void *)NULL);
		pthread_setname_np(pid, "heapinfo_pthread");

		sleep(5);
		printf("EXPECTED PEAK ALLOC SIZE AND STACK SIZE PER GROUP\n");
		printf("-------------------------------------------------\n");
