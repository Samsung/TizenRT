...
static void add_dirs(int count)
{
	...
	strcat(folder_name, "/0\0");
	len = strlen(folder_name);
	mkdir(VFS_FOLDER_PATH, 0777);
	for (i = 0; i < count; i++) {
		folder_name[len - 1] = '0' + i;
...
static void itc_fs_vfs_mount_p_read_mode(void)
{
	...
	ret = mount(MOUNT_DEV_DIR, CONFIG_MOUNT_POINT, TARGET_FS_NAME, MS_RDONLY, NULL);
	TC_ASSERT_EQ("mount", ret, OK);

	ret = mkdir(VFS_FOLDER_PATH, 0777);
	TC_ASSERT_EQ_CLEANUP("mkdir", ret, ERROR, rmdir(VFS_FOLDER_PATH); umount(CONFIG_MOUNT_POINT));
...
static void itc_fs_vfs_write_read_p(void)
{
	...
	TC_ASSERT_EQ_CLEANUP("read", strncmp(read_buf, write_buf, 10), 0, close(fd));

	memset(read_buf, 0, BUFFER_LEN);
	ret = read(fd, read_buf, 10);
	TC_ASSERT_EQ_CLEANUP("read", ret, 5, close(fd));
	TC_ASSERT_EQ_CLEANUP("read", strncmp(read_buf, write_buf + 10, 5), 0, close(fd));
...
static void itc_fs_vfs_fsync_p(void)
{
	...
	fd = open(VFS_FILE_PATH, O_WRONLY | O_TRUNC | O_CREAT);
	TC_ASSERT_GEQ("open", fd, 0);
	fd1 = open(VFS_FILE_PATH, O_RDONLY);
	TC_ASSERT_GEQ_CLEANUP("open", fd1, 0, close(fd));

	len = strlen(write_buf);
	ret = write(fd, write_buf, len);
	TC_ASSERT_EQ_CLEANUP("write", ret, len, close(fd); close(fd1));

	ret = fsync(fd);
	TC_ASSERT_GEQ_CLEANUP("fsync", ret, 0, close(fd); close(fd1));

	memset(read_buf, 0, BUFFER_LEN);
	ret = read(fd1, read_buf, BUFFER_LEN);
	TC_ASSERT_EQ_CLEANUP("read", ret, strlen(write_buf), close(fd); close(fd1));
	TC_ASSERT_EQ_CLEANUP("read", strncmp(read_buf, write_buf, ret), 0, close(fd); close(fd1));

	ret = close(fd);
	TC_ASSERT_EQ_CLEANUP("close", ret, OK, close(fd1));
	ret = close(fd1);
	TC_ASSERT_EQ("close", ret, OK);
...
static void itc_fs_vfs_lseek_n(void)
{
	...
	ret = lseek(fd, 0, -1);
	TC_ASSERT_EQ_CLEANUP("lseek", ret, ERROR, close(fd));
...
static void itc_fs_vfs_lseek_p(void)
{
	...
	ret = lseek(fd, 0, SEEK_SET);
	TC_ASSERT_EQ_CLEANUP("lseek", ret, 0, close(fd));

	ret = write(fd, write_buf, 15 );
	TC_ASSERT_EQ_CLEANUP("write", ret, 15, close(fd));

	ret = lseek(fd, 0, SEEK_CUR);
	TC_ASSERT_EQ_CLEANUP("lseek", ret, 15, close(fd));
	...
	TC_ASSERT_EQ_CLEANUP("read", strncmp(read_buf, write_buf, 15), 0, close(fd));
...
static void itc_fs_vfs_pwrite_pread_p(void)
{
	...
	ret = pwrite(-1, write_buf, 10, 0);
	TC_ASSERT_EQ("pwrite", ret, ERROR);

	ret = pread(-1, read_buf, 10, 0);
	TC_ASSERT_EQ("pread", ret, ERROR);

	fd = open(VFS_FILE_PATH, O_WRONLY | O_TRUNC | O_CREAT);
	TC_ASSERT_GEQ("open", fd, 0);

	ret = pwrite(fd, write_buf, 0, 0);
	TC_ASSERT_EQ_CLEANUP("pwrite", ret, 0, close(fd));

	ret = pwrite(fd, write_buf, 10, 0);
	TC_ASSERT_EQ_CLEANUP("pwrite", ret, 10, close(fd));

	ret = lseek(fd, 4, SEEK_SET);
	TC_ASSERT_EQ_CLEANUP("lseek", ret, 4, close(fd));

	ret = pwrite(fd, write_buf + 7, 8, 7);
	TC_ASSERT_EQ_CLEANUP("pwrite", ret, 8, close(fd));
	...
	memset(read_buf, 0, BUFFER_LEN);
	ret = pread(fd, read_buf, 0, 0);
	TC_ASSERT_EQ_CLEANUP("pread", ret, 0, close(fd));

	memset(read_buf, 0, BUFFER_LEN);
	ret = pread(fd, read_buf, 10, 0);
	TC_ASSERT_EQ_CLEANUP("pread", ret, 10, close(fd));
	TC_ASSERT_EQ_CLEANUP("pread", strncmp(read_buf, write_buf, 10), 0, close(fd));
	...
	TC_ASSERT_EQ_CLEANUP("pread", strncmp(read_buf, write_buf + 9, 6), 0, close(fd));
...
static void itc_fs_vfs_mkdir_n(void)
{
	...
	TC_ASSERT_EQ_CLEANUP("mkdir", ret, ERROR, rmdir(folder_name_2); rmdir(folder_name_1));
...
static void itc_fs_vfs_seekdir_telldir_p(void)
{
	...
	dirp = opendir(VFS_FOLDER_PATH);
	TC_ASSERT_NEQ_CLEANUP("opendir", dirp, NULL, remove_dirs(count));

	ret = telldir(dirp);
	TC_ASSERT_EQ_CLEANUP("telldir", ret, 0, closedir(dirp); remove_dirs(count));

	offset = 2;
	seekdir(dirp, offset);
	TC_ASSERT_NEQ_CLEANUP("seekdir", dirp, NULL, closedir(dirp); remove_dirs(count));

	ret = telldir(dirp);
	TC_ASSERT_EQ_CLEANUP("telldir", ret, 2, closedir(dirp); remove_dirs(count));

	st_dirent = readdir(dirp);
	TC_ASSERT_NEQ_CLEANUP("readdir", st_dirent, NULL, closedir(dirp); remove_dirs(count));
	TC_ASSERT_EQ_CLEANUP("readdir", st_dirent->d_name[0], '2', closedir(dirp); remove_dirs(count));

	ret = telldir(dirp);
	TC_ASSERT_EQ_CLEANUP("telldir", ret, 3, closedir(dirp); remove_dirs(count));
	...
	ret = telldir(dirp);
	TC_ASSERT_EQ_CLEANUP("telldir", ret, 4, closedir(dirp); remove_dirs(count));

	st_dirent = readdir(dirp);
	TC_ASSERT_NEQ_CLEANUP("readdir", st_dirent, NULL, closedir(dirp); remove_dirs(count));
	TC_ASSERT_EQ_CLEANUP("readdir", st_dirent->d_name[0], '4', closedir(dirp); remove_dirs(count));

	ret = telldir(dirp);
	TC_ASSERT_EQ_CLEANUP("telldir", ret, 5, closedir(dirp); remove_dirs(count));
	...
	st_dirent = readdir(dirp);
	TC_ASSERT_EQ_CLEANUP("readdir", st_dirent, NULL, closedir(dirp); remove_dirs(count));
	...
	st_dirent = readdir(dirp);
	TC_ASSERT_NEQ_CLEANUP("readdir", st_dirent, NULL, closedir(dirp); remove_dirs(count));
	TC_ASSERT_EQ_CLEANUP("readdir", st_dirent->d_name[0], '0', closedir(dirp); remove_dirs(count));

	ret = telldir(dirp);
	TC_ASSERT_EQ_CLEANUP("telldir", ret, 1, closedir(dirp); remove_dirs(count));
...
static void itc_fs_vfs_closedir_n(void)
{
	...
	ret = closedir(dirp);
	TC_ASSERT_EQ_CLEANUP("closedir", ret, OK, remove_dirs(dir_count));
...
static void itc_libc_stdio_fseek_n(void)
{
	...
	ret = fseek(fp, 0, -1);
	TC_ASSERT_EQ_CLEANUP("fseek", ret, ERROR, fclose(fp));
...
static void itc_libc_stdio_fseek_ftell_p(void)
{
	...
	ret = ftell(fp);
	TC_ASSERT_EQ_CLEANUP("ftell", ret, 0, fclose(fp));
	...
	ret = ftell(fp);
	TC_ASSERT_EQ_CLEANUP("ftell", ret, 15, fclose(fp));
	...
	ret = fseek(fp, -5, SEEK_END);
	TC_ASSERT_EQ_CLEANUP("fseek", ret, OK, fclose(fp));

	ret = ftell(fp);
	TC_ASSERT_EQ_CLEANUP("ftell", ret, 10, fclose(fp));
