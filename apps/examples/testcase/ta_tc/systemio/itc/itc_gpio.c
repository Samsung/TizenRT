...
iotbus_gpio_context_h g_gpio_h;
iotbus_gpio_context_h g_gpio_h2;

int gpio_flag_callback = 0;
int gpio_callback_data = -1;
...
void itc_gpio_set_get_direction_p(void)
{
	...
	for (index = 0; index < ncount; index++) {
		...
		if (ret != 0) {
			SYSIO_ITC_PRINT("\nitc_gpio_set_get_direction: iotbus_gpio_set_direction FAIL for direction : %s\n", ptr_dir[index]);
			check = false;
			continue;
		}
		...
		if (ret != 0) {
			SYSIO_ITC_PRINT("\nitc_gpio_set_get_direction: iotbus_gpio_get_direction FAIL for direction : %s\n", ptr_dir[index]);
			check = false;
			continue;
		}

		if (get_direction != set_direction[index]) {
			SYSIO_ITC_PRINT("\nitc_gpio_set_get_direction: FAIL, set and get values not same for direction : %s\n", ptr_dir[index]);
			check = false;
		}
	}
...
void itc_gpio_set_get_edge_mode_p(void)
{
	...
	for (index = 0; index < ncount; index++) {
		...
		if (ret != 0) {
			SYSIO_ITC_PRINT("\nitc_gpio_set_get_edge_mode: iotbus_gpio_set_edge_mode FAIL for edge mode : %s\n", ptr_edge[index]);
			check = false;
			continue;
		}
		ret = iotbus_gpio_get_edge_mode(g_gpio_h, &get_edge);
		if (ret != 0) {
			SYSIO_ITC_PRINT("\nitc_gpio_set_get_edge_mode: iotbus_gpio_get_edge_mode FAIL for edge mode : %s\n", ptr_edge[index]);
			check = false;
			continue;
		}
		if (get_edge != set_edge[index]) {
			SYSIO_ITC_PRINT("\nitc_gpio_set_get_edge_mode: FAIL, set and get values not same for edge mode : %s\n", ptr_edge[index]);
			check = false;
		}
	}
...
void itc_gpio_set_get_drive_mode_p(void)
{
	...
	for (index = 0; index < ncount; index++) {
		...
		if (ret != 0) {
			check = false;
			SYSIO_ITC_PRINT("\nitc_gpio_set_get_drive_mode: iotbus_gpio_set_drive_mode FAIL for drive : %s\n", ptr_drive[index]);
			continue;
		}
		ret = iotbus_gpio_get_drive_mode(g_gpio_h, &get_drive);
		if (ret != 0) {
			SYSIO_ITC_PRINT("\nitc_gpio_set_get_drive_mode: iotbus_gpio_get_drive_mode FAIL for drive : %s\n", ptr_drive[index]);
			check = false;
			continue;
		}
		if (get_drive != set_drive[index]) {
			SYSIO_ITC_PRINT("\nitc_gpio_set_get_drive_mode: FAIL, set and get values not same for drive : %s\n", ptr_drive[index]);
			check = false;
		}
	}
...
/**
* @testcase         itc_gpio_all_direction_read_write_p
...
* @postcondition    close gpio context
*/
void itc_gpio_all_direction_read_write_p(void)
{
	int ret;
	g_gpio_h = NULL;
	int count_dir;
	...
	for (i = 0; i < count_dir; i++) {
		...
		if (ret != 0) {
			SYSIO_ITC_PRINT("itc_gpio_all_direction_read_write_p: iotbus_gpio_set_direction FAIL for direction %s\n", ptr_dir[i]);
			check = false;
			continue;
		}
		ret = iotbus_gpio_get_direction(g_gpio_h, &get_direction);
		if (ret != 0) {
			SYSIO_ITC_PRINT("itc_gpio_all_direction_read_write_p: iotbus_gpio_get_direction FAIL for direction %s\n", ptr_dir[i]);
			check = false;
			continue;
		}
		if (get_direction != set_direction[i]) {
			SYSIO_ITC_PRINT("itc_gpio_all_direction_read_write_p: FAIL, set and get values not same for direction %s\n", ptr_dir[i]);
			check = false;
		}
	...
	TC_ASSERT_EQ_CLEANUP("itc_gpio_all_direction_read_write_p", check, true, iotbus_gpio_close(g_gpio_h));
	iotbus_gpio_close(g_gpio_h);
	TC_SUCCESS_RESULT();
}

/**
* @testcase         itc_gpio_all_edge_read_write_p
...
* @postcondition    close gpio context
*/
void itc_gpio_all_edge_read_write_p(void)
{
	int ret;
	g_gpio_h = NULL;
	int count_edge;
	...
	for (i = 0; i < count_edge; i++) {
		...
		if (ret != 0) {
			SYSIO_ITC_PRINT("itc_gpio_all_edge_read_write_p: iotbus_gpio_set_edge_mode FAIL for edge %s\n", ptr_edge[i]);
			check = false;
			continue;
		}
		ret = iotbus_gpio_get_edge_mode(g_gpio_h, &get_edge);
		if (ret != 0) {
			SYSIO_ITC_PRINT("itc_gpio_all_edge_read_write_p: iotbus_gpio_get_edge_mode FAIL for edge %s\n", ptr_edge[i]);
			check = false;
			continue;
		}
		if (get_edge != set_edge[i]) {
			SYSIO_ITC_PRINT("itc_gpio_all_edge_read_write_p: FAIL, set and get values not same for edge %s\n", ptr_edge[i]);
			check = false;
		}
	...
	TC_ASSERT_EQ_CLEANUP("itc_gpio_all_edge_read_write_p", check, true, iotbus_gpio_close(g_gpio_h));
	iotbus_gpio_close(g_gpio_h);
	TC_SUCCESS_RESULT();
}

/**
* @testcase         itc_gpio_all_drive_read_write_p
...
* @postcondition    close gpio context
*/
void itc_gpio_all_drive_read_write_p(void)
{
	int ret;
	g_gpio_h = NULL;
	int count_drive;
	...
	for (i = 0; i < count_drive; i++) {
		...
		if (ret != 0) {
			SYSIO_ITC_PRINT("itc_gpio_all_drive_read_write_p: iotbus_gpio_set_drive_mode FAIL for drive : %s\n", ptr_drive[i]);
			check = false;
			continue;
		}
		ret = iotbus_gpio_get_drive_mode(g_gpio_h, &get_drive);
		if (ret != 0) {
			SYSIO_ITC_PRINT("itc_gpio_all_drive_read_write_p: iotbus_gpio_get_drive_mode FAIL for drive : %s\n", ptr_drive[i]);
			check = false;
			continue;
		}
		if (get_drive != set_drive[i]) {
			SYSIO_ITC_PRINT("itc_gpio_all_drive_read_write_p: FAIL, set and get values not same for drive : %s\n", ptr_drive[i]);
			check = false;
		}
	...
	TC_ASSERT_EQ_CLEANUP("itc_gpio_all_drive_read_write_p", check, true, iotbus_gpio_close(g_gpio_h));
	iotbus_gpio_close(g_gpio_h);
	TC_SUCCESS_RESULT();
}

/**
* @testcase         itc_gpio_set_get_direction_edge_drive_mode_p
...
* @postcondition    close gpio context
*/
void itc_gpio_set_get_direction_edge_drive_mode_p(void)
{
	int ret;
	g_gpio_h = NULL;
	int count_dir;
		...
		for (i = 0; i < count_dir; i++) {
			for (j = 0; j < count_mode; j++) {
				for (k = 0; k < count_drive; k++) {
					...
					ret = iotbus_gpio_set_direction(g_gpio_h, set_direction[i]);
					if (ret != 0) {
						SYSIO_ITC_PRINT("itc_gpio_set_get_direction_edge_drive_mode_p: iotbus_gpio_set_direction FAIL for direction %s\n", ptr_dir[i]);
						check = false;
						continue;
					}
					ret = iotbus_gpio_get_direction(g_gpio_h, &get_direction);
					if (ret != 0) {
						SYSIO_ITC_PRINT("itc_gpio_set_get_direction_edge_drive_mode_p: iotbus_gpio_get_direction FAIL for direction %s\n", ptr_dir[i]);
						check = false;
						continue;
					}
					if (get_direction != set_direction[i]) {
						SYSIO_ITC_PRINT("itc_gpio_set_get_direction_edge_drive_mode_p: FAIL, set and get values not same for direction %s\n", ptr_dir[i]);
						check = false;
					}

					ret = iotbus_gpio_set_edge_mode(g_gpio_h, set_edge[j]);
					if (ret != 0) {
						SYSIO_ITC_PRINT("itc_gpio_set_get_direction_edge_drive_mode_p: iotbus_gpio_set_edge_mode FAIL for edge mode : %s\n", ptr_edge[j]);
						check = false;
						continue;
					}
					ret = iotbus_gpio_get_edge_mode(g_gpio_h, &get_edge);
					if (ret != 0) {
						SYSIO_ITC_PRINT("itc_gpio_set_get_direction_edge_drive_mode_p: iotbus_gpio_get_edge_mode FAIL for edge mode : %s\n", ptr_edge[j]);
						check = false;
						continue;
					}
					if (get_edge != set_edge[j]) {
						SYSIO_ITC_PRINT("itc_gpio_set_get_direction_edge_drive_mode_p: FAIL, set and get values not same for edge mode : %s\n", ptr_edge[j]);
						check = false;
					}

					ret = iotbus_gpio_set_drive_mode(g_gpio_h, set_drive[k]);
					if (ret != 0) {
						SYSIO_ITC_PRINT("itc_gpio_set_get_direction_edge_drive_mode_p: iotbus_gpio_set_drive_mode FAIL for drive : %s\n", ptr_drive[k]);
						check = false;
						continue;
					}
					ret = iotbus_gpio_get_drive_mode(g_gpio_h, &get_drive);
					if (ret != 0) {
						SYSIO_ITC_PRINT("itc_gpio_set_get_direction_edge_drive_mode_p: iotbus_gpio_get_drive_mode FAIL for drive : %s\n", ptr_drive[k]);
						check = false;
						continue;
					}
	...
	TC_ASSERT_EQ_CLEANUP("itc_gpio_set_get_direction_edge_drive_mode_p", check, true, iotbus_gpio_close(g_gpio_h));
	iotbus_gpio_close(g_gpio_h);
	TC_SUCCESS_RESULT();
}

/**
* @testcase         itc_gpio_open_close_different_pin_p
* @brief            gpio_open returns handle of gpio context and gpio_close closes the context
...
* @postcondition    none
*/
void itc_gpio_open_close_different_pin_p(void)
{
...
/**
* @testcase         itc_gpio_get_pin_after_close_n
* @brief            gpio_open returns handle of gpio context and gpio_close closes the context
...
* @postcondition    none
*/
void itc_gpio_get_pin_after_close_n(void)
{
	int gpio_pin = 41;
	int ret;
	g_gpio_h = NULL;
...
/**
* @testcase         itc_gpio_get_pin_after_invalid_open_n
* @brief            gpio_open returns handle of gpio context and gpio_close closes the context
* @scenario         initializes gpio_context based on gpio pin and then closes the gpio_context, then get gpio pin
* @apicovered       iotbus_gpio_get_pin
* @precondition     none
* @postcondition    none
*/
void itc_gpio_get_pin_after_invalid_open_n(void)
{
	int ret;
...
/**
* @testcase         itc_gpio_read_after_close_n
...
* @postcondition    close gpio context
*/
void itc_gpio_read_after_close_n(void)
{
	int ret;
	int gpio_pin = 41;
	g_gpio_h = NULL;
...
/**
* @testcase         itc_gpio_write_after_close_n
...
* @postcondition    close gpio context
*/
void itc_gpio_write_after_close_n(void)
{
	int ret;
	int gpio_pin = 41;
	g_gpio_h = NULL;
...
/**
* @testcase         itc_gpio_set_direction_n
...
* @postcondition    close gpio context
*/
static void itc_gpio_set_direction_n(void)
{
	int gpio_pin = 41;
	int ret;
	g_gpio_h = NULL;
...
/**
* @testcase         itc_gpio_get_direction_n
...
* @postcondition    close gpio context
*/
static void itc_gpio_get_direction_n(void)
{
	int ret;
	g_gpio_h = NULL;
	iotbus_gpio_direction_e get_direction;
...
/**
* @testcase         itc_gpio_set_direction_after_close_n
* @brief            sets the gpio direction
...
* @postcondition    close gpio context
*/
static void itc_gpio_set_direction_after_close_n(void)
{
	int gpio_pin = 41;
	int ret;
	g_gpio_h = NULL;
...
/**
* @testcase         itc_gpio_get_direction_after_close_n
* @brief            gets the gpio direction
...
* @postcondition    close gpio context
*/
static void itc_gpio_get_direction_after_close_n(void)
{
	int gpio_pin = 41;
	int ret;
	g_gpio_h = NULL;
...
/**
* @testcase         itc_gpio_set_edge_mode_n
...
* @postcondition    close gpio context
*/
static void itc_gpio_set_edge_mode_n(void)
{
	int gpio_pin = 41;
	int ret;
	g_gpio_h = NULL;
...
/**
* @testcase         itc_gpio_get_edge_mode_n
...
* @postcondition    close gpio context
*/
static void itc_gpio_get_edge_mode_n(void)
{
	int ret;
	g_gpio_h = NULL;
	iotbus_gpio_edge_e get_edge;
...
/**
* @testcase         itc_gpio_set_edge_mode_after_close_n
* @brief            sets the gpio edge mode
...
* @postcondition    close gpio context
*/
static void itc_gpio_set_edge_mode_after_close_n(void)
{
	int gpio_pin = 41;
	int ret;
	g_gpio_h = NULL;
...
/**
* @testcase         itc_gpio_get_edge_mode_after_close_n
* @brief            gets the gpio edge mode
...
* @postcondition    close gpio context
*/
static void itc_gpio_get_edge_mode_after_close_n(void)
{
	int gpio_pin = 41;
	int ret;
	g_gpio_h = NULL;
...
/**
* @testcase         itc_gpio_set_drive_mode_n
...
* @postcondition    close gpio context
*/
static void itc_gpio_set_drive_mode_n(void)
{
	int gpio_pin = 41;
	int ret;
	g_gpio_h = NULL;
...
/**
* @testcase         itc_gpio_get_drive_mode_n
...
* @postcondition    close gpio context
*/
static void itc_gpio_get_drive_mode_n(void)
{
	int ret;
	g_gpio_h = NULL;
	iotbus_gpio_drive_e get_drive;
...
/**
* @testcase         itc_gpio_set_drive_mode_after_close_n
* @brief            sets the gpio drive mode
...
* @postcondition    close gpio context
*/
static void itc_gpio_set_drive_mode_after_close_n(void)
{
	int gpio_pin = 41;
	int ret;
	g_gpio_h = NULL;
...
/**
* @testcase         itc_gpio_get_drive_mode_after_close_n
* @brief            gets the gpio drive mode
...
* @postcondition    close gpio context
*/
static void itc_gpio_get_drive_mode_after_close_n(void)
{
	int gpio_pin = 41;
	int ret;
	g_gpio_h = NULL;
...
int itc_gpio_main(void)
{
	...
	itc_gpio_register_unregister_callback_p();//TC FAIL, callback is not invoked
	itc_gpio_all_direction_read_write_p();
	itc_gpio_all_edge_read_write_p();
	itc_gpio_all_drive_read_write_p();
	itc_gpio_set_get_direction_edge_drive_mode_p();
	itc_gpio_open_close_different_pin_p();
	itc_gpio_reopen_p();
	//itc_gpio_reclose_p();//TC FAIL, it should not return 0 on re-closing it. Also remaining TCs donot work after its execution
	itc_gpio_open_n();
	itc_gpio_close_n();
	itc_gpio_get_pin_n();
	itc_gpio_get_pin_after_close_n();//TC FAIL, it should not return opened pin after it has already been close
	itc_gpio_get_pin_after_invalid_open_n();
	itc_gpio_read_after_close_n();
	itc_gpio_write_after_close_n();
	itc_gpio_set_direction_n();
	itc_gpio_get_direction_n();
	itc_gpio_set_direction_after_close_n();
	itc_gpio_get_direction_after_close_n();//TC FAIL, ret value should be less than 0
	itc_gpio_set_edge_mode_n();
	itc_gpio_get_edge_mode_n();
	itc_gpio_set_edge_mode_after_close_n();
	itc_gpio_get_edge_mode_after_close_n();//TC FAIL, ret value should be less than 0
	itc_gpio_set_drive_mode_n();
	itc_gpio_get_drive_mode_n();
	itc_gpio_set_drive_mode_after_close_n();
	itc_gpio_get_drive_mode_after_close_n();//TC FAIL, ret value should be less than 0
