...
* @postcondition    none
*/
static void itc_systemio_iotbus_uart_init_stop_p(void)
{
...
*/
static void itc_systemio_iotbus_uart_set_baudrate_p(void)
{
...
* @postcondition    closes uart_context
*/
static void itc_systemio_iotbus_uart_set_mode_p(void)
{
	int i_bytesize = 8;
	int i_stop_bits = 1;
	int ret = IOTBUS_ERROR_NONE;
	int mode[] = { IOTBUS_UART_PARITY_NONE, IOTBUS_UART_PARITY_EVEN, IOTBUS_UART_PARITY_ODD };
	char *str_mode[] = { "IOTBUS_UART_PARITY_NONE", "IOTBUS_UART_PARITY_EVEN", "IOTBUS_UART_PARITY_ODD" };
	...
	for (index = 0; index < i_modes; index++) {
		...
		if (ret != IOTBUS_ERROR_NONE) {
			SYSIO_ITC_PRINT("iotbus_uart_set_mode failed for MODE ::%s \n", str_mode[index]);
			fail_count++;
		}
	}
	TC_ASSERT_EQ_CLEANUP("itc_systemio_iotbus_uart_set_mode_p", fail_count, 0, iotbus_uart_stop(h_uart));
...
static void itc_systemio_iotbus_uart_set_mode_p_all(void)
{
	...
	int ret = IOTBUS_ERROR_NONE;
	int mode[] = { IOTBUS_UART_PARITY_NONE, IOTBUS_UART_PARITY_EVEN, IOTBUS_UART_PARITY_ODD };
	char mode_str[3][32] = { "IOTBUS_UART_PARITY_NONE", "IOTBUS_UART_PARITY_EVEN", "IOTBUS_UART_PARITY_ODD" };
	int i_modes = sizeof(mode) / sizeof(int);
	int index = 0;
...
* @postcondition    closes uart_context
*/
static void itc_systemio_iotbus_uart_set_flowcontrol_p(void)
{
	int i_size = 4;
	int ret = IOTBUS_ERROR_NONE;
	...
	TC_ASSERT_EQ_CLEANUP("iotbus_uart_set_flowcontrol fail count", count_fail, 0, iotbus_uart_stop(h_uart));
...
* @postcondition    closes uart_context
*/
static void itc_systemio_iotbus_uart_write_read_p(void)
{
	int ret = IOTBUS_ERROR_NONE;
	char sz_input_text[32] = "UART READ/WRITE ITC TESTING!";
	char sz_output_text[32];
...
* @postcondition    closes uart_context
*/
static void itc_systemio_iotbus_uart_flush_p(void)
{
	int ret = IOTBUS_ERROR_NONE;
...
static void itc_systemio_iotbus_uart_write_read_flush_p_multi_handle(void)
{
	int ret = IOTBUS_ERROR_NONE;
	char sz_input_text[32] = "UART READ/WRITE ITC TESTING!";
	char sz_input_text1[32] = "UART READ/WRITE ITC TESTING1!";
	char sz_output_text[32];
	...
	ret = iotbus_uart_write(h_uart1, sz_input_text, sizeof(sz_input_text));
	TC_ASSERT_EQ_CLEANUP("iotbus_uart_write", ret < 0, false, iotbus_uart_stop(h_uart1); iotbus_uart_stop(h_uart2));

	ret = iotbus_uart_write(h_uart2, sz_input_text1, sizeof(sz_input_text1));
	TC_ASSERT_EQ_CLEANUP("iotbus_uart_write", ret < 0, false, iotbus_uart_stop(h_uart1); iotbus_uart_stop(h_uart2));
	...
	TC_ASSERT_EQ_CLEANUP("iotbus_uart_read", strcmp(sz_input_text, sz_output_text), 0, iotbus_uart_stop(h_uart1); iotbus_uart_stop(h_uart2));
