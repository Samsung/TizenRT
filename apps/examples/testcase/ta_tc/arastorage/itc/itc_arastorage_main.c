...
void itc_arastorage_startup_p(void)
{
    ...
    snprintf(g_query, QUERY_LENGTH, "CREATE ATTRIBUTE %s DOMAIN int IN %s;", g_attribute_set[0],
             RELATION_NAME);
    ret = db_exec(g_query);
    TC_ASSERT_EQ("db_exec", ret, DB_OK);

    memset(g_query, 0, QUERY_LENGTH);
    snprintf(g_query, QUERY_LENGTH, "CREATE ATTRIBUTE %s DOMAIN long IN %s;", g_attribute_set[1],
             RELATION_NAME);
    ret = db_exec(g_query);
    TC_ASSERT_EQ("db_exec", ret, DB_OK);

    memset(g_query, 0, QUERY_LENGTH);
    snprintf(g_query, QUERY_LENGTH, "CREATE ATTRIBUTE %s DOMAIN string(32) IN %s;",
             g_attribute_set[2],
             RELATION_NAME);
    ret = db_exec(g_query);
    TC_ASSERT_EQ("db_exec", ret, DB_OK);
    ...
    memset(g_query, 0, QUERY_LENGTH);
    snprintf(g_query, QUERY_LENGTH, "CREATE ATTRIBUTE %s DOMAIN double IN %s;", g_attribute_set[3],
             RELATION_NAME);
    ret = db_exec(g_query);
    ...
    for (index = 0; index < DATA_SET_NUM; index++) {
        memset(g_query, 0, QUERY_LENGTH);
        snprintf(g_query, "INSERT (%d, %ld, \'%s\', %f) INTO %s;", QUERY_LENGTH,
                 g_arastorage_data_set[index].int_value, g_arastorage_data_set[index].long_value,
                 g_arastorage_data_set[index].string_value, g_arastorage_data_set[index].double_value,
                 RELATION_NAME);
        ret = db_exec(g_query);
        TC_ASSERT_EQ("db_exec", ret, 0);
    }
    ...
    for (index = 0; index < DATA_SET_NUM; index++) {
        memset(g_query, 0, QUERY_LENGTH);
        snprintf(g_query, QUERY_LENGTH, "INSERT (%d, %ld, \'%s\') INTO %s;",
                 g_arastorage_data_set[index].int_value, g_arastorage_data_set[index].long_value,
                 g_arastorage_data_set[index].string_value, RELATION_NAME);
        ret = db_exec(g_query);
        TC_ASSERT_EQ("db_exec", ret, DB_OK);
    }
    g_check = true;
...
void itc_arastorage_db_print_header_p(void)
{
    ...
    snprintf(g_query, QUERY_LENGTH, "SELECT %s, %s, %s FROM %s WHERE %s > 5;", g_attribute_set[0],
...
#ifdef CONFIG_ARCH_FLOAT_H
    snprintf(query, QUERY_LENGTH, "SELECT %s, %s, %s, %s FROM %s WHERE %s > 5;",
            g_attribute_set[0],
