...
static void itc_audio_pcm_open_p_with_different_flag(void)
{
	...
	int ret = 0;
	bool check = true;
	unsigned int flag_arr[] = { PCM_IN, PCM_OUT, PCM_MMAP, PCM_NOIRQ, PCM_MONOTONIC };
	const char *ptr_flag[] = { "PCM_IN", "PCM_OUT", "PCM_MMAP", "PCM_NOIRQ", "PCM_MONOTONIC" };
	count = sizeof(flag_arr) / sizeof(flag_arr[0]);

	for (index = 0; index < count; index++) {
		g_pcm = NULL;
		g_pcm = pcm_open(0, 0, flag_arr[index], NULL);
		if (g_pcm == NULL || pcm_get_file_descriptor(g_pcm) <= 0) {
...
/**
* @testcase         itc_audio_pcm_open_p_reopen
* @brief            open and initlaize pcm structure
* @scenario         Open pcm and check its validation
* @apicovered       pcm_open, pcm_close
* @precondition     NA
* @postcondition    NA
*/
static void itc_audio_pcm_open_p_reopen(void)
{
	...
	TC_ASSERT_NEQ("pcm_open", ptr_pcm, NULL);
	TC_ASSERT_GT_CLEANUP("pcm_open", pcm_get_file_descriptor(ptr_pcm), 0, pcm_close(g_pcm));
...
static void itc_audio_pcm_is_ready_n_after_close(void)
{
	...
	TC_ASSERT_NEQ("pcm_is_ready", ret, 0);
...
static void itc_audio_pcm_open_by_name_n(void)
{
	...
	TC_ASSERT_EQ_CLEANUP("pcm_open", ptr_pcm, 0, pcm_close(g_pcm));
...
static int audio_itc_launcher(int argc, char **args)
{
	total_pass = 0;
	total_fail = 0;
...
#ifdef CONFIG_TASH
	tash_cmd_install("audio_itc", audio_itc_launcher, TASH_EXECMD_SYNC);
#else
	audio_itc_launcher(argc, argv);
#endif
