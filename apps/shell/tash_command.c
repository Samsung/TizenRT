...
static int tash_cpu1_off(int argc, char *argv[])
{
	...
	fd= open("/dev/cpu1", O_RDWR);
	if (fd < 0) {
		printf("Cannot open /dev/cpu1 file with fd = %d errno = %d\n", fd, errno);
		return 0;
	}
	...
	{
		printf("IOC_CPU_GET_STATE ioctl failed for /dev/cpu1 : %d\n", errno);
		close(fd);
		return 0;
	}
