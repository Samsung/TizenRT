...
*/
fsm_result_t fsm_32bit_run(const FSM_SPARSE32_DEFINITION_T *p_fsm,	/* The FSM definition */
	...
	while (event != FSM_EVENT_NULL) {
		...
		for (p_transition = p_fsm->p_table[*p_state].p_transitions, p_end_trans = p_fsm->p_table[*p_state + 1].p_transitions;; p_transition++) {
			...
			if (p_transition == p_end_trans) {
				...
				BTLOG_FSM("fsm_32bit_run - failed (state=%u, event=%u)", *p_state, event);
		...
		*p_state = TRANSITION32_FIELD(NEW_STATE, p_transition->trans);
		...
		BTLOG_FSM("fsm_32bit_run (state=[%u->%u], event=%u, action=%u)", orig_state, *p_state, event, action_index);
