...
#define HTTP_LISTENING_HANDLER_STACKSIZE 4096

#ifdef CONFIG_TLS_WITH_SSS
#define HTTP_CLIENT_HANDLER_STACKSIZE    24576
#else
#define HTTP_CLIENT_HANDLER_STACKSIZE    8192
#endif
...
pthread_addr_t http_server_handler(pthread_addr_t arg)
{
	...
	while (server->state == HTTP_SERVER_RUN) {
		...
		if (select(server->listen_fd + 1, &readfds, (fd_set *)0, (fd_set *)0, NULL) < 0) {
			HTTP_LOGE("Error: Fail to listen\n");
		}

		if (FD_ISSET(server->listen_fd, &readfds)) {

			FD_CLR(server->listen_fd, &readfds);
			FD_SET(server->listen_fd, &readfds);

			addrlen = sizeof(struct sockaddr_in);
			sock_fd = accept(*(volatile int *)&server->listen_fd, (struct sockaddr *)&client_addr, &addrlen);
			if (sock_fd < 0) {
			...
			if (setsockopt(sock_fd, SOL_SOCKET, SO_RCVTIMEO, (struct timeval *)&tv, sizeof(struct timeval)) < 0) {
				HTTP_LOGE("Error: Fail to setsockopt\n");
			}

			HTTP_LOGD("Client %d is accepted ipaddr: %d.%d.%d.%d\n", sock_fd, (int)((client_addr.sin_addr.s_addr & 0xFF)), (int)((client_addr.sin_addr.s_addr & 0xFF00) >> 8), (int)((client_addr.sin_addr.s_addr & 0xFF0000) >> 16), (int)((client_addr.sin_addr.s_addr & 0xFF000000) >> 24));
...
int http_server_start(struct http_server_t *server)
{
	...
	for (i = 0; i < HTTP_CONF_MAX_CLIENT_HANDLE; i++) {
		pthread_attr_init(&attr);
		pthread_attr_setschedpolicy(&attr, SCHED_RR);
		pthread_attr_setstacksize(&attr, HTTP_CLIENT_HANDLER_STACKSIZE);
