...
static int wmm_ac_add_ts(struct wpa_supplicant *wpa_s, const u8 *addr, const struct wmm_tspec_element *tspec)
{
	...
	if (wpa_s->tspecs[ac][idx]) {
		wpa_printf(MSG_ERROR, "WMM AC: tspec (ac=%d, dir=%d) already exists!", ac, dir);
		return -1;
	}
	...
	if (dir != WMM_AC_DIR_DOWNLINK) {
		...
		wpa_printf(MSG_DEBUG, "WMM AC: Add TS: addr=" MACSTR " TSID=%u admitted time=%u, ret=%d", MAC2STR(addr), tsid, admitted_time, ret);
		if (ret < 0) {
			os_free(_tspec);
			return -1;
		}
	...
	wpa_msg(wpa_s, MSG_INFO, WMM_AC_EVENT_TSPEC_ADDED "tsid=%d addr=" MACSTR " admitted_time=%d", tsid, MAC2STR(addr), admitted_time);

	return 0;
}

static void wmm_ac_del_ts_idx(struct wpa_supplicant *wpa_s, u8 ac, enum ts_dir_idx dir)
{
	...
	wpa_msg(wpa_s, MSG_INFO, WMM_AC_EVENT_TSPEC_REMOVED "tsid=%d addr=" MACSTR, tsid, MAC2STR(wpa_s->bssid));
...
static void wmm_ac_del_req(struct wpa_supplicant *wpa_s, int failed)
{
	...
	if (failed) {
		wpa_msg(wpa_s, MSG_INFO, WMM_AC_EVENT_TSPEC_REQ_FAILED "tsid=%u", wmm_ac_get_tsid(&req->tspec));
	}
...
static void wmm_ac_addts_req_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_printf(MSG_DEBUG, "Timeout getting ADDTS response (tsid=%d up=%d)", wmm_ac_get_tsid(&addts_req->tspec), wmm_ac_get_user_priority(&addts_req->tspec));

	wmm_ac_del_req(wpa_s, 1);
}

static int wmm_ac_send_addts_request(struct wpa_supplicant *wpa_s, const struct wmm_ac_addts_request *req)
{
	struct wpabuf *buf;
	int ret;

	wpa_printf(MSG_DEBUG, "Sending ADDTS Request to " MACSTR, MAC2STR(req->address));
	...
	ret = wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, req->address, wpa_s->own_addr, wpa_s->bssid, wpabuf_head(buf), wpabuf_len(buf), 0);
	if (ret) {
...
static int wmm_ac_send_delts(struct wpa_supplicant *wpa_s, const struct wmm_tspec_element *tspec, const u8 *address)
{
	struct wpabuf *buf;
	int ret;
	...
	ret = wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, address, wpa_s->own_addr, wpa_s->bssid, wpabuf_head(buf), wpabuf_len(buf), 0);
	if (ret) {
...
static int wmm_ac_find_tsid(struct wpa_supplicant *wpa_s, u8 tsid, enum ts_dir_idx *dir)
{
	int ac;
	enum ts_dir_idx idx;
	...
	for (ac = 0; ac < WMM_AC_NUM; ac++) {
		for (idx = 0; idx < TS_DIR_IDX_COUNT; idx++) {
			if (wpa_s->tspecs[ac][idx] && wmm_ac_get_tsid(wpa_s->tspecs[ac][idx]) == tsid) {
				if (dir) {
					*dir = idx;
				}
				return ac;
...
static struct wmm_ac_addts_request *wmm_ac_build_addts_req(struct wpa_supplicant *wpa_s, const struct wmm_ac_ts_setup_params *params, const u8 *address)
{
	...
	tspec->surplus_bandwidth_allowance = host_to_le16(params->surplus_bandwidth_allowance);

	return addts_req;
}

static int param_in_range(const char *name, long value, long min_val, long max_val)
{
	if (value < min_val || (max_val >= 0 && value > max_val)) {
		wpa_printf(MSG_DEBUG, "WMM AC: param %s (%ld) is out of range (%ld-%ld)", name, value, min_val, max_val);
		return 0;
	}
...
static int wmm_ac_should_replace_ts(struct wpa_supplicant *wpa_s, u8 tsid, u8 ac, u8 dir)
{
	enum ts_dir_idx idx;
	...
	if (cur_ac >= 0) {
		if (cur_ac != ac) {
			wpa_printf(MSG_DEBUG, "WMM AC: TSID %i already exists on different ac (%d)", tsid, cur_ac);
			return -1;
		}
	...
	case WMM_AC_DIR_DOWNLINK:
		...
		replace_ts |= existing_ts & (BIT(TS_DIR_IDX_DOWNLINK) | BIT(TS_DIR_IDX_BIDI));
		break;
	case WMM_AC_DIR_BIDIRECTIONAL:
...
static int wmm_ac_ts_req_is_valid(struct wpa_supplicant *wpa_s, const struct wmm_ac_ts_setup_params *params)
{
	enum wmm_ac req_ac;
...
#define PARAM_IN_RANGE(field, min_value, max_value) \
	...
	if (!PARAM_IN_RANGE(tsid, 0, WMM_AC_MAX_TID) || !PARAM_IN_RANGE(user_priority, 0, WMM_AC_MAX_USER_PRIORITY) || !PARAM_IN_RANGE(nominal_msdu_size, 1, WMM_AC_MAX_NOMINAL_MSDU) || !PARAM_IN_RANGE(mean_data_rate, 1, -1) || !PARAM_IN_RANGE(minimum_phy_rate, 1, -1) || !PARAM_IN_RANGE(surplus_bandwidth_allowance, WMM_AC_MIN_SBA_UNITY, -1)) {
		return 0;
	}
#undef PARAM_IN_RANGE

	if (!(params->direction == WMM_TSPEC_DIRECTION_UPLINK || params->direction == WMM_TSPEC_DIRECTION_DOWNLINK || params->direction == WMM_TSPEC_DIRECTION_BI_DIRECTIONAL)) {
		wpa_printf(MSG_DEBUG, "WMM AC: invalid TS direction: %d", params->direction);
		return 0;
	}
	...
	if (wmm_ac_should_replace_ts(wpa_s, params->tsid, req_ac, params->direction) < 0) {
		return 0;
	}
...
static struct wmm_ac_assoc_data *wmm_ac_process_param_elem(struct wpa_supplicant *wpa_s, const u8 *ies, size_t ies_len)
{
	...
	wpa_printf(MSG_DEBUG, "WMM AC: AC mandatory: AC_BE=%u AC_BK=%u AC_VI=%u AC_VO=%u", assoc_data->ac_params[WMM_AC_BE].acm, assoc_data->ac_params[WMM_AC_BK].acm, assoc_data->ac_params[WMM_AC_VI].acm, assoc_data->ac_params[WMM_AC_VO].acm);

	return assoc_data;
}

static int wmm_ac_init(struct wpa_supplicant *wpa_s, const u8 *ies, size_t ies_len, const struct wmm_params *wmm_params)
{
	struct wmm_ac_assoc_data *assoc_data;
	u8 ac;
...
int wpas_wmm_ac_addts(struct wpa_supplicant *wpa_s, struct wmm_ac_ts_setup_params *params)
{
	...
	if (wpa_s->addts_request) {
		wpa_printf(MSG_DEBUG, "WMM AC: can't add TS - ADDTS request is already pending");
		return -1;
	}
	...
	wpa_printf(MSG_DEBUG, "WMM AC: TS setup request (addr=" MACSTR " tsid=%u user priority=%u direction=%d)", MAC2STR(wpa_s->bssid), params->tsid, params->user_priority, params->direction);
...
static void wmm_ac_handle_delts(struct wpa_supplicant *wpa_s, const u8 *sa, const struct wmm_tspec_element *tspec)
{
	int ac;
	u8 tsid;
	enum ts_dir_idx idx;
	...
	wpa_printf(MSG_DEBUG, "WMM AC: DELTS frame has been received TSID=%u addr=" MACSTR, tsid, MAC2STR(sa));
	...
	wpa_printf(MSG_DEBUG, "TS was deleted successfully (tsid=%u address=" MACSTR ")", tsid, MAC2STR(sa));
}

static void wmm_ac_handle_addts_resp(struct wpa_supplicant *wpa_s, const u8 *sa, const u8 resp_dialog_token, const u8 status_code, const struct wmm_tspec_element *tspec)
{
	struct wmm_ac_addts_request *req = wpa_s->addts_request;
	u8 ac, tsid, up, dir;
	int replace_tspecs;
	...
	if (!req || req->dialog_token != resp_dialog_token) {
		wpa_printf(MSG_DEBUG, "WMM AC: no req with dialog=%u, ignoring frame", resp_dialog_token);
		return;
	}

	/* make sure the params are the same */
	if (os_memcmp(req->address, sa, ETH_ALEN) != 0 || tsid != wmm_ac_get_tsid(&req->tspec) || up != wmm_ac_get_user_priority(&req->tspec) || dir != wmm_ac_get_direction(&req->tspec)) {
	...
	wpa_printf(MSG_DEBUG, "ADDTS response status=%d tsid=%u up=%u direction=%u", status_code, tsid, up, dir);
	...
	wpa_printf(MSG_DEBUG, "WMM AC: adding a new TS with TSID=%u address=" MACSTR " medium time=%u access category=%d dir=%d ", tsid, MAC2STR(sa), le_to_host16(tspec->medium_time), ac, dir);
...
void wmm_ac_rx_action(struct wpa_supplicant *wpa_s, const u8 *da, const u8 *sa, const u8 *data, size_t len)
{
	...
	if (action != WMM_ACTION_CODE_ADDTS_RESP && action != WMM_ACTION_CODE_DELTS) {
		wpa_printf(MSG_DEBUG, "WMM AC: Unknown action (%d), ignoring action frame", action);
		return;
	}

	/* WMM AC action frame */
	if (os_memcmp(da, wpa_s->own_addr, ETH_ALEN) != 0) {
		wpa_printf(MSG_DEBUG, "WMM AC: frame destination addr=" MACSTR " is other than ours, ignoring frame", MAC2STR(da));
		return;
	}

	if (os_memcmp(sa, wpa_s->bssid, ETH_ALEN) != 0) {
		wpa_printf(MSG_DEBUG, "WMM AC: ignore frame with sa " MACSTR " different other than our bssid", MAC2STR(da));
		return;
	}

	if (len < 2 + sizeof(struct wmm_tspec_element)) {
		wpa_printf(MSG_DEBUG, "WMM AC: Short ADDTS response ignored (len=%lu)", (unsigned long)len);
		return;
	}
	...
	if (ieee802_11_parse_elems(data + 2, len - 2, &elems, 1) != ParseOK) {
		wpa_printf(MSG_DEBUG, "WMM AC: Could not parse WMM AC action from " MACSTR, MAC2STR(sa));
		return;
	}
...
int wpas_wmm_ac_status(struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	...
	if (!assoc_info) {
		return wpa_scnprintf(buf, buflen - pos, "Not associated to a WMM AP, WMM AC is Disabled\n");
	}
	...
	for (ac = 0; ac < WMM_AC_NUM; ac++) {
		...
		pos += wpa_scnprintf(buf + pos, buflen - pos, "%s: acm=%d uapsd=%d\n", get_ac_str(ac), assoc_info->ac_params[ac].acm, assoc_info->ac_params[ac].uapsd);
		...
		for (idx = 0; idx < TS_DIR_IDX_COUNT; idx++) {
			...
			pos += wpa_scnprintf(buf + pos, buflen - pos, "\tTSID=%u UP=%u\n" "\tAddress = " MACSTR "\n" "\tWMM AC dir = %s\n" "\tTotal admitted time = %u\n\n", tsid, up, MAC2STR(wpa_s->bssid), dir_str, le_to_host16(tspec->medium_time));
		}

		if (!ts_count) {
			pos += wpa_scnprintf(buf + pos, buflen - pos, "\t(No Traffic Stream)\n\n");
		}
	}
...
void wmm_ac_save_tspecs(struct wpa_supplicant *wpa_s)
{
	...
	wpa_printf(MSG_DEBUG, "WMM AC: Successfully saved %d TSPECs", wpa_s->last_tspecs_count);
}
...
int wmm_ac_restore_tspecs(struct wpa_supplicant *wpa_s)
{
	...
	wpa_printf(MSG_DEBUG, "WMM AC: Restore %u saved tspecs", wpa_s->last_tspecs_count);
