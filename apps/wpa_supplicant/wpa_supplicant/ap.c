...
static void wpas_conf_ap_vht(struct wpa_supplicant *wpa_s, struct hostapd_config *conf, struct hostapd_hw_modes *mode)
{
#ifdef CONFIG_P2P
	u8 center_chan = 0;
...
#else							/* CONFIG_P2P */
	conf->vht_oper_centr_freq_seg0_idx = conf->channel + conf->secondary_channel * 2;
#endif							/* CONFIG_P2P */
}
...
void wpa_supplicant_conf_ap_ht(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid, struct hostapd_config *conf)
{
	...
	if (wpa_s->hw.modes) {
		...
		if (!no_ht && mode && mode->ht_capab) {
			conf->ieee80211n = 1;
#ifdef CONFIG_P2P
			if (conf->hw_mode == HOSTAPD_MODE_IEEE80211A && (mode->ht_capab & HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET) && ssid->ht40) {
				conf->secondary_channel = wpas_p2p_get_ht40_mode(wpa_s, mode, conf->channel);
			}
			if (conf->secondary_channel) {
			 ...
			 */
			conf->ht_capab |= mode->ht_capab & (HT_CAP_INFO_GREEN_FIELD | HT_CAP_INFO_SHORT_GI20MHZ | HT_CAP_INFO_SHORT_GI40MHZ | HT_CAP_INFO_RX_STBC_MASK | HT_CAP_INFO_TX_STBC | HT_CAP_INFO_MAX_AMSDU_SIZE);
...
static int wpa_supplicant_conf_ap(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid, struct hostapd_config *conf)
{
	...
	if (conf->hw_mode == NUM_HOSTAPD_MODES) {
		wpa_printf(MSG_ERROR, "Unsupported AP mode frequency: %d MHz", ssid->frequency);
		return -1;
	}
...
#ifdef CONFIG_P2P
	if (conf->hw_mode == HOSTAPD_MODE_IEEE80211G && (ssid->mode == WPAS_MODE_P2P_GO || ssid->mode == WPAS_MODE_P2P_GROUP_FORMATION)) {
	...
	if (ssid->p2p_group) {
		...
		os_memcpy(bss->ip_addr_mask, wpa_s->parent->conf->ip_addr_mask, 4);
		os_memcpy(bss->ip_addr_start, wpa_s->parent->conf->ip_addr_start, 4);
		os_memcpy(bss->ip_addr_end, wpa_s->parent->conf->ip_addr_end, 4);
	}
#endif							/* CONFIG_P2P */
	...
	} else if (ssid->wep_key_len[0] || ssid->wep_key_len[1] || ssid->wep_key_len[2] || ssid->wep_key_len[3]) {
	...
	if (bss->wpa && bss->ieee802_1x) {
		...
		if (bss->default_wep_key_len) {
			cipher = bss->default_wep_key_len >= 13 ? WPA_CIPHER_WEP104 : WPA_CIPHER_WEP40;
		}
	...
	if (bss->wpa_group_rekey < 86400 && (bss->wpa & 2) && (bss->wpa_group == WPA_CIPHER_CCMP || bss->wpa_group == WPA_CIPHER_GCMP || bss->wpa_group == WPA_CIPHER_CCMP_256 || bss->wpa_group == WPA_CIPHER_GCMP_256)) {
		/*
	 ...
	 */
	if (bss->ssid.security_policy != SECURITY_WPA_PSK && bss->ssid.security_policy != SECURITY_PLAINTEXT) {
		goto no_wps;
	}
	if (bss->ssid.security_policy == SECURITY_WPA_PSK && (!(bss->rsn_pairwise & (WPA_CIPHER_CCMP | WPA_CIPHER_GCMP)) || !(bss->wpa & 2))) {
		goto no_wps;
...
static void ap_public_action_rx(void *ctx, const u8 *buf, size_t len, int freq)
{
	...
	wpas_p2p_rx_action(wpa_s, mgmt->da, mgmt->sa, mgmt->bssid, mgmt->u.action.category, buf + IEEE80211_HDRLEN + 1, len - IEEE80211_HDRLEN - 1, freq);
#endif							/* CONFIG_P2P */
}

static void ap_wps_event_cb(void *ctx, enum wps_event event, union wps_event_data *data)
{
#ifdef CONFIG_P2P
	struct wpa_supplicant *wpa_s = ctx;
	...
	if (event == WPS_EV_FAIL) {
		...
		if (wpa_s->parent && wpa_s->parent != wpa_s && wpa_s == wpa_s->global->p2p_group_formation) {
			/*
...
static void ap_sta_authorized_cb(void *ctx, const u8 *mac_addr, int authorized, const u8 *p2p_dev_addr)
{
	wpas_notify_sta_authorized(ctx, mac_addr, authorized, p2p_dev_addr);
}

#ifdef CONFIG_P2P
static void ap_new_psk_cb(void *ctx, const u8 *mac_addr, const u8 *p2p_dev_addr, const u8 *psk, size_t psk_len)
{
...
static int ap_probe_req_rx(void *ctx, const u8 *sa, const u8 *da, const u8 *bssid, const u8 *ie, size_t ie_len, int ssi_signal)
{
	struct wpa_supplicant *wpa_s = ctx;
	unsigned int freq = 0;
	...
	return wpas_p2p_probe_req_rx(wpa_s, sa, da, bssid, ie, ie_len, freq, ssi_signal);
}
...
static void wpas_ap_configured_cb(void *ctx)
{
	...
	if (wpa_s->ap_configured_cb) {
		wpa_s->ap_configured_cb(wpa_s->ap_configured_cb_ctx, wpa_s->ap_configured_cb_data);
	}
}
...
int wpa_supplicant_create_ap(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
	...
	wpa_printf(MSG_DEBUG, "Setting up AP (SSID='%s')", wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
...
#ifdef CONFIG_P2P
	if (ssid->mode == WPAS_MODE_P2P_GO || ssid->mode == WPAS_MODE_P2P_GROUP_FORMATION) {
		params.p2p = 1;
	}
#endif							/* CONFIG_P2P */
	...
	os_memcpy(wpa_s->ap_iface->conf->wmm_ac_params, wpa_s->conf->wmm_ac_params, sizeof(wpa_s->conf->wmm_ac_params));
...
void wpa_supplicant_ap_deinit(struct wpa_supplicant *wpa_s)
{
	...
	wpa_s->ap_iface->driver_ap_teardown = ! !(wpa_s->drv_flags & WPA_DRIVER_FLAGS_AP_TEARDOWN_SUPPORT);

	hostapd_interface_deinit(wpa_s->ap_iface);
	hostapd_interface_free(wpa_s->ap_iface);
	wpa_s->ap_iface = NULL;
	wpa_drv_deinit_ap(wpa_s);
	wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_DISCONNECTED "bssid=" MACSTR " reason=%d locally_generated=1", MAC2STR(wpa_s->own_addr), WLAN_REASON_DEAUTH_LEAVING);
}
...
void ap_mgmt_rx(void *ctx, struct rx_mgmt *rx_mgmt)
{
	...
	ieee802_11_mgmt(wpa_s->ap_iface->bss[0], rx_mgmt->frame, rx_mgmt->frame_len, &fi);
#endif							/* NEED_AP_MLME */
}
...
int wpa_supplicant_ap_wps_pbc(struct wpa_supplicant *wpa_s, const u8 *bssid, const u8 *p2p_dev_addr)
{
	if (!wpa_s->ap_iface) {
		return -1;
	}
...
int wpa_supplicant_ap_wps_pin(struct wpa_supplicant *wpa_s, const u8 *bssid, const char *pin, char *buf, size_t buflen, int timeout)
{
	int ret, ret_len = 0;
	...
	ret = hostapd_wps_add_pin(wpa_s->ap_iface->bss[0], bssid, "any", pin, timeout);
	if (ret) {
		return -1;
	}
	return ret_len;
...
int wpas_wps_ap_pin_set(struct wpa_supplicant *wpa_s, const char *pin, int timeout)
{
	struct hostapd_data *hapd;
	char pin_txt[9];
	int ret;
...
void wpa_supplicant_ap_pwd_auth_fail(struct wpa_supplicant *wpa_s)
{
	...
	wpa_printf(MSG_DEBUG, "WPS: AP PIN authentication failure number %u", hapd->ap_pin_failures);
...
struct wpabuf *wpas_ap_wps_nfc_config_token(struct wpa_supplicant *wpa_s, int ndef)
{
	struct hostapd_data *hapd;
...
struct wpabuf *wpas_ap_wps_nfc_handover_sel(struct wpa_supplicant *wpa_s, int ndef)
{
	struct hostapd_data *hapd;
...
int wpas_ap_wps_nfc_report_handover(struct wpa_supplicant *wpa_s, const struct wpabuf *req, const struct wpabuf *sel)
{
	struct hostapd_data *hapd;
...
int ap_ctrl_iface_wpa_get_status(struct wpa_supplicant *wpa_s, char *buf, size_t buflen, int verbose)
{
	...
	ret = os_snprintf(pos, end - pos, "pairwise_cipher=%s\n" "group_cipher=%s\n" "key_mgmt=%s\n", wpa_cipher_txt(conf->rsn_pairwise), wpa_cipher_txt(conf->wpa_group), wpa_key_mgmt_txt(conf->wpa_key_mgmt, conf->wpa));
...
int wpa_supplicant_ap_update_beacon(struct wpa_supplicant *wpa_s)
{
	...
	if (ssid == NULL || wpa_s->ap_iface == NULL || ssid->mode == WPAS_MODE_INFRA || ssid->mode == WPAS_MODE_IBSS) {
		return -1;
	}
	...
	if (ssid->mode == WPAS_MODE_P2P_GO) {
		...
		iface->conf->bss[0]->p2p = P2P_ENABLED | P2P_GROUP_OWNER | P2P_GROUP_FORMATION;
	}
#endif							/* CONFIG_P2P */
...
#ifdef CONFIG_WPS_NFC
int wpas_ap_wps_add_nfc_pw(struct wpa_supplicant *wpa_s, u16 pw_id, const struct wpabuf *pw, const u8 *pubkey_hash)
{
	struct hostapd_data *hapd;
	...
	if (wpa_s->parent->conf->wps_nfc_dh_pubkey == NULL || wpa_s->parent->conf->wps_nfc_dh_privkey == NULL) {
	...
	wps->dh_privkey = wpabuf_dup(wpa_s->parent->conf->wps_nfc_dh_privkey);
	wps->dh_pubkey = wpabuf_dup(wpa_s->parent->conf->wps_nfc_dh_pubkey);
	...
	return wps_registrar_add_nfc_pw_token(hapd->wps->registrar, pubkey_hash, pw_id, pw ? wpabuf_head(pw) : NULL, pw ? wpabuf_len(pw) : 0, 1);
}
#endif							/* CONFIG_WPS_NFC */
...
#ifdef NEED_AP_MLME
void wpas_event_dfs_radar_detected(struct wpa_supplicant *wpa_s, struct dfs_event *radar)
{
	if (!wpa_s->ap_iface || !wpa_s->ap_iface->bss[0]) {
		return;
	}
	wpa_printf(MSG_DEBUG, "DFS radar detected on %d MHz", radar->freq);
	hostapd_dfs_radar_detected(wpa_s->ap_iface, radar->freq, radar->ht_enabled, radar->chan_offset, radar->chan_width, radar->cf1, radar->cf2);
}
...
void wpas_event_dfs_cac_started(struct wpa_supplicant *wpa_s, struct dfs_event *radar)
{
	...
	hostapd_dfs_start_cac(wpa_s->ap_iface, radar->freq, radar->ht_enabled, radar->chan_offset, radar->chan_width, radar->cf1, radar->cf2);
}
...
void wpas_event_dfs_cac_finished(struct wpa_supplicant *wpa_s, struct dfs_event *radar)
{
	...
	hostapd_dfs_complete_cac(wpa_s->ap_iface, 1, radar->freq, radar->ht_enabled, radar->chan_offset, radar->chan_width, radar->cf1, radar->cf2);
}

void wpas_event_dfs_cac_aborted(struct wpa_supplicant *wpa_s, struct dfs_event *radar)
{
	if (!wpa_s->ap_iface || !wpa_s->ap_iface->bss[0]) {
		return;
	}
	wpa_printf(MSG_DEBUG, "DFS CAC aborted on %d MHz", radar->freq);
	hostapd_dfs_complete_cac(wpa_s->ap_iface, 0, radar->freq, radar->ht_enabled, radar->chan_offset, radar->chan_width, radar->cf1, radar->cf2);
}
...
void wpas_event_dfs_cac_nop_finished(struct wpa_supplicant *wpa_s, struct dfs_event *radar)
{
	...
	hostapd_dfs_nop_finished(wpa_s->ap_iface, radar->freq, radar->ht_enabled, radar->chan_offset, radar->chan_width, radar->cf1, radar->cf2);
}
#endif							/* NEED_AP_MLME */
