...
static void wpa_priv_cmd_register(struct wpa_priv_interface *iface, struct sockaddr_un *from)
{
	...
	if (iface->l2) {
		wpa_printf(MSG_DEBUG, "Cleaning up forgotten l2_packet " "instance");
	...
	wpa_printf(MSG_DEBUG, "Driver wrapper '%s' initialized for interface " "'%s'", iface->driver_name, iface->ifname);

	os_memcpy(&iface->drv_addr, from, sizeof(iface->drv_addr));
	iface->wpas_registered = 1;

	if (iface->driver->set_param && iface->driver->set_param(iface->drv_priv, NULL) < 0) {
...
static void wpa_priv_cmd_get_bssid(struct wpa_priv_interface *iface, struct sockaddr_un *from)
{
	...
	if (iface->driver->get_bssid == NULL || iface->driver->get_bssid(iface->drv_priv, bssid) < 0) {
		goto fail;
	}
...
static void wpa_priv_cmd_get_ssid(struct wpa_priv_interface *iface, struct sockaddr_un *from)
{
	...
	sendto(iface->fd, ssid, sizeof(ssid), 0, (struct sockaddr *)from, sizeof(*from));
	return;
...
static void wpa_priv_cmd_set_key(struct wpa_priv_interface *iface, void *buf, size_t len)
{
	...
	res = iface->driver->set_key(iface->ifname, iface->drv_priv, params->alg, params->addr, params->key_idx, params->set_tx, params->seq_len ? params->seq : NULL, params->seq_len, params->key_len ? params->key : NULL, params->key_len);
...
static void wpa_priv_cmd_get_capa(struct wpa_priv_interface *iface, struct sockaddr_un *from)
{
	...
	if (iface->driver->get_capa == NULL || iface->driver->get_capa(iface->drv_priv, &capa) < 0) {
		goto fail;
	}

	sendto(iface->fd, &capa, sizeof(capa), 0, (struct sockaddr *)from, sizeof(*from));
	return;
...
static void wpa_priv_cmd_l2_register(struct wpa_priv_interface *iface, struct sockaddr_un *from, void *buf, size_t len)
{
	...
	if (proto != ETH_P_EAPOL && proto != ETH_P_RSN_PREAUTH) {
		wpa_printf(MSG_DEBUG, "Refused l2_packet connection for " "ethertype 0x%x", proto);
		return;
	}
	...
	iface->l2 = l2_packet_init(iface->ifname, NULL, proto, wpa_priv_l2_rx, iface, reg_cmd[1]);
	if (iface->l2 == NULL) {
		wpa_printf(MSG_DEBUG, "Failed to initialize l2_packet " "instance for protocol %d", proto);
		return;
	}

	if (l2_packet_get_own_addr(iface->l2, own_addr) < 0) {
		wpa_printf(MSG_DEBUG, "Failed to get own address from " "l2_packet");
		l2_packet_deinit(iface->l2);
		iface->l2 = NULL;
		return;
	}

	res = sendto(iface->fd, own_addr, ETH_ALEN, 0, (struct sockaddr *)from, sizeof(*from));
...
static void wpa_priv_cmd_l2_send(struct wpa_priv_interface *iface, struct sockaddr_un *from, void *buf, size_t len)
{
	...
	if (len < ETH_ALEN + 2) {
		wpa_printf(MSG_DEBUG, "Too short L2 send packet (len=%lu)", (unsigned long)len);
		return;
	}
	...
	if (proto != ETH_P_EAPOL && proto != ETH_P_RSN_PREAUTH) {
		wpa_printf(MSG_DEBUG, "Refused l2_packet send for ethertype " "0x%x", proto);
		return;
	}

	res = l2_packet_send(iface->l2, dst_addr, proto, buf + ETH_ALEN + 2, len - ETH_ALEN - 2);
	wpa_printf(MSG_DEBUG, "L2 send: res=%d", res);
}

static void wpa_priv_cmd_set_country(struct wpa_priv_interface *iface, char *buf)
{
	if (iface->drv_priv == NULL || iface->driver->set_country == NULL || *buf == '\0') {
		return;
	}
...
static struct wpa_priv_interface *wpa_priv_interface_init(const char *dir, const char *params)
{
	...
	if (bind(iface->fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
		...
		if (connect(iface->fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
			wpa_printf(MSG_DEBUG, "Socket exists, but does not " "allow connections - assuming it was " "leftover from forced program termination");
			if (unlink(iface->sock_name) < 0) {
				wpa_printf(MSG_ERROR, "Could not unlink existing ctrl_iface socket '%s': %s", iface->sock_name, strerror(errno));
				goto fail;
			}
			if (bind(iface->fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
				wpa_printf(MSG_ERROR, "wpa-priv-iface-init: bind(PF_UNIX): %s", strerror(errno));
				goto fail;
			}
			wpa_printf(MSG_DEBUG, "Successfully replaced leftover " "socket '%s'", iface->sock_name);
		} else {
			wpa_printf(MSG_INFO, "Socket exists and seems to be " "in use - cannot override it");
			wpa_printf(MSG_INFO, "Delete '%s' manually if it is " "not used anymore", iface->sock_name);
			goto fail;
		}
	}
...
static void wpa_priv_send_assoc(struct wpa_priv_interface *iface, int event, union wpa_event_data *data)
{
	...
	if (data) {
		buflen += data->assoc_info.req_ies_len + data->assoc_info.resp_ies_len + data->assoc_info.beacon_ies_len;
	}
...
static void wpa_priv_send_interface_status(struct wpa_priv_interface *iface, union wpa_event_data *data)
{
	...
	wpa_priv_send_event(iface, PRIVSEP_EVENT_INTERFACE_STATUS, buf, sizeof(int) + len);
...
void wpa_supplicant_event(void *ctx, enum wpa_event_type event, union wpa_event_data *data)
{
	...
	if (!iface->wpas_registered) {
		wpa_printf(MSG_DEBUG, "Driver event received, but " "wpa_supplicant not registered");
		return;
	}
	...
	case EVENT_MICHAEL_MIC_FAILURE:
		...
		wpa_priv_send_event(iface, PRIVSEP_EVENT_MICHAEL_MIC_FAILURE, &data->michael_mic_failure.unicast, sizeof(int));
		break;
	...
	case EVENT_PMKID_CANDIDATE:
		...
		wpa_priv_send_event(iface, PRIVSEP_EVENT_PMKID_CANDIDATE, &data->pmkid_candidate, sizeof(struct pmkid_candidate));
		break;
	case EVENT_STKSTART:
		if (data == NULL) {
			return;
		}
		wpa_priv_send_event(iface, PRIVSEP_EVENT_STKSTART, &data->stkstart.peer, ETH_ALEN);
		break;
	case EVENT_FT_RESPONSE:
		wpa_priv_send_ft_response(iface, data);
		break;
	default:
		wpa_printf(MSG_DEBUG, "Unsupported driver event %d - TODO", event);
		break;
	}
}
