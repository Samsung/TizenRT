...
int wpa_blacklist_add(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
	...
	if (e) {
		e->count++;
		wpa_printf(MSG_DEBUG, "BSSID " MACSTR " blacklist count " "incremented to %d", MAC2STR(bssid), e->count);
		return e->count;
	}
	...
	wpa_s->blacklist = e;
	wpa_printf(MSG_DEBUG, "Added BSSID " MACSTR " into blacklist", MAC2STR(bssid));
...
int wpa_blacklist_del(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
	...
	while (e) {
		if (os_memcmp(e->bssid, bssid, ETH_ALEN) == 0) {
			...
			wpa_printf(MSG_DEBUG, "Removed BSSID " MACSTR " from " "blacklist", MAC2STR(bssid));
			os_free(e);
			return 0;
		}
		prev = e;
...
void wpa_blacklist_clear(struct wpa_supplicant *wpa_s)
{
	...
	while (e) {
		...
		prev = e;
		e = e->next;
		wpa_printf(MSG_DEBUG, "Removed BSSID " MACSTR " from " "blacklist (clear)", MAC2STR(prev->bssid));
		os_free(prev);
	}
