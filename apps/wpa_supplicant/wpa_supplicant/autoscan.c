...
int autoscan_init(struct wpa_supplicant *wpa_s, int req_scan)
{
	...
	if (ops == NULL) {
		wpa_printf(MSG_ERROR, "autoscan: Could not find module " "matching the parameter '%s'", name);
		return -1;
	}
	...
	wpa_printf(MSG_DEBUG, "autoscan: Initialized module '%s' with " "parameters '%s'", ops->name, params);
	if (!req_scan) {
		return 0;
	}
...
{
	if (wpa_s->autoscan && wpa_s->autoscan_priv) {
		wpa_printf(MSG_DEBUG, "autoscan: Deinitializing module '%s'", wpa_s->autoscan->name);
