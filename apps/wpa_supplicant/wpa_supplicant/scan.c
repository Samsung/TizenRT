...
static void wpa_supplicant_gen_assoc_event(struct wpa_supplicant *wpa_s)
{
	...
	wpa_dbg(wpa_s, MSG_DEBUG, "Already associated with a configured " "network - generating associated event");
	os_memset(&data, 0, sizeof(data));
	wpa_supplicant_event(wpa_s, EVENT_ASSOC, &data);
}

#ifdef CONFIG_WPS
static int wpas_wps_in_use(struct wpa_supplicant *wpa_s, enum wps_request_type *req_type)
{
	struct wpa_ssid *ssid;
	int wps = 0;
...
#ifdef CONFIG_P2P
	if (!wpa_s->global->p2p_disabled && wpa_s->global->p2p && !wpa_s->conf->p2p_disabled) {
		wpa_s->wps->dev.p2p = 1;
		if (!wps) {
			wps = 1;
...
int wpa_supplicant_enabled_networks(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->conf->cred && wpa_s->conf->interworking && wpa_s->conf->auto_interworking) {
		count++;
	}
	if (count == 0 && disabled > 0) {
		wpa_dbg(wpa_s, MSG_DEBUG, "No enabled networks (%d disabled " "networks)", disabled);
	}
	return count;
}
...
static void wpa_supplicant_assoc_try(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid)
{
	...
	if (ssid == NULL) {
		wpa_dbg(wpa_s, MSG_DEBUG, "wpa_supplicant_assoc_try: Reached " "end of scan list - go back to beginning");
...
static void wpas_trigger_scan_cb(struct wpa_radio_work *work, int deinit)
{
	...
	if (wpa_s->clear_driver_scan_cache) {
		wpa_printf(MSG_DEBUG, "Request driver to clear scan cache due to local BSS flush");
	...
	if (ret) {
		...
		wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_SCAN_FAILED "ret=%d%s", ret, retry ? " retry=1" : "");
		radio_work_done(work);
...
static void wpa_supplicant_delayed_sched_scan_timeout(void *eloop_ctx, void *timeout_ctx)
{
...
static void wpa_supplicant_sched_scan_timeout(void *eloop_ctx, void *timeout_ctx)
{
	struct wpa_supplicant *wpa_s = eloop_ctx;
...
static struct wpa_driver_scan_filter *wpa_supplicant_build_filter_ssids(struct wpa_config *conf, size_t *num_ssids)
{
...
static void wpa_supplicant_optimize_freqs(struct wpa_supplicant *wpa_s, struct wpa_driver_scan_params *params)
{
#ifdef CONFIG_P2P
	if (params->freqs == NULL && wpa_s->p2p_in_provisioning && wpa_s->go_params) {
		...
		if (wpa_s->p2p_in_provisioning < 5 && wpa_s->go_params->freq > 0) {
			wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Scan only GO " "preferred frequency %d MHz", wpa_s->go_params->freq);
			params->freqs = os_calloc(2, sizeof(int));
			if (params->freqs) {
				params->freqs[0] = wpa_s->go_params->freq;
			}
		} else if (wpa_s->p2p_in_provisioning < 8 && wpa_s->go_params->freq_list[0]) {
			wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Scan only common " "channels");
	...
	if (params->freqs == NULL && wpa_s->p2p_in_invitation) {
		...
		if (wpa_s->p2p_in_invitation < 5 && wpa_s->p2p_invite_go_freq > 0) {
			wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Scan only GO preferred frequency %d MHz during invitation", wpa_s->p2p_invite_go_freq);
			params->freqs = os_calloc(2, sizeof(int));
			if (params->freqs) {
	...
	if (params->freqs == NULL && wpa_s->after_wps && wpa_s->wps_freq) {
		 ...
		 * during provisioning.
		 */
		wpa_dbg(wpa_s, MSG_DEBUG, "WPS: Scan only frequency %u MHz " "that was used during provisioning", wpa_s->wps_freq);
		params->freqs = os_calloc(2, sizeof(int));
		if (params->freqs) {
	...
	if (params->freqs == NULL && wpa_s->known_wps_freq && wpa_s->wps_freq) {
		...
		wpa_dbg(wpa_s, MSG_DEBUG, "WPS: Scan only frequency %u MHz", wpa_s->wps_freq);
		params->freqs = os_calloc(2, sizeof(int));
		if (params->freqs) {
			params->freqs[0] = wpa_s->wps_freq;
		}
...
static void wpas_add_interworking_elements(struct wpa_supplicant *wpa_s, struct wpabuf *buf)
{
	wpabuf_put_u8(buf, WLAN_EID_INTERWORKING);
	wpabuf_put_u8(buf, is_zero_ether_addr(wpa_s->conf->hessid) ? 1 : 1 + ETH_ALEN);
...
static struct wpabuf *wpa_supplicant_extra_ies(struct wpa_supplicant *wpa_s)
{
	...
	if (wps) {
		...
		wps_ie = wps_build_probe_req_ie(wps == 2 ? DEV_PW_PUSHBUTTON : DEV_PW_DEFAULT, &wpa_s->wps->dev, wpa_s->wps->uuid, req_type, 0, NULL);
		if (wps_ie) {
...
static void wpa_setband_scan_freqs_list(struct wpa_supplicant *wpa_s, enum hostapd_hw_mode band, struct wpa_driver_scan_params *params)
{
...
static void wpa_setband_scan_freqs(struct wpa_supplicant *wpa_s, struct wpa_driver_scan_params *params)
{
...
static void wpa_set_scan_ssids(struct wpa_supplicant *wpa_s, struct wpa_driver_scan_params *params, size_t max_ssids)
{
	unsigned int i;
	struct wpa_ssid *ssid;
	...
	for (i = 0; i < wpa_s->scan_id_count; i++) {
		...
		for (j = 0; j < params->num_ssids; j++) {
			if (params->ssids[j].ssid_len == ssid->ssid_len && params->ssids[j].ssid && os_memcmp(params->ssids[j].ssid, ssid->ssid, ssid->ssid_len) == 0) {
				break;
			}
		}
		...
		wpa_printf(MSG_DEBUG, "Scan SSID (manual request): %s", wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
...
static int wpa_set_ssids_from_scan_req(struct wpa_supplicant *wpa_s, struct wpa_driver_scan_params *params, size_t max_ssids)
{
	unsigned int i;
	...
	if (wpa_s->num_ssids_from_scan_req > max_ssids) {
		...
		wpa_printf(MSG_DEBUG, "Over max scan SSIDs from scan req: %u", (unsigned int)max_ssids);
	}
...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (wpa_s->conf->ap_scan != 0 && (wpa_s->drv_flags & WPA_DRIVER_FLAGS_WIRED)) {
		wpa_dbg(wpa_s, MSG_DEBUG, "Using wired authentication - " "overriding ap_scan configuration");
	...
	if (p2p_in_prog && p2p_in_prog != 2 && (!ssid || (ssid->mode != WPAS_MODE_AP && ssid->mode != WPAS_MODE_P2P_GO))) {
	...
	if (connect_without_scan) {
		...
		if (ssid) {
			wpa_printf(MSG_DEBUG, "Start a pre-selected network " "without scan step");
			wpa_supplicant_associate(wpa_s, NULL, ssid);
			return;
		}
	}
	...
	if (wpa_s->last_scan_req == MANUAL_SCAN_REQ && wpa_set_ssids_from_scan_req(wpa_s, &params, max_ssids)) {
		wpa_printf(MSG_DEBUG, "Use specific SSIDs from SCAN command");
		goto ssid_list_set;
	}
#ifdef CONFIG_P2P
	if ((wpa_s->p2p_in_provisioning || wpa_s->show_group_started) && wpa_s->go_params && !wpa_s->conf->passive_scan) {
		wpa_printf(MSG_DEBUG, "P2P: Use specific SSID for scan during P2P group formation (p2p_in_provisioning=%d show_group_started=%d)", wpa_s->p2p_in_provisioning, wpa_s->show_group_started);
	...
	if (wpa_s->last_scan_req != MANUAL_SCAN_REQ && wpa_s->conf->ap_scan == 2) {
		...
		for (tssid = wpa_s->conf->ssid; wpa_s->last_scan_req != MANUAL_SCAN_REQ && tssid; tssid = tssid->next) {
	...
	if (ssid && max_ssids == 1) {
		...
		if (!wpa_s->prev_scan_wildcard) {
			...
			wpa_dbg(wpa_s, MSG_DEBUG, "Starting AP scan for " "wildcard SSID (Interleave with specific)");
		} else {
			wpa_s->prev_scan_ssid = ssid;
			wpa_s->prev_scan_wildcard = 0;
			wpa_dbg(wpa_s, MSG_DEBUG, "Starting AP scan for specific SSID: %s", wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
		}
	} else if (ssid) {
		/* max_ssids > 1 */

		wpa_s->prev_scan_ssid = ssid;
		wpa_dbg(wpa_s, MSG_DEBUG, "Include wildcard SSID in " "the scan request");
		params.num_ssids++;
	} else if (wpa_s->last_scan_req == MANUAL_SCAN_REQ && wpa_s->manual_scan_passive && params.num_ssids == 0) {
...
ssid_list_set:
	...
	if (wpa_s->last_scan_req == MANUAL_SCAN_REQ && wpa_s->manual_scan_only_new) {
		wpa_printf(MSG_DEBUG, "Request driver to clear scan cache due to manual only_new=1 scan");
		params.only_new_results = 1;
	}

	if (wpa_s->last_scan_req == MANUAL_SCAN_REQ && params.freqs == NULL && wpa_s->manual_scan_freqs) {
	...
	if (wpa_s->conf->scan_cur_freq && !params.freqs) {
		...
		if (params.freqs) {
			...
			if (num > 0) {
				wpa_dbg(wpa_s, MSG_DEBUG, "Scan only the " "current operating channels since " "scan_cur_freq is enabled");
			} else {
				os_free(params.freqs);
				params.freqs = NULL;
			}
...
#ifdef CONFIG_P2P
	if (wpa_s->p2p_in_provisioning || wpa_s->p2p_in_invitation || (wpa_s->show_group_started && wpa_s->go_params)) {
		/*
...
scan:
	 ...
	 */
	if (wpa_s->scan_for_connection && wpa_s->last_scan_req == NORMAL_SCAN_REQ && !scan_params->freqs && !params.freqs && wpas_is_p2p_prioritized(wpa_s) && wpa_s->p2p_group_interface == NOT_P2P_GROUP_INTERFACE && non_p2p_network_enabled(wpa_s)) {
		unsigned int num = wpa_s->num_multichan_concurrent;
	...
	if (ret && wpa_s->last_scan_req == MANUAL_SCAN_REQ && params.freqs && !wpa_s->manual_scan_freqs) {
...
void wpa_supplicant_update_scan_int(struct wpa_supplicant *wpa_s, int sec)
{
	...
	cancelled = eloop_cancel_timeout_one(wpa_supplicant_scan, wpa_s, NULL, &remaining);
	...
	if (cancelled) {
		eloop_register_timeout(new_int.sec, new_int.usec, wpa_supplicant_scan, wpa_s, NULL);
	}
...
void wpa_supplicant_req_scan(struct wpa_supplicant *wpa_s, int sec, int usec)
{
	...
	if (res == 1) {
		wpa_dbg(wpa_s, MSG_DEBUG, "Rescheduling scan request: %d.%06d sec", sec, usec);
	} else if (res == 0) {
		wpa_dbg(wpa_s, MSG_DEBUG, "Ignore new scan request for %d.%06d sec since an earlier request is scheduled to trigger sooner", sec, usec);
	} else {
...
int wpa_supplicant_delayed_sched_scan(struct wpa_supplicant *wpa_s, int sec, int usec)
{
	...
	eloop_register_timeout(sec, usec, wpa_supplicant_delayed_sched_scan_timeout, wpa_s, NULL);
...
int wpa_supplicant_req_sched_scan(struct wpa_supplicant *wpa_s)
{
...
#ifdef CONFIG_WPS
		if (!wpas_network_disabled(wpa_s, ssid) && ssid->key_mgmt == WPA_KEY_MGMT_WPS) {
			/*
			 ...
			 * be limited.
			 */
			wpa_dbg(wpa_s, MSG_DEBUG, "Use normal scan instead of " "sched_scan for WPS");
			return -1;
		}
#endif							/* CONFIG_WPS */
	}
	...
	if (wpa_s->normal_scans < 3 && (need_ssids <= wpa_s->max_scan_ssids || wpa_s->max_scan_ssids >= (int)max_sched_scan_ssids)) {
		/*
		 ...
		 */
		wpa_dbg(wpa_s, MSG_DEBUG, "Use normal scan instead of " "sched_scan for initial scans (normal_scans=%d)", wpa_s->normal_scans);
		return -1;
	}
	...
	params.filter_ssids = os_calloc(wpa_s->max_match_sets, sizeof(struct wpa_driver_scan_filter));
	...
	if (!ssid || !wpa_s->prev_sched_ssid) {
		...
		if (wpa_s->conf->sched_scan_interval) {
			wpa_s->sched_scan_interval = wpa_s->conf->sched_scan_interval;
		}
	...
	while (ssid) {
		...
		if (params.num_filter_ssids < wpa_s->max_match_sets && params.filter_ssids && ssid->ssid && ssid->ssid_len) {
			wpa_dbg(wpa_s, MSG_DEBUG, "add to filter ssid: %s", wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
			os_memcpy(params.filter_ssids[params.num_filter_ssids].ssid, ssid->ssid, ssid->ssid_len);
			...
			wpa_dbg(wpa_s, MSG_DEBUG, "Not enough room for SSID " "filter for sched_scan - drop filter");
			os_free(params.filter_ssids);
		...
		if (ssid->scan_ssid && ssid->ssid && ssid->ssid_len) {
			...
			wpa_dbg(wpa_s, MSG_DEBUG, "add to active scan ssid: %s", wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
			params.ssids[params.num_ssids].ssid = ssid->ssid;
			params.ssids[params.num_ssids].ssid_len = ssid->ssid_len;
			params.num_ssids++;
			if (params.num_ssids >= max_sched_scan_ssids) {
				...
				do {
					ssid = ssid->next;
				} while (ssid && (wpas_network_disabled(wpa_s, ssid) || !ssid->scan_ssid));
				break;
			}
		}
...
scan:
	if (ssid || !wpa_s->first_sched_scan) {
		wpa_dbg(wpa_s, MSG_DEBUG, "Starting sched scan: interval %d timeout %d", wpa_s->sched_scan_interval, wpa_s->sched_scan_timeout);
	} else {
		wpa_dbg(wpa_s, MSG_DEBUG, "Starting sched scan: interval %d (no timeout)", wpa_s->sched_scan_interval);
	}
	...
	ret = wpa_supplicant_start_sched_scan(wpa_s, scan_params, wpa_s->sched_scan_interval);
	wpabuf_free(extra_ie);
	os_free(params.filter_ssids);
	if (ret) {
	...
	if (ssid || !wpa_s->first_sched_scan) {
		...
		eloop_register_timeout(wpa_s->sched_scan_timeout, 0, wpa_supplicant_sched_scan_timeout, wpa_s, NULL);
...
#define MIN(a, b) a < b ? a : b
	...
	wpa_a = wpa_scan_get_vendor_ie(wa, WPA_IE_VENDOR_TYPE) != NULL || wpa_scan_get_ie(wa, WLAN_EID_RSN) != NULL;
	wpa_b = wpa_scan_get_vendor_ie(wb, WPA_IE_VENDOR_TYPE) != NULL || wpa_scan_get_ie(wb, WLAN_EID_RSN) != NULL;
	...
	if ((wa->caps & IEEE80211_CAP_PRIVACY) == 0 && (wb->caps & IEEE80211_CAP_PRIVACY)) {
		return 1;
	}
	if ((wa->caps & IEEE80211_CAP_PRIVACY) && (wb->caps & IEEE80211_CAP_PRIVACY) == 0) {
		return -1;
	}
...
static void dump_scan_res(struct wpa_scan_results *scan_res)
{
	...
	for (i = 0; i < scan_res->num; i++) {
		...
		if (r->flags & WPA_SCAN_LEVEL_DBM) {
			...
			wpa_printf(MSG_EXCESSIVE, MACSTR " freq=%d qual=%d " "noise=%d%s level=%d snr=%d%s flags=0x%x age=%u est=%u", MAC2STR(r->bssid), r->freq, r->qual, r->noise, noise_valid ? "" : "~", r->level, r->snr, r->snr >= GREAT_SNR ? "*" : "", r->flags, r->age, r->est_throughput);
		} else {
			wpa_printf(MSG_EXCESSIVE, MACSTR " freq=%d qual=%d " "noise=%d level=%d flags=0x%x age=%u est=%u", MAC2STR(r->bssid), r->freq, r->qual, r->noise, r->level, r->flags, r->age, r->est_throughput);
		}
		pos = (u8 *)(r + 1);
		if (r->ie_len) {
...
static void filter_scan_res(struct wpa_supplicant *wpa_s, struct wpa_scan_results *res)
{
	size_t i, j;
...
static void scan_snr(struct wpa_scan_res *res)
{
	if (res->flags & WPA_SCAN_NOISE_INVALID) {
		res->noise = IS_5GHZ(res->freq) ? DEFAULT_NOISE_FLOOR_5GHZ : DEFAULT_NOISE_FLOOR_2GHZ;
	}
...
static void scan_est_throughput(struct wpa_supplicant *wpa_s, struct wpa_scan_res *res)
{
	...
	if (capab == CAPAB_HT40 || capab == CAPAB_VHT) {
		...
		if (ie && ie[1] >= 2 && (ie[3] & HT_INFO_HT_PARAM_SECONDARY_CHNL_OFF_MASK)) {
...
struct wpa_driver_scan_params *wpa_scan_clone_params(const struct wpa_driver_scan_params *src)
{
	struct wpa_driver_scan_params *params;
	size_t i;
	u8 *n;
	...
	if (src->filter_ssids) {
		params->filter_ssids = os_malloc(sizeof(*params->filter_ssids) * src->num_filter_ssids);
		if (params->filter_ssids == NULL) {
			goto failed;
		}
		os_memcpy(params->filter_ssids, src->filter_ssids, sizeof(*params->filter_ssids) * src->num_filter_ssids);
...
int wpas_start_pno(struct wpa_supplicant *wpa_s)
{
	...
	if ((wpa_s->wpa_state > WPA_SCANNING) && (wpa_s->wpa_state <= WPA_COMPLETED)) {
	...
	if (wpa_s->wpa_state == WPA_SCANNING) {
		...
		if (wpa_s->sched_scanning) {
			wpa_printf(MSG_DEBUG, "Schedule PNO on completion of " "ongoing sched scan");
			wpa_supplicant_cancel_sched_scan(wpa_s);
	...
	if (num_ssid > WPAS_MAX_SCAN_SSIDS) {
		wpa_printf(MSG_DEBUG, "PNO: Use only the first %u SSIDs from " "%u", WPAS_MAX_SCAN_SSIDS, (unsigned int)num_ssid);
	...
	params.filter_ssids = os_calloc(num_match_ssid, sizeof(struct wpa_driver_scan_filter));
	if (params.filter_ssids == NULL) {
		return -1;
	}
	...
	interval = wpa_s->conf->sched_scan_interval ? wpa_s->conf->sched_scan_interval : 10;
