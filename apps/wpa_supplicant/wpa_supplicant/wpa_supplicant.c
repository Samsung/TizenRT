				   ...
				   struct wpa_ssid *ssid)
{
	...
	if (ssid) {
		...
		wpa_s->connect_without_scan =
			((ssid->mode == WPAS_MODE_MESH) ||(ssid->mode == WPAS_MODE_AP) )? ssid : NULL;
...
int wpa_supplicant_driver_init(struct wpa_supplicant *wpa_s)
{
	...
	wpa_dbg(wpa_s, MSG_INFO, "Own MAC address: " MACSTR,
				       ...
				       void *cb_ctx)
{
	...
	eloop_register_timeout(RRM_NEIGHBOR_REPORT_TIMEOUT, 0,
			       (eloop_timeout_handler)wpas_rrm_neighbor_rep_timeout_handler,
			       &wpa_s->rrm, NULL);
