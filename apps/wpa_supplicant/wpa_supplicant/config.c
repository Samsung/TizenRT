...
static int wpa_config_parse_str(const struct parse_data *data, struct wpa_ssid *ssid, int line, const char *value)
{
	...
	if (tmp == NULL) {
		wpa_printf(MSG_ERROR, "Line %d: failed to parse %s '%s'.", line, data->name, data->key_data ? "[KEY DATA REMOVED]" : value);
		return -1;
	}
	...
	if (data->param3 && res_len < (size_t)data->param3) {
		wpa_printf(MSG_ERROR, "Line %d: too short %s (len=%lu " "min_len=%ld)", line, data->name, (unsigned long)res_len, (long)data->param3);
		os_free(tmp);
		return -1;
	}

	if (data->param4 && res_len > (size_t)data->param4) {
		wpa_printf(MSG_ERROR, "Line %d: too long %s (len=%lu " "max_len=%ld)", line, data->name, (unsigned long)res_len, (long)data->param4);
		os_free(tmp);
		return -1;
	}
...
static int wpa_config_parse_int(const struct parse_data *data, struct wpa_ssid *ssid, int line, const char *value)
{
	...
	*dst = val;
	...
	if (data->param3 && *dst < (long)data->param3) {
		wpa_printf(MSG_ERROR, "Line %d: too small %s (value=%d " "min_value=%ld)", line, data->name, *dst, (long)data->param3);
		*dst = (long)data->param3;
		return -1;
	}

	if (data->param4 && *dst > (long)data->param4) {
		wpa_printf(MSG_ERROR, "Line %d: too large %s (value=%d " "max_value=%ld)", line, data->name, *dst, (long)data->param4);
...
static int wpa_config_parse_addr_list(const struct parse_data *data, int line, const char *value, u8 **list, size_t *num, char *name, u8 abort_on_error, u8 masked)
{
	const char *pos;
	...
	while (pos && *pos) {
		...
		if (hwaddr_masked_aton(pos, addr, &addr[ETH_ALEN], masked)) {
			if (abort_on_error || count == 0) {
				wpa_printf(MSG_ERROR, "Line %d: Invalid %s address '%s'", line, name, value);
				os_free(buf);
				return -1;
			}
			/* continue anyway since this could have been from a
			 * truncated configuration file line */
			wpa_printf(MSG_INFO, "Line %d: Ignore likely truncated %s address '%s'", line, name, pos);
		} else {
			...
			count++;
			wpa_printf(MSG_MSGDUMP, "%s: addr=" MACSTR " mask=" MACSTR, name, MAC2STR(addr), MAC2STR(&addr[ETH_ALEN]));
		}
...
#ifndef NO_CONFIG_WRITE
static char *wpa_config_write_addr_list(const struct parse_data *data, const u8 *list, size_t num, char *name)
{
	char *value, *end, *pos;
	int res;
	size_t i;
...
{
	if (value[0] == '\0' || os_strcmp(value, "\"\"") == 0 || os_strcmp(value, "any") == 0) {
		ssid->bssid_set = 0;
...
static int wpa_config_parse_bssid_blacklist(const struct parse_data *data, struct wpa_ssid *ssid, int line, const char *value)
{
	return wpa_config_parse_addr_list(data, line, value, &ssid->bssid_blacklist, &ssid->num_bssid_blacklist, "bssid_blacklist", 1, 1);
}

#ifndef NO_CONFIG_WRITE
static char *wpa_config_write_bssid_blacklist(const struct parse_data *data, struct wpa_ssid *ssid)
{
	return wpa_config_write_addr_list(data, ssid->bssid_blacklist, ssid->num_bssid_blacklist, "bssid_blacklist");
}
#endif							/* NO_CONFIG_WRITE */

static int wpa_config_parse_bssid_whitelist(const struct parse_data *data, struct wpa_ssid *ssid, int line, const char *value)
{
	return wpa_config_parse_addr_list(data, line, value, &ssid->bssid_whitelist, &ssid->num_bssid_whitelist, "bssid_whitelist", 1, 1);
}

#ifndef NO_CONFIG_WRITE
static char *wpa_config_write_bssid_whitelist(const struct parse_data *data, struct wpa_ssid *ssid)
{
	return wpa_config_write_addr_list(data, ssid->bssid_whitelist, ssid->num_bssid_whitelist, "bssid_whitelist");
}
#endif							/* NO_CONFIG_WRITE */
...
static int wpa_config_parse_psk(const struct parse_data *data, struct wpa_ssid *ssid, int line, const char *value)
{
	...
	if (*value == '"') {
		...
		wpa_printf(MSG_ERROR, "Line %d: ASCII passphrase not " "supported.", line);
		return -1;
...
static char *wpa_config_write_psk(const struct parse_data *data, struct wpa_ssid *ssid)
{
	...
	if (ssid->passphrase) {
		return wpa_config_write_string_ascii((const u8 *)ssid->passphrase, os_strlen(ssid->passphrase));
	}
...
static char *wpa_config_write_key_mgmt(const struct parse_data *data, struct wpa_ssid *ssid)
{
	...
	if (ssid->key_mgmt & WPA_KEY_MGMT_PSK_SHA256) {
		ret = os_snprintf(pos, end - pos, "%sWPA-PSK-SHA256", pos == buf ? "" : " ");
		if (os_snprintf_error(end - pos, ret)) {
			end[-1] = '\0';
			return buf;
		}
	...
	if (ssid->key_mgmt & WPA_KEY_MGMT_IEEE8021X_SUITE_B) {
		ret = os_snprintf(pos, end - pos, "%sWPA-EAP-SUITE-B", pos == buf ? "" : " ");
		if (os_snprintf_error(end - pos, ret)) {
			end[-1] = '\0';
			return buf;
		}
	...
	if (ssid->key_mgmt & WPA_KEY_MGMT_IEEE8021X_SUITE_B_192) {
		ret = os_snprintf(pos, end - pos, "%sWPA-EAP-SUITE-B-192", pos == buf ? "" : " ");
		if (os_snprintf_error(end - pos, ret)) {
			end[-1] = '\0';
			return buf;
		}
...
static int wpa_config_parse_pairwise(const struct parse_data *data, struct wpa_ssid *ssid, int line, const char *value)
{
	...
	if (val & ~WPA_ALLOWED_PAIRWISE_CIPHERS) {
		wpa_printf(MSG_ERROR, "Line %d: not allowed pairwise cipher " "(0x%x).", line, val);
		return -1;
	}
...
static int wpa_config_parse_group(const struct parse_data *data, struct wpa_ssid *ssid, int line, const char *value)
{
	...
	if (val & ~WPA_ALLOWED_GROUP_CIPHERS) {
		wpa_printf(MSG_ERROR, "Line %d: not allowed group cipher " "(0x%x).", line, val);
		return -1;
	}
...
static int wpa_config_parse_eap(const struct parse_data *data, struct wpa_ssid *ssid, int line, const char *value)
{
	int last, errors = 0;
	char *start, *end, *buf;
	...
	while (*start != '\0') {
		...
		*end = '\0';
		...
		if (methods[num_methods].vendor == EAP_VENDOR_IETF && methods[num_methods].method == EAP_TYPE_NONE) {
			wpa_printf(MSG_ERROR, "Line %d: unknown EAP method " "'%s'", line, start);
			wpa_printf(MSG_ERROR, "You may need to add support for" " this EAP method during wpa_supplicant\n" "build time configuration.\n" "See README for more information.");
			errors++;
...
#ifndef NO_CONFIG_WRITE
static char *wpa_config_write_eap(const struct parse_data *data, struct wpa_ssid *ssid)
{
	int i, ret;
	char *buf, *pos, *end;
	...
	for (i = 0; eap_methods[i].vendor != EAP_VENDOR_IETF || eap_methods[i].method != EAP_TYPE_NONE; i++) {
...
static int wpa_config_parse_password(const struct parse_data *data, struct wpa_ssid *ssid, int line, const char *value)
{
	u8 *hash;
	...
	if (os_strncmp(value, "hash:", 5) != 0) {
		...
		tmp = wpa_config_parse_string(value, &res_len);
		if (tmp == NULL) {
			wpa_printf(MSG_ERROR, "Line %d: failed to parse " "password.", line);
			return -1;
		}
		wpa_hexdump_ascii_key(MSG_MSGDUMP, data->name, (u8 *)tmp, res_len);
	...
	if (os_strlen(value + 5) != 2 * 16) {
		wpa_printf(MSG_ERROR, "Line %d: Invalid password hash length " "(expected 32 hex digits)", line);
		return -1;
	}
...
#ifndef NO_CONFIG_WRITE
static char *wpa_config_write_password(const struct parse_data *data, struct wpa_ssid *ssid)
{
	char *buf;
...
static int wpa_config_parse_wep_key(u8 *key, size_t *len, int line, const char *value, int idx)
{
	...
	if (buf == NULL) {
		wpa_printf(MSG_ERROR, "Line %d: Invalid WEP key %d '%s'.", line, idx, value);
		return -1;
	}
	if (*len > MAX_WEP_KEY_LEN) {
		wpa_printf(MSG_ERROR, "Line %d: Too long WEP key %d '%s'.", line, idx, value);
		os_free(buf);
		return -1;
	}
	if (*len && *len != 5 && *len != 13 && *len != 16) {
		wpa_printf(MSG_ERROR, "Line %d: Invalid WEP key length %u - " "this network block will be ignored", line, (unsigned int)*len);
	}
...
static int wpa_config_parse_go_p2p_dev_addr(const struct parse_data *data, struct wpa_ssid *ssid, int line, const char *value)
{
	if (value[0] == '\0' || os_strcmp(value, "\"\"") == 0 || os_strcmp(value, "any") == 0) {
	...
	if (hwaddr_aton(value, ssid->go_p2p_dev_addr)) {
		wpa_printf(MSG_ERROR, "Line %d: Invalid GO P2P Device Address '%s'.", line, value);
		return -1;
	}
	ssid->bssid_set = 1;
	wpa_printf(MSG_MSGDUMP, "GO P2P Device Address " MACSTR, MAC2STR(ssid->go_p2p_dev_addr));
	return 0;
}

#ifndef NO_CONFIG_WRITE
static char *wpa_config_write_go_p2p_dev_addr(const struct parse_data *data, struct wpa_ssid *ssid)
{
	char *value;
	int res;
...
static int wpa_config_parse_p2p_client_list(const struct parse_data *data, struct wpa_ssid *ssid, int line, const char *value)
{
	return wpa_config_parse_addr_list(data, line, value, &ssid->p2p_client_list, &ssid->num_p2p_clients, "p2p_client_list", 0, 0);
}

#ifndef NO_CONFIG_WRITE
static char *wpa_config_write_p2p_client_list(const struct parse_data *data, struct wpa_ssid *ssid)
{
	return wpa_config_write_addr_list(data, ssid->p2p_client_list, ssid->num_p2p_clients, "p2p_client_list");
}
#endif							/* NO_CONFIG_WRITE */

static int wpa_config_parse_psk_list(const struct parse_data *data, struct wpa_ssid *ssid, int line, const char *value)
{
...
#ifndef NO_CONFIG_WRITE
static char *wpa_config_write_psk_list(const struct parse_data *data, struct wpa_ssid *ssid)
{
	return NULL;
}
#endif							/* NO_CONFIG_WRITE */
...
static int wpa_config_parse_mesh_basic_rates(const struct parse_data *data, struct wpa_ssid *ssid, int line, const char *value)
{
	int *rates = wpa_config_parse_int_array(value);

	if (rates == NULL) {
		wpa_printf(MSG_ERROR, "Line %d: Invalid mesh_basic_rates '%s'", line, value);
		return -1;
	}
	if (rates[0] == 0) {
		os_free(rates);
...
static char *wpa_config_write_mesh_basic_rates(const struct parse_data *data, struct wpa_ssid *ssid)
{
	...
	for (i = 0; i < NUM_SSID_FIELDS; i++) {
		...
		if (os_strcmp(var, field->name) == 0) {
			...
			if (ret && has_newline(ret)) {
				wpa_printf(MSG_ERROR, "Found newline in value for %s; not returning it", var);
				os_free(ret);
				ret = NULL;
			}
...
char *wpa_config_get_no_key(struct wpa_ssid *ssid, const char *var)
{
	...
	for (i = 0; i < NUM_SSID_FIELDS; i++) {
		...
		if (os_strcmp(var, field->name) == 0) {
			...
			if (field->key_data) {
				if (res && res[0]) {
					wpa_printf(MSG_DEBUG, "Do not allow " "key_data field to be " "exposed");
					str_clear_free(res);
					return os_strdup("*");
				}
...
static int wpa_config_set_cred_req_conn_capab(struct wpa_cred *cred, const char *value)
{
	u8 *proto;
	int **port;
	int *ports, *nports;
	...
	proto = os_realloc_array(cred->req_conn_capab_proto, cred->num_req_conn_capab + 1, sizeof(u8));
	if (proto == NULL) {
		return -1;
	}
	cred->req_conn_capab_proto = proto;

	port = os_realloc_array(cred->req_conn_capab_port, cred->num_req_conn_capab + 1, sizeof(int *));
	if (port == NULL) {
		return -1;
	}
...
int wpa_config_set_cred(struct wpa_cred *cred, const char *var, const char *value, int line)
{
	...
	if (os_strcmp(var, "eap") == 0) {
		...
		if (method.vendor == EAP_VENDOR_IETF && method.method == EAP_TYPE_NONE) {
			wpa_printf(MSG_ERROR, "Line %d: unknown EAP type '%s' " "for a credential", line, value);
			return -1;
		}
		os_free(cred->eap_method);
	...
	if (os_strcmp(var, "domain") == 0) {
		char **new_domain;
		new_domain = os_realloc_array(cred->domain, cred->num_domain + 1, sizeof(char *));
		if (new_domain == NULL) {
			os_free(val);
			return -1;
		}
	...
	if (os_strcmp(var, "roaming_consortium") == 0) {
		if (len < 3 || len > sizeof(cred->roaming_consortium)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid " "roaming_consortium length %d (3..15 " "expected)", line, (int)len);
			os_free(val);
			return -1;
		}
	...
	if (os_strcmp(var, "required_roaming_consortium") == 0) {
		if (len < 3 || len > sizeof(cred->required_roaming_consortium)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid " "required_roaming_consortium length %d " "(3..15 expected)", line, (int)len);
			os_free(val);
			return -1;
		}
	...
	if (os_strcmp(var, "excluded_ssid") == 0) {
		...
		if (len > SSID_MAX_LEN) {
			wpa_printf(MSG_ERROR, "Line %d: invalid " "excluded_ssid length %d", line, (int)len);
			os_free(val);
			return -1;
		}

		e = os_realloc_array(cred->excluded_ssid, cred->num_excluded_ssid + 1, sizeof(struct excluded_ssid));
		if (e == NULL) {
			os_free(val);
			return -1;
		}
	...
	if (os_strcmp(var, "roaming_partner") == 0) {
		...
		p = os_realloc_array(cred->roaming_partner, cred->num_roaming_partner + 1, sizeof(struct roaming_partner));
		if (p == NULL) {
			os_free(val);
			return -1;
		}
...
char *wpa_config_get_cred_no_key(struct wpa_cred *cred, const char *var)
{
	...
	if (os_strcmp(var, "eap") == 0) {
		...
		return alloc_strdup(eap_get_name(cred->eap_method[0].vendor, cred->eap_method[0].method));
	}
	...
	if (os_strcmp(var, "req_conn_capab") == 0) {
		...
		for (i = 0; i < cred->num_req_conn_capab; i++) {
			...
			if (ports) {
				int j;
				for (j = 0; ports[j] != -1; j++) {
					ret = os_snprintf(pos, end - pos, "%s%d", j > 0 ? "," : ":", ports[j]);
					if (os_snprintf_error(end - pos, ret)) {
						return buf;
					}
					pos += ret;
	...
	if (os_strcmp(var, "roaming_consortium") == 0) {
		...
		wpa_snprintf_hex(buf, buflen, cred->roaming_consortium, cred->roaming_consortium_len);
		return buf;
	}
	...
	if (os_strcmp(var, "required_roaming_consortium") == 0) {
		...
		wpa_snprintf_hex(buf, buflen, cred->required_roaming_consortium, cred->required_roaming_consortium_len);
		return buf;
	}
...
void wpa_config_debug_dump_networks(struct wpa_config *config)
{
	...
	for (prio = 0; prio < config->num_prio; prio++) {
		...
		while (ssid) {
			wpa_printf(MSG_DEBUG, "   id=%d ssid='%s'", ssid->id, wpa_ssid_txt(ssid->ssid, ssid->ssid_len));
			ssid = ssid->pnext;
		}
	}
}
...
static int wpa_global_config_parse_str(const struct global_parse_data *data, struct wpa_config *config, int line, const char *pos)
{
	...
	if (data->param2 && len < (size_t)data->param2) {
		wpa_printf(MSG_ERROR, "Line %d: too short %s (len=%lu " "min_len=%ld)", line, data->name, (unsigned long)len, (long)data->param2);
		return -1;
	}
...
static int wpa_global_config_parse_bin(const struct global_parse_data *data, struct wpa_config *config, int line, const char *pos)
{
	size_t len;
...
static int wpa_config_process_freq_list(const struct global_parse_data *data, struct wpa_config *config, int line, const char *value)
{
	int *freqs;
...
#ifdef CONFIG_P2P
static int wpa_global_config_parse_ipv4(const struct global_parse_data *data, struct wpa_config *config, int line, const char *pos)
{
	u32 *dst;
...
static int wpa_config_process_country(const struct global_parse_data *data, struct wpa_config *config, int line, const char *pos)
{
	...
	wpa_printf(MSG_DEBUG, "country='%c%c'", config->country[0], config->country[1]);
	return 0;
}
...
static int wpa_config_process_load_dynamic_eap(const struct global_parse_data *data, struct wpa_config *config, int line, const char *so)
{
	...
	if (ret == -2) {
		wpa_printf(MSG_DEBUG, "This EAP type was already loaded - not " "reloading.");
	} else if (ret) {
		wpa_printf(MSG_ERROR, "Line %d: Failed to load dynamic EAP " "method '%s'.", line, so);
		return -1;
	}
...
static int wpa_config_process_uuid(const struct global_parse_data *data, struct wpa_config *config, int line, const char *pos)
{
	char buf[40];
...
static int wpa_config_process_device_type(const struct global_parse_data *data, struct wpa_config *config, int line, const char *pos)
{
	return wps_dev_type_str2bin(pos, config->device_type);
}

static int wpa_config_process_os_version(const struct global_parse_data *data, struct wpa_config *config, int line, const char *pos)
{
...
static int wpa_config_process_wps_vendor_ext_m1(const struct global_parse_data *data, struct wpa_config *config, int line, const char *pos)
{
	struct wpabuf *tmp;
	...
	if (!len) {
		wpa_printf(MSG_ERROR, "Line %d: " "invalid wps_vendor_ext_m1", line);
		return -1;
	}
	...
	if (tmp) {
		...
		wpa_printf(MSG_ERROR, "Can not allocate " "memory for wps_vendor_ext_m1");
		return -1;
	}
...
#ifdef CONFIG_P2P
static int wpa_config_process_sec_device_type(const struct global_parse_data *data, struct wpa_config *config, int line, const char *pos)
{
	int idx;

	if (config->num_sec_device_types >= MAX_SEC_DEVICE_TYPES) {
		wpa_printf(MSG_ERROR, "Line %d: too many sec_device_type " "items", line);
		return -1;
	}
...
static int wpa_config_process_p2p_pref_chan(const struct global_parse_data *data, struct wpa_config *config, int line, const char *pos)
{
	...
	wpa_hexdump(MSG_DEBUG, "P2P: Preferred class/channel pairs", (u8 *)config->p2p_pref_chan, config->num_p2p_pref_chan * sizeof(struct p2p_channel));
...
static int wpa_config_process_p2p_no_go_freq(const struct global_parse_data *data, struct wpa_config *config, int line, const char *pos)
{
	int ret;
	...
	wpa_printf(MSG_DEBUG, "P2P: p2p_no_go_freq with %u items", config->p2p_no_go_freq.num);
...
static int wpa_config_process_hessid(const struct global_parse_data *data, struct wpa_config *config, int line, const char *pos)
{
...
static int wpa_config_process_sae_groups(const struct global_parse_data *data, struct wpa_config *config, int line, const char *pos)
{
...
static int wpa_config_process_ap_vendor_elements(const struct global_parse_data *data, struct wpa_config *config, int line, const char *pos)
{
	struct wpabuf *tmp;
	int len = os_strlen(pos) / 2;
	u8 *p;
	...
	if (tmp) {
		...
		wpa_printf(MSG_ERROR, "Cannot allocate memory for " "ap_vendor_elements");
		return -1;
	}
...
static int wpa_config_process_no_ctrl_interface(const struct global_parse_data *data, struct wpa_config *config, int line, const char *pos)
{
...
#ifdef CONFIG_P2P
static int wpa_config_get_ipv4(const char *name, struct wpa_config *config, long offset, char *buf, size_t buflen, int pretty_print)
{
