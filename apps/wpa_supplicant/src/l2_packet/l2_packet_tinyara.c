...
static void wpa_receive_eapol_cb (struct wpa_supplicant *wpa_s,
					   struct wpa_eapol_key_work *cwork)
{
	wpa_printf (MSG_DEBUG, "Post EAPOL Rx");

	cwork->l2->rx_callback(wpa_s->l2->rx_callback_ctx, cwork->addr,
			  cwork->buf, cwork->len);
 ...
 void l2_packet_receive(void *ctx, const u8 *src_addr, u8 * ptr ,u16 len)
{
	...
	wpa_printf (MSG_DEBUG, "%s: recvd EAPOL src=" MACSTR " len=%d",
				__func__, MAC2STR(src_addr), (int) len);
