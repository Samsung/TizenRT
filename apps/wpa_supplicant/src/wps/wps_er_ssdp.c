...
static void wps_er_ssdp_rx(int sd, void *eloop_ctx, void *sock_ctx)
{
	...
	nread = recvfrom(sd, buf, sizeof(buf) - 1, 0, (struct sockaddr *)&addr, &addr_len);
	if (nread <= 0) {
		return;
	}
	buf[nread] = '\0';
	...
	wpa_printf(MSG_DEBUG, "WPS ER: Received SSDP from %s", inet_ntoa(addr.sin_addr));
	wpa_hexdump_ascii(MSG_MSGDUMP, "WPS ER: Received SSDP contents", (u8 *)buf, nread);
	...
	for (start = buf; start && *start; start = pos) {
		...
		if (os_strstr(start, "schemas-wifialliance-org:device:" "WFADevice:1")) {
			wfa = 1;
		}
		if (os_strstr(start, "schemas-wifialliance-org:service:" "WFAWLANConfig:1")) {
			wfa = 1;
		}
		if (os_strncasecmp(start, "LOCATION:", 9) == 0) {
			...
			if (pos2) {
				...
				if (uuid_str2bin(pos2, uuid) < 0) {
					wpa_printf(MSG_DEBUG, "WPS ER: " "Invalid UUID in USN: %s", pos2);
					return;
				}
			}
		}
	...
	wpa_printf(MSG_DEBUG, "WPS ER: AP discovered: %s " "(packet source: %s  max-age: %d)", location, inet_ntoa(addr.sin_addr), max_age);
...
void wps_er_send_ssdp_msearch(struct wps_er *er)
{
	...
	if (sendto(er->multicast_sd, wpabuf_head(msg), wpabuf_len(msg), 0, (struct sockaddr *)&dest, sizeof(dest)) < 0) {
		wpa_printf(MSG_DEBUG, "WPS ER: M-SEARCH sendto failed: " "%d (%s)", errno, strerror(errno));
	}
...
int wps_er_ssdp_init(struct wps_er *er)
{
	if (add_ssdp_network(er->ifname)) {
		wpa_printf(MSG_INFO, "WPS ER: Failed to add routing entry for " "SSDP");
		return -1;
	}

	er->multicast_sd = ssdp_open_multicast_sock(er->ip_addr, er->forced_ifname ? er->ifname : NULL);
	if (er->multicast_sd < 0) {
		wpa_printf(MSG_INFO, "WPS ER: Failed to open multicast socket " "for SSDP");
		return -1;
	}

	er->ssdp_sd = ssdp_listener_open();
	if (er->ssdp_sd < 0) {
		wpa_printf(MSG_INFO, "WPS ER: Failed to open SSDP listener " "socket");
		return -1;
	}

	if (eloop_register_sock(er->multicast_sd, EVENT_TYPE_READ, wps_er_ssdp_rx, er, NULL) || eloop_register_sock(er->ssdp_sd, EVENT_TYPE_READ, wps_er_ssdp_rx, er, NULL)) {
		return -1;
	}
