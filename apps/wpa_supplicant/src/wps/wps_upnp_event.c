...
static struct wps_event_ *event_dequeue(struct subscription *s)
{
	...
	if (e) {
		wpa_printf(MSG_DEBUG, "WPS UPnP: Dequeue event %p for " "subscription %p", e, s);
		dl_list_del(&e->list);
	}
	return e;
}
...
static void event_retry(struct wps_event_ *e, int do_next_address)
{
	...
	if (e->retry >= dl_list_len(&s->addr_list)) {
		wpa_printf(MSG_DEBUG, "WPS UPnP: Giving up on sending event " "for %s", e->addr->domain_and_port);
		event_delete(e);
...
static void event_addr_failure(struct wps_event_ *e)
{
	...
	e->addr->num_failures++;
	wpa_printf(MSG_DEBUG, "WPS UPnP: Failed to send event %p to %s " "(num_failures=%u)", e, e->addr->domain_and_port, e->addr->num_failures);
	 ...
	 */
	wpa_printf(MSG_DEBUG, "WPS UPnP: Deleting subscription %p " "address %s due to errors", s, e->addr->domain_and_port);
	...
	if (dl_list_empty(&s->addr_list)) {
		...
		wpa_printf(MSG_DEBUG, "WPS UPnP: Removing subscription %p " "with no addresses", s);
		dl_list_del(&s->list);
...
static void event_http_cb(void *ctx, struct http_client *c, enum http_client_event event)
{
	...
	wpa_printf(MSG_DEBUG, "WPS UPnP: HTTP client callback: e=%p c=%p " "event=%d", e, c, event);
	switch (event) {
	case HTTP_CLIENT_OK:
		wpa_printf(MSG_DEBUG, "WPS UPnP: Got event %p reply OK from %s", e, e->addr->domain_and_port);
...
void event_send_all_later(struct upnp_wps_device_sm *sm)
{
	...
	eloop_register_timeout(EVENT_DELAY_SECONDS, EVENT_DELAY_MSEC, event_send_all_later_handler, NULL, sm);
}
...
int event_add(struct subscription *s, const struct wpabuf *data, int probereq)
{
	...
	if (len >= MAX_EVENTS_QUEUED) {
		wpa_printf(MSG_DEBUG, "WPS UPnP: Too many events queued for " "subscriber %p", s);
		if (probereq) {
			return 1;
		}
	...
	if (s->last_event_failed && probereq && len > 0) {
		 ...
		 */
		wpa_printf(MSG_DEBUG, "WPS UPnP: Do not queue more Probe " "Request frames for subscription %p since last " "delivery failed", s);
		return -1;
	}
	...
	wpa_printf(MSG_DEBUG, "WPS UPnP: Queue event %p for subscriber %p " "(queue len %u)", e, s, len + 1);
