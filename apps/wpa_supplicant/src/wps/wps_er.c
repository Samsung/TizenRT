...
static struct wps_er_sta *wps_er_sta_get(struct wps_er_ap *ap, const u8 *addr, const u8 *uuid)
{
	struct wps_er_sta *sta;
	dl_list_for_each(sta, &ap->sta, struct wps_er_sta, list) {
		if ((addr == NULL || os_memcmp(sta->addr, addr, ETH_ALEN) == 0) && (uuid == NULL || os_memcmp(uuid, sta->uuid, WPS_UUID_LEN) == 0)) {
			return sta;
		}
	}
	return NULL;
...
static void wps_er_sta_remove_all(struct wps_er_ap *ap)
{
	...
	dl_list_for_each_safe(sta, prev, &ap->sta, struct wps_er_sta, list) wps_er_sta_free(sta);
}

static struct wps_er_ap *wps_er_ap_get(struct wps_er *er, struct in_addr *addr, const u8 *uuid, const u8 *mac_addr)
{
	struct wps_er_ap *ap;
	dl_list_for_each(ap, &er->ap, struct wps_er_ap, list) {
		if ((addr == NULL || ap->addr.s_addr == addr->s_addr) && (uuid == NULL || os_memcmp(uuid, ap->uuid, WPS_UUID_LEN) == 0) && (mac_addr == NULL || os_memcmp(mac_addr, ap->mac_addr, ETH_ALEN) == 0)) {
			return ap;
		}
	}
	return NULL;
...
{
	wpa_printf(MSG_DEBUG, "WPS ER: Unsubscribed from AP %s (%s)", inet_ntoa(ap->addr), ap->location);
	dl_list_del(&ap->list);
	wps_er_ap_free(ap);
...
static void wps_er_http_unsubscribe_cb(void *ctx, struct http_client *c, enum http_client_event event)
{
	...
	case HTTP_CLIENT_TIMEOUT:
		wpa_printf(MSG_DEBUG, "WPS ER: Failed to unsubscribe from " "events");
		break;
	}
	http_client_free(ap->http);
	ap->http = NULL;
...
static void wps_er_ap_unsubscribe(struct wps_er *er, struct wps_er_ap *ap)
{
	...
	if (ap->event_sub_url == NULL) {
		wpa_printf(MSG_DEBUG, "WPS ER: No eventSubURL - cannot " "subscribe");
		goto fail;
	}
	if (ap->http) {
		wpa_printf(MSG_DEBUG, "WPS ER: Pending HTTP request - cannot " "send subscribe request");
		goto fail;
	}
	...
	wpabuf_printf(req, "UNSUBSCRIBE %s HTTP/1.1\r\n" "HOST: %s:%d\r\n" "SID: uuid:%s\r\n" "\r\n", path, inet_ntoa(dst.sin_addr), ntohs(dst.sin_port), sid);
	os_free(url);
	wpa_hexdump_ascii(MSG_MSGDUMP, "WPS ER: Unsubscription request", wpabuf_head(req), wpabuf_len(req));
...
static struct wps_er_ap_settings *wps_er_ap_get_settings(struct wps_er *er, const u8 *uuid)
{
...
int wps_er_ap_cache_settings(struct wps_er *er, struct in_addr *addr)
{
	...
	os_memcpy(&settings->ap_settings, ap->ap_settings, sizeof(struct wps_credential));

	return 0;
}

static int wps_er_ap_use_cached_settings(struct wps_er *er, struct wps_er_ap *ap)
{
	struct wps_er_ap_settings *s;
...
{
	wpa_printf(MSG_DEBUG, "WPS ER: Removing AP entry for %s (%s)", inet_ntoa(ap->addr), ap->location);
...
static int wps_er_get_sid(struct wps_er_ap *ap, char *sid)
{
	...
	if (!sid) {
		wpa_printf(MSG_DEBUG, "WPS ER: No SID received from %s (%s)", inet_ntoa(ap->addr), ap->location);
		return -1;
	}

	pos = os_strstr(sid, "uuid:");
	if (!pos) {
		wpa_printf(MSG_DEBUG, "WPS ER: Invalid SID received from " "%s (%s): '%s'", inet_ntoa(ap->addr), ap->location, sid);
		return -1;
	}
	...
	wpa_printf(MSG_DEBUG, "WPS ER: SID for subscription with %s (%s): %s", inet_ntoa(ap->addr), ap->location, txt);
...
static const char *wps_er_find_wfadevice(const char *data)
{
	...
	while (!found) {
		...
		for (;;) {
			...
			data = end;
			if (!os_strncasecmp(tagname, "device", 6) && *tag != '/' && (tagname[6] == '>' || !isgraph(tagname[6]))) {
				break;
			}
		}
		...
		found = os_strcasecmp(val, "urn:schemas-wifialliance-org:" "device:WFADevice:1") == 0;
		os_free(val);
	}
...
static void wps_er_parse_device_description(struct wps_er_ap *ap, struct wpabuf *reply)
{
	...
	wpa_hexdump_ascii(MSG_MSGDUMP, "WPS ER: Device info", wpabuf_head(reply), wpabuf_len(reply));
	...
	if (tmp == NULL) {
		wpa_printf(MSG_DEBUG, "WPS ER: WFADevice:1 device not found - " "trying to parse invalid data");
	} else {
		data = tmp;
	}
	...
	ap->scpd_url = http_link_update(xml_get_first_item(data, "SCPDURL"), ap->location);
	wpa_printf(MSG_DEBUG, "WPS ER: SCPDURL='%s'", ap->scpd_url);

	ap->control_url = http_link_update(xml_get_first_item(data, "controlURL"), ap->location);
	wpa_printf(MSG_DEBUG, "WPS ER: controlURL='%s'", ap->control_url);

	ap->event_sub_url = http_link_update(xml_get_first_item(data, "eventSubURL"), ap->location);
...
void wps_er_ap_add(struct wps_er *er, const u8 *uuid, struct in_addr *addr, const char *location, int max_age)
{
	...
	wpa_printf(MSG_DEBUG, "WPS ER: Added AP entry for %s (%s)", inet_ntoa(ap->addr), ap->location);

	/* Fetch device description */
	ap->http = http_client_url(ap->location, NULL, 10000, wps_er_http_dev_desc_cb, ap);
}
...
static void wps_er_ap_remove_all(struct wps_er *er)
{
	...
	dl_list_for_each_safe(ap, prev, &er->ap, struct wps_er_ap, list) wps_er_ap_remove_entry(er, ap);
	dl_list_for_each_safe(s, prev_s, &er->ap_settings, struct wps_er_ap_settings, list) os_free(s);
}
...
static struct wps_er_sta *wps_er_add_sta_data(struct wps_er_ap *ap, const u8 *addr, struct wps_parse_attr *attr, int probe_req)
{
	...
	if (attr->serial_number) {
		os_free(sta->serial_number);
		sta->serial_number = dup_binstr(attr->serial_number, attr->serial_number_len);
	}
...
static void wps_er_process_wlanevent_probe_req(struct wps_er_ap *ap, const u8 *addr, struct wpabuf *msg)
{
	...
	wpa_printf(MSG_DEBUG, "WPS ER: WLANEvent - Probe Request - from " MACSTR, MAC2STR(addr));
	wpa_hexdump_buf(MSG_MSGDUMP, "WPS ER: WLANEvent - Enrollee's message " "(TLVs from Probe Request)", msg);

	if (wps_validate_probe_req(msg, addr) < 0) {
		wpa_printf(MSG_INFO, "WPS-STRICT: ER: Ignore invalid proxied " "Probe Request frame from " MACSTR, MAC2STR(addr));
		return;
	}

	if (wps_parse_msg(msg, &attr) < 0) {
		wpa_printf(MSG_DEBUG, "WPS ER: Failed to parse TLVs in " "WLANEvent message");
		return;
	}
...
static const char *soap_prefix = "<?xml version=\"1.0\"?>\n" "<s:Envelope xmlns:s=\"http://schemas.xmlsoap.org/soap/envelope/\" " "s:encodingStyle=\"http://schemas.xmlsoap.org/soap/encoding/\">\n" "<s:Body>\n";
...
static struct wpabuf *wps_er_soap_hdr(const struct wpabuf *msg, const char *name, const char *arg_name, const char *path, const struct sockaddr_in *dst, char **len_ptr, char **body_ptr)
{
	...
	wpabuf_printf(buf, "POST %s HTTP/1.1\r\n" "Host: %s:%d\r\n" "Content-Type: text/xml; charset=\"utf-8\"\r\n" "Content-Length: ", path, inet_ntoa(dst->sin_addr), ntohs(dst->sin_port));
...
static void wps_er_soap_end(struct wpabuf *buf, const char *name, char *len_ptr, char *body_ptr)
{
	...
	os_snprintf(len_buf, sizeof(len_buf), "%d", (int)((char *)wpabuf_put(buf, 0) - body_ptr));
...
static void wps_er_sta_send_msg(struct wps_er_sta *sta, struct wpabuf *msg)
{
	...
	if (sta->http) {
		wpa_printf(MSG_DEBUG, "WPS ER: Pending HTTP request for STA - " "ignore new request");
		wpabuf_free(msg);
		return;
	}
	...
	buf = wps_er_soap_hdr(msg, "PutWLANResponse", "NewMessage", path, &dst, &len_ptr, &body_ptr);
	wpabuf_free(msg);
	os_free(url);
	if (buf == NULL) {
		return;
	...
	sta->http = http_client_addr(&dst, buf, 1000, wps_er_http_put_wlan_response_cb, sta);
	if (sta->http == NULL) {
		wpabuf_free(buf);
	}
}
...
static void wps_er_sta_process(struct wps_er_sta *sta, struct wpabuf *msg, enum wsc_op_code op_code)
{
	...
	if (res == WPS_CONTINUE) {
		...
		wpa_printf(MSG_DEBUG, "WPS ER: Protocol run %s with the " "enrollee (res=%d)", res == WPS_DONE ? "succeeded" : "failed", res);
		wps_deinit(sta->wps);
		sta->wps = NULL;
		if (res == WPS_DONE) {
...
static void wps_er_process_wlanevent_eap(struct wps_er_ap *ap, const u8 *addr, struct wpabuf *msg)
{
	...
	wpa_printf(MSG_DEBUG, "WPS ER: WLANEvent - EAP - from " MACSTR, MAC2STR(addr));
	wpa_hexdump_buf(MSG_MSGDUMP, "WPS ER: WLANEvent - Enrollee's message " "(TLVs from EAP-WSC)", msg);
...
static void wps_er_process_wlanevent(struct wps_er_ap *ap, struct wpabuf *event)
{
	...
	wpa_hexdump(MSG_MSGDUMP, "WPS ER: Received WLANEvent", wpabuf_head(event), wpabuf_len(event));
	...
	if (hwaddr_aton((char *)data + 1, wlan_event_mac) < 0) {
		wpa_printf(MSG_DEBUG, "WPS ER: Invalid WLANEventMAC in " "WLANEvent");
		return;
	}
...
static void wps_er_http_event(struct wps_er *er, struct http_request *req, unsigned int ap_id)
{
	...
	if (ap == NULL) {
		wpa_printf(MSG_DEBUG, "WPS ER: HTTP event from unknown AP id " "%u", ap_id);
		wps_er_http_resp_not_found(req);
		return;
	}
	wpa_printf(MSG_MSGDUMP, "WPS ER: HTTP event from AP id %u: %s", ap_id, http_request_get_data(req));

	event = xml_get_base64_item(http_request_get_data(req), "WLANEvent", &ret);
	if (event == NULL) {
		wpa_printf(MSG_DEBUG, "WPS ER: Could not extract WLANEvent " "from the event notification");
		/*
...
static void wps_er_http_notify(struct wps_er *er, struct http_request *req)
{
	...
	if (os_strncmp(uri, "/event/", 7) == 0) {
		...
		if (event_id != er->event_id) {
			wpa_printf(MSG_DEBUG, "WPS ER: HTTP event for an " "unknown event id %u", event_id);
			return;
		}
		pos = os_strchr(uri + 7, '/');
		if (pos == NULL) {
...
static void wps_er_http_req(void *ctx, struct http_request *req)
{
	...
	wpa_printf(MSG_DEBUG, "WPS ER: HTTP request: '%s' (type %d) from " "%s:%d", http_request_get_uri(req), type, inet_ntoa(cli->sin_addr), ntohs(cli->sin_port));
	...
	case HTTPREAD_HDR_TYPE_NOTIFY:
		...
		break;
	default:
		wpa_printf(MSG_DEBUG, "WPS ER: Unsupported HTTP request type " "%d", type);
		buf = wpabuf_alloc(200);
		if (buf == NULL) {
			http_request_deinit(req);
			return;
...
struct wps_er *wps_er_init(struct wps_context *wps, const char *ifname, const char *filter)
{
	...
	if (filter && os_strncmp(filter, "ifname=", 7) == 0) {
		...
		if (end) {
			size_t len = end - pos;
			os_strlcpy(er->ifname, pos, len < sizeof(er->ifname) ? len + 1 : sizeof(er->ifname));
			filter = end + 1;
		} else {
	...
	if (filter) {
		if (inet_aton(filter, &er->filter_addr) == 0) {
			wpa_printf(MSG_INFO, "WPS UPnP: Invalid filter " "address %s", filter);
			wps_er_deinit(er, NULL, NULL);
			return NULL;
		}
		wpa_printf(MSG_DEBUG, "WPS UPnP: Only accepting connections " "with %s", filter);
	}
	if (get_netif_info(er->ifname, &er->ip_addr, &er->ip_addr_text, er->mac_addr)) {
		wpa_printf(MSG_INFO, "WPS UPnP: Could not get IP/MAC address " "for %s. Does it have IP address?", er->ifname);
		wps_er_deinit(er, NULL, NULL);
		return NULL;
	}
	...
	wpa_printf(MSG_DEBUG, "WPS ER: Start (ifname=%s ip_addr=%s)", er->ifname, er->ip_addr_text);
...
void wps_er_refresh(struct wps_er *er)
{
	...
	dl_list_for_each(ap, &er->ap, struct wps_er_ap, list) {
		...
		dl_list_for_each(sta, &ap->sta, struct wps_er_sta, list) wps_er_sta_event(er->wps, sta, WPS_EV_ER_ENROLLEE_ADD);
	}
...
static void wps_er_deinit_finish(void *eloop_data, void *user_ctx)
{
	...
	dl_list_for_each_safe(ap, tmp, &er->ap_unsubscribing, struct wps_er_ap, list) {
		wpa_printf(MSG_DEBUG, "WPS ER: AP entry for %s (%s) still in ap_unsubscribing list - free it", inet_ntoa(ap->addr), ap->location);
		dl_list_del(&ap->list);
		wps_er_ap_free(ap);
	}
...
void wps_er_deinit(struct wps_er *er, void (*cb)(void *ctx), void *ctx)
{
	...
	eloop_register_timeout(dl_list_empty(&er->ap_unsubscribing) ? 0 : 5, 0, wps_er_deinit_finish, er, NULL);
...
static void wps_er_http_set_sel_reg_cb(void *ctx, struct http_client *c, enum http_client_event event)
{
	...
	if (data.set_sel_reg.uuid) {
		ap->er->wps->event_cb(ap->er->wps->cb_ctx, WPS_EV_ER_SET_SELECTED_REGISTRAR, &data);
	}
}
...
static void wps_er_send_set_sel_reg(struct wps_er_ap *ap, struct wpabuf *msg)
{
	...
	if (ap->http) {
		wpa_printf(MSG_DEBUG, "WPS ER: Pending HTTP request for AP - " "ignore new request");
		return;
	}

	if (ap->wps) {
		wpa_printf(MSG_DEBUG, "WPS ER: Pending WPS operation for AP - " "skip SetSelectedRegistrar");
		return;
	}
	...
	buf = wps_er_soap_hdr(msg, "SetSelectedRegistrar", "NewMessage", path, &dst, &len_ptr, &body_ptr);
	os_free(url);
	if (buf == NULL) {
		return;
	}
...
void wps_er_set_sel_reg(struct wps_er *er, int sel_reg, u16 dev_passwd_id, u16 sel_reg_config_methods)
{
	...
	if (wps_build_version(msg) || wps_er_build_selected_registrar(msg, sel_reg) || wps_er_build_dev_password_id(msg, dev_passwd_id) || wps_er_build_sel_reg_config_methods(msg, sel_reg_config_methods) || wps_build_wfa_ext(msg, 0, auth_macs, count) || wps_er_build_uuid_r(msg, er->wps->uuid)) {
		wpabuf_free(msg);
		return;
	}
	...
	dl_list_for_each(ap, &er->ap, struct wps_er_ap, list) {
		if (er->set_sel_reg_uuid_filter && os_memcmp(ap->uuid, er->set_sel_reg_uuid_filter, WPS_UUID_LEN) != 0) {
			continue;
		}
		data.set_sel_reg.uuid = ap->uuid;
...
int wps_er_pbc(struct wps_er *er, const u8 *uuid, const u8 *addr)
{
	...
	if (wps_registrar_pbc_overlap(er->wps->registrar, NULL, NULL)) {
		wpa_printf(MSG_DEBUG, "WPS ER: PBC overlap - do not start PBC " "mode");
		return -2;
	}
...
static void wps_er_http_put_message_cb(void *ctx, struct http_client *c, enum http_client_event event)
{
	...
	if (msg) {
		...
		os_free(msg);
		if (buf == NULL) {
			wpa_printf(MSG_DEBUG, "WPS ER: Could not extract " "NewOutMessage from PutMessage response");
			wps_deinit(ap->wps);
			ap->wps = NULL;
			return;
		}
...
static void wps_er_ap_put_message(struct wps_er_ap *ap, const struct wpabuf *msg)
{
	...
	if (ap->http) {
		wpa_printf(MSG_DEBUG, "WPS ER: Pending HTTP operation ongoing " "with the AP - cannot continue learn");
		return;
	}
	...
	buf = wps_er_soap_hdr(msg, "PutMessage", "NewInMessage", path, &dst, &len_ptr, &body_ptr);
	os_free(url);
	if (buf == NULL) {
		return;
	}
...
static void wps_er_ap_process(struct wps_er_ap *ap, struct wpabuf *msg)
{
	...
	if (res == WPS_CONTINUE) {
		...
		if (next) {
			...
			wpa_printf(MSG_DEBUG, "WPS ER: Failed to build " "message");
			wps_deinit(ap->wps);
			ap->wps = NULL;
		}
		...
		wpa_printf(MSG_DEBUG, "WPS ER: Failed to process message from " "AP (res=%d)", res);
		wps_deinit(ap->wps);
		ap->wps = NULL;
	}
}
...
static void wps_er_ap_learn_m1(struct wps_er_ap *ap, struct wpabuf *m1)
{
	...
	if (ap->wps) {
		wpa_printf(MSG_DEBUG, "WPS ER: Protocol run already in " "progress with this AP");
		return;
	}
...
static void wps_er_ap_learn(struct wps_er_ap *ap, const char *dev_info)
{
	...
	wpa_printf(MSG_DEBUG, "WPS ER: Received GetDeviceInfo response (M1) " "from the AP");
	info = xml_get_base64_item(dev_info, "NewDeviceInfo", &ret);
	if (info == NULL) {
		wpa_printf(MSG_DEBUG, "WPS ER: Could not extract " "NewDeviceInfo from GetDeviceInfo response");
		return;
	}
...
static int wps_er_send_get_device_info(struct wps_er_ap *ap, void (*m1_handler)(struct wps_er_ap *ap, struct wpabuf *m1))
{
	...
	if (ap->http) {
		wpa_printf(MSG_DEBUG, "WPS ER: Pending HTTP operation ongoing " "with the AP - cannot get device info");
		return -1;
	}
	...
	buf = wps_er_soap_hdr(NULL, "GetDeviceInfo", NULL, path, &dst, &len_ptr, &body_ptr);
	os_free(url);
	if (buf == NULL) {
		return -1;
	}
	...
	ap->http = http_client_addr(&dst, buf, 10000, wps_er_http_get_dev_info_cb, ap);
	if (ap->http == NULL) {
		wpabuf_free(buf);
		return -1;
	}
...
int wps_er_learn(struct wps_er *er, const u8 *uuid, const u8 *addr, const u8 *pin, size_t pin_len)
{
	...
	if (ap == NULL) {
		wpa_printf(MSG_DEBUG, "WPS ER: AP not found for learn " "request");
		return -1;
	}
	if (uuid == NULL) {
		uuid = ap->uuid;
	...
	if (ap->wps) {
		wpa_printf(MSG_DEBUG, "WPS ER: Pending operation ongoing " "with the AP - cannot start learn");
		return -1;
	}
...
int wps_er_set_config(struct wps_er *er, const u8 *uuid, const u8 *addr, const struct wps_credential *cred)
{
	...
	ap = wps_er_ap_get(er, NULL, uuid, addr);
	if (ap == NULL) {
		wpa_printf(MSG_DEBUG, "WPS ER: AP not found for set config " "request");
		return -1;
	}
	...
	os_memcpy(ap->ap_settings, cred, sizeof(*cred));
	ap->ap_settings->cred_attr = NULL;
	wpa_printf(MSG_DEBUG, "WPS ER: Updated local AP settings based set " "config request");
...
int wps_er_config(struct wps_er *er, const u8 *uuid, const u8 *addr, const u8 *pin, size_t pin_len, const struct wps_credential *cred)
{
	...
	ap = wps_er_ap_get(er, NULL, uuid, addr);
	if (ap == NULL) {
		wpa_printf(MSG_DEBUG, "WPS ER: AP not found for config " "request");
		return -1;
	}
	if (uuid == NULL) {
		uuid = ap->uuid;
	...
	if (ap->wps) {
		wpa_printf(MSG_DEBUG, "WPS ER: Pending operation ongoing " "with the AP - cannot start config");
		return -1;
	}
...
struct wpabuf *wps_er_nfc_config_token(struct wps_er *er, const u8 *uuid, const u8 *addr)
{
	...
	if (ap->ap_settings == NULL) {
		wpa_printf(MSG_DEBUG, "WPS ER: No settings known for the " "selected AP");
		return NULL;
	}
