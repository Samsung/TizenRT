...
static struct wps_nfc_pw_token *wps_get_nfc_pw_token(struct dl_list *tokens, u16 pw_id)
{
...
static void wps_registrar_remove_pin(struct wps_registrar *reg, struct wps_uuid_pin *pin);

static void wps_registrar_add_authorized_mac(struct wps_registrar *reg, const u8 *addr)
{
	int i;
	...
	for (i = 0; i < WPS_MAX_AUTHORIZED_MACS; i++)
		if (os_memcmp(reg->authorized_macs[i], addr, ETH_ALEN) == 0) {
			wpa_printf(MSG_DEBUG, "WPS: Authorized MAC was " "already in the list");
	...
	wpa_hexdump(MSG_DEBUG, "WPS: Authorized MACs", (u8 *)reg->authorized_macs, sizeof(reg->authorized_macs));
}

static void wps_registrar_remove_authorized_mac(struct wps_registrar *reg, const u8 *addr)
{
	int i;
	...
	if (i == WPS_MAX_AUTHORIZED_MACS) {
		wpa_printf(MSG_DEBUG, "WPS: Authorized MAC was not in the " "list");
...
static struct wps_registrar_device *wps_device_get(struct wps_registrar *reg, const u8 *addr)
{
...
static void wps_registrar_add_pbc_session(struct wps_registrar *reg, const u8 *addr, const u8 *uuid_e)
{
	...
	while (pbc) {
		if (os_memcmp(pbc->addr, addr, ETH_ALEN) == 0 && os_memcmp(pbc->uuid_e, uuid_e, WPS_UUID_LEN) == 0) {
			if (prev) {
				prev->next = pbc->next;
			} else {
...
static void wps_registrar_remove_pbc_session(struct wps_registrar *reg, const u8 *uuid_e, const u8 *p2p_dev_addr)
{
	struct wps_pbc_session *pbc, *prev = NULL, *tmp;

	pbc = reg->pbc_sessions;
	while (pbc) {
		if (os_memcmp(pbc->uuid_e, uuid_e, WPS_UUID_LEN) == 0 || (p2p_dev_addr && !is_zero_ether_addr(reg->p2p_dev_addr) && os_memcmp(reg->p2p_dev_addr, p2p_dev_addr, ETH_ALEN) == 0)) {
			if (prev) {
				prev->next = pbc->next;
			} else {
				reg->pbc_sessions = pbc->next;
			...
			tmp = pbc;
			pbc = pbc->next;
			wpa_printf(MSG_DEBUG, "WPS: Removing PBC session for " "addr=" MACSTR, MAC2STR(tmp->addr));
...
int wps_registrar_pbc_overlap(struct wps_registrar *reg, const u8 *addr, const u8 *uuid_e)
{
	...
	for (pbc = reg->pbc_sessions; pbc; pbc = pbc->next) {
		wpa_printf(MSG_DEBUG, "WPS: Consider PBC session with " MACSTR, MAC2STR(pbc->addr));
...
{
	wpa_printf(MSG_DEBUG, "WPS:  * Wi-Fi Protected Setup State (%d)", wps->wps_state);
...
static int wps_build_sel_pbc_reg_uuid_e(struct wps_registrar *reg, struct wpabuf *msg)
{
	u16 id = reg->pbc ? DEV_PW_PUSHBUTTON : DEV_PW_DEFAULT;
	if (!reg->sel_reg_union) {
		return 0;
...
static void wps_set_pushbutton(u16 *methods, u16 conf_methods)
{
	*methods |= WPS_CONFIG_PUSHBUTTON;
	...
	if ((*methods & WPS_CONFIG_VIRT_PUSHBUTTON) != WPS_CONFIG_VIRT_PUSHBUTTON && (*methods & WPS_CONFIG_PHY_PUSHBUTTON) != WPS_CONFIG_PHY_PUSHBUTTON) {
		/*
...
static int wps_build_sel_reg_config_methods(struct wps_registrar *reg, struct wpabuf *msg)
{
	...
	wpa_printf(MSG_DEBUG, "WPS:  * Selected Registrar Config Methods (%x)", methods);
...
static void wps_registrar_invalidate_unused(struct wps_registrar *reg)
{
	...
	dl_list_for_each(pin, &reg->pins, struct wps_uuid_pin, list) {
		if (pin->wildcard_uuid == 1 && !(pin->flags & PIN_LOCKED)) {
			wpa_printf(MSG_DEBUG, "WPS: Invalidate previously " "configured wildcard PIN");
...
int wps_registrar_add_pin(struct wps_registrar *reg, const u8 *addr, const u8 *uuid, const u8 *pin, size_t pin_len, int timeout)
{
	...
	eloop_cancel_timeout(wps_registrar_set_selected_timeout, reg, NULL);
	eloop_register_timeout(WPS_PBC_WALK_TIME, 0, wps_registrar_set_selected_timeout, reg, NULL);
...
static void wps_registrar_expire_pins(struct wps_registrar *reg)
{
	...
	dl_list_for_each_safe(pin, prev, &reg->pins, struct wps_uuid_pin, list) {
		if ((pin->flags & PIN_EXPIRES) && os_reltime_before(&pin->expiration, &now)) {
			wpa_hexdump(MSG_DEBUG, "WPS: Expired PIN for UUID", pin->uuid, WPS_UUID_LEN);
			wps_registrar_remove_pin(reg, pin);
		}
	}
}
 ...
 */
static int wps_registrar_invalidate_wildcard_pin(struct wps_registrar *reg, const u8 *dev_pw, size_t dev_pw_len)
{
	struct wps_uuid_pin *pin, *prev;

	dl_list_for_each_safe(pin, prev, &reg->pins, struct wps_uuid_pin, list) {
		if (dev_pw && pin->pin && (dev_pw_len != pin->pin_len || os_memcmp_const(dev_pw, pin->pin, dev_pw_len) != 0)) {
			continue;    /* different PIN */
		}
		if (pin->wildcard_uuid) {
			wpa_hexdump(MSG_DEBUG, "WPS: Invalidated PIN for UUID", pin->uuid, WPS_UUID_LEN);
			wps_registrar_remove_pin(reg, pin);
			return 0;
		}
	}
...
static const u8 *wps_registrar_get_pin(struct wps_registrar *reg, const u8 *uuid, size_t *pin_len)
{
	...
	if (!found) {
		...
		dl_list_for_each(pin, &reg->pins, struct wps_uuid_pin, list) {
			if (pin->wildcard_uuid == 1 || pin->wildcard_uuid == 2) {
				wpa_printf(MSG_DEBUG, "WPS: Found a wildcard " "PIN. Assigned it for this UUID-E");
				pin->wildcard_uuid++;
	...
	if (found->flags & PIN_LOCKED) {
		wpa_printf(MSG_DEBUG, "WPS: Selected PIN locked - do not " "allow concurrent re-use");
		return NULL;
	}
...
int wps_registrar_button_pushed(struct wps_registrar *reg, const u8 *p2p_dev_addr)
{
	if (p2p_dev_addr == NULL && wps_registrar_pbc_overlap(reg, NULL, NULL)) {
		wpa_printf(MSG_DEBUG, "WPS: PBC overlap - do not start PBC " "mode");
	...
	eloop_cancel_timeout(wps_registrar_set_selected_timeout, reg, NULL);
	eloop_cancel_timeout(wps_registrar_pbc_timeout, reg, NULL);
	eloop_register_timeout(WPS_PBC_WALK_TIME, 0, wps_registrar_pbc_timeout, reg, NULL);
	return 0;
}
...
void wps_registrar_complete(struct wps_registrar *registrar, const u8 *uuid_e, const u8 *dev_pw, size_t dev_pw_len)
{
	...
	if (dev_pw && wps_registrar_invalidate_wildcard_pin(registrar, dev_pw, dev_pw_len) == 0) {
		wpa_hexdump_key(MSG_DEBUG, "WPS: Invalidated wildcard PIN", dev_pw, dev_pw_len);
	}
}
...
void wps_registrar_probe_req_rx(struct wps_registrar *reg, const u8 *addr, const struct wpabuf *wps_data, int p2p_wildcard)
{
	...
	wpa_hexdump_buf(MSG_MSGDUMP, "WPS: Probe Request with WPS data received", wps_data);
	...
	if (attr.config_methods == NULL) {
		wpa_printf(MSG_DEBUG, "WPS: No Config Methods attribute in " "Probe Request");
		return;
	}

	if (attr.dev_password_id == NULL) {
		wpa_printf(MSG_DEBUG, "WPS: No Device Password Id attribute " "in Probe Request");
		return;
	}

	if (reg->enrollee_seen_cb && attr.uuid_e && attr.primary_dev_type && attr.request_type && !p2p_wildcard) {
		char *dev_name = NULL;
		if (attr.dev_name) {
		...
		reg->enrollee_seen_cb(reg->cb_ctx, addr, attr.uuid_e, attr.primary_dev_type, WPA_GET_BE16(attr.config_methods), WPA_GET_BE16(attr.dev_password_id), *attr.request_type, dev_name);
		os_free(dev_name);
	}
	...
	wpa_printf(MSG_DEBUG, "WPS: Probe Request for PBC received from " MACSTR, MAC2STR(addr));
	if (attr.uuid_e == NULL) {
		wpa_printf(MSG_DEBUG, "WPS: Invalid Probe Request WPS IE: No " "UUID-E included");
		return;
	}
	wpa_hexdump(MSG_DEBUG, "WPS: UUID-E from Probe Request", attr.uuid_e, WPS_UUID_LEN);

#ifdef WPS_WORKAROUNDS
	if (reg->pbc_ignore_start.sec && os_memcmp(attr.uuid_e, reg->pbc_ignore_uuid, WPS_UUID_LEN) == 0) {
		struct os_reltime now, dur;
		os_get_reltime(&now);
		os_reltime_sub(&now, &reg->pbc_ignore_start, &dur);
		if (dur.sec >= 0 && dur.sec < 5) {
			wpa_printf(MSG_DEBUG, "WPS: Ignore PBC activation " "based on Probe Request from the Enrollee " "that just completed PBC provisioning");
			skip_add = 1;
		} else {
			reg->pbc_ignore_start.sec = 0;
		}
...
static void wps_cb_reg_success(struct wps_registrar *reg, const u8 *mac_addr, const u8 *uuid_e, const u8 *dev_pw, size_t dev_pw_len)
{
...
static void wps_cb_set_sel_reg(struct wps_registrar *reg)
{
	...
	wpa_printf(MSG_DEBUG, "WPS: wps_cb_set_sel_reg: sel_reg=%d " "config_methods=0x%x pbc=%d methods=0x%x", reg->selected_registrar, reg->wps->config_methods, reg->pbc, methods);

	reg->set_sel_reg_cb(reg->cb_ctx, reg->selected_registrar, reg->pbc ? DEV_PW_PUSHBUTTON : DEV_PW_DEFAULT, methods);
}
...
static int wps_set_ie(struct wps_registrar *reg)
{
	...
	if (wps_build_version(beacon) || wps_build_wps_state(reg->wps, beacon) || wps_build_ap_setup_locked(reg->wps, beacon) || wps_build_selected_registrar(reg, beacon) || wps_build_sel_reg_dev_password_id(reg, beacon) || wps_build_sel_reg_config_methods(reg, beacon) || wps_build_sel_pbc_reg_uuid_e(reg, beacon) || (reg->dualband && wps_build_rf_bands(&reg->wps->dev, beacon, 0)) || wps_build_wfa_ext(beacon, 0, auth_macs, count) || wps_build_vendor_ext(&reg->wps->dev, beacon)) {
		wpabuf_free(beacon);
		wpabuf_free(probe);
		return -1;
	}
#ifdef CONFIG_P2P
	if (wps_build_dev_name(&reg->wps->dev, beacon) || wps_build_primary_dev_type(&reg->wps->dev, beacon)) {
		wpabuf_free(beacon);
		wpabuf_free(probe);
		return -1;
	}
	...
	if (wps_build_version(probe) || wps_build_wps_state(reg->wps, probe) || wps_build_ap_setup_locked(reg->wps, probe) || wps_build_selected_registrar(reg, probe) || wps_build_sel_reg_dev_password_id(reg, probe) || wps_build_sel_reg_config_methods(reg, probe) || wps_build_resp_type(probe, reg->wps->ap ? WPS_RESP_AP : WPS_RESP_REGISTRAR) || wps_build_uuid_e(probe, reg->wps->uuid) || wps_build_device_attrs(&reg->wps->dev, probe) || wps_build_probe_config_methods(reg, probe) || (reg->dualband && wps_build_rf_bands(&reg->wps->dev, probe, 0)) || wps_build_wfa_ext(probe, 0, auth_macs, count) || wps_build_vendor_ext(&reg->wps->dev, probe)) {
		wpabuf_free(beacon);
		wpabuf_free(probe);
		return -1;
	}
	...
	if (reg->static_wep_only) {
		...
		wpa_printf(MSG_DEBUG, "WPS: Add Microsoft Provisioning IE " "into Beacon/Probe Response frames");
...
static int wps_get_dev_password(struct wps_data *wps)
{
	...
	if (wps->pbc) {
		...
		if (wps->nfc_pw_token->pw_id == DEV_PW_NFC_CONNECTION_HANDOVER) {
			wpa_printf(MSG_DEBUG, "WPS: Using NFC connection " "handover and abbreviated WPS handshake " "without Device Password");
			return 0;
		}
		wpa_printf(MSG_DEBUG, "WPS: Use OOB Device Password from NFC " "Password Token");
		pin = wps->nfc_pw_token->dev_pw;
		pin_len = wps->nfc_pw_token->dev_pw_len;
	} else if (wps->dev_pw_id >= 0x10 && wps->wps->ap_nfc_dev_pw_id == wps->dev_pw_id && wps->wps->ap_nfc_dev_pw) {
		...
		if (pin && wps->dev_pw_id >= 0x10) {
			wpa_printf(MSG_DEBUG, "WPS: No match for OOB Device " "Password ID, but PIN found");
			/*
	...
	if (pin == NULL) {
		wpa_printf(MSG_DEBUG, "WPS: No Device Password available for " "the Enrollee (context %p registrar %p)", wps->wps, wps->wps->registrar);
...
static int wps_build_r_hash(struct wps_data *wps, struct wpabuf *msg)
{
	...
	wpa_hexdump(MSG_DEBUG, "WPS: R-S2", wps->snonce + WPS_SECRET_NONCE_LEN, WPS_SECRET_NONCE_LEN);

	if (wps->dh_pubkey_e == NULL || wps->dh_pubkey_r == NULL) {
		wpa_printf(MSG_DEBUG, "WPS: DH public keys not available for " "R-Hash derivation");
		return -1;
	}
...
static int wps_build_r_snonce2(struct wps_data *wps, struct wpabuf *msg)
{
	...
	wpabuf_put_be16(msg, WPS_SECRET_NONCE_LEN);
	wpabuf_put_data(msg, wps->snonce + WPS_SECRET_NONCE_LEN, WPS_SECRET_NONCE_LEN);
	return 0;
}
...
static int wps_build_cred_ssid(struct wpabuf *msg, const struct wps_credential *cred)
{
	...
	wpa_hexdump_ascii(MSG_DEBUG, "WPS: SSID for Credential", cred->ssid, cred->ssid_len);
...
{
	if (wps_build_cred_network_idx(msg, cred) || wps_build_cred_ssid(msg, cred) || wps_build_cred_auth_type(msg, cred) || wps_build_cred_encr_type(msg, cred) || wps_build_cred_network_key(msg, cred) || wps_build_mac_addr(msg, cred->mac_addr)) {
		return -1;
	}
	return 0;
}
...
int wps_build_cred(struct wps_data *wps, struct wpabuf *msg)
{
	...
	if (wps->auth_type == WPS_AUTH_WPA2PSK || wps->auth_type == WPS_AUTH_WPAPSK) {
		if (wps->encr_type & WPS_ENCR_AES) {
			...
			wpa_printf(MSG_DEBUG, "WPS: No suitable encryption " "type for WPA/WPA2");
			return -1;
		}
	} else {
		if (wps->encr_type & WPS_ENCR_NONE) {
			...
			wpa_printf(MSG_DEBUG, "WPS: No suitable encryption " "type for non-WPA/WPA2 mode");
			return -1;
		}
	}
	...
	if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->wps->ap && !wps->wps->registrar->disable_auto_conf) {
		u8 r[16];
		...
		wpa_hexdump_ascii_key(MSG_DEBUG, "WPS: Generated passphrase", wps->new_psk, wps->new_psk_len);
		os_memcpy(wps->cred.key, wps->new_psk, wps->new_psk_len);
		wps->cred.key_len = wps->new_psk_len;
	} else if (!wps->wps->registrar->force_per_enrollee_psk && wps->use_psk_key && wps->wps->psk_set) {
		char hex[65];
		wpa_printf(MSG_DEBUG, "WPS: Use PSK format for Network Key");
		wpa_snprintf_hex(hex, sizeof(hex), wps->wps->psk, 32);
		os_memcpy(wps->cred.key, hex, 32 * 2);
		wps->cred.key_len = 32 * 2;
	} else if (!wps->wps->registrar->force_per_enrollee_psk && wps->wps->network_key) {
		...
		if (random_pool_ready() != 1 || random_get_bytes(wps->new_psk, wps->new_psk_len) < 0) {
			wpa_printf(MSG_INFO, "WPS: Could not generate random PSK");
			os_free(wps->new_psk);
			wps->new_psk = NULL;
			return -1;
		}
		wpa_hexdump_key(MSG_DEBUG, "WPS: Generated per-device PSK", wps->new_psk, wps->new_psk_len);
...
static struct wpabuf *wps_build_m2(struct wps_data *wps)
{
	...
	if (wps->nfc_pw_token && wps->nfc_pw_token->pk_hash_provided_oob && wps->nfc_pw_token->pw_id == DEV_PW_NFC_CONNECTION_HANDOVER) {
		...
		if (plain == NULL || wps_build_cred(wps, plain) || wps_build_key_wrap_auth(wps, plain) || wps_build_encr_settings(wps, msg, plain)) {
			wpabuf_free(msg);
			wpabuf_free(plain);
			return NULL;
		}
...
static struct wpabuf *wps_build_m4(struct wps_data *wps)
{
	...
	if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_M4) || wps_build_enrollee_nonce(wps, msg) || wps_build_r_hash(wps, msg) || wps_build_r_snonce1(wps, plain) || wps_build_key_wrap_auth(wps, plain) || wps_build_encr_settings(wps, msg, plain) || wps_build_wfa_ext(msg, 0, NULL, 0) || wps_build_authenticator(wps, msg)) {
		wpabuf_free(plain);
		wpabuf_free(msg);
		return NULL;
	}
...
static struct wpabuf *wps_build_m6(struct wps_data *wps)
{
	...
	if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_M6) || wps_build_enrollee_nonce(wps, msg) || wps_build_r_snonce2(wps, plain) || wps_build_key_wrap_auth(wps, plain) || wps_build_encr_settings(wps, msg, plain) || wps_build_wfa_ext(msg, 0, NULL, 0) || wps_build_authenticator(wps, msg)) {
		wpabuf_free(plain);
		wpabuf_free(msg);
		return NULL;
	}
...
static struct wpabuf *wps_build_m8(struct wps_data *wps)
{
	...
	if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_M8) || wps_build_enrollee_nonce(wps, msg) || ((wps->wps->ap || wps->er) && wps_build_cred(wps, plain)) || (!wps->wps->ap && !wps->er && wps_build_ap_settings(wps, plain)) || wps_build_key_wrap_auth(wps, plain) || wps_build_encr_settings(wps, msg, plain) || wps_build_wfa_ext(msg, 0, NULL, 0) || wps_build_authenticator(wps, msg)) {
		wpabuf_free(plain);
		wpabuf_free(msg);
		return NULL;
	}
...
struct wpabuf *wps_registrar_get_msg(struct wps_data *wps, enum wsc_op_code *op_code)
{
	...
	if (wps->ext_reg) {
		wpa_printf(MSG_DEBUG, "WPS: Using external Registrar, but no " "pending message available");
		return NULL;
	}
#endif							/* CONFIG_WPS_UPNP */
	...
	case SEND_WSC_NACK:
		...
		*op_code = WSC_NACK;
		break;
	default:
		wpa_printf(MSG_DEBUG, "WPS: Unsupported state %d for building " "a message", wps->state);
		msg = NULL;
		break;
	}
...
static int wps_process_e_snonce1(struct wps_data *wps, const u8 *e_snonce1)
{
	...
	if (os_memcmp_const(wps->peer_hash1, hash, WPS_HASH_LEN) != 0) {
		wpa_printf(MSG_DEBUG, "WPS: E-Hash1 derived from E-S1 does " "not match with the pre-committed value");
		wps->config_error = WPS_CFG_DEV_PASSWORD_AUTH_FAILURE;
		wps_pwd_auth_fail_event(wps->wps, 0, 1, wps->mac_addr_e);
		return -1;
	}

	wpa_printf(MSG_DEBUG, "WPS: Enrollee proved knowledge of the first " "half of the device password");
...
static int wps_process_e_snonce2(struct wps_data *wps, const u8 *e_snonce2)
{
	...
	if (os_memcmp_const(wps->peer_hash2, hash, WPS_HASH_LEN) != 0) {
		wpa_printf(MSG_DEBUG, "WPS: E-Hash2 derived from E-S2 does " "not match with the pre-committed value");
	...
	wpa_printf(MSG_DEBUG, "WPS: Enrollee proved knowledge of the second " "half of the device password");
...
static int wps_process_auth_type_flags(struct wps_data *wps, const u8 *auth)
{
	...
	if (auth == NULL) {
		wpa_printf(MSG_DEBUG, "WPS: No Authentication Type flags " "received");
		return -1;
	}

	auth_types = WPA_GET_BE16(auth);

	wpa_printf(MSG_DEBUG, "WPS: Enrollee Authentication Type flags 0x%x", auth_types);
	wps->auth_type = wps->wps->auth_types & auth_types;
	if (wps->auth_type == 0) {
		wpa_printf(MSG_DEBUG, "WPS: No match in supported " "authentication types (own 0x%x Enrollee 0x%x)", wps->wps->auth_types, auth_types);
#ifdef WPS_WORKAROUNDS
		/*
		 ...
		 */
		wpa_printf(MSG_DEBUG, "WPS: Workaround - assume Enrollee " "does not advertise supported authentication types " "correctly");
...
static int wps_process_encr_type_flags(struct wps_data *wps, const u8 *encr)
{
	...
	if (encr == NULL) {
		wpa_printf(MSG_DEBUG, "WPS: No Encryption Type flags " "received");
		return -1;
	}
	...
	if (wps->encr_type == 0) {
		wpa_printf(MSG_DEBUG, "WPS: No match in supported " "encryption types (own 0x%x Enrollee 0x%x)", wps->wps->encr_types, encr_types);
#ifdef WPS_WORKAROUNDS
		/*
		 * Some deployed implementations seem to advertise incorrect
		 * information in this attribute. For example, Linksys WRT350N
		 ...
		 * assume that the Enrollee supports everything we do.
		 */
		wpa_printf(MSG_DEBUG, "WPS: Workaround - assume Enrollee " "does not advertise supported encryption types " "correctly");
...
{
	if (conn == NULL) {
		wpa_printf(MSG_DEBUG, "WPS: No Connection Type flags " "received");
		return -1;
	}
...
static int wps_process_config_methods(struct wps_data *wps, const u8 *methods)
{
	...
	wpa_printf(MSG_DEBUG, "WPS: Enrollee Config Methods 0x%x" "%s%s%s%s%s%s%s%s%s", m, m & WPS_CONFIG_USBA ? " [USBA]" : "", m & WPS_CONFIG_ETHERNET ? " [Ethernet]" : "", m & WPS_CONFIG_LABEL ? " [Label]" : "", m & WPS_CONFIG_DISPLAY ? " [Display]" : "", m & WPS_CONFIG_EXT_NFC_TOKEN ? " [Ext NFC Token]" : "", m & WPS_CONFIG_INT_NFC_TOKEN ? " [Int NFC Token]" : "", m & WPS_CONFIG_NFC_INTERFACE ? " [NFC]" : "", m & WPS_CONFIG_PUSHBUTTON ? " [PBC]" : "", m & WPS_CONFIG_KEYPAD ? " [Keypad]" : "");
	...
	if (!(m & WPS_CONFIG_DISPLAY) && !wps->use_psk_key) {
		 ...
		 */
		wpa_printf(MSG_DEBUG, "WPS: Prefer PSK format key due to " "Enrollee not supporting display");
		wps->use_psk_key = 1;
	}
...
{
	if (state == NULL) {
		wpa_printf(MSG_DEBUG, "WPS: No Wi-Fi Protected Setup State " "received");
		return -1;
	}
...
static int wps_registrar_p2p_dev_addr_match(struct wps_data *wps)
{
	...
	if (os_memcmp(reg->p2p_dev_addr, wps->p2p_dev_addr, ETH_ALEN) != 0) {
		wpa_printf(MSG_DEBUG, "WPS: No match on P2P Device Address " "filtering for PBC: expected " MACSTR " was " MACSTR " - indicate PBC session overlap", MAC2STR(reg->p2p_dev_addr), MAC2STR(wps->p2p_dev_addr));
		return 0;
	}
#endif							/* CONFIG_P2P */
	return 1;
...
static int wps_registrar_skip_overlap(struct wps_data *wps)
{
	...
	if (os_memcmp(reg->p2p_dev_addr, wps->p2p_dev_addr, ETH_ALEN) == 0) {
		wpa_printf(MSG_DEBUG, "WPS: Skip PBC overlap due to selected " "Enrollee match");
		return 1;
	}
#endif							/* CONFIG_P2P */
	return 0;
...
static enum wps_process_res wps_process_m1(struct wps_data *wps, struct wps_parse_attr *attr)
{
	...
	if (wps->state != RECV_M1) {
		wpa_printf(MSG_DEBUG, "WPS: Unexpected state (%d) for " "receiving M1", wps->state);
		return WPS_FAILURE;
	}

	if (wps_process_uuid_e(wps, attr->uuid_e) || wps_process_mac_addr(wps, attr->mac_addr) || wps_process_enrollee_nonce(wps, attr->enrollee_nonce) || wps_process_pubkey(wps, attr->public_key, attr->public_key_len) || wps_process_auth_type_flags(wps, attr->auth_type_flags) || wps_process_encr_type_flags(wps, attr->encr_type_flags) || wps_process_conn_type_flags(wps, attr->conn_type_flags) || wps_process_config_methods(wps, attr->config_methods) || wps_process_wps_state(wps, attr->wps_state) || wps_process_device_attrs(&wps->peer_dev, attr) || wps_process_rf_bands(&wps->peer_dev, attr->rf_bands) || wps_process_assoc_state(wps, attr->assoc_state) || wps_process_dev_password_id(wps, attr->dev_password_id) || wps_process_config_error(wps, attr->config_error) || wps_process_os_version(&wps->peer_dev, attr->os_version)) {
		return WPS_FAILURE;
	}

	if (wps->dev_pw_id < 0x10 && wps->dev_pw_id != DEV_PW_DEFAULT && wps->dev_pw_id != DEV_PW_P2PS_DEFAULT && wps->dev_pw_id != DEV_PW_USER_SPECIFIED && wps->dev_pw_id != DEV_PW_MACHINE_SPECIFIED && wps->dev_pw_id != DEV_PW_REGISTRAR_SPECIFIED &&
#ifdef CONFIG_WPS_NFC
		wps->dev_pw_id != DEV_PW_NFC_CONNECTION_HANDOVER &&
#endif							/* CONFIG_WPS_NFC */
		(wps->dev_pw_id != DEV_PW_PUSHBUTTON || !wps->wps->registrar->pbc)) {
		wpa_printf(MSG_DEBUG, "WPS: Unsupported Device Password ID %d", wps->dev_pw_id);
		wps->state = SEND_M2D;
		return WPS_CONTINUE;
	}
#ifdef CONFIG_WPS_NFC
	if (wps->dev_pw_id >= 0x10 || wps->dev_pw_id == DEV_PW_NFC_CONNECTION_HANDOVER) {
		struct wps_nfc_pw_token *token;
		const u8 *addr[1];
		u8 hash[WPS_HASH_LEN];

		wpa_printf(MSG_DEBUG, "WPS: Searching for NFC token match for id=%d (ctx %p registrar %p)", wps->dev_pw_id, wps->wps, wps->wps->registrar);
		token = wps_get_nfc_pw_token(&wps->wps->registrar->nfc_pw_tokens, wps->dev_pw_id);
		if (token && token->peer_pk_hash_known) {
			wpa_printf(MSG_DEBUG, "WPS: Found matching NFC " "Password Token");
			dl_list_del(&token->list);
			wps->nfc_pw_token = token;

			addr[0] = attr->public_key;
			sha256_vector(1, addr, &attr->public_key_len, hash);
			if (os_memcmp_const(hash, wps->nfc_pw_token->pubkey_hash, WPS_OOB_PUBKEY_HASH_LEN) != 0) {
			...
			wpa_printf(MSG_DEBUG, "WPS: Found matching NFC " "Password Token (no peer PK hash)");
			wps->nfc_pw_token = token;
		} else if (wps->dev_pw_id >= 0x10 && wps->wps->ap_nfc_dev_pw_id == wps->dev_pw_id && wps->wps->ap_nfc_dev_pw) {
	...
	if (wps->dev_pw_id == DEV_PW_PUSHBUTTON) {
		if ((wps->wps->registrar->force_pbc_overlap || wps_registrar_pbc_overlap(wps->wps->registrar, wps->mac_addr_e, wps->uuid_e) || !wps_registrar_p2p_dev_addr_match(wps)) && !wps_registrar_skip_overlap(wps)) {
			wpa_printf(MSG_DEBUG, "WPS: PBC overlap - deny PBC " "negotiation");
			wps->state = SEND_M2D;
			wps->config_error = WPS_CFG_MULTIPLE_PBC_DETECTED;
			wps_pbc_overlap_event(wps->wps);
			wps_fail_event(wps->wps, WPS_M1, WPS_CFG_MULTIPLE_PBC_DETECTED, WPS_EI_NO_ERROR, wps->mac_addr_e);
			wps->wps->registrar->force_pbc_overlap = 1;
			return WPS_CONTINUE;
		}
		wps_registrar_add_pbc_session(wps->wps->registrar, wps->mac_addr_e, wps->uuid_e);
		wps->pbc = 1;
	}
#ifdef WPS_WORKAROUNDS
	/*
	 ...
	 * used.
	 */
	if (!wps->use_psk_key && wps->peer_dev.manufacturer && os_strncmp(wps->peer_dev.manufacturer, "Apple ", 6) == 0 && wps->peer_dev.model_name && os_strcmp(wps->peer_dev.model_name, "AirPort") == 0) {
		wpa_printf(MSG_DEBUG, "WPS: Workaround - Force Network Key in " "PSK format");
		wps->use_psk_key = 1;
	}
#endif							/* WPS_WORKAROUNDS */
...
static enum wps_process_res wps_process_m3(struct wps_data *wps, const struct wpabuf *msg, struct wps_parse_attr *attr)
{
	...
	if (wps->state != RECV_M3) {
		wpa_printf(MSG_DEBUG, "WPS: Unexpected state (%d) for " "receiving M3", wps->state);
		wps->state = SEND_WSC_NACK;
		return WPS_CONTINUE;
	}

	if (wps->pbc && wps->wps->registrar->force_pbc_overlap && !wps_registrar_skip_overlap(wps)) {
		wpa_printf(MSG_DEBUG, "WPS: Reject negotiation due to PBC " "session overlap");
		wps->state = SEND_WSC_NACK;
		wps->config_error = WPS_CFG_MULTIPLE_PBC_DETECTED;
		return WPS_CONTINUE;
	}

	if (wps_process_registrar_nonce(wps, attr->registrar_nonce) || wps_process_authenticator(wps, attr->authenticator, msg) || wps_process_e_hash1(wps, attr->e_hash1) || wps_process_e_hash2(wps, attr->e_hash2)) {
		wps->state = SEND_WSC_NACK;
		return WPS_CONTINUE;
	}
...
static enum wps_process_res wps_process_m5(struct wps_data *wps, const struct wpabuf *msg, struct wps_parse_attr *attr)
{
	...
	if (wps->state != RECV_M5) {
		wpa_printf(MSG_DEBUG, "WPS: Unexpected state (%d) for " "receiving M5", wps->state);
		wps->state = SEND_WSC_NACK;
		return WPS_CONTINUE;
	}
	...
	if (wps_process_registrar_nonce(wps, attr->registrar_nonce) || wps_process_authenticator(wps, attr->authenticator, msg)) {
		wps->state = SEND_WSC_NACK;
		return WPS_CONTINUE;
	}
	...
	if (decrypted == NULL) {
		wpa_printf(MSG_DEBUG, "WPS: Failed to decrypted Encrypted " "Settings attribute");
		wps->state = SEND_WSC_NACK;
		return WPS_CONTINUE;
	}
	...
	if (wps_parse_msg(decrypted, &eattr) < 0 || wps_process_key_wrap_auth(wps, decrypted, eattr.key_wrap_auth) || wps_process_e_snonce1(wps, eattr.e_snonce1)) {
		wpabuf_free(decrypted);
		wps->state = SEND_WSC_NACK;
		return WPS_CONTINUE;
	}
...
static void wps_sta_cred_cb(struct wps_data *wps)
{
	...
	wpa_printf(MSG_DEBUG, "WPS: Update local configuration based on the " "AP configuration");
	if (wps->wps->cred_cb) {
...
static int wps_process_ap_settings_r(struct wps_data *wps, struct wps_parse_attr *attr)
{
	...
	if (wps->new_ap_settings) {
		wpa_printf(MSG_INFO, "WPS: Update AP configuration based on " "new settings");
...
static enum wps_process_res wps_process_m7(struct wps_data *wps, const struct wpabuf *msg, struct wps_parse_attr *attr)
{
	...
	if (wps->state != RECV_M7) {
		wpa_printf(MSG_DEBUG, "WPS: Unexpected state (%d) for " "receiving M7", wps->state);
		wps->state = SEND_WSC_NACK;
		return WPS_CONTINUE;
	}
	...
	decrypted = wps_decrypt_encr_settings(wps, attr->encr_settings, attr->encr_settings_len);
	if (decrypted == NULL) {
		wpa_printf(MSG_DEBUG, "WPS: Failed to decrypt Encrypted " "Settings attribute");
		wps->state = SEND_WSC_NACK;
		return WPS_CONTINUE;
	}

	if (wps_validate_m7_encr(decrypted, wps->wps->ap || wps->er, attr->version2 != NULL) < 0) {
		wpabuf_free(decrypted);
		wps->state = SEND_WSC_NACK;
		return WPS_CONTINUE;
	}
	...
	wpa_printf(MSG_DEBUG, "WPS: Processing decrypted Encrypted Settings " "attribute");
	if (wps_parse_msg(decrypted, &eattr) < 0 || wps_process_key_wrap_auth(wps, decrypted, eattr.key_wrap_auth) || wps_process_e_snonce2(wps, eattr.e_snonce2) || wps_process_ap_settings_r(wps, &eattr)) {
		wpabuf_free(decrypted);
		wps->state = SEND_WSC_NACK;
		return WPS_CONTINUE;
	}
...
static enum wps_process_res wps_process_wsc_msg(struct wps_data *wps, const struct wpabuf *msg)
{
	...
	if (*attr.msg_type != WPS_M1 && (attr.registrar_nonce == NULL || os_memcmp(wps->nonce_r, attr.registrar_nonce, WPS_NONCE_LEN) != 0)) {
	...
	case WPS_M1:
		...
		if (wps->wps->wps_upnp && attr.mac_addr) {
			...
			upnp_wps_device_send_wlan_event(wps->wps->wps_upnp, attr.mac_addr, UPNP_WPS_WLANEVENT_TYPE_EAP, msg);
		}
#endif							/* CONFIG_WPS_UPNP */
	...
	case WPS_M3:
		...
		if (ret == WPS_FAILURE || wps->state == SEND_WSC_NACK) {
			wps_fail_event(wps->wps, WPS_M3, wps->config_error, wps->error_indication, wps->mac_addr_e);
		}
		break;
	case WPS_M5:
		...
		if (ret == WPS_FAILURE || wps->state == SEND_WSC_NACK) {
			wps_fail_event(wps->wps, WPS_M5, wps->config_error, wps->error_indication, wps->mac_addr_e);
		}
		break;
	case WPS_M7:
		...
		if (ret == WPS_FAILURE || wps->state == SEND_WSC_NACK) {
			wps_fail_event(wps->wps, WPS_M7, wps->config_error, wps->error_indication, wps->mac_addr_e);
		}
		break;
	default:
...
static enum wps_process_res wps_process_wsc_ack(struct wps_data *wps, const struct wpabuf *msg)
{
...
#ifdef CONFIG_WPS_UPNP
	if (wps->wps->wps_upnp && wps->ext_reg && wps->state == RECV_M2D_ACK && upnp_wps_subscribers(wps->wps->wps_upnp)) {
		if (wps->wps->upnp_msgs) {
			return WPS_CONTINUE;
		}
		wpa_printf(MSG_DEBUG, "WPS: Wait for response from an " "external Registrar");
		return WPS_PENDING;
	}
#endif							/* CONFIG_WPS_UPNP */

	if (attr.registrar_nonce == NULL || os_memcmp(wps->nonce_r, attr.registrar_nonce, WPS_NONCE_LEN) != 0) {
		wpa_printf(MSG_DEBUG, "WPS: Mismatch in registrar nonce");
		return WPS_FAILURE;
	}

	if (attr.enrollee_nonce == NULL || os_memcmp(wps->nonce_e, attr.enrollee_nonce, WPS_NONCE_LEN) != 0) {
	...
	if (wps->state == RECV_M2D_ACK) {
		...
		wpa_printf(MSG_DEBUG, "WPS: No more registrars available - " "terminate negotiation");
	}
...
static enum wps_process_res wps_process_wsc_nack(struct wps_data *wps, const struct wpabuf *msg)
{
	...
	if (wps->wps->wps_upnp && wps->ext_reg) {
		wpa_printf(MSG_DEBUG, "WPS: Negotiation using external " "Registrar terminated by the Enrollee");
		return WPS_FAILURE;
	}
#endif							/* CONFIG_WPS_UPNP */
	...
	if (attr.config_error == NULL) {
		wpa_printf(MSG_DEBUG, "WPS: No Configuration Error attribute " "in WSC_NACK");
		return WPS_FAILURE;
	}

	config_error = WPA_GET_BE16(attr.config_error);
	wpa_printf(MSG_DEBUG, "WPS: Enrollee terminated negotiation with " "Configuration Error %d", config_error);

	switch (old_state) {
	case RECV_M3:
		wps_fail_event(wps->wps, WPS_M2, config_error, wps->error_indication, wps->mac_addr_e);
		break;
	case RECV_M5:
		wps_fail_event(wps->wps, WPS_M4, config_error, wps->error_indication, wps->mac_addr_e);
		break;
	case RECV_M7:
		wps_fail_event(wps->wps, WPS_M6, config_error, wps->error_indication, wps->mac_addr_e);
		break;
	case RECV_DONE:
		wps_fail_event(wps->wps, WPS_M8, config_error, wps->error_indication, wps->mac_addr_e);
		break;
	default:
		break;
	}
...
static enum wps_process_res wps_process_wsc_done(struct wps_data *wps, const struct wpabuf *msg)
{
	...
	if (wps->state != RECV_DONE && (!wps->wps->wps_upnp || !wps->ext_reg)) {
		wpa_printf(MSG_DEBUG, "WPS: Unexpected state (%d) for " "receiving WSC_Done", wps->state);
		return WPS_FAILURE;
	}
...
#ifdef CONFIG_WPS_UPNP
	if (wps->wps->wps_upnp && wps->ext_reg) {
		wpa_printf(MSG_DEBUG, "WPS: Negotiation using external " "Registrar completed successfully");
	...
	if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->new_psk && wps->wps->ap && !wps->wps->registrar->disable_auto_conf) {
		struct wps_credential cred;

		wpa_printf(MSG_DEBUG, "WPS: Moving to Configured state based " "on first Enrollee connection");
		...
		wpa_hexdump_ascii_key(MSG_DEBUG, "WPS: Generated random passphrase", wps->new_psk, wps->new_psk_len);
		if (wps->wps->cred_cb) {
	...
	if (wps->new_psk) {
		if (wps_cb_new_psk(wps->wps->registrar, wps->mac_addr_e, wps->p2p_dev_addr, wps->new_psk, wps->new_psk_len)) {
			wpa_printf(MSG_DEBUG, "WPS: Failed to configure the " "new PSK");
		}
		os_free(wps->new_psk);
		wps->new_psk = NULL;
	}

	wps_cb_reg_success(wps->wps->registrar, wps->mac_addr_e, wps->uuid_e, wps->dev_password, wps->dev_password_len);

	if (wps->pbc) {
		wps_registrar_remove_pbc_session(wps->wps->registrar, wps->uuid_e, wps->p2p_dev_addr);
...
enum wps_process_res wps_registrar_process_msg(struct wps_data *wps, enum wsc_op_code op_code, const struct wpabuf *msg)
{
	...
	wpa_printf(MSG_DEBUG, "WPS: Processing received message (len=%lu " "op_code=%d)", (unsigned long)wpabuf_len(msg), op_code);

#ifdef CONFIG_WPS_UPNP
	if (wps->wps->wps_upnp && op_code == WSC_MSG && wps->ext_reg == 1) {
		struct wps_parse_attr attr;
		if (wps_parse_msg(msg, &attr) == 0 && attr.msg_type && *attr.msg_type == WPS_M3) {
	...
	if (wps->wps->wps_upnp && wps->ext_reg && wps->wps->upnp_msgs == NULL && (op_code == WSC_MSG || op_code == WSC_Done || op_code == WSC_NACK)) {
		struct wps_parse_attr attr;
		int type;
		...
		wpa_printf(MSG_DEBUG, "WPS: Sending received message (type %d)" " to external Registrar for processing", type);
		upnp_wps_device_send_wlan_event(wps->wps->wps_upnp, wps->mac_addr_e, UPNP_WPS_WLANEVENT_TYPE_EAP, msg);
		if (op_code == WSC_MSG) {
			return WPS_PENDING;
		}
	} else if (wps->wps->wps_upnp && wps->ext_reg && op_code == WSC_MSG) {
		wpa_printf(MSG_DEBUG, "WPS: Skip internal processing - using " "external Registrar");
		return WPS_CONTINUE;
	}
#endif							/* CONFIG_WPS_UPNP */
	...
	case WSC_Done:
		...
		if (ret == WPS_FAILURE) {
			wps->state = SEND_WSC_NACK;
			wps_fail_event(wps->wps, WPS_WSC_DONE, wps->config_error, wps->error_indication, wps->mac_addr_e);
		}
		return ret;
	default:
...
static void wps_registrar_set_selected_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_printf(MSG_DEBUG, "WPS: Selected Registrar timeout - " "unselect internal Registrar");
	reg->selected_registrar = 0;
	reg->pbc = 0;
	wps_registrar_selected_registrar_changed(reg, 0);
}
...
#ifdef CONFIG_WPS_UPNP
static void wps_registrar_sel_reg_add(struct wps_registrar *reg, struct subscription *s)
{
	int i, j;
	wpa_printf(MSG_DEBUG, "WPS: External Registrar selected (dev_pw_id=%d " "config_methods=0x%x)", s->dev_password_id, s->config_methods);
	reg->sel_reg_union = 1;
	...
	for (j = 0; i < WPS_MAX_AUTHORIZED_MACS && j < WPS_MAX_AUTHORIZED_MACS; j++) {
		...
		wpa_printf(MSG_DEBUG, "WPS: Add authorized MAC into union: " MACSTR, MAC2STR(s->authorized_macs[j]));
		os_memcpy(reg->authorized_macs_union[i], s->authorized_macs[j], ETH_ALEN);
		i++;
	}
	wpa_hexdump(MSG_DEBUG, "WPS: Authorized MACs union", (u8 *)reg->authorized_macs_union, sizeof(reg->authorized_macs_union));
}
#endif							/* CONFIG_WPS_UPNP */
...
static void wps_registrar_sel_reg_union(struct wps_registrar *reg)
{
	...
	dl_list_for_each(s, &reg->wps->wps_upnp->subscriptions, struct subscription, list) {
		struct subscr_addr *sa;
		sa = dl_list_first(&s->addr_list, struct subscr_addr, list);
		if (sa) {
			wpa_printf(MSG_DEBUG, "WPS: External Registrar %s:%d", inet_ntoa(sa->saddr.sin_addr), ntohs(sa->saddr.sin_port));
		}
		if (s->selected_registrar) {
			wps_registrar_sel_reg_add(reg, s);
		} else {
			wpa_printf(MSG_DEBUG, "WPS: External Registrar not " "selected");
		}
	}
#endif							/* CONFIG_WPS_UPNP */
}
...
void wps_registrar_selected_registrar_changed(struct wps_registrar *reg, u16 dev_pw_id)
{
	...
	reg->sel_reg_dev_password_id_override = -1;
	reg->sel_reg_config_methods_override = -1;
	os_memcpy(reg->authorized_macs_union, reg->authorized_macs, WPS_MAX_AUTHORIZED_MACS * ETH_ALEN);
	wpa_hexdump(MSG_DEBUG, "WPS: Authorized MACs union (start with own)", (u8 *)reg->authorized_macs_union, sizeof(reg->authorized_macs_union));
	if (reg->selected_registrar) {
		u16 methods;
		...
		wpa_printf(MSG_DEBUG, "WPS: Internal Registrar selected " "(pbc=%d)", reg->pbc);
...
int wps_registrar_get_info(struct wps_registrar *reg, const u8 *addr, char *buf, size_t buflen)
{
	...
	ret = os_snprintf(buf + len, buflen - len, "wpsUuid=%s\n" "wpsPrimaryDeviceType=%s\n" "wpsDeviceName=%s\n" "wpsManufacturer=%s\n" "wpsModelName=%s\n" "wpsModelNumber=%s\n" "wpsSerialNumber=%s\n", uuid, wps_dev_type_bin2str(d->dev.pri_dev_type, devtype, sizeof(devtype)), d->dev.device_name ? d->dev.device_name : "", d->dev.manufacturer ? d->dev.manufacturer : "", d->dev.model_name ? d->dev.model_name : "", d->dev.model_number ? d->dev.model_number : "", d->dev.serial_number ? d->dev.serial_number : "");
...
int wps_registrar_config_ap(struct wps_registrar *reg, struct wps_credential *cred)
{
	...
	if (!(cred->encr_type & (WPS_ENCR_NONE | WPS_ENCR_TKIP | WPS_ENCR_AES))) {
		if (cred->encr_type & WPS_ENCR_WEP) {
			wpa_printf(MSG_INFO, "WPS: Reject new AP settings " "due to WEP configuration");
			return -1;
		}

		wpa_printf(MSG_INFO, "WPS: Reject new AP settings due to " "invalid encr_type 0x%x", cred->encr_type);
		return -1;
	}

	if ((cred->encr_type & (WPS_ENCR_TKIP | WPS_ENCR_AES)) == WPS_ENCR_TKIP) {
		wpa_printf(MSG_DEBUG, "WPS: Upgrade encr_type TKIP -> " "TKIP+AES");
		cred->encr_type |= WPS_ENCR_AES;
	}

	if ((cred->auth_type & (WPS_AUTH_WPAPSK | WPS_AUTH_WPA2PSK)) == WPS_AUTH_WPAPSK) {
		wpa_printf(MSG_DEBUG, "WPS: Upgrade auth_type WPAPSK -> " "WPAPSK+WPA2PSK");
		cred->auth_type |= WPS_AUTH_WPA2PSK;
	}
...
int wps_registrar_add_nfc_pw_token(struct wps_registrar *reg, const u8 *pubkey_hash, u16 pw_id, const u8 *dev_pw, size_t dev_pw_len, int pk_hash_provided_oob)
{
	...
	if (pw_id == DEV_PW_NFC_CONNECTION_HANDOVER && (pubkey_hash == NULL || !pk_hash_provided_oob)) {
		wpa_printf(MSG_DEBUG, "WPS: Unexpected NFC Password Token " "addition - missing public key hash");
		return -1;
	}
	...
	if (dev_pw) {
		wpa_snprintf_hex_uppercase((char *)token->dev_pw, sizeof(token->dev_pw), dev_pw, dev_pw_len);
	...
	wpa_printf(MSG_DEBUG, "WPS: Added NFC Device Password %u to Registrar", pw_id);
...
int wps_registrar_add_nfc_password_token(struct wps_registrar *reg, const u8 *oob_dev_pw, size_t oob_dev_pw_len)
{
	...
	if (oob_dev_pw_len < WPS_OOB_PUBKEY_HASH_LEN + 2 || oob_dev_pw_len > WPS_OOB_PUBKEY_HASH_LEN + 2 + WPS_OOB_DEVICE_PASSWORD_LEN) {
		return -1;
	}
