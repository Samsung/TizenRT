...
void p2p_buf_add_capability(struct wpabuf *buf, u8 dev_capab, u8 group_capab)
{
	...
	wpa_printf(MSG_DEBUG, "P2P: * Capability dev=%02x group=%02x", dev_capab, group_capab);
}
...
void p2p_buf_add_go_intent(struct wpabuf *buf, u8 go_intent)
{
	...
	wpa_printf(MSG_DEBUG, "P2P: * GO Intent: Intent %u Tie breaker %u", go_intent >> 1, go_intent & 0x01);
}
...
void p2p_buf_add_listen_channel(struct wpabuf *buf, const char *country, u8 reg_class, u8 channel)
{
	...
	wpa_printf(MSG_DEBUG, "P2P: * Listen Channel: Regulatory Class %u " "Channel %u", reg_class, channel);
}
...
void p2p_buf_add_operating_channel(struct wpabuf *buf, const char *country, u8 reg_class, u8 channel)
{
	...
	wpabuf_put_u8(buf, reg_class);	/* Regulatory Class */
	wpabuf_put_u8(buf, channel);	/* Channel Number */
	wpa_printf(MSG_DEBUG, "P2P: * Operating Channel: Regulatory Class %u " "Channel %u", reg_class, channel);
}
...
void p2p_buf_add_channel_list(struct wpabuf *buf, const char *country, struct p2p_channels *chan)
{
	...
	wpa_hexdump(MSG_DEBUG, "P2P: * Channel List", len + 2, (u8 *)wpabuf_put(buf, 0) - len - 2);
}
...
void p2p_buf_add_device_info(struct wpabuf *buf, struct p2p_data *p2p, struct p2p_device *peer)
{
	...
	if (peer && peer->wps_method != WPS_NOT_READY) {
		...
		} else if (peer->wps_method == WPS_PIN_DISPLAY || peer->wps_method == WPS_PIN_KEYPAD) {
			methods |= WPS_CONFIG_DISPLAY | WPS_CONFIG_KEYPAD;
			methods |= WPS_CONFIG_P2PS;
		}
	} else if (p2p->cfg->config_methods) {
		methods |= p2p->cfg->config_methods & (WPS_CONFIG_PUSHBUTTON | WPS_CONFIG_DISPLAY | WPS_CONFIG_KEYPAD | WPS_CONFIG_P2PS);
	} else {
		methods |= WPS_CONFIG_PUSHBUTTON;
		methods |= WPS_CONFIG_DISPLAY | WPS_CONFIG_KEYPAD;
		methods |= WPS_CONFIG_P2PS;
...
void p2p_buf_add_config_timeout(struct wpabuf *buf, u8 go_timeout, u8 client_timeout)
{
	...
	wpa_printf(MSG_DEBUG, "P2P: * Configuration Timeout: GO %d (*10ms)  " "client %d (*10ms)", go_timeout, client_timeout);
}
...
void p2p_buf_add_intended_addr(struct wpabuf *buf, const u8 *interface_addr)
{
	...
	wpa_printf(MSG_DEBUG, "P2P: * Intended P2P Interface Address " MACSTR, MAC2STR(interface_addr));
}
...
void p2p_buf_add_ext_listen_timing(struct wpabuf *buf, u16 period, u16 interval)
{
	...
	wpa_printf(MSG_DEBUG, "P2P: * Extended Listen Timing (period %u msec  " "interval %u msec)", period, interval);
}
...
void p2p_buf_add_service_hash(struct wpabuf *buf, struct p2p_data *p2p)
{
	...
	wpabuf_put_data(buf, p2p->p2ps_seek_hash, p2p->p2ps_seek_count * P2PS_HASH_LEN);
	wpa_hexdump(MSG_DEBUG, "P2P: * Service Hash", p2p->p2ps_seek_hash, p2p->p2ps_seek_count * P2PS_HASH_LEN);
}
...
void p2p_buf_add_advertisement_id(struct wpabuf *buf, u32 id, const u8 *mac)
{
	...
	wpa_printf(MSG_DEBUG, "P2P: * Advertisement ID (%x) " MACSTR, id, MAC2STR(mac));
}

static int p2ps_wildcard_hash(struct p2p_data *p2p, const u8 *hash, u8 hash_count)
{
	u8 i;
	const u8 *test = hash;
...
static int p2p_wfa_service_adv(struct p2p_data *p2p)
{
	...
	for (adv = p2p->p2ps_adv_list; adv; adv = adv->next) {
		if (os_strncmp(adv->svc_name, P2PS_WILD_HASH_STR, os_strlen(P2PS_WILD_HASH_STR)) == 0) {
			return 1;
		}
	}
...
static int p2p_buf_add_service_info(struct wpabuf *buf, struct p2p_data *p2p, u32 adv_id, u16 config_methods, const char *svc_name, u8 **ie_len, u8 **pos, size_t *total_len, u8 *attr_len)
{
	size_t svc_len;
	size_t remaining;
	size_t info_len;
	...
	if (svc_len > 255) {
		p2p_dbg(p2p, "Invalid service name length (%u bytes), failed to add advertised service info", (unsigned int)svc_len);
		return -1;
	}
...
void p2p_buf_add_service_instance(struct wpabuf *buf, struct p2p_data *p2p, u8 hash_count, const u8 *hash, struct p2ps_advertisement *adv_list)
{
	...
	wpa_hexdump(MSG_DEBUG, "P2PS: Probe Request service hash values", hash, hash_count * P2PS_HASH_LEN);
	p2ps_wildcard = p2ps_wildcard_hash(p2p, hash, hash_count) && p2p_wfa_service_adv(p2p);
	...
	if (p2ps_wildcard) {
		...
		p2p_buf_add_service_info(tmp_buf, p2p, 0, 0, P2PS_WILD_HASH_STR, &ie_len, &pos, &total_len, attr_len);
	}
	...
	for (adv = adv_list; adv && total_len <= MAX_SVC_ADV_LEN; adv = adv->next) {
		...
		for (i = 0; i < hash_count; i++) {
			...
			if (os_memcmp(test, adv->hash, P2PS_HASH_LEN) == 0 && p2p_buf_add_service_info(tmp_buf, p2p, adv->id, adv->config_methods, adv->svc_name, &ie_len, &pos, &total_len, attr_len)) {
				break;
			}
...
void p2p_buf_add_session_id(struct wpabuf *buf, u32 id, const u8 *mac)
{
	...
	wpabuf_put_le32(buf, id);
	wpabuf_put_data(buf, mac, ETH_ALEN);
	wpa_printf(MSG_DEBUG, "P2P: * Session ID Info (%x) " MACSTR, id, MAC2STR(mac));
}
...
static int p2p_add_wps_string(struct wpabuf *buf, enum wps_attribute attr, const char *val)
{
	size_t len;
...
int p2p_build_wps_ie(struct p2p_data *p2p, struct wpabuf *buf, int pw_id, int all_attr)
{
	...
	if (all_attr) {
		...
		if (wps_build_uuid_e(buf, p2p->cfg->uuid) < 0 || p2p_add_wps_string(buf, ATTR_MANUFACTURER, p2p->cfg->manufacturer) < 0 || p2p_add_wps_string(buf, ATTR_MODEL_NAME, p2p->cfg->model_name) < 0 || p2p_add_wps_string(buf, ATTR_MODEL_NUMBER, p2p->cfg->model_number) < 0 || p2p_add_wps_string(buf, ATTR_SERIAL_NUMBER, p2p->cfg->serial_number) < 0) {
			return -1;
		}
	...
	if (all_attr && p2p->cfg->num_sec_dev_types) {
		if (wpabuf_tailroom(buf) < 4 + WPS_DEV_TYPE_LEN * p2p->cfg->num_sec_dev_types) {
			return -1;
		}
		wpabuf_put_be16(buf, ATTR_SECONDARY_DEV_TYPE_LIST);
		wpabuf_put_be16(buf, WPS_DEV_TYPE_LEN * p2p->cfg->num_sec_dev_types);
		wpabuf_put_data(buf, p2p->cfg->sec_dev_type, WPS_DEV_TYPE_LEN * p2p->cfg->num_sec_dev_types);
	}
