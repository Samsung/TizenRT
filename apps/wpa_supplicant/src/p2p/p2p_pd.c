...
static void p2p_build_wps_ie_config_methods(struct wpabuf *buf, u16 config_methods)
{
	u8 *len;
...
static void p2ps_add_new_group_info(struct p2p_data *p2p, struct wpabuf *buf)
{
	...
	found = p2p->cfg->get_go_info(p2p->cfg->cb_ctx, intended_addr, ssid, &ssid_len, &group_iface);
	if (found) {
...
static void p2ps_add_pd_req_attrs(struct p2p_data *p2p, struct p2p_device *dev, struct wpabuf *buf, u16 config_methods)
{
	...
	if (prov->conncap & (P2PS_SETUP_GROUP_OWNER | P2PS_SETUP_NEW)) {
	...
	if (p2p->cfg->get_persistent_group) {
		shared_group = p2p->cfg->get_persistent_group(p2p->cfg->cb_ctx, dev->info.p2p_device_addr, NULL, 0, go_dev_addr, ssid, &ssid_len);
	}

	/* Add Operating Channel if conncap includes GO */
	if (shared_group || (prov->conncap & (P2PS_SETUP_GROUP_OWNER | P2PS_SETUP_NEW))) {
		u8 tmp;
		...
		if (p2p->op_reg_class && p2p->op_channel) {
			...
			p2p_buf_add_operating_channel(buf, p2p->cfg->country, p2p->cfg->op_reg_class, p2p->cfg->op_channel);
		}
	}
	...
	if (shared_group || prov->conncap == P2PS_SETUP_NEW || prov->conncap == (P2PS_SETUP_GROUP_OWNER | P2PS_SETUP_NEW) || prov->conncap == (P2PS_SETUP_GROUP_OWNER | P2PS_SETUP_CLIENT)) {
		/* Add Config Timeout */
	...
	p2p_buf_add_feature_capability(buf, sizeof(feat_cap_mask), feat_cap_mask);
...
static struct wpabuf *p2p_build_prov_disc_req(struct p2p_data *p2p, struct p2p_device *dev, int join)
{
	struct wpabuf *buf;
	u8 *len;
	size_t extra = 0;
	...
	if (p2p->p2ps_prov) {
		extra += os_strlen(p2p->p2ps_prov->info) + 1 + sizeof(struct p2ps_provision);
	}
	...
	if (p2p->p2ps_prov) {
		...
		p2p_buf_add_group_id(buf, go->info.p2p_device_addr, go->oper_ssid, go->oper_ssid_len);
	}
	p2p_buf_update_ie_hdr(buf, len);
...
static struct wpabuf *p2p_build_prov_disc_resp(struct p2p_data *p2p, struct p2p_device *dev, u8 dialog_token, enum p2p_status_code status, u16 config_methods, u32 adv_id, const u8 *group_id, size_t group_id_len, const u8 *persist_ssid, size_t persist_ssid_len)
{
	struct wpabuf *buf;
	size_t extra = 0;
	int persist = 0;
	...
	if (p2p->p2ps_prov && p2p->p2ps_prov->adv_id == adv_id) {
		...
		group_capab = P2P_GROUP_CAPAB_PERSISTENT_GROUP | P2P_GROUP_CAPAB_PERSISTENT_RECONN;
		if (p2p->cross_connect) {
			group_capab |= P2P_GROUP_CAPAB_CROSS_CONN;
		}
		if (p2p->cfg->p2p_intra_bss) {
		...
		if (persist_ssid && p2p->cfg->get_persistent_group && (status == P2P_SC_SUCCESS || status == P2P_SC_SUCCESS_DEFERRED)) {
			u8 ssid[SSID_MAX_LEN];
			size_t ssid_len;
			u8 go_dev_addr[ETH_ALEN];

			persist = p2p->cfg->get_persistent_group(p2p->cfg->cb_ctx, dev->info.p2p_device_addr, persist_ssid, persist_ssid_len, go_dev_addr, ssid, &ssid_len);
			if (persist) {
				p2p_buf_add_persistent_group_info(buf, go_dev_addr, ssid, ssid_len);
			}
		}
		...
		if (!persist && (status == P2P_SC_SUCCESS || status == P2P_SC_SUCCESS_DEFERRED)) {
			p2p_buf_add_connection_capability(buf, prov->conncap);
		}
...
static int p2ps_setup_p2ps_prov(struct p2p_data *p2p, u32 adv_id, u32 session_id, u16 method, const u8 *session_mac, const u8 *adv_mac)
{
...
int p2p_send_prov_disc_req(struct p2p_data *p2p, struct p2p_device *dev, int join, int force_freq)
{
	...
	if (freq <= 0) {
		p2p_dbg(p2p, "No Listen/Operating frequency known for the peer " MACSTR " to send Provision Discovery Request", MAC2STR(dev->info.p2p_device_addr));
		return -1;
	}

	if (dev->flags & P2P_DEV_GROUP_CLIENT_ONLY) {
		if (!(dev->info.dev_capab & P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY)) {
			p2p_dbg(p2p, "Cannot use PD with P2P Device " MACSTR " that is in a group and is not discoverable", MAC2STR(dev->info.p2p_device_addr));
			return -1;
		}
	...
	if (p2p->p2ps_prov) {
		...
		p2p_dbg(p2p, "Building PD Request based on P2PS config method 0x%x status %d --> req_config_methods 0x%x", p2p->p2ps_prov->method, p2p->p2ps_prov->status, dev->req_config_methods);
	}
	...
	if (p2p_send_action(p2p, freq, dev->info.p2p_device_addr, p2p->cfg->dev_addr, dev->info.p2p_device_addr, wpabuf_head(req), wpabuf_len(req), 200) < 0) {
		p2p_dbg(p2p, "Failed to send Action frame");
		wpabuf_free(req);
		return -1;
	}
