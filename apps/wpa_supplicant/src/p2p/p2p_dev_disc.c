...
static struct wpabuf *p2p_build_dev_disc_req(struct p2p_data *p2p, struct p2p_device *go, const u8 *dev_id)
{
	struct wpabuf *buf;
	u8 *len;
...
{
	p2p_dbg(p2p, "Device Discoverability Request TX callback: success=%d", success);
...
int p2p_send_dev_disc_req(struct p2p_data *p2p, struct p2p_device *dev)
{
	...
	p2p_dbg(p2p, "Sending Device Discoverability Request to GO " MACSTR " for client " MACSTR, MAC2STR(go->info.p2p_device_addr), MAC2STR(dev->info.p2p_device_addr));
	...
	if (p2p_send_action(p2p, dev->oper_freq, go->info.p2p_device_addr, p2p->cfg->dev_addr, go->info.p2p_device_addr, wpabuf_head(req), wpabuf_len(req), wait_time) < 0) {
...
{
	p2p_dbg(p2p, "Device Discoverability Response TX callback: success=%d", success);
	p2p->cfg->send_action_done(p2p->cfg->cb_ctx);
}

static void p2p_send_dev_disc_resp(struct p2p_data *p2p, u8 dialog_token, const u8 *addr, int freq, u8 status)
{
	struct wpabuf *resp;
	...
	p2p_dbg(p2p, "Sending Device Discoverability Response to " MACSTR " (status %u freq %d)", MAC2STR(addr), status, freq);

	p2p->pending_action_state = P2P_PENDING_DEV_DISC_RESPONSE;
	if (p2p_send_action(p2p, freq, addr, p2p->cfg->dev_addr, p2p->cfg->dev_addr, wpabuf_head(resp), wpabuf_len(resp), 200) < 0) {
		p2p_dbg(p2p, "Failed to send Action frame");
	}
...
void p2p_process_dev_disc_req(struct p2p_data *p2p, const u8 *sa, const u8 *data, size_t len, int rx_freq)
{
	...
	p2p_dbg(p2p, "Received Device Discoverability Request from " MACSTR " (freq=%d)", MAC2STR(sa), rx_freq);
	...
	if (msg.dialog_token == 0) {
		...
		p2p_send_dev_disc_resp(p2p, msg.dialog_token, sa, rx_freq, P2P_SC_FAIL_INVALID_PARAMS);
		p2p_parse_free(&msg);
		return;
	}
	...
	for (g = 0; g < p2p->num_groups; g++) {
		if (p2p_group_go_discover(p2p->groups[g], msg.device_id, sa, rx_freq) == 0) {
	...
	p2p_send_dev_disc_resp(p2p, msg.dialog_token, sa, rx_freq, P2P_SC_FAIL_UNABLE_TO_ACCOMMODATE);
	p2p_parse_free(&msg);
}
...
void p2p_process_dev_disc_resp(struct p2p_data *p2p, const u8 *sa, const u8 *data, size_t len)
{
	...
	p2p_dbg(p2p, "Received Device Discoverability Response from " MACSTR, MAC2STR(sa));
	...
	if (msg.dialog_token != go->dialog_token) {
		p2p_dbg(p2p, "Ignore Device Discoverability Response with unexpected dialog token %u (expected %u)", msg.dialog_token, go->dialog_token);
		p2p_parse_free(&msg);
		return;
	}
	...
	if (p2p->go_neg_peer == NULL || os_memcmp(p2p->pending_client_disc_addr, p2p->go_neg_peer->info.p2p_device_addr, ETH_ALEN) != 0 || os_memcmp(p2p->go_neg_peer->member_in_go_dev, go->info.p2p_device_addr, ETH_ALEN) != 0) {
...
void p2p_go_disc_req_cb(struct p2p_data *p2p, int success)
{
	...
	p2p_send_dev_disc_resp(p2p, p2p->pending_dev_disc_dialog_token, p2p->pending_dev_disc_addr, p2p->pending_dev_disc_freq, success ? P2P_SC_SUCCESS : P2P_SC_FAIL_UNABLE_TO_ACCOMMODATE);
...
void p2p_process_go_disc_req(struct p2p_data *p2p, const u8 *da, const u8 *sa, const u8 *data, size_t len, int rx_freq)
{
	...
	tu = 100;
	if (p2p->cfg->start_listen(p2p->cfg->cb_ctx, rx_freq, 1024 * tu / 1000, ies) < 0) {
