...
int linux_set_iface_flags(int sock, const char *ifname, int dev_up)
{
	...
	if (ioctl(sock, SIOCSIFFLAGS, &ifr) != 0) {
		ret = errno ? -errno : -999;
		wpa_printf(MSG_ERROR, "Could not set interface %s flags (%s): " "%s", ifname, dev_up ? "UP" : "DOWN", strerror(errno));
		return ret;
	}
...
int linux_set_ifhwaddr(int sock, const char *ifname, const u8 *addr)
{
	...
	if (ioctl(sock, SIOCSIFHWADDR, &ifr)) {
		wpa_printf(MSG_DEBUG, "Could not set interface %s hwaddr: %s", ifname, strerror(errno));
		return -1;
	}
...
int linux_br_add(int sock, const char *brname)
{
	if (ioctl(sock, SIOCBRADDBR, brname) < 0) {
		wpa_printf(MSG_DEBUG, "Could not add bridge %s: %s", brname, strerror(errno));
		return -1;
	}
...
int linux_br_del(int sock, const char *brname)
{
	if (ioctl(sock, SIOCBRDELBR, brname) < 0) {
		wpa_printf(MSG_DEBUG, "Could not remove bridge %s: %s", brname, strerror(errno));
		return -1;
	}
...
int linux_br_add_if(int sock, const char *brname, const char *ifname)
{
	...
	if (ioctl(sock, SIOCBRADDIF, &ifr) < 0) {
		wpa_printf(MSG_DEBUG, "Could not add interface %s into bridge " "%s: %s", ifname, brname, strerror(errno));
		return -1;
	}
...
int linux_br_del_if(int sock, const char *brname, const char *ifname)
{
	...
	if (ioctl(sock, SIOCBRDELIF, &ifr) < 0) {
		wpa_printf(MSG_DEBUG, "Could not remove interface %s from " "bridge %s: %s", ifname, brname, strerror(errno));
		return -1;
	}
