...
{
	wpa_printf(MSG_DEBUG, "EAP-WSC: %s -> %s", eap_wsc_state_txt(data->state), eap_wsc_state_txt(state));
	data->state = state;
}
...
static void eap_wsc_ext_reg_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_printf(MSG_DEBUG, "EAP-WSC: Timeout while waiting for an External " "Registrar");
...
static void *eap_wsc_init(struct eap_sm *sm)
{
	...
	if (sm->identity && sm->identity_len == WSC_ID_REGISTRAR_LEN && os_memcmp(sm->identity, WSC_ID_REGISTRAR, WSC_ID_REGISTRAR_LEN) == 0) {
		registrar = 0;    /* Supplicant is Registrar */
	} else if (sm->identity && sm->identity_len == WSC_ID_ENROLLEE_LEN && os_memcmp(sm->identity, WSC_ID_ENROLLEE, WSC_ID_ENROLLEE_LEN)
			   == 0) {
		registrar = 1;    /* Supplicant is Enrollee */
	} else {
		wpa_hexdump_ascii(MSG_INFO, "EAP-WSC: Unexpected identity", sm->identity, sm->identity_len);
		return NULL;
	}
	...
	if (registrar) {
		if (sm->wps == NULL || sm->wps->registrar == NULL) {
			wpa_printf(MSG_INFO, "EAP-WSC: WPS Registrar not " "initialized");
			os_free(data);
			return NULL;
		}
	} else {
		if (sm->user == NULL || sm->user->password == NULL) {
			 ...
			 */
			wpa_printf(MSG_DEBUG, "EAP-WSC: No AP PIN (password) " "configured for Enrollee functionality - " "allow for probing capabilities (M1)");
		} else {
...
#ifdef CONFIG_P2P
	if (sm->assoc_p2p_ie) {
		wpa_printf(MSG_DEBUG, "EAP-WSC: Prefer PSK format for P2P " "client");
		cfg.use_psk_key = 1;
...
static struct wpabuf *eap_wsc_build_start(struct eap_sm *sm, struct eap_wsc_data *data, u8 id)
{
	struct wpabuf *req;

	req = eap_msg_alloc(EAP_VENDOR_WFA, EAP_VENDOR_TYPE_WSC, 2, EAP_CODE_REQUEST, id);
	if (req == NULL) {
		wpa_printf(MSG_ERROR, "EAP-WSC: Failed to allocate memory for " "request");
		return NULL;
	}
...
static struct wpabuf *eap_wsc_build_msg(struct eap_wsc_data *data, u8 id)
{
	...
	req = eap_msg_alloc(EAP_VENDOR_WFA, EAP_VENDOR_TYPE_WSC, plen, EAP_CODE_REQUEST, id);
	if (req == NULL) {
		wpa_printf(MSG_ERROR, "EAP-WSC: Failed to allocate memory for " "request");
		return NULL;
	}
...
static struct wpabuf *eap_wsc_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	case MESG:
		if (data->out_buf == NULL) {
			...
			if (data->out_buf == NULL) {
				wpa_printf(MSG_DEBUG, "EAP-WSC: Failed to " "receive message from WPS");
				return NULL;
			}
			data->out_used = 0;
		}
	...
	default:
		wpa_printf(MSG_DEBUG, "EAP-WSC: Unexpected state %d in " "buildReq", data->state);
		return NULL;
	}
}
...
static int eap_wsc_process_cont(struct eap_wsc_data *data, const u8 *buf, size_t len, u8 op_code)
{
	...
	if (op_code != data->in_op_code) {
		wpa_printf(MSG_DEBUG, "EAP-WSC: Unexpected Op-Code %d in " "fragment (expected %d)", op_code, data->in_op_code);
	...
	wpa_printf(MSG_DEBUG, "EAP-WSC: Received %lu bytes, waiting for %lu " "bytes more", (unsigned long)len, (unsigned long)wpabuf_tailroom(data->in_buf));
...
static void eap_wsc_process(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
	...
	if (flags & WSC_FLAGS_LF) {
		...
		if (message_length < end - pos || message_length > 50000) {
			wpa_printf(MSG_DEBUG, "EAP-WSC: Invalid Message " "Length");
			return;
		}
	}

	wpa_printf(MSG_DEBUG, "EAP-WSC: Received packet: Op-Code %d " "Flags 0x%x Message Length %d", op_code, flags, message_length);

	if (data->state == WAIT_FRAG_ACK) {
		if (op_code != WSC_FRAG_ACK) {
			wpa_printf(MSG_DEBUG, "EAP-WSC: Unexpected Op-Code %d " "in WAIT_FRAG_ACK state", op_code);
			eap_wsc_state(data, FAIL);
			return;
		}
	...
	if (op_code != WSC_ACK && op_code != WSC_NACK && op_code != WSC_MSG && op_code != WSC_Done) {
	...
	if (flags & WSC_FLAGS_MF) {
		if (eap_wsc_process_fragment(data, flags, op_code, message_length, pos, end - pos) < 0) {
			eap_wsc_state(data, FAIL);
		} else {
	...
	switch (res) {
	case WPS_DONE:
		wpa_printf(MSG_DEBUG, "EAP-WSC: WPS processing completed " "successfully - report EAP failure");
		eap_wsc_state(data, FAIL);
		break;
	case WPS_CONTINUE:
		eap_wsc_state(data, MESG);
...
int eap_server_wsc_register(void)
{
	...
	eap = eap_server_method_alloc(EAP_SERVER_METHOD_INTERFACE_VERSION, EAP_VENDOR_WFA, EAP_VENDOR_TYPE_WSC, "WSC");
	if (eap == NULL) {
		return -1;
	}
