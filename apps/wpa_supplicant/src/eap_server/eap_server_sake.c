...
static void *eap_sake_init(struct eap_sm *sm)
{
	...
	wpa_printf(MSG_DEBUG, "EAP-SAKE: Initialized Session ID %d", data->session_id);
...
static struct wpabuf *eap_sake_build_msg(struct eap_sake_data *data, u8 id, size_t length, u8 subtype)
{
	...
	msg = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_SAKE, plen, EAP_CODE_REQUEST, id);
	if (msg == NULL) {
...
static struct wpabuf *eap_sake_build_identity(struct eap_sm *sm, struct eap_sake_data *data, u8 id)
{
	...
	eap_sake_add_attr(msg, EAP_SAKE_AT_SERVERID, sm->server_id, sm->server_id_len);
...
static struct wpabuf *eap_sake_build_confirm(struct eap_sm *sm, struct eap_sake_data *data, u8 id)
{
	...
	msg = eap_sake_build_msg(data, id, 2 + EAP_SAKE_MIC_LEN, EAP_SAKE_SUBTYPE_CONFIRM);
	if (msg == NULL) {
		data->state = FAILURE;
		return NULL;
	}
	...
	if (eap_sake_compute_mic(data->tek.auth, data->rand_s, data->rand_p, sm->server_id, sm->server_id_len, data->peerid, data->peerid_len, 0, wpabuf_head(msg), wpabuf_len(msg), mic, mic)) {
...
static struct wpabuf *eap_sake_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	default:
		wpa_printf(MSG_DEBUG, "EAP-SAKE: Unknown state %d in buildReq", data->state);
		break;
	}
	return NULL;
}
...
static Boolean eap_sake_check(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
	...
	wpa_printf(MSG_INFO, "EAP-SAKE: Unexpected subtype=%d in state=%d", subtype, data->state);
...
static void eap_sake_process_challenge(struct eap_sm *sm, struct eap_sake_data *data, const struct wpabuf *respData, const u8 *payload, size_t payloadlen)
{
	...
	if (!attr.rand_p || !attr.mic_p) {
		wpa_printf(MSG_INFO, "EAP-SAKE: Response/Challenge did not " "include AT_RAND_P or AT_MIC_P");
		return;
	}
	...
	if (sm->user == NULL || sm->user->password == NULL || sm->user->password_len != 2 * EAP_SAKE_ROOT_SECRET_LEN) {
		wpa_printf(MSG_INFO, "EAP-SAKE: Plaintext password with " "%d-byte key not configured", 2 * EAP_SAKE_ROOT_SECRET_LEN);
		data->state = FAILURE;
		return;
	}
	eap_sake_derive_keys(sm->user->password, sm->user->password + EAP_SAKE_ROOT_SECRET_LEN, data->rand_s, data->rand_p, (u8 *)&data->tek, data->msk, data->emsk);

	eap_sake_compute_mic(data->tek.auth, data->rand_s, data->rand_p, sm->server_id, sm->server_id_len, data->peerid, data->peerid_len, 1, wpabuf_head(respData), wpabuf_len(respData), attr.mic_p, mic_p);
...
static void eap_sake_process_confirm(struct eap_sm *sm, struct eap_sake_data *data, const struct wpabuf *respData, const u8 *payload, size_t payloadlen)
{
	...
	if (!attr.mic_p) {
		wpa_printf(MSG_INFO, "EAP-SAKE: Response/Confirm did not " "include AT_MIC_P");
		return;
	}
...
int eap_server_sake_register(void)
{
	...
	eap = eap_server_method_alloc(EAP_SERVER_METHOD_INTERFACE_VERSION, EAP_VENDOR_IETF, EAP_TYPE_SAKE, "SAKE");
	if (eap == NULL) {
		return -1;
	}
