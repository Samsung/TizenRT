...
{
	wpa_printf(MSG_DEBUG, "EAP-PEAP: %s -> %s", eap_peap_state_txt(data->state), eap_peap_state_txt(state));
	data->state = state;
}
...
{
	if (data->state == FAILURE || data->state == FAILURE_REQ || data->state == SUCCESS_REQ || data->tlv_request != TLV_REQ_NONE) {
		eap_peap_state(data, FAILURE);
		return;
	}
...
static struct wpabuf *eap_peap_build_start(struct eap_sm *sm, struct eap_peap_data *data, u8 id)
{
	...
	if (req == NULL) {
		wpa_printf(MSG_ERROR, "EAP-PEAP: Failed to allocate memory for" " request");
		eap_peap_state(data, FAILURE);
		return NULL;
	}
...
static struct wpabuf *eap_peap_build_phase2_soh(struct eap_sm *sm, struct eap_peap_data *data, u8 id)
{
	...
	buf = eap_msg_alloc(EAP_VENDOR_MICROSOFT, 0x21, wpabuf_len(buf1), EAP_CODE_REQUEST, id);
	if (buf == NULL) {
		wpabuf_free(buf1);
		return NULL;
	}
	...
	wpa_hexdump_key(MSG_DEBUG, "EAP-PEAP: Encrypting Phase 2 SOH data", req, req_len);
...
static int eap_peap_derive_cmk(struct eap_sm *sm, struct eap_peap_data *data)
{
	 ...
	 */
	tk = eap_server_tls_derive_key(sm, &data->ssl, "client EAP encryption", EAP_TLS_KEY_LEN);
	if (tk == NULL) {
		return -1;
	}
...
static struct wpabuf *eap_peap_build_phase2_tlv(struct eap_sm *sm, struct eap_peap_data *data, u8 id)
{
	...
	mlen = 6;					/* Result TLV */
	if (data->peap_version == 0 && data->tlv_request == TLV_REQ_SUCCESS && data->crypto_binding != NO_BINDING) {
	...
	buf = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_TLV, mlen, EAP_CODE_REQUEST, id);
	if (buf == NULL) {
		return NULL;
	}
	...
	/* Status */
	wpabuf_put_be16(buf, data->tlv_request == TLV_REQ_SUCCESS ? EAP_TLV_RESULT_SUCCESS : EAP_TLV_RESULT_FAILURE);
	...
	if (data->peap_version == 0 && data->tlv_request == TLV_REQ_SUCCESS && data->crypto_binding != NO_BINDING) {
...
#ifdef EAP_SERVER_TNC
		if (data->soh_response) {
			wpa_printf(MSG_DEBUG, "EAP-PEAP: Adding MS-SOH " "Response TLV");
		...
		if (eap_peap_derive_cmk(sm, data) < 0 || random_get_bytes(data->binding_nonce, 32)) {
			wpabuf_free(buf);
			return NULL;
		}
...
static struct wpabuf *eap_peap_build_phase2_term(struct eap_sm *sm, struct eap_peap_data *data, u8 id, int success)
{
	...
	wpa_hexdump_key(MSG_DEBUG, "EAP-PEAP: Encrypting Phase 2 data", (u8 *)hdr, req_len);
...
static struct wpabuf *eap_peap_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	if (data->ssl.state == WAIT_FRAG_ACK) {
		return eap_server_tls_build_msg(&data->ssl, EAP_TYPE_PEAP, data->peap_version, id);
	}
	...
	case FAILURE_REQ:
		...
		break;
	default:
		wpa_printf(MSG_DEBUG, "EAP-PEAP: %s - unexpected state %d", __func__, data->state);
		return NULL;
	}
...
static void eap_peap_process_phase2_tlv(struct eap_sm *sm, struct eap_peap_data *data, struct wpabuf *in_data)
{
	...
	while (left >= 4) {
		...
		if ((size_t)tlv_len > left) {
			wpa_printf(MSG_DEBUG, "EAP-PEAP: TLV underrun " "(tlv_len=%d left=%lu)", tlv_len, (unsigned long)left);
			eap_peap_state(data, FAILURE);
			return;
		}
		switch (tlv_type) {
		...
		case EAP_TLV_CRYPTO_BINDING_TLV:
			...
			break;
		default:
			wpa_printf(MSG_DEBUG, "EAP-PEAP: Unsupported TLV Type " "%d%s", tlv_type, mandatory ? " (mandatory)" : "");
			if (mandatory) {
				eap_peap_state(data, FAILURE);
				return;
			}
	...
	if (left) {
		wpa_printf(MSG_DEBUG, "EAP-PEAP: Last TLV too short in " "Request (left=%lu)", (unsigned long)left);
		eap_peap_state(data, FAILURE);
		return;
	}
	...
	} else if (!crypto_tlv && data->crypto_binding_sent && data->crypto_binding == REQUIRE_BINDING) {
	...
	if (result_tlv) {
		...
		if (result_tlv_len < 2) {
			wpa_printf(MSG_INFO, "EAP-PEAP: Too short Result TLV " "(len=%lu)", (unsigned long)result_tlv_len);
			eap_peap_state(data, FAILURE);
			return;
		}
		...
		if (status == EAP_TLV_RESULT_SUCCESS) {
			wpa_printf(MSG_INFO, "EAP-PEAP: TLV Result - Success " "- requested %s", requested);
			...
			wpa_printf(MSG_INFO, "EAP-PEAP: TLV Result - Failure " "- requested %s", requested);
			eap_peap_state(data, FAILURE);
		} else {
			wpa_printf(MSG_INFO, "EAP-PEAP: Unknown TLV Result " "Status %d", status);
			eap_peap_state(data, FAILURE);
		}
	}
}
...
#ifdef EAP_SERVER_TNC
static void eap_peap_process_phase2_soh(struct eap_sm *sm, struct eap_peap_data *data, struct wpabuf *in_data)
{
	const u8 *pos, *vpos;
	size_t left;
	...
	if (pos == NULL) {
		wpa_printf(MSG_DEBUG, "EAP-PEAP: Not a valid SoH EAP " "Extensions Method header - skip TNC");
		goto auth_method;
	}
	...
	while (left >= 4) {
		...
		case EAP_TLV_VENDOR_SPECIFIC_TLV:
			if (tlv_len < 4) {
				wpa_printf(MSG_DEBUG, "EAP-PEAP: Too short " "vendor specific TLV (len=%d)", (int)tlv_len);
				eap_peap_state(data, FAILURE);
				return;
			}
			...
			if (vpos + vtlv_len > pos + left) {
				wpa_printf(MSG_DEBUG, "EAP-PEAP: Vendor TLV " "underrun");
				eap_peap_state(data, FAILURE);
				return;
			}
			...
			wpa_printf(MSG_DEBUG, "EAP-PEAP: Unsupported MS-TLV " "Type %d%s", tlv_type, mandatory ? " (mandatory)" : "");
			if (mandatory) {
				eap_peap_state(data, FAILURE);
				return;
			}
...
auth_method:
	...
	wpa_printf(MSG_DEBUG, "EAP-PEAP: try EAP vendor %d type %d", sm->user->methods[0].vendor, next_type);
...
static void eap_peap_process_phase2_response(struct eap_sm *sm, struct eap_peap_data *data, struct wpabuf *in_data)
{
	...
	if (data->phase2_priv == NULL) {
		wpa_printf(MSG_DEBUG, "EAP-PEAP: %s - Phase2 not " "initialized?!", __func__);
		return;
	}
	...
	if (wpabuf_len(in_data) > sizeof(*hdr) && *pos == EAP_TYPE_NAK) {
		...
		wpa_hexdump(MSG_DEBUG, "EAP-PEAP: Phase2 type Nak'ed; " "allowed types", pos + 1, left - 1);
	...
	if (data->phase2_method->check(sm, data->phase2_priv, in_data)) {
		wpa_printf(MSG_DEBUG, "EAP-PEAP: Phase2 check() asked to " "ignore the packet");
		return;
	}
	...
	if (sm->method_pending == METHOD_PENDING_WAIT) {
		wpa_printf(MSG_DEBUG, "EAP-PEAP: Phase2 method is in " "pending wait state - save decrypted response");
		wpabuf_free(data->pending_phase2_resp);
	...
	case PHASE2_SOH:
		...
		if (data->state != PHASE2_SOH && sm->tnc && data->peap_version == 0) {
			...
			wpa_printf(MSG_DEBUG, "EAP-PEAP: Try to initialize " "TNC (NAP SOH)");
			next_vendor = EAP_VENDOR_IETF;
			next_type = EAP_TYPE_NONE;
			break;
		}
...
static void eap_peap_process_phase2(struct eap_sm *sm, struct eap_peap_data *data, const struct wpabuf *respData, struct wpabuf *in_buf)
{
	...
	wpa_printf(MSG_DEBUG, "EAP-PEAP: received %lu bytes encrypted data for" " Phase 2", (unsigned long)wpabuf_len(in_buf));
	...
	if (in_decrypted == NULL) {
		wpa_printf(MSG_INFO, "EAP-PEAP: Failed to decrypt Phase 2 " "data");
		eap_peap_state(data, FAILURE);
		return;
	}

	wpa_hexdump_buf_key(MSG_DEBUG, "EAP-PEAP: Decrypted Phase 2 EAP", in_decrypted);

	if (data->peap_version == 0 && data->state != PHASE2_TLV) {
		const struct eap_hdr *resp;
		struct eap_hdr *nhdr;
		struct wpabuf *nbuf = wpabuf_alloc(sizeof(struct eap_hdr) + wpabuf_len(in_decrypted));
		if (nbuf == NULL) {
...
static int eap_peap_process_version(struct eap_sm *sm, void *priv, int peer_version)
{
	...
	if (data->force_version >= 0 && peer_version != data->force_version) {
		wpa_printf(MSG_INFO, "EAP-PEAP: peer did not select the forced" " version (forced=%d peer=%d) - reject", data->force_version, peer_version);
		return -1;
	}
	if (peer_version < data->peap_version) {
		wpa_printf(MSG_DEBUG, "EAP-PEAP: peer ver=%d, own ver=%d; " "use version %d", peer_version, data->peap_version, peer_version);
...
static void eap_peap_process_msg(struct eap_sm *sm, void *priv, const struct wpabuf *respData)
{
	...
	case FAILURE_REQ:
		...
		break;
	default:
		wpa_printf(MSG_DEBUG, "EAP-PEAP: Unexpected state %d in %s", data->state, __func__);
		break;
	}
}
...
static u8 *eap_peap_getKey(struct eap_sm *sm, void *priv, size_t *len)
{
	...
	if (data->crypto_binding_used) {
		...
		if (eapKeyData) {
			...
			*len = EAP_TLS_KEY_LEN;
			wpa_hexdump(MSG_DEBUG, "EAP-PEAP: Derived key", eapKeyData, EAP_TLS_KEY_LEN);
		} else {
	...
	eapKeyData = eap_server_tls_derive_key(sm, &data->ssl, "client EAP encryption", EAP_TLS_KEY_LEN);
	if (eapKeyData) {
		*len = EAP_TLS_KEY_LEN;
		wpa_hexdump(MSG_DEBUG, "EAP-PEAP: Derived key", eapKeyData, EAP_TLS_KEY_LEN);
	} else {
...
int eap_server_peap_register(void)
{
	...
	eap = eap_server_method_alloc(EAP_SERVER_METHOD_INTERFACE_VERSION, EAP_VENDOR_IETF, EAP_TYPE_PEAP, "PEAP");
	if (eap == NULL) {
		return -1;
	}
