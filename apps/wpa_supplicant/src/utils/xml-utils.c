...
static xml_node_t *get_node_uri_iter(struct xml_node_ctx *ctx, xml_node_t *root, char *uri)
{
	char *end;
	xml_node_t *node;
	const char *name;
...
static xml_node_t *get_node_iter(struct xml_node_ctx *ctx, xml_node_t *root, const char *path)
{
	char *end;
	xml_node_t *node;
	const char *name;
...
static void node_to_tnds(struct xml_node_ctx *ctx, xml_node_t *out, xml_node_t *in, const char *uri)
{
	xml_node_t *node;
	xml_node_t *tnds;
	const char *name;
...
static int add_ddfname(struct xml_node_ctx *ctx, xml_node_t *parent, const char *urn)
{
	xml_node_t *node;
...
static xml_node_t *get_first_child_node(struct xml_node_ctx *ctx, xml_node_t *node, const char *name)
{
	const char *lname;
	xml_node_t *child;
...
static char *get_node_text(struct xml_node_ctx *ctx, xml_node_t *node, const char *node_name)
{
...
static xml_node_t *add_mo_node(struct xml_node_ctx *ctx, xml_node_t *root, xml_node_t *node, const char *uri)
{
...
static xml_node_t *tnds_to_mo_iter(struct xml_node_ctx *ctx, xml_node_t *root, xml_node_t *node, const char *uri)
{
	xml_node_t *child;
	const char *name;
	char *nodename;
	...
	xml_node_for_each_sibling(ctx, node) {
		...
		if (strcmp(name, "Node") == 0) {
			if (root && !uri) {
				printf("Invalid TNDS tree structure - " "multiple top level nodes\n");
				xml_node_get_text_free(ctx, nodename);
				return NULL;
			}
...
xml_node_t *soap_build_envelope(struct xml_node_ctx *ctx, xml_node_t *node)
{
	...
	envelope = xml_node_create_root(ctx, "http://www.w3.org/2003/05/soap-envelope", "soap12", &ns, "Envelope");
	if (envelope == NULL) {
		return NULL;
	}
