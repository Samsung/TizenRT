...
int xml_validate(struct xml_node_ctx *ctx, xml_node_t *node, const char *xml_schema_fname, char **ret_err)
{
	...
	xmlSchemaSetParserErrors(pctx, (xmlSchemaValidityErrorFunc) add_str, (xmlSchemaValidityWarningFunc) add_str, &errors);
	schema = xmlSchemaParse(pctx);
	xmlSchemaFreeParserCtxt(pctx);

	vctx = xmlSchemaNewValidCtxt(schema);
	xmlSchemaSetValidErrors(vctx, (xmlSchemaValidityErrorFunc) add_str, (xmlSchemaValidityWarningFunc) add_str, &errors);
...
{
	xmlNodePtr node;
	node = xmlNewChild((xmlNodePtr)parent, (xmlNsPtr) ns, (const xmlChar *)name, NULL);
	return (xml_node_t *)node;
}
...
{
	xmlNodePtr node;
	node = xmlNewTextChild((xmlNodePtr)parent, (xmlNsPtr) ns, (const xmlChar *)name, (const xmlChar *)value);
	return (xml_node_t *)node;
}
...
xml_node_t *xml_node_create_text_ns(struct xml_node_ctx *ctx, xml_node_t *parent, const char *ns_uri, const char *name, const char *value)
{
	...
	node = xmlNewTextChild((xmlNodePtr)parent, NULL, (const xmlChar *)name, (const xmlChar *)value);
...
int xml_node_add_attr(struct xml_node_ctx *ctx, xml_node_t *node, xml_namespace_t *ns, const char *name, const char *value)
{
	...
	if (ns) {
		attr = xmlNewNsProp((xmlNodePtr)node, (xmlNsPtr) ns, (const xmlChar *)name, (const xmlChar *)value);
	} else {
		attr = xmlNewProp((xmlNodePtr)node, (const xmlChar *)name, (const xmlChar *)value);
	}
...
{
	return (char *)xmlGetNsProp((xmlNodePtr)node, (const xmlChar *)name, (const xmlChar *)ns_uri);
}
...
struct xml_node_ctx *xml_node_init_ctx(void *upper_ctx, const void *env)
{
	...
	LIBXML_TEST_VERSION return xctx;
}
