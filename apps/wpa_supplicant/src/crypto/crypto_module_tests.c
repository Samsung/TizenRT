...
static int test_siv(void)
{
	...
	if (os_memcmp(out, iv_c, sizeof(iv_c)) != 0) {
		wpa_printf(MSG_ERROR, "AES-SIV mode encryption returned invalid cipher text");
		return 1;
	}
	...
	if (os_memcmp(out, plaintext, sizeof(plaintext)) != 0) {
		wpa_printf(MSG_ERROR, "AES-SIV mode decryption returned invalid plain text");
		return 1;
	}
	...
	if (aes_siv_encrypt(key_2, plaintext_2, sizeof(plaintext_2), 3, addr, len, out)) {
		wpa_printf(MSG_ERROR, "AES-SIV mode encryption failed");
		return 1;
	}
...
static int test_omac1_vector(const struct omac1_test_vector *tv, unsigned int i)
{
	u8 key[] = {
		0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6,
		0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c
	...
	if (omac1_aes_128(tv->k, tv->msg, tv->msg_len, result) || os_memcmp(result, tv->tag, 16) != 0) {
	...
	if (tv->msg_len > 1) {
		...
		if (omac1_aes_128_vector(tv->k, 2, addr, len, result) || os_memcmp(result, tv->tag, 16) != 0) {
		...
		if (omac1_aes_128_vector(tv->k, 3, addr, len, result) || os_memcmp(result, tv->tag, 16) != 0) {
	...
	addr[2] = &msg[2];
	len[2] = 1;
	if (omac1_aes_128(key, msg, sizeof(msg), result) || omac1_aes_128_vector(key, 3, addr, len, result2) || os_memcmp(result, result2, 16) != 0) {
...
static int test_eax(void)
{
	...
	if (aes_128_eax_encrypt(key, nonce, sizeof(nonce), hdr, sizeof(hdr), data, sizeof(data), tag)) {
		wpa_printf(MSG_ERROR, "AES-128 EAX mode encryption failed");
		return 1;
	}
	if (os_memcmp(data, cipher, sizeof(data)) != 0) {
		wpa_printf(MSG_ERROR, "AES-128 EAX mode encryption returned invalid cipher text");
		return 1;
	}
	...
	if (aes_128_eax_decrypt(key, nonce, sizeof(nonce), hdr, sizeof(hdr), data, sizeof(data), tag)) {
		wpa_printf(MSG_ERROR, "AES-128 EAX mode decryption failed");
		return 1;
	}
	if (os_memcmp(data, msg, sizeof(data)) != 0) {
		wpa_printf(MSG_ERROR, "AES-128 EAX mode decryption returned invalid plain text");
		return 1;
	}
...
static int test_cbc(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(vectors); i++) {
		...
		if (aes_128_cbc_encrypt(tv->key, tv->iv, buf, tv->len) || os_memcmp(buf, tv->cipher, tv->len) != 0) {
			wpa_printf(MSG_ERROR, "AES-CBC encrypt %d failed", i);
			ret++;
		}

		os_memcpy(buf, tv->cipher, tv->len);
		if (aes_128_cbc_decrypt(tv->key, tv->iv, buf, tv->len) || os_memcmp(buf, tv->plain, tv->len) != 0) {
...
static int test_ecb(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(vectors); i++) {
		...
		if (hexstr2bin(tv->key, key, sizeof(key)) || hexstr2bin(tv->plaintext, plain, sizeof(plain)) || hexstr2bin(tv->ciphertext, cipher, sizeof(cipher))) {
			wpa_printf(MSG_ERROR, "Invalid AES-ECB test vector %u", i);
			ret++;
			continue;
		}

		if (aes_128_encrypt_block(key, plain, out) < 0 || os_memcmp(out, cipher, 16) != 0) {
...
static int test_md5(void)
{
	struct {
	...
	} tests[] = {
		{
			...
			"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz" "0123456789", "\xd1\x74\xab\x98\xd2\x77\xd9\xf5" "\xa5\x61\x1c\x2c\x9f\x41\x9d\x9f"
		}, {
			"12345678901234567890123456789012345678901234567890" "123456789012345678901234567890", "\x57\xed\xf4\xa2\x2b\xe3\xc9\x55" "\xac\x49\xda\x2e\x21\x07\xb6\x7a"
		}
	};
	unsigned int i;
	u8 hash[16];
	...
	for (i = 0; i < ARRAY_SIZE(tests); i++) {
		...
		if (md5_vector(1, addr, len, hash) < 0 || os_memcmp(hash, tests[i].hash, 16) != 0) {
...
static int test_eap_fast(void)
{
	...
	if (sha1_t_prf(pac_key, sizeof(pac_key), "PAC to master secret label hash", seed, sizeof(seed), buf, sizeof(master_secret)) < 0 || os_memcmp(master_secret, buf, sizeof(master_secret)) != 0) {
		wpa_printf(MSG_INFO, "T-PRF test - FAILED!");
		errors++;
	}

	wpa_printf(MSG_INFO, "- PRF (TLS, SHA1/MD5) test case / key_block");
	if (tls_prf_sha1_md5(master_secret, sizeof(master_secret), "key expansion", seed, sizeof(seed), buf, sizeof(key_block)) || os_memcmp(key_block, buf, sizeof(key_block)) != 0) {
		wpa_printf(MSG_INFO, "PRF test - FAILED!");
		errors++;
	}

	wpa_printf(MSG_INFO, "- T-PRF (SHA1) test case / IMCK");
	if (sha1_t_prf(sks, sizeof(sks), "Inner Methods Compound Keys", isk, sizeof(isk), buf, sizeof(imck)) < 0 || os_memcmp(imck, buf, sizeof(imck)) != 0) {
		wpa_printf(MSG_INFO, "T-PRF test - FAILED!");
		errors++;
	}
	...
	if (sha1_t_prf(simck, 40, "Session Key Generating Function", (u8 *)"", 0, buf, sizeof(msk)) < 0 || os_memcmp(msk, buf, sizeof(msk)) != 0) {
		wpa_printf(MSG_INFO, "T-PRF test - FAILED!");
		errors++;
	}

	wpa_printf(MSG_INFO, "- T-PRF (SHA1) test case / EMSK");
	if (sha1_t_prf(simck, 40, "Extended Session Key Generating Function", (u8 *)"", 0, buf, sizeof(msk)) < 0 || os_memcmp(emsk, buf, sizeof(emsk)) != 0) {
		wpa_printf(MSG_INFO, "T-PRF test - FAILED!");
		errors++;
	}
	...
	if (hmac_sha1(cmk, 20, tlv, sizeof(tlv), tlv + sizeof(tlv) - 20) < 0 || os_memcmp(tlv + sizeof(tlv) - 20, compound_mac, sizeof(compound_mac)) != 0) {
...
static int test_sha1(void)
{
	...
	if (sha1_prf(key0, sizeof(key0), "prefix", data0, sizeof(data0) - 1, res, sizeof(prf0)) == 0 && os_memcmp(res, prf0, sizeof(prf0)) == 0) {
	...
	if (sha1_prf(key1, sizeof(key1) - 1, "prefix", data1, sizeof(data1) - 1, res, sizeof(prf1)) == 0 && os_memcmp(res, prf1, sizeof(prf1)) == 0) {
	...
	if (sha1_prf(key2, sizeof(key2), "prefix", data2, sizeof(data2), res, sizeof(prf2)) == 0 && os_memcmp(res, prf2, sizeof(prf2)) == 0) {
	...
	for (i = 0; i < NUM_PASSPHRASE_TESTS; i++) {
		...
		if (pbkdf2_sha1(test->passphrase, (const u8 *)test->ssid, strlen(test->ssid), 4096, psk, 32) == 0 && os_memcmp(psk, test->psk, 32) == 0) {
	...
	for (i = 0; i < NUM_RFC6070_TESTS; i++) {
		...
		if (pbkdf2_sha1(test->p, (const u8 *)test->s, strlen(test->s), test->c, dk, test->dk_len) == 0 && os_memcmp(dk, test->dk, test->dk_len) == 0) {
			wpa_printf(MSG_INFO, "Test case %d - OK", i);
		} else {
			wpa_printf(MSG_INFO, "Test case %d - FAILED!", i);
			ret++;
	...
	{
		{
			...
			0xa2, 0x1b, 0x1f, 0x5d, 0x4c, 0xf4, 0xf7, 0x3a, 0x4d, 0xd9, 0x39, 0x75, 0x0f, 0x7a, 0x06, 0x6a, 0x7f, 0x98, 0xcc, 0x13, 0x1c, 0xb1, 0x6a, 0x66, 0x92, 0x75, 0x90, 0x21, 0xcf, 0xab, 0x81, 0x81
		}
	}, {
		{
			0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20
		}, 32, "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq", 56, {
			0x10, 0x4f, 0xdc, 0x12, 0x57, 0x32, 0x8f, 0x08, 0x18, 0x4b, 0xa7, 0x31, 0x31, 0xc5, 0x3c, 0xae, 0xe6, 0x98, 0xe3, 0x61, 0x19, 0x42, 0x11, 0x49, 0xea, 0x8c, 0x71, 0x24, 0x56, 0x69, 0x7d, 0x30
		}
	}, {
		{
			0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20
		}, 32, "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq" "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq", 112, {
	...
	}, {
		{
			...
			0xcd, 0xcb, 0x12, 0x20, 0xd1, 0xec, 0xcc, 0xea, 0x91, 0xe5, 0x3a, 0xba, 0x30, 0x92, 0xf9, 0x62, 0xe5, 0x49, 0xfe, 0x6c, 0xe9, 0xed, 0x7f, 0xdc, 0x43, 0x19, 0x1f, 0xbd, 0xe4, 0x5c, 0x30, 0xb0
		}
	}, {
		{
			0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25
		}, 37, {
		...
		}, 32, "Test With Truncation", 20, {
			0x75, 0x46, 0xaf, 0x01, 0x84, 0x1f, 0xc0, 0x9b, 0x1a, 0xb9, 0xc3, 0x74, 0x9a, 0x5f, 0x1c, 0x17, 0xd4, 0xf5, 0x89, 0x66, 0x8a, 0x58, 0x7b, 0x27, 0x00, 0xa9, 0xc9, 0x7c, 0x11, 0x93, 0xcf, 0x42
		}
	}, {
		{
			...
			0x69, 0x53, 0x02, 0x5e, 0xd9, 0x6f, 0x0c, 0x09, 0xf8, 0x0a, 0x96, 0xf7, 0x8e, 0x65, 0x38, 0xdb, 0xe2, 0xe7, 0xb8, 0x20, 0xe3, 0xdd, 0x97, 0x0e, 0x7d, 0xdd, 0x39, 0x09, 0x1b, 0x32, 0x35, 0x2f
		}
	}, {
		{
			0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa
		}, 80, "Test Using Larger Than Block-Size Key and Larger Than One " "Block-Size Data", 73, {
			0x63, 0x55, 0xac, 0x22, 0xe8, 0x90, 0xd0, 0xa3, 0xc8, 0x48, 0x1a, 0x5c, 0xa4, 0x82, 0x5b, 0xc8, 0x84, 0xd3, 0xe7, 0xa1, 0xff, 0x98, 0xa2, 0xfc, 0x2a, 0xc7, 0xd8, 0xe0, 0x64, 0xc3, 0xb2, 0xe6
		}
	}
};
...
static int test_sha256(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(hmac_tests); i++) {
		...
		if (hmac_sha256(t->key, t->key_len, t->data, t->data_len, hash) < 0 || os_memcmp(hash, t->hash, 32) != 0) {
			wpa_printf(MSG_INFO, " FAIL");
			errors++;
		} else {
			wpa_printf(MSG_INFO, " OK");
		...
		addr[0] = t->data;
		len[0] = t->data_len;
		if (hmac_sha256_vector(t->key, t->key_len, 1, addr, len, hash) < 0 || os_memcmp(hash, t->hash, 32) != 0) {
			wpa_printf(MSG_INFO, " FAIL");
			errors++;
		} else {
			wpa_printf(MSG_INFO, " OK");
		...
		if (len[0]) {
			...
			addr[1] = t->data + 1;
			len[1] = t->data_len - 1;
			if (hmac_sha256_vector(t->key, t->key_len, 2, addr, len, hash) < 0 || os_memcmp(hash, t->hash, 32) != 0) {
				wpa_printf(MSG_INFO, " FAIL");
				errors++;
			} else {
				wpa_printf(MSG_INFO, " OK");
...
static int test_ms_funcs(void)
{
	...
	if (nt_password_hash((u8 *)password, os_strlen(password), buf) || os_memcmp(password_hash, buf, sizeof(password_hash)) != 0) {
		wpa_printf(MSG_ERROR, "nt_password_hash failed");
		errors++;
	}

	if (generate_nt_response(auth_challenge, peer_challenge, (u8 *)username, os_strlen(username), (u8 *)password, os_strlen(password), buf) || os_memcmp(nt_response, buf, sizeof(nt_response)) != 0) {
		wpa_printf(MSG_ERROR, "generate_nt_response failed");
		errors++;
	}

	if (hash_nt_password_hash(password_hash, buf) || os_memcmp(password_hash_hash, buf, sizeof(password_hash_hash)) != 0) {
		wpa_printf(MSG_ERROR, "hash_nt_password_hash failed");
		errors++;
	}

	if (generate_authenticator_response((u8 *)password, os_strlen(password), peer_challenge, auth_challenge, (u8 *)username, os_strlen(username), nt_response, buf) || os_memcmp(authenticator_response, buf, sizeof(authenticator_response)) != 0) {
		wpa_printf(MSG_ERROR, "generate_authenticator_response failed");
		errors++;
	}

	if (get_master_key(password_hash_hash, nt_response, buf) || os_memcmp(master_key, buf, sizeof(master_key)) != 0) {
		wpa_printf(MSG_ERROR, "get_master_key failed");
		errors++;
	}

	if (get_asymetric_start_key(master_key, buf, sizeof(send_start_key), 1, 1) || os_memcmp(send_start_key, buf, sizeof(send_start_key)) != 0) {
...
int crypto_module_tests(void)
{
	...
	if (test_siv() || test_omac1() || test_eax() || test_cbc() || test_ecb() || test_key_wrap() || test_md5() || test_sha1() || test_sha256() || test_ms_funcs()) {
		ret = -1;
	}
