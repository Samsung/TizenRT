...
static int hostapd_wps_upnp_init(struct hostapd_data *hapd, struct wps_context *wps);
static void hostapd_wps_upnp_deinit(struct hostapd_data *hapd);
#endif							/* CONFIG_WPS_UPNP */

static int hostapd_wps_probe_req_rx(void *ctx, const u8 *addr, const u8 *da, const u8 *bssid, const u8 *ie, size_t ie_len, int ssi_signal);
...
static int wps_for_each(struct hostapd_iface *iface, void *ctx)
{
	...
	for (j = 0; j < iface->num_bss; j++) {
		...
		if (hapd != data->calling_hapd && (hapd->conf->wps_independent || data->calling_hapd->conf->wps_independent)) {
			continue;
		}
...
static int hostapd_wps_for_each(struct hostapd_data *hapd, int (*func)(struct hostapd_data *h, void *ctx), void *ctx)
{
	...
	if (iface->interfaces == NULL || iface->interfaces->for_each_interface == NULL) {
		return wps_for_each(iface, &data);
	}
	return iface->interfaces->for_each_interface(iface->interfaces, wps_for_each, &data);
}

static int hostapd_wps_new_psk_cb(void *ctx, const u8 *mac_addr, const u8 *p2p_dev_addr, const u8 *psk, size_t psk_len)
{
	...
	if (hapd->new_psk_cb) {
		hapd->new_psk_cb(hapd->new_psk_cb_ctx, mac_addr, p2p_dev_addr, psk, psk_len);
	}
	...
	if (ssid->wpa_psk_file) {
		...
		if (f == NULL) {
			wpa_printf(MSG_DEBUG, "Failed to add the PSK to " "'%s'", ssid->wpa_psk_file);
			return -1;
		}
...
static void hostapd_wps_pin_needed_cb(void *ctx, const u8 *uuid_e, const struct wps_device_data *dev)
{
	...
	len = os_snprintf(txt, sizeof(txt), WPS_EVENT_PIN_NEEDED "%s " MACSTR " [%s|%s|%s|%s|%s|%s]", uuid, MAC2STR(dev->mac_addr), dev->device_name, dev->manufacturer, dev->model_name, dev->model_number, dev->serial_number, wps_dev_type_bin2str(dev->pri_dev_type, devtype, sizeof(devtype)));
	...
	if (hapd->conf->wps_pin_requests) {
		...
		os_get_time(&t);
		fprintf(f, "%ld\t%s\t" MACSTR "\t%s\t%s\t%s\t%s\t%s" "\t%s\n", t.sec, uuid, MAC2STR(dev->mac_addr), dev->device_name, dev->manufacturer, dev->model_name, dev->model_number, dev->serial_number, wps_dev_type_bin2str(dev->pri_dev_type, devtype, sizeof(devtype)));
		fclose(f);
	}
}
...
static int wps_stop_registrar(struct hostapd_data *hapd, void *ctx)
{
	...
	if (hapd != data->current_hapd && hapd->wps != NULL) {
		wps_registrar_complete(hapd->wps->registrar, data->uuid_e, data->dev_pw, data->dev_pw_len);
	}
	return 0;
}

static void hostapd_wps_reg_success_cb(void *ctx, const u8 *mac_addr, const u8 *uuid_e, const u8 *dev_pw, size_t dev_pw_len)
{
	struct hostapd_data *hapd = ctx;
	char uuid[40];
	...
	wpa_msg(hapd->msg_ctx, MSG_INFO, WPS_EVENT_REG_SUCCESS MACSTR " %s", MAC2STR(mac_addr), uuid);
...
static void hostapd_wps_enrollee_seen_cb(void *ctx, const u8 *addr, const u8 *uuid_e, const u8 *pri_dev_type, u16 config_methods, u16 dev_password_id, u8 request_type, const char *dev_name)
{
	...
	wpa_msg_ctrl(hapd->msg_ctx, MSG_INFO, WPS_EVENT_ENROLLEE_SEEN MACSTR " %s %s 0x%x %u %u [%s]", MAC2STR(addr), uuid, wps_dev_type_bin2str(pri_dev_type, devtype, sizeof(devtype)), config_methods, dev_password_id, request_type, dev_name);
}
...
static void wps_reload_config(void *eloop_data, void *user_ctx)
{
	...
	if (iface->interfaces == NULL || iface->interfaces->reload_config(iface) < 0) {
		wpa_printf(MSG_WARNING, "WPS: Failed to reload the updated " "configuration");
	}
}
...
static void hapd_new_ap_event(struct hostapd_data *hapd, const u8 *attr, size_t attr_len)
{
...
static int hapd_wps_reconfig_in_memory(struct hostapd_data *hapd, const struct wps_credential *cred)
{
	...
	if (bss->wpa) {
		...
		bss->wpa_group = wpa_select_ap_group_cipher(bss->wpa, bss->wpa_pairwise, bss->rsn_pairwise);
		...
		if (cred->key_len >= 8 && cred->key_len < 64) {
			...
			if (bss->ssid.wpa_psk && hexstr2bin((const char *)cred->key, bss->ssid.wpa_psk->psk, PMK_LEN) == 0) {
...
static int hapd_wps_cred_cb(struct hostapd_data *hapd, void *ctx)
{
	...
	if ((hapd->conf->wps_cred_processing == 1 || hapd->conf->wps_cred_processing == 2) && cred->cred_attr) {
		hapd_new_ap_event(hapd, cred->cred_attr, cred->cred_attr_len);
	} else if (hapd->conf->wps_cred_processing == 1 || hapd->conf->wps_cred_processing == 2) {
		struct wpabuf *attr;
	...
	if (cred->key_len == 0) {
		...
		if (hapd->wps->network_key == NULL || hapd->wps->network_key_len < cred->key_len) {
			hapd->wps->network_key_len = 0;
			os_free(hapd->wps->network_key);
	...
	if (oconf == NULL) {
		wpa_printf(MSG_WARNING, "WPS: Could not open current " "configuration file");
		os_free(tmp_fname);
		return -1;
	}
	...
	if (nconf == NULL) {
		wpa_printf(MSG_WARNING, "WPS: Could not write updated " "configuration file");
		os_free(tmp_fname);
		fclose(oconf);
		return -1;
	}
	...
	if (wpa) {
		...
		if (cred->key_len >= 8 && cred->key_len < 64) {
			...
			wpa_printf(MSG_WARNING, "WPS: Invalid key length %lu " "for WPA/WPA2", (unsigned long)cred->key_len);
		}
	...
	if (rename(tmp_fname, hapd->iface->config_fname) < 0) {
		wpa_printf(MSG_WARNING, "WPS: Failed to rename the updated " "configuration file: %s", strerror(errno));
		os_free(tmp_fname);
		return -1;
	}
...
static int wps_pwd_auth_fail(struct hostapd_data *hapd, void *ctx)
{
	...
	wpa_printf(MSG_DEBUG, "WPS: AP PIN authentication failure number %u " "(%u consecutive)", hapd->ap_pin_failures, hapd->ap_pin_failures_consecutive);
	...
	if (!hapd->conf->ap_setup_locked && hapd->ap_pin_failures_consecutive >= 10) {
		...
		} else if (hapd->ap_pin_lockout_time < 365 * 24 * 60 * 60 && (hapd->ap_pin_failures % 3) == 0) {
			hapd->ap_pin_lockout_time *= 2;
		}

		wpa_printf(MSG_DEBUG, "WPS: Disable AP PIN for %u seconds", hapd->ap_pin_lockout_time);
		eloop_cancel_timeout(hostapd_wps_reenable_ap_pin, hapd, NULL);
		eloop_register_timeout(hapd->ap_pin_lockout_time, 0, hostapd_wps_reenable_ap_pin, hapd, NULL);
	}
...
static int wps_ap_pin_success(struct hostapd_data *hapd, void *ctx)
{
	...
	wpa_printf(MSG_DEBUG, "WPS: Clear consecutive AP PIN failure counter " "- total validation failures %u (%u consecutive)", hapd->ap_pin_failures, hapd->ap_pin_failures_consecutive);
...
static void hostapd_wps_event_success(struct hostapd_data *hapd, struct wps_event_success *success)
{
...
static void hostapd_wps_event_fail(struct hostapd_data *hapd, struct wps_event_fail *fail)
{
	...
	if (fail->error_indication > 0 && fail->error_indication < NUM_WPS_EI_VALUES) {
		wpa_msg(hapd->msg_ctx, MSG_INFO, WPS_EVENT_FAIL "msg=%d config_error=%d reason=%d (%s)", fail->msg, fail->config_error, fail->error_indication, wps_ei_str(fail->error_indication));
	} else {
		wpa_msg(hapd->msg_ctx, MSG_INFO, WPS_EVENT_FAIL "msg=%d config_error=%d", fail->msg, fail->config_error);
	}
}
...
static int hostapd_wps_rf_band_cb(void *ctx)
{
	...
	return hapd->iconf->hw_mode == HOSTAPD_MODE_IEEE80211A ? WPS_RF_50GHZ : hapd->iconf->hw_mode == HOSTAPD_MODE_IEEE80211AD ? WPS_RF_60GHZ : WPS_RF_24GHZ;	/* FIX: dualband AP */
}
...
static int get_uuid_cb(struct hostapd_iface *iface, void *ctx)
{
	...
	for (j = 0; j < iface->num_bss; j++) {
		struct hostapd_data *hapd = iface->bss[j];
		if (hapd->wps && !hapd->conf->wps_independent && !is_nil_uuid(hapd->wps->uuid)) {
			*uuid = hapd->wps->uuid;
			return 1;
		}
	}
...
static int interface_count(struct hostapd_iface *iface)
{
	...
	iface->interfaces->for_each_interface(iface->interfaces, count_interface_cb, &count);
	return count;
}

static int hostapd_wps_set_vendor_ext(struct hostapd_data *hapd, struct wps_context *wps)
{
	int i;
...
int hostapd_init_wps(struct hostapd_data *hapd, struct hostapd_bss_config *conf)
{
	...
	if (is_nil_uuid(hapd->conf->uuid)) {
		...
		if (uuid && !conf->wps_independent) {
			...
			wpa_hexdump(MSG_DEBUG, "WPS: Clone UUID from another " "interface", wps->uuid, UUID_LEN);
		} else {
			uuid_gen_mac_addr(hapd->own_addr, wps->uuid);
			wpa_hexdump(MSG_DEBUG, "WPS: UUID based on MAC " "address", wps->uuid, UUID_LEN);
		}
	} else {
	...
	wps->dev.device_name = hapd->conf->device_name ? os_strdup(hapd->conf->device_name) : NULL;
	wps->dev.manufacturer = hapd->conf->manufacturer ? os_strdup(hapd->conf->manufacturer) : NULL;
	wps->dev.model_name = hapd->conf->model_name ? os_strdup(hapd->conf->model_name) : NULL;
	wps->dev.model_number = hapd->conf->model_number ? os_strdup(hapd->conf->model_number) : NULL;
	wps->dev.serial_number = hapd->conf->serial_number ? os_strdup(hapd->conf->serial_number) : NULL;
	wps->config_methods = wps_config_methods_str2bin(hapd->conf->config_methods);
	if ((wps->config_methods & (WPS_CONFIG_DISPLAY | WPS_CONFIG_VIRT_DISPLAY | WPS_CONFIG_PHY_DISPLAY)) == WPS_CONFIG_DISPLAY) {
	...
	if ((wps->config_methods & (WPS_CONFIG_PUSHBUTTON | WPS_CONFIG_VIRT_PUSHBUTTON | WPS_CONFIG_PHY_PUSHBUTTON)) == WPS_CONFIG_PUSHBUTTON) {
	...
	if (conf->wps_rf_bands) {
		...
		wps->dev.rf_bands = hapd->iconf->hw_mode == HOSTAPD_MODE_IEEE80211A ? WPS_RF_50GHZ : hapd->iconf->hw_mode == HOSTAPD_MODE_IEEE80211AD ? WPS_RF_60GHZ : WPS_RF_24GHZ;	/* FIX: dualband AP */
	}
	...
	if (conf->ssid.wpa_psk_file) {
		...
		wpa_snprintf_hex((char *)wps->network_key, 2 * PMK_LEN + 1, conf->ssid.wpa_psk->psk, PMK_LEN);
	...
	cfg.disable_auto_conf = (hapd->conf->wps_cred_processing == 1) && conf->skip_cred_build;
	if (conf->ssid.security_policy == SECURITY_STATIC_WEP) {
		cfg.static_wep_only = 1;
	}
	cfg.dualband = interface_count(hapd->iface) > 1;
	if ((wps->dev.rf_bands & (WPS_RF_50GHZ | WPS_RF_24GHZ)) == (WPS_RF_50GHZ | WPS_RF_24GHZ)) {
		cfg.dualband = 1;
	}
	if (cfg.dualband) {
...
static int wps_add_pin(struct hostapd_data *hapd, void *ctx)
{
	...
	ret = wps_registrar_add_pin(hapd->wps->registrar, data->addr, data->uuid, data->pin, data->pin_len, data->timeout);
	if (ret == 0) {
		data->added++;
	}
	return ret;
...
{
	return hostapd_wps_for_each(hapd, wps_button_pushed, (void *)p2p_dev_addr);
}
...
static int hostapd_wps_probe_req_rx(void *ctx, const u8 *addr, const u8 *da, const u8 *bssid, const u8 *ie, size_t ie_len, int ssi_signal)
{
	...
	if (ieee802_11_parse_elems(ie, ie_len, &elems, 0) == ParseFailed) {
		wpa_printf(MSG_DEBUG, "WPS: Could not parse ProbeReq from " MACSTR, MAC2STR(addr));
		return 0;
	}

	if (elems.ssid && elems.ssid_len > 0 && (elems.ssid_len != hapd->conf->ssid.ssid_len || os_memcmp(elems.ssid, hapd->conf->ssid.ssid, elems.ssid_len) != 0)) {
		return 0;    /* Not for us */
	}
	...
	if (wpabuf_len(wps_ie) > 0) {
		int p2p_wildcard = 0;
#ifdef CONFIG_P2P
		if (elems.ssid && elems.ssid_len == P2P_WILDCARD_SSID_LEN && os_memcmp(elems.ssid, P2P_WILDCARD_SSID, P2P_WILDCARD_SSID_LEN) == 0) {
			p2p_wildcard = 1;
		}
#endif							/* CONFIG_P2P */
		wps_registrar_probe_req_rx(hapd->wps->registrar, addr, wps_ie, p2p_wildcard);
#ifdef CONFIG_WPS_UPNP
		/* FIX: what exactly should be included in the WLANEvent?
		 * WPS attributes? Full ProbeReq frame? */
		if (!p2p_wildcard) {
			upnp_wps_device_send_wlan_event(hapd->wps_upnp, addr, UPNP_WPS_WLANEVENT_TYPE_PROBE, wps_ie);
		}
#endif							/* CONFIG_WPS_UPNP */
	}
...
static int hostapd_rx_req_put_wlan_response(void *priv, enum upnp_wps_wlanevent_type ev_type, const u8 *mac_addr, const struct wpabuf *msg, enum wps_msg_type msg_type)
{
	struct hostapd_data *hapd = priv;
	struct sta_info *sta;
	struct upnp_pending_message *p;

	wpa_printf(MSG_DEBUG, "WPS UPnP: PutWLANResponse ev_type=%d mac_addr=" MACSTR, ev_type, MAC2STR(mac_addr));
	wpa_hexdump(MSG_MSGDUMP, "WPS UPnP: PutWLANResponse NewMessage", wpabuf_head(msg), wpabuf_len(msg));
	if (ev_type != UPNP_WPS_WLANEVENT_TYPE_EAP) {
		wpa_printf(MSG_DEBUG, "WPS UPnP: Ignored unexpected " "PutWLANResponse WLANEventType %d", ev_type);
		return -1;
	}
	...
	if (!sta) {
		 ...
		 */
		wpa_printf(MSG_DEBUG, "WPS UPnP: No matching STA found based " "on NewWLANEventMAC; try wildcard match");
...
int hostapd_wps_config_ap(struct hostapd_data *hapd, const char *ssid, const char *auth, const char *encr, const char *key)
{
	...
	len = os_strlen(ssid);
	if ((len & 1) || len > 2 * sizeof(cred.ssid) || hexstr2bin(ssid, cred.ssid, len / 2)) {
		return -1;
	}
	cred.ssid_len = len / 2;
	...
	if (key) {
		len = os_strlen(key);
		if ((len & 1) || len > 2 * sizeof(cred.key) || hexstr2bin(key, cred.key, len / 2)) {
			return -1;
		}
		cred.key_len = len / 2;
	}
...
static int wps_add_nfc_password_token(struct hostapd_data *hapd, void *ctx)
{
	...
	ret = wps_registrar_add_nfc_password_token(hapd->wps->registrar, data->oob_dev_pw, data->oob_dev_pw_len);
	if (ret == 0) {
		data->added++;
	}
	return ret;
...
static int hostapd_wps_add_nfc_password_token(struct hostapd_data *hapd, struct wps_parse_attr *attr)
{
...
static int hostapd_wps_nfc_tag_process(struct hostapd_data *hapd, const struct wpabuf *wps)
{
...
struct wpabuf *hostapd_wps_nfc_config_token(struct hostapd_data *hapd, int ndef)
{
	...
	ret = wps_get_oob_cred(hapd->wps, hostapd_wps_rf_band_cb(hapd), hapd->iconf->channel);
	if (ndef && ret) {
		struct wpabuf *tmp;
...
struct wpabuf *hostapd_wps_nfc_hs_cr(struct hostapd_data *hapd, int ndef)
{
	...
	if (hapd->conf->wps_nfc_dh_pubkey == NULL) {
		struct wps_context *wps = hapd->wps;
		if (wps_nfc_gen_dh(&hapd->conf->wps_nfc_dh_pubkey, &hapd->conf->wps_nfc_dh_privkey) < 0) {
			return NULL;
		}
		hostapd_wps_nfc_clear(wps);
	...
	ret = wps_build_nfc_handover_sel(hapd->wps, hapd->conf->wps_nfc_dh_pubkey, hapd->own_addr, hapd->iface->freq);
...
int hostapd_wps_nfc_report_handover(struct hostapd_data *hapd, const struct wpabuf *req, const struct wpabuf *sel)
{
	...
	ret = wps_registrar_add_nfc_pw_token(hapd->wps->registrar, attr.oob_dev_password, DEV_PW_NFC_CONNECTION_HANDOVER, NULL, 0, 1);
...
struct wpabuf *hostapd_wps_nfc_token_gen(struct hostapd_data *hapd, int ndef)
{
	if (hapd->conf->wps_nfc_pw_from_config) {
		return wps_nfc_token_build(ndef, hapd->conf->wps_nfc_dev_pw_id, hapd->conf->wps_nfc_dh_pubkey, hapd->conf->wps_nfc_dev_pw);
	}

	return wps_nfc_token_gen(ndef, &hapd->conf->wps_nfc_dev_pw_id, &hapd->conf->wps_nfc_dh_pubkey, &hapd->conf->wps_nfc_dh_privkey, &hapd->conf->wps_nfc_dev_pw);
}
...
int hostapd_wps_nfc_token_enable(struct hostapd_data *hapd)
{
	...
	if (!hapd->conf->wps_nfc_dh_pubkey || !hapd->conf->wps_nfc_dh_privkey || !hapd->conf->wps_nfc_dev_pw || !hapd->conf->wps_nfc_dev_pw_id) {
		return -1;
	}

	hostapd_wps_nfc_clear(wps);
	wpa_printf(MSG_DEBUG, "WPS: Enable NFC Tag (Dev Pw Id %u) for AP interface %s (context %p)", hapd->conf->wps_nfc_dev_pw_id, hapd->conf->iface, wps);
	...
	if (wps->ap_nfc_dev_pw) {
		wpa_snprintf_hex_uppercase((char *)wpabuf_put(wps->ap_nfc_dev_pw, wpabuf_len(pw) * 2), wpabuf_len(pw) * 2 + 1, wpabuf_head(pw), wpabuf_len(pw));
	}

	if (!wps->ap_nfc_dh_pubkey || !wps->ap_nfc_dh_privkey || !wps->ap_nfc_dev_pw) {
		hostapd_wps_nfc_clear(wps);
		return -1;
	}
...
{
	wpa_printf(MSG_DEBUG, "WPS: Disable NFC token for AP interface %s", hapd->conf->iface);
