...
static int hostapd_get_sta_tx_rx(struct hostapd_data *hapd, struct sta_info *sta, char *buf, size_t buflen)
{
...
static int hostapd_get_sta_conn_time(struct sta_info *sta, char *buf, size_t buflen)
{
...
static int hostapd_ctrl_iface_sta_mib(struct hostapd_data *hapd, struct sta_info *sta, char *buf, size_t buflen)
{
	...
	ret = os_snprintf(buf + len, buflen - len, "\naid=%d\ncapability=0x%x\n" "listen_interval=%d\nsupported_rates=", sta->aid, sta->capability, sta->listen_interval);
	if (os_snprintf_error(buflen - len, ret)) {
		return len;
	}
	len += ret;
	...
	for (i = 0; i < sta->supported_rates_len; i++) {
		ret = os_snprintf(buf + len, buflen - len, "%02x%s", sta->supported_rates[i], i + 1 < sta->supported_rates_len ? " " : "");
		if (os_snprintf_error(buflen - len, ret)) {
			return len;
		}
		len += ret;
...
static int p2p_manager_disconnect(struct hostapd_data *hapd, u16 stype, u8 minor_reason_code, const u8 *addr)
{
	...
	wpa_dbg(hapd->msg_ctx, MSG_DEBUG, "P2P: Disconnect STA " MACSTR " with minor reason code %u (stype=%u (%s))", MAC2STR(addr), minor_reason_code, stype, fc2str(mgmt->frame_control));
	...
	if (stype == WLAN_FC_STYPE_DEAUTH) {
		...
		mgmt->u.disassoc.reason_code = host_to_le16(WLAN_REASON_PREV_AUTH_NOT_VALID);
	...
	*pos++ = minor_reason_code;
	...
	ret = hapd->driver->send_frame(hapd->drv_priv, (u8 *)mgmt, pos - (u8 *)mgmt, 1);
	os_free(mgmt);
...
int hostapd_ctrl_iface_deauthenticate(struct hostapd_data *hapd, const char *txtaddr)
{
	...
	if (pos) {
		...
		if (hapd->driver->send_frame(hapd->drv_priv, (u8 *)&mgmt, IEEE80211_HDRLEN + sizeof(mgmt.u.deauth), encrypt) < 0) {
			return -1;
		}
		return 0;
	}
	...
	if (pos) {
		return p2p_manager_disconnect(hapd, WLAN_FC_STYPE_DEAUTH, atoi(pos + 5), addr);
	}
#endif							/* CONFIG_P2P_MANAGER */
...
int hostapd_ctrl_iface_disassociate(struct hostapd_data *hapd, const char *txtaddr)
{
	...
	pos = os_strstr(txtaddr, " p2p=");
	if (pos) {
		return p2p_manager_disconnect(hapd, WLAN_FC_STYPE_DISASSOC, atoi(pos + 5), addr);
	}
#endif							/* CONFIG_P2P_MANAGER */
...
int hostapd_ctrl_iface_status(struct hostapd_data *hapd, char *buf, size_t buflen)
{
	...
	ret = os_snprintf(buf + len, buflen - len, "state=%s\n" "phy=%s\n" "freq=%d\n" "num_sta_non_erp=%d\n" "num_sta_no_short_slot_time=%d\n" "num_sta_no_short_preamble=%d\n" "olbc=%d\n" "num_sta_ht_no_gf=%d\n" "num_sta_no_ht=%d\n" "num_sta_ht_20_mhz=%d\n" "num_sta_ht40_intolerant=%d\n" "olbc_ht=%d\n" "ht_op_mode=0x%x\n", hostapd_state_text(iface->state), iface->phy, iface->freq, iface->num_sta_non_erp, iface->num_sta_no_short_slot_time, iface->num_sta_no_short_preamble, iface->olbc, iface->num_sta_ht_no_gf, iface->num_sta_no_ht, iface->num_sta_ht_20mhz, iface->num_sta_ht40_intolerant, iface->olbc_ht, iface->ht_op_mode);
	if (os_snprintf_error(buflen - len, ret)) {
		return len;
	}
	len += ret;
	...
	if (!iface->cac_started || !iface->dfs_cac_ms) {
		ret = os_snprintf(buf + len, buflen - len, "cac_time_seconds=%d\n" "cac_time_left_seconds=N/A\n", iface->dfs_cac_ms / 1000);
	} else {
		...
		ret = os_snprintf(buf + len, buflen - len, "cac_time_seconds=%u\n" "cac_time_left_seconds=%u\n", iface->dfs_cac_ms / 1000, left_time);
	}
	if (os_snprintf_error(buflen - len, ret)) {
		return len;
	}
	...
	ret = os_snprintf(buf + len, buflen - len, "channel=%u\n" "secondary_channel=%d\n" "ieee80211n=%d\n" "ieee80211ac=%d\n" "vht_oper_chwidth=%d\n" "vht_oper_centr_freq_seg0_idx=%d\n" "vht_oper_centr_freq_seg1_idx=%d\n", iface->conf->channel, iface->conf->secondary_channel, iface->conf->ieee80211n, iface->conf->ieee80211ac, iface->conf->vht_oper_chwidth, iface->conf->vht_oper_centr_freq_seg0_idx, iface->conf->vht_oper_centr_freq_seg1_idx);
	if (os_snprintf_error(buflen - len, ret)) {
		return len;
	}
	len += ret;
	...
	for (i = 0; i < iface->num_bss; i++) {
		...
		ret = os_snprintf(buf + len, buflen - len, "bss[%d]=%s\n" "bssid[%d]=" MACSTR "\n" "ssid[%d]=%s\n" "num_sta[%d]=%d\n", (int)i, bss->conf->iface, (int)i, MAC2STR(bss->own_addr), (int)i, wpa_ssid_txt(bss->conf->ssid.ssid, bss->conf->ssid.ssid_len), (int)i, bss->num_sta);
		if (os_snprintf_error(buflen - len, ret)) {
			return len;
		}
		len += ret;
