...
int hostapd_p2p_set_noa(struct hostapd_data *hapd, u8 count, int start, int duration)
{
	...
	if (count != 255) {
		wpa_printf(MSG_DEBUG, "P2P: Non-periodic NoA - set " "NoA parameters");
	...
	if (hapd->num_sta_no_p2p == 0) {
		wpa_printf(MSG_DEBUG, "P2P: No legacy STAs connected - update " "periodic NoA parameters");
		return hostapd_driver_set_noa(hapd, count, start, duration);
	}

	wpa_printf(MSG_DEBUG, "P2P: Legacy STA(s) connected - do not enable " "periodic NoA");
