...
u8 *hostapd_eid_vht_capabilities(struct hostapd_data *hapd, u8 *eid)
{
	...
	if (mode->mode == HOSTAPD_MODE_IEEE80211G && hapd->conf->vendor_vht && mode->vht_capab == 0 && hapd->iface->hw_features) {
		int i;
...
u8 *hostapd_eid_vht_operation(struct hostapd_data *hapd, u8 *eid)
{
	...
	*pos++ = sizeof(*oper);
	 ...
	 */
	oper->vht_op_info_chan_center_freq_seg0_idx = hapd->iconf->vht_oper_centr_freq_seg0_idx;
	oper->vht_op_info_chan_center_freq_seg1_idx = hapd->iconf->vht_oper_centr_freq_seg1_idx;
...
static int check_valid_vht_mcs(struct hostapd_hw_modes *mode, const u8 *sta_vht_capab)
{
	 ...
	 * stream cases.
	 */
	os_memcpy(&ap_vht_cap.vht_supported_mcs_set, mode->vht_mcs_set, sizeof(ap_vht_cap.vht_supported_mcs_set));
...
u16 copy_sta_vht_capab(struct hostapd_data *hapd, struct sta_info *sta, const u8 *vht_capab)
{
	...
	if (!vht_capab || hapd->conf->disable_11ac || !check_valid_vht_mcs(hapd->iface->current_mode, vht_capab)) {
	...
	sta->flags |= WLAN_STA_VHT;
	os_memcpy(sta->vht_capabilities, vht_capab, sizeof(struct ieee80211_vht_capabilities));
...
u16 copy_sta_vendor_vht(struct hostapd_data *hapd, struct sta_info *sta, const u8 *ie, size_t len)
{
	...
	if (!ie || len < 5 + 2 + sizeof(struct ieee80211_vht_capabilities) || hapd->conf->disable_11ac) {
		goto no_capab;
	}
	...
	if (vht_capab_len < sizeof(struct ieee80211_vht_capabilities) || (int)vht_capab_len > ie + len - vht_capab - 2) {
		goto no_capab;
	}
	vht_capab += 2;
...
u8 *hostapd_eid_vendor_vht(struct hostapd_data *hapd, u8 *eid)
{
	...
	*pos++ = (5 +				/* The Vendor OUI, type and subtype */
			  2 + sizeof(struct ieee80211_vht_capabilities) + 2 + sizeof(struct ieee80211_vht_operation));
