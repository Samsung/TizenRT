 ...
 *
 * @see ui_view_add_widget()
 * @see ui_view_add_image_widget()
 * @see ui_view_add_text_widget()
 * @see ui_view_add_scroll_widget()
 */
...
/**
 * @brief Callback function type which is called when the canvas widget is drawing.
 *
 * @param widget Canvas widget
 * @param dt Delta Time (msec)
 *
 * @see ui_canvas_widget_create()
 */
typedef void (*canvas_draw_callback)(ui_widget_t widget, uint32_t dt);

typedef void (*global_widget_event_callback)(ui_widget_t widget, uint32_t dt, ui_touch_event_t touch_event, ui_coord_t touch_coord); 
...
typedef enum {
	UI_GLOBAL_WIDGET_TOP_SWIPE = 0,
	UI_GLOBAL_WIDGET_BOTTOM_SWIPE,
	UI_GLOBAL_WIDGET_LEFT_SWIPE,
	UI_GLOBAL_WIDGET_RIGHT_SWIPE,
	UI_GLOBAL_WIDGET_BUTTON,
	UI_GLOBAL_WIDGET_TYPE_NUM
} ui_global_widget_event_type_t;
...
/**
 * @brief Set the anchor point of image widget.
 *
 * The pivot point of image asset is located at the achor point of image widget.
 * Default coordinates of the anchor point is (0, 0)
 *
 * @param[in] image Handle of the image asset
 * @param[in] x x coordinate of the anchor point
 * @param[in] y y coordinate of the anchor point
 * @return On success, UI_OK is returned. On failure, the defined error type is returned.
 *
 * @see ui_image_asset_set_pivot_point()
 * @see ui_error_t
 */
ui_error_t ui_image_widget_set_anchor_point(ui_asset_t image, int32_t x, int32_t y);
 ...
 * If user calls ui_image_widget_set_crop_area(),
 * user should call ui_image_widget_resize_image_asset() afterward to operate properly.
 * Otherwise, the cropped image will be scaled to the previous image size.
 *
 * @param[in] widget Handle of the image widget
 ...
 * @see ui_image_widget_resize_image_asset()
 * @see ui_error_t
 */
ui_error_t ui_image_widget_set_crop_area(ui_widget_t widget, ui_rect_t crop_rect);

/**
 * @brief Resize the image asset in the image widget.
 *
 * Resize the image asset which set to the image widget.
 * If the image asset of image widget is not set, UI_INVALID_PARAM is retured.
 *
 * @param[in] widget Handle of the image widget
 * @param[in] width Resized width of the image asset
 * @param[in] height Resized height of the image asset
 * @return On success, UI_OK is returned. On failure, the defined error type is returned.
 *
 * @see ui_image_widget_set_image()
 * @see ui_error_t
 ...
 * In order to operate properly, the created widget needs to set a parent (@ref ui_widget_add_child).
 *
 * @param[in] width Width of the canvas widget (Pixel)
 * @param[in] height Height of the canvas widget (Pixel)
 * @param[in] draw_cb Function pointer which used for drawing the canvas widget
 * @return On success, the handle of canvas widget is returned. On failure, UI_NULL is returned.
 *
 * @see canvas_draw_callback
 */
ui_widget_t ui_canvas_widget_create(int32_t width, int32_t height, canvas_draw_callback draw_cb);

/**
 * @brief Draw a line at the given canvas widget.
 *
 * Draw a line from (start_x, start_y) coordinate to (end_x, end_y) coordinate with the given color.
 *
 * @param[in] widget Handle of the canvas widget to be drawn a line
...
/**
 * @brief Draw a horizontal line at the given canvas widget.
 *
 * Draw a horizontal line with the given length and the color.
 *
 * @param[in] widget Handle of the canvas widget to be drawn a horizontal line
...
/**
 * @brief Draw a vertical line at the given canvas widget.
 *
 * Draw a vertical line with the given length and the color.
 *
 * @param[in] widget Handle of the canvas widget to be drawn a vertical line
 * @param[in] x x coordinate of the start point
 * @param[in] y y coordinate of the start point
...
/**
 * @brief Draw a rectangle at the given canvas widget.
 *
 * Draw a rectangle with the given information.
 *
 * @param[in] widget Handle of the canvas widget to be drawn a rectangle
...
/**
 * @brief Draw a circle at the given canvas widget.
 *
 * Draw a circle with the given information.
 *
 * @param[in] widget Handle of the canvas widget to be drawn a circle
...
/**
 * @brief Draw an ellipse at the given canvas widget.
 *
 * Draw an ellipse with the given information.
 *
 * @param[in] widget Handle of the canvas widget to be drawn an ellipse
...
/**
 * @brief Draw an arc at the given canvas widget.
 *
 * Draw an arc with the given information.
 *
 * @param[in] widget Handle of the canvas widget to be drawn an arc
...
/**
 * @brief Fill the inside of figure at the given canvas widget.
 *
 * Fill the inside of figure with given color.
 *
 * @param[in] widget Handle of the canvas widget to be filled
 * @param[in] x x coordinate of the figure's inside
 * @param[in] y y coordinate of the figure's inside
 * @param[in] color Color of the arc
 * @return On success, UI_OK is returned. On failure, the defined error type is returned.
 *
 */
...
/**
 * @brief Create a frame animation widget
 *
 * In order to operate properly, the created widget needs to set a parent (@ref ui_widget_add_child).
 * This empty widget can be used as the container widget which groups certain widgets.
 *
...
/**
 * @brief Set the total duration of the frame animation widget.
 *
 * @param[in] widget Handle of the frame animation widget
 * @param[in] duration Total duration of all frames (millisecond)
 * @return On success, UI_OK is returned. On failure, the defined error type is returned.
 */
ui_error_t ui_frame_animation_widget_set_duration(ui_widget_t widget, uint32_t duration);

/**
 * @brief Play the frame animation once.
 *
 * @param[in] widget Handle of the frame animation widget
 * @return On success, UI_OK is returned. On failure, the defined error type is returned.
 */
...
/**
 * @brief Play the frame animation with looping.
 *
 * @param[in] widget Handle of the frame animation widget
 * @return On success, UI_OK is returned. On failure, the defined error type is returned.
 */
...
/**
 * @brief Pause the frame animation.
 *
 * @param[in] widget Handle of the frame animation widget
 * @return On success, UI_OK is returned. On failure, the defined error type is returned.
 */
...
/**
 * @brief Resume the frame animation.
 *
 * @param[in] widget Handle of the frame animation widget
 * @return On success, UI_OK is returned. On failure, the defined error type is returned.
 */
...
/**
 * @brief Stop the frame animation.
 *
 * @param[in] widget Handle of the frame animation widget
 * @return On success, UI_OK is returned. On failure, the defined error type is returned.
 */
...
/**
 * @brief Create a global widget widget
 *
 * In order to operate properly, the created widget needs to set a event type (@ref ui_core_set_global_widget).
 *
 * @param[in] transit_type Type of the transition which will be applied to the created widget.
 * @return On success, the handle of widget is returned. On failure, UI_NULL is returned.
 *
