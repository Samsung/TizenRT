...
#ifndef __OCF_THINGS_TYPES_H__
#define __OCF_THINGS_TYPES_H__
...
/**
 * @brief Enumeration for OCF Things error code.
 */
typedef enum {
    OCF_THINGS_ERROR_NONE                =  0, /**< Successful */
    OCF_THINGS_ERROR_INVALID_PARAMETER   = -1, /**< Invalid parameter (If parameter is null or empty)*/
    OCF_THINGS_ERROR_OPERATION_FAILED    = -2, /**< Operation Failed */
} ocf_things_error_e;

/**
 * @brief Enumeration for OCF Things status.
 */
typedef enum {
    OCF_THINGS_STATUS_INIT = 0,                         /**< Initial state of OCF Things */
    OCF_THINGS_STATUS_ES_STARTED,                       /**< Easy-setup is started */
    OCF_THINGS_STATUS_ES_DONE,                          /**< Easy-setup is done */
    OCF_THINGS_STATUS_ES_FAILED_ON_OWNERSHIP_TRANSFER,  /**< Easy-setup failed due to Ownership-Transfer failure */
    OCF_THINGS_STATUS_CONNECTING_TO_AP,                 /**< Connecting to target Wi-Fi access point */
    OCF_THINGS_STATUS_CONNECTED_TO_AP,                  /**< Connected to target Wi-Fi access point */
    OCF_THINGS_STATUS_CONNECTING_TO_AP_FAILED,          /**< Failed to connect to target Wi-Fi access point */
    OCF_THINGS_STATUS_REGISTERING_TO_CLOUD,             /**< Trying to Sign-up/Sign-in/Publish-Resource(s) to Cloud */
    OCF_THINGS_STATUS_REGISTERED_TO_CLOUD,              /**< Publish resource(s) to cloud is complete. Now the Thing is ready to be controlled via Cloud */
    OCF_THINGS_STATUS_REGISTERING_FAILED_ON_SIGN_IN,    /**< Failed to sign-in to Cloud */
    OCF_THINGS_STATUS_REGISTERING_FAILED_ON_PUB_RES     /**< Failed to publish resources to Cloud */
 ...
 */
typedef struct _ocf_things_representation
{
     ...
     *          To free an object, ocf_things_destroy_representation_inst() in ocf_things.h should be used.
     * @param[in]  rep Instance of Representation.
     * @param[in]  key Property Name which represents the value.
     ...
     * @return @c true if value exist, otherwise @c false
     */
    bool    (*set_object_value)         (struct _ocf_things_representation* rep, const char* key, const struct _ocf_things_representation* value);
     ...
     * @remarks This API will return deep-copied array value as out parameter, so application must free it after use.\n
     *          To free each object in array, ocf_things_destroy_representation_inst() in ocf_things.h should be used.
     * @param[in]  rep Instance of Representation.
     ...
     * @return @c true if value exist, otherwise @c false
     */
    bool    (*get_object_array_value)   (struct _ocf_things_representation* rep, const char* key, struct _ocf_things_representation*** array, size_t* length);
     ...
     * @return @c true if setting value is successful, otherwise @c false
     */
    bool    (*set_object_array_value)   (struct _ocf_things_representation* rep, const char* key, const struct _ocf_things_representation** array, size_t length);

} ocf_things_representation_s;

/**
 * @brief Structure for representing the Get Request Message.
 */
typedef struct _ocf_things_get_request_message
{
     ...
     */
    bool    (*get_query_value)          (struct _ocf_things_get_request_message* req_msg, const char* key, char** value);
     ...
     */
    bool    (*has_property_key)         (struct _ocf_things_get_request_message* req_msg, const char* key);

} ocf_things_get_request_message_s;

/**
 * @brief Structure for representing the Set Request Message.
 */
typedef struct _ocf_things_set_request_message
{
    char*                               resource_uri;   /**< Resource URI */
    char*                               query;          /**< One or more query parameters of the request message. Ex: key1=value1?key2=value2?... */
    struct _ocf_things_representation*  rep;            /**< Representation of the set request message */
     ...
     * @return @c true if query exist, otherwise @c false
     */
    bool    (*get_query_value)          (struct _ocf_things_set_request_message* req_msg, const char* key, char** value);

} ocf_things_set_request_message_s;

#endif /* __OCF_THINGS_TYPES_H__ */
