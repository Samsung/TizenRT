/****************************************************************************
 *
 * Copyright 2019 Samsung Electronics All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing,
 * software distributed under the License is distributed on an
 * "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND,
 * either express or implied. See the License for the specific
 * language governing permissions and limitations under the License.
 *
 ****************************************************************************/

#ifndef __UI_VIEW_INTERNAL_H__
#define __UI_VIEW_INTERNAL_H__

#include <araui/ui_view.h>
#include <araui/ui_commons.h>
#include "ui_widget_internal.h"

typedef void (*state_changed_callback)(ui_view_t view);
typedef void (*view_transit_callback)(ui_view_t view, uint32_t dt);

typedef enum {
	UI_VIEW_STATE_CREATED = 0,
	UI_VIEW_STATE_SHOWN,
	UI_VIEW_STATE_HIDDEN,
	UI_VIEW_STATE_DESTROYED,
	UI_VIEW_STATE_MAX
} ui_view_state_t;

typedef struct ui_view_body_s ui_view_body_t;

typedef struct {
	ui_view_body_t *next_view;
	view_transit_callback transit_cb;
	uint32_t duration;
	uint32_t time;
} ui_view_transit_t;

struct ui_view_body_s {
	ui_widget_body_t *root;
	ui_widget_body_t *focus;
	state_changed_callback state_changed_cb[UI_VIEW_STATE_MAX];
	ui_view_state_t state;
	ui_view_transit_t transit_info;
};

#ifdef __cplusplus
extern "C" {
#endif

ui_error_t ui_view_list_init(void);
ui_error_t ui_view_list_deinit(void);

ui_view_body_t *ui_view_list_begin(void);
ui_view_body_t *ui_view_list_next(void);

ui_error_t ui_view_update_list_init(void);
ui_error_t ui_view_update_list_deinit(void);

ui_rect_t *ui_view_update_list_begin(void);
ui_rect_t *ui_view_update_list_next(void);
ui_error_t ui_view_add_update_list(ui_rect_t update);
ui_error_t ui_view_update_list_clear(void);

ui_view_body_t *ui_view_get_current(void);

#ifdef __cplusplus
}
#endif

#endif
