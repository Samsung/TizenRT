...
ble_result ble_manager_init(ble_client_init_config *client_config, ble_server_init_config *server_config)
{
	if (client_config == NULL && server_config == NULL) {
		BLE_ERR_RECORD(BLE_MANAGER_INVALID_ARGS);
...
{
	blemgr_mode_e mode = BLEMGR_CLIENT_MODE;
	blemgr_msg_s msg = {BLE_EVT_CMD_DEL_BOND, BLE_MANAGER_FAIL, (void *)(&mode), NULL};
	int res = blemgr_post_message(&msg);
...
{
	blemgr_msg_params param = { 2, {(void *)handle, (void *)out_data} };
...
{
	blemgr_msg_params param = { 2, {(void *)handle, (void *)in_data} };
