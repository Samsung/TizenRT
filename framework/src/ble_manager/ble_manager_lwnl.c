...
trble_result_e ble_drv_coc_set_param(uint8_t param_type, uint16_t value)
{
	trble_result_e res = TRBLE_SUCCESS;
	lwnl_msg_params msg_data = { 2, {(void *)&param_type, (void *)&value}};
	lwnl_msg msg = {BLE_INTF_NAME, {LWNL_REQ_BLE_CMD_COC_SET_PARAM}, sizeof(msg_data), (void *)&msg_data, (void *)&res};
	if (_send_msg(&msg) < 0) {
		res = TRBLE_FILE_ERROR;
	}
	return res;
