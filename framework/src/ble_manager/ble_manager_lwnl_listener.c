...
static void _lwnl_call_event(int fd, lwnl_cb_status status, int len)
{
	...
	case LWNL_EVT_BLE_CLIENT_INDI:
		...
		break;
	case LWNL_EVT_BLE_CLIENT_COC_CONNECT:
		LWNL_SET_MSG(&g_msg, BLE_EVT_CLIENT_COC_CONNECT, BLE_MANAGER_FAIL, param, NULL);
		break;
	case LWNL_EVT_BLE_CLIENT_COC_DISCONNECT:
		LWNL_SET_MSG(&g_msg, BLE_EVT_CLIENT_COC_DISCONNECT, BLE_MANAGER_FAIL, param, NULL);
		break;
	case LWNL_EVT_BLE_CLIENT_COC_SEND:
		LWNL_SET_MSG(&g_msg, BLE_EVT_CLIENT_COC_SEND, BLE_MANAGER_FAIL, param, NULL);
		break;
	case LWNL_EVT_BLE_CLIENT_COC_RECV:
		LWNL_SET_MSG(&g_msg, BLE_EVT_CLIENT_COC_RECV, BLE_MANAGER_FAIL, param, NULL);
		break;
	case LWNL_EVT_BLE_SCAN_STATE:
		LWNL_SET_MSG(&g_msg, BLE_EVT_SCAN_STATE, BLE_MANAGER_FAIL, param, NULL);
		break;
	case LWNL_EVT_BLE_COMMON_WATCHDOG_RESET:
		LWNL_SET_MSG(&g_msg, BLE_EVT_COMMON_WATCHDOG_RESET, BLE_MANAGER_FAIL, param, NULL);
		break;
	default:
