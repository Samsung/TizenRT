...
void logReceivedData(int csi_buff_len, unsigned char *csi_buff, int accuracy)
{
		...
		for(; sub_carrier_data_index < csi_buff_len; sub_carrier_data_index += increment) {
			...
			CSIFW_LOGI("%f,", get_parsed_sub_carrier(csi_buff + sub_carrier_data_index + 1 + acc, acc)); //Img number
			CSIFW_LOGI("%f], ", get_parsed_sub_carrier(csi_buff + sub_carrier_data_index, acc)); //real number
			sub_carrier += 1;
		}
...
void getParsedData(unsigned char *rawDatabuff, int raw_csi_buff_len, csi_action_param_t *config, float *parsedDatabuff, uint16_t *parsed_csi_buff_len)
{
	...
	int increment = 2 + (2 * acc);
	for (int parsedDataIdx = 0; sub_carrier_data_index < raw_csi_buff_len; parsedDataIdx += 2, sub_carrier_data_index += increment) {
