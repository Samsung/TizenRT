...
static inline int _send_msg(lwnl80211_msg *msg)
{
...
{
	WU_ENTER;
	wifi_utils_result_e res = WIFI_UTILS_SUCCESS;
	lwnl80211_msg msg = {WU_INTF_NAME, LWNL80211_DEINIT, 0, NULL, 0};
	int ret = _send_msg(&msg);
	if (ret < 0) {
		res = WIFI_UTILS_FAIL;
	}
...
wifi_utils_result_e wifi_utils_scan_ap(void *arg)
{
	...
	lwnl80211_msg msg = {WU_INTF_NAME, LWNL80211_SCAN_AP, 0, NULL, 0};
	int res = _send_msg(&msg);
	if (res < 0) {
		res = WIFI_UTILS_FAIL;
	}
