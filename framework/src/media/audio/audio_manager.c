...
static audio_manager_result_t set_audio_volume(audio_io_direction_t direct, uint8_t volume)
{
	...
	config = &card->config[card->device_id];
	scaled_volume = (volume * config->max_volume) / AUDIO_DEVICE_MAX_VOLUME;
	if ((volume * config->max_volume) % AUDIO_DEVICE_MAX_VOLUME > 0) {
		scaled_volume++;
	}
	caps_desc.caps.ac_controls.hw[0] = scaled_volume;
	caps_desc.caps.ac_len = sizeof(struct audio_caps_s);
	caps_desc.caps.ac_type = AUDIO_TYPE_FEATURE;
	...
	config->volume = caps_desc.caps.ac_controls.hw[0];
	medvdbg("Volume = %d(%d/%d)\n", caps_desc.caps.ac_controls.hw[0], volume, AUDIO_DEVICE_MAX_VOLUME);
...
audio_manager_result_t get_input_audio_gain(uint8_t *gain)
{
	...
	/* scale here */
	if (card->config[card->device_id].volume == card->config[card->device_id].max_volume) {
		*gain = AUDIO_DEVICE_MAX_VOLUME;
	} else {
		*gain = card->config[card->device_id].volume * AUDIO_DEVICE_MAX_VOLUME / card->config[card->device_id].max_volume;
	}
...
audio_manager_result_t get_output_audio_volume(uint8_t *volume)
{
	...
	if (card->config[card->device_id].volume == card->config[card->device_id].max_volume) {
		...
		*volume = card->config[card->device_id].volume * AUDIO_DEVICE_MAX_VOLUME / card->config[card->device_id].max_volume;
	}
