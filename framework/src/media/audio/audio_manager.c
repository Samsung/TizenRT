...
struct audio_resample_s {
	...
	uint8_t format;				// TODO format value from the user, format converting will be added later.
};
...
audio_manager_result_t set_audio_stream_in(unsigned int channels, unsigned int sample_rate, int format)
{
	...
	card->resample.format = pcm_format_to_bits(pcm_get_format(card->pcm)) >> 3;	// ToDo: Change into the argument "format".
...
unsigned int get_user_input_frames_to_byte(unsigned int frames)
{
	...
	card = &g_audio_in_cards[g_actual_audio_in_card_id];
	buffer_size = frames * card->resample.user_channel * card->resample.format;
	return buffer_size;
}
...
unsigned int get_user_input_bytes_to_frame(unsigned int bytes)
{
	...
	card = &g_audio_in_cards[g_actual_audio_in_card_id];
	frame_size = bytes / card->resample.user_channel / card->resample.format;
...
audio_manager_result_t get_input_audio_gain(uint8_t *gain)
{
	...
	medvdbg("ret = %d, Max volume: %d, Current Volume : %d card id : %d device id : %d\n", ret, card->config[card->device_id].max_volume, card->config[card->device_id].volume, g_actual_audio_in_card_id, card->device_id);
...
audio_manager_result_t get_output_audio_volume(uint8_t *volume)
{
	...
	medvdbg("ret = %d, Max volume: %d, Current Volume : %d card id : %d device id : %d\n", ret, card->config[card->device_id].max_volume, card->config[card->device_id].volume, g_actual_audio_out_card_id, card->device_id);
