...
static audio_manager_result_t get_audio_card_out(audio_card_operation_t card_operation, audio_handler_t **handler)
{
	...
	audio_card_info_t *curr;
	medvdbg("Try to get an audio_handler\n");
	switch (card_operation) {
	case ENABLE:
		*handler = (audio_card_info_t *)malloc(sizeof(audio_card_info_t));
		medvdbg("Out handler addr = %x\n", *handler);
		if ((*handler) == NULL) {
...
static audio_manager_result_t find_audio_card(audio_card_type_t card_type)
{
	...
	while ((dir_entry = readdir(dir_info)) != NULL) {
		...
		if (type == type_chr) {
			if (card_type == INPUT) {
				medvdbg("Before get_audio_card_in(ENABLE)\n");
				if ((ret = get_audio_card_in(ENABLE, &card_ptr)) != AUDIO_MANAGER_SUCCESS) {
					meddbg("Input audio card %d is unavailable", card_id);
					return AUDIO_MANAGER_CARD_NOT_READY;
				}
			} else {
				medvdbg("Before get_audio_card_out(ENABLE)\n");
				if ((ret = get_audio_card_out(ENABLE, &card_ptr)) != AUDIO_MANAGER_SUCCESS) {
...
{
	if (get_audio_card_in(FIND, handler) != AUDIO_MANAGER_SUCCESS) {
		return AUDIO_MANAGER_NO_AVAIL_CARD;
	}
...
{
	if (get_audio_card_out(FIND, handler) != AUDIO_MANAGER_SUCCESS) {
		return AUDIO_MANAGER_NO_AVAIL_CARD;
	}
...
{
	return get_audio_card_in(DESTROY, NULL);
}

audio_manager_result_t destroy_audio_handler_out(void)
{
	return get_audio_card_out(DESTROY, NULL);
}
