...
std::shared_ptr<InputStreamBuffer> InputStreamBuffer::create(size_t bufferSize, size_t threshold)
{
	medvdbg("InputStreamBuffer::create(%d,%d)\n", bufferSize, threshold);
	...
	std::shared_ptr<InputStreamBuffer> instance(new InputStreamBuffer(bufferSize, threshold));
	if (instance->init(bufferSize)) {
		return instance;
	}

	meddbg("InputStreamBuffer::create(%d,%d), init failed!\n", bufferSize, threshold);
	return nullptr;
}

InputStreamBuffer::InputStreamBuffer(size_t bufferSize, size_t threshold)
