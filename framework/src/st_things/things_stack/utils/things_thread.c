...
int things_thread_create(pthread_t *__newthread, pthread_attr_t *__attr, void *(*__start_routine)(void *), void *__arg)
{
	...
	if (__newthread == NULL || __start_routine == NULL) {
		THINGS_LOG_E(TAG, "Input invalied.(mandatory var: __newthread=0x%X, __start_routine=0x%X", __newthread, __start_routine);
		return ret;
	}
	...
	if (__attr == NULL) {
		if (pthread_attr_init(&attr) != 0) {
			THINGS_LOG_E(TAG, "pthread_attr_init() is failed.");
			return ret;
		}

		if (pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED) != 0) {
			THINGS_LOG_E(TAG, "pthread_attr_setdetachstate() is failed.");
			return ret;
		}
	...
	if (__attr == NULL) {
		if (pthread_attr_destroy(&attr) != 0) {
			THINGS_LOG_E(TAG, "pthread_attr_destroy() is failed.");
		}
	}
