...
void things_wifi_soft_ap_sta_join(void)
{
	THINGS_LOG_D(THINGS_INFO, TAG, "T%d --> %s", getpid(), __FUNCTION__);

}

void things_wifi_soft_ap_sta_leave(void)
{
	THINGS_LOG_D(THINGS_INFO, TAG, "T%d --> %s", getpid(), __FUNCTION__);
}
...
static const wifi_manager_cb_s wifi_callbacks = {
	...
	things_wifi_soft_ap_sta_join,
	things_wifi_soft_ap_sta_leave,
...
int things_wifi_search_cb(access_point_info_s *** p_info, int *p_count)
{
	...
	ret = pthread_mutex_init(&g_wifi_mutex, NULL);
	if (ret != 0) {
		return -1;
	}
	ret = pthread_cond_init(&g_wifi_cond, NULL);
	if (ret != 0) {
		return -1;
	}
	...
	if (p_info) {
		(*p_count) = g_wifi_scan_count;
		*p_info = (access_point_info_s **) malloc(sizeof(access_point_info_s *) * (*p_count));
		while (wifi_scan_iter != NULL) {
			(*p_info)[iter] = create_access_info();
			strcpy((*p_info)[iter]->e_ssid, wifi_scan_iter->ssid);
			strcpy((*p_info)[iter]->security_key, "1130505*");
			strcpy((*p_info)[iter]->auth_type, "WPA2-PSK");
			strcpy((*p_info)[iter]->enc_type, "AES");
			strcpy((*p_info)[iter]->channel, "1");
			strcpy((*p_info)[iter]->signal_level, "-46");
			strcpy((*p_info)[iter++]->bss_id, wifi_scan_iter->bssid);
			wifi_scan_iter = wifi_scan_iter->next;
		}
		ret = 1;
	}

	pthread_mutex_destroy(&g_wifi_mutex);
	pthread_cond_destroy(&g_wifi_cond);
...
int things_reset(void *remote_owner, es_enrollee_reset_e resetType)
{
	THINGS_LOG_D(THINGS_DEBUG, TAG, THINGS_FUNC_ENTRY);
	...
	if (resetType >= RST_ENUM_EOF || resetType < RST_NEED_CONFIRM) {
		THINGS_LOG_D(THINGS_INFO, TAG, "Not support es_enrollee_reset_e value(%d). So, Set value with RST_NEED_CONFIRM", resetType);
...
static void *__attribute__((optimize("O0"))) t_things_reset_loop(reset_args_s *args)
{
	...
	// remove this code because ARTIK051 remove message_handling_loop
...
//    while( b_reset_continue_flag == true && things_get_reset_mask(RST_CONTROL_QUEUE_EMPTY) == false)
//    {
//        usleep(300000);     // 0.3 seconds wait.
//    }
...
GOTO_OUT:
	...
	// 13. If there is Reset Result Call-Back Function, then Notify result of Reset to st_things-Application.
...
OCEntityHandlerResult things_abort(pthread_t *h_thread_abort, es_enrollee_abort_e level)
{
	s_abort_s *ARGs = NULL;
