...
static int check_cb_skip(int state, char *ap_name, char *ip_addr)
{
	...
	if ((state == 1 && ((ap_name == NULL || strlen(ap_name) == 0) || (ip_addr == NULL || strlen(ip_addr) == 0 || strcmp(ip_addr, IP_NULL_VAL) == 0))) || (state != 1 && state != 0)) {
	...
	if (app_state != state) {
		ret = 1;
	} else if (state == 1 && (strcmp(ap_name, app_ap_name) != 0 || strcmp(ip_addr, app_ip_addr) != 0)) {
		ret = 1;
	}
