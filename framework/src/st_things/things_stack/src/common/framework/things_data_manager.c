...
struct st_attribute_s const gstAttr_x_com_samsung_provisioninginfo[] = {
	{
...
const struct st_attribute_s const gstAttr_x_com_samsung_accesspointlist[] = {
	{
...
static const struct st_resource_s const gstResources[] = {
	{
...
bool util_str_copy(char **dest, const char *src)
{
	...
	(*dest) = (char *)things_malloc(srcLen + 1);
	if (NULL == *dest) {
...
static struct st_attribute_s *create_property()
{
	struct st_attribute_s *property = things_malloc(sizeof(st_attribute_s));
...
static struct st_resource_s *create_resource()
{
	struct st_resource_s *resource = things_malloc(sizeof(st_resource_s));
...
static int init_things_info_file(cJSON *cjson, char *origin_json_str)
{
	...
	if (g_things_info_file_path[0] == 0) {
		...
		THINGS_LOG_D(THINGS_DEBUG, TAG, "OCF_INFO file path : %s", g_things_info_file_path);
	}

	FILE *fp = fopen(g_things_info_file_path, "r");
	if (fp) {
		fclose(fp);
		...
		for (int device_num = 0; device_num < device_cnt; device_num++) {
			...
			if (resources != NULL) {
						...
						if (res->type != NULL) {
							...
							if (links) {
								...
								for (int linkiter = 0; linkiter < linkCnt; linkiter++) {
									...
									struct st_resource_s *link_resource = create_resource();
				...
				if (single) {
					...
					int resCnt = sizeof(gstResources) / sizeof(st_resource_s);
...
static things_resource_s *register_resource(things_server_builder_s *p_builder, struct st_resource_s *resource, const char *id)
{
	things_resource_s *ret = NULL;
	char res_uri[MAX_URI_LENGTH_OCF] = { 0 };

	if (NULL != resource) {
		THINGS_LOG_D(THINGS_DEBUG, TAG, "RESOURCE TO REGISTER : %s", resource->uri);

		memset(res_uri, 0, (size_t) MAX_URI_LENGTH_OCF);
		/*SVACE warning fix */
		strncat(res_uri, resource->uri, MAX_URI_LENGTH_OCF);
...
static things_resource_s *register_device_resource(things_server_builder_s *p_builder, st_device_s *device, const char *id)
{
	...
	if (NULL != device) {
		memset(res_uri, 0, (size_t) MAX_URI_LENGTH_OCF);
		strncat(res_uri, OC_RSRVD_DEVICE_URI, MAX_URI_LENGTH_OCF);
...
static things_resource_s *register_platform_resource(things_server_builder_s *p_builder, st_device_s *device, const char *id)
{
	...
	if (NULL != device) {
		memset(res_uri, 0, (size_t) MAX_URI_LENGTH_OCF);
		strncat(res_uri, OC_RSRVD_PLATFORM_URI, MAX_URI_LENGTH_OCF);
...
bool dm_register_device_id(void)
{
	int i = 0;
	char *id = NULL;
	bool write_file_for_device_id = false;
	...
	if (write_file_for_device_id == true) {
		dm_update_device_iInfo_list(dev_list, device_cnt);
	}
...
int dm_register_resource(things_server_builder_s *p_builder)
{
	...
	for (int device_num = 0; device_num < device_cnt; device_num++) {
		...
		st_device_s *device = NULL;
		struct st_resource_s *resource = NULL;
		...
		if (NULL != device) {
			...
			if (device->col_cnt < 1) {
				...
				memset(res_uri, 0, (size_t) MAX_URI_LENGTH_OCF);
				strncat(res_uri, device->collection[0].uri, MAX_URI_LENGTH_OCF);
		...
		if (device) {
			for (int index = 0; index < MAX_DEVICE_CAPABILTY_CNT; index++) {
				if (device->pchild_resources[index] == NULL) {
					...
					break;
				} else if (strcmp(device->pchild_resources[index]->uri, uri) == 0) {
...
int dm_update_things_cloud(es_cloud_signup_s *cl_data)
{
	...
	if (g_things_cloud_file_path[0] == 0 || cl_data == NULL) {
		THINGS_LOG_V_ERROR(THINGS_ERROR, TAG, "ocf_cloud file path is NULL(%s). or Invalid argument.(0x%X)", g_things_cloud_file_path, cl_data);
		ret = 0;
		return ret;
	}
...
//for PHASE#2
char *things_get_device_type(void)
{
	return dm_get_things_device_type(0);
}

char *things_get_device_name(void)
{
	int device_cnt = (int)hashmap_count(g_device_hmap);
	if (device_cnt > 0) {
		st_device_s *device = NULL;
...
char *things_get_server_uuid(void)
{
	int device_cnt = (int)hashmap_count(g_device_hmap);
	if (device_cnt > 0) {
		st_device_s *device = NULL;
...
int things_get_attributes_by_resource_type(const char *res_type, int *count, st_attribute_s *** attributes)
{
	...
	if (resource_type_cnt > 0) {
		...
		if (strcmp(res_type->rt, res_type) == 0) {
...
int things_get_child_resources(const char *col_res_uri, int *count, st_resource_s *** child_resources)
{
	st_device_s *device = NULL;
