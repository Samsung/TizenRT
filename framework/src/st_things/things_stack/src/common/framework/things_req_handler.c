...
static int verify_request(OCEntityHandlerRequest *eh_request, const char *uri, int req_type)
{
	...
	if ((strstr(eh_request->query, "if=") == NULL)
		...
		|| (true == resource->is_supporting_interface_type(resource, eh_request->query))) {
		result = 1;
	}
	...
	if (result == 1 && req_type == OC_REST_POST) {
		...
		if (!(resource->is_supporting_interface_type(resource, OIC_INTERFACE_SENSOR))
			&& !(resource->is_supporting_interface_type(resource, OC_RSRVD_INTERFACE_READ))) {
			// [Jay] If no query or..
			//       query with "supporting interface types"  then...
			if ((strstr(eh_request->query, "if=") == NULL)
				|| ((strstr(eh_request->query, "if=") != NULL)
					&& (resource->is_supporting_interface_type(resource, eh_request->query))
				   )
			   ) {
				// [Jay] Verify request(Attributes.) with Validator func implemented
				const int num = resource->get_num_of_res_types(resource);
				...
				if (resource->size > 1) {
					...
					for (int iter = 0; iter < resource->size; ++iter) {
						...
						res->get_representation(res, &tRep);
						tRep->things_get_object_value(tRep, OC_RSRVD_REPRESENTATION, tchild_rep);
						res->set_representation(res, tchild_rep);

						res = res->get_next(res);
					}
					...
					for (int iter = 0; iter < resource->size; ++iter) {
						...
						if (num > 0) {
							for (int i = 0; i < num; i++) {
								...
								result |= dm_validate_attribute_in_request(resource->get_res_type(child, i), (const void *) /*eh_request->payload */ resource->rep->payload);
								if (!result) {
									goto EXIT_VALIDATION;
								}
							}
						...
						resource = resource->get_next(resource);
					...
					if (num > 0) {
						for (int iter = 0; iter < num; iter++) {
							...
							result |= dm_validate_attribute_in_request(resource->get_res_type(resource, iter), (const void *)eh_request->payload);
						}
					}
				}
			}
...
static OCEntityHandlerResult convert_ap_infor(things_resource_s *target_resource, access_point_info_s **p_list, int list_cnt)
{
	...
	if (!(list_cnt == 0 || p_list == NULL)) {
		...
		if (target_resource->rep == NULL) {
			...
			target_resource->set_representation(target_resource, rep);
		} else {
...
static OCEntityHandlerResult get_provisioning_info(things_resource_s *target_resource)
{
	...
	// if(eh_result == OC_EH_OK)
	{
		...
		if (target_resource->rep == NULL) {
			...
			target_resource->set_representation(target_resource, pstRep);
		}
...
static OCEntityHandlerResult trigger_abort_request(things_resource_s *target_resource, es_enrollee_abort_e abort_es)
{
	OCEntityHandlerResult eh_result = OC_EH_ERROR;
...
static OCEntityHandlerResult set_provisioning_info(things_resource_s *target_resource)
{
	...
	if (target_resource->rep->things_get_bool_value(target_resource->rep, SEC_ATTRIBUTE_PROV_RESET, &reset) == true) {
		...
		eh_result = trigger_abort_request(target_resource, (es_enrollee_abort_e) abort_es);
	} else {
		...
		while (gp_oic_req_queue->length(gp_oic_req_queue) > 0) {
			...
			if (is_can_not_response_case(target_resource, req_type, eh_result) == false) {
				if (eh_result != OC_EH_OK && eh_result != OC_EH_SLOW) {
					...
					const char *rt = temp->get_res_type(temp, 0);
					if (0 == strcmp(rt, SEC_RTYPE_TEMPERATURE) || 0 == strcmp(rt, OIC_RTYPE_TEMPERATURE)) {
						OCRepPayload *rep_payload = target_resource->get_rep_payload(target_resource);
			...
			// [Jay] Need to design How to release memory allocated for the things_resource_s list.
...
int notify_things_observers(const char *uri, const char *query)
{
	...
	if (NULL != uri) {
		...
		// if(res)
		{
			...
			strcat(res_uri, URI_DEVICE_COL);
			strcat(res_uri, device_num);
	...
	if (flag & OC_REQUEST_FLAG) {
		...
		if (things_get_reset_mask(RST_CONTROL_MODULE_DISABLE) == true) {
			...
			if (verify_request(entity_handler_request, uri, (int)entity_handler_request->method) > 0) {
				...
				resource->set_dev_addr(resource, &(entity_handler_request->devAddr));
