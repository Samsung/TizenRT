...
#ifdef _SVR_ENC_
		secure_fwrite(svrdb, 1, svrdb_size, fp);
		secure_fclose(fp);
#else
...
static int get_mac_addr(unsigned char *p_id_buf, size_t p_id_buf_size, unsigned int *p_id_out_len)
{
	...
	if (wifi_utils_get_info(&st_wifi_info) != WIFI_UTILS_SUCCESS) {
...
#ifdef _SVR_ENC_
//    ssSetDeviceUniqueIDCallback(&get_mac_addr);
#endif
	res = SM_InitSvrDb();
	if (OIC_SEC_OK != res) {
...
//#ifdef USE_SSS
#if 1
	res = SSSRootCAHandler(subject_uuid, cred_id);
	if (OC_STACK_OK != res) {
		return res;
	}
	if (OC_STACK_OK != res) {
	...
	res = CredSaveTrustCertChain(subject_uuid, g_global_ci_root_ca, sizeof(g_global_ci_root_ca), OIC_ENCODING_DER, TRUST_CA, &cred_id);
	if (OC_STACK_OK != res) {
		THINGS_LOG_D_ERROR(THINGS_ERROR, TAG, "SRPCredSaveTrustCertChain #1 error");
		return res;
	}
	THINGS_LOG_D(THINGS_DEBUG, TAG, "thawte_Primary_Root_CA.der saved w/ cred ID=%d", cred_id);

	//For global CI : thawte_Primary_Root_CA-G2_ECC.der
	res = CredSaveTrustCertChain(subject_uuid, g_global_ci_root_ca_g2, sizeof(g_global_ci_root_ca_g2), OIC_ENCODING_DER, TRUST_CA, &cred_id);
	if (OC_STACK_OK != res) {
		THINGS_LOG_D_ERROR(THINGS_ERROR, TAG, "SRPCredSaveTrustCertChain #2 error");
		return res;
	}
	THINGS_LOG_D(THINGS_DEBUG, TAG, "thawte_Primary_Root_CA-G2_ECC.der saved w/ cred ID=%d", cred_id);

	//For global CI : thawte_Primary_Root_CA-G3_SHA256.der
	res = CredSaveTrustCertChain(subject_uuid, g_global_ci_root_ca_g3, sizeof(g_global_ci_root_ca_g3), OIC_ENCODING_DER, TRUST_CA, &cred_id);
	if (OC_STACK_OK != res) {
		THINGS_LOG_D_ERROR(THINGS_ERROR, TAG, "SRPCredSaveTrustCertChain #3 error");
		return res;
	}
	THINGS_LOG_D(THINGS_DEBUG, TAG, "thawte_Primary_Root_CA-G3_SHA256.der saved w/ cred ID=%d", cred_id);
