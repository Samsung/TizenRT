...
const char *get_int_type(struct things_resource_s *res, int index)
{
...
void /*OCRepPayload */ *get_rep_payload(struct things_resource_s *res)
{
	if (res->rep != NULL) {
		...
		if (NULL != res->get_children(res) && (NULL != strstr(res->uri, URI_DEVICE_COL)) && (strstr(res->query, OIC_INTERFACE_BATCH) != NULL)) {
			p_temp = res->next;
			rep_payload = p_temp->create_payload(p_temp, p_temp->query);
			p_temp = p_temp->next;
		}

		else {
			rep_payload = res->create_payload(res, res->query);
			// Create payload for the children resource(s)
			p_temp = res->get_children(res);
		}

		while (NULL != p_temp) {
			if (NULL != p_temp->rep) {
				OCRepPayloadAppend(rep_payload, p_temp->create_payload(p_temp, p_temp->query));
			}
			p_temp = p_temp->next;
		}
...
void add_child(things_resource_s *root, things_resource_s *child)
{
	...
	p_node = root;
	while (NULL != p_node->get_next(p_node)) {
		p_node = p_node->get_next(p_node);
	}
...
things_resource_s *create_resource_inst_impl(void *requesthd, void *resourcehd, void *query, void *rep_payload)
{
	...
	res->set_dev_addr = &set_dev_addr;
	res->get_dev_addr = &get_dev_addr;

	res->set_uri = &set_uri;
	res->set_error = &set_error;

	res->set_command_id = &set_command_id;

	res->add_child = &add_child;
	res->get_children = &get_children;

	res->get_uri = &get_uri;
	res->get_res_type = &get_res_type;
	res->get_num_of_res_types = &get_num_of_res_types;
	res->get_int_type = &get_int_type;
	res->get_num_of_inf_types = &get_num_of_inf_types;
	res->set_representation = &set_representation;
	res->get_representation = &get_representation;

	res->get_query = &get_query;
	res->get_rep_payload = &get_rep_payload;
	res->create_payload = &create_payload;
	res->is_supporting_interface_type = &is_supporting_interface_type;
	res->is_supporting_resource_type = &is_supporting_resource_type;

	res->next = NULL;
	res->get_next = &get_next;
...
things_resource_s *things_create_resource_inst(void *requesthd, void *resourcehd, void *query, void *rep_payload)
{
	...
	if (NULL != rep_payload) {
		...
		while (pl) {
			if (NULL != pl->uri) {
				...
				pTempRes->set_uri(pTempRes, pl->uri);
				res->add_child(res, pTempRes);
				//res->size++;
			} else {
...
things_resource_s *clone_resource_inst(things_resource_s *pori)
{
	...
	pclone->set_uri(pclone, pori->uri);
	...
	while (p_temp) {
		...
		p_new->set_uri(p_new, p_temp->uri);
		p_temp->add_child(p_temp, p_new);
		p_temp = p_temp->next;
	}
