...
typedef struct things_resource_s {
	...
	void *request_handle;
	void *resource_handle;	// 4 Bytes, OCResourceHandle handle;
	...
	void (*set_dev_addr)(struct things_resource_s *, void *OCDevAddr);
	void *(*get_dev_addr)(struct things_resource_s *);
	...
	*/
	void (*set_uri)(struct things_resource_s *, const char *uri);
	/**
	* @brief API for setting error when returning request handling result
	* @param things_resource_s * : Instance of target resource
	* @param double        : error code
	*/
	void (*set_error)(struct things_resource_s *, double errorCode);
	...
	*/
	bool(*get_query)(struct things_resource_s *, char *key, char **value);
	...
	*/
	void (*get_uri)(struct things_resource_s *, char **uri);
	...
	*/
	int (*get_num_of_res_types)(struct things_resource_s *);
	int (*get_num_of_inf_types)(struct things_resource_s *);
	/**
	...
	*/
	const char *(*get_res_type)(struct things_resource_s *, int index);
	const char *(*get_int_type)(struct things_resource_s *, int index);
	/**
	...
	*/
	struct things_resource_s *(*get_children)(struct things_resource_s *);
	...
	*/
	void (*set_representation)(struct things_resource_s *, struct things_representation_s *rep);
	...
	*/
	bool(*get_representation)(struct things_resource_s *, struct things_representation_s **rep);
	...
	*/
	void *(*get_rep_payload)(struct things_resource_s *);
	...
	*/
	void *(*create_payload)(struct things_resource_s *, char *);
	...
	*/
	bool(*is_supporting_interface_type)(struct things_resource_s *, char *ifType);
	...
	*/
	bool(*is_supporting_resource_type)(struct things_resource_s *, char *res_type);
	...
	*/
	void (*set_command_id)(struct things_resource_s *res, char *cmd_id);
	...
	*/
	struct things_resource_s *(*get_next)(struct things_resource_s *);
	...
	*/
	void (*add_child)(struct things_resource_s *, struct things_resource_s *);
...
*/
things_resource_s *things_create_resource_inst(void *, void *, void *, void *);
...
*/
things_resource_s *things_resource_clone(things_resource_s *);
