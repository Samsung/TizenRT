...
static const uint8_t rs_box[256] = { 0x52, 0x09, 0x6a, 0xd5, 0x30, 0x36, 0xa5, 0x38, 0xbf, 0x40, 0xa3, 0x9e, 0x81, 0xf3, 0xd7, 0xfb,
...
static void block_copy(uint8_t *output, const uint8_t *input)
{
	uint8_t i;
	for (i = 0; i < KEYLEN; ++i) {
		output[i] = input[i];
...
static void xor_iv(uint8_t *buf)
{
	uint8_t i;
	for (i = 0; i < KEYLEN; ++i) {
		buf[i] ^= iv[i];
