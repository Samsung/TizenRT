...
uint8_t *g_cert_chain;
#endif

// pkss
// temp iotivity key stored at sss
const char g_iot_ss_cert[664] = { 0x30, 0x82, 0x2, 0x94, 0x30, 0x82, 0x2, 0x38, 0xa0, 0x3, 0x2, 0x1, 0x2, 0x2, 0x14, 0x48, 0x41, 0x30, 0x31, 0x50, 0x31, 0x37, 0x30, 0x31,
...
const char g_iot_ss_cacert[1260] = { 0x30, 0x82, 0x2, 0x5d, 0x30, 0x82, 0x2, 0x1, 0xa0, 0x3, 0x2, 0x1, 0x2, 0x2, 0x1, 0x1, 0x30, 0xc, 0x6, 0x8, 0x2a, 0x86, 0x48, 0xce,
...
#if 0
OCStackResult sss_root_ca_handler(OicUuid_t *subject_uuid, uint16_t cred_id)
{
	...
	if ((res = see_get_certificate(buf, &buf_len, g_iotivity_cacert_index, CERT_PEM)) != 0) {
	...
	res = CredSaveTrustCertChain(subject_uuid, buf, buf_len, OIC_ENCODING_PEM, TRUST_CA, &cred_id);
	if (OC_STACK_OK != res) {
		THINGS_LOG_D_ERROR(THINGS_ERROR, TAG, "SRPCredSaveTrustCertChain #1 error");
		free(buf);
		return res;
	}
	THINGS_LOG_D(THINGS_DEBUG, TAG, "global_ocf_connect_cert_root.der HW saved w/ cred ID=%d", cred_id);
	free(buf);
	return res;
}
#endif
...
void *things_get_hw_key(const char *service, const char *usage, const char *key_type)
{
	...
	mbedtls_pk_context *p_key;

	p_key = malloc(sizeof(mbedtls_pk_context));

	if (p_key == NULL) {
		printf("%s : p_key context alloc failed\n", __func__);
		return NULL;
	}
...
int things_free_hw_key(void *key_context)
{
	THINGS_LOG(DEBUG, TAG, "in");
...
int things_get_own_cert_from_hw(const void *key_context, uint8_t **cert_chain, size_t *cert_chain_len)
{
	int ret;
	unsigned char *buf = NULL;
	unsigned int buf_len = SEE_BUF_MAX_SIZE;

	unsigned char *t1_buf = NULL;
	unsigned int t1_buf_len, t2_buf_len;
	...
	if (*cert_chain && *cert_chain == g_cert_chain) {
		return 0;
	}
	...
	if ((ret = see_get_certificate(buf, &buf_len, g_iotivity_cert_index, CERT_PEM)) != 0) {
	...
	memcpy(t1_buf, buf, buf_len);
	t1_buf_len = buf_len;

	buf_len = SEE_BUF_MAX_SIZE;

	if ((ret = see_get_certificate(buf, &buf_len, g_iotivity_cacert_index, CERT_PEM)) != 0) {
		printf("see_get_cert fail %d\n", ret);
		free(buf);
		free(t1_buf);
		return -1;
	...
	t2_buf_len = buf_len - i;

	*cert_chain = malloc(t1_buf_len + t2_buf_len);
	...
	memcpy(*cert_chain, t1_buf, t1_buf_len);
	memcpy(*cert_chain + t1_buf_len, buf + i, t2_buf_len);

	*cert_chain_len = t1_buf_len + t2_buf_len;

	free(buf);
	free(t1_buf);

	g_cert_chain = *cert_chain;
...
int things_setup_pk_context_from_hw(mbedtls_pk_context *ctx, void *key_context)
{
	THINGS_LOG(DEBUG, TAG, "in");

	if (ctx == NULL || key_context == NULL) {
		return -1;
	}

	mbedtls_pk_context *p_key = key_context;

	mbedtls_pk_init(p_key);
	const mbedtls_pk_info_t *pk_info;

	if ((pk_info = mbedtls_pk_info_from_type(MBEDTLS_PK_ECKEY)) == NULL) {
		mbedtls_pk_free(p_key);
		free(p_key);
		return -1;
	}

	if (mbedtls_pk_setup(p_key, pk_info)) {
		mbedtls_pk_free(p_key);
		free(p_key);
		return -1;
	}

	mbedtls_ecdsa_init((mbedtls_ecdsa_context *) p_key->pk_ctx);
	((mbedtls_ecdsa_context *)(p_key->pk_ctx))->key_index = g_iotivity_key_index;
	((mbedtls_ecdsa_context *)(p_key->pk_ctx))->grp.id = g_iotivity_key_curve;

	ctx->pk_info = p_key->pk_info;
	ctx->pk_ctx = p_key->pk_ctx;
...
void things_sss_factory_key(unsigned int cacrt_idx, unsigned int crt_idx, unsigned int key_idx, unsigned int key_type)
{
