...
static OCRepPayload *make_rep_payload(OCResourceHandle rsc_handle, OCDevAddr *dev_addr)
{
	...
	OCResourceProperty p = OCGetResourceProperties(rsc_handle);	//(OCResourceHandle *)g_cloud_resource.handle);
...
void set_ssid_in_wifi_resource(const char *ssid)
{
	...
	} else if (strcmp(ssid, g_wifi_data.ssid) == 0) {
...
void update_wifi_resource(OCRepPayload *input)
{
	...
	if (strcmp(ssid, g_wifi_resource.ssid) == 0) {
...
OCRepPayload *construct_response_of_wifi(const char *interface)
{
	...
	if (interface != NULL && !strcmp(interface, OC_RSRVD_INTERFACE_BATCH)) {	// Temporary Code.
...
OCRepPayload *construct_response_of_prov(OCEntityHandlerRequest *eh_request)
{
	...
	if (!eh_request->query || (eh_request->query && !strcmp(eh_request->query, "")) || (eh_request->query && compare_resource_interface(eh_request->query, OC_RSRVD_INTERFACE_LL)) || (eh_request->query && compare_resource_interface(eh_request->query, OC_RSRVD_INTERFACE_DEFAULT))) {
		...
		if (!eh_request->query || !strcmp(eh_request->query, "") || compare_resource_interface(eh_request->query, OC_RSRVD_INTERFACE_LL) == false) {
...
es_enrollee_state_e get_enrollee_state(void)
{
	es_enrollee_state_e Status = ES_STATE_INIT;
...
OCStackResult set_enrollee_state(es_enrollee_state_e es_state)
{
	THINGS_LOG_D(THINGS_DEBUG, ES_RH_TAG, "set_enrollee_state IN");

	pthread_mutex_lock(&g_status_mutex);
	OCStackResult result = OC_STACK_ERROR;
	es_enrollee_state_e preState = g_prov_resource.status;
...
const char *get_prov_status(es_enrollee_state_e status)
{
	switch (status) {
	case ES_STATE_INIT:
