...
static int es_cloud_state_set_and_notify(cloud_status_e state, es_error_code_e es_err, OCClientResponse *response, ci_error_code_e *ci_err);
...
OCStackApplicationResult handle_logout_cb(void *ctx, OCDoHandle handle, OCClientResponse *client_response)
{
	...
	if (g_qis_cloud_thread_running != CISESS_BUSY && g_qis_cloud_thread_running != CISESS_APDISCON) {
		es_enrollee_state_e es_err = ES_STATE_INIT;
...
bool es_cloud_session_stop_trigger(es_enrollee_state_e es_state)
{
	//THINGS_LOG_D(THINGS_DEBUG, TAG, THINGS_FUNC_ENTRY);
...
static int es_cloud_state_set_and_notify(cloud_status_e state, es_error_code_e es_err, OCClientResponse *response, ci_error_code_e *ci_err)
{
	...
	if (strState == NULL || strcmp(strState, "UNKNOWN") == 0 || state == ES_STATE_INIT || state == ES_STATE_CONNECTED_TO_ENROLLER) {
