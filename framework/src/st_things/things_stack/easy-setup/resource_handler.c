...
void set_ssid_in_wifi_resource(const char *ssid)
{
	...
	} else if (strncmp(ssid, g_wifi_data.ssid, MAX_SSID_LEN) == 0) {
...
void update_wifi_resource(OCRepPayload *input)
{
	...
	if (strncmp(ssid, g_wifi_resource.ssid, MAX_SSID_LEN) == 0) {
		THINGS_LOG_D(ES_RH_TAG, "Already connected SSID(%s).", ssid);
		things_free(ssid);
		return;
	}

	things_strncpy(g_wifi_data.ssid, ssid, MAX_SSID_LEN);
	THINGS_LOG_D(ES_RH_TAG, "g_wifi_data.ssid : %s", g_wifi_data.ssid);
...
OCRepPayload *construct_response_of_wifi(const char *interface)
{
	...
	if (interface != NULL && !strncmp(interface, OC_RSRVD_INTERFACE_BATCH, sizeof(OC_RSRVD_INTERFACE_BATCH))) {	// Temporary Code.
	...
	if (interface != NULL && !strncmp(interface, OC_RSRVD_INTERFACE_BATCH, sizeof(OC_RSRVD_INTERFACE_BATCH))) {
