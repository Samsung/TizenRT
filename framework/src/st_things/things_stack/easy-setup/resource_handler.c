...
bool start_cloud_provisioning(void *cloud_info)
{
	...
	if (CIinfo == NULL) {
		THINGS_LOG_V(ES_RH_TAG, "Invalid arguments.");
		return false;
	}
	...
	THINGS_LOG_D( ES_RH_TAG, "Domain = %s, IP = %s", CIinfo->domain, CIinfo->ip);
	...
	if (CIinfo->domain) {
		...
		if ((CIServer = (char *)things_malloc(sizeof(char) * length)) == NULL) {
			THINGS_LOG_V(ES_RH_TAG, "Memory allocation is failed.");
			goto GOTO_OUT;
		}
		...
		THINGS_LOG_V(ES_RH_TAG, "Invalid arguments.(domain = 0x%X, ip = 0x%X)", CIinfo->domain, CIinfo->ip);
		goto GOTO_OUT;
	}
	...
	THINGS_LOG_D( ES_RH_TAG, "CIServer = %s", CIServer);

	if (OCRepPayloadSetPropString(payload, THINGS_RSRVD_ES_VENDOR_ACCESS_TOKEN, CIinfo->access_token) == false) {
		THINGS_LOG_D( ES_RH_TAG, "Inserting \"%s\" property is failed.(value=\"%s\")", THINGS_RSRVD_ES_VENDOR_ACCESS_TOKEN, CIinfo->access_token);
		goto GOTO_OUT;
	}
	...
	if (CIinfo->access_token_type != NULL) {
		if (OCRepPayloadSetPropString(payload, THINGS_RSRVD_ES_VENDOR_ACCESS_TOKEN_TYPE, CIinfo->access_token_type) == false) {
			THINGS_LOG_D( ES_RH_TAG, "Inserting \"%s\" property is failed.(value=\"%s\")", THINGS_RSRVD_ES_VENDOR_ACCESS_TOKEN_TYPE, CIinfo->access_token_type);
			goto GOTO_OUT;
		}
	}

	if (OCRepPayloadSetPropString(payload, SC_RSRVD_ES_VENDOR_REFRESH_TOKEN, CIinfo->refresh_token) == false) {
		THINGS_LOG_D( ES_RH_TAG, "Inserting \"%s\" property is failed.(value=\"%s\")", SC_RSRVD_ES_VENDOR_REFRESH_TOKEN, CIinfo->refresh_token);
		goto GOTO_OUT;
	}

	if (OCRepPayloadSetPropString(payload, THINGS_RSRVD_ES_VENDOR_USER_ID, CIinfo->user_id) == false) {
		THINGS_LOG_D( ES_RH_TAG, "Inserting \"%s\" property is failed.(value=\"%s\")", THINGS_RSRVD_ES_VENDOR_USER_ID, CIinfo->user_id);
		goto GOTO_OUT;
	}

	if (OCRepPayloadSetPropString(payload, THINGS_RSRVD_ES_AUTHPROVIDER, CIinfo->auth_provider) == false) {
		THINGS_LOG_D( ES_RH_TAG, "Inserting \"%s\" property is failed.(value=\"%s\")", THINGS_RSRVD_ES_AUTHPROVIDER, CIinfo->auth_provider);
		goto GOTO_OUT;
	}

	if (OCRepPayloadSetPropString(payload, THINGS_RSRVD_ES_CISERVER, CIServer) == false) {
		THINGS_LOG_D( ES_RH_TAG, "Inserting \"%s\" property is failed.(value=\"%s\")", THINGS_RSRVD_ES_CISERVER, CIServer);
		goto GOTO_OUT;
	}

	if (OCRepPayloadSetPropString(payload, THINGS_RSRVD_ES_VENDOR_CLIENTID, CIinfo->client_id) == false) {
		THINGS_LOG_D( ES_RH_TAG, "Inserting \"%s\" property is failed.(value=\"%s\")", THINGS_RSRVD_ES_VENDOR_CLIENTID, CIinfo->client_id);
		goto GOTO_OUT;
	}
