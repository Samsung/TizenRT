...
int messaging_parse_packet(char *packet, char *buf, int buflen, pid_t *sender_pid, int *msg_type)
{
	 ...
	 * +--------------------------------------------------------------------------------------------------------+
	 */
	memcpy(&msg_version, packet, sizeof(uint32_t));
	memcpy(&offset, packet + offsetof(packet_header_t, offset), sizeof(uint32_t));
	...
	case 1:
		memcpy(sender_pid, packet + offsetof(packet_header_t, sender_pid), sizeof(pid_t));
		memcpy(msg_type, packet + offsetof(packet_header_t, msg_type), sizeof(uint32_t));
