 ...
 ****************************************************************************/
/****************************************************************************
 * messaging_unicast_send_sync
 ****************************************************************************/
int messaging_unicast_send_sync(const char *port_name, msg_send_data_t *send_data, msg_recv_buf_t *reply_buf)
{
	int ret;
	if (port_name == NULL) {
	...
	if (send_data->msg == NULL || send_data->msglen <= 0 || send_data->priority < 0) {
		msgdbg("[Messaging] unicast send sync fail : invalid param of send data.\n");
		return ERROR;
	}

	if (reply_buf->buf == NULL || reply_buf->buflen <= 0) {
		msgdbg("[Messaging] unicast send sync fail : invalid param of reply buf\n");
		return ERROR;
	}

	ret = messaging_send_internal(port_name, MSG_SEND_SYNC, send_data, NULL);
	if (ret == ERROR) {
		return ERROR;
	}
...
/****************************************************************************
 * messaging_unicast_send_async
 ****************************************************************************/
int messaging_unicast_send_async(const char *port_name, msg_send_data_t *send_data, msg_recv_buf_t *reply_buf, msg_callback_info_t *cb_info)
{
	int ret;
	if (port_name == NULL) {
	...
	if (reply_buf->buf == NULL || reply_buf->buflen <= 0 || cb_info->cb_func == NULL) {
		msgdbg("[Messaging] unicast send async fail : invalid param of reply buf or callback info.\n");
		return ERROR;
	}

	ret = messaging_send_internal(port_name, MSG_SEND_ASYNC, send_data, cb_info);
	if (ret == ERROR) {
		return ERROR;
	}
...
/****************************************************************************
 * messaging_unicast_send_noreply
 ****************************************************************************/
int messaging_unicast_send_noreply(const char *port_name, msg_send_data_t *send_data)
{
	int ret;
	if (port_name == NULL) {
		msgdbg("[Messaging] unicast send async fail : no port name.\n");
	...
	ret = messaging_send_internal(port_name, MSG_SEND_NOREPLY, send_data, NULL);
	if (ret == ERROR) {
		return ERROR;
	}
...
/****************************************************************************
 * messaging_unicast_reply
 ****************************************************************************/
int messaging_unicast_reply(const char *port_name, pid_t sender_pid, msg_send_data_t *reply_data)
{
	int ret = OK;
	char *reply_portname;
	msg_send_data_t reply;

	if (port_name == NULL || sender_pid < 0 || reply_data->msg == NULL || reply_data->msglen <= 0) {
	...
	ret = messaging_send_packet(reply_portname, MSG_SEND_NOREPLY, &reply, NULL, 0);
	if (ret != OK) {
		msgdbg("[Messaging] unicast reply fail : sending packet error.\n");
	}
