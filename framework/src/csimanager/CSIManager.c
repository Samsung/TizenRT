...
//int SYSTEM_TAG_CSIFW; not supported in open TRT
...
static void csi_network_state_listener(CONNECTION_STATE state)
{
	...
	if (state == WIFI_DISCONNECTED) {
		if (p_csifw_ctx->csifw_state == CSI_FRAMEWORK_STATE_STARTED) {
			CSIFW_LOGD("Stopping CSI Service due to WiFi disconnect");
			if (stop_csi_framework(p_csifw_ctx) != 1) {
				CSIFW_LOGE("CSI Stop Service failed");
			}
			p_csifw_ctx->csifw_state = CSI_FRAMEWORK_STATE_STARTED_WAITING_FOR_NW;
		}
	} else if (state == WIFI_CONNECTED) {
		if (p_csifw_ctx->csifw_state == CSI_FRAMEWORK_STATE_STARTED_WAITING_FOR_NW) {
			CSIFW_LOGD("Starting service on wifi reconnection");
			if (start_csi_framework(p_csifw_ctx) != 0) {
				CSIFW_LOGE("CSI Start service failed");
			}
			// The state is set to STARTED even if start_csi_framework fails
			// Because csi_network_state_listener is called only when network state changes.
			// If State is set to Stopped or Waiting_NW then Application will never STOP FW properly.
			// After Wifi Connect, if Data is not comming, then Application has to Stop -> Start CSIFW.
			p_csifw_ctx->csifw_state = CSI_FRAMEWORK_STATE_STARTED;
		}
	}
...
static void CSIRawDataListener(CSIFW_RES res, int raw_csi_buff_len, unsigned char *raw_csi_buff, int raw_csi_data_len)
{
	...
	if (p_csifw_ctx->parsed_data_cb_count > 0 && p_csifw_ctx->parsed_data_cb_started_count > 0) {
	...
	for (int i = 0; i < CSIFW_MAX_NUM_APPS; i++) {
		...
		if (p_csifw_ctx->csi_services[i].svc_id != 0 && p_csifw_ctx->csi_services[i].svc_state == CSI_SERVICE_START) {
			if (p_csifw_ctx->csi_services[i].raw_data_cb) {
				p_csifw_ctx->csi_services[i].raw_data_cb(res, raw_csi_buff_len, raw_csi_buff, p_csifw_ctx->csi_services[i].service_data);
			}
			//send parsed
			if (p_csifw_ctx->csi_services[i].parsed_data_cb) {
				p_csifw_ctx->csi_services[i].parsed_data_cb(res, p_csifw_ctx->ParsedDataBufferLen, p_csifw_ctx->parsed_buffptr, p_csifw_ctx->csi_services[i].service_data);
			}
		}
	}
...
CSIFW_RES csifw_registerService(csifw_service_handle *p_hnd, service_callbacks_t *p_svc_cb, csi_config_type_t config_type, unsigned int interval)
{
	...
	if (g_pcsifw_context->csifw_state == CSI_FRAMEWORK_STATE_UNINITIALIZED) {
		if (config_type <= MIN_CSI_CONFIG_TYPE || config_type >= MAX_CSI_CONFIG_TYPE || interval < CSIFW_MIN_INTERVAL_MS) {
			CSIFW_LOGE("Either invalid CSI config type: %d. (Valid config range: %d-%d)", 
				config_type, HT_CSI_DATA, NON_HT_CSI_DATA_ACC1);
...
CSIFW_RES csifw_start(csifw_service_handle hnd)
{
	...
	if (p_csifw_ctx->csifw_state == CSI_FRAMEWORK_STATE_STOPPED || 
		p_csifw_ctx->csifw_state == CSI_FRAMEWORK_STATE_INITIALIZED) {
...
CSIFW_RES csifw_stop(csifw_service_handle hnd)
{
	...
	if (p_csifw_ctx->csifw_state == CSI_FRAMEWORK_STATE_STARTED) {
		stop_csi_framework(p_csifw_ctx);
		CSIFW_LOGD("stop_csi_framework done.!");
	}
	p_csifw_ctx->csifw_state = CSI_FRAMEWORK_STATE_STOPPED;

	CSIFW_LOGI("Service [%d] stopped successfully, remaining services count is %d", 
		p_svc_info->svc_id, p_csifw_ctx->service_count);
...
CSIFW_RES csifw_unregisterService(csifw_service_handle hnd)
{
	...
	//remove service
	remove_service(p_csifw_ctx, p_svc_info);
	
	//free service handle
	if (p_csifw_ctx->service_count == 0 && p_csifw_ctx->csifw_state == CSI_FRAMEWORK_STATE_STOPPED) {
		...
		//destroy context if no more services are available
		destroy_csifw_context();
	}
...
CSIFW_RES csifw_set_interval(csifw_service_handle hnd, unsigned int interval)
{
	...
	if (p_csifw_ctx->csi_interval == interval) {
		CSIFW_LOGI("Interval unchanged (current: %u)", p_csifw_ctx->csi_interval);
		goto on_error;
	}

	p_csifw_ctx->csi_interval = interval;

	if (p_csifw_ctx->csifw_state == CSI_FRAMEWORK_STATE_STOPPED || 
		p_csifw_ctx->csifw_state == CSI_FRAMEWORK_STATE_STARTED_WAITING_FOR_NW) {
		CSIFW_LOGI("Interval updated: %u", p_csifw_ctx->csi_interval);
		goto on_error;
	}

	if (p_csifw_ctx->csi_config == HT_CSI_DATA || 
		p_csifw_ctx->csi_config == HT_CSI_DATA_ACC1) {
		ping_generator_change_interval(p_csifw_ctx->csi_interval);
	} else {
...
CSIFW_RES csifw_get_interval(csifw_service_handle hnd, unsigned int *p_interval)
{
	...
	p_csifw_ctx = g_pcsifw_context;
	*p_interval = p_csifw_ctx->csi_interval;
...
CSIFW_RES csifw_get_config(csifw_service_handle hnd, csi_config_type_t *config_type)
{
	...
	p_csifw_ctx = g_pcsifw_context;
	*config_type = p_csifw_ctx->csi_config;
...
static int get_service_idx(unsigned int id)
{
	...
	csifw_context_t *p_csifw_ctx = g_pcsifw_context;
	for (int i = 0; i < CSIFW_MAX_NUM_APPS; i++) {
		if (p_csifw_ctx->csi_services[i].svc_id == id) {
			return i;
		}
	}
	return -1;
...
static int get_empty_idx(void)
{
	...
	for (int i = 0; i < CSIFW_MAX_NUM_APPS; i++) {
		if (p_csifw_ctx->csi_services[i].svc_id == 0) {
			return i;
		}
	}
	return -1;
...
static CSIFW_RES remove_service(csifw_context_t *p_csifw_ctx, csifw_service_info_t *p_svc_info)
{
	...
	if ((p_csifw_ctx->parsed_buffptr) && (p_csifw_ctx->parsed_data_cb_count <= 0)) {
...
static void nw_state_notify_service(CONNECTION_STATE state)
{
	...
	for (int i = 0; i < CSIFW_MAX_NUM_APPS; i++) {
		//send raw
		if (p_csifw_ctx->csi_services[i].svc_id) {
			if (p_csifw_ctx->csi_services[i].raw_data_cb) {
				p_csifw_ctx->csi_services[i].raw_data_cb(res, 0, NULL, p_csifw_ctx->csi_services[i].service_data);
			} else if (p_csifw_ctx->csi_services[i].parsed_data_cb) {
				p_csifw_ctx->csi_services[i].parsed_data_cb(res, 0, NULL, p_csifw_ctx->csi_services[i].service_data);
			}
		}
	}
}

csifw_context_t *get_csifw_context()
{
	return g_pcsifw_context;
}
...
static CSIFW_RES check_hnd_validity(csifw_service_handle hnd)
{
	...
	if (((uintptr_t)p_svc_info - (uintptr_t)p_csifw_ctx->csi_services) % sizeof(csifw_service_info_t) != 0) {
		CSIFW_LOGE("Invalid handle alignment");
		return CSIFW_INVALID_ARG;
	}

	// Check if pointer is within valid range
	if (p_svc_info < &p_csifw_ctx->csi_services[0] || 
		p_svc_info > &p_csifw_ctx->csi_services[CSIFW_MAX_NUM_APPS-1]) {
		CSIFW_LOGE("Handle out of bounds");
		return CSIFW_INVALID_ARG;
	}

	// Check if service is actually registered
...
static int all_services_stopped(void)
{
	...
	// If there are no services, consider them all stopped
	if (p_csifw_ctx->service_count == 0) {
	...
	for (int i = 0; i < CSIFW_MAX_NUM_APPS; i++) {
		if ((p_csifw_ctx->csi_services[i].svc_id != 0)
			&& (p_csifw_ctx->csi_services[i].svc_state != CSI_SERVICE_STOP)) {
			CSIFW_LOGD("Service %d is still running (state: %d)\n", i, p_csifw_ctx->csi_services[i].svc_state);
			return 0;
		}
	}
