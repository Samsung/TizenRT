...
int iotbus_pwm_set_frequency(iotbus_pwm_context_h pwm, uint32_t frequency)
{
	int fd;
	int ret;
	struct pwm_info_s *info;
...
int iotbus_pwm_set_characteristics(iotbus_pwm_context_h pwm, uint16_t duty_cycle, uint32_t frequency)
{
	int fd;
	int ret;
	struct pwm_info_s *info;
