    ...
    def process_partition_aes_crc(self, aes_type, aes_key):
        ...
        if self.is_data_partition() or (aes_type == 'RANDOM' and self.partition_name != 'bl2' and self.partition_name != 'bl2_B' and self.partition_name != 'bl1_control') or (self.need_add_crc == False):
            ...
            if aes_type == 'FIXED':
                ...
                cmd = f'python3 {aes_tool} encrypt -infile {self.bin_name} -keywords {aes_key}  -aes {128} -outfile {self.aes_bin_name} -startaddress {start_address}'
...
class Partitions:
    ...
    def gen_ota_bin_for_xip(self, ota_aes_en, aes_key, sc):
        ...
        if ota_aes_en:
            ...
            cmd = f'python3 {aes_tool} encrypt -infile {ota_sign_bin} -keywords {aes_key} -aes {128} -outfile {aes_bin_name} -startaddress {start_address}'
