...
def sign(key, public_key_format, align, version, pad_sig, header_size,
    ...
    img.create(key, public_key_format, enckey, dependencies, boot_record,
               ...
               action_type, signature, hash_outfile, pubkeyfile)
    img.save(outfile, hex_addr)

@click.option('-k', '--key', type=str, help='private key file')
@click.option('-d', '--hash', type=str, help='hash value')
@click.option('-o', '--outfile', type=str, help='output sig json file')
@click.command(help="mcuboot sign hash")
def sign_hash(key, hash, outfile):
    key = load_key(key) if key else None
    #key.pad_sig = True

    print(f'mcuboot sign_hash hash={hash}')
    sig = key.sign(bytes.fromhex(hash))
    hex_sig = ''.join(format(byte, '02x') for byte in sig)

    signature = {"signature":""}
    signature['signature'] = hex_sig
    print(f'sig={hex_sig}')
...
@click.option('-k', '--key', type=str, help='private key file')
@click.option('-d', '--digest', type=str, help='digest json file')
@click.option('-o', '--sigfile', type=str, help='output sig json file')
@click.command(help="ECDSA sign")
def ecdsa_sign(key, digest, sigfile='sign.json'):
    key = load_key(key) if key else None
    #key.pad_sig = True
    ...
    if os.path.exists(sigfile):
        ...
        logging.error(f'{sigfile} not exists')
        exit(1)

    app_digest = digest['app_digest']
    ota_digest = digest['ota_digest']

    print(f'ecdsa_sign app_digest={app_digest}, ota_digest={ota_digest}')
    if (app_digest != ''):
        print(f'sign app_digest')
        sig = key.sign(bytes.fromhex(app_digest))
        hex_sig = ''.join(format(byte, '02x') for byte in sig)
        data['bl2_sig_app'] = hex_sig
        print(f'sig={hex_sig}')
    else:
        print(f'app_digest empty, not sign')

    if (ota_digest != ''):
        print(f'sign ota_digest')
        sig = key.sign(bytes.fromhex(ota_digest))
        hex_sig = ''.join(format(byte, '02x') for byte in sig)
        data['bl2_sig_ota'] = hex_sig
        print(f'sig={hex_sig}')
    else:
        print(f'ota_digest empty, not sig')
...
imgtool.add_command(sign_hash)
imgtool.add_command(ecdsa_sign)
