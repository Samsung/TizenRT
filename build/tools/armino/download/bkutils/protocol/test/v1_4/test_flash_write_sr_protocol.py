from bkutils.protocol.bootrom.v2_0.v1_4 import FlashWriteSRProtocol
import unittest


class TestFlashWriteSRProtocol(unittest.TestCase):

    def setUp(self):
        self.protocol = FlashWriteSRProtocol()

    def test_cmd(self):
        REG_ADDR = 0x12
        REG_VALUE = [0x12, 0x34]
        cmd = self.protocol.cmd(REG_ADDR, REG_VALUE)
        Scmd = 0x0d
    ...
    def test_expect_length(self):
        sr_size = 4
        expected_length = 16    # 11 + 1 + sr_size
        self.assertEqual(self.protocol.expect_length(sr_size), expected_length, "expect_length did not return the expected length")
