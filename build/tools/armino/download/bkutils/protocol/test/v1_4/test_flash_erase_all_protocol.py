from bkutils.protocol.bootrom.v2_0.v1_4 import FlashEraseAllProtocol
import unittest


class TestFlashEraseAllProtocol(unittest.TestCase):

    def setUp(self):
        self.protocol = FlashEraseAllProtocol()

    def test_cmd(self):
        timeout_sec = 10
        cmd = self.protocol.cmd(timeout_sec)
        Scmd_byte = 0x0A
        payload = bytes([timeout_sec])
