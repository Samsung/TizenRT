from bkutils.protocol.bootrom.v2_0 import FlashCustomWriteExtProtocol
import unittest


class TestFlashCustomWriteExtProtocol(unittest.TestCase):

    def setUp(self):
        self.protocol = FlashCustomWriteExtProtocol()

    def test_cmd(self):
        ADDR = 0x12345678
        DATA = [0x12, 0x34, 0x56, 0x78]
        cmd = self.protocol.cmd(ADDR, DATA)
        Scmd = 0xE6
        ACK = 1
    ...
    def test_response_check(self):
        ...
        status = 0x0
        ADDR = 0x12345678
        payload = bytes([ADDR&0xff, (ADDR>>8)&0xff, (ADDR>>16)&0xff, (ADDR>>24)&0xff, W_size&0xff])
