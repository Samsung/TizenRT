...
class ChipEnterCbusModeProtocol(BaseBootRomProtocol):
    ...
    def cmd(self, part_addr:list, part_size:list, flash_addr:list):
        ...
        payload = []
        payload.extend(data)
        return self.command_generate(0x11, payload)
    ...
    def response_check(self, response_content):
        return super().response_check(response_content) and self.get_response_cmd(response_content)==bytes([0x11]) and self.get_response_payload(response_content)==bytes([0x00])
