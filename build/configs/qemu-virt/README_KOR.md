...
- `full_dramboot_xipelf`에서는 커널은 DRAM에서 실행되고 `common`, `app1`은 flash에서 XIP 형태로 실행
...
|------|-----------|-----|----------------|------|
| `flat_dramboot` | Flat (`CONFIG_BUILD_FLAT=y`) | Single Core | - | 커널과 앱이 하나의 이미지로 링크되며 `bl1/kernel/bootparam` 조합으로 내려받음 |
| `full_dramboot_xipelf` | Protected (`CONFIG_BUILD_PROTECTED=y`) | Single Core | XIP-ELF (`CONFIG_XIP_ELF=y`) | 커널은 DRAM에서 실행되고 `common`, `app1`은 flash에서 실행되며 `bl1/kernel/common/app1/bootparam` 조합으로 내려받음 |
...
`flat_dramboot`를 빌드하려면 다음을 사용합니다.
...
`flat_dramboot`에서는 다음 layout으로 내려받습니다.
...
GitHub Actions 워크플로 `qemu-virt-runtime-tests.yml`은 다음을 자동 수행합니다.
...
로컬 Docker 검증도 `full_dramboot_xipelf`, `flat_dramboot` 두 구성 모두 완료했습니다.

## 6. 알려진 제한 사항

- 현재 자동화 범위는 `virtio-blk`까지만 포함
- `virtio-net` 및 네트워크 검증은 제외
- CI 검증 대상은 `flat_dramboot`, `full_dramboot_xipelf` 두 구성만 포함
