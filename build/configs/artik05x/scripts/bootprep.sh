...
sizes=`grep -A 2 'config FLASH_PART_SIZE' ${PARTITION_KCONFIG} | sed -n 's/\tdefault "\(.*\)".*/\1/p'`
partition_sizes=${CONFIG_FLASH_PART_SIZE:=${sizes}}
...
PARTITION_DECLARATION="typedef struct __attribute__((packed))
{
	...
	uint8_t type[8];	/* Partition type - smartfs, none, ftl etc */
...
echo "$PARTITION_TYPEDEF partitions[$num_partitions+1]={"	>> ${CFILE}
...
do
	...
	echo -e "\t{\"$part\",\"$parttype\",\"\",$addr,$size,0}," >>  ${CFILE}
	offset=$size
done
# END of Partition table marker
	echo -e "\t{\"000\",\"none\",\"\",0,0,0}" >>  ${CFILE}
echo "};" >> ${CFILE}

echo -e "\nSTEP1/3: Successfully created ${CFILE}\n"


echo -e "\n Create a dummy linker script\n"

DUMMYSCRIPT=dummy.ld
SECTIONS="
SECTIONS
{
  . = 0;
...
ELF=${FILENAME}.elf
	...
	${LD} ${LDOPTION} ${DUMMYSCRIPT} -o ${ELF} ${OFILE}
