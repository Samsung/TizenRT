...
partsize_list_default=`grep -A 2 'config FLASH_PART_LIST' ${BOARD_KCONFIG} | sed -n 's/\tdefault "\(.*\)".*/\1/p'`
partsize_list=${CONFIG_FLASH_PART_LIST:=${partsize_list_default}}
partname_list_default=`grep -A 2 'config FLASH_PART_NAME' ${BOARD_KCONFIG} | sed -n 's/\tdefault "\(.*\)".*/\1/p'`
