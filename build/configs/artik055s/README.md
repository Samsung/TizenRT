...
This is the configuration for the production phase of ARTIK055S product. When you download it to the board, the `artik_onboarding` app will start automatically when it boots.
```bash
$ cd os/tools
$ ./configure.sh artik055s/onboard
```
...
```bash
$ cd os/tools
$ ./configure.sh artik055s/minimal
```
> * OS kernel
...
```bash
$ cd os/tools
$ ./configure.sh artik055s/typical
```
> * OS kernel
> * Customized C Library
> * Security(TLS)
...
```bash
$ cd build/output/bin
$ gzip -c tinyara_head.bin > ../../configs/artik055s/bin/factory.bin
```
```bash
$ cd os
