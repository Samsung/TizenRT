...
{
	port=$TTYDEV
	python3 ${SCRIPT_PATH} pre_download -p $port -b $WORKER_BAUD -d $DEBUG --link_type $WORK_SPACE
}
...
function board_download()
{
	...
	python3 ${SCRIPT_PATH} download -p $port -i "${OUTPUT_PATH}/${bin}@tizen_separator@${start_addr}-${size}" --partition $partition
}
...
function board_erase()
{
	...
	python3 ${SCRIPT_PATH} erase -p $port -f "$start_addr-$size" --partition $partition
}
...
function post_download()
{
	...
	if [ $POST_REBOOT -eq 1 ]; then
		python3 ${SCRIPT_PATH} post_download -p $port -r
	else
		python3 ${SCRIPT_PATH} post_download -p $port
	fi
}
