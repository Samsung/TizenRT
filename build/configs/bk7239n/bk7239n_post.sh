...
if [ "${CONFIG_BINARY_SIGNING}" = "y" ]; then
    if [ ! -f "${BUILDDIR}/tools/armino/beken_utils/deployment_main.sh" ]; then
        python3 -B ${BUILDDIR}/configs/bk7239n/copy_build_files.py ${TOPDIR} ${BUILDDIR}/output/bin ${BUILDDIR}/output/bin/install_for_deployment
    else
        python3 -B ${BUILDDIR}/configs/bk7239n/copy_build_files.py ${TOPDIR} ${BUILDDIR}/output/bin ${BUILDDIR}/tools/armino/beken_utils/input_dir
