...
OMStatus onert_micro::import::configure_kernel_CircleConv2DWeightGrad(const OMConfigureArgs &config_args)
{
  ...
  const circle::Tensor *input_activation = runtime_kernel.inputs[inputActivationTensorIdx];
  ...
  if (input_grad->type() == circle::TensorType_FLOAT32 && input_activation->type() != circle::TensorType_FLOAT32)
  {
  ...
  status = utils::checkCondition(input_grad_shape.dimensionsCount() == input_activation_shape.dimensionsCount());
  if (status != Ok)
    return status;
