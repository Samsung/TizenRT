...
static int ndef_parse_record(const u8 *data, u32 size, struct ndef_record *record)
{
	const u8 *pos = data + 1;
...
static struct wpabuf *ndef_parse_records(const struct wpabuf *buf, int (*filter)(struct ndef_record *))
{
...
static struct wpabuf *ndef_build_record(u8 flags, const void *type, u8 type_length, void *id, u8 id_length, const struct wpabuf *payload)
{
	...
	if (record == NULL) {
		wpa_printf(MSG_ERROR, "NDEF : Failed to allocate " "record for build");
		return NULL;
	}
...
{
	if (record->type == NULL || record->type_length != os_strlen(wifi_handover_type)) {
		return 0;
	}
	if (os_memcmp(record->type, wifi_handover_type, os_strlen(wifi_handover_type)) != 0) {
		return 0;
	}
	return 1;
}
...
{
	return ndef_build_record(FLAG_MESSAGE_BEGIN | FLAG_MESSAGE_END | FLAG_TNF_RFC2046, wifi_handover_type, os_strlen(wifi_handover_type), NULL, 0, buf);
}

static int p2p_filter(struct ndef_record *record)
{
	if (record->type == NULL || record->type_length != os_strlen(p2p_handover_type)) {
		return 0;
	}
	if (os_memcmp(record->type, p2p_handover_type, os_strlen(p2p_handover_type)) != 0) {
		return 0;
	}
	return 1;
}
...
{
	return ndef_build_record(FLAG_MESSAGE_BEGIN | FLAG_MESSAGE_END | FLAG_TNF_RFC2046, p2p_handover_type, os_strlen(p2p_handover_type), NULL, 0, buf);
}
