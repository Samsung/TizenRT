...
static void http_client_got_response(struct httpread *handle, void *cookie, enum httpread_event e)
{
	...
	wpa_printf(MSG_DEBUG, "HTTP: httpread callback: handle=%p cookie=%p " "e=%d", handle, cookie, e);
	...
	case HTTPREAD_EVENT_FILE_READY:
		if (httpread_hdr_type_get(c->hread) == HTTPREAD_HDR_TYPE_REPLY) {
			...
			if (reply_code == 200 /* OK */) {
				wpa_printf(MSG_DEBUG, "HTTP: Response OK from " "%s:%d", inet_ntoa(c->dst.sin_addr), ntohs(c->dst.sin_port));
				c->cb(c->cb_ctx, c, HTTP_CLIENT_OK);
			} else {
				wpa_printf(MSG_DEBUG, "HTTP: Error %d from " "%s:%d", reply_code, inet_ntoa(c->dst.sin_addr), ntohs(c->dst.sin_port));
...
static void http_client_tx_ready(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	wpa_printf(MSG_DEBUG, "HTTP: Send client request to %s:%d (%lu of %lu " "bytes remaining)", inet_ntoa(c->dst.sin_addr), ntohs(c->dst.sin_port), (unsigned long)wpabuf_len(c->req), (unsigned long)wpabuf_len(c->req) - c->req_pos);

	res = send(c->sd, wpabuf_head_u8(c->req) + c->req_pos, wpabuf_len(c->req) - c->req_pos, 0);
	if (res < 0) {
	...
	if ((size_t)res < wpabuf_len(c->req) - c->req_pos) {
		wpa_printf(MSG_DEBUG, "HTTP: Sent %d of %lu bytes; %lu bytes " "remaining", res, (unsigned long)wpabuf_len(c->req), (unsigned long)wpabuf_len(c->req) - c->req_pos - res);
		c->req_pos += res;
		return;
	}

	wpa_printf(MSG_DEBUG, "HTTP: Full client request sent to %s:%d", inet_ntoa(c->dst.sin_addr), ntohs(c->dst.sin_port));
	eloop_unregister_sock(c->sd, EVENT_TYPE_WRITE);
	wpabuf_free(c->req);
	c->req = NULL;

	c->hread = httpread_create(c->sd, http_client_got_response, c, c->max_response, HTTP_CLIENT_TIMEOUT_SEC);
	if (c->hread == NULL) {
		c->cb(c->cb_ctx, c, HTTP_CLIENT_FAILED);
		return;
	}
...
struct http_client *http_client_addr(struct sockaddr_in *dst, struct wpabuf *req, size_t max_response, void (*cb)(void *ctx, struct http_client *c, enum http_client_event event), void *cb_ctx)
{
	...
	if (fcntl(c->sd, F_SETFL, O_NONBLOCK) != 0) {
		wpa_printf(MSG_DEBUG, "HTTP: fnctl(O_NONBLOCK) failed: %s", strerror(errno));
		http_client_free(c);
		return NULL;
	}
	...
	if (eloop_register_sock(c->sd, EVENT_TYPE_WRITE, http_client_tx_ready, c, NULL)) {
		http_client_free(c);
		return NULL;
	}

	if (eloop_register_timeout(HTTP_CLIENT_TIMEOUT_SEC, 0, http_client_timeout, c, NULL)) {
		http_client_free(c);
		return NULL;
	}
...
char *http_client_url_parse(const char *url, struct sockaddr_in *dst, char **ret_path)
{
	...
	if (inet_aton(addr, &dst->sin_addr) == 0) {
		/* TODO: name lookup */
		wpa_printf(MSG_DEBUG, "HTTP: Unsupported address in URL '%s' " "(addr='%s' port='%s')", url, addr, port);
		os_free(u);
		return NULL;
	}
...
struct wpabuf *http_client_get_body(struct http_client *c)
{
	...
	wpabuf_set(&c->body, httpread_data_get(c->hread), httpread_length_get(c->hread));
	return &c->body;
}
