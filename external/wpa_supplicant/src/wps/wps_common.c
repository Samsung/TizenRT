...
int wps_derive_keys(struct wps_data *wps)
{
	...
	wpa_hexdump_key(MSG_DEBUG, "WPS: KeyWrapKey", wps->keywrapkey, WPS_KEYWRAPKEY_LEN);
...
void wps_derive_psk(struct wps_data *wps, const u8 *dev_passwd, size_t dev_passwd_len)
{
	...
	hmac_sha256(wps->authkey, WPS_AUTHKEY_LEN, dev_passwd, (dev_passwd_len + 1) / 2, hash);
	os_memcpy(wps->psk1, hash, WPS_PSK_LEN);
	hmac_sha256(wps->authkey, WPS_AUTHKEY_LEN, dev_passwd + (dev_passwd_len + 1) / 2, dev_passwd_len / 2, hash);
	os_memcpy(wps->psk2, hash, WPS_PSK_LEN);

	wpa_hexdump_ascii_key(MSG_DEBUG, "WPS: Device Password", dev_passwd, dev_passwd_len);
...
struct wpabuf *wps_decrypt_encr_settings(struct wps_data *wps, const u8 *encr, size_t encr_len)
{
	...
	if (aes_128_cbc_decrypt(wps->keywrapkey, encr, wpabuf_mhead(decrypted), wpabuf_len(decrypted))) {
		wpabuf_free(decrypted);
		return NULL;
	}

	wpa_hexdump_buf_key(MSG_MSGDUMP, "WPS: Decrypted Encrypted Settings", decrypted);
	...
	for (i = 0; i < pad; i++) {
		if (*pos-- != pad) {
			wpa_printf(MSG_DEBUG, "WPS: Invalid PKCS#5 v2.0 pad " "string");
			wpabuf_free(decrypted);
			return NULL;
		}
	}
...
struct wpabuf *wps_get_oob_cred(struct wps_context *wps, int rf_band, int channel)
{
	...
	if (plain == NULL) {
		wpa_printf(MSG_ERROR, "WPS: Failed to allocate memory for OOB " "credential");
		return NULL;
	}
	...
	if (wps_build_cred(&data, plain) || (rf_band && wps_build_rf_bands_attr(plain, rf_band)) || (channel && wps_build_ap_channel(plain, channel)) || wps_build_mac_addr(plain, wps->dev.mac_addr) || wps_build_wfa_ext(plain, 0, NULL, 0)) {
	...
	if (wps->wps_state == WPS_STATE_NOT_CONFIGURED && data.new_psk && wps->ap) {
		...
		wpa_printf(MSG_DEBUG, "WPS: Moving to Configured state based " "on credential token generation");
		...
		wpa_hexdump_ascii_key(MSG_DEBUG, "WPS: Generated random passphrase", data.new_psk, data.new_psk_len);
		if (wps->cred_cb) {
...
struct wpabuf *wps_build_nfc_pw_token(u16 dev_pw_id, const struct wpabuf *pubkey, const struct wpabuf *dev_pw)
{
	...
	if (wps_build_oob_dev_pw(data, dev_pw_id, pubkey, wpabuf_head(dev_pw), wpabuf_len(dev_pw)) || wps_build_wfa_ext(data, 0, NULL, 0)) {
		wpa_printf(MSG_ERROR, "WPS: Failed to build NFC password " "token");
		wpabuf_free(data);
		return NULL;
	}
...
int wps_oob_use_cred(struct wps_context *wps, struct wps_parse_attr *attr)
{
	...
	for (i = 0; i < attr->num_cred; i++) {
		...
		if (wps_parse_msg(&msg, &cattr) < 0 || wps_process_cred(&cattr, &local_cred)) {
			wpa_printf(MSG_ERROR, "WPS: Failed to parse OOB " "credential");
			return -1;
		}
...
char *wps_dev_type_bin2str(const u8 dev_type[WPS_DEV_TYPE_LEN], char *buf, size_t buf_len)
{
	...
	ret = os_snprintf(buf, buf_len, "%u-%08X-%u", WPA_GET_BE16(dev_type), WPA_GET_BE32(&dev_type[2]), WPA_GET_BE16(&dev_type[6]));
...
struct wpabuf *wps_build_wsc_ack(struct wps_data *wps)
{
	...
	if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_WSC_ACK) || wps_build_enrollee_nonce(wps, msg) || wps_build_registrar_nonce(wps, msg) || wps_build_wfa_ext(msg, 0, NULL, 0)) {
		wpabuf_free(msg);
		return NULL;
	}
...
struct wpabuf *wps_build_wsc_nack(struct wps_data *wps)
{
	...
	if (wps_build_version(msg) || wps_build_msg_type(msg, WPS_WSC_NACK) || wps_build_enrollee_nonce(wps, msg) || wps_build_registrar_nonce(wps, msg) || wps_build_config_error(msg, wps->config_error) || wps_build_wfa_ext(msg, 0, NULL, 0)) {
		wpabuf_free(msg);
		return NULL;
	}
...
struct wpabuf *wps_nfc_token_gen(int ndef, int *id, struct wpabuf **pubkey, struct wpabuf **privkey, struct wpabuf **dev_pw)
{
	...
	if (random_get_bytes(wpabuf_put(pw, WPS_OOB_DEVICE_PASSWORD_LEN), WPS_OOB_DEVICE_PASSWORD_LEN) || random_get_bytes((u8 *)&val, sizeof(val))) {
		wpabuf_free(pw);
		return NULL;
	}
...
struct wpabuf *wps_build_nfc_handover_req(struct wps_context *ctx, struct wpabuf *nfc_dh_pubkey)
{
	...
	wpa_printf(MSG_DEBUG, "WPS: Building attributes for NFC connection " "handover request");

	if (nfc_dh_pubkey == NULL) {
		wpa_printf(MSG_DEBUG, "WPS: No NFC OOB Device Password " "configured");
		return NULL;
	}
	...
	if (wps_build_oob_dev_pw(msg, DEV_PW_NFC_CONNECTION_HANDOVER, nfc_dh_pubkey, NULL, 0) || wps_build_uuid_e(msg, ctx->uuid) || wps_build_wfa_ext(msg, 0, NULL, 0)) {
		wpabuf_free(msg);
		return NULL;
	}
...
static int wps_build_ssid(struct wpabuf *msg, struct wps_context *wps)
{
	...
	wpa_hexdump_ascii(MSG_DEBUG, "WPS: SSID in Connection Handover Select", wps->ssid, wps->ssid_len);
...
static int wps_build_ap_freq(struct wpabuf *msg, int freq)
{
	...
	if (wps_build_rf_bands_attr(msg, rf_band) || wps_build_ap_channel(msg, ap_channel)) {
		return -1;
	}
...
struct wpabuf *wps_build_nfc_handover_sel(struct wps_context *ctx, struct wpabuf *nfc_dh_pubkey, const u8 *bssid, int freq)
{
	...
	wpa_printf(MSG_DEBUG, "WPS: Building attributes for NFC connection " "handover select");
	...
	if (wps_build_oob_dev_pw(msg, DEV_PW_NFC_CONNECTION_HANDOVER, nfc_dh_pubkey, NULL, 0) || wps_build_ssid(msg, ctx) || wps_build_ap_freq(msg, freq) || (bssid && wps_build_mac_addr(msg, bssid)) || wps_build_wfa_ext(msg, 0, NULL, 0)) {
		wpabuf_free(msg);
		return NULL;
	}
...
struct wpabuf *wps_build_nfc_handover_req_p2p(struct wps_context *ctx, struct wpabuf *nfc_dh_pubkey)
{
	...
	wpa_printf(MSG_DEBUG, "WPS: Building attributes for NFC connection " "handover request (P2P)");
	...
	if (wps_build_manufacturer(&ctx->dev, msg) || wps_build_model_name(&ctx->dev, msg) || wps_build_model_number(&ctx->dev, msg) || wps_build_oob_dev_pw(msg, DEV_PW_NFC_CONNECTION_HANDOVER, nfc_dh_pubkey, NULL, 0) || wps_build_rf_bands(&ctx->dev, msg, 0) || wps_build_serial_number(&ctx->dev, msg) || wps_build_uuid_e(msg, ctx->uuid) || wps_build_wfa_ext(msg, 0, NULL, 0)) {
		wpabuf_free(msg);
		return NULL;
	}
...
struct wpabuf *wps_build_nfc_handover_sel_p2p(struct wps_context *ctx, int nfc_dev_pw_id, struct wpabuf *nfc_dh_pubkey, struct wpabuf *nfc_dev_pw)
{
	...
	wpa_printf(MSG_DEBUG, "WPS: Building attributes for NFC connection " "handover select (P2P)");

	if (nfc_dh_pubkey == NULL || (nfc_dev_pw_id != DEV_PW_NFC_CONNECTION_HANDOVER && nfc_dev_pw == NULL)) {
		wpa_printf(MSG_DEBUG, "WPS: No NFC OOB Device Password " "configured");
		return NULL;
	}
	...
	if (wps_build_manufacturer(&ctx->dev, msg) || wps_build_model_name(&ctx->dev, msg) || wps_build_model_number(&ctx->dev, msg) || wps_build_oob_dev_pw(msg, nfc_dev_pw_id, nfc_dh_pubkey, dev_pw, dev_pw_len) || wps_build_rf_bands(&ctx->dev, msg, 0) || wps_build_serial_number(&ctx->dev, msg) || wps_build_uuid_e(msg, ctx->uuid) || wps_build_wfa_ext(msg, 0, NULL, 0)) {
		wpabuf_free(msg);
		return NULL;
	}
