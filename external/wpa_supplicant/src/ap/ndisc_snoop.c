...
static int sta_has_ip6addr(struct sta_info *sta, struct in6_addr *addr)
{
	...
	dl_list_for_each(ip6addr, &sta->ip6addr, struct ip6addr, list) {
		if (ip6addr->addr.s6_addr32[0] == addr->s6_addr32[0] && ip6addr->addr.s6_addr32[1] == addr->s6_addr32[1] && ip6addr->addr.s6_addr32[2] == addr->s6_addr32[2] && ip6addr->addr.s6_addr32[3] == addr->s6_addr32[3]) {
			return 1;
		}
	}
...
static void handle_ndisc(void *ctx, const u8 *src_addr, const u8 *buf, size_t len)
{
	...
	case NEIGHBOR_SOLICITATION:
		...
		if (!(saddr->s6_addr32[0] == 0 && saddr->s6_addr32[1] == 0 && saddr->s6_addr32[2] == 0 && saddr->s6_addr32[3] == 0)) {
			...
			wpa_printf(MSG_DEBUG, "ndisc_snoop: Learned new IPv6 address %s for " MACSTR, addrtxt, MAC2STR(sta->addr));
...
{
	hapd->sock_ndisc = x_snoop_get_l2_packet(hapd, handle_ndisc, L2_PACKET_FILTER_NDISC);
	if (hapd->sock_ndisc == NULL) {
		wpa_printf(MSG_DEBUG, "ndisc_snoop: Failed to initialize L2 packet processing for NDISC packets: %s", strerror(errno));
		return -1;
	}
