...
int hostapd_build_ap_extra_ies(struct hostapd_data *hapd, struct wpabuf **beacon_ret, struct wpabuf **proberesp_ret, struct wpabuf **assocresp_ret)
{
	...
	*beacon_ret = *proberesp_ret = *assocresp_ret = NULL;
	...
	if (hapd->wps_beacon_ie) {
		if (wpabuf_resize(&beacon, wpabuf_len(hapd->wps_beacon_ie)) < 0) {
			goto fail;
		}
	...
	if (hapd->wps_probe_resp_ie) {
		if (wpabuf_resize(&proberesp, wpabuf_len(hapd->wps_probe_resp_ie)) < 0) {
			goto fail;
		}
	...
	if (hapd->p2p_beacon_ie) {
		if (wpabuf_resize(&beacon, wpabuf_len(hapd->p2p_beacon_ie)) < 0) {
			goto fail;
		}
	...
	if (hapd->p2p_probe_resp_ie) {
		if (wpabuf_resize(&proberesp, wpabuf_len(hapd->p2p_probe_resp_ie)) < 0) {
			goto fail;
		}
...
int hostapd_set_ap_wps_ie(struct hostapd_data *hapd)
{
	...
	ret = hapd->driver->set_ap_wps_ie(hapd->drv_priv, beacon, proberesp, assocresp);
...
{
	if (authorized) {
		return hostapd_sta_set_flags(hapd, sta->addr, hostapd_sta_flags_to_drv(sta->flags), WPA_STA_AUTHORIZED, ~0);
	}

	return hostapd_sta_set_flags(hapd, sta->addr, hostapd_sta_flags_to_drv(sta->flags), 0, ~WPA_STA_AUTHORIZED);
}
...
int hostapd_set_sta_flags(struct hostapd_data *hapd, struct sta_info *sta)
{
	...
	if (((!hapd->conf->ieee802_1x && !hapd->conf->wpa) || sta->auth_alg == WLAN_AUTH_FT) && sta->flags & WLAN_STA_AUTHORIZED) {
...
int hostapd_vlan_if_add(struct hostapd_data *hapd, const char *ifname)
{
	...
	u8 if_addr[ETH_ALEN];
	return hostapd_if_add(hapd, WPA_IF_AP_VLAN, ifname, hapd->own_addr, NULL, NULL, force_ifname, if_addr, NULL, 0);
}
...
int hostapd_set_wds_sta(struct hostapd_data *hapd, char *ifname_wds, const u8 *addr, int aid, int val)
{
	...
	return hapd->driver->set_wds_sta(hapd->drv_priv, addr, aid, val, bridge, ifname_wds);
}
...
int hostapd_sta_auth(struct hostapd_data *hapd, const u8 *addr, u16 seq, u16 status, const u8 *ie, size_t len)
{
	...
	return hapd->driver->sta_auth(hapd->drv_priv, hapd->own_addr, addr, seq, status, ie, len);
}
...
int hostapd_if_add(struct hostapd_data *hapd, enum wpa_driver_if_type type, const char *ifname, const u8 *addr, void *bss_ctx, void **drv_priv, char *force_ifname, u8 *if_addr, const char *bridge, int use_existing)
{
	...
	return hapd->driver->if_add(hapd->drv_priv, type, ifname, addr, bss_ctx, drv_priv, force_ifname, if_addr, bridge, use_existing);
}

int hostapd_if_remove(struct hostapd_data *hapd, enum wpa_driver_if_type type, const char *ifname)
{
	if (hapd->driver == NULL || hapd->drv_priv == NULL || hapd->driver->if_remove == NULL) {
		return -1;
	}
...
int hostapd_set_freq(struct hostapd_data *hapd, enum hostapd_hw_mode mode, int freq, int channel, int ht_enabled, int vht_enabled, int sec_channel_offset, int vht_oper_chwidth, int center_segment0, int center_segment1)
{
	...
	if (hostapd_set_freq_params(&data, mode, freq, channel, ht_enabled, vht_enabled, sec_channel_offset, vht_oper_chwidth, center_segment0, center_segment1, hapd->iface->current_mode ? hapd->iface->current_mode->vht_capab : 0)) {
		return -1;
	}
...
int hostapd_set_tx_queue_params(struct hostapd_data *hapd, int queue, int aifs, int cw_min, int cw_max, int burst_time)
{
	...
	return hapd->driver->set_tx_queue_params(hapd->drv_priv, queue, aifs, cw_min, cw_max, burst_time);
}
...
int hostapd_drv_set_key(const char *ifname, struct hostapd_data *hapd, enum wpa_alg alg, const u8 *addr, int key_idx, int set_tx, const u8 *seq, size_t seq_len, const u8 *key, size_t key_len)
{
	...
	return hapd->driver->set_key(ifname, hapd->drv_priv, alg, addr, key_idx, set_tx, seq, seq_len, key, key_len);
}
...
int hostapd_drv_sta_disassoc(struct hostapd_data *hapd, const u8 *addr, int reason)
{
	...
	return hapd->driver->sta_disassoc(hapd->drv_priv, hapd->own_addr, addr, reason);
}
...
int hostapd_drv_send_action(struct hostapd_data *hapd, unsigned int freq, unsigned int wait, const u8 *dst, const u8 *data, size_t len)
{
	...
	return hapd->driver->send_action(hapd->drv_priv, freq, wait, dst, hapd->own_addr, hapd->own_addr, data, len, 0);
}
...
int hostapd_start_dfs_cac(struct hostapd_iface *iface, enum hostapd_hw_mode mode, int freq, int channel, int ht_enabled, int vht_enabled, int sec_channel_offset, int vht_oper_chwidth, int center_segment0, int center_segment1)
{
	...
	if (!iface->conf->ieee80211h) {
		wpa_printf(MSG_ERROR, "Can't start DFS CAC, DFS functionality " "is not enabled");
		return -1;
	}

	if (hostapd_set_freq_params(&data, mode, freq, channel, ht_enabled, vht_enabled, sec_channel_offset, vht_oper_chwidth, center_segment0, center_segment1, iface->current_mode->vht_capab)) {
...
int hostapd_drv_do_acs(struct hostapd_data *hapd)
{
	...
	for (i = 0; i < mode->num_channels; i++) {
		...
		if (!acs_ch_list_all && !freq_range_list_includes(&hapd->iface->conf->acs_ch_list, chan->chan)) {
			continue;
		}
	...
	params.ht40_enabled = ! !(hapd->iface->conf->ht_capab & HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET);
	...
	if (hapd->iface->conf->ieee80211ac && params.ht40_enabled) {
		if (hapd->iface->conf->vht_oper_chwidth == VHT_CHANWIDTH_80MHZ) {
			params.ch_width = 80;
		} else if (hapd->iface->conf->vht_oper_chwidth == VHT_CHANWIDTH_160MHZ || hapd->iface->conf->vht_oper_chwidth == VHT_CHANWIDTH_80P80MHZ) {
			params.ch_width = 160;
		}
	}
