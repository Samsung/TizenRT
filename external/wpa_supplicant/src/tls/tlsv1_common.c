...
int tls_parse_cert(const u8 *buf, size_t len, struct crypto_public_key **pk)
{
	...
	*pk = crypto_public_key_from_cert(buf, len);
	...
	if (cert == NULL) {
		wpa_printf(MSG_DEBUG, "TLSv1: Failed to parse X.509 " "certificate");
		return -1;
	}
	...
	*pk = crypto_public_key_import(cert->public_key, cert->public_key_len);
	...
	if (*pk == NULL) {
		wpa_printf(MSG_ERROR, "TLSv1: Failed to import " "server public key");
		return -1;
	}
...
int tls_verify_hash_init(struct tls_verify_hash *verify)
{
	...
	if (verify->md5_client == NULL || verify->md5_server == NULL || verify->md5_cert == NULL || verify->sha1_client == NULL || verify->sha1_server == NULL || verify->sha1_cert == NULL) {
		tls_verify_hash_free(verify);
		return -1;
	}
#ifdef CONFIG_TLSV12
	...
	if (verify->sha256_client == NULL || verify->sha256_server == NULL || verify->sha256_cert == NULL) {
		tls_verify_hash_free(verify);
		return -1;
	}
#endif							/* CONFIG_TLSV12 */
...
int tls_prf(u16 ver, const u8 *secret, size_t secret_len, const char *label, const u8 *seed, size_t seed_len, u8 *out, size_t outlen)
{
	...
	return tls_prf_sha1_md5(secret, secret_len, label, seed, seed_len, out, outlen);
}
...
int tls_verify_signature(u16 tls_version, struct crypto_public_key *pk, const u8 *data, size_t data_len, const u8 *pos, size_t len, u8 *alert)
{
	...
	if (end - pos > slen) {
		wpa_hexdump(MSG_MSGDUMP, "Additional data after Signature", pos + slen, end - pos - slen);
		end = pos + slen;
	}
	...
	if (tls_version >= TLS_VERSION_1_2) {
		 ...
		 * H
		 */
		if (buflen >= 19 + 32 && os_memcmp(buf, "\x30\x31\x30\x0d\x06\x09\x60\x86\x48\x01" "\x65\x03\x04\x02\x01\x05\x00\x04\x20", 19) == 0) {
