...
static void l2_packet_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	wpa_printf(MSG_DEBUG, "%s: src=" MACSTR " len=%d", __func__, MAC2STR(ll.sll_addr), (int)res);
	...
	if (l2->fd_br_rx >= 0) {
		 ...
		 */
		if (l2->num_rx_br <= 1) {
			wpa_printf(MSG_DEBUG, "l2_packet_receive: Main packet socket for %s seems to have working RX - close workaround bridge socket", l2->ifname);
...
static void l2_packet_receive_br(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	res = recvfrom(sock, buf, sizeof(buf), 0, (struct sockaddr *)&ll, &fromlen);
	if (res < 0) {
		wpa_printf(MSG_DEBUG, "l2_packet_receive_br - recvfrom: %s", strerror(errno));
		return;
	}
...
struct l2_packet_data *l2_packet_init_bridge(const char *br_ifname, const char *ifname, const u8 *own_addr, unsigned short protocol, void (*rx_callback)(void *ctx, const u8 *src_addr, const u8 *buf, size_t len), void *rx_callback_ctx, int l2_hdr)
{
	...
	if (l2->fd_br_rx < 0) {
		wpa_printf(MSG_DEBUG, "%s: socket(PF_PACKET-fd_br_rx): %s", __func__, strerror(errno));
	...
	if (bind(l2->fd_br_rx, (struct sockaddr *)&ll, sizeof(ll)) < 0) {
		wpa_printf(MSG_DEBUG, "%s: bind[PF_PACKET-fd_br_rx]: %s", __func__, strerror(errno));
		/* try to continue without the workaround RX socket */
		close(l2->fd_br_rx);
		l2->fd_br_rx = -1;
		return l2;
	...
	if (setsockopt(l2->fd_br_rx, SOL_SOCKET, SO_ATTACH_FILTER, &ethertype_sock_filter, sizeof(struct sock_fprog))) {
		wpa_printf(MSG_DEBUG, "l2_packet_linux: setsockopt(SO_ATTACH_FILTER) failed: %s", strerror(errno));
		/* try to continue without the workaround RX socket */
		close(l2->fd_br_rx);
		l2->fd_br_rx = -1;
		return l2;
...
int l2_packet_set_packet_filter(struct l2_packet_data *l2, enum l2_packet_filter_type type)
{
	...
	if (setsockopt(l2->fd, SOL_SOCKET, SO_ATTACH_FILTER, sock_filter, sizeof(struct sock_fprog))) {
		wpa_printf(MSG_ERROR, "l2_packet_linux: setsockopt(SO_ATTACH_FILTER) failed: %s", strerror(errno));
		return -1;
	}
