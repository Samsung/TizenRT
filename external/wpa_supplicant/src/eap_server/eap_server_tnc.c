...
{
	wpa_printf(MSG_DEBUG, "EAP-TNC: %s -> %s", eap_tnc_state_txt(data->state), eap_tnc_state_txt(new_state));
...
static struct wpabuf *eap_tnc_build_start(struct eap_sm *sm, struct eap_tnc_data *data, u8 id)
{
	...
	if (req == NULL) {
		wpa_printf(MSG_ERROR, "EAP-TNC: Failed to allocate memory for " "request");
...
static struct wpabuf *eap_tnc_build(struct eap_sm *sm, struct eap_tnc_data *data)
{
	...
	wpa_hexdump_ascii(MSG_MSGDUMP, "EAP-TNC: Request", wpabuf_head(req), wpabuf_len(req));
...
static struct wpabuf *eap_tnc_build_frag_ack(u8 id, u8 code)
{
	...
	if (msg == NULL) {
		wpa_printf(MSG_ERROR, "EAP-TNC: Failed to allocate memory " "for fragment ack");
		return NULL;
	}
...
static struct wpabuf *eap_tnc_build_msg(struct eap_tnc_data *data, u8 id)
{
	...
	req = eap_msg_alloc(EAP_VENDOR_IETF, EAP_TYPE_TNC, plen, EAP_CODE_REQUEST, id);
	if (req == NULL) {
		return NULL;
	}
	...
	if (data->out_used == wpabuf_len(data->out_buf)) {
		wpa_printf(MSG_DEBUG, "EAP-TNC: Sending out %lu bytes " "(message sent completely)", (unsigned long)send_len);
		wpabuf_free(data->out_buf);
		data->out_buf = NULL;
		data->out_used = 0;
		if (data->was_fail) {
		...
		wpa_printf(MSG_DEBUG, "EAP-TNC: Sending out %lu bytes " "(%lu more to send)", (unsigned long)send_len, (unsigned long)wpabuf_len(data->out_buf) - data->out_used);
		if (data->state == FAIL) {
			data->was_fail = 1;
...
static struct wpabuf *eap_tnc_buildReq(struct eap_sm *sm, void *priv, u8 id)
{
	...
	case CONTINUE:
		if (data->out_buf == NULL) {
			...
			if (data->out_buf == NULL) {
				wpa_printf(MSG_DEBUG, "EAP-TNC: Failed to " "generate message");
				return NULL;
			}
			data->out_used = 0;
		}
	...
	case RECOMMENDATION:
		if (data->out_buf == NULL) {
			...
			if (data->out_buf == NULL) {
				wpa_printf(MSG_DEBUG, "EAP-TNC: Failed to " "generate recommendation message");
				return NULL;
			}
			data->out_used = 0;
		}
...
static Boolean eap_tnc_check(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
	...
	if ((*pos & EAP_TNC_VERSION_MASK) != EAP_TNC_VERSION) {
		wpa_printf(MSG_DEBUG, "EAP-TNC: Unsupported version %d", *pos & EAP_TNC_VERSION_MASK);
		return TRUE;
	}
...
static void tncs_process(struct eap_tnc_data *data, struct wpabuf *inbuf)
{
	...
	res = tncs_process_if_tnccs(data->tncs, wpabuf_head(inbuf), wpabuf_len(inbuf));
	switch (res) {
...
static int eap_tnc_process_cont(struct eap_tnc_data *data, const u8 *buf, size_t len)
{
	...
	wpa_printf(MSG_DEBUG, "EAP-TNC: Received %lu bytes, waiting for %lu " "bytes more", (unsigned long)len, (unsigned long)wpabuf_tailroom(data->in_buf));
...
static int eap_tnc_process_fragment(struct eap_tnc_data *data, u8 flags, u32 message_length, const u8 *buf, size_t len)
{
	...
	if (data->in_buf == NULL && !(flags & EAP_TNC_FLAGS_LENGTH_INCLUDED)) {
		wpa_printf(MSG_DEBUG, "EAP-TNC: No Message Length field in a " "fragmented packet");
		return -1;
	}
	...
	if (data->in_buf == NULL) {
		...
		if (data->in_buf == NULL) {
			wpa_printf(MSG_DEBUG, "EAP-TNC: No memory for " "message");
			return -1;
		}
		wpabuf_put_data(data->in_buf, buf, len);
		wpa_printf(MSG_DEBUG, "EAP-TNC: Received %lu bytes in first " "fragment, waiting for %lu bytes more", (unsigned long)len, (unsigned long)wpabuf_tailroom(data->in_buf));
	}
...
static void eap_tnc_process(struct eap_sm *sm, void *priv, struct wpabuf *respData)
{
	...
	if (len == 1 && (data->state == DONE || data->state == FAIL)) {
		wpa_printf(MSG_DEBUG, "EAP-TNC: Peer acknowledged the last " "message");
		return;
	}
	...
	if (flags & EAP_TNC_FLAGS_LENGTH_INCLUDED) {
		...
		if (message_length < (u32)(end - pos) || message_length > 75000) {
			wpa_printf(MSG_DEBUG, "EAP-TNC: Invalid Message " "Length (%d; %ld remaining in this msg)", message_length, (long)(end - pos));
			eap_tnc_set_state(data, FAIL);
			return;
		}
	}
	wpa_printf(MSG_DEBUG, "EAP-TNC: Received packet: Flags 0x%x " "Message Length %u", flags, message_length);

	if (data->state == WAIT_FRAG_ACK) {
		if (len > 1) {
			wpa_printf(MSG_DEBUG, "EAP-TNC: Unexpected payload " "in WAIT_FRAG_ACK state");
			eap_tnc_set_state(data, FAIL);
			return;
		}
	...
	if (flags & EAP_TNC_FLAGS_MORE_FRAGMENTS) {
		if (eap_tnc_process_fragment(data, flags, message_length, pos, end - pos) < 0) {
			eap_tnc_set_state(data, FAIL);
		} else {
	...
	wpa_hexdump_ascii(MSG_MSGDUMP, "EAP-TNC: Received payload", wpabuf_head(data->in_buf), wpabuf_len(data->in_buf));
...
int eap_server_tnc_register(void)
{
	...
	eap = eap_server_method_alloc(EAP_SERVER_METHOD_INTERFACE_VERSION, EAP_VENDOR_IETF, EAP_TYPE_TNC, "TNC");
	if (eap == NULL) {
		return -1;
	}
