...
static struct eap_server_erp_key *eap_erp_get_key(struct eap_sm *sm, const char *keyname)
{
...
static struct wpabuf *eap_sm_buildInitiateReauthStart(struct eap_sm *sm, u8 id)
{
	const char *domain;
	size_t plen = 1;
	struct wpabuf *msg;
	...
	msg = eap_msg_alloc(EAP_VENDOR_IETF, EAP_ERP_TYPE_REAUTH_START, plen, EAP_CODE_INITIATE, id);
	if (msg == NULL) {
		return NULL;
	}
...
int eap_user_get(struct eap_sm *sm, const u8 *identity, size_t identity_len, int phase2)
{
	...
	if (sm->eapol_cb->get_eap_user(sm->eapol_ctx, identity, identity_len, phase2, user) != 0) {
		eap_user_free(user);
		return -1;
	}
...
SM_STATE(EAP, INITIALIZE)
{
	...
	wpa_msg(sm->msg_ctx, MSG_INFO, WPA_EVENT_EAP_STARTED MACSTR, MAC2STR(sm->peer_addr));
}
	...
	if (eap_sm_Policy_doPickUp(sm, sm->respMethod)) {
		...
		if (sm->m && sm->m->initPickUp) {
			...
			if (sm->eap_method_priv == NULL) {
				wpa_printf(MSG_DEBUG, "EAP: Failed to " "initialize EAP method %d", sm->currentMethod);
				sm->m = NULL;
				sm->currentMethod = EAP_TYPE_NONE;
			}
		} else {
...
SM_STATE(EAP, IDLE)
{
	...
	sm->eap_if.retransWhile = eap_sm_calculateTimeout(sm, sm->retransCount, sm->eap_if.eapSRTT, sm->eap_if.eapRTTVAR, sm->methodTimeout);
}
...
SM_STATE(EAP, METHOD_REQUEST)
{
	...
	sm->eap_if.eapReqData = sm->m->buildReq(sm, sm->eap_method_priv, sm->currentId);
	if (sm->m->getTimeout) {
...
static void eap_server_erp_init(struct eap_sm *sm)
{
	...
	WPA_PUT_BE16(len, 8);
	if (hmac_sha256_kdf(sm->eap_if.eapSessionId, sm->eap_if.eapSessionIdLen, "EMSK", len, sizeof(len), EMSKname, EAP_EMSK_NAME_LEN) < 0) {
...
SM_STATE(EAP, METHOD_RESPONSE)
{
	...
	if (sm->m->isDone(sm, sm->eap_method_priv)) {
		...
		if (sm->m->getSessionId) {
			sm->eap_if.eapSessionId = sm->m->getSessionId(sm, sm->eap_method_priv, &sm->eap_if.eapSessionIdLen);
...
try_another_method:
	...
	if (sm->currentMethod == EAP_TYPE_IDENTITY || sm->currentMethod == EAP_TYPE_NOTIFICATION) {
		sm->methodState = METHOD_CONTINUE;
	} else {
		sm->methodState = METHOD_PROPOSED;
	}

	wpa_msg(sm->msg_ctx, MSG_INFO, WPA_EVENT_EAP_PROPOSED_METHOD "vendor=%u method=%u", vendor, sm->currentMethod);
	eap_log_msg(sm, "Propose EAP method vendor=%u method=%u", vendor, sm->currentMethod);
}
...
SM_STATE(EAP, FAILURE)
{
	...
	wpa_msg(sm->msg_ctx, MSG_INFO, WPA_EVENT_EAP_FAILURE MACSTR, MAC2STR(sm->peer_addr));
}
...
SM_STATE(EAP, SUCCESS)
{
	...
	wpa_msg(sm->msg_ctx, MSG_INFO, WPA_EVENT_EAP_SUCCESS MACSTR, MAC2STR(sm->peer_addr));
}
...
SM_STATE(EAP, INITIATE_REAUTH_START)
{
	...
	sm->currentId = eap_sm_nextId(sm, sm->currentId);
	wpa_printf(MSG_DEBUG, "EAP: building EAP-Initiate-Re-auth-Start: Identifier %d", sm->currentId);
	sm->lastId = sm->currentId;
	wpabuf_free(sm->eap_if.eapReqData);
...
static void erp_send_finish_reauth(struct eap_sm *sm, struct eap_server_erp_key *erp, u8 id, u8 flags, u16 seq, const char *nai)
{
	size_t plen;
	struct wpabuf *msg;
	...
	msg = eap_msg_alloc(EAP_VENDOR_IETF, EAP_ERP_TYPE_REAUTH, plen, EAP_CODE_FINISH, id);
	if (msg == NULL) {
		return;
	}
	...
	wpa_printf(MSG_DEBUG, "EAP: Send EAP-Finish/Re-auth (%s)", flags & 0x80 ? "failure" : "success");
	...
	if (hmac_sha256_kdf(erp->rRK, erp->rRK_len, "Re-authentication Master Session Key@ietf.org", seed, sizeof(seed), sm->eap_if.eapKeyData, erp->rRK_len) < 0) {
	...
	sm->eap_if.eapKeyAvailable = TRUE;
	wpa_hexdump_key(MSG_DEBUG, "EAP: ERP rMSK", sm->eap_if.eapKeyData, sm->eap_if.eapKeyDataLen);
	sm->eap_if.eapSuccess = TRUE;
...
SM_STATE(EAP, INITIATE_RECEIVED)
{
	...
	pos = eap_hdr_validate(EAP_VENDOR_IETF, EAP_ERP_TYPE_REAUTH, sm->eap_if.eapRespData, &len);
	if (pos == NULL) {
	...
	wpa_hexdump_ascii(MSG_DEBUG, "EAP: EAP-Initiate/Re-auth - keyName-NAI", parse.keyname, parse.keyname_len);
	...
	if (!sm->eap_server) {
		 ...
		 */
		wpa_printf(MSG_DEBUG, "EAP: Use keyName-NAI as user identity for backend authentication");
		eap_server_clear_identity(sm);
	...
	max_len = end - start;
	if (max_len < 1 + (erp->cryptosuite == EAP_ERP_CS_HMAC_SHA256_256 ? 32 : 16)) {
	...
	if (hash_len) {
		if (hmac_sha256(erp->rIK, erp->rIK_len, hdr, end - hdr - hash_len, hash) < 0) {
			goto fail;
		}
	...
	if (!hash_len && max_len >= 1 + 32 && end[-33] == EAP_ERP_CS_HMAC_SHA256_256) {
		if (hmac_sha256(erp->rIK, erp->rIK_len, hdr, end - hdr - 32, hash) < 0) {
			goto fail;
		}
		if (os_memcmp(end - 32, hash, 32) == 0) {
			wpa_printf(MSG_DEBUG, "EAP: Authentication Tag match using HMAC-SHA256-256");
			hash_len = 32;
	...
	if (!hash_len && end[-17] == EAP_ERP_CS_HMAC_SHA256_128) {
		...
		if (os_memcmp(end - 16, hash, 16) == 0) {
			wpa_printf(MSG_DEBUG, "EAP: Authentication Tag match using HMAC-SHA256-128");
			hash_len = 16;
	...
	if (!hash_len) {
		wpa_printf(MSG_DEBUG, "EAP: No supported cryptosuite matched Authentication Tag");
		goto fail;
	}
	end -= 1 + hash_len;
	 ...
	 * that contains them.
	 */
	wpa_hexdump(MSG_DEBUG, "EAP: EAP-Initiate/Re-Auth TVs/TLVs", tlvs, end - tlvs);
	if (erp_parse_tlvs(tlvs, end, &parse, 0) < 0) {
		goto fail;
	}

	wpa_printf(MSG_DEBUG, "EAP: ERP key %s SEQ updated to %u", erp->keyname_nai, seq);
	erp->recv_seq = seq;
...
SM_STEP(EAP)
{
	if (sm->eap_if.eapRestart && sm->eap_if.portEnabled) {
		...
		if (sm->num_rounds == EAP_MAX_AUTH_ROUNDS + 1) {
			wpa_printf(MSG_DEBUG, "EAP: more than %d " "authentication rounds - abort", EAP_MAX_AUTH_ROUNDS);
			sm->num_rounds++;
		...
		case EAP_INITIALIZE:
			if (sm->backend_auth) {
				...
				} else if (sm->rxResp && (sm->respMethod == EAP_TYPE_NAK || (sm->respMethod == EAP_TYPE_EXPANDED && sm->respVendor == EAP_VENDOR_IETF && sm->respVendorMethod == EAP_TYPE_NAK))) {
					SM_ENTER(EAP, NAK);
				} else {
		...
		case EAP_RETRANSMIT:
			...
			break;
		case EAP_RECEIVED:
			if (sm->rxResp && (sm->respId == sm->currentId) && (sm->respMethod == EAP_TYPE_NAK || (sm->respMethod == EAP_TYPE_EXPANDED && sm->respVendor == EAP_VENDOR_IETF && sm->respVendorMethod == EAP_TYPE_NAK))
				&& (sm->methodState == METHOD_PROPOSED)) {
				SM_ENTER(EAP, NAK);
			} else if (sm->rxResp && (sm->respId == sm->currentId) && ((sm->respMethod == sm->currentMethod) || (sm->respMethod == EAP_TYPE_EXPANDED && sm->respVendor == EAP_VENDOR_IETF && sm->respVendorMethod == sm->currentMethod))) {
				...
				wpa_printf(MSG_DEBUG, "EAP: RECEIVED->DISCARD: " "rxResp=%d respId=%d currentId=%d " "respMethod=%d currentMethod=%d", sm->rxResp, sm->respId, sm->currentId, sm->respMethod, sm->currentMethod);
		...
		case EAP_METHOD_RESPONSE:
			...
			if (sm->methodState == METHOD_END) {
				...
				wpa_printf(MSG_DEBUG, "EAP: Method has pending " "processing - wait before proceeding to " "METHOD_REQUEST state");
			} else if (sm->method_pending == METHOD_PENDING_CONT) {
				wpa_printf(MSG_DEBUG, "EAP: Method has completed " "pending processing - reprocess pending " "EAP message");
				sm->method_pending = METHOD_PENDING_NONE;
				SM_ENTER(EAP, METHOD_RESPONSE);
			} else {
...
static int eap_sm_calculateTimeout(struct eap_sm *sm, int retransCount, int eapSRTT, int eapRTTVAR, int methodTimeout)
{
	...
	if (sm->try_initiate_reauth) {
		wpa_printf(MSG_DEBUG, "EAP: retransmit timeout 1 second for EAP-Initiate-Re-auth-Start");
		return 1;
	}
	...
	if (methodTimeout) {
		 ...
		 */
		wpa_printf(MSG_DEBUG, "EAP: retransmit timeout %d seconds " "(from EAP method hint)", methodTimeout);
	...
	wpa_printf(MSG_DEBUG, "EAP: retransmit timeout %d seconds " "(from dynamic back off; retransCount=%d)", rto, retransCount);
...
static void eap_sm_parseEapResp(struct eap_sm *sm, const struct wpabuf *resp)
{
	...
	if (resp == NULL || wpabuf_len(resp) < sizeof(*hdr)) {
		wpa_printf(MSG_DEBUG, "EAP: parseEapResp: invalid resp=%p " "len=%lu", resp, resp ? (unsigned long)wpabuf_len(resp) : 0);
		return;
	}
	...
	if (plen > wpabuf_len(resp)) {
		wpa_printf(MSG_DEBUG, "EAP: Ignored truncated EAP-Packet " "(len=%lu plen=%lu)", (unsigned long)wpabuf_len(resp), (unsigned long)plen);
		return;
	}
	...
	wpa_printf(MSG_DEBUG, "EAP: parseEapResp: rxResp=%d rxInitiate=%d respId=%d respMethod=%u respVendor=%u respVendorMethod=%u", sm->rxResp, sm->rxInitiate, sm->respId, sm->respMethod, sm->respVendor, sm->respVendorMethod);
}
...
void eap_sm_process_nak(struct eap_sm *sm, const u8 *nak_list, size_t len)
{
	...
	wpa_printf(MSG_MSGDUMP, "EAP: processing NAK (current EAP method " "index %d)", sm->user_eap_method_index);

	wpa_hexdump(MSG_MSGDUMP, "EAP: configured methods", (u8 *)sm->user->methods, EAP_MAX_METHODS * sizeof(sm->user->methods[0]));
	wpa_hexdump(MSG_MSGDUMP, "EAP: list of methods supported by the peer", nak_list, len);

	i = sm->user_eap_method_index;
	while (i < EAP_MAX_METHODS && (sm->user->methods[i].vendor != EAP_VENDOR_IETF || sm->user->methods[i].method != EAP_TYPE_NONE)) {
...
not_found:
		...
		if (i + 1 < EAP_MAX_METHODS) {
			os_memmove(&sm->user->methods[i], &sm->user->methods[i + 1], (EAP_MAX_METHODS - i - 1) * sizeof(sm->user->methods[0]));
		}
		sm->user->methods[EAP_MAX_METHODS - 1].vendor = EAP_VENDOR_IETF;
		sm->user->methods[EAP_MAX_METHODS - 1].method = EAP_TYPE_NONE;
	}

	wpa_hexdump(MSG_MSGDUMP, "EAP: new list of configured methods", (u8 *)sm->user->methods, EAP_MAX_METHODS * sizeof(sm->user->methods[0]));
}
...
static void eap_sm_Policy_update(struct eap_sm *sm, const u8 *nak_list, size_t len)
{
	...
	if (sm->user->phase2) {
		wpa_printf(MSG_DEBUG, "EAP: EAP-Nak received after Phase2 user" " info was selected - reject");
		sm->decision = DECISION_FAILURE;
		return;
	}
...
static EapType eap_sm_Policy_getNextMethod(struct eap_sm *sm, int *vendor)
{
	...
	if (sm->identity == NULL || sm->currentId == -1) {
		*vendor = EAP_VENDOR_IETF;
		...
		sm->update_user = TRUE;
	} else if (sm->user && idx < EAP_MAX_METHODS && (sm->user->methods[idx].vendor != EAP_VENDOR_IETF || sm->user->methods[idx].method != EAP_TYPE_NONE)) {
...
static int eap_sm_Policy_getDecision(struct eap_sm *sm)
{
	...
	if (sm->m && sm->currentMethod != EAP_TYPE_IDENTITY && sm->m->isSuccess(sm, sm->eap_method_priv)) {
		wpa_printf(MSG_DEBUG, "EAP: getDecision: method succeeded -> " "SUCCESS");
		sm->update_user = TRUE;
		return DECISION_SUCCESS;
	}

	if (sm->m && sm->m->isDone(sm, sm->eap_method_priv) && !sm->m->isSuccess(sm, sm->eap_method_priv)) {
		wpa_printf(MSG_DEBUG, "EAP: getDecision: method failed -> " "FAILURE");
		sm->update_user = TRUE;
		return DECISION_FAILURE;
	}

	if ((sm->user == NULL || sm->update_user) && sm->identity && !sm->start_reauth) {
		/*
		 ...
		 */
		int id_req = 0;
		if (sm->user && sm->currentMethod == EAP_TYPE_IDENTITY && sm->user->methods[0].vendor == EAP_VENDOR_IETF && sm->user->methods[0].method == EAP_TYPE_IDENTITY) {
			id_req = 1;
		}
		if (eap_user_get(sm, sm->identity, sm->identity_len, 0) != 0) {
			wpa_printf(MSG_DEBUG, "EAP: getDecision: user not " "found from database -> FAILURE");
			return DECISION_FAILURE;
		}
		if (id_req && sm->user && sm->user->methods[0].vendor == EAP_VENDOR_IETF && sm->user->methods[0].method == EAP_TYPE_IDENTITY) {
			wpa_printf(MSG_DEBUG, "EAP: getDecision: stop " "identity request loop -> FAILURE");
			sm->update_user = TRUE;
			return DECISION_FAILURE;
		}
	...
	if (sm->user && sm->user_eap_method_index < EAP_MAX_METHODS && (sm->user->methods[sm->user_eap_method_index].vendor != EAP_VENDOR_IETF || sm->user->methods[sm->user_eap_method_index].method != EAP_TYPE_NONE)) {
		wpa_printf(MSG_DEBUG, "EAP: getDecision: another method " "available -> CONTINUE");
		return DECISION_CONTINUE;
	}

	if (!sm->identity && eap_get_erp_send_reauth_start(sm) && !sm->initiate_reauth_start_sent) {
		wpa_printf(MSG_DEBUG, "EAP: getDecision: send EAP-Initiate/Re-auth-Start");
		return DECISION_INITIATE_REAUTH_START;
	}

	if (sm->identity == NULL || sm->currentId == -1) {
		wpa_printf(MSG_DEBUG, "EAP: getDecision: no identity known " "yet -> CONTINUE");
		return DECISION_CONTINUE;
	}

	wpa_printf(MSG_DEBUG, "EAP: getDecision: no more methods available -> " "FAILURE");
	return DECISION_FAILURE;
}
...
struct eap_sm *eap_server_sm_init(void *eapol_ctx, const struct eapol_callbacks *eapol_cb, struct eap_config *conf)
{
	...
	if (conf->eap_fast_a_id) {
		...
		if (sm->eap_fast_a_id) {
			os_memcpy(sm->eap_fast_a_id, conf->eap_fast_a_id, conf->eap_fast_a_id_len);
...
void eap_server_mschap_rx_callback(struct eap_sm *sm, const char *source, const u8 *username, size_t username_len, const u8 *challenge, const u8 *response)
{
	...
	wpa_snprintf_hex_sep(hex_challenge, sizeof(hex_challenge), challenge, sizeof(challenge), ':');
	wpa_snprintf_hex_sep(hex_response, sizeof(hex_response), response, 24, ':');
	wpa_printf(MSG_DEBUG, "[%s/user=%s] asleap -C %s -R %s", source, user, hex_challenge, hex_response);
}
#endif							/* CONFIG_TESTING_OPTIONS */
