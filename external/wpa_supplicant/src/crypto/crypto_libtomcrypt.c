...
struct crypto_cipher *crypto_cipher_init(enum crypto_cipher_alg alg, const u8 *iv, const u8 *key, size_t key_len)
{
	...
	if (rc4) {
		...
		if (res != CRYPT_OK) {
			wpa_printf(MSG_DEBUG, "LibTomCrypt: Cipher start " "failed: %s", error_to_string(res));
			os_free(ctx);
			return NULL;
		}
	}
...
int crypto_cipher_encrypt(struct crypto_cipher *ctx, const u8 *plain, u8 *crypt, size_t len)
{
	...
	if (res != CRYPT_OK) {
		wpa_printf(MSG_DEBUG, "LibTomCrypt: CBC encryption " "failed: %s", error_to_string(res));
		return -1;
	}
	return 0;
}
...
int crypto_cipher_decrypt(struct crypto_cipher *ctx, const u8 *crypt, u8 *plain, size_t len)
{
	...
	if (res != CRYPT_OK) {
		wpa_printf(MSG_DEBUG, "LibTomCrypt: CBC decryption " "failed: %s", error_to_string(res));
		return -1;
	}
...
struct crypto_public_key *crypto_public_key_import(const u8 *key, size_t len)
{
	...
	if (res != CRYPT_OK) {
		wpa_printf(MSG_ERROR, "LibTomCrypt: Failed to import " "public key (res=%d '%s')", res, error_to_string(res));
		os_free(pk);
		return NULL;
	}

	if (pk->rsa.type != PK_PUBLIC) {
		wpa_printf(MSG_ERROR, "LibTomCrypt: Public key was not of " "correct type");
		rsa_free(&pk->rsa);
		os_free(pk);
		return NULL;
	}
...
struct crypto_private_key *crypto_private_key_import(const u8 *key, size_t len, const char *passwd)
{
	...
	res = rsa_import(key, len, &pk->rsa);
	if (res != CRYPT_OK) {
		wpa_printf(MSG_ERROR, "LibTomCrypt: Failed to import " "private key (res=%d '%s')", res, error_to_string(res));
		os_free(pk);
		return NULL;
	}

	if (pk->rsa.type != PK_PRIVATE) {
		wpa_printf(MSG_ERROR, "LibTomCrypt: Private key was not of " "correct type");
		rsa_free(&pk->rsa);
		os_free(pk);
		return NULL;
	}
...
static int pkcs1_generate_encryption_block(u8 block_type, size_t modlen, const u8 *in, size_t inlen, u8 *out, size_t *outlen)
{
	...
	if (modlen < 12 || modlen > *outlen || inlen > modlen - 11) {
		wpa_printf(MSG_DEBUG, "PKCS #1: %s - Invalid buffer " "lengths (modlen=%lu outlen=%lu inlen=%lu)", __func__, (unsigned long)modlen, (unsigned long)*outlen, (unsigned long)inlen);
		return -1;
	}
	...
	*pos++ = block_type;		/* BT */
	...
	case 2:
		if (os_get_random(pos, ps_len) < 0) {
			wpa_printf(MSG_DEBUG, "PKCS #1: %s - Failed to get " "random data for PS", __func__);
			return -1;
		}
		while (ps_len--) {
			if (*pos == 0x00) {
		...
		break;
	default:
		wpa_printf(MSG_DEBUG, "PKCS #1: %s - Unsupported block type " "%d", __func__, block_type);
		return -1;
	}
	*pos++ = 0x00;
...
static int crypto_rsa_encrypt_pkcs1(int block_type, rsa_key *key, int key_type, const u8 *in, size_t inlen, u8 *out, size_t *outlen)
{
	...
	if (pkcs1_generate_encryption_block(block_type, modlen, in, inlen, out, outlen) < 0) {
		return -1;
	}
	...
	if (res != CRYPT_OK) {
		wpa_printf(MSG_DEBUG, "LibTomCrypt: rsa_exptmod failed: %s", error_to_string(res));
		return -1;
	}
	*outlen = len;
...
{
	return crypto_rsa_encrypt_pkcs1(2, &key->rsa, PK_PUBLIC, in, inlen, out, outlen);
}

int crypto_private_key_sign_pkcs1(struct crypto_private_key *key, const u8 *in, size_t inlen, u8 *out, size_t *outlen)
{
	return crypto_rsa_encrypt_pkcs1(1, &key->rsa, PK_PRIVATE, in, inlen, out, outlen);
}
...
int crypto_public_key_decrypt_pkcs1(struct crypto_public_key *key, const u8 *crypt, size_t crypt_len, u8 *plain, size_t *plain_len)
{
	...
	if (len < 3 + 8 + 16 /* min hash len */  ||
		...
		wpa_printf(MSG_INFO, "LibTomCrypt: Invalid signature EB " "structure");
		return -1;
	}
	...
	if (pos - plain - 2 < 8) {
		...
		wpa_printf(MSG_INFO, "LibTomCrypt: Too short signature " "padding");
		return -1;
	}

	if (pos + 16 /* min hash len */  >= plain + len || *pos != 0x00) {
		wpa_printf(MSG_INFO, "LibTomCrypt: Invalid signature EB " "structure (2)");
		return -1;
	}
	pos++;
	len -= pos - plain;
...
int crypto_global_init(void)
{
	...
	if (register_hash(&md4_desc) < 0 || register_hash(&md5_desc) < 0 || register_hash(&sha1_desc) < 0 || register_cipher(&aes_desc) < 0 || register_cipher(&des_desc) < 0 || register_cipher(&des3_desc) < 0) {
		wpa_printf(MSG_ERROR, "TLSv1: Failed to register " "hash/cipher functions");
		return -1;
	}
...
int crypto_mod_exp(const u8 *base, size_t base_len, const u8 *power, size_t power_len, const u8 *modulus, size_t modulus_len, u8 *result, size_t *result_len)
{
	...
	if (mp_read_unsigned_bin(b, (u8 *)base, base_len) != CRYPT_OK || mp_read_unsigned_bin(p, (u8 *)power, power_len) != CRYPT_OK || mp_read_unsigned_bin(m, (u8 *)modulus, modulus_len) != CRYPT_OK) {
		goto fail;
	}
