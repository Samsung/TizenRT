...
static void handle_data(struct hostap_driver_data *drv, u8 *buf, size_t len, u16 stype)
{
...
static void handle_tx_callback(struct hostap_driver_data *drv, u8 *buf, size_t len, int ok)
{
	struct ieee80211_hdr *hdr;
	u16 fc;
	union wpa_event_data event;
...
static void handle_frame(struct hostap_driver_data *drv, u8 *buf, size_t len)
{
	 ...
	 * these to user space */
	if (len < 24) {
		wpa_printf(MSG_MSGDUMP, "handle_frame: too short (%lu)", (unsigned long)len);
		return;
	}
...
static int hostap_init_sockets(struct hostap_driver_data *drv, u8 *own_addr)
{
	...
	wpa_printf(MSG_DEBUG, "Opening raw packet socket for ifindex %d", addr.sll_ifindex);
...
static int hostap_send_mlme(void *priv, const u8 *msg, size_t len, int noack, unsigned int freq)
{
...
static int hostap_send_eapol(void *priv, const u8 *addr, const u8 *data, size_t data_len, int encrypt, const u8 *own_addr, u32 flags)
{
	struct hostap_driver_data *drv = priv;
	struct ieee80211_hdr *hdr;
	size_t len;
	...
	*((u16 *)pos) = htons(ETH_P_PAE);
	...
	if (res < 0) {
		wpa_printf(MSG_ERROR, "hostap_send_eapol - packet len: %lu - " "failed: %d (%s)", (unsigned long)len, errno, strerror(errno));
	}
	os_free(hdr);
...
static int hostap_sta_set_flags(void *priv, const u8 *addr, int total_flags, int flags_or, int flags_and)
{
	struct hostap_driver_data *drv = priv;
...
static int hostap_set_iface_flags(void *priv, int dev_up)
{
	...
	if (dev_up) {
		...
		if (ioctl(drv->ioctl_sock, SIOCSIFMTU, &ifr) != 0) {
			wpa_printf(MSG_INFO, "Setting MTU failed - trying to survive with current value: ioctl[SIOCSIFMTU]: %s", strerror(errno));
		}
	}
...
static int hostap_get_seqnum(const char *ifname, void *priv, const u8 *addr, int idx, u8 *seq)
{
	struct hostap_driver_data *drv = priv;
	struct prism2_hostapd_param *param;
	u8 *buf;
...
static int hostap_ioctl_prism2param(void *priv, int param, int value)
{
	...
	*i++ = value;
	...
	if (ioctl(drv->ioctl_sock, PRISM2_IOCTL_PRISM2_PARAM, &iwr) < 0) {
		wpa_printf(MSG_ERROR, "ioctl[PRISM2_IOCTL_PRISM2_PARAM]: %s", strerror(errno));
		return -1;
	}
...
static int hostap_set_ieee8021x(void *priv, struct wpa_bss_params *params)
{
	...
	if (hostap_ioctl_prism2param(drv, PRISM2_PARAM_HOST_DECRYPT, 1) || hostap_ioctl_prism2param(drv, PRISM2_PARAM_HOST_ENCRYPT, 1)) {
		printf("Could not setup host-based encryption in kernel " "driver.\n");
		return -1;
	}
...
static int hostap_set_ssid(void *priv, const u8 *buf, int len)
{
	...
	if (ioctl(drv->ioctl_sock, SIOCSIWESSID, &iwr) < 0) {
		wpa_printf(MSG_ERROR, "ioctl[SIOCSIWESSID,len=%d]: %s", len, strerror(errno));
		return -1;
	}
...
static int hostap_read_sta_data(void *priv, struct hostap_sta_driver_data *data, const u8 *addr)
{
	struct hostap_driver_data *drv = priv;
	...
	snprintf(buf, sizeof(buf), "/proc/net/hostap/%s/" MACSTR, drv->iface, MAC2STR(addr));
...
static int hostapd_ioctl_set_generic_elem(struct hostap_driver_data *drv)
{
	...
	if (drv->wps_ie) {
		os_memcpy(&param->u.generic_elem.data[drv->generic_ie_len], drv->wps_ie, drv->wps_ie_len);
	}
	wpa_hexdump(MSG_DEBUG, "hostap: Set generic IE", param->u.generic_elem.data, elem_len);
...
static int hostap_set_generic_elem(void *priv, const u8 *elem, size_t elem_len)
{
	struct hostap_driver_data *drv = priv;
...
static int hostap_set_ap_wps_ie(void *priv, const struct wpabuf *beacon, const struct wpabuf *proberesp, const struct wpabuf *assocresp)
{
	struct hostap_driver_data *drv = priv;
...
static void hostapd_wireless_event_wireless_custom(struct hostap_driver_data *drv, char *custom)
{
	...
	if (strncmp(custom, "MLME-MICHAELMICFAILURE.indication", 33) == 0) {
		...
		if (pos == NULL) {
			wpa_printf(MSG_DEBUG, "MLME-MICHAELMICFAILURE.indication " "without sender address ignored");
			return;
		}
		pos += 5;
		if (hwaddr_aton(pos, addr) == 0) {
			...
			wpa_printf(MSG_DEBUG, "MLME-MICHAELMICFAILURE.indication " "with invalid MAC address");
		}
	}
}

static void hostapd_wireless_event_wireless(struct hostap_driver_data *drv, char *data, int len)
{
	...
	while (pos + IW_EV_LCP_LEN <= end) {
		...
		wpa_printf(MSG_DEBUG, "Wireless event: cmd=0x%x len=%d", iwe->cmd, iwe->len);
		if (iwe->len <= IW_EV_LCP_LEN) {
			return;
		}

		custom = pos + IW_EV_POINT_LEN;
		if (drv->we_version > 18 && (iwe->cmd == IWEVMICHAELMICFAILURE || iwe->cmd == IWEVCUSTOM)) {
...
static void hostapd_wireless_event_rtm_newlink(void *ctx, struct ifinfomsg *ifi, u8 *buf, size_t len)
{
	...
	while (RTA_OK(attr, attrlen)) {
		if (attr->rta_type == IFLA_WIRELESS) {
			hostapd_wireless_event_wireless(drv, ((char *)attr) + rta_len, attr->rta_len - rta_len);
		}
...
static int hostap_get_we_version(struct hostap_driver_data *drv)
{
	...
	if (ioctl(drv->ioctl_sock, SIOCGIWRANGE, &iwr) < 0) {
		...
		wpa_printf(MSG_DEBUG, "SIOCGIWRANGE: WE(compiled)=%d " "WE(source)=%d enc_capa=0x%x", range->we_version_compiled, range->we_version_source, range->enc_capa);
...
static void *hostap_init(struct hostapd_data *hapd, struct wpa_init_params *params)
{
	...
	if (hostap_ioctl_prism2param(drv, PRISM2_PARAM_HOSTAPD, 1)) {
		wpa_printf(MSG_ERROR, "Could not enable hostapd mode for interface %s", drv->iface);
		close(drv->ioctl_sock);
		os_free(drv);
		return NULL;
	}

	if (hostap_init_sockets(drv, params->own_addr) || hostap_wireless_event_init(drv)) {
		close(drv->ioctl_sock);
		os_free(drv);
		return NULL;
	}
...
static int hostap_sta_deauth(void *priv, const u8 *own_addr, const u8 *addr, int reason)
{
	struct hostap_driver_data *drv = priv;
	...
	return hostap_send_mlme(drv, (u8 *)&mgmt, IEEE80211_HDRLEN + sizeof(mgmt.u.deauth), 0);
}
...
static int hostap_sta_disassoc(void *priv, const u8 *own_addr, const u8 *addr, int reason)
{
	struct hostap_driver_data *drv = priv;
	struct ieee80211_mgmt mgmt;
	...
	return hostap_send_mlme(drv, (u8 *)&mgmt, IEEE80211_HDRLEN + sizeof(mgmt.u.disassoc), 0);
}

static struct hostapd_hw_modes *hostap_get_hw_feature_data(void *priv, u16 *num_modes, u16 *flags)
{
...
static void wpa_driver_hostap_poll_client(void *priv, const u8 *own_addr, const u8 *addr, int qos)
{
	struct ieee80211_hdr hdr;
