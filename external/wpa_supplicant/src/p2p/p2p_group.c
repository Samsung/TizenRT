...
static void p2p_group_add_common_ies(struct p2p_group *group, struct wpabuf *ie)
{
...
static struct wpabuf *p2p_group_build_beacon_ie(struct p2p_group *group)
{
	...
	if (group->p2p->vendor_elem && group->p2p->vendor_elem[VENDOR_ELEM_BEACON_P2P_GO]) {
...
static int wifi_display_add_dev_info_descr(struct wpabuf *buf, struct p2p_group_member *m)
{
	const u8 *pos, *end;
...
static struct wpabuf *wifi_display_build_go_ie(struct p2p_group *group)
{
	...
	wfd_subelems = wpabuf_alloc(wpabuf_len(group->p2p->wfd_ie_probe_resp) + group->num_members * 24 + 100);
...
{
	p2p_buf_add_group_id(buf, group->p2p->cfg->dev_addr, group->cfg->ssid, group->cfg->ssid_len);
}
...
static struct wpabuf *p2p_group_build_probe_resp_ie(struct p2p_group *group)
{
	...
	if (group->p2p->vendor_elem && group->p2p->vendor_elem[VENDOR_ELEM_PROBE_RESP_P2P_GO]) {
		struct wpabuf *extra;
...
void p2p_group_update_ies(struct p2p_group *group)
{
	...
	wpa_hexdump_buf(MSG_MSGDUMP, "P2P: Update GO Probe Response P2P IE", probe_resp_ie);
 ...
 */
static struct wpabuf *p2p_build_client_info(const u8 *addr, struct wpabuf *p2p_ie, u8 *dev_capab, u8 *dev_addr)
{
	const u8 *spos;
	...
	if (p2p_parse_p2p_ie(p2p_ie, &msg) || msg.capability == NULL || msg.p2p_device_info == NULL) {
		return NULL;
	}
...
int p2p_group_notif_assoc(struct p2p_group *group, const u8 *addr, const u8 *ie, size_t len)
{
	...
	if (m->p2p_ie) {
		m->client_info = p2p_build_client_info(addr, m->p2p_ie, &m->dev_capab, m->dev_addr);
	}
#ifdef CONFIG_WIFI_DISPLAY
	...
	group->members = m;
	group->num_members++;
	p2p_dbg(group->p2p, "Add client " MACSTR " to group (p2p=%d wfd=%d client_info=%d); num_members=%u/%u", MAC2STR(addr), m->p2p_ie ? 1 : 0, m->wfd_ie ? 1 : 0, m->client_info ? 1 : 0, group->num_members, group->cfg->max_clients);
	if (group->num_members == group->cfg->max_clients) {
		group->beacon_update = 1;
	}
	p2p_group_update_ies(group);
...
struct wpabuf *p2p_group_assoc_resp_ie(struct p2p_group *group, u8 status)
{
	...
	if (group->p2p->vendor_elem && group->p2p->vendor_elem[VENDOR_ELEM_P2P_ASSOC_RESP]) {
...
void p2p_group_notif_disassoc(struct p2p_group *group, const u8 *addr)
{
	if (p2p_group_remove_member(group, addr)) {
		p2p_dbg(group->p2p, "Remove client " MACSTR " from group; num_members=%u/%u", MAC2STR(addr), group->num_members, group->cfg->max_clients);
 ...
 */
static int p2p_match_dev_type_member(struct p2p_group_member *m, struct wpabuf *wps)
{
	const u8 *pos, *end;
...
static struct p2p_group_member *p2p_group_get_client(struct p2p_group *group, const u8 *dev_id)
{
	struct p2p_group_member *m;
...
static struct p2p_group_member *p2p_group_get_client_iface(struct p2p_group *group, const u8 *interface_addr)
{
	struct p2p_group_member *m;
...
int p2p_group_go_discover(struct p2p_group *group, const u8 *dev_id, const u8 *searching_dev, int rx_freq)
{
	...
	if (m == NULL || m->client_info == NULL) {
		p2p_dbg(group->p2p, "Requested client was not in this group " MACSTR, MAC2STR(group->cfg->interface_addr));
		return -1;
	}
	...
	p2p_dbg(group->p2p, "Schedule GO Discoverability Request to be sent to " MACSTR, MAC2STR(dev_id));
	...
	if (p2p->cfg->send_action(p2p->cfg->cb_ctx, freq, m->addr, group->cfg->interface_addr, group->cfg->interface_addr, wpabuf_head(req), wpabuf_len(req), 200) < 0) {
...
u8 p2p_group_presence_req(struct p2p_group *group, const u8 *client_interface_addr, const u8 *noa, size_t noa_len)
{
	...
	if (group->p2p->cfg->get_noa) {
		curr_noa_len = group->p2p->cfg->get_noa(group->p2p->cfg->cb_ctx, group->cfg->interface_addr, curr_noa, sizeof(curr_noa));
	} else {
		curr_noa_len = -1;
	}
	if (curr_noa_len < 0) {
...
int p2p_group_is_group_id_match(struct p2p_group *group, const u8 *group_id, size_t group_id_len)
{
	...
	return os_memcmp(group_id + ETH_ALEN, group->cfg->ssid, group->cfg->ssid_len) == 0;
}
