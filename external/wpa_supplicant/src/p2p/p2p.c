...
static void p2p_process_presence_req(struct p2p_data *p2p, const u8 *da, const u8 *sa, const u8 *data, size_t len, int rx_freq);
static void p2p_process_presence_resp(struct p2p_data *p2p, const u8 *da, const u8 *sa, const u8 *data, size_t len);
...
static void p2p_expire_peers(struct p2p_data *p2p)
{
	...
	dl_list_for_each_safe(dev, n, &p2p->devices, struct p2p_device, list) {
		...
		if (p2p->cfg->go_connected && p2p->cfg->go_connected(p2p->cfg->cb_ctx, dev->info.p2p_device_addr)) {
			/*
		...
		for (i = 0; i < p2p->num_groups; i++) {
			if (p2p_group_is_client_connected(p2p->groups[i], dev->info.p2p_device_addr)) {
				break;
			}
		}
		if (i < p2p->num_groups) {
...
{
	p2p_dbg(p2p, "State %s -> %s", p2p_state_txt(p2p->state), p2p_state_txt(new_state));
	p2p->state = new_state;
...
{
	p2p_dbg(p2p, "Set timeout (state=%s): %u.%06u sec", p2p_state_txt(p2p->state), sec, usec);
...
static void p2p_listen_in_find(struct p2p_data *p2p, int dev_disc)
{
	...
	tu = (r % ((p2p->max_disc_int - p2p->min_disc_int) + 1) + p2p->min_disc_int) * 100;
 ...
 * creating a new one.
 */
static struct p2p_device *p2p_create_device(struct p2p_data *p2p, const u8 *addr)
{
	...
	dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
		count++;
		if (oldest == NULL || os_reltime_before(&dev->last_seen, &oldest->last_seen)) {
			oldest = dev;
		}
	}
...
static void p2p_copy_client_info(struct p2p_device *dev, struct p2p_client_info *cli)
{
...
static int p2p_add_group_clients(struct p2p_data *p2p, const u8 *go_dev_addr, const u8 *go_interface_addr, int freq, const u8 *gi, size_t gi_len, struct os_reltime *rx_time)
{
	...
	for (c = 0; c < info.num_clients; c++) {
		...
		if (dev) {
			if (dev->flags & (P2P_DEV_GROUP_CLIENT_ONLY | P2P_DEV_PROBE_REQ_ONLY)) {
				...
				dev->info.dev_capab |= cli->dev_capab & P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY;
			}
			...
			p2p_copy_client_info(dev, cli);
			dev->oper_freq = freq;
			p2p->cfg->dev_found(p2p->cfg->cb_ctx, dev->info.p2p_device_addr, &dev->info, 1);
...
static void p2p_copy_wps_info(struct p2p_data *p2p, struct p2p_device *dev, int probe_req, const struct p2p_message *msg)
{
	os_memcpy(dev->info.device_name, msg->device_name, sizeof(dev->info.device_name));

	if (msg->manufacturer && msg->manufacturer_len < sizeof(dev->info.manufacturer)) {
	...
	if (msg->model_number && msg->model_number_len < sizeof(dev->info.model_number)) {
	...
	if (msg->serial_number && msg->serial_number_len < sizeof(dev->info.serial_number)) {
		os_memset(dev->info.serial_number, 0, sizeof(dev->info.serial_number));
		os_memcpy(dev->info.serial_number, msg->serial_number, msg->serial_number_len);
	}

	if (msg->pri_dev_type) {
		os_memcpy(dev->info.pri_dev_type, msg->pri_dev_type, sizeof(dev->info.pri_dev_type));
	} else if (msg->wps_pri_dev_type) {
		os_memcpy(dev->info.pri_dev_type, msg->wps_pri_dev_type, sizeof(dev->info.pri_dev_type));
	}

	if (msg->wps_sec_dev_type_list) {
		os_memcpy(dev->info.wps_sec_dev_type_list, msg->wps_sec_dev_type_list, msg->wps_sec_dev_type_list_len);
	...
	if (msg->capability) {
		 ...
		 */
		dev->info.dev_capab &= P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY;
		dev->info.dev_capab |= msg->capability[0] & ~P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY;
	...
	if (!probe_req) {
		...
		new_config_methods = msg->config_methods ? msg->config_methods : msg->wps_config_methods;
		if (new_config_methods && dev->info.config_methods != new_config_methods) {
			p2p_dbg(p2p, "Update peer " MACSTR " config_methods 0x%x -> 0x%x", MAC2STR(dev->info.p2p_device_addr), dev->info.config_methods, new_config_methods);
			dev->info.config_methods = new_config_methods;
		}
	}
}

static void p2p_update_peer_vendor_elems(struct p2p_device *dev, const u8 *ies, size_t ies_len)
{
	const u8 *pos, *end;
	u8 id, len;
	...
	for (pos = ies; pos + 1 < end; pos += len) {
		...
		if (len >= 4) {
			...
			if (type == WPA_IE_VENDOR_TYPE || type == WMM_IE_VENDOR_TYPE || type == WPS_IE_VENDOR_TYPE || type == P2P_IE_VENDOR_TYPE || type == WFD_IE_VENDOR_TYPE) {
				continue;
			}
		}
...
static int p2p_compare_wfd_info(struct p2p_device *dev, const struct p2p_message *msg)
{
	if (dev->info.wfd_subelems && msg->wfd_subelems) {
		...
		return os_memcmp(dev->info.wfd_subelems->buf, msg->wfd_subelems->buf, dev->info.wfd_subelems->used);
	}
	if (dev->info.wfd_subelems || msg->wfd_subelems) {
		return 1;
	}
...
int p2p_add_device(struct p2p_data *p2p, const u8 *addr, int freq, struct os_reltime *rx_time, int level, const u8 *ies, size_t ies_len, int scan_res)
{
	...
	if (!is_zero_ether_addr(p2p->peer_filter) && os_memcmp(p2p_dev_addr, p2p->peer_filter, ETH_ALEN) != 0) {
		p2p_dbg(p2p, "Do not add peer filter for " MACSTR " due to peer filter", MAC2STR(p2p_dev_addr));
		p2p_parse_free(&msg);
		return 0;
	}
	 ...
	 */
	if (dev->last_seen.sec > 0 && os_reltime_before(rx_time, &dev->last_seen) && (!(dev->flags & P2P_DEV_LAST_SEEN_AS_GROUP_CLIENT) || os_reltime_expired(&dev->last_seen, rx_time, P2P_DEV_GROUP_CLIENT_RESP_THRESHOLD))) {
		p2p_dbg(p2p, "Do not update peer entry based on old frame (rx_time=%u.%06u last_seen=%u.%06u flags=0x%x)", (unsigned int)rx_time->sec, (unsigned int)rx_time->usec, (unsigned int)dev->last_seen.sec, (unsigned int)dev->last_seen.usec, dev->flags);
		p2p_parse_free(&msg);
		return -1;
	}
	...
	dev->flags &= ~(P2P_DEV_PROBE_REQ_ONLY | P2P_DEV_GROUP_CLIENT_ONLY | P2P_DEV_LAST_SEEN_AS_GROUP_CLIENT);

	if (os_memcmp(addr, p2p_dev_addr, ETH_ALEN) != 0) {
		os_memcpy(dev->interface_addr, addr, ETH_ALEN);
	}
	if (msg.ssid && msg.ssid[1] <= sizeof(dev->oper_ssid) && (msg.ssid[1] != P2P_WILDCARD_SSID_LEN || os_memcmp(msg.ssid + 2, P2P_WILDCARD_SSID, P2P_WILDCARD_SSID_LEN)
			!= 0)) {
	...
	if (msg.adv_service_instance && msg.adv_service_instance_len) {
		...
		dev->info.p2ps_instance = wpabuf_alloc_copy(msg.adv_service_instance, msg.adv_service_instance_len);
	}

	if (freq >= 2412 && freq <= 2484 && msg.ds_params && *msg.ds_params >= 1 && *msg.ds_params <= 14) {
		int ds_freq;
		if (*msg.ds_params == 14) {
			ds_freq = 2484;
		} else {
		...
		if (freq != ds_freq) {
			p2p_dbg(p2p, "Update Listen frequency based on DS Parameter Set IE: %d -> %d MHz", freq, ds_freq);
			freq = ds_freq;
		}
	}

	if (dev->listen_freq && dev->listen_freq != freq && scan_res) {
		p2p_dbg(p2p, "Update Listen frequency based on scan results (" MACSTR " %d -> %d MHz (DS param %d)", MAC2STR(dev->info.p2p_device_addr), dev->listen_freq, freq, msg.ds_params ? *msg.ds_params : -1);
	}
	if (scan_res) {
	...
	for (i = 0; i < P2P_MAX_WPS_VENDOR_EXT; i++) {
		...
		dev->info.wps_vendor_ext[i] = wpabuf_alloc_copy(msg.wps_vendor_ext[i], msg.wps_vendor_ext_len[i]);
	...
	if (scan_res) {
		p2p_add_group_clients(p2p, p2p_dev_addr, addr, freq, msg.group_info, msg.group_info_len, rx_time);
	}
	...
	if (dev->flags & P2P_DEV_REPORTED && !wfd_changed && (!msg.adv_service_instance || (dev->flags & P2P_DEV_P2PS_REPORTED))) {
		return 0;
	}

	p2p_dbg(p2p, "Peer found with Listen frequency %d MHz (rx_time=%u.%06u)", freq, (unsigned int)rx_time->sec, (unsigned int)rx_time->usec);
	if (dev->flags & P2P_DEV_USER_REJECTED) {
		p2p_dbg(p2p, "Do not report rejected device");
		return 0;
	}

	if (dev->info.config_methods == 0 && (freq == 2412 || freq == 2437 || freq == 2462)) {
		/*
		 ...
		 * information.
		 */
		p2p_dbg(p2p, "Do not report peer " MACSTR " with unknown config methods", MAC2STR(addr));
		return 0;
	}

	p2p->cfg->dev_found(p2p->cfg->cb_ctx, addr, &dev->info, !(dev->flags & P2P_DEV_REPORTED_ONCE));
	dev->flags |= P2P_DEV_REPORTED | P2P_DEV_REPORTED_ONCE;
...
static int p2p_get_next_prog_freq(struct p2p_data *p2p)
{
	...
	p2p_dbg(p2p, "Next progressive search channel: reg_class %u channel %u -> %d MHz", reg_class, channel, freq);
...
static void p2p_search(struct p2p_data *p2p)
{
	...
	if (p2p->find_type == P2P_FIND_PROGRESSIVE && (freq = p2p_get_next_prog_freq(p2p)) > 0) {
	...
	res = p2p->cfg->p2p_scan(p2p->cfg->cb_ctx, type, freq, p2p->num_req_dev_types, p2p->req_dev_types, p2p->find_dev_id, pw_id);
	if (res < 0) {
...
static int p2p_run_after_scan(struct p2p_data *p2p)
{
	...
	if (p2p->after_scan_tx) {
		...
		p2p->cfg->send_action(p2p->cfg->cb_ctx, p2p->after_scan_tx->freq, p2p->after_scan_tx->dst, p2p->after_scan_tx->src, p2p->after_scan_tx->bssid, (u8 *)(p2p->after_scan_tx + 1), p2p->after_scan_tx->len, p2p->after_scan_tx->wait_time);
	...
	case P2P_AFTER_SCAN_LISTEN:
		...
		p2p_listen(p2p, p2p->pending_listen_sec * 1000 + p2p->pending_listen_usec / 1000);
		return 1;
	case P2P_AFTER_SCAN_CONNECT:
		p2p_dbg(p2p, "Start previously requested connect with " MACSTR, MAC2STR(p2p->after_scan_peer));
...
int p2p_find(struct p2p_data *p2p, unsigned int timeout, enum p2p_discovery_type type, unsigned int num_req_dev_types, const u8 *req_dev_types, const u8 *dev_id, unsigned int search_delay, u8 seek_count, const char **seek, int freq)
{
	...
	if (seek_count == 0 || !seek) {
		...
		for (i = 0; i < seek_count; i++) {
			...
			os_memcpy(&p2p->p2ps_seek_hash[count * P2PS_HASH_LEN], buf, P2PS_HASH_LEN);
			count++;
		}
	...
	case P2P_FIND_START_WITH_FULL:
		if (freq > 0) {
			 ...
			 */
			res = p2p->cfg->p2p_scan(p2p->cfg->cb_ctx, P2P_SCAN_SPECIFIC, freq, p2p->num_req_dev_types, p2p->req_dev_types, dev_id, DEV_PW_DEFAULT);
			break;
		}
	/* fall through */
	case P2P_FIND_PROGRESSIVE:
		res = p2p->cfg->p2p_scan(p2p->cfg->cb_ctx, P2P_SCAN_FULL, 0, p2p->num_req_dev_types, p2p->req_dev_types, dev_id, DEV_PW_DEFAULT);
		break;
	case P2P_FIND_ONLY_SOCIAL:
		res = p2p->cfg->p2p_scan(p2p->cfg->cb_ctx, P2P_SCAN_SOCIAL, 0, p2p->num_req_dev_types, p2p->req_dev_types, dev_id, DEV_PW_DEFAULT);
		break;
	default:
		return -1;
	}
...
static int p2p_prepare_channel_pref(struct p2p_data *p2p, unsigned int force_freq, unsigned int pref_freq, int go)
{
	u8 op_class, op_channel;
	unsigned int freq = force_freq ? force_freq : pref_freq;

	p2p_dbg(p2p, "Prepare channel pref - force_freq=%u pref_freq=%u go=%d", force_freq, pref_freq, go);
	if (p2p_freq_to_channel(freq, &op_class, &op_channel) < 0) {
		p2p_dbg(p2p, "Unsupported frequency %u MHz", freq);
		return -1;
	}

	if (!p2p_channels_includes(&p2p->cfg->channels, op_class, op_channel) && (go || !p2p_channels_includes(&p2p->cfg->cli_channels, op_class, op_channel))) {
		p2p_dbg(p2p, "Frequency %u MHz (oper_class %u channel %u) not allowed for P2P", freq, op_class, op_channel);
		return -1;
	}
...
static void p2p_prepare_channel_best(struct p2p_data *p2p)
{
	...
	if (!p2p->cfg->cfg_op_channel && p2p->best_freq_overall > 0 && p2p_supported_freq(p2p, p2p->best_freq_overall) && p2p_freq_to_channel(p2p->best_freq_overall, &op_class, &op_channel)
		== 0) {
		p2p_dbg(p2p, "Select best overall channel as operating channel preference");
		p2p->op_reg_class = op_class;
		p2p->op_channel = op_channel;
	} else if (!p2p->cfg->cfg_op_channel && p2p->best_freq_5 > 0 && p2p_supported_freq(p2p, p2p->best_freq_5) && p2p_freq_to_channel(p2p->best_freq_5, &op_class, &op_channel)
			   == 0) {
		p2p_dbg(p2p, "Select best 5 GHz channel as operating channel preference");
		p2p->op_reg_class = op_class;
		p2p->op_channel = op_channel;
	} else if (!p2p->cfg->cfg_op_channel && p2p->best_freq_24 > 0 && p2p_supported_freq(p2p, p2p->best_freq_24) && p2p_freq_to_channel(p2p->best_freq_24, &op_class, &op_channel) == 0) {
		p2p_dbg(p2p, "Select best 2.4 GHz channel as operating channel preference");
		p2p->op_reg_class = op_class;
		p2p->op_channel = op_channel;
	} else if (p2p->cfg->num_pref_chan > 0 && p2p_channels_includes(&p2p->cfg->channels, p2p->cfg->pref_chan[0].op_class, p2p->cfg->pref_chan[0].chan)) {
	...
	} else if (p2p_channel_select(&p2p->cfg->channels, op_classes_vht, &p2p->op_reg_class, &p2p->op_channel) == 0) {
		p2p_dbg(p2p, "Select possible VHT channel (op_class %u channel %u) as operating channel preference", p2p->op_reg_class, p2p->op_channel);
	} else if (p2p_channel_select(&p2p->cfg->channels, op_classes_ht40, &p2p->op_reg_class, &p2p->op_channel) == 0) {
		p2p_dbg(p2p, "Select possible HT40 channel (op_class %u channel %u) as operating channel preference", p2p->op_reg_class, p2p->op_channel);
	} else if (p2p_channel_select(&p2p->cfg->channels, op_classes_5ghz, &p2p->op_reg_class, &p2p->op_channel) == 0) {
		p2p_dbg(p2p, "Select possible 5 GHz channel (op_class %u channel %u) as operating channel preference", p2p->op_reg_class, p2p->op_channel);
	} else if (p2p_channels_includes(&p2p->cfg->channels, p2p->cfg->op_reg_class, p2p->cfg->op_channel)) {
		p2p_dbg(p2p, "Select pre-configured channel as operating channel preference");
		p2p->op_reg_class = p2p->cfg->op_reg_class;
		p2p->op_channel = p2p->cfg->op_channel;
	} else if (p2p_channel_random_social(&p2p->cfg->channels, &p2p->op_reg_class, &p2p->op_channel) == 0) {
		p2p_dbg(p2p, "Select random available social channel (op_class %u channel %u) as operating channel preference", p2p->op_reg_class, p2p->op_channel);
	} else {
		/* Select any random available channel from the first available
		 * operating class */
		p2p_channel_select(&p2p->cfg->channels, NULL, &p2p->op_reg_class, &p2p->op_channel);
		p2p_dbg(p2p, "Select random available channel %d from operating class %d as operating channel preference", p2p->op_channel, p2p->op_reg_class);
	}
...
{
	p2p_dbg(p2p, "Prepare channel - force_freq=%u pref_freq=%u go=%d", force_freq, pref_freq, go);
	...
	p2p_dbg(p2p, "Own preference for operation channel: Operating Class %u Channel %u%s", p2p->op_reg_class, p2p->op_channel, force_freq ? " (forced)" : "");
...
static void p2p_set_dev_persistent(struct p2p_device *dev, int persistent_group)
{
	switch (persistent_group) {
	case 0:
		dev->flags &= ~(P2P_DEV_PREFER_PERSISTENT_GROUP | P2P_DEV_PREFER_PERSISTENT_RECONN);
		break;
	case 1:
...
int p2p_connect(struct p2p_data *p2p, const u8 *peer_addr, enum p2p_wps_method wps_method, int go_intent, const u8 *own_interface_addr, unsigned int force_freq, int persistent_group, const u8 *force_ssid, size_t force_ssid_len, int pd_before_go_neg, unsigned int pref_freq, u16 oob_pw_id)
{
	...
	p2p_dbg(p2p, "Request to start group negotiation - peer=" MACSTR "  GO Intent=%d  Intended Interface Address=" MACSTR " wps_method=%d persistent_group=%d pd_before_go_neg=%d " "oob_pw_id=%u", MAC2STR(peer_addr), go_intent, MAC2STR(own_interface_addr), wps_method, persistent_group, pd_before_go_neg, oob_pw_id);

	dev = p2p_get_device(p2p, peer_addr);
	if (dev == NULL || (dev->flags & P2P_DEV_PROBE_REQ_ONLY)) {
		p2p_dbg(p2p, "Cannot connect to unknown P2P Device " MACSTR, MAC2STR(peer_addr));
		return -1;
	}
	...
	if (dev->flags & P2P_DEV_GROUP_CLIENT_ONLY) {
		if (!(dev->info.dev_capab & P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY)) {
			p2p_dbg(p2p, "Cannot connect to P2P Device " MACSTR " that is in a group and is not discoverable", MAC2STR(peer_addr));
			return -1;
		}
		if (dev->oper_freq <= 0) {
			p2p_dbg(p2p, "Cannot connect to P2P Device " MACSTR " with incomplete information", MAC2STR(peer_addr));
			return -1;
		}
	...
	p2p->ssid_set = 0;
	if (force_ssid) {
		wpa_hexdump_ascii(MSG_DEBUG, "P2P: Forced SSID", force_ssid, force_ssid_len);
...
int p2p_authorize(struct p2p_data *p2p, const u8 *peer_addr, enum p2p_wps_method wps_method, int go_intent, const u8 *own_interface_addr, unsigned int force_freq, int persistent_group, const u8 *force_ssid, size_t force_ssid_len, unsigned int pref_freq, u16 oob_pw_id)
{
	...
	p2p_dbg(p2p, "Request to authorize group negotiation - peer=" MACSTR "  GO Intent=%d  Intended Interface Address=" MACSTR " wps_method=%d  persistent_group=%d oob_pw_id=%u", MAC2STR(peer_addr), go_intent, MAC2STR(own_interface_addr), wps_method, persistent_group, oob_pw_id);

	dev = p2p_get_device(p2p, peer_addr);
	if (dev == NULL) {
		p2p_dbg(p2p, "Cannot authorize unknown P2P Device " MACSTR, MAC2STR(peer_addr));
		return -1;
	}
...
void p2p_add_dev_info(struct p2p_data *p2p, const u8 *addr, struct p2p_device *dev, struct p2p_message *msg)
{
	...
	if (msg->listen_channel) {
		...
		if (freq < 0) {
			p2p_dbg(p2p, "Unknown peer Listen channel: " "country=%c%c(0x%02x) reg_class=%u channel=%u", msg->listen_channel[0], msg->listen_channel[1], msg->listen_channel[2], msg->listen_channel[3], msg->listen_channel[4]);
		} else {
			p2p_dbg(p2p, "Update peer " MACSTR " Listen channel: %u -> %u MHz", MAC2STR(dev->info.p2p_device_addr), dev->listen_freq, freq);
			dev->listen_freq = freq;
		}
	}
	...
	if (dev->flags & P2P_DEV_PROBE_REQ_ONLY) {
		...
		p2p_dbg(p2p, "Created device entry based on GO Neg Req: " MACSTR " dev_capab=0x%x group_capab=0x%x name='%s' " "listen_freq=%d", MAC2STR(dev->info.p2p_device_addr), dev->info.dev_capab, dev->info.group_capab, dev->info.device_name, dev->listen_freq);
	}
...
void p2p_go_complete(struct p2p_data *p2p, struct p2p_device *peer)
{
	...
	p2p_dbg(p2p, "GO Negotiation with " MACSTR " completed (%s will be GO)", MAC2STR(peer->info.p2p_device_addr), go ? "local end" : "peer");
...
static void p2p_rx_p2p_action(struct p2p_data *p2p, const u8 *sa, const u8 *data, size_t len, int rx_freq)
{
...
static void p2p_rx_action_public(struct p2p_data *p2p, const u8 *da, const u8 *sa, const u8 *bssid, const u8 *data, size_t len, int freq)
{
	if (len < 1) {
		return;
	}
...
static void p2p_add_dev_from_probe_req(struct p2p_data *p2p, const u8 *addr, const u8 *ie, size_t ie_len)
{
	struct p2p_message msg;
	struct p2p_device *dev;
	...
	if (msg.ssid == NULL || msg.ssid[1] != P2P_WILDCARD_SSID_LEN || os_memcmp(msg.ssid + 2, P2P_WILDCARD_SSID, P2P_WILDCARD_SSID_LEN)
		!= 0) {
	...
	if (msg.listen_channel) {
		os_memcpy(dev->country, msg.listen_channel, 3);
		dev->listen_freq = p2p_channel_to_freq(msg.listen_channel[3], msg.listen_channel[4]);
	}
	...
	p2p_dbg(p2p, "Created device entry based on Probe Req: " MACSTR " dev_capab=0x%x group_capab=0x%x name='%s' listen_freq=%d", MAC2STR(dev->info.p2p_device_addr), dev->info.dev_capab, dev->info.group_capab, dev->info.device_name, dev->listen_freq);
}
...
static int dev_type_match(const u8 *dev_type, const u8 *req_dev_type)
{
	...
	if (os_memcmp(dev_type, req_dev_type, 2) == 0 && WPA_GET_BE32(&req_dev_type[2]) == 0 && WPA_GET_BE16(&req_dev_type[6]) == 0) {
...
int p2p_match_dev_type(struct p2p_data *p2p, struct wpabuf *wps)
{
	...
	if (dev_type_list_match(p2p->cfg->pri_dev_type, attr.req_dev_type, attr.num_req_dev_type)) {
		return 1;    /* Own Primary Device Type matches */
	}

	for (i = 0; i < p2p->cfg->num_sec_dev_types; i++) {
		if (dev_type_list_match(p2p->cfg->sec_dev_type[i], attr.req_dev_type, attr.num_req_dev_type)) {
...
struct wpabuf *p2p_build_probe_resp_ies(struct p2p_data *p2p, const u8 *query_hash, u8 query_count)
{
	...
	p2p_buf_add_capability(buf, p2p->dev_capab & ~P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY, 0);
	if (p2p->ext_listen_interval) {
		p2p_buf_add_ext_listen_timing(buf, p2p->ext_listen_period, p2p->ext_listen_interval);
	}
	p2p_buf_add_device_info(buf, p2p, NULL);
	p2p_buf_update_ie_hdr(buf, len);

	if (query_count) {
		p2p_buf_add_service_instance(buf, p2p, query_count, query_hash, p2p->p2ps_adv_list);
	}
...
static int p2p_service_find_asp(struct p2p_data *p2p, const u8 *hash)
{
	...
	while (adv_data) {
		...
		if (any_wfa && os_strncmp(adv_data->svc_name, P2PS_WILD_HASH_STR, os_strlen(P2PS_WILD_HASH_STR)) == 0) {
...
static enum p2p_probe_req_status p2p_reply_probe(struct p2p_data *p2p, const u8 *addr, const u8 *dst, const u8 *bssid, const u8 *ie, size_t ie_len, unsigned int rx_freq)
{
	...
	if (dst && !is_broadcast_ether_addr(dst) && os_memcmp(dst, p2p->cfg->dev_addr, ETH_ALEN) != 0) {
	...
	if (bssid && !is_broadcast_ether_addr(bssid)) {
		...
		p2p_dbg(p2p, "Probe Req BSSID " MACSTR " not wildcard - ignore it", MAC2STR(bssid));
		return P2P_PREQ_NOT_PROCESSED;
	}

	if (elems.ssid == NULL || elems.ssid_len != P2P_WILDCARD_SSID_LEN || os_memcmp(elems.ssid, P2P_WILDCARD_SSID, P2P_WILDCARD_SSID_LEN) != 0) {
	...
	if (msg.service_hash && msg.service_hash_count) {
		...
		p2p_dbg(p2p, "in_listen=%d drv_in_listen=%d when received P2PS Probe Request at %u MHz; own Listen channel %u, pending listen freq %u MHz", p2p->in_listen, p2p->drv_in_listen, rx_freq, p2p->cfg->channel, p2p->pending_listen_freq);

		if (!p2p->in_listen && !p2p->drv_in_listen && p2p->pending_listen_freq && rx_freq && rx_freq != p2p->pending_listen_freq) {
			p2p_dbg(p2p, "Do not reply to Probe Request frame that was received on %u MHz while waiting to start Listen state on %u MHz", rx_freq, p2p->pending_listen_freq);
			p2p_parse_free(&msg);
		...
		if (!p2p->in_listen || !p2p->drv_in_listen) {
			...
			p2p_dbg(p2p, "Not in Listen state (in_listen=%d drv_in_listen=%d) - ignore Probe Request", p2p->in_listen, p2p->drv_in_listen);
			p2p_parse_free(&msg);
			return P2P_PREQ_NOT_LISTEN;
		}
	}

	if (msg.device_id && os_memcmp(msg.device_id, p2p->cfg->dev_addr, ETH_ALEN) != 0) {
		/* Device ID did not match */
		p2p_dbg(p2p, "Probe Req requested Device ID " MACSTR " did not match - ignore it", MAC2STR(msg.device_id));
		p2p_parse_free(&msg);
		return P2P_PREQ_NOT_PROCESSED;
	}
	...
	resp = wpabuf_put(buf, offsetof(struct ieee80211_mgmt, u.probe_resp.variable));

	resp->frame_control = host_to_le16((WLAN_FC_TYPE_MGMT << 2) | (WLAN_FC_STYPE_PROBE_RESP << 4));
	...
	resp->u.probe_resp.capab_info = host_to_le16(WLAN_CAPABILITY_SHORT_PREAMBLE | WLAN_CAPABILITY_PRIVACY | WLAN_CAPABILITY_SHORT_SLOT_TIME);
...
enum p2p_probe_req_status p2p_probe_req_rx(struct p2p_data *p2p, const u8 *addr, const u8 *dst, const u8 *bssid, const u8 *ie, size_t ie_len, unsigned int rx_freq)
{
	 ...
	 */
	if ((p2p->state == P2P_CONNECT || p2p->state == P2P_CONNECT_LISTEN) && p2p->go_neg_peer && os_memcmp(addr, p2p->go_neg_peer->info.p2p_device_addr, ETH_ALEN)
		== 0 && !(p2p->go_neg_peer->flags & P2P_DEV_WAIT_GO_NEG_CONFIRM)) {
	...
	if ((p2p->state == P2P_INVITE || p2p->state == P2P_INVITE_LISTEN) && p2p->invite_peer && (p2p->invite_peer->flags & P2P_DEV_WAIT_INV_REQ_ACK) && os_memcmp(addr, p2p->invite_peer->info.p2p_device_addr, ETH_ALEN)
		== 0) {
...
static int p2p_assoc_req_ie_wlan_ap(struct p2p_data *p2p, const u8 *bssid, u8 *buf, size_t len, struct wpabuf *p2p_ie)
{
	struct wpabuf *tmp;
	u8 *lpos;
	size_t tmplen;
	...
	p2p_dbg(p2p, "BSS P2P manageability %s", msg.manageability ? "enabled" : "disabled");
	...
	if (p2p->num_groups > 0) {
		...
		if ((p2p->dev_capab & P2P_DEV_CAPAB_CONCURRENT_OPER) && (p2p->dev_capab & P2P_DEV_CAPAB_INFRA_MANAGED) && p2p->cross_connect) {
			group_capab |= P2P_GROUP_CAPAB_CROSS_CONN;
		}
	}
	p2p_buf_add_capability(tmp, p2p->dev_capab, group_capab);
	if ((p2p->dev_capab & P2P_DEV_CAPAB_CONCURRENT_OPER) && (p2p->dev_capab & P2P_DEV_CAPAB_INFRA_MANAGED)) {
...
int p2p_assoc_req_ie(struct p2p_data *p2p, const u8 *bssid, u8 *buf, size_t len, int p2p_group, struct wpabuf *p2p_ie)
{
	...
	if (p2p->ext_listen_interval) {
		p2p_buf_add_ext_listen_timing(tmp, p2p->ext_listen_period, p2p->ext_listen_interval);
	}
...
int p2p_service_add_asp(struct p2p_data *p2p, int auto_accept, u32 adv_id, const char *adv_str, u8 svc_state, u16 config_methods, const char *svc_info)
{
	...
	if (!(config_methods & p2p->cfg->config_methods)) {
		p2p_dbg(p2p, "Config methods not supported svc: 0x%x dev: 0x%x", config_methods, p2p->cfg->config_methods);
		return -1;
	}
...
inserted:
	p2p_dbg(p2p, "Added ASP advertisement adv_id=0x%x config_methods=0x%x svc_state=0x%x adv_str='%s'", adv_id, adv_data->config_methods, svc_state, adv_str);
...
void p2p_wps_success_cb(struct p2p_data *p2p, const u8 *mac_addr)
{
	...
	if (os_memcmp(mac_addr, p2p->go_neg_peer->intended_addr, ETH_ALEN) != 0) {
		p2p_dbg(p2p, "Ignore WPS registration success notification for " MACSTR " (GO Negotiation peer " MACSTR ")", MAC2STR(mac_addr), MAC2STR(p2p->go_neg_peer->intended_addr));
		return;					/* Ignore unexpected peer address */
	}

	p2p_dbg(p2p, "Group Formation completed successfully with " MACSTR, MAC2STR(mac_addr));
...
void p2p_group_formation_failed(struct p2p_data *p2p)
{
	...
	p2p_dbg(p2p, "Group Formation failed with " MACSTR, MAC2STR(p2p->go_neg_peer->intended_addr));
...
struct p2p_data *p2p_init(const struct p2p_config *cfg)
{
	...
	if (cfg->pref_chan) {
		p2p->cfg->pref_chan = os_malloc(cfg->num_pref_chan * sizeof(struct p2p_channel));
		if (p2p->cfg->pref_chan) {
			os_memcpy(p2p->cfg->pref_chan, cfg->pref_chan, cfg->num_pref_chan * sizeof(struct p2p_channel));
		} else {
...
int p2p_unauthorize(struct p2p_data *p2p, const u8 *addr)
{
	...
	if (p2p->after_scan_tx && os_memcmp(addr, p2p->after_scan_tx->dst, ETH_ALEN) == 0) {
		os_free(p2p->after_scan_tx);
		p2p->after_scan_tx = NULL;
	}
...
static int p2p_pre_find_operation(struct p2p_data *p2p, struct p2p_device *dev)
{
	...
	if (dev->req_config_methods && !(dev->flags & P2P_DEV_PD_FOR_JOIN)) {
		p2p_dbg(p2p, "Send pending Provision Discovery Request to " MACSTR " (config methods 0x%x)", MAC2STR(dev->info.p2p_device_addr), dev->req_config_methods);
...
static void p2p_retry_pd(struct p2p_data *p2p)
{
	...
	dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
		if (os_memcmp(p2p->pending_pd_devaddr, dev->info.p2p_device_addr, ETH_ALEN) != 0) {
			continue;
		}
		...
		p2p_dbg(p2p, "Send pending Provision Discovery Request to " MACSTR " (config methods 0x%x)", MAC2STR(dev->info.p2p_device_addr), dev->req_config_methods);
		p2p_send_prov_disc_req(p2p, dev, dev->flags & P2P_DEV_PD_FOR_JOIN, p2p->pd_force_freq);
		return;
	}
}
...
static void p2p_prov_disc_cb(struct p2p_data *p2p, int success)
{
	...
	if (!success) {
		...
		if (p2p->user_initiated_pd && (p2p->state == P2P_SEARCH || p2p->state == P2P_LISTEN_ONLY)) {
...
static int p2p_check_after_scan_tx_continuation(struct p2p_data *p2p)
{
	if (p2p->after_scan_tx_in_progress) {
		...
		if (p2p->start_after_scan != P2P_AFTER_SCAN_NOTHING && p2p_run_after_scan(p2p)) {
			return 1;
		}
		if (p2p->state == P2P_SEARCH) {
...
static void p2p_prov_disc_resp_cb(struct p2p_data *p2p, int success)
{
	...
	if (!p2p->cfg->prov_disc_resp_cb || p2p->cfg->prov_disc_resp_cb(p2p->cfg->cb_ctx) < 1) {
		goto continue_search;
	}

	p2p_dbg(p2p, "Post-Provision Discovery operations started - do not try to continue other P2P operations");
	return;
...
int p2p_scan_res_handler(struct p2p_data *p2p, const u8 *bssid, int freq, struct os_reltime *rx_time, int level, const u8 *ies, size_t ies_len)
{
	if (os_reltime_before(rx_time, &p2p->find_start)) {
		 ...
		 */
		p2p_dbg(p2p, "Ignore old scan result for " MACSTR " (rx_time=%u.%06u)", MAC2STR(bssid), (unsigned int)rx_time->sec, (unsigned int)rx_time->usec);
		return 0;
	}
...
void p2p_scan_ie(struct p2p_data *p2p, struct wpabuf *ies, const u8 *dev_id)
{
	...
	if (p2p->cfg->reg_class && p2p->cfg->channel) {
		p2p_buf_add_listen_channel(ies, p2p->cfg->country, p2p->cfg->reg_class, p2p->cfg->channel);
	}
	if (p2p->ext_listen_interval) {
		p2p_buf_add_ext_listen_timing(ies, p2p->ext_listen_period, p2p->ext_listen_interval);
	}
...
static void p2p_go_neg_req_cb(struct p2p_data *p2p, int success)
{
	...
	if (!success && (dev->info.dev_capab & P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY) && !is_zero_ether_addr(dev->member_in_go_dev)) {
		p2p_dbg(p2p, "Peer " MACSTR " did not acknowledge request - try to use device discoverability through its GO", MAC2STR(dev->info.p2p_device_addr));
		p2p->cfg->send_action_done(p2p->cfg->cb_ctx);
	...
	if (!success && p2p->go_neg_peer && (p2p->go_neg_peer->flags & P2P_DEV_PEER_WAITING_RESPONSE)) {
		unsigned int r;
		/*
...
static void p2p_go_neg_resp_failure_cb(struct p2p_data *p2p, int success, const u8 *addr)
{
...
static void p2p_go_neg_conf_cb(struct p2p_data *p2p, enum p2p_send_action_result result)
{
	...
	if (result == P2P_SEND_ACTION_NO_ACK) {
		 ...
		 */
		if (dev && dev->go_neg_conf && dev->go_neg_conf_sent <= P2P_GO_NEG_CNF_MAX_RETRY_COUNT) {
			p2p_dbg(p2p, "GO Negotiation Confirm retry %d", dev->go_neg_conf_sent);
			p2p->pending_action_state = P2P_PENDING_GO_NEG_CONFIRM;
			if (p2p_send_action(p2p, dev->go_neg_conf_freq, dev->info.p2p_device_addr, p2p->cfg->dev_addr, dev->info.p2p_device_addr, wpabuf_head(dev->go_neg_conf), wpabuf_len(dev->go_neg_conf), 0) >= 0) {
...
void p2p_send_action_cb(struct p2p_data *p2p, unsigned int freq, const u8 *dst, const u8 *src, const u8 *bssid, enum p2p_send_action_result result)
{
	...
	p2p_dbg(p2p, "Action frame TX callback (state=%d freq=%u dst=" MACSTR " src=" MACSTR " bssid=" MACSTR " result=%d p2p_state=%s)", p2p->pending_action_state, freq, MAC2STR(dst), MAC2STR(src), MAC2STR(bssid), result, p2p_state_txt(p2p->state));
...
void p2p_listen_cb(struct p2p_data *p2p, unsigned int freq, unsigned int duration)
{
	...
	if (freq != p2p->pending_listen_freq) {
		p2p_dbg(p2p, "Unexpected listen callback for freq=%u duration=%u (pending_listen_freq=%u)", freq, duration, p2p->pending_listen_freq);
		return;
	}

	p2p_dbg(p2p, "Starting Listen timeout(%u,%u) on freq=%u based on callback", p2p->pending_listen_sec, p2p->pending_listen_usec, p2p->pending_listen_freq);
	p2p->in_listen = 1;
	...
	if (p2p->pending_listen_sec || p2p->pending_listen_usec) {
		 ...
		 */
		p2p_set_timeout(p2p, p2p->pending_listen_sec, p2p->pending_listen_usec + 20000);
	}
...
int p2p_listen_end(struct p2p_data *p2p, unsigned int freq)
{
	...
	if (p2p->state == P2P_CONNECT_LISTEN && p2p->go_neg_peer) {
		...
		if (p2p->search_delay) {
			...
			p2p_set_timeout(p2p, p2p->search_delay / 1000, (p2p->search_delay % 1000) * 1000);
			return 1;
		}
		p2p_search(p2p);
		return 1;
...
{
	p2p->cfg->send_action_done(p2p->cfg->cb_ctx);
	if (p2p->go_neg_peer && (p2p->go_neg_peer->flags & P2P_DEV_WAIT_GO_NEG_CONFIRM)) {
		p2p_dbg(p2p, "Wait for GO Negotiation Confirm timed out - assume GO Negotiation failed");
		p2p_go_neg_failed(p2p, -1);
		return;
	}
	if (p2p->go_neg_peer && (p2p->go_neg_peer->flags & P2P_DEV_PEER_WAITING_RESPONSE) && p2p->go_neg_peer->connect_reqs < 120) {
...
static void p2p_timeout_wait_peer_connect(struct p2p_data *p2p)
{
	...
	if (p2p->cfg->is_concurrent_session_active && p2p->cfg->is_concurrent_session_active(p2p->cfg->cb_ctx)) {
		p2p_set_timeout(p2p, 0, 500000);
	} else {
		p2p_set_timeout(p2p, 0, 200000);
	}
...
static void p2p_timeout_prov_disc_req(struct p2p_data *p2p)
{
	...
	if (p2p->pd_retries) {
		...
		if (p2p->cfg->prov_disc_fail) {
			p2p->cfg->prov_disc_fail(p2p->cfg->cb_ctx, p2p->pending_pd_devaddr, for_join ? P2P_PROV_DISC_TIMEOUT_JOIN : P2P_PROV_DISC_TIMEOUT, adv_id, adv_mac, NULL);
		}
	...
	if (p2p->invite_peer && p2p->invite_peer->invitation_reqs < 100) {
		...
		if (p2p->invite_peer) {
			...
			if (p2p->cfg->invitation_result) {
				p2p->cfg->invitation_result(p2p->cfg->cb_ctx, -1, NULL, NULL, p2p->invite_peer->info.p2p_device_addr, 0, 0);
			}
		}
		p2p_set_state(p2p, P2P_IDLE);
	}
...
int p2p_reject(struct p2p_data *p2p, const u8 *peer_addr)
{
	...
	dev = p2p_get_device(p2p, peer_addr);
	p2p_dbg(p2p, "Local request to reject connection attempts by peer " MACSTR, MAC2STR(peer_addr));
	if (dev == NULL) {
...
int p2p_get_peer_info_txt(const struct p2p_peer_info *info, char *buf, size_t buflen)
{
	...
	if (dev->info.wfd_subelems) {
		...
		pos += wpa_snprintf_hex(pos, end - pos, wpabuf_head(dev->info.wfd_subelems), wpabuf_len(dev->info.wfd_subelems));
...
static struct wpabuf *p2p_build_presence_req(u32 duration1, u32 interval1, u32 duration2, u32 interval2)
{
	struct wpabuf *req;
...
int p2p_presence_req(struct p2p_data *p2p, const u8 *go_interface_addr, const u8 *own_interface_addr, unsigned int freq, u32 duration1, u32 interval1, u32 duration2, u32 interval2)
{
	...
	p2p_dbg(p2p, "Send Presence Request to GO " MACSTR " (own interface " MACSTR ") freq=%u dur1=%u int1=%u " "dur2=%u int2=%u", MAC2STR(go_interface_addr), MAC2STR(own_interface_addr), freq, duration1, interval1, duration2, interval2);
	...
	p2p->pending_action_state = P2P_NO_PENDING_ACTION;
	if (p2p_send_action(p2p, freq, go_interface_addr, own_interface_addr, go_interface_addr, wpabuf_head(req), wpabuf_len(req), 200) < 0) {
...
static struct wpabuf *p2p_build_presence_resp(u8 status, const u8 *noa, size_t noa_len, u8 dialog_token)
{
	struct wpabuf *resp;
	u8 *len;
...
static void p2p_process_presence_req(struct p2p_data *p2p, const u8 *da, const u8 *sa, const u8 *data, size_t len, int rx_freq)
{
	...
	for (g = 0; g < p2p->num_groups; g++) {
		if (os_memcmp(da, p2p_group_get_interface_addr(p2p->groups[g]), ETH_ALEN) == 0) {
			group = p2p->groups[g];
			break;
		}
	}
	if (group == NULL) {
		p2p_dbg(p2p, "Ignore P2P Presence Request for unknown group " MACSTR, MAC2STR(da));
		return;
	}
...
fail:
	...
	resp = p2p_build_presence_resp(status, noa_len > 0 ? noa : NULL, noa_len > 0 ? noa_len : 0, msg.dialog_token);
	if (parsed) {
		p2p_parse_free(&msg);
	}
	if (resp == NULL) {
	...
	p2p->pending_action_state = P2P_NO_PENDING_ACTION;
	if (p2p_send_action(p2p, rx_freq, sa, da, da, wpabuf_head(resp), wpabuf_len(resp), 200) < 0) {
		p2p_dbg(p2p, "Failed to send Action frame");
	}
	wpabuf_free(resp);
}
...
static void p2p_process_presence_resp(struct p2p_data *p2p, const u8 *da, const u8 *sa, const u8 *data, size_t len)
{
	...
	if (p2p->cfg->presence_resp) {
		p2p->cfg->presence_resp(p2p->cfg->cb_ctx, sa, *msg.status, msg.noa, msg.noa_len);
	}
	...
	wpa_hexdump(MSG_DEBUG, "P2P: P2P Presence Response - NoA", msg.noa, msg.noa_len);
	/* TODO: process NoA */
	p2p_parse_free(&msg);
}
...
static void p2p_ext_listen_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (p2p->ext_listen_interval) {
		...
		eloop_register_timeout(p2p->ext_listen_interval_sec, p2p->ext_listen_interval_usec, p2p_ext_listen_timeout, p2p, NULL);
	}

	if ((p2p->cfg->is_p2p_in_progress && p2p->cfg->is_p2p_in_progress(p2p->cfg->cb_ctx)) || (p2p->pending_action_state == P2P_PENDING_PD && p2p->pd_retries > 0)) {
...
{
	if (period > 65535 || interval > 65535 || period > interval || (period == 0 && interval > 0) || (period > 0 && interval == 0)) {
		p2p_dbg(p2p, "Invalid Extended Listen Timing request: period=%u interval=%u", period, interval);
		return -1;
	}
	...
	p2p_dbg(p2p, "Enabling Extended Listen Timing: period %u msec, interval %u msec", period, interval);
...
void p2p_deauth_notif(struct p2p_data *p2p, const u8 *bssid, u16 reason_code, const u8 *ie, size_t ie_len)
{
	...
	p2p_dbg(p2p, "Deauthentication notification BSSID " MACSTR " reason_code=%u minor_reason_code=%u", MAC2STR(bssid), reason_code, *msg.minor_reason_code);
...
void p2p_disassoc_notif(struct p2p_data *p2p, const u8 *bssid, u16 reason_code, const u8 *ie, size_t ie_len)
{
	...
	p2p_dbg(p2p, "Disassociation notification BSSID " MACSTR " reason_code=%u minor_reason_code=%u", MAC2STR(bssid), reason_code, *msg.minor_reason_code);
...
int p2p_set_listen_channel(struct p2p_data *p2p, u8 reg_class, u8 channel, u8 forced)
{
	...
	if (p2p->cfg->channel_forced && forced == 0) {
		p2p_dbg(p2p, "Listen channel was previously configured - do not override based on optimization");
		return -1;
	}

	p2p_dbg(p2p, "Set Listen channel: reg_class %u channel %u", reg_class, channel);
...
int p2p_set_oper_channel(struct p2p_data *p2p, u8 op_reg_class, u8 op_channel, int cfg_op_channel)
{
	...
	p2p_dbg(p2p, "Set Operating channel: reg_class %u channel %u", op_reg_class, op_channel);
...
int p2p_send_action(struct p2p_data *p2p, unsigned int freq, const u8 *dst, const u8 *src, const u8 *bssid, const u8 *buf, size_t len, unsigned int wait_time)
{
	...
	return p2p->cfg->send_action(p2p->cfg->cb_ctx, freq, dst, src, bssid, buf, len, wait_time);
}

void p2p_set_best_channels(struct p2p_data *p2p, int freq_24, int freq_5, int freq_overall)
{
	p2p_dbg(p2p, "Best channel: 2.4 GHz: %d,  5 GHz: %d,  overall: %d", freq_24, freq_5, freq_overall);
...
static struct wpabuf *p2p_build_nfc_handover(struct p2p_data *p2p, int client_freq, const u8 *go_dev_addr, const u8 *ssid, size_t ssid_len)
{
	struct wpabuf *buf;
	u8 op_class, channel;
	...
	if (p2p->num_groups > 0) {
		...
		if (p2p_freq_to_channel(freq, &op_class, &channel) < 0) {
			p2p_dbg(p2p, "Unknown GO operating frequency %d MHz for NFC handover", freq);
			wpabuf_free(buf);
			return NULL;
		}
		...
		if (p2p_freq_to_channel(client_freq, &op_class, &channel) < 0) {
			p2p_dbg(p2p, "Unknown client operating frequency %d MHz for NFC handover", client_freq);
			wpabuf_free(buf);
			return NULL;
		}
	}

	p2p_buf_add_oob_go_neg_channel(buf, p2p->cfg->country, op_class, channel, role);
	...
	} else if (client_freq > 0 && go_dev_addr && !is_zero_ether_addr(go_dev_addr) && ssid && ssid_len > 0) {
		/*
...
int p2p_process_nfc_connection_handover(struct p2p_data *p2p, struct p2p_nfc_params *params)
{
	...
	if (p2p_parse_ies_separate(params->wsc_attr, params->wsc_len, params->p2p_attr, params->p2p_len, &msg)) {
	...
	if (msg.oob_dev_password) {
		os_memcpy(params->oob_dev_pw, msg.oob_dev_password, msg.oob_dev_password_len);
	...
	if (msg.oob_go_neg_channel[3] == 0 && msg.oob_go_neg_channel[4] == 0) {
		...
		freq = p2p_channel_to_freq(msg.oob_go_neg_channel[3], msg.oob_go_neg_channel[4]);
	}
	if (freq < 0) {
	...
	if (!(dev->flags & P2P_DEV_REPORTED)) {
		p2p->cfg->dev_found(p2p->cfg->cb_ctx, p2p_dev_addr, &dev->info, !(dev->flags & P2P_DEV_REPORTED_ONCE));
		dev->flags |= P2P_DEV_REPORTED | P2P_DEV_REPORTED_ONCE;
	}
	p2p_parse_free(&msg);
...
void p2p_set_authorized_oob_dev_pw_id(struct p2p_data *p2p, u16 dev_pw_id, int go_intent, const u8 *own_interface_addr)
{
	...
	p2p_dbg(p2p, "NFC OOB Password (id=%u) authorized for static handover", dev_pw_id);
