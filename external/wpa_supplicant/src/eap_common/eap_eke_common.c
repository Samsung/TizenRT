...
int eap_eke_dh_init(u8 group, u8 *ret_priv, u8 *ret_pub)
{
	...
	wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: DH private value", ret_priv, dh->prime_len);

	/* y = g ^ x (mod p) */
	pub_len = dh->prime_len;
	if (crypto_mod_exp(&gen, 1, ret_priv, dh->prime_len, dh->prime, dh->prime_len, ret_pub, &pub_len) < 0) {
		return -1;
	}
...
static int eap_eke_prf(u8 prf, const u8 *key, size_t key_len, const u8 *data, size_t data_len, const u8 *data2, size_t data2_len, u8 *res)
{
	const u8 *addr[2];
	size_t len[2];
	size_t num_elem = 1;
...
static int eap_eke_prf_hmac_sha1(const u8 *key, size_t key_len, const u8 *data, size_t data_len, u8 *res, size_t len)
{
...
static int eap_eke_prf_hmac_sha256(const u8 *key, size_t key_len, const u8 *data, size_t data_len, u8 *res, size_t len)
{
...
static int eap_eke_prfplus(u8 prf, const u8 *key, size_t key_len, const u8 *data, size_t data_len, u8 *res, size_t len)
{
	if (prf == EAP_EKE_PRF_HMAC_SHA1) {
...
int eap_eke_derive_key(struct eap_eke_session *sess, const u8 *password, size_t password_len, const u8 *id_s, size_t id_s_len, const u8 *id_p, size_t id_p_len, u8 *key)
{
	...
	if (eap_eke_prf(sess->prf, zeros, sess->prf_len, password, password_len, NULL, 0, temp) < 0) {
		return -1;
	}
	wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: temp = prf(0+, password)", temp, sess->prf_len);
	...
	if (eap_eke_prfplus(sess->prf, temp, sess->prf_len, id, id_s_len + id_p_len, key, key_len) < 0) {
		os_free(id);
		return -1;
	}
	os_free(id);
	wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: key = prf+(temp, ID_S | ID_P)", key, key_len);
...
int eap_eke_dhcomp(struct eap_eke_session *sess, const u8 *key, const u8 *dhpub, u8 *ret_dhcomp)
{
	...
	wpa_hexdump(MSG_DEBUG, "EAP-EKE: DHComponent = Encr(key, y)", ret_dhcomp, AES_BLOCK_SIZE + dh_len);
...
int eap_eke_shared_secret(struct eap_eke_session *sess, const u8 *key, const u8 *dhpriv, const u8 *peer_dhcomp)
{
	...
	wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: Decrypted peer DH pubkey", peer_pub, dh->prime_len);

	/* SharedSecret = prf(0+, g ^ (x_s * x_p) (mod p)) */
	len = dh->prime_len;
	if (crypto_mod_exp(peer_pub, dh->prime_len, dhpriv, dh->prime_len, dh->prime, dh->prime_len, modexp, &len) < 0) {
		return -1;
	}
	if (len < dh->prime_len) {
	...
	if (eap_eke_prf(sess->prf, zeros, sess->auth_len, modexp, dh->prime_len, NULL, 0, sess->shared_secret) < 0) {
		return -1;
	}
	wpa_hexdump_key(MSG_DEBUG, "EAP-EKE: SharedSecret", sess->shared_secret, sess->auth_len);
...
int eap_eke_derive_ke_ki(struct eap_eke_session *sess, const u8 *id_s, size_t id_s_len, const u8 *id_p, size_t id_p_len)
{
	...
	if (eap_eke_prfplus(sess->prf, sess->shared_secret, sess->prf_len, data, data_len, buf, ke_len + ki_len) < 0) {
		os_free(data);
		return -1;
	}
...
int eap_eke_derive_ka(struct eap_eke_session *sess, const u8 *id_s, size_t id_s_len, const u8 *id_p, size_t id_p_len, const u8 *nonce_p, const u8 *nonce_s)
{
	...
	if (eap_eke_prfplus(sess->prf, sess->shared_secret, sess->prf_len, data, data_len, sess->ka, sess->prf_len) < 0) {
		os_free(data);
		return -1;
	}
	os_free(data);
...
int eap_eke_derive_msk(struct eap_eke_session *sess, const u8 *id_s, size_t id_s_len, const u8 *id_p, size_t id_p_len, const u8 *nonce_p, const u8 *nonce_s, u8 *msk, u8 *emsk)
{
	...
	pos += sess->nonce_len;
	os_memcpy(pos, nonce_s, sess->nonce_len);
	if (eap_eke_prfplus(sess->prf, sess->shared_secret, sess->prf_len, data, data_len, buf, EAP_MSK_LEN + EAP_EMSK_LEN) < 0) {
		os_free(data);
		return -1;
	}
	os_free(data);
...
static int eap_eke_mac(u8 mac, const u8 *key, const u8 *data, size_t data_len, u8 *res)
{
	if (mac == EAP_EKE_MAC_HMAC_SHA1) {
...
int eap_eke_decrypt_prot(struct eap_eke_session *sess, const u8 *prot, size_t prot_len, u8 *data, size_t *data_len)
{
	...
	if (eap_eke_mac(sess->mac, sess->ki, prot + block_size, prot_len - block_size - icv_len, icv) < 0) {
		return -1;
	}
...
int eap_eke_auth(struct eap_eke_session *sess, const char *label, const struct wpabuf *msgs, u8 *auth)
{
	...
	return eap_eke_prf(sess->prf, sess->ka, sess->auth_len, (const u8 *)label, os_strlen(label), wpabuf_head(msgs), wpabuf_len(msgs), auth);
}
