...
void pmksa_candidate_free(struct wpa_sm *sm)
{
	...
	dl_list_for_each_safe(entry, n, &sm->pmksa_candidates, struct rsn_pmksa_candidate, list) {
		dl_list_del(&entry->list);
		os_free(entry);
	}
}

static void rsn_preauth_receive(void *ctx, const u8 *src_addr, const u8 *buf, size_t len)
{
	...
	if (sm->preauth_eapol == NULL || is_zero_ether_addr(sm->preauth_bssid) || os_memcmp(sm->preauth_bssid, src_addr, ETH_ALEN) != 0) {
		wpa_printf(MSG_WARNING, "RSN pre-auth frame received from " "unexpected source " MACSTR " - dropped", MAC2STR(src_addr));
		return;
	}
...
static void rsn_preauth_eapol_cb(struct eapol_sm *eapol, enum eapol_supp_result result, void *ctx)
{
	...
	if (result == EAPOL_SUPP_RESULT_SUCCESS) {
		...
		if (res == 0) {
			...
			sm->pmk_len = pmk_len;
			pmksa_cache_add(sm->pmksa, pmk, pmk_len, NULL, 0, sm->preauth_bssid, sm->own_addr, sm->network_ctx, WPA_KEY_MGMT_IEEE8021X);
		} else {
...
static void rsn_preauth_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "RSN: pre-authentication with " MACSTR " timed out", MAC2STR(sm->preauth_bssid));
	rsn_preauth_deinit(sm);
	rsn_preauth_candidate_process(sm);
}

static int rsn_preauth_eapol_send(void *ctx, int type, const u8 *buf, size_t len)
{
	struct wpa_sm *sm = ctx;
	u8 *msg;
	size_t msglen;
	...
	res = l2_packet_send(sm->l2_preauth, sm->preauth_bssid, ETH_P_RSN_PREAUTH, msg, msglen);
	os_free(msg);
	return res;
}
...
int rsn_preauth_init(struct wpa_sm *sm, const u8 *dst, struct eap_peer_config *eap_conf)
{
	...
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: starting pre-authentication with " MACSTR, MAC2STR(dst));

	sm->l2_preauth = l2_packet_init(sm->ifname, sm->own_addr, ETH_P_RSN_PREAUTH, rsn_preauth_receive, sm, 0);
	if (sm->l2_preauth == NULL) {
		wpa_printf(MSG_WARNING, "RSN: Failed to initialize L2 packet " "processing for pre-authentication");
		return -2;
	}

	if (sm->bridge_ifname) {
		sm->l2_preauth_br = l2_packet_init(sm->bridge_ifname, sm->own_addr, ETH_P_RSN_PREAUTH, rsn_preauth_receive, sm, 0);
		if (sm->l2_preauth_br == NULL) {
			wpa_printf(MSG_WARNING, "RSN: Failed to initialize L2 " "packet processing (bridge) for " "pre-authentication");
			ret = -2;
			goto fail;
		}
	}
	...
	if (sm->preauth_eapol == NULL) {
		os_free(ctx);
		wpa_printf(MSG_WARNING, "RSN: Failed to initialize EAPOL " "state machines for pre-authentication");
		ret = -3;
		goto fail;
	}
	...
	eloop_register_timeout(sm->dot11RSNAConfigSATimeout, 0, rsn_preauth_timeout, sm, NULL);
...
void rsn_preauth_candidate_process(struct wpa_sm *sm)
{
	...
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: processing PMKSA candidate " "list");
	if (sm->preauth_eapol || sm->proto != WPA_PROTO_RSN || wpa_sm_get_state(sm) != WPA_COMPLETED || (sm->key_mgmt != WPA_KEY_MGMT_IEEE8021X && sm->key_mgmt != WPA_KEY_MGMT_IEEE8021X_SHA256 && sm->key_mgmt != WPA_KEY_MGMT_IEEE8021X_SUITE_B && sm->key_mgmt != WPA_KEY_MGMT_IEEE8021X_SUITE_B_192)) {
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: not in suitable " "state for new pre-authentication");
		return;					/* invalid state for new pre-auth */
	}

	dl_list_for_each_safe(candidate, n, &sm->pmksa_candidates, struct rsn_pmksa_candidate, list) {
		struct rsn_pmksa_cache_entry *p = NULL;
		p = pmksa_cache_get(sm->pmksa, candidate->bssid, NULL, NULL);
		if (os_memcmp(sm->bssid, candidate->bssid, ETH_ALEN) != 0 && (p == NULL || p->opportunistic)) {
			wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: PMKSA " "candidate " MACSTR " selected for pre-authentication", MAC2STR(candidate->bssid));
			dl_list_del(&candidate->list);
			rsn_preauth_init(sm, candidate->bssid, sm->eap_conf_ctx);
			os_free(candidate);
			return;
		}
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: PMKSA candidate " MACSTR " does not need pre-authentication anymore", MAC2STR(candidate->bssid));
	...
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: no more pending PMKSA " "candidates");
}
...
void pmksa_candidate_add(struct wpa_sm *sm, const u8 *bssid, int prio, int preauth)
{
	...
	if (!preauth) {
		wpa_printf(MSG_DEBUG, "RSN: Ignored PMKID candidate without " "preauth flag");
		return;
	}
	...
	cand = NULL;
	dl_list_for_each(pos, &sm->pmksa_candidates, struct rsn_pmksa_candidate, list) {
	...
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: added PMKSA cache " "candidate " MACSTR " prio %d", MAC2STR(bssid), prio);
	rsn_preauth_candidate_process(sm);
}
...
void rsn_preauth_scan_result(struct wpa_sm *sm, const u8 *bssid, const u8 *ssid, const u8 *rsn)
{
	...
	if (ssid[1] != sm->ssid_len || os_memcmp(ssid + 2, sm->ssid, sm->ssid_len) != 0) {
	...
	if (pmksa && (!pmksa->opportunistic || !(ie.capabilities & WPA_CAPABILITY_PREAUTH))) {
		return;
	}

	/* Give less priority to candidates found from normal scan results. */
	pmksa_candidate_add(sm, bssid, PMKID_CANDIDATE_PRIO_SCAN, ie.capabilities & WPA_CAPABILITY_PREAUTH);
}
...
int rsn_preauth_get_status(struct wpa_sm *sm, char *buf, size_t buflen, int verbose)
{
	...
	if (sm->preauth_eapol) {
		ret = os_snprintf(pos, end - pos, "Pre-authentication " "EAPOL state machines:\n");
