...
static int wpa_config_validate_network(struct wpa_ssid *ssid, int line)
{
	...
	if ((ssid->group_cipher & WPA_CIPHER_CCMP) && !(ssid->pairwise_cipher & WPA_CIPHER_CCMP) && !(ssid->pairwise_cipher & WPA_CIPHER_NONE)) {
		...
		wpa_printf(MSG_DEBUG, "Line %d: removed CCMP from group cipher" " list since it was not allowed for pairwise " "cipher", line);
...
static struct wpa_cred *wpa_config_read_cred(FILE *f, int *line, int id)
{
	...
	while (wpa_config_get_line(buf, sizeof(buf), f, line, &pos)) {
		...
		pos2 = os_strchr(pos, '=');
		if (pos2 == NULL) {
			wpa_printf(MSG_ERROR, "Line %d: Invalid cred line " "'%s'.", *line, pos);
			errors++;
			continue;
		}
	...
	if (!end) {
		wpa_printf(MSG_ERROR, "Line %d: cred block was not " "terminated properly.", *line);
		errors++;
	}
...
static struct wpa_config_blob *wpa_config_read_blob(FILE *f, int *line, const char *name)
{
	...
	wpa_printf(MSG_MSGDUMP, "Line: %d - start of a new named blob '%s'", *line, name);
	...
	while (wpa_config_get_line(buf, sizeof(buf), f, line, &pos)) {
		...
		if (nencoded == NULL) {
			wpa_printf(MSG_ERROR, "Line %d: not enough memory for " "blob", *line);
			os_free(encoded);
			return NULL;
		}
		encoded = nencoded;
	...
	if (!end) {
		wpa_printf(MSG_ERROR, "Line %d: blob was not terminated " "properly", *line);
		os_free(encoded);
		return NULL;
	}
	...
	while (wpa_config_get_line(buf, sizeof(buf), f, &line, &pos)) {
		if (os_strcmp(pos, "network={") == 0) {
			...
			if (wpa_config_process_blob(config, f, &line, pos + 12)
				< 0) {
				wpa_printf(MSG_ERROR, "Line %d: failed to " "process blob.", line);
				errors++;
				continue;
			}
#endif							/* CONFIG_NO_CONFIG_BLOBS */
...
static void write_global_bin(FILE *f, const char *field, const struct wpabuf *val)
{
	size_t i;
	const u8 *pos;
...
static void wpa_config_write_global(FILE *f, struct wpa_config *config)
{
	...
	if (config->p2p_pref_chan) {
		...
		for (i = 0; i < config->num_p2p_pref_chan; i++) {
			fprintf(f, "%s%u:%u", i > 0 ? "," : "", config->p2p_pref_chan[i].op_class, config->p2p_pref_chan[i].chan);
		}
		fprintf(f, "\n");
	}
...
#endif							/* ANDROID */
		struct stat buffer;
		if (stat(orig_name, &buffer) == 0) {
			/* if file already exist then delete */
			if (unlink(orig_name) != 0) {
				wpa_printf(MSG_ERROR, "Remove error=%d", errno);
			}
		}
		if (chmod_ret != 0 || rename(tmp_name, orig_name) != 0) {
			wpa_printf(MSG_ERROR, "Rename error=%d", errno);
			ret = -1;
		}
		os_free(tmp_name);
	}

	wpa_printf(MSG_DEBUG, "Configuration file '%s' written %ssuccessfully", orig_name, ret ? "un" : "");
	return ret;
