...
static void wpa_supplicant_set_config_blob(void *ctx, struct wpa_config_blob *blob)
{
	...
	if (wpa_s->conf->update_config) {
		...
		if (ret) {
			wpa_printf(MSG_DEBUG, "Failed to update config after " "blob set");
		}
	}
}

static const struct wpa_config_blob *wpa_supplicant_get_config_blob(void *ctx, const char *name)
{
	struct wpa_supplicant *wpa_s = ctx;
 ...
 */
static int wpa_supplicant_eapol_send(void *ctx, int type, const u8 *buf, size_t len)
{
	struct wpa_supplicant *wpa_s = ctx;
	...
	if (wpa_key_mgmt_wpa_psk(wpa_s->key_mgmt) || wpa_s->key_mgmt == WPA_KEY_MGMT_NONE) {
		/* Current SSID is not using IEEE 802.1X/EAP, so drop possible
		 * EAPOL frames (mainly, EAPOL-Start) from EAPOL state
		 * machines. */
		wpa_printf(MSG_DEBUG, "WPA: drop TX EAPOL in non-IEEE 802.1X " "mode (type=%d len=%lu)", type, (unsigned long)len);
		return -1;
	}
	...
	if (pmksa_cache_get_current(wpa_s->wpa) && type == IEEE802_1X_TYPE_EAPOL_START) {
		...
		if (!wpa_s->current_ssid || wpa_s->current_ssid->eap_workaround) {
			wpa_printf(MSG_DEBUG, "RSN: Timeout on waiting for the AP to initiate 4-way handshake for PMKSA caching or EAP authentication - try to force it to start EAP authentication");
		} else {
	...
	if (is_zero_ether_addr(wpa_s->bssid)) {
		wpa_printf(MSG_DEBUG, "BSSID not set when trying to send an " "EAPOL frame");
		if (wpa_drv_get_bssid(wpa_s, bssid) == 0 && !is_zero_ether_addr(bssid)) {
			dst = bssid;
			wpa_printf(MSG_DEBUG, "Using current BSSID " MACSTR " from the driver as the EAPOL destination", MAC2STR(dst));
		} else {
			dst = wpa_s->last_eapol_src;
			wpa_printf(MSG_DEBUG, "Using the source address of the" " last received EAPOL frame " MACSTR " as " "the EAPOL destination", MAC2STR(dst));
		}
	} else {
 ...
 */
static int wpa_eapol_set_wep_key(void *ctx, int unicast, int keyidx, const u8 *key, size_t keylen)
{
	struct wpa_supplicant *wpa_s = ctx;
	...
	return wpa_drv_set_key(wpa_s, WPA_ALG_WEP, unicast ? wpa_s->bssid : NULL, keyidx, unicast, NULL, 0, key, keylen);
}
...
static void wpa_supplicant_eapol_cb(struct eapol_sm *eapol, enum eapol_supp_result result, void *ctx)
{
	struct wpa_supplicant *wpa_s = ctx;
	int res, pmk_len;
	u8 pmk[PMK_LEN];

	wpa_printf(MSG_DEBUG, "EAPOL authentication completed - result=%s", result_str(result));
	...
	if (result != EAPOL_SUPP_RESULT_SUCCESS || !(wpa_s->drv_flags & WPA_DRIVER_FLAGS_4WAY_HANDSHAKE)) {
		return;
	}
	...
	wpa_printf(MSG_DEBUG, "Configure PMK for driver-based RSN 4-way " "handshake");

	pmk_len = PMK_LEN;
	if (wpa_key_mgmt_ft(wpa_s->key_mgmt)) {
#ifdef CONFIG_IEEE80211R
		u8 buf[2 * PMK_LEN];
		wpa_printf(MSG_DEBUG, "RSN: Use FT XXKey as PMK for " "driver-based 4-way hs and FT");
	...
	wpa_hexdump_key(MSG_DEBUG, "RSN: Configure PMK for driver-based 4-way " "handshake", pmk, pmk_len);
...
static int wpa_get_beacon_ie(struct wpa_supplicant *wpa_s)
{
	...
	dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) {
		...
		if (ssid == NULL || ((bss->ssid_len == ssid->ssid_len && os_memcmp(bss->ssid, ssid->ssid, ssid->ssid_len) == 0) || ssid->ssid_len == 0)) {
			curr = bss;
			break;
		}
	}
...
static int wpa_supplicant_set_key(void *_wpa_s, enum wpa_alg alg, const u8 *addr, int key_idx, int set_tx, const u8 *seq, size_t seq_len, const u8 *key, size_t key_len)
{
	...
	if (key_idx > 0 && addr && is_broadcast_ether_addr(addr) && alg != WPA_ALG_NONE && key_len <= sizeof(wpa_s->last_gtk)) {
		os_memcpy(wpa_s->last_gtk, key, key_len);
		wpa_s->last_gtk_len = key_len;
	}
#endif							/* CONFIG_TESTING_GET_GTK */
	return wpa_drv_set_key(wpa_s, alg, addr, key_idx, set_tx, seq, seq_len, key, key_len);
}
...
#ifdef CONFIG_IEEE80211R
static int wpa_supplicant_update_ft_ies(void *ctx, const u8 *md, const u8 *ies, size_t ies_len)
{
	struct wpa_supplicant *wpa_s = ctx;
...
static int wpa_supplicant_send_ft_action(void *ctx, u8 action, const u8 *target_ap, const u8 *ies, size_t ies_len)
{
	struct wpa_supplicant *wpa_s = ctx;
	int ret;
	u8 *data, *pos;
	...
	*pos++ = action;
	...
	ret = wpa_drv_send_action(wpa_s, wpa_s->assoc_freq, 0, wpa_s->bssid, wpa_s->own_addr, wpa_s->bssid, data, data_len, 0);
	os_free(data);
...
static int wpa_supplicant_tdls_get_capa(void *ctx, int *tdls_supported, int *tdls_ext_setup, int *tdls_chan_switch)
{
	struct wpa_supplicant *wpa_s = ctx;
...
static int wpa_supplicant_send_tdls_mgmt(void *ctx, const u8 *dst, u8 action_code, u8 dialog_token, u16 status_code, u32 peer_capab, int initiator, const u8 *buf, size_t len)
{
	struct wpa_supplicant *wpa_s = ctx;
	return wpa_drv_send_tdls_mgmt(wpa_s, dst, action_code, dialog_token, status_code, peer_capab, initiator, buf, len);
}
...
static int wpa_supplicant_tdls_enable_channel_switch(void *ctx, const u8 *addr, u8 oper_class, const struct hostapd_freq_params *params)
{
	struct wpa_supplicant *wpa_s = ctx;
...
void wpas_send_ctrl_req(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid, const char *field_name, const char *txt)
{
	...
	len = os_snprintf(buf, buflen, "%s-%d:%s needed for SSID ", field_name, ssid->id, txt);
...
static void wpa_supplicant_eap_param_needed(void *ctx, enum wpa_ctrl_req_type field, const char *default_txt)
{
	struct wpa_supplicant *wpa_s = ctx;
	struct wpa_ssid *ssid = wpa_s->current_ssid;
...
#ifdef CONFIG_AP
	if (wpa_s->ap_iface) {
		wpa_printf(MSG_DEBUG, "AP mode active - skip EAPOL Supplicant " "port status: %s", authorized ? "Authorized" : "Unauthorized");
		return;
	}
#endif							/* CONFIG_AP */
...
static void wpa_supplicant_cert_cb(void *ctx, int depth, const char *subject, const char *altsubject[], int num_altsubject, const char *cert_hash, const struct wpabuf *cert)
{
	struct wpa_supplicant *wpa_s = ctx;

	wpas_notify_certification(wpa_s, depth, subject, altsubject, num_altsubject, cert_hash, cert);
}

static void wpa_supplicant_status_cb(void *ctx, const char *status, const char *parameter)
{
	struct wpa_supplicant *wpa_s = ctx;
...
static void wpa_supplicant_set_anon_id(void *ctx, const u8 *id, size_t len)
{
	...
	if (wpa_s->conf->update_config) {
		...
		if (res) {
			wpa_printf(MSG_DEBUG, "Failed to update config after " "anonymous_id update");
		}
	}
}
#endif							/* IEEE8021X_EAPOL */
...
#ifndef CONFIG_NO_WPA
static void wpa_supplicant_set_rekey_offload(void *ctx, const u8 *kek, size_t kek_len, const u8 *kck, size_t kck_len, const u8 *replay_ctr)
{
	struct wpa_supplicant *wpa_s = ctx;
...
static int wpa_supplicant_key_mgmt_set_pmk(void *ctx, const u8 *pmk, size_t pmk_len)
{
	struct wpa_supplicant *wpa_s = ctx;

	if (wpa_s->conf->key_mgmt_offload && (wpa_s->drv_flags & WPA_DRIVER_FLAGS_KEY_MGMT_OFFLOAD)) {
		return wpa_drv_set_key(wpa_s, WPA_ALG_PMK, NULL, 0, 0, NULL, 0, pmk, pmk_len);
	} else {
		return 0;
	}
}
...
#ifdef IEEE8021X_EAPOL
		conf.proactive_key_caching = ssid->proactive_key_caching < 0 ? wpa_s->conf->okc : ssid->proactive_key_caching;
...
#ifdef CONFIG_P2P
		if (ssid->p2p_group && wpa_s->current_bss && !wpa_s->p2p_disable_ip_addr_req) {
			struct wpabuf *p2p;
