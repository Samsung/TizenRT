...
static void bgscan_learn_add_neighbor(struct bgscan_learn_bss *bss, const u8 *bssid)
{
	u8 *n;
...
static struct bgscan_learn_bss *bgscan_learn_get_bss(struct bgscan_learn_data *data, const u8 *bssid)
{
...
static void bgscan_learn_save(struct bgscan_learn_data *data)
{
	...
	dl_list_for_each(bss, &data->bss, struct bgscan_learn_bss, list) {
		...
		for (i = 0; i < bss->num_neigh; i++) {
			fprintf(f, "NEIGHBOR " MACSTR " " MACSTR "\n", MAC2STR(bss->bssid), MAC2STR(bss->neigh + i * ETH_ALEN));
		}
	}
...
static int *bgscan_learn_get_freqs(struct bgscan_learn_data *data, size_t *count)
{
	struct bgscan_learn_bss *bss;
	int *freqs = NULL, *n;
...
static int *bgscan_learn_get_probe_freq(struct bgscan_learn_data *data, int *freqs, size_t count)
{
	int idx, *n;
	...
	do {
		if (!in_array(freqs, data->supp_freqs[idx])) {
			wpa_printf(MSG_DEBUG, "bgscan learn: Probe new freq " "%u", data->supp_freqs[idx]);
...
static void bgscan_learn_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (data->ssid->scan_freq) {
		...
		wpa_printf(MSG_DEBUG, "bgscan learn: BSSes in this ESS have " "been seen on %u channels", (unsigned int)count);
...
static int bgscan_learn_get_params(struct bgscan_learn_data *data, const char *params)
{
	const char *pos;
	...
	pos = os_strchr(pos, ':');
	if (pos == NULL) {
		wpa_printf(MSG_ERROR, "bgscan learn: Missing scan interval " "for high signal");
		return -1;
	}
	pos++;
...
static int bgscan_learn_bss_match(struct bgscan_learn_data *data, struct wpa_scan_res *bss)
{
	const u8 *ie;
	...
	if (data->ssid->ssid_len != ie[1] || os_memcmp(data->ssid->ssid, ie + 2, ie[1]) != 0) {
...
static int bgscan_learn_notify_scan(void *priv, struct wpa_scan_results *scan_res)
{
	struct bgscan_learn_data *data = priv;
	size_t i, j;
#define MAX_BSS 50
	...
	wpa_printf(MSG_DEBUG, "bgscan learn: %u matching BSSes in scan " "results", (unsigned int)num_bssid);
	...
	for (i = 0; i < scan_res->num; i++) {
		...
		if (bss && bss->freq != res->freq) {
			wpa_printf(MSG_DEBUG, "bgscan learn: Update BSS " MACSTR " freq %d -> %d", MAC2STR(res->bssid), bss->freq, res->freq);
			bss->freq = res->freq;
		} else if (!bss) {
			wpa_printf(MSG_DEBUG, "bgscan learn: Add BSS " MACSTR " freq=%d", MAC2STR(res->bssid), res->freq);
			bss = os_zalloc(sizeof(*bss));
			if (!bss) {
				continue;
			}
...
static void bgscan_learn_notify_signal_change(void *priv, int above, int current_signal, int current_noise, int current_txrate)
{
	struct bgscan_learn_data *data = priv;
	int scan = 0;
	struct os_reltime now;

	if (data->short_interval == data->long_interval || data->signal_threshold == 0) {
		return;
	}

	wpa_printf(MSG_DEBUG, "bgscan learn: signal level changed " "(above=%d current_signal=%d current_noise=%d " "current_txrate=%d)", above, current_signal, current_noise, current_txrate);
	if (data->scan_interval == data->long_interval && !above) {
		...
		wpa_printf(MSG_DEBUG, "bgscan learn: Start using long bgscan " "interval");
		data->scan_interval = data->long_interval;
		eloop_cancel_timeout(bgscan_learn_timeout, data, NULL);
		eloop_register_timeout(data->scan_interval, 0, bgscan_learn_timeout, data, NULL);
	} else if (!above) {
