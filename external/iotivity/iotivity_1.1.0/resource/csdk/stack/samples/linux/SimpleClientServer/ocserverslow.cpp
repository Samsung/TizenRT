...
OCRepPayload *constructResponse(OCEntityHandlerRequest *ehRequest)
{
  ...
  if (OC_REST_PUT == ehRequest->method)
  {
    ...
    {
      OIC_LOG(DEBUG, TAG, ("Incoming payload not a representation"));
      return nullptr;
    }
  ...
  if (!response)
  {
    OIC_LOG_V(DEBUG, TAG, "Memory allocation for response payload failed.");
  }
...
void ProcessGetPutRequest(OCEntityHandlerRequest *ehRequest)
{
  ...
  if (!getResp)
  {
    OIC_LOG(DEBUG, TAG, "Failed to construct response");
    return;
  }
  ...
  {
    OIC_LOG(DEBUG, TAG, "Error sending response");
  }
...
OCEntityHandlerRequest *CopyRequest(OCEntityHandlerRequest *entityHandlerRequest)
{
  ...
  if (copyOfRequest)
  {
    ...
    if (copyOfRequest->query)
    {
      ...
      if (!copyOfRequest->query)
      {
        OIC_LOG(DEBUG, TAG, "Copy failed due to allocation failure");
  ...
  {
    OIC_LOG(DEBUG, TAG, "Error copying client request");
  }
                                        ...
                                        OCEntityHandlerRequest *entityHandlerRequest, void * /*callbackParam*/)
{
  ...
  if (flag & OC_REQUEST_FLAG)
  {
    ...
    if (entityHandlerRequest)
    {
      ...
      {
        OIC_LOG(DEBUG, TAG, "Error queuing request for slow response");
        // Indicate to the stack that this is a slow response
        result = OC_EH_ERROR;
      }
    }
    ...
    {
      OIC_LOG(DEBUG, TAG, "Invalid request");
      result = OC_EH_ERROR;
    }
  }
  return result;
...
int createLEDResource(char *uri, LEDResource *ledResource, bool resourceState, int resourcePower)
{
  if (!uri)
  {
    OIC_LOG(DEBUG, TAG, "Resource URI cannot be NULL");
    return -1;
  }
