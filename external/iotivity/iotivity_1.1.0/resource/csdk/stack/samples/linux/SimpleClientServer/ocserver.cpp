...
OCRepPayload *getPayload(const char *uri, int64_t power, bool state)
{
  ...
  if (!payload)
  {
    OIC_LOG(DEBUG, TAG, PCF("Failed to allocate Payload"));
    return nullptr;
  }
...
OCRepPayload *constructResponse(OCEntityHandlerRequest *ehRequest)
{
  ...
  {
    OIC_LOG(DEBUG, TAG, PCF("Incoming payload not a representation"));
    return nullptr;
  }
                                        ...
                                        OCRepPayload **payload)
{
  ...
  if (queryPassed)
  {
    ...
    if (!getResp)
    {
      OIC_LOG(DEBUG, TAG, "constructResponse failed");
      return OC_EH_ERROR;
    }
                                        ...
                                        OCRepPayload **payload)
{
  ...
  if (!putResp)
  {
    OIC_LOG(DEBUG, TAG, "Failed to construct Json response");
    return OC_EH_ERROR;
  }
...
void *ChangeLightRepresentation(void *param)
{
  ...
  while (!gQuitFlag)
  {
    ...
    if (gLightUnderObservation)
    {
      ...
      {
        OIC_LOG(DEBUG, TAG, "Incorrect notification type selected");
      }
    }
  }
  return NULL;
...
void *presenceNotificationGenerator(void *param)
{
  ...
  for (int i = 0; i < numPresenceResources; i++)
  {
    ...
    if (res != OC_STACK_OK)
    {
      OIC_LOG_V(DEBUG, TAG, "\"Presence Notification Generator\" failed to create resource "
  ...
  for (int i = 0; i < numPresenceResources; i++)
  {
    ...
    if (res != OC_STACK_OK)
    {
      OIC_LOG_V(DEBUG, TAG, "\"Presence Notification Generator\" failed to delete "\
...
int main(int argc, char *argv[])
{
  ...
  {
    OIC_LOG(DEBUG, TAG, "OCStack presence/discovery error");
    return 0;
  }
#endif
