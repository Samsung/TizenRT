                                 ...
                                 uint8_t numOptions)
{
  ...
  if (ret != OC_STACK_OK)
  {
    OIC_LOG_V(DEBUG, TAG, "OCDoResource returns error %d with method %d", ret, method);
  }
#ifdef WITH_PRESENCE
                                  ...
                                  OCClientResponse *clientResponse)
{
  ...
  if (clientResponse)
  {
    ...
    if (gNumObserveNotifies > 15) //large number to test observing in DELETE case.
    {
      ...
      {
        OIC_LOG(DEBUG, TAG, "Cancelling with LOW QOS");
        if (OCCancel(handle, OC_LOW_QOS, NULL, 0) != OC_STACK_OK)
        {
          OIC_LOG(DEBUG, TAG, "Observe cancel error");
        }
        return OC_STACK_DELETE_TRANSACTION;
      }
      else if (TestCase == TEST_OBS_REQ_NON_CANCEL_IMM)
      {
        OIC_LOG(DEBUG, TAG, "Cancelling with HIGH QOS");
                                    ...
                                    OCClientResponse *clientResponse)
{
  ...
  if (clientResponse)
  {
    ...
    if (gNumPresenceNotifies == 20)
    {
      ...
      {
        OIC_LOG(DEBUG, TAG, "Presence cancel error");
      }
      return OC_STACK_DELETE_TRANSACTION;
    }
  }
...
int InitPlatformDiscovery(OCQualityOfService qos)
{
  ...
  if (ret != OC_STACK_OK)
  {
    OIC_LOG(DEBUG, TAG, "OCStack device error");
  }
