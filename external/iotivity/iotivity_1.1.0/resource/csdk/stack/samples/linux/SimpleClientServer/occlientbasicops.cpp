...
#include "../../../../../c_common/oic_malloc/include/oic_malloc.h"
#include "../../../../../c_common/oic_string/include/oic_string.h"
                                 ...
                                 OCHeaderOption *options, uint8_t numOptions)
{
  ...
  if (ret != OC_STACK_OK)
  {
    OIC_LOG_V(DEBUG, TAG, "OCDoResource returns error %d with method %d",
              ret, method);
  }
                                  ...
                                  OCClientResponse *clientResponse)
{
  ...
  {
    OIC_LOG(DEBUG, TAG, "<====Callback Context for PUT fail====>");
  }
  ...
  {
    OIC_LOG(DEBUG, TAG, "<====PUT Callback fail to receive clientResponse====>\n");
  }
                                   ...
                                   OCClientResponse *clientResponse)
{
  ...
  {
    OIC_LOG(DEBUG, TAG, "<====Callback Context for POST fail====>");
  }
  ...
  {
    OIC_LOG(DEBUG, TAG, "<====POST Callback fail to receive clientResponse====>\n");
  }
                                  ...
                                  OCClientResponse *clientResponse)
{
  ...
  {
    OIC_LOG(DEBUG, TAG, "<====Callback Context for GET fail====>");
  }
  ...
  {
    OIC_LOG(DEBUG, TAG, "<====GET Callback fail to receive clientResponse====>\n");
  }
  return OC_STACK_DELETE_TRANSACTION;
}
                                        ...
                                        OCClientResponse *clientResponse)
{
  ...
  {
    OIC_LOG(DEBUG, TAG, "<====DISCOVERY Callback fail to receive clientResponse====>\n");
  }
...
int InitPutRequest(OCQualityOfService qos)
{
  ...
  if (!resource)
  {
    OIC_LOG_V(DEBUG, TAG, "Resource null, can't do PUT request\n");
    return -1;
  }
  query << resource->uri;
...
int InitPostRequest(OCQualityOfService qos)
{
  ...
  if (!resource)
  {
    OIC_LOG_V(DEBUG, TAG, "Resource null, can't do POST request\n");
    return -1;
  }
  ...
  if (OC_STACK_OK != result)
  {
    ...
    OIC_LOG(DEBUG, TAG, "First POST call did not succeed");
  }
  ...
  if (OC_STACK_OK != result)
  {
    OIC_LOG(DEBUG, TAG, "Second POST call did not succeed");
  }
...
int InitGetRequest(OCQualityOfService qos)
{
  ...
  if (!resource)
  {
    OIC_LOG_V(DEBUG, TAG, "Resource null, can't do GET request\n");
    return -1;
  }
  query << resource->uri;
...
int InitDiscovery()
{
  ...
  if (UnicastDiscovery)
  {
    ...
    {
      OIC_LOG(DEBUG, TAG, "!! Bad input for IP address. !!");
  ...
  if (ret != OC_STACK_OK)
  {
    OIC_LOG(DEBUG, TAG, "OCStack resource error");
  }
  return ret;
}
...
#   define UUID_LENGTH 37
  ...
  while (res)
  {
    ...
    {
      OIC_LOG(DEBUG, TAG, "Could Not Retrieve the Server ID");
    }
                   ...
                   const OCClientResponse *clientResponse)
{
  ...
  {
    OIC_LOG(DEBUG, TAG, "Memory not allocated to ResourceNode");
    OICFree(sid_cpy);
    OICFree(uri_cpy);
    return -1;
  }
...
int main(int argc, char *argv[])
{
  ...
  {
    OIC_LOG(DEBUG, TAG, "OCStack init error");
    return 0;
  }
  ...
  while (!gQuitFlag)
  {
    ...
    {
      OIC_LOG(DEBUG, TAG, "OCStack process error");
      return 0;
    }
    sleep(2);
  }
  ...
  {
    OIC_LOG(DEBUG, TAG, "OCStack stop error");
  }
  return 0;
}
