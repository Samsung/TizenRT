...
static void rtc_clk_32k_enable_internal(int dac, int dres, int dbias)
{
	...
	CLEAR_PERI_REG_MASK(RTC_IO_XTAL_32K_PAD_REG, RTC_IO_X32P_RDE | RTC_IO_X32P_RUE | RTC_IO_X32N_RUE | RTC_IO_X32N_RDE | RTC_IO_X32N_MUX_SEL | RTC_IO_X32P_MUX_SEL);
...
void rtc_clk_slow_freq_set(rtc_slow_freq_t slow_freq)
{
	...
	REG_SET_FIELD(RTC_CNTL_CLK_CONF_REG, RTC_CNTL_DIG_XTAL32K_EN, (slow_freq == RTC_SLOW_FREQ_32K_XTAL) ? 1 : 0);
...
void rtc_clk_bbpll_set(rtc_xtal_freq_t xtal_freq, rtc_cpu_freq_t cpu_freq)
{
	...
	uint32_t delay_pll_en = (rtc_clk_slow_freq_get() == RTC_SLOW_FREQ_RTC) ? DELAY_PLL_ENABLE_WITH_150K : DELAY_PLL_ENABLE_WITH_32K;
...
{
	int freq = 0;
	if (s_cur_pll == RTC_PLL_NONE || (cpu_freq == RTC_CPU_FREQ_240M && s_cur_pll == RTC_PLL_320M) || (cpu_freq != RTC_CPU_FREQ_240M && s_cur_pll == RTC_PLL_480M)) {
...
void rtc_clk_cpu_freq_set(rtc_cpu_freq_t cpu_freq)
{
	...
	SET_PERI_REG_MASK(RTC_CNTL_OPTIONS0_REG, RTC_CNTL_BB_I2C_FORCE_PD | RTC_CNTL_BBPLL_FORCE_PD | RTC_CNTL_BBPLL_I2C_FORCE_PD);
	...
	if (cpu_freq == RTC_CPU_FREQ_XTAL) {
		...
		CLEAR_PERI_REG_MASK(RTC_CNTL_OPTIONS0_REG, RTC_CNTL_BIAS_I2C_FORCE_PD | RTC_CNTL_BB_I2C_FORCE_PD | RTC_CNTL_BBPLL_FORCE_PD | RTC_CNTL_BBPLL_I2C_FORCE_PD);
...
{
	return (val & 0xffff) == ((val >> 16) & 0xffff) && val != 0 && val != UINT32_MAX;
}
