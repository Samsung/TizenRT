 ...
 * @param[in]  ip_info: set a IP address of this interface
 *
 * @return ESP_OK
 ...
 *
 * Only station interface need to be brought up, since station interface will be shut down when disconnected.
 *
 ...
 *
 * Only station interface need to be shut down, since station interface will be brought up when connected.
 *
 ...
 *
 * There is an IP information copy in adapter library, if interface is up, get the IP information from
 * interface, otherwise get from copy.
 *
 * @param[in]   tcpip_if: the interface which we want to get an IP information
 * @param[out]  ip_info: If successful, an IP information will be returned in this argument.
 *
 * @return ESP_OK
 *         ESP_ERR_TCPIP_ADAPTER_INVALID_PARAMS
 */
 ...
 *
 * There is an IP information copy in adapter library, if interface is up, also set interface's IP.
 * DHCP client/server should be stopped before set an new IP information.
 *
 * This function is mainly used for setting  a static IP.
 *
 ...
 *
 * There is an DNS Server information copy in adapter library, set DNS Server for appointed interface and type.
 *
 ...
 *
 * There is an IPv6 information copy in adapter library, if interface is up,and IPv6 info
...
/**
 * @brief  Start a DHCP server
 *
 * @note   Currently a DHCP server is bind to softAP interface.
 *
 * @param[in]  tcpip_if: the interface with which we will start a DHCP server
 *
 * @return ESP_OK
 *         ESP_ERR_TCPIP_ADAPTER_INVALID_PARAMS
 *         ESP_ERR_TCPIP_ADAPTER_DHCP_ALREADY_STARTED
...
/**
 * @brief  Stop a DHCP server
 *
 * @note   Currently a DHCP server is bind to softAP interface.
 *
 * @param[in]  tcpip_if: the interface with which we will stop a DHCP server
 *
 * @return ESP_OK
 *         ESP_ERR_TCPIP_ADAPTER_INVALID_PARAMS
...
/**
 * @brief  Get a DHCP client status
 *
 * @param[in]  tcpip_if: the interface with which we will get status of a DHCP client
 * @param[out]  status: If successful, the status of a DHCP client will be return in this argument.
 *
 * @return ESP_OK
 */
 ...
 *
 * @note   This function is not implemented now.
 *
 * @param[in]  opt_op: option operate type, 1 for SET, 2 for GET.
 * @param[in]  opt_id: option index, 32 for ROUTER, 50 for IP POLL, 51 for LEASE TIME, 52 for REQUEST TIME
 * @param[in]  opt_val: option parameter
...
/**
 * @brief  Start a DHCP client
 *
 * @note   Currently a DHCP client is bind to station interface.
 *
 * @param[in]  tcpip_if: the interface with which we will start a DHCP client
 *
 * @return ESP_OK
 *         ESP_ERR_TCPIP_ADAPTER_INVALID_PARAMS
 *         ESP_ERR_TCPIP_ADAPTER_DHCP_ALREADY_STARTED
...
/**
 * @brief  Stop a DHCP client
 *
 * @note   Currently a DHCP client is bind to station interface.
 *
 * @param[in]  tcpip_if: the interface with which we will stop a DHCP client
 *
 * @return ESP_OK
 *         ESP_ERR_TCPIP_ADAPTER_INVALID_PARAMS
 *         ESP_ERR_TCPIP_ADAPTER_DHCP_ALREADY_STOPED
