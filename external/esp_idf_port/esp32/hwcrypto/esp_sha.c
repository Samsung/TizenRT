...
*/
static pthread_mutex_t memory_block_lock = PTHREAD_MUTEX_INITIALIZER;
...
{
	pthread_mutex_lock(&memory_block_lock);
}

void esp_sha_unlock_memory_block(void)
{
	pthread_mutex_unlock(&memory_block_lock);
}

/* Lock to hold when changing SHA engine state,
   allows checking of sha_engines_all_idle()
*/
static pthread_mutex_t state_change_lock = PTHREAD_MUTEX_INITIALIZER;

inline static bool sha_engines_all_idle()
{
	return !engine_states[0].in_use && !engine_states[1].in_use && !engine_states[2].in_use;
}
...
static void esp_sha_lock_engine_inner(sha_engine_state *engine)
{
	pthread_mutex_lock(&state_change_lock);
	...
	pthread_mutex_unlock(&state_change_lock);
}
...
{
	while (1) {
		if (DPORT_REG_READ(SHA_1_BUSY_REG) == 0 && DPORT_REG_READ(SHA_256_BUSY_REG) == 0 && DPORT_REG_READ(SHA_384_BUSY_REG) == 0 && DPORT_REG_READ(SHA_512_BUSY_REG) == 0) {
			break;
		}
	}
}
