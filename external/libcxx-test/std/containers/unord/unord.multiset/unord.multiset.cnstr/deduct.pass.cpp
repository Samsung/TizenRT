...
int tc_containers_unord_unord_multiset_unord_multiset_cnstr_deduct(void) {
    ...
    TC_ASSERT_EXPR(std::is_permutation(s.begin(), s.end(), std::begin(expected_s), std::end(expected_s)));
    }
    ...
    TC_ASSERT_EXPR(std::is_permutation(s.begin(), s.end(), std::begin(expected_s), std::end(expected_s)));
    TC_ASSERT_EXPR(s.get_allocator().get_id() == 40);
    }
    ...
    ASSERT_SAME_TYPE(decltype(s), decltype(source));
    TC_ASSERT_EXPR(s.size() == 0);
    TC_ASSERT_EXPR(s.get_allocator().get_id() == 41);
    }
    ...
    ASSERT_SAME_TYPE(decltype(s), decltype(source));
    TC_ASSERT_EXPR(s.size() == 0);
    TC_ASSERT_EXPR(s.get_allocator().get_id() == 42);
    }
    ...
    TC_ASSERT_EXPR(std::is_permutation(s.begin(), s.end(), std::begin(expected_s), std::end(expected_s)));
    TC_ASSERT_EXPR(s.get_allocator().get_id() == 43);
    }
    ...
    TC_ASSERT_EXPR(std::is_permutation(s.begin(), s.end(), std::begin(expected_s), std::end(expected_s)));
    TC_ASSERT_EXPR(s.get_allocator().get_id() == 44);
    }
