...
constexpr bool tests()
{
...

  {
    ...
    TC_ASSERT_EXPR(arr[0] == 'm');
    TC_ASSERT_EXPR(arr[1] == 'e');
    TC_ASSERT_EXPR(arr[2] == 'o');
    TC_ASSERT_EXPR(arr[3] == 'w');
    TC_ASSERT_EXPR(arr[4] == '\0');
  }
...

  {
    ...
    for (int i = 0; i < 3; ++i)
      TC_ASSERT_EXPR(arr[i].get() == i && source[i].get() == 0);
  }
...

  {
    ...
    TC_ASSERT_EXPR(arr[0].a == 3);
    TC_ASSERT_EXPR(arr[0].b == .1);
  }

  return true;
}

int tc_containers_sequences_array_array_creation_to_array(void) {
  tests();
  static_assert(tests(), "");
  return 0;
}
