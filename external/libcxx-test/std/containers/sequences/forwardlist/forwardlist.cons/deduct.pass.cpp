...
int tc_containers_sequences_forwardlist_forwardlist_cons_deduct(void) {
...
//  Test the explicit deduction guides
    ...
    TC_ASSERT_EXPR(std::equal(fwl.begin(), fwl.end(), std::begin(arr), std::end(arr)));
    }
    ...
    TC_ASSERT_EXPR(std::distance(fwl.begin(), fwl.end()) == 4); // no size for forward_list
    auto it = fwl.begin();
...
//  std::forward_list fwl(std::allocator<int>()); // deque (allocator &)
    ...
    TC_ASSERT_EXPR(std::distance(fwl.begin(), fwl.end()) == 1); // no size for forward_list
    }
    ...
    TC_ASSERT_EXPR(std::distance(fwl.begin(), fwl.end()) == 5); // no size for forward_list
    auto it = fwl.begin();
    std::advance(it, 2);
    ...
    TC_ASSERT_EXPR(std::distance(fwl.begin(), fwl.end()) == 0); // no size for forward_list
    }
