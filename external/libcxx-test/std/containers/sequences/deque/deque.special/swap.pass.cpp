...
void testN(int start, int N, int M)
{
    ...
    C c2_save = c2;
    swap(c1, c2);
    TC_ASSERT_EXPR(c1 == c2_save);
    TC_ASSERT_EXPR(c2 == c1_save);
...
int tc_containers_sequences_deque_deque_special_swap(void) {
    {
    ...
    }
    {
        ...
        swap(c1, c2);
        TC_ASSERT_EXPR((c1 == std::deque<int, A>(a2, a2+sizeof(a2)/sizeof(a2[0]))));
        TC_ASSERT_EXPR(c1.get_allocator().get_id() == 1);
        TC_ASSERT_EXPR((c2 == std::deque<int, A>(a1, a1+sizeof(a1)/sizeof(a1[0]))));
