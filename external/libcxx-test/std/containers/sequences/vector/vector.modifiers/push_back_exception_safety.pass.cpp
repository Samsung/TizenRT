...
int tc_containers_sequences_vector_vector_modifiers_push_back_exception_safety(void) {
    CMyClass instance(42);
    std::vector<CMyClass> vec;

    vec.push_back(instance);
    std::vector<CMyClass> vec2(vec);
    TC_ASSERT_EXPR(is_contiguous_container_asan_correct(vec));
    TC_ASSERT_EXPR(is_contiguous_container_asan_correct(vec2));
