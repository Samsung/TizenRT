...
TEST_CONSTEXPR_CXX20 bool test() {
    ...
    }
    {
        ...
        TC_ASSERT_EXPR(testComparisons(c1, c2, false, true));
    }
    {
    ...
    }
    {
        ...
        TC_ASSERT_EXPR(testComparisons(c1, c2, false, false));
    }
    {
        int items1[2] = {1, 2};
        int items2[3] = {1, 2, 0};
    ...
    {
        TC_ASSERT_EXPR((std::vector<int>() == std::vector<int>()));
        TC_ASSERT_EXPR(!(std::vector<int>() != std::vector<int>()));
        TC_ASSERT_EXPR(!(std::vector<int>() < std::vector<int>()));
        TC_ASSERT_EXPR((std::vector<int>() <= std::vector<int>()));
        TC_ASSERT_EXPR(!(std::vector<int>() > std::vector<int>()));
...
int tc_containers_sequences_vector_compare(void) {
    test();
#if TEST_STD_VER > 17
    static_assert(test());
#endif
