...
//  Test the explicit deduction guides
    ...
    TC_ASSERT_EXPR(std::equal(vec.begin(), vec.end(), std::begin(arr), std::end(arr)));
    }
    ...
    TC_ASSERT_EXPR(vec.size() == 4);
    TC_ASSERT_EXPR(vec[0] == INT_MAX);
    TC_ASSERT_EXPR(vec[1] == 1L);
    TC_ASSERT_EXPR(vec[2] == 2L);
    }
...
//  std::vector vec(std::allocator<int>()); // vector (allocator &)
    ...
    TC_ASSERT_EXPR(vec.size() == 5);
    TC_ASSERT_EXPR(vec[2] == 3U);
    }
...
//  A couple of vector<bool> tests, too!
    ...
    TC_ASSERT_EXPR(vec.size() == 3);
    TC_ASSERT_EXPR(vec[0] && vec[1] && vec[2]);
    }
...
int tc_containers_sequences_vector_vector_cons_deduct(void) {
    tests();
#if TEST_STD_VER > 17
    static_assert(tests());
#endif
