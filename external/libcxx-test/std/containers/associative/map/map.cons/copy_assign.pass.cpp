...
int tc_containers_associative_map_map_cons_copy_assign(void) {
    {
        ...
        TC_ASSERT_EXPR(*std::next(mo.begin()) == V(2, 1));
        TC_ASSERT_EXPR(*std::next(mo.begin(), 2) == V(3, 1));
    }
    {
        typedef std::pair<const int, double> V;
        const V ar[] =
    ...
    TC_ASSERT_EXPR(balanced_allocs());
    {
        typedef std::pair<const int, double> V;
        V ar[] =
        {
