...
int tc_containers_associative_multiset_multiset_cons_deduct(void) {
  {
    ...
    TC_ASSERT_EXPR(std::equal(s.begin(), s.end(), std::begin(expected_s),
                      std::end(expected_s)));
  }
...

  {
    ...
    TC_ASSERT_EXPR(std::equal(s.begin(), s.end(), std::begin(expected_s),
                      std::end(expected_s)));
    TC_ASSERT_EXPR(s.get_allocator().get_id() == 45);
  }
