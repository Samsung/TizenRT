...
constexpr void test_mdspan_types(const H& handle, const M& map, const A& acc) {
  ...
  TC_ASSERT_EXPR(MDS::rank() == MDS::extents_type::rank());

  // rank_dynamic()
  ASSERT_SAME_TYPE(decltype(m.rank_dynamic()), typename MDS::rank_type);
  ASSERT_NOEXCEPT(m.rank_dynamic());
  TC_ASSERT_EXPR(MDS::rank_dynamic() == MDS::extents_type::rank_dynamic());
  ...
  if constexpr (MDS::rank() > 0) {
    ...
    for (typename MDS::rank_type r = 0; r < MDS::rank(); r++) {
      ...
      TC_ASSERT_EXPR(MDS::static_extent(r) == MDS::extents_type::static_extent(r));
      ASSERT_SAME_TYPE(decltype(m.extent(r)), typename MDS::index_type);
      ASSERT_NOEXCEPT(m.extent(r));
      TC_ASSERT_EXPR(m.extent(r) == m.extents().extent(r));
      size *= m.extent(r);
    }
    TC_ASSERT_EXPR(m.size() == size);
  } else {
  ...
  TC_ASSERT_EXPR(m.empty() == (m.size() == 0));
  ...
  TC_ASSERT_EXPR(!noexcept(MDS::is_always_unique()));
  TC_ASSERT_EXPR(!noexcept(MDS::is_always_exhaustive()));
  TC_ASSERT_EXPR(!noexcept(MDS::is_always_strided()));
  TC_ASSERT_EXPR(!noexcept(m.is_unique()));
  TC_ASSERT_EXPR(!noexcept(m.is_exhaustive()));
  TC_ASSERT_EXPR(!noexcept(m.is_strided()));
  TC_ASSERT_EXPR(MDS::is_always_unique() == M::is_always_unique());
  TC_ASSERT_EXPR(MDS::is_always_exhaustive() == M::is_always_exhaustive());
  TC_ASSERT_EXPR(MDS::is_always_strided() == M::is_always_strided());
  TC_ASSERT_EXPR(m.is_unique() == map.is_unique());
  TC_ASSERT_EXPR(m.is_exhaustive() == map.is_exhaustive());
  TC_ASSERT_EXPR(m.is_strided() == map.is_strided());
  ...
  if constexpr (MDS::rank() > 0) {
    if (m.is_strided()) {
      for (typename MDS::rank_type r = 0; r < MDS::rank(); r++) {
        ...
        TC_ASSERT_EXPR(!noexcept(m.stride(r)));
        TC_ASSERT_EXPR(m.stride(r) == map.stride(r));
      }
    }
  }
...
int tc_containers_views_mdspan_mdspan_properties(void) {
  test();
  static_assert(test());
  return 0;
}
