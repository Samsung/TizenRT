...
// TODO enable for GCC 13, when the CI pipeline is switched, doesn't work with GCC 12
  ...
  if constexpr (std::is_same_v<Layout, std::layout_left>) {
...

    {
      ...
      std::span s(idx);
      TC_ASSERT_EXPR(!check_operator_constraints(std::mdspan(data, construct_mapping(Layout(), std::extents<int, D>(1))), s));
    }
    {
      std::array idx{IntConfig<o, o, t, t>(0)};
      std::span s(idx);
...
int tc_containers_views_mdspan_mdspan_index_operator(void) {
  test();
  static_assert(test());
