...
constexpr bool test() {
  ...
  struct A {};
  TC_ASSERT_EXPR((test_ctor<int, int*>()));
  TC_ASSERT_EXPR((test_ctor<int, sized_sentinel<int*>>()));
  TC_ASSERT_EXPR((test_ctor<A, A*>()));
  TC_ASSERT_EXPR((test_ctor<A, sized_sentinel<A*>>()));
  return true;
}
...
int tc_containers_views_views_span_span_cons_iterator_sentinel(void) {
  test();
#ifndef TEST_HAS_NO_EXCEPTIONS
  test_exceptions();
#endif
