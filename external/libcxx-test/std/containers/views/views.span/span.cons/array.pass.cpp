...
//  Types the same (dynamic sized)
    ...
    TC_ASSERT_EXPR(s1.size() + s2.size() + s3.size() + s4.size() == 12);
    }
...
//  types different (dynamic sized)
    ...
    TC_ASSERT_EXPR(s1.size() + s2.size() + s3.size() + s4.size() + s5.size() + s6.size() == 18);
    }
...
constexpr bool testSpan()
{
    ...
    TC_ASSERT_EXPR(s1.data() == val && s1.size() == 2);
    TC_ASSERT_EXPR(s2.data() == val && s2.size() == 2);
    TC_ASSERT_EXPR(s3.data() == val && s3.size() == 2);
    TC_ASSERT_EXPR(s4.data() == val && s4.size() == 2);

    std::span<const int> s5 = {{1,2}};
    std::span<const int, 2> s6 = {{1,2}};
    TC_ASSERT_EXPR(s5.size() == 2);  // and it dangles
    TC_ASSERT_EXPR(s6.size() == 2);  // and it dangles
...
int tc_containers_views_views_span_span_cons_array(void) {
    testSpan<int>();
    testSpan<double>();
    testSpan<A>();
    testSpan<std::string>();
