...
void testRuntimeSpan(Span sp)
{
    ...
    static_assert(S1::extent == Count);
    static_assert(S2::extent == std::dynamic_extent, "");
    TC_ASSERT_EXPR(s1.data() == s2.data());
    TC_ASSERT_EXPR(s1.size() == s2.size());
    TC_ASSERT_EXPR(std::equal(s1.begin(), s1.end(), sp.begin() + Offset));
}
...
void testRuntimeSpan(Span sp)
{
    ...
    TC_ASSERT_EXPR(s1.data() == s2.data());
    TC_ASSERT_EXPR(s1.size() == s2.size());
    TC_ASSERT_EXPR(std::equal(s1.begin(), s1.end(), sp.begin() + Offset, sp.end()));
}
...
int tc_containers_views_views_span_span_sub_subspan(void) {
    {
