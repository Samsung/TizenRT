...
int tc_utilities_optional_optional_object_optional_object_ctor_move(void) {
    ...
    }
    {
        ...
        TC_ASSERT_EXPR(rhs.has_value() == false);
        TC_ASSERT_EXPR(T::alive == 0);
    }
    TestTypes::TestType::reset();
    {
        ...
        TC_ASSERT_EXPR(rhs.has_value());
        TC_ASSERT_EXPR(lhs.value().value == 42);
        TC_ASSERT_EXPR(rhs.value().value == -1);
