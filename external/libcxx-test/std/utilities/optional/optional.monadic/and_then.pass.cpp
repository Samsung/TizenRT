...
constexpr void test_val_types() {
  // Test & overload
  {
    // Without & qualifier on F's operator()
    {
      ...
      TC_ASSERT_EXPR(i.and_then(LVal{}) == 1);
      TC_ASSERT_EXPR(i.and_then(NOLVal{}) == std::nullopt);
    ...
    //With & qualifier on F's operator()
    {
      std::optional<int> i{0};
      RefQual l{};
      TC_ASSERT_EXPR(i.and_then(l) == 1);
      NORefQual nl{};
      TC_ASSERT_EXPR(i.and_then(nl) == std::nullopt);
  ...
  // Test const& overload
  {
    // Without & qualifier on F's operator()
    {
      ...
      TC_ASSERT_EXPR(i.and_then(CLVal{}) == 1);
      TC_ASSERT_EXPR(i.and_then(NOCLVal{}) == std::nullopt);
  ...
  // Test && overload
  {
    ...
    {
      std::optional<int> i{0};
      TC_ASSERT_EXPR(std::move(i).and_then(RVal{}) == 1);
      TC_ASSERT_EXPR(std::move(i).and_then(NORVal{}) == std::nullopt);
    ...
    {
      std::optional<int> i{0};
      TC_ASSERT_EXPR(i.and_then(RVRefQual{}) == 1);
      TC_ASSERT_EXPR(i.and_then(NORVRefQual{}) == std::nullopt);
  ...
  // Test const&& overload
  {
    ...
    {
      const std::optional<int> i{0};
      TC_ASSERT_EXPR(std::move(i).and_then(CRVal{}) == 1);
      TC_ASSERT_EXPR(std::move(i).and_then(NOCRVal{}) == std::nullopt);
    ...
    //With & qualifier on F's operator()
    {
      const std::optional<int> i{0};
      const RVCRefQual l{};
      TC_ASSERT_EXPR(i.and_then(std::move(l)) == 1);
      const NORVCRefQual nl{};
      TC_ASSERT_EXPR(i.and_then(std::move(nl)) == std::nullopt);
...
int tc_utilities_optional_optional_monadic_and_then_pass(void) {
    test_full();
#if TEST_STD_VER > 23
