...
test_int_1()
{
    ...
    // member function pointer
    {
        ...
        A_int_1 a;
        TC_ASSERT_EXPR(r1(a) == 3);
        std::function<int (A_int_1*)> r2(fp);
        A_int_1* ap = &a;
        TC_ASSERT_EXPR(r2(ap) == 3);
    }
    // const member function pointer
    {
        ...
        std::function<int (A_int_1)> r1(fp);
        A_int_1 a;
        TC_ASSERT_EXPR(r1(a) == 4);
        std::function<int (A_int_1*)> r2(fp);
        A_int_1* ap = &a;
        TC_ASSERT_EXPR(r2(ap) == 4);
    }
    // member data pointer
    {
        ...
        std::function<int& (A_int_1&)> r1(fp);
        A_int_1 a;
        TC_ASSERT_EXPR(r1(a) == 5);
        r1(a) = 6;
        TC_ASSERT_EXPR(r1(a) == 6);
        std::function<int& (A_int_1*)> r2(fp);
        A_int_1* ap = &a;
        TC_ASSERT_EXPR(r2(ap) == 6);
        r2(ap) = 7;
        TC_ASSERT_EXPR(r2(ap) == 7);
    }
}
...
test_void_2()
{
    ...
    // function pointer
    {
        ...
        int j = 4;
        r1(i, j);
        TC_ASSERT_EXPR(count == save_count+7);
        save_count = count;
    }
    // functor
    {
        ...
        int j = 5;
        r1(i, j);
        TC_ASSERT_EXPR(count == save_count+9);
        save_count = count;
    }
    // member function pointer
    {
...
void test_int_2()
{
    ...
    // member function pointer
    {
        ...
        A_int_2 a;
        int i = 3;
        TC_ASSERT_EXPR(r1(a, i) == i+1);
        std::function<int (A_int_2*, int)> r2(fp);
        A_int_2* ap = &a;
        TC_ASSERT_EXPR(r2(ap, i) == i+1);
    }
    // const member function pointer
    {
        ...
        A_int_2 a;
        int i = 4;
        TC_ASSERT_EXPR(r1(a, i) == i+2);
        std::function<int (A_int_2*, int)> r2(fp);
        A_int_2* ap = &a;
        TC_ASSERT_EXPR(r2(ap, i) == i+2);
    }
}

int tc_utilities_function_objects_func_wrap_func_wrap_func_func_wrap_func_inv_invoke(void) {
    test_void_0();
    test_int_0();
    test_void_1();
    test_int_1();
