...
void test_counted()
{
    ...
    TC_ASSERT_EXPR(ret.first == It(values +1));
    TC_ASSERT_EXPR(ret.second == FIt(p +1));
    ...
    TC_ASSERT_EXPR(ret.first == It(values+N));
    TC_ASSERT_EXPR(ret.second == FIt(p + N));
...
int tc_utilities_memory_specialized_algorithms_uninitialized_move_uninitialized_move_n(void) {
    test_counted();
    test_ctor_throws();
