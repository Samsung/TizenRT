...
int tc_utilities_memory_util_smartptr_util_smartptr_weak_util_smartptr_ownerless_owner_less(void) {
    ...
    TC_ASSERT_EXPR(!cs(p1, p2));
    TC_ASSERT_EXPR(!cs(p2, p1));
    TC_ASSERT_EXPR(cs(p1 ,p3) || cs(p3, p1));
    TC_ASSERT_EXPR(cs(p3, p1) == cs(p3, p2));
    ASSERT_NOEXCEPT(cs(p1, p1));

    TC_ASSERT_EXPR(!cs(p1, w2));
    TC_ASSERT_EXPR(!cs(p2, w1));
    TC_ASSERT_EXPR(cs(p1, w3) || cs(p3, w1));
    TC_ASSERT_EXPR(cs(p3, w1) == cs(p3, w2));
    ...
    TC_ASSERT_EXPR(!cs(w1, w2));
    TC_ASSERT_EXPR(!cs(w2, w1));
    TC_ASSERT_EXPR(cs(w1, w3) || cs(w3, w1));
    TC_ASSERT_EXPR(cs(w3, w1) == cs(w3, w2));
    ASSERT_NOEXCEPT(cs(w1, w1));

    TC_ASSERT_EXPR(!cs(w1, p2));
    TC_ASSERT_EXPR(!cs(w2, p1));
    TC_ASSERT_EXPR(cs(w1, p3) || cs(w3, p1));
    TC_ASSERT_EXPR(cs(w3, p1) == cs(w3, p2));
    ASSERT_NOEXCEPT(cs(w1, p1));
    ASSERT_NOEXCEPT(cs(p1, w1));
    }
#if TEST_STD_VER > 14
    ...
    std::owner_less<> cmp;
    TC_ASSERT_EXPR(!cmp(sp1, sp2));
    TC_ASSERT_EXPR(!cmp(sp1, wp1));
    TC_ASSERT_EXPR(!cmp(sp1, sp3));
    TC_ASSERT_EXPR(!cmp(wp1, sp1));
    TC_ASSERT_EXPR(!cmp(wp1, wp1));
    ...
    TC_ASSERT_EXPR(s.find(vp) == s.end());
    }
#endif
