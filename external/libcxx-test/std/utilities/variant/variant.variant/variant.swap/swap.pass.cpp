...
struct NonThrowingNonNoexceptType {
  ...
  operator=(NonThrowingNonNoexceptType &&) noexcept(false) {
    TC_ASSERT_EXPR(false); // never called by the tests.
    return *this;
  }
  int value;
};
...
struct ThrowsOnSecondMove {
  ...
  ThrowsOnSecondMove &operator=(ThrowsOnSecondMove &&) {
    TC_ASSERT_EXPR(false); // not called by test
    return *this;
  }
};
...
void test_swap_valueless_by_exception() {
  ...
  { // both empty
    ...
    V v2;
    makeEmpty(v2);
    TC_ASSERT_EXPR(MakeEmptyT::alive == 0);
    { // member swap
      v1.swap(v2);
      TC_ASSERT_EXPR(v1.valueless_by_exception());
      TC_ASSERT_EXPR(v2.valueless_by_exception());
      TC_ASSERT_EXPR(MakeEmptyT::alive == 0);
    }
    { // non-member swap
      swap(v1, v2);
  ...
  { // only one empty
    ...
    { // member swap
      v1.swap(v2);
      TC_ASSERT_EXPR(v1.valueless_by_exception());
      TC_ASSERT_EXPR(std::get<0>(v2) == 42);
      // swap again
      v2.swap(v1);
      TC_ASSERT_EXPR(v2.valueless_by_exception());
      TC_ASSERT_EXPR(std::get<0>(v1) == 42);
    }
    { // non-member swap
      swap(v1, v2);
      TC_ASSERT_EXPR(v1.valueless_by_exception());
...
void test_swap_same_alternative() {
  {
    ...
    v1.swap(v2);
    TC_ASSERT_EXPR(T::swap_called == 1);
    TC_ASSERT_EXPR(std::get<0>(v1).value == 100);
    TC_ASSERT_EXPR(std::get<0>(v2).value == 42);
    swap(v1, v2);
    TC_ASSERT_EXPR(T::swap_called == 2);
    TC_ASSERT_EXPR(std::get<0>(v1).value == 42);
    TC_ASSERT_EXPR(std::get<0>(v2).value == 100);
  }
  {
    ...
    V v2(std::in_place_index<0>, 100);
    v1.swap(v2);
    TC_ASSERT_EXPR(T::swap_called == 0);
    TC_ASSERT_EXPR(T::move_called == 1);
    TC_ASSERT_EXPR(T::move_assign_called == 2);
    TC_ASSERT_EXPR(std::get<0>(v1).value == 100);
    TC_ASSERT_EXPR(std::get<0>(v2).value == 42);
    T::reset();
    swap(v1, v2);
...
#endif // _LIBCPP_NO_EXCEPTIONS
    TC_ASSERT_EXPR(T::swap_called == 1);
    TC_ASSERT_EXPR(T::move_called == 0);
    TC_ASSERT_EXPR(T::move_assign_called == 0);
    TC_ASSERT_EXPR(std::get<0>(v1).value == 42);
    TC_ASSERT_EXPR(std::get<0>(v2).value == 100);
  }
  {
...
#endif // _LIBCPP_NO_EXCEPTIONS
    TC_ASSERT_EXPR(T::move_called == 1); // call threw
    TC_ASSERT_EXPR(T::move_assign_called == 0);
    TC_ASSERT_EXPR(std::get<0>(v1).value ==
...
#endif // _LIBCPP_NO_EXCEPTIONS
    TC_ASSERT_EXPR(T::move_called == 1);
    TC_ASSERT_EXPR(T::move_assign_called == 1);  // call threw and didn't complete
    TC_ASSERT_EXPR(std::get<0>(v1).value == -1); // v1 was moved from
    TC_ASSERT_EXPR(std::get<0>(v2).value == 100);
  }
#endif
}
...
void test_swap_different_alternatives() {
  {
    ...
    TC_ASSERT_EXPR(T::move_called <= 2);
    TC_ASSERT_EXPR(T::move_assign_called == 0);
    TC_ASSERT_EXPR(std::get<1>(v1) == 100);
    TC_ASSERT_EXPR(std::get<0>(v2).value == 42);
    T::reset();
    swap(v1, v2);
    TC_ASSERT_EXPR(T::swap_called == 0);
    ...
    TC_ASSERT_EXPR(T::move_assign_called == 0);
    TC_ASSERT_EXPR(std::get<0>(v1).value == 42);
    TC_ASSERT_EXPR(std::get<1>(v2) == 100);
  }
#ifndef TEST_HAS_NO_EXCEPTIONS
  {
...
#endif // _LIBCPP_NO_EXCEPTIONS
    TC_ASSERT_EXPR(T1::swap_called == 0);
    TC_ASSERT_EXPR(T1::move_called == 1); // throws
    TC_ASSERT_EXPR(T1::move_assign_called == 0);
    // FIXME: libc++ shouldn't move from T2 here.
    LIBCPP_ASSERT(T2::move_called == 1);
    TC_ASSERT_EXPR(T2::move_called <= 1);
    TC_ASSERT_EXPR(std::get<0>(v1).value == 42);
    if (T2::move_called != 0)
      TC_ASSERT_EXPR(v2.valueless_by_exception());
    else
      TC_ASSERT_EXPR(std::get<1>(v2).value == 100);
  }
  {
    ...
    TC_ASSERT_EXPR(T1::move_called <= 1);
    TC_ASSERT_EXPR(T2::swap_called == 0);
    TC_ASSERT_EXPR(T2::move_called == 1); // throws
    TC_ASSERT_EXPR(T2::move_assign_called == 0);
    if (T1::move_called != 0)
      TC_ASSERT_EXPR(v1.valueless_by_exception());
    else
      TC_ASSERT_EXPR(std::get<0>(v1).value == 42);
...
// FIXME: The tests below are just very libc++ specific
#ifdef _LIBCPP_VERSION
  {
    ...
    V v2(std::in_place_index<1>, 100);
    v1.swap(v2);
    TC_ASSERT_EXPR(T2::move_called == 2);
    TC_ASSERT_EXPR(std::get<1>(v1).value == 100);
    TC_ASSERT_EXPR(std::get<0>(v2).value == 42);
    TC_ASSERT_EXPR(std::get<0>(v2).move_count == 1);
  }
  {
    using T1 = NonThrowingNonNoexceptType;
