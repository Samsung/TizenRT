...
void test_move_ctor_basic() {
  ...
  }
  {
    ...
    TC_ASSERT_EXPR(std::get<1>(v2) == 42);
  }
  {
  ...
  }
  {
    ...
    TC_ASSERT_EXPR(v2.index() == 1);
    TC_ASSERT_EXPR(std::get<1>(v2).value == 42);
  }
  {
    ...
    TC_ASSERT_EXPR(v2.index() == 0);
    TC_ASSERT_EXPR(std::get<0>(v).value == -1);
    TC_ASSERT_EXPR(std::get<0>(v2).value == 42);
  }
  {
    ...
    TC_ASSERT_EXPR(v2.index() == 1);
    TC_ASSERT_EXPR(std::get<1>(v).value == -1);
    TC_ASSERT_EXPR(std::get<1>(v2).value == 42);
  }
