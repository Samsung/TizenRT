...
void test_const_lvalue_get() {
  ...
  }
  {
    ...
    ASSERT_NOT_NOEXCEPT(std::get<int>(v));
    ASSERT_SAME_TYPE(decltype(std::get<int>(v)), const int &);
    TC_ASSERT_EXPR(std::get<int>(v) == 42);
  }
  {
    using V = std::variant<int, const long>;
    constexpr V v(42l);
  ...
  }
  {
    ...
    ASSERT_NOT_NOEXCEPT(std::get<const long>(v));
    ASSERT_SAME_TYPE(decltype(std::get<const long>(v)), const long &);
    TC_ASSERT_EXPR(std::get<const long>(v) == 42);
  }
// FIXME: Remove these once reference support is reinstated
#if !defined(TEST_VARIANT_HAS_NO_REFERENCES)
  {
    ...
    TC_ASSERT_EXPR(&std::get<int &>(v) == &x);
  }
  {
    using V = std::variant<int &&>;
    int x = 42;
...
void test_rvalue_get() {
  {
    ...
    TC_ASSERT_EXPR(std::get<int>(std::move(v)) == 42);
  }
  {
    using V = std::variant<int, const long>;
    V v(42l);
    ASSERT_SAME_TYPE(decltype(std::get<const long>(std::move(v))),
                     const long &&);
    TC_ASSERT_EXPR(std::get<const long>(std::move(v)) == 42);
  }
// FIXME: Remove these once reference support is reinstated
#if !defined(TEST_VARIANT_HAS_NO_REFERENCES)
  {
    ...
    V v(x);
    ASSERT_SAME_TYPE(decltype(std::get<int &>(std::move(v))), int &);
    TC_ASSERT_EXPR(&std::get<int &>(std::move(v)) == &x);
  }
  {
    using V = std::variant<const int &>;
    int x = 42;
    ...
    ASSERT_SAME_TYPE(decltype(std::get<const int &>(std::move(v))),
                     const int &);
    TC_ASSERT_EXPR(&std::get<const int &>(std::move(v)) == &x);
  }
  {
    using V = std::variant<int &&>;
    int x = 42;
...
void test_throws_for_all_value_categories() {
  ...
  { // lvalue test cases
    TC_ASSERT_EXPR(test(one, v0));
    TC_ASSERT_EXPR(test(zero, v1));
  }
  { // const lvalue test cases
    TC_ASSERT_EXPR(test(one, cv0));
    TC_ASSERT_EXPR(test(zero, cv1));
  }
  { // rvalue test cases
    TC_ASSERT_EXPR(test(one, std::move(v0)));
    TC_ASSERT_EXPR(test(zero, std::move(v1)));
  }
  { // const rvalue test cases
    TC_ASSERT_EXPR(test(one, std::move(cv0)));
    TC_ASSERT_EXPR(test(zero, std::move(cv1)));
  }
#endif
}
