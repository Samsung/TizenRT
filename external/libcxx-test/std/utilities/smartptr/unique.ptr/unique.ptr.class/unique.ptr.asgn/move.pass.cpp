...
TEST_CONSTEXPR_CXX23 void test_basic() {
  ...
  const int expect_alive = IsArray ? 5 : 1;
  {
    ...
    if (!TEST_IS_CONSTANT_EVALUATED)
      TC_ASSERT_EXPR(A::count == (expect_alive * 2));
    s2 = std::move(s1);
    if (!TEST_IS_CONSTANT_EVALUATED)
      TC_ASSERT_EXPR(A::count == expect_alive);
    TC_ASSERT_EXPR(s2.get() == p);
    TC_ASSERT_EXPR(s1.get() == 0);
  }
  if (!TEST_IS_CONSTANT_EVALUATED)
    TC_ASSERT_EXPR(A::count == 0);
  {
    ...
    TC_ASSERT_EXPR(s1.get_deleter().state() == 0);
  }
  if (!TEST_IS_CONSTANT_EVALUATED)
    TC_ASSERT_EXPR(A::count == 0);
  {
    ...
    if (!TEST_IS_CONSTANT_EVALUATED)
      TC_ASSERT_EXPR(A::count == expect_alive);
    TC_ASSERT_EXPR(d1.state() == 5);
    TC_ASSERT_EXPR(d2.state() == 5);
  }
  if (!TEST_IS_CONSTANT_EVALUATED)
    TC_ASSERT_EXPR(A::count == 0);
  {
...
int tc_utilities_smartptr_unique_ptr_unique_ptr_class_unique_ptr_asgn_move(void) {
  test();
#if TEST_STD_VER >= 23
  static_assert(test());
#endif
