...
void test_allocation_pattern(F do_pattern) {
  ...
  TC_ASSERT_EXPR(tr.successful_allocations.size() == tr.deallocations.size());
  TC_ASSERT_EXPR(std::is_permutation(
...
int tc_utilities_utility_mem_res_mem_res_pool_mem_res_pool_mem_unsync_deallocate_matches_allocate(void) {
