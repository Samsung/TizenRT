...
void test_return() {
  ...
  // assert not null
  { TC_ASSERT_EXPR(std::pmr::new_delete_resource()); }
  // assert same return value
  { TC_ASSERT_EXPR(std::pmr::new_delete_resource() == std::pmr::new_delete_resource()); }
}
...
int tc_utilities_utility_mem_res_mem_res_global_new_delete_resource(void) {
