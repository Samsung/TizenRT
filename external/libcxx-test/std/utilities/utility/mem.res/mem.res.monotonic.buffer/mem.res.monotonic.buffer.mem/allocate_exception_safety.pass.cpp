...
void test_exception_safety() {
  ...
  res            = r1.allocate(last_new_size, 16);
  TC_ASSERT_EXPR(res != buffer);
  TC_ASSERT_EXPR(globalMemCounter.checkNewCalledEq(2));
  TC_ASSERT_EXPR(globalMemCounter.checkDeleteCalledEq(0));

  mono1.release();
  TC_ASSERT_EXPR(globalMemCounter.checkNewCalledEq(2));
  TC_ASSERT_EXPR(globalMemCounter.checkDeleteCalledEq(2));
}

int tc_utilities_utility_mem_res_mem_res_monotonic_buffer_mem_res_monotonic_buffer_mem_allocate_exception_safety(void) {
