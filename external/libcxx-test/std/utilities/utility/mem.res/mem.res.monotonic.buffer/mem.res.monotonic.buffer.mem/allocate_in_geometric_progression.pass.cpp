...
void test_geometric_progression() {
  ...
  TC_ASSERT_EXPR(globalMemCounter.last_new_size >= next_buffer_size);
  ...
  while (new_called < 5) {
    ...
    if (globalMemCounter.new_called > new_called) {
      TC_ASSERT_EXPR(globalMemCounter.new_called == new_called + 1);
      TC_ASSERT_EXPR(globalMemCounter.last_new_size >= next_buffer_size);
      next_buffer_size = globalMemCounter.last_new_size + 1;
      new_called += 1;
    }
...
int tc_utilities_utility_mem_res_mem_res_monotonic_buffer_mem_res_monotonic_buffer_mem_allocate_in_geometric_progression(void) {
