...
void test_pmr_uses_alloc(Args&&... args) {
  ...
  std::pmr::memory_resource* M = &R;
  {
    ...
    TC_ASSERT_EXPR(doTestUsesAllocV0<T>(std::forward<Args>(args)...));
    TC_ASSERT_EXPR((doTestUsesAllocV1<T>(M, std::forward<Args>(args)...)));
    TC_ASSERT_EXPR((doTestUsesAllocV2<T>(M, std::forward<Args>(args)...)));
  }
  {
    // Test T(std::allocator_arg_t, Alloc const&, Args...) construction
    using T = UsesAllocatorV1<Alloc, sizeof...(Args)>;
    TC_ASSERT_EXPR((doTest<T>(UA_AllocArg, std::forward<Args>(args)...)));
  }
  {
    // Test T(Args..., Alloc const&) construction
    using T = UsesAllocatorV2<Alloc, sizeof...(Args)>;
    TC_ASSERT_EXPR((doTest<T>(UA_AllocLast, std::forward<Args>(args)...)));
  }
  {
...
void test_non_pmr_uses_alloc(AllocObj const& A, Args&&... args) {
  {
    using T = NotUsesAllocator<Alloc, sizeof...(Args)>;
    TC_ASSERT_EXPR(doTestUsesAllocV0<T>(std::forward<Args>(args)...));
    TC_ASSERT_EXPR((doTestUsesAllocV1<T>(A, std::forward<Args>(args)...)));
    TC_ASSERT_EXPR((doTestUsesAllocV2<T>(A, std::forward<Args>(args)...)));
  }
  {
    using T = UsesAllocatorV1<Alloc, sizeof...(Args)>;
    TC_ASSERT_EXPR(doTestUsesAllocV0<T>(std::forward<Args>(args)...));
...
int tc_utilities_utility_mem_res_mem_poly_allocator_class_mem_poly_allocator_mem_construct_types(void) {
