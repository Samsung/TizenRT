...
void test() {
...

  {
    ...
    TC_ASSERT_EXPR(last_size == 13);
    TC_ASSERT_EXPR(last_alignment == alignof(std::max_align_t));
  ...
  }
  {
    ...
    std::fill(ptr, ptr + 13, '0');
    TC_ASSERT_EXPR(last_size == 13);
    TC_ASSERT_EXPR(last_alignment == 64);
...
int tc_utilities_utility_mem_res_mem_poly_allocator_class_mem_poly_allocator_mem_allocate_deallocate_bytes(void) {
  test<std::byte>();
  test<S>();
