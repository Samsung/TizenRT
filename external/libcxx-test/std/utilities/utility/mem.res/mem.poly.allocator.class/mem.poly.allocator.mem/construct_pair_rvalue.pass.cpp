...
void test_pmr_uses_allocator(std::pair<TT, UU>&& p) {
  {
    ...
    TC_ASSERT_EXPR((doTest<T, U>(UA_None, UA_None, std::move(p))));
  }
  {
    using T = UsesAllocatorV1<Alloc, 1>;
    using U = UsesAllocatorV2<Alloc, 1>;
    TC_ASSERT_EXPR((doTest<T, U>(UA_AllocArg, UA_AllocLast, std::move(p))));
  }
  {
    using T = UsesAllocatorV2<Alloc, 1>;
    using U = UsesAllocatorV3<Alloc, 1>;
    TC_ASSERT_EXPR((doTest<T, U>(UA_AllocLast, UA_AllocArg, std::move(p))));
  }
  {
    using T = UsesAllocatorV3<Alloc, 1>;
    using U = NotUsesAllocator<Alloc, 1>;
    TC_ASSERT_EXPR((doTest<T, U>(UA_AllocArg, UA_None, std::move(p))));
  }
}
...
int tc_utilities_utility_mem_res_mem_poly_allocator_class_mem_poly_allocator_mem_construct_pair_rvalue(void) {
