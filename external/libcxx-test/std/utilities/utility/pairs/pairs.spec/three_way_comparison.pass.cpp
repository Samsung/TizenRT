...
constexpr bool test() {
  {
    ...
    TC_ASSERT_EXPR((P1(1, 1) <=> P2(1, 2)) == std::strong_ordering::less);
    TC_ASSERT_EXPR((P1(2, 1) <=> P2(1, 2)) == std::strong_ordering::greater);
    TC_ASSERT_EXPR((P1(0, 0) <=> P2(0, 0)) == std::strong_ordering::equal);
  }
  {
    ...
    TC_ASSERT_EXPR((P1(1, 1) <=> P2(1.0, 2.0)) == std::partial_ordering::less);
    TC_ASSERT_EXPR((P1(2, 1) <=> P2(1.0, 2.0)) == std::partial_ordering::greater);
    TC_ASSERT_EXPR((P1(0, 0) <=> P2(0.0, 0.0)) == std::partial_ordering::equivalent);
  }
  { static_assert(!HasSpaceship<std::pair<int, int>, std::pair<std::string, int>>); }
  {
    ...
    TC_ASSERT_EXPR((P(1, 1) <=> P(1, 2)) == std::strong_ordering::less);
    TC_ASSERT_EXPR((P(2, 1) <=> P(1, 2)) == std::strong_ordering::greater);
    TC_ASSERT_EXPR((P(0, 0) <=> P(0, 0)) == std::strong_ordering::equal);
  }
  {
    ...
    TC_ASSERT_EXPR((P(1, {1}) <=> P(1, {2})) == std::weak_ordering::less);
    TC_ASSERT_EXPR((P(2, {1}) <=> P(1, {2})) == std::weak_ordering::greater);
    TC_ASSERT_EXPR((P(0, {0}) <=> P(0, {0})) == std::weak_ordering::equivalent);
  }
  {
    ...
    TC_ASSERT_EXPR((P(1, 1.0) <=> P(1, 2.0)) == std::partial_ordering::less);
    TC_ASSERT_EXPR((P(1, 1.0) <=> P(1, 1.0)) == std::partial_ordering::equivalent);
    TC_ASSERT_EXPR((P(1, -0.0) <=> P(1, 0.0)) == std::partial_ordering::equivalent);
    TC_ASSERT_EXPR((P(1, 2.0) <=> P(1, 1.0)) == std::partial_ordering::greater);
    TC_ASSERT_EXPR((P(1, nan) <=> P(2, nan)) == std::partial_ordering::less);
    TC_ASSERT_EXPR((P(2, nan) <=> P(1, nan)) == std::partial_ordering::greater);
    TC_ASSERT_EXPR((P(1, nan) <=> P(1, nan)) == std::partial_ordering::unordered);
  }
  {
    using P              = std::pair<double, int>;
    constexpr double nan = std::numeric_limits<double>::quiet_NaN();
    ASSERT_SAME_TYPE(decltype(P() <=> P()), std::partial_ordering);
    TC_ASSERT_EXPR((P(2.0, 1) <=> P(1.0, 2)) == std::partial_ordering::greater);
    TC_ASSERT_EXPR((P(1.0, 1) <=> P(1.0, 2)) == std::partial_ordering::less);
    TC_ASSERT_EXPR((P(nan, 1) <=> P(nan, 2)) == std::partial_ordering::unordered);
  }
  {
    struct NoRelative {
  ...
  {
    {
      using P              = std::pair<int, double>;
      constexpr double nan = std::numeric_limits<double>::quiet_NaN();
      TC_ASSERT_EXPR((P(1, 2.0) <=> P(1, nan)) == std::partial_ordering::unordered);
    }
    {
      using P              = std::pair<double, int>;
      constexpr double nan = std::numeric_limits<double>::quiet_NaN();
      TC_ASSERT_EXPR((P(1.0, 1) <=> P(nan, 2)) == std::partial_ordering::unordered);
    }
  }
...
int tc_utilities_utility_pairs_pairs_spec_three_way_comparison(void) {
  test();
  static_assert(test());
