...
constexpr bool test() {
  ...
  // The correct assignment operator of the underlying type is used
  {
    ...
    t2 = t1;
    TC_ASSERT_EXPR(t2.first.v.constCopyAssign == 1);
    TC_ASSERT_EXPR(t2.second.v.constCopyAssign == 1);
  }

  return true;
}

int tc_utilities_utility_pairs_pairs_pair_assign_const_copy_convert(void) {
  test();
