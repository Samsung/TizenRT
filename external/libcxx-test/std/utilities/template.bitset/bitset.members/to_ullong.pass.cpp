...
TEST_CONSTEXPR_CXX23 void test_to_ullong() {
    ...
    { // test values bigger than can fit into the bitset
        ...
        std::bitset<N> v(val);
        TC_ASSERT_EXPR(v.to_ullong() == (val & mask)); // we shouldn't return bit patterns from outside the limits of the bitset.
    }
}
...
int tc_utilities_template_bitset_bitset_members_to_ullong(void) {
  test();
#if TEST_STD_VER > 20
  static_assert(test());
#endif
