...
void test_perfect_forwarding() {
    {
        ...
        Tup tup;
        Tup const& ctup = tup;
        TC_ASSERT_EXPR(do_forwarding_test<>(tup));
        TC_ASSERT_EXPR(do_forwarding_test<>(ctup));
    }
    {
        using Tup = std::tuple<int>;
        Tup tup(42);
        Tup const& ctup = tup;
        TC_ASSERT_EXPR(do_forwarding_test<int&>(tup));
        TC_ASSERT_EXPR(do_forwarding_test<int const&>(ctup));
        TC_ASSERT_EXPR(do_forwarding_test<int&&>(std::move(tup)));
        TC_ASSERT_EXPR(do_forwarding_test<int const&&>(std::move(ctup)));
    }
    {
        ...
        Tup const& ctup = tup;
        TC_ASSERT_EXPR((do_forwarding_test<int&, const char*&, unsigned&>(tup)));
        TC_ASSERT_EXPR((do_forwarding_test<int&, const char* const&, unsigned &>(ctup)));
        TC_ASSERT_EXPR((do_forwarding_test<int&, const char*&&, unsigned&&>(std::move(tup))));
        TC_ASSERT_EXPR((do_forwarding_test<int&, const char* const&&, unsigned &&>(std::move(ctup))));
    }
    // test with pair<T, U>
    {
        ...
        Tup const& ctup = tup;
        TC_ASSERT_EXPR((do_forwarding_test<int&, const char*&>(tup)));
        TC_ASSERT_EXPR((do_forwarding_test<int&, const char* const&>(ctup)));
        TC_ASSERT_EXPR((do_forwarding_test<int&, const char*&&>(std::move(tup))));
        TC_ASSERT_EXPR((do_forwarding_test<int&, const char* const&&>(std::move(ctup))));
    }
    // test with array<T, I>
    {
        ...
        Tup tup = {42, 101, -1};
        Tup const& ctup = tup;
        TC_ASSERT_EXPR((do_forwarding_test<int&, int&, int&>(tup)));
        TC_ASSERT_EXPR((do_forwarding_test<int const&, int const&, int const&>(ctup)));
        TC_ASSERT_EXPR((do_forwarding_test<int&&, int&&, int&&>(std::move(tup))));
        TC_ASSERT_EXPR((do_forwarding_test<int const&&, int const&&, int const&&>(std::move(ctup))));
    }
}
...
int tc_utilities_tuple_tuple_tuple_tuple_apply_make_from_tuple(void) {
