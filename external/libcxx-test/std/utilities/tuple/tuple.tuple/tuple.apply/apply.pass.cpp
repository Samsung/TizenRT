...
void check_apply_quals_and_types(Tuple&& t) {
    ...
    TrackedCallable const& cobj = obj;
    {
        ...
        TC_ASSERT_EXPR(ret.quals == CQ_LValue);
        TC_ASSERT_EXPR(ret.arg_types == expect_args);
        TC_ASSERT_EXPR(ret.args == t);
    }
    {
        auto ret = std::apply(cobj, std::forward<Tuple>(t));
        TC_ASSERT_EXPR(ret.quals == CQ_ConstLValue);
        TC_ASSERT_EXPR(ret.arg_types == expect_args);
        TC_ASSERT_EXPR(ret.args == t);
    }
    {
        auto ret = std::apply(std::move(obj), std::forward<Tuple>(t));
        TC_ASSERT_EXPR(ret.quals == CQ_RValue);
        TC_ASSERT_EXPR(ret.arg_types == expect_args);
        TC_ASSERT_EXPR(ret.args == t);
    }
    {
        auto ret = std::apply(std::move(cobj), std::forward<Tuple>(t));
        TC_ASSERT_EXPR(ret.quals == CQ_ConstRValue);
        TC_ASSERT_EXPR(ret.arg_types == expect_args);
        TC_ASSERT_EXPR(ret.args == t);
    }
}
...
int tc_utilities_tuple_tuple_tuple_tuple_apply_apply(void) {
