...
constexpr bool test() {
  ...
  // make sure the right assignment operator of the type in the tuple is used
  {
    ...
    t2 = t1;
    TC_ASSERT_EXPR(std::get<0>(t2).v.constCopyAssign == 1);
    TC_ASSERT_EXPR(std::get<1>(t2).v.constCopyAssign == 1);
  }

  return true;
}

int tc_utilities_tuple_tuple_tuple_tuple_assign_const_pair_copy(void) {
  test();
