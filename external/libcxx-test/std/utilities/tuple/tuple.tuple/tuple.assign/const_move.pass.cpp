...
constexpr bool test() {
  // reference types
  {
    ...
    TC_ASSERT_EXPR(std::get<1>(t2) == 3.0);
  }
  ...
  // user defined const move assignment
  {
    ...
    t2 = std::move(t1);
    TC_ASSERT_EXPR(std::get<0>(t2).val == 1);
  }
  ...
  // make sure the right assignment operator of the type in the tuple is used
  {
    ...
    t2 = std::move(t1);
    TC_ASSERT_EXPR(std::get<0>(t2).constMoveAssign == 1);
    TC_ASSERT_EXPR(std::get<1>(t2).constCopyAssign == 1);
  }

  return true;
}

int tc_utilities_tuple_tuple_tuple_tuple_assign_const_move(void) {
  test();
