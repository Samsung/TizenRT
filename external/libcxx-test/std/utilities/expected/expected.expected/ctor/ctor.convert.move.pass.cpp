...
constexpr bool test() {
  // convert the value
  {
    ...
    TC_ASSERT_EXPR(e2.value().data.get() == 5);
    TC_ASSERT_EXPR(e1.has_value());
    TC_ASSERT_EXPR(e1.value().get() == 0);
  }
  ...
  // convert the error
  {
    ...
    TC_ASSERT_EXPR(!e2.has_value());
    TC_ASSERT_EXPR(e2.error().data.get() == 5);
    TC_ASSERT_EXPR(!e1.has_value());
    TC_ASSERT_EXPR(e1.error().get() == 0);
  }
...
int tc_utilities_expected_expected_expected_ctor_ctor_convert_move(void) {
  test();
  static_assert(test());
  testException();
  return 0;
