...
void test_no_inner_alloc()
{
    ...
    P.reset();
    {
        ...
        const PairIn in(x, y);
        A.construct(ptr, in);
        TC_ASSERT_EXPR(checkConstruct<int const&>(ptr->first, UA_AllocArg, CA));
...
void test_with_inner_alloc()
{
    ...
    AllocController PInner;
    {
        ...
        TC_ASSERT_EXPR(checkConstruct<int const&>(ptr->second, UA_AllocLast));
#if TEST_STD_VER >= 20
    ...
    POuter.reset();
    {
        ...
        const PairIn in(x, y);
        A.construct(ptr, in);
        TC_ASSERT_EXPR(checkConstruct<int const&>(ptr->first, UA_AllocArg, I));
        TC_ASSERT_EXPR(checkConstruct<int const&>(ptr->second, UA_None));
#if TEST_STD_VER >= 20
        TC_ASSERT_EXPR((POuter.checkConstruct<std::piecewise_construct_t&&,
...
int tc_utilities_allocator_adaptor_allocator_adaptor_members_construct_pair_const_lvalue_pair(void) {
    test_no_inner_alloc();
