...
struct G
{
    ...
    G(std::allocator_arg_t, allocator_type&&) { TC_ASSERT_EXPR(false); }
    G(const allocator_type&) { constructed = true; }
};

bool G::constructed = false;

int tc_utilities_allocator_adaptor_allocator_adaptor_members_construct(void) {
...

    {
        ...
        a.construct(s, 1, 2);
        TC_ASSERT_EXPR(S::constructed);
        TC_ASSERT_EXPR(A3<E>::constructed);
        s->~S();
    }

    {
        ...
        a.construct(s, 1, 2);
        TC_ASSERT_EXPR(!S::constructed);
        TC_ASSERT_EXPR(A3<F>::constructed);
        s->~S();
    }
