...
void test(
    std::basic_format_args<std::basic_format_context<OutIt, CharT>> args) {
  {
    ...
    *context.out() = CharT('c');
    ...
    TC_ASSERT_EXPR(str[0].size() == 1);
    TC_ASSERT_EXPR(str[0].front() == CharT('a'));
    TC_ASSERT_EXPR(str[1].size() == 1);
    TC_ASSERT_EXPR(str[1].front() == CharT('b'));
    TC_ASSERT_EXPR(str[2].size() == 1);
    TC_ASSERT_EXPR(str[2].front() == CharT('c'));
  }
}
...
int tc_utilities_format_format_formatter_format_context_format_context_advance_to(void) {
  test();
