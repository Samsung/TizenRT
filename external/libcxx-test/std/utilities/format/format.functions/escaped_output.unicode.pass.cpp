...
auto test_format_to_n =
    []<class CharT, class... Args>(
        std::basic_string_view<CharT> expected, test_format_string<CharT, Args...> fmt, Args&&... args) {
      {
        ...
        TC_ASSERT_EXPR(result.size == static_cast<std::ptrdiff_t>(expected.size()));
        TC_ASSERT_EXPR(result.out == out.end());
        TC_ASSERT_EXPR(out == expected);
      }
#ifndef TEST_HAS_NO_LOCALIZATION
      {
      ...
      }
      {
        ...
        TC_ASSERT_EXPR(result.size == static_cast<std::ptrdiff_t>(expected.size()));
        TC_ASSERT_EXPR(result.out == out.end());
        TC_ASSERT_EXPR(out == expected.substr(0, n));
      }
    };
...
int tc_utilities_format_format_functions_escaped_output_unicode(void) {
  test<char>();
#ifndef TEST_HAS_NO_WIDE_CHARACTERS
  test<wchar_t>();
#endif
