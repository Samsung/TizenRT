...
auto test =
    []<class CharT, class... Args>(
        std::basic_string_view<CharT> expected, test_format_string<CharT, Args...> fmt, Args&&... args) constexpr {
      {
        ...
        TC_ASSERT_EXPR(result.size == formatted_size);
        TC_ASSERT_EXPR(out.empty());
      }
      {
        ...
        TC_ASSERT_EXPR(result.size == formatted_size);
        TC_ASSERT_EXPR(std::equal(out.begin(), out.end(), expected.begin(), expected.begin() + size));
      }
      {
        ...
        TC_ASSERT_EXPR(result.size == formatted_size);
        TC_ASSERT_EXPR(out == expected.substr(0, size));
      }
      {
        ...
        TC_ASSERT_EXPR(result.size == formatted_size);
        TC_ASSERT_EXPR(result.out == out.begin() + size);
        TC_ASSERT_EXPR(out.substr(0, size) == expected.substr(0, size));
      }
      {
        ...
        TC_ASSERT_EXPR(result.size == formatted_size);
        TC_ASSERT_EXPR(result.out == buffer);
        TC_ASSERT_EXPR(buffer[0] == CharT(0));
      }
    };
...
int tc_utilities_format_format_functions_format_to_n_locale(void) {
