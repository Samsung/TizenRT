...
constexpr void test_comparator_invocation_count() {
  ...
  TC_ASSERT_EXPR(compare_invocation_count == 0);
  ...
  TC_ASSERT_EXPR(compare_invocation_count <= 3);
#else
  TC_ASSERT_EXPR(compare_invocation_count <= 6);
#endif
}
...
int tc_libcxx_algorithms_alg_sorting_alg_three_way_lexicographical_compare_three_way_comp(void) {
  test();
  static_assert(test());
