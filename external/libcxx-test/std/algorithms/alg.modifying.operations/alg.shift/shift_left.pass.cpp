...
constexpr bool test()
{
    ...
    for (int n = 0; n <= 15; ++n) {
        for (int k = 0; k <= n+2; ++k) {
            ...
            if (0 <= k && k < n) {
                TC_ASSERT_EXPR(it == Iter(work+n-k));
                TC_ASSERT_EXPR(std::equal(orig+k, orig+n, work, work+n-k));
            } else {
                TC_ASSERT_EXPR(it == Iter(work));
    ...
    // n > 0 && n < len
    {
        ...
        TC_ASSERT_EXPR(std::equal(std::begin(expected), std::end(expected), b, it));
    }
    {
...
int tc_algorithms_alg_modifying_operations_alg_shift_shift_left(void) {
