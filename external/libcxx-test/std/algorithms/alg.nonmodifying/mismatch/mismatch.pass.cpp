...
int tc_libcxx_algorithms_alg_nonmodifying_mismatch_mismatch(void) {
    ...
    TC_ASSERT_EXPR(std::mismatch(II(ia), II(ia + sa), II(ib))
            == (std::pair<II, II>(II(ia+3), II(ib+3))));

    TC_ASSERT_EXPR(std::mismatch(RAI(ia), RAI(ia + sa), RAI(ib))
            == (std::pair<RAI, RAI>(RAI(ia+3), RAI(ib+3))));

#if TEST_STD_VER > 11 // We have the four iteration version
    TC_ASSERT_EXPR(std::mismatch(II(ia), II(ia + sa), II(ib), II(ib+sb))
            == (std::pair<II, II>(II(ia+3), II(ib+3))));

    TC_ASSERT_EXPR(std::mismatch(RAI(ia), RAI(ia + sa), RAI(ib), RAI(ib+sb))
            == (std::pair<RAI, RAI>(RAI(ia+3), RAI(ib+3))));


    TC_ASSERT_EXPR(std::mismatch(II(ia), II(ia + sa), II(ib), II(ib+2))
