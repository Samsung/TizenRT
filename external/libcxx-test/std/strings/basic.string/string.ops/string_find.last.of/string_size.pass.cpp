...
static int test0()
{
    ...
    TC_ASSERT_FUNC((test(S("qanej"), S("dfkap"), 0, S::npos)));
    TC_ASSERT_FUNC((test(S("clbao"), S("ihqrfebgad"), 0, S::npos)));
    ...
    TC_ASSERT_FUNC((test(S("lhcdo"), S("oebqi"), 2, S::npos)));
    ...
    TC_ASSERT_FUNC((test(S("khbpm"), S("aobjd"), 4, 2)));
    TC_ASSERT_FUNC((test(S("pbsji"), S("pcbahntsje"), 4, 3)));
    TC_ASSERT_FUNC((test(S("mprdj"), S("fhepcrntkoagbmldqijs"), 4, 4)));
    TC_ASSERT_FUNC((test(S("eqmpa"), S(""), 5, S::npos)));
    TC_ASSERT_FUNC((test(S("omigs"), S("kocgb"), 5, 3)));
    TC_ASSERT_FUNC((test(S("onmje"), S("fbslrjiqkm"), 5, 3)));
    TC_ASSERT_FUNC((test(S("oqmrj"), S("jeidpcmalhfnqbgtrsko"), 5, 4)));
    TC_ASSERT_FUNC((test(S("schfa"), S(""), 6, S::npos)));
    TC_ASSERT_FUNC((test(S("igdsc"), S("qngpd"), 6, 2)));
    TC_ASSERT_FUNC((test(S("brqgo"), S("rodhqklgmb"), 6, 4)));
    TC_ASSERT_FUNC((test(S("tnrph"), S("thdjgafrlbkoiqcspmne"), 6, 4)));
    ...
    TC_ASSERT_FUNC((test(S("gfshlcmdjr"), S("nadkh"), 1, S::npos)));
    ...
    TC_ASSERT_FUNC((test(S("drtasbgmfp"), S("ktsrmnqagd"), 9, 7)));
    TC_ASSERT_FUNC((test(S("lsaijeqhtr"), S("rtdhgcisbnmoaqkfpjle"), 9, 9)));
    TC_ASSERT_FUNC((test(S("elgofjmbrq"), S(""), 10, S::npos)));
    TC_ASSERT_FUNC((test(S("mjqdgalkpc"), S("dplqa"), 10, 8)));
    TC_ASSERT_FUNC((test(S("kthqnfcerm"), S("dkacjoptns"), 10, 6)));
    TC_ASSERT_FUNC((test(S("dfsjhanorc"), S("hqfimtrgnbekpdcsjalo"), 10, 9)));
    TC_ASSERT_FUNC((test(S("eqsgalomhb"), S(""), 11, S::npos)));
    TC_ASSERT_FUNC((test(S("akiteljmoh"), S("lofbc"), 11, 8)));
    TC_ASSERT_FUNC((test(S("hlbdfreqjo"), S("astoegbfpn"), 11, 9)));
    TC_ASSERT_FUNC((test(S("taqobhlerg"), S("pdgreqomsncafklhtibj"), 11, 9)));
    TC_ASSERT_FUNC((test(S("snafbdlghrjkpqtoceim"), S(""), 0, S::npos)));
    TC_ASSERT_FUNC((test(S("aemtbrgcklhndjisfpoq"), S("lbtqd"), 0, S::npos)));
    ...
    TC_ASSERT_FUNC((test(S("lhosrngtmfjikbqpcade"), S("aqibs"), 1, S::npos)));
    ...
    TC_ASSERT_FUNC((test(S("copqdhstbingamjfkler"), S("djkqc"), 19, 16)));
    ...
    TC_ASSERT_FUNC((test(S("gjnhidfsepkrtaqbmclo"), S("nocfa"), 20, 19)));
    TC_ASSERT_FUNC((test(S("spocfaktqdbiejlhngmr"), S("bgtajmiedc"), 20, 18)));
    TC_ASSERT_FUNC((test(S("rphmlekgfscndtaobiqj"), S("lsckfnqgdahejiopbtmr"), 20, 19)));
    TC_ASSERT_FUNC((test(S("liatsqdoegkmfcnbhrpj"), S(""), 21, S::npos)));
    TC_ASSERT_FUNC((test(S("binjagtfldkrspcomqeh"), S("gfsrt"), 21, 12)));
    TC_ASSERT_FUNC((test(S("latkmisecnorjbfhqpdg"), S("pfsocbhjtm"), 21, 17)));
    TC_ASSERT_FUNC((test(S("lecfratdjkhnsmqpoigb"), S("tpflmdnoicjgkberhqsa"), 21, 19)));
    return 0;
}
...
static int test1()
{
    ...
    TC_ASSERT_FUNC((test(S("gmfhd"), S("kantesmpgj"), 1)));
    TC_ASSERT_FUNC((test(S("odaft"), S("oknlrstdpiqmjbaghcfe"), 4)));
    TC_ASSERT_FUNC((test(S("eolhfgpjqk"), S(""), S::npos)));
    TC_ASSERT_FUNC((test(S("nbatdlmekr"), S("bnrpe"), 9)));
    TC_ASSERT_FUNC((test(S("jdmciepkaq"), S("jtdaefblso"), 8)));
    TC_ASSERT_FUNC((test(S("hkbgspoflt"), S("oselktgbcapndfjihrmq"), 9)));
    TC_ASSERT_FUNC((test(S("gprdcokbnjhlsfmtieqa"), S(""), S::npos)));
    TC_ASSERT_FUNC((test(S("qjghlnftcaismkropdeb"), S("bjaht"), 19)));
    TC_ASSERT_FUNC((test(S("pnalfrdtkqcmojiesbhg"), S("hjlcmgpket"), 19)));
    TC_ASSERT_FUNC((test(S("pniotcfrhqsmgdkjbael"), S("htaobedqikfplcgjsmrn"), 19)));
    return 0;
}
