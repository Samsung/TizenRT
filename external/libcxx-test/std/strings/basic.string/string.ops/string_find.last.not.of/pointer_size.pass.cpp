...
static int test0()
{
    ...
    TC_ASSERT_FUNC((test(S("cdaih"), "dmajblfhsg", 1, 0)));
    ...
    TC_ASSERT_FUNC((test(S("qnsoh"), "kojhpmbsfe", 2, 1)));
    ...
    TC_ASSERT_FUNC((test(S("eqmpa"), "", 5, 4)));
    TC_ASSERT_FUNC((test(S("omigs"), "kocgb", 5, 4)));
    TC_ASSERT_FUNC((test(S("onmje"), "fbslrjiqkm", 5, 4)));
    TC_ASSERT_FUNC((test(S("oqmrj"), "jeidpcmalhfnqbgtrsko", 5, S::npos)));
    TC_ASSERT_FUNC((test(S("schfa"), "", 6, 4)));
    TC_ASSERT_FUNC((test(S("igdsc"), "qngpd", 6, 4)));
    ...
    TC_ASSERT_FUNC((test(S("daiprenocl"), "ashjd", 0, S::npos)));
    TC_ASSERT_FUNC((test(S("litpcfdghe"), "mgojkldsqh", 0, S::npos)));
    ...
    TC_ASSERT_FUNC((test(S("nkodajteqp"), "ofdrqmkebl", 1, 0)));
    ...
    TC_ASSERT_FUNC((test(S("qnmodrtkeb"), "pejafmnokr", 5, 4)));
    TC_ASSERT_FUNC((test(S("cpebqsfmnj"), "odnqkgijrhabfmcestlp", 5, S::npos)));
    TC_ASSERT_FUNC((test(S("lmofqdhpki"), "", 9, 9)));
    TC_ASSERT_FUNC((test(S("hnefkqimca"), "rtjpa", 9, 8)));
    TC_ASSERT_FUNC((test(S("drtasbgmfp"), "ktsrmnqagd", 9, 9)));
    TC_ASSERT_FUNC((test(S("lsaijeqhtr"), "rtdhgcisbnmoaqkfpjle", 9, S::npos)));
    TC_ASSERT_FUNC((test(S("elgofjmbrq"), "", 10, 9)));
    TC_ASSERT_FUNC((test(S("mjqdgalkpc"), "dplqa", 10, 9)));
    TC_ASSERT_FUNC((test(S("kthqnfcerm"), "dkacjoptns", 10, 9)));
    TC_ASSERT_FUNC((test(S("dfsjhanorc"), "hqfimtrgnbekpdcsjalo", 10, S::npos)));
    TC_ASSERT_FUNC((test(S("eqsgalomhb"), "", 11, 9)));
    TC_ASSERT_FUNC((test(S("akiteljmoh"), "lofbc", 11, 9)));
    TC_ASSERT_FUNC((test(S("hlbdfreqjo"), "astoegbfpn", 11, 8)));
    ...
    TC_ASSERT_FUNC((test(S("pnracgfkjdiholtbqsem"), "tboimldpjh", 0, S::npos)));
    ...
    TC_ASSERT_FUNC((test(S("rbtaqjhgkneisldpmfoc"), "gtfblmqinc", 1, 0)));
    TC_ASSERT_FUNC((test(S("gpifsqlrdkbonjtmheca"), "mkqpbtdalgniorhfescj", 1, S::npos)));
    TC_ASSERT_FUNC((test(S("hdpkobnsalmcfijregtq"), "", 10, 10)));
    TC_ASSERT_FUNC((test(S("jtlshdgqaiprkbcoenfm"), "pblas", 10, 9)));
    TC_ASSERT_FUNC((test(S("fkdrbqltsgmcoiphneaj"), "arosdhcfme", 10, 9)));
    ...
    TC_ASSERT_FUNC((test(S("mrtaefilpdsgocnhqbjk"), "lgokshjtpb", 19, 16)));
    TC_ASSERT_FUNC((test(S("kojatdhlcmigpbfrqnes"), "bqjhtkfepimcnsgrlado", 19, S::npos)));
    TC_ASSERT_FUNC((test(S("eaintpchlqsbdgrkjofm"), "", 20, 19)));
    TC_ASSERT_FUNC((test(S("gjnhidfsepkrtaqbmclo"), "nocfa", 20, 18)));
    TC_ASSERT_FUNC((test(S("spocfaktqdbiejlhngmr"), "bgtajmiedc", 20, 19)));
    TC_ASSERT_FUNC((test(S("rphmlekgfscndtaobiqj"), "lsckfnqgdahejiopbtmr", 20, S::npos)));
    TC_ASSERT_FUNC((test(S("liatsqdoegkmfcnbhrpj"), "", 21, 19)));
    TC_ASSERT_FUNC((test(S("binjagtfldkrspcomqeh"), "gfsrt", 21, 19)));
    TC_ASSERT_FUNC((test(S("latkmisecnorjbfhqpdg"), "pfsocbhjtm", 21, 19)));
...
static int test1()
{
    ...
    TC_ASSERT_FUNC((test(S("nhmko"), "", 4)));
    TC_ASSERT_FUNC((test(S("lahfb"), "irkhs", 4)));
    TC_ASSERT_FUNC((test(S("gmfhd"), "kantesmpgj", 4)));
    TC_ASSERT_FUNC((test(S("odaft"), "oknlrstdpiqmjbaghcfe", S::npos)));
    TC_ASSERT_FUNC((test(S("eolhfgpjqk"), "", 9)));
    TC_ASSERT_FUNC((test(S("nbatdlmekr"), "bnrpe", 8)));
    TC_ASSERT_FUNC((test(S("jdmciepkaq"), "jtdaefblso", 9)));
    TC_ASSERT_FUNC((test(S("hkbgspoflt"), "oselktgbcapndfjihrmq", S::npos)));
    TC_ASSERT_FUNC((test(S("gprdcokbnjhlsfmtieqa"), "", 19)));
    TC_ASSERT_FUNC((test(S("qjghlnftcaismkropdeb"), "bjaht", 18)));
    TC_ASSERT_FUNC((test(S("pnalfrdtkqcmojiesbhg"), "hjlcmgpket", 17)));
