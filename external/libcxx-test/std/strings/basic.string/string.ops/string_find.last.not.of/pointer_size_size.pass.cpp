...
{
    TC_ASSERT_EXPR(s.find_last_not_of(str, pos, n) == x);
    if (x != S::npos)
...
static int test0()
{
    ...
    TC_ASSERT_FUNC((test(S("eqkst"), "sgkec", 0, 4, S::npos)));
    ...
    TC_ASSERT_FUNC((test(S("ehmja"), "dabckmepqj", 0, 9, S::npos)));
    ...
    TC_ASSERT_FUNC((test(S("jfdam"), "joflqbdkhtegimscpanr", 1, 10, S::npos)));
    ...
    TC_ASSERT_FUNC((test(S("dirnm"), "tpdrchmkji", 2, 5, 1)));
    TC_ASSERT_FUNC((test(S("hrgdc"), "ijagfkblst", 2, 9, 1)));
    ...
    TC_ASSERT_FUNC((test(S("hoser"), "jfmpr", 4, 5, 3)));
    ...
    TC_ASSERT_FUNC((test(S("nbmit"), "rnfpqatdeo", 4, 9, 3)));
    TC_ASSERT_FUNC((test(S("jgmib"), "bntjlqrfik", 4, 10, 2)));
    TC_ASSERT_FUNC((test(S("ncrfj"), "kcrtmpolnaqejghsfdbi", 4, 0, 4)));
    TC_ASSERT_FUNC((test(S("ncsik"), "lobheanpkmqidsrtcfgj", 4, 1, 4)));
    TC_ASSERT_FUNC((test(S("sgbfh"), "athdkljcnreqbgpmisof", 4, 10, 3)));
    TC_ASSERT_FUNC((test(S("dktbn"), "qkdmjialrscpbhefgont", 4, 19, 2)));
    TC_ASSERT_FUNC((test(S("fthqm"), "dmasojntqleribkgfchp", 4, 20, S::npos)));
    TC_ASSERT_FUNC((test(S("klopi"), "", 5, 0, 4)));
    TC_ASSERT_FUNC((test(S("dajhn"), "psthd", 5, 0, 4)));
    TC_ASSERT_FUNC((test(S("jbgno"), "rpmjd", 5, 1, 4)));
    TC_ASSERT_FUNC((test(S("hkjae"), "dfsmk", 5, 2, 4)));
    return 0;
}
...
static int test1()
{
    TC_ASSERT_FUNC((test(S("gbhqo"), "skqne", 5, 4, 4)));
    TC_ASSERT_FUNC((test(S("ktdor"), "kipnf", 5, 5, 4)));
    TC_ASSERT_FUNC((test(S("ldprn"), "hmrnqdgifl", 5, 0, 4)));
    TC_ASSERT_FUNC((test(S("egmjk"), "fsmjcdairn", 5, 1, 4)));
    TC_ASSERT_FUNC((test(S("armql"), "pcdgltbrfj", 5, 5, 3)));
    TC_ASSERT_FUNC((test(S("cdhjo"), "aekfctpirg", 5, 9, 4)));
    TC_ASSERT_FUNC((test(S("jcons"), "ledihrsgpf", 5, 10, 3)));
    TC_ASSERT_FUNC((test(S("cbrkp"), "mqcklahsbtirgopefndj", 5, 0, 4)));
    TC_ASSERT_FUNC((test(S("fhgna"), "kmlthaoqgecrnpdbjfis", 5, 1, 4)));
    TC_ASSERT_FUNC((test(S("ejfcd"), "sfhbamcdptojlkrenqgi", 5, 10, 1)));
    TC_ASSERT_FUNC((test(S("kqjhe"), "pbniofmcedrkhlstgaqj", 5, 19, 2)));
    TC_ASSERT_FUNC((test(S("pbdjl"), "mongjratcskbhqiepfdl", 5, 20, S::npos)));
    TC_ASSERT_FUNC((test(S("gajqn"), "", 6, 0, 4)));
    TC_ASSERT_FUNC((test(S("stedk"), "hrnat", 6, 0, 4)));
    TC_ASSERT_FUNC((test(S("tjkaf"), "gsqdt", 6, 1, 4)));
    TC_ASSERT_FUNC((test(S("dthpe"), "bspkd", 6, 2, 4)));
    TC_ASSERT_FUNC((test(S("klhde"), "ohcmb", 6, 4, 4)));
    TC_ASSERT_FUNC((test(S("bhlki"), "heatr", 6, 5, 4)));
    TC_ASSERT_FUNC((test(S("lqmoh"), "pmblckedfn", 6, 0, 4)));
    TC_ASSERT_FUNC((test(S("mtqin"), "aceqmsrbik", 6, 1, 4)));
    TC_ASSERT_FUNC((test(S("dpqbr"), "lmbtdehjrn", 6, 5, 4)));
    TC_ASSERT_FUNC((test(S("kdhmo"), "teqmcrlgib", 6, 9, 4)));
    TC_ASSERT_FUNC((test(S("jblqp"), "njolbmspac", 6, 10, 3)));
    TC_ASSERT_FUNC((test(S("qmjgl"), "pofnhidklamecrbqjgst", 6, 0, 4)));
    TC_ASSERT_FUNC((test(S("rothp"), "jbhckmtgrqnosafedpli", 6, 1, 4)));
    TC_ASSERT_FUNC((test(S("ghknq"), "dobntpmqklicsahgjerf", 6, 10, 1)));
    ...
    TC_ASSERT_FUNC((test(S("pcofgeniam"), "qcpjibosfl", 0, 5, S::npos)));
    TC_ASSERT_FUNC((test(S("rlfjgesqhc"), "lrhmefnjcq", 0, 9, S::npos)));
    ...
    TC_ASSERT_FUNC((test(S("cfpegndlkt"), "cpmajdqnolikhgsbretf", 0, 19, S::npos)));
    ...
    TC_ASSERT_FUNC((test(S("getcrsaoji"), "ekosa", 1, 2, 0)));
    TC_ASSERT_FUNC((test(S("fjiknedcpq"), "anqhk", 1, 4, 1)));
    TC_ASSERT_FUNC((test(S("tkejgnafrm"), "jekca", 1, 5, 0)));
    TC_ASSERT_FUNC((test(S("jnakolqrde"), "ikemsjgacf", 1, 0, 1)));
    TC_ASSERT_FUNC((test(S("lcjptsmgbe"), "arolgsjkhm", 1, 1, 1)));
    TC_ASSERT_FUNC((test(S("itfsmcjorl"), "oftkbldhre", 1, 5, 0)));
    TC_ASSERT_FUNC((test(S("omchkfrjea"), "gbkqdoeftl", 1, 9, 1)));
    TC_ASSERT_FUNC((test(S("cigfqkated"), "sqcflrgtim", 1, 10, S::npos)));
    TC_ASSERT_FUNC((test(S("tscenjikml"), "fmhbkislrjdpanogqcet", 1, 0, 1)));
    TC_ASSERT_FUNC((test(S("qcpaemsinf"), "rnioadktqlgpbcjsmhef", 1, 1, 1)));
    TC_ASSERT_FUNC((test(S("gltkojeipd"), "oakgtnldpsefihqmjcbr", 1, 10, S::npos)));
    TC_ASSERT_FUNC((test(S("qistfrgnmp"), "gbnaelosidmcjqktfhpr", 1, 19, S::npos)));
    ...
    TC_ASSERT_FUNC((test(S("aqkocrbign"), "rjqdablmfs", 5, 1, 4)));
    TC_ASSERT_FUNC((test(S("ijsmdtqgce"), "enkgpbsjaq", 5, 5, 5)));
    TC_ASSERT_FUNC((test(S("clobgsrken"), "kdsgoaijfh", 5, 9, 3)));
    ...
    TC_ASSERT_FUNC((test(S("bpjlgmiedh"), "csehfgomljdqinbartkp", 5, 10, 1)));
    TC_ASSERT_FUNC((test(S("pamkeoidrj"), "qahoegcmplkfsjbdnitr", 5, 19, S::npos)));
    ...
    TC_ASSERT_FUNC((test(S("dirhtsnjkc"), "spqfoiclmtagejbndkrh", 9, 19, 3)));
    TC_ASSERT_FUNC((test(S("dlroktbcja"), "nmotklspigjrdhcfaebq", 9, 20, S::npos)));
    TC_ASSERT_FUNC((test(S("ncjpmaekbs"), "", 10, 0, 9)));
    TC_ASSERT_FUNC((test(S("hlbosgmrak"), "hpmsd", 10, 0, 9)));
    TC_ASSERT_FUNC((test(S("pqfhsgilen"), "qnpor", 10, 1, 9)));
    TC_ASSERT_FUNC((test(S("gqtjsbdckh"), "otdma", 10, 2, 9)));
    TC_ASSERT_FUNC((test(S("cfkqpjlegi"), "efhjg", 10, 4, 9)));
    TC_ASSERT_FUNC((test(S("beanrfodgj"), "odpte", 10, 5, 9)));
    TC_ASSERT_FUNC((test(S("adtkqpbjfi"), "bctdgfmolr", 10, 0, 9)));
    TC_ASSERT_FUNC((test(S("iomkfthagj"), "oaklidrbqg", 10, 1, 9)));
    return 0;
}
...
static int test2()
{
    TC_ASSERT_FUNC((test(S("sdpcilonqj"), "dnjfsagktr", 10, 5, 8)));
    TC_ASSERT_FUNC((test(S("gtfbdkqeml"), "nejaktmiqg", 10, 9, 9)));
    TC_ASSERT_FUNC((test(S("bmeqgcdorj"), "pjqonlebsf", 10, 10, 8)));
    TC_ASSERT_FUNC((test(S("etqlcanmob"), "dshmnbtolcjepgaikfqr", 10, 0, 9)));
    TC_ASSERT_FUNC((test(S("roqmkbdtia"), "iogfhpabtjkqlrnemcds", 10, 1, 9)));
    TC_ASSERT_FUNC((test(S("kadsithljf"), "ngridfabjsecpqltkmoh", 10, 10, 7)));
    TC_ASSERT_FUNC((test(S("sgtkpbfdmh"), "athmknplcgofrqejsdib", 10, 19, 5)));
    TC_ASSERT_FUNC((test(S("qgmetnabkl"), "ldobhmqcafnjtkeisgrp", 10, 20, S::npos)));
    TC_ASSERT_FUNC((test(S("cqjohampgd"), "", 11, 0, 9)));
    TC_ASSERT_FUNC((test(S("hobitmpsan"), "aocjb", 11, 0, 9)));
    TC_ASSERT_FUNC((test(S("tjehkpsalm"), "jbrnk", 11, 1, 9)));
    TC_ASSERT_FUNC((test(S("ngfbojitcl"), "tqedg", 11, 2, 9)));
    TC_ASSERT_FUNC((test(S("rcfkdbhgjo"), "nqskp", 11, 4, 9)));
    TC_ASSERT_FUNC((test(S("qghptonrea"), "eaqkl", 11, 5, 7)));
    TC_ASSERT_FUNC((test(S("hnprfgqjdl"), "reaoicljqm", 11, 0, 9)));
    TC_ASSERT_FUNC((test(S("hlmgabenti"), "lsftgajqpm", 11, 1, 9)));
    TC_ASSERT_FUNC((test(S("ofcjanmrbs"), "rlpfogmits", 11, 5, 9)));
    TC_ASSERT_FUNC((test(S("jqedtkornm"), "shkncmiaqj", 11, 9, 7)));
    TC_ASSERT_FUNC((test(S("rfedlasjmg"), "fpnatrhqgs", 11, 10, 8)));
    TC_ASSERT_FUNC((test(S("talpqjsgkm"), "sjclemqhnpdbgikarfot", 11, 0, 9)));
    TC_ASSERT_FUNC((test(S("lrkcbtqpie"), "otcmedjikgsfnqbrhpla", 11, 1, 9)));
    TC_ASSERT_FUNC((test(S("cipogdskjf"), "bonsaefdqiprkhlgtjcm", 11, 10, 8)));
    ...
    TC_ASSERT_FUNC((test(S("miklnresdgbhqcojftap"), "prabcjfqnoeskilmtgdh", 0, 19, S::npos)));
    ...
    TC_ASSERT_FUNC((test(S("jitlfrqemsdhkopncabg"), "ciboh", 1, 2, 0)));
    TC_ASSERT_FUNC((test(S("mhtaepscdnrjqgbkifol"), "haois", 1, 4, 0)));
    ...
    TC_ASSERT_FUNC((test(S("emgasrilpknqojhtbdcf"), "gdtfjchpmr", 1, 9, 0)));
    TC_ASSERT_FUNC((test(S("hnfiagdpcklrjetqbsom"), "ponmcqblet", 1, 10, 0)));
    ...
    TC_ASSERT_FUNC((test(S("sitodfgnrejlahcbmqkp"), "ligeojhafnkmrcsqtbdp", 1, 10, 0)));
    TC_ASSERT_FUNC((test(S("fraghmbiceknltjpqosd"), "lsimqfnjarbopedkhcgt", 1, 19, S::npos)));
    ...
    TC_ASSERT_FUNC((test(S("gqpskidtbclomahnrjfe"), "kdlmh", 10, 4, 9)));
    ...
    TC_ASSERT_FUNC((test(S("kpdbgjmtherlsfcqoina"), "eqbdrkcfah", 10, 9, 8)));
    ...
    TC_ASSERT_FUNC((test(S("ifeopcnrjbhkdgatmqls"), "stgbhfmdaljnpqoicker", 10, 10, 8)));
    TC_ASSERT_FUNC((test(S("ckqhaiesmjdnrgolbtpf"), "oihcetflbjagdsrkmqpn", 10, 19, S::npos)));
    ...
    TC_ASSERT_FUNC((test(S("kghbfipeomsntdalrqjc"), "dtcoj", 19, 5, 17)));
    ...
    TC_ASSERT_FUNC((test(S("rqnoclbdejgiphtfsakm"), "nlmcjaqgbsortfdihkpe", 19, 10, 18)));
    TC_ASSERT_FUNC((test(S("plkqbhmtfaeodjcrsing"), "racfnpmosldibqkghjet", 19, 19, 7)));
    TC_ASSERT_FUNC((test(S("oegalhmstjrfickpbndq"), "fjhdsctkqeiolagrnmbp", 19, 20, S::npos)));
    TC_ASSERT_FUNC((test(S("rdtgjcaohpblniekmsfq"), "", 20, 0, 19)));
    TC_ASSERT_FUNC((test(S("ofkqbnjetrmsaidphglc"), "ejanp", 20, 0, 19)));
    TC_ASSERT_FUNC((test(S("grkpahljcftesdmonqib"), "odife", 20, 1, 19)));
    TC_ASSERT_FUNC((test(S("jimlgbhfqkteospardcn"), "okaqd", 20, 2, 19)));
    TC_ASSERT_FUNC((test(S("gftenihpmslrjkqadcob"), "lcdbi", 20, 4, 18)));
    TC_ASSERT_FUNC((test(S("bmhldogtckrfsanijepq"), "fsqbj", 20, 5, 18)));
    TC_ASSERT_FUNC((test(S("nfqkrpjdesabgtlcmoih"), "bigdomnplq", 20, 0, 19)));
    TC_ASSERT_FUNC((test(S("focalnrpiqmdkstehbjg"), "apiblotgcd", 20, 1, 19)));
    TC_ASSERT_FUNC((test(S("rhqdspkmebiflcotnjga"), "acfhdenops", 20, 5, 18)));
    TC_ASSERT_FUNC((test(S("rahdtmsckfboqlpniegj"), "jopdeamcrk", 20, 9, 18)));
    TC_ASSERT_FUNC((test(S("fbkeiopclstmdqranjhg"), "trqncbkgmh", 20, 10, 17)));
    TC_ASSERT_FUNC((test(S("lifhpdgmbconstjeqark"), "tomglrkencbsfjqpihda", 20, 0, 19)));
    return 0;
}
...
static int test3()
{
    TC_ASSERT_FUNC((test(S("pboqganrhedjmltsicfk"), "gbkhdnpoietfcmrslajq", 20, 1, 19)));
    TC_ASSERT_FUNC((test(S("klchabsimetjnqgorfpd"), "rtfnmbsglkjaichoqedp", 20, 10, 19)));
    TC_ASSERT_FUNC((test(S("sirfgmjqhctndbklaepo"), "ohkmdpfqbsacrtjnlgei", 20, 19, 1)));
    TC_ASSERT_FUNC((test(S("rlbdsiceaonqjtfpghkm"), "dlbrteoisgphmkncajfq", 20, 20, S::npos)));
    TC_ASSERT_FUNC((test(S("ecgdanriptblhjfqskom"), "", 21, 0, 19)));
    TC_ASSERT_FUNC((test(S("fdmiarlpgcskbhoteqjn"), "sjrlo", 21, 0, 19)));
    TC_ASSERT_FUNC((test(S("rlbstjqopignecmfadkh"), "qjpor", 21, 1, 19)));
    TC_ASSERT_FUNC((test(S("grjpqmbshektdolcafni"), "odhfn", 21, 2, 19)));
    TC_ASSERT_FUNC((test(S("sakfcohtqnibprjmlged"), "qtfin", 21, 4, 19)));
    TC_ASSERT_FUNC((test(S("mjtdglasihqpocebrfkn"), "hpqfo", 21, 5, 19)));
    TC_ASSERT_FUNC((test(S("okaplfrntghqbmeicsdj"), "fabmertkos", 21, 0, 19)));
    TC_ASSERT_FUNC((test(S("sahngemrtcjidqbklfpo"), "brqtgkmaej", 21, 1, 19)));
    TC_ASSERT_FUNC((test(S("dlmsipcnekhbgoaftqjr"), "nfrdeihsgl", 21, 5, 18)));
    TC_ASSERT_FUNC((test(S("ahegrmqnoiklpfsdbcjt"), "hlfrosekpi", 21, 9, 19)));
    TC_ASSERT_FUNC((test(S("hdsjbnmlegtkqripacof"), "atgbkrjdsm", 21, 10, 19)));
    TC_ASSERT_FUNC((test(S("pcnedrfjihqbalkgtoms"), "blnrptjgqmaifsdkhoec", 21, 0, 19)));
    TC_ASSERT_FUNC((test(S("qjidealmtpskrbfhocng"), "ctpmdahebfqjgknloris", 21, 1, 19)));
    TC_ASSERT_FUNC((test(S("qeindtagmokpfhsclrbj"), "apnkeqthrmlbfodiscgj", 21, 10, 19)));
    TC_ASSERT_FUNC((test(S("kpfegbjhsrnodltqciam"), "jdgictpframeoqlsbknh", 21, 19, 7)));
