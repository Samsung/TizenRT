...
extern "C" {
	...
	int tc_strings_main(void)
	{
		...
		tc_strings_char_traits();
		usleep(1000);
		tc_strings_basic_string();
		usleep(1000);
		tc_strings_c_strings();
		usleep(1000);

		return 0;
	}

	// Menu function for STRINGS sub-tests (called from utc_libcxx.c)
	void tc_strings_menu(int argc, char *argv[])
	{
		// No arguments: run all strings tests
		if (argc <= 1) {
			tc_strings_main();
			return;
		}

		if (strcmp(argv[1], "chartraits") == 0) {
			printf("\nRunning char.traits tests...\n");
			tc_strings_char_traits();
		}
		else if (strcmp(argv[1], "basicstring") == 0) {
			printf("\nRunning basic.string tests...\n");
			tc_strings_basic_string();
		}
		else if (strcmp(argv[1], "cstrings") == 0) {
			printf("\nRunning c.strings tests...\n");
			tc_strings_c_strings();
		}
		else {
			printf("\nUnknown strings sub-category: %s\n", argv[1]);
			printf("Available: chartraits, basicstring, cstrings\n");
		}
	}
}
