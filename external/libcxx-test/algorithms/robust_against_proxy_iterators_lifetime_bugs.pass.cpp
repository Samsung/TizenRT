 ...
 public:
  struct Value {
    ...
    Value(const Value& rhs) : i_(rhs.i_) {
      TC_ASSERT_EXPR(lifetime_cache.contains(&rhs));
      TC_ASSERT_EXPR(!rhs.moved_from_);
    ...
    Value& operator=(const Value& rhs) {
      TC_ASSERT_EXPR(lifetime_cache.contains(this) && lifetime_cache.contains(&rhs));
      TC_ASSERT_EXPR(!rhs.moved_from_);
    ...
    friend bool operator<(const Value& lhs, const Value& rhs) {
      TC_ASSERT_EXPR(lifetime_cache.contains(&lhs) && lifetime_cache.contains(&rhs));
      TC_ASSERT_EXPR(!lhs.moved_from_ && !rhs.moved_from_);
  ...
  struct Reference {
    ...
    operator Value() const {
      TC_ASSERT_EXPR(lifetime_cache.contains(this));
      TC_ASSERT_EXPR(!moved_from_);
  ...
  friend LifetimeIterator operator+(const LifetimeIterator& lhs, difference_type n) {
    TC_ASSERT_EXPR(!lhs.moved_from_);
