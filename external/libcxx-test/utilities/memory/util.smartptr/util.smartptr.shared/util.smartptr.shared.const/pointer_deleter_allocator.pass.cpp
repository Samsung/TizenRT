...
int tc_utilities_memory_util_smartptr_util_smartptr_shared_util_smartptr_shared_const_pointer_deleter_allocator(void) {
    {
...

    {
        test_allocator_statistics alloc_stats;
        {
            ...
            TC_ASSERT_EXPR(p.use_count() == 1);
            TC_ASSERT_EXPR(p.get() == ptr);
            TC_ASSERT_EXPR(test_deleter<A const>::count == 1);
            TC_ASSERT_EXPR(test_deleter<A const>::dealloc_count == 0);
#ifndef TEST_HAS_NO_RTTI
        ...
        TC_ASSERT_EXPR(A::count == 0);
        TC_ASSERT_EXPR(test_deleter<A const>::count == 0);
        TC_ASSERT_EXPR(test_deleter<A const>::dealloc_count == 1);
        TC_ASSERT_EXPR(alloc_stats.count == 0);
        TC_ASSERT_EXPR(alloc_stats.alloc_count == 0);
