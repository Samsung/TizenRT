...
void test_deleter_value_category() {
  ...
  { // Test non-reference deleter conversions
    ...
    u1 = std::move(u2);
    TC_ASSERT_EXPR(checkArg<TD2&&>(u1.get_deleter()));
  }
  ...
  { // Test assignment to const&.
    ...
    u1.get_deleter().reset();
    u1 = std::move(u2);
    TC_ASSERT_EXPR(checkArg<CD2&&>(u1.get_deleter()));
  }
  ...
  { // Test assignment from const ref
    ...
    u1.get_deleter().reset();
    u1 = std::move(u2);
    TC_ASSERT_EXPR(checkArg<TD2 const&>(u1.get_deleter()));
  }
  ...
  { // Test assignment from const ref
    ...
    u1.get_deleter().reset();
    u1 = std::move(u2);
    TC_ASSERT_EXPR(checkArg<CD2 const&>(u1.get_deleter()));
  }
}
...
int tc_utilities_smartptr_unique_ptr_unique_ptr_class_unique_ptr_asgn_move_convert(void) {
  test();
#if TEST_STD_VER >= 23
  static_assert(test());
#endif
