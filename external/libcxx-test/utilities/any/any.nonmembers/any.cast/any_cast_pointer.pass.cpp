...
void test_cast_nullptr() {
    std::any *a = nullptr;
    TC_ASSERT_EXPR(nullptr == std::any_cast<int>(a));
    TC_ASSERT_EXPR(nullptr == std::any_cast<int const>(a));

    const std::any *ca = nullptr;
    TC_ASSERT_EXPR(nullptr == std::any_cast<int>(ca));
    TC_ASSERT_EXPR(nullptr == std::any_cast<int const>(ca));
}
...
void test_cast() {
    ...
    Type::reset();
    {
        ...
        TC_ASSERT_EXPR(std::any_cast<int>(&a) == nullptr);
        TC_ASSERT_EXPR(std::any_cast<int const>(&a) == nullptr);
        TC_ASSERT_EXPR(std::any_cast<int const volatile>(&a) == nullptr);

        // Try a cast to the right type, but as a pointer.
        TC_ASSERT_EXPR(std::any_cast<Type*>(&a) == nullptr);
        TC_ASSERT_EXPR(std::any_cast<Type const*>(&a) == nullptr);

        // Check getting a unqualified type from a non-const any.
        Type* v = std::any_cast<Type>(&a);
        TC_ASSERT_EXPR(v != nullptr);
        ...
        TC_ASSERT_EXPR(cv != nullptr);
        TC_ASSERT_EXPR(cv == v);
        TC_ASSERT_EXPR(cv->value == 999);
...
void test_cast_non_copyable_type()
{
    ...
    std::any a(42);
    std::any const& ca = a;
    TC_ASSERT_EXPR(std::any_cast<NoCopy>(&a) == nullptr);
    TC_ASSERT_EXPR(std::any_cast<NoCopy>(&ca) == nullptr);
}
...
void test_cast_function_pointer() {
    ...
    // to ask.
    TC_ASSERT_EXPR(std::any_cast<void()>(&a) == nullptr);
