...
int tc_utilities_function_objects_arithmetic_operations_multiplies(void) {
...
#endif
    TC_ASSERT_EXPR(f(3, 2) == 6);
#if TEST_STD_VER > 11
    typedef std::multiplies<> F2;
    const F2 f2 = F2();
    TC_ASSERT_EXPR(f2(3,2) == 6);
    TC_ASSERT_EXPR(f2(3.0, 2) == 6);
    TC_ASSERT_EXPR(f2(3, 2.5) == 7.5); // exact in binary
