...
constexpr bool test() {
  // Empty tuple
  {
    ...
    TC_ASSERT_EXPR((T0() <=> T0()) == std::strong_ordering::equal);
  }
  // Mixed types with integers, which compare strongly ordered
  {
    ...
    TC_ASSERT_EXPR((T1(1) <=> T2(1)) == std::strong_ordering::equal);
    TC_ASSERT_EXPR((T1(1) <=> T2(0)) == std::strong_ordering::greater);
    TC_ASSERT_EXPR((T1(1) <=> T2(2)) == std::strong_ordering::less);
  }
  {
    typedef std::tuple<long, unsigned int> T1;
    typedef std::tuple<short, unsigned long> T2;
    ASSERT_SAME_TYPE(decltype(T1() <=> T2()), std::strong_ordering);
    TC_ASSERT_EXPR((T1(1, 2) <=> T2(1, 2)) == std::strong_ordering::equal);
    TC_ASSERT_EXPR((T1(1, 2) <=> T2(0, 2)) == std::strong_ordering::greater);
    TC_ASSERT_EXPR((T1(1, 2) <=> T2(2, 2)) == std::strong_ordering::less);
    TC_ASSERT_EXPR((T1(1, 2) <=> T2(1, 1)) == std::strong_ordering::greater);
    TC_ASSERT_EXPR((T1(1, 2) <=> T2(1, 3)) == std::strong_ordering::less);
  }
  {
    typedef std::tuple<long, int, unsigned short> T1;
    typedef std::tuple<short, long, unsigned int> T2;
    ASSERT_SAME_TYPE(decltype(T1() <=> T2()), std::strong_ordering);
    TC_ASSERT_EXPR((T1(1, 2, 3) <=> T2(1, 2, 3)) == std::strong_ordering::equal);
    TC_ASSERT_EXPR((T1(1, 2, 3) <=> T2(0, 2, 3)) == std::strong_ordering::greater);
    TC_ASSERT_EXPR((T1(1, 2, 3) <=> T2(2, 2, 3)) == std::strong_ordering::less);
    TC_ASSERT_EXPR((T1(1, 2, 3) <=> T2(1, 1, 3)) == std::strong_ordering::greater);
    TC_ASSERT_EXPR((T1(1, 2, 3) <=> T2(1, 3, 3)) == std::strong_ordering::less);
    TC_ASSERT_EXPR((T1(1, 2, 3) <=> T2(1, 2, 2)) == std::strong_ordering::greater);
    TC_ASSERT_EXPR((T1(1, 2, 3) <=> T2(1, 2, 4)) == std::strong_ordering::less);
  }
  // Mixed types with floating point, which compare partially ordered
  {
    ...
    TC_ASSERT_EXPR((T1(1) <=> T2(1)) == std::partial_ordering::equivalent);
    TC_ASSERT_EXPR((T1(1) <=> T2(0.9)) == std::partial_ordering::greater);
    TC_ASSERT_EXPR((T1(1) <=> T2(1.1)) == std::partial_ordering::less);
  }
  {
    typedef std::tuple<long, float> T1;
    typedef std::tuple<double, unsigned int> T2;
    ASSERT_SAME_TYPE(decltype(T1() <=> T2()), std::partial_ordering);
    TC_ASSERT_EXPR((T1(1, 2) <=> T2(1, 2)) == std::partial_ordering::equivalent);
    TC_ASSERT_EXPR((T1(1, 2) <=> T2(0.9, 2)) == std::partial_ordering::greater);
    TC_ASSERT_EXPR((T1(1, 2) <=> T2(1.1, 2)) == std::partial_ordering::less);
    TC_ASSERT_EXPR((T1(1, 2) <=> T2(1, 1)) == std::partial_ordering::greater);
    TC_ASSERT_EXPR((T1(1, 2) <=> T2(1, 3)) == std::partial_ordering::less);
  }
  {
    typedef std::tuple<short, float, double> T1;
    typedef std::tuple<double, long, unsigned int> T2;
    ASSERT_SAME_TYPE(decltype(T1() <=> T2()), std::partial_ordering);
    TC_ASSERT_EXPR((T1(1, 2, 3) <=> T2(1, 2, 3)) == std::partial_ordering::equivalent);
    TC_ASSERT_EXPR((T1(1, 2, 3) <=> T2(0.9, 2, 3)) == std::partial_ordering::greater);
    TC_ASSERT_EXPR((T1(1, 2, 3) <=> T2(1.1, 2, 3)) == std::partial_ordering::less);
    TC_ASSERT_EXPR((T1(1, 2, 3) <=> T2(1, 1, 3)) == std::partial_ordering::greater);
    TC_ASSERT_EXPR((T1(1, 2, 3) <=> T2(1, 3, 3)) == std::partial_ordering::less);
    TC_ASSERT_EXPR((T1(1, 2, 3) <=> T2(1, 2, 2)) == std::partial_ordering::greater);
    TC_ASSERT_EXPR((T1(1, 2, 3) <=> T2(1, 2, 4)) == std::partial_ordering::less);
  }
  {
    ...
    TC_ASSERT_EXPR((T1(nan) <=> T2(nan)) == std::partial_ordering::unordered);
  }
  {
    typedef std::tuple<double, double> T1;
    typedef std::tuple<float, float> T2;
    constexpr double nan = std::numeric_limits<double>::quiet_NaN();
    TC_ASSERT_EXPR((T1(nan, 2) <=> T2(nan, 2)) == std::partial_ordering::unordered);
    TC_ASSERT_EXPR((T1(1, nan) <=> T2(1, nan)) == std::partial_ordering::unordered);
  }
  {
    typedef std::tuple<double, float, float> T1;
    typedef std::tuple<double, double, float> T2;
    constexpr double nan = std::numeric_limits<double>::quiet_NaN();
    TC_ASSERT_EXPR((T1(nan, 2, 3) <=> T2(nan, 2, 3)) == std::partial_ordering::unordered);
    TC_ASSERT_EXPR((T1(1, nan, 3) <=> T2(1, nan, 3)) == std::partial_ordering::unordered);
    TC_ASSERT_EXPR((T1(1, 2, nan) <=> T2(1, 2, nan)) == std::partial_ordering::unordered);
  }
  ...
  {
    {
      ...
      constexpr double nan = std::numeric_limits<double>::quiet_NaN();
      ASSERT_SAME_TYPE(decltype(T1() <=> T2()), std::partial_ordering);
      TC_ASSERT_EXPR((T1(nan) <=> T2(1)) == std::partial_ordering::unordered);
    }
    {
      typedef std::tuple<double, double> T1;
      ...
      constexpr double nan = std::numeric_limits<double>::quiet_NaN();
      ASSERT_SAME_TYPE(decltype(T1() <=> T2()), std::partial_ordering);
      TC_ASSERT_EXPR((T1(nan, 2) <=> T2(1, 2)) == std::partial_ordering::unordered);
      TC_ASSERT_EXPR((T1(1, nan) <=> T2(1, 2)) == std::partial_ordering::unordered);
    }
    {
      ...
      constexpr double nan = std::numeric_limits<double>::quiet_NaN();
      ASSERT_SAME_TYPE(decltype(T1() <=> T2()), std::partial_ordering);
      TC_ASSERT_EXPR((T1(nan, 2, 3) <=> T2(1, 2, 3)) == std::partial_ordering::unordered);
      TC_ASSERT_EXPR((T1(1, nan, 3) <=> T2(1, 2, 3)) == std::partial_ordering::unordered);
      TC_ASSERT_EXPR((T1(1, 2, nan) <=> T2(1, 2, 3)) == std::partial_ordering::unordered);
    }
  }
...
int tc_utilities_tuple_tuple_tuple_tuple_rel_three_way(void) {
  test();
  static_assert(test());
