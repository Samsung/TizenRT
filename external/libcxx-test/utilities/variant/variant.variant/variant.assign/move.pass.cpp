...
void test_move_assignment_same_index() {
  ...
  }
  {
    ...
    TC_ASSERT_EXPR(MoveAssign::move_construct == 0);
    TC_ASSERT_EXPR(MoveAssign::move_assign == 1);
  }
#ifndef TEST_HAS_NO_EXCEPTIONS
  using MET = MakeEmptyT;
  {
...
void test_move_assignment_different_index() {
  ...
  }
  {
    ...
    TC_ASSERT_EXPR(v1.index() == 1);
    TC_ASSERT_EXPR(std::get<1>(v1).value == 42);
    TC_ASSERT_EXPR(MoveAssign::move_construct == 1);
    TC_ASSERT_EXPR(MoveAssign::move_assign == 0);
  }
#ifndef TEST_HAS_NO_EXCEPTIONS
  using MET = MakeEmptyT;
  {
