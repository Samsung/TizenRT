...
struct CopyCannotThrow {
  ...
  CopyCannotThrow(CopyCannotThrow &&) noexcept { TC_ASSERT_EXPR(false); }
  CopyCannotThrow &operator=(const CopyCannotThrow &) noexcept = default;
  CopyCannotThrow &operator=(CopyCannotThrow &&) noexcept { TC_ASSERT_EXPR(false); return *this; }
};
...
void test_copy_assignment_empty_empty() {
  ...
  using MET = MakeEmptyT;
  {
    ...
    makeEmpty(v2);
    V &vref = (v1 = v2);
    TC_ASSERT_EXPR(&vref == &v1);
    TC_ASSERT_EXPR(v1.valueless_by_exception());
    TC_ASSERT_EXPR(v1.index() == std::variant_npos);
  }
#endif // TEST_HAS_NO_EXCEPTIONS
}
...
void test_copy_assignment_empty_non_empty() {
  ...
  }
  {
    ...
    V &vref = (v1 = v2);
    TC_ASSERT_EXPR(&vref == &v1);
    TC_ASSERT_EXPR(v1.index() == 2);
    TC_ASSERT_EXPR(std::get<2>(v1) == "hello");
  }
#endif // TEST_HAS_NO_EXCEPTIONS
}
...
void test_copy_assignment_same_index() {
  ...
  }
  {
    ...
    V &vref = (v1 = v2);
    TC_ASSERT_EXPR(&vref == &v1);
    TC_ASSERT_EXPR(v1.index() == 1);
    TC_ASSERT_EXPR(std::get<1>(v1) == 42);
  }
  {
    ...
    TC_ASSERT_EXPR(&vref == &v1);
    TC_ASSERT_EXPR(v1.index() == 1);
    TC_ASSERT_EXPR(std::get<1>(v1).value == 42);
    TC_ASSERT_EXPR(CopyAssign::copy_construct == 0);
    TC_ASSERT_EXPR(CopyAssign::move_construct == 0);
    TC_ASSERT_EXPR(CopyAssign::copy_assign == 1);
  }
#ifndef TEST_HAS_NO_EXCEPTIONS
  using MET = MakeEmptyT;
  {
...
#endif // _LIBCPP_NO_EXCEPTIONS
    TC_ASSERT_EXPR(v1.index() == 1);
    TC_ASSERT_EXPR(&std::get<1>(v1) == &mref);
  }
#endif // TEST_HAS_NO_EXCEPTIONS
...
void test_copy_assignment_different_index() {
  ...
  }
  {
    ...
    TC_ASSERT_EXPR(CopyAssign::copy_construct == 0);
    TC_ASSERT_EXPR(CopyAssign::move_construct == 0);
    TC_ASSERT_EXPR(CopyAssign::alive == 1);
    V &vref = (v1 = v2);
    TC_ASSERT_EXPR(&vref == &v1);
    TC_ASSERT_EXPR(v1.index() == 1);
    TC_ASSERT_EXPR(std::get<1>(v1).value == 42);
    TC_ASSERT_EXPR(CopyAssign::alive == 2);
    TC_ASSERT_EXPR(CopyAssign::copy_construct == 1);
    TC_ASSERT_EXPR(CopyAssign::move_construct == 1);
    TC_ASSERT_EXPR(CopyAssign::copy_assign == 0);
  }
#ifndef TEST_HAS_NO_EXCEPTIONS
  {
  ...
  }
  {
    ...
    TC_ASSERT_EXPR(MoveThrows::alive == 1);
    // Test that copy construction is used directly if move construction may throw.
    v1 = v2;
    TC_ASSERT_EXPR(v1.index() == 1);
    TC_ASSERT_EXPR(v2.index() == 1);
    TC_ASSERT_EXPR(MoveThrows::alive == 2);
  }
  {
    ...
    TC_ASSERT_EXPR(CopyCannotThrow::alive == 1);
    v1 = v2;
    TC_ASSERT_EXPR(v1.index() == 1);
    TC_ASSERT_EXPR(v2.index() == 1);
    TC_ASSERT_EXPR(CopyCannotThrow::alive == 2);
  }
  {
    using V = std::variant<int, CopyThrows, std::string>;
    ...
    TC_ASSERT_EXPR(v1.index() == 2);
    TC_ASSERT_EXPR(std::get<2>(v1) == "hello");
    TC_ASSERT_EXPR(v2.index() == 2);
    TC_ASSERT_EXPR(std::get<2>(v2) == "hello");
  }
  {
    using V = std::variant<int, MoveThrows, std::string>;
