...
int tc_containers_unord_unord_multimap_swap_member(void) {
    {
    ...
    }
    {
        ...
        TC_ASSERT_EXPR(c2.size() == 6);
        {
            ...
            s.insert("one");
            s.insert("four");
            TC_ASSERT_EXPR(s.find(c2.find(1)->second) != s.end());
            s.erase(s.find(c2.find(1)->second));
            TC_ASSERT_EXPR(s.find(std::next(c2.find(1))->second) != s.end());
        }
        {
            std::set<std::string> s;
            s.insert("two");
            s.insert("four");
            TC_ASSERT_EXPR(s.find(c2.find(2)->second) != s.end());
            s.erase(s.find(c2.find(2)->second));
            TC_ASSERT_EXPR(s.find(std::next(c2.find(2))->second) != s.end());
        }
