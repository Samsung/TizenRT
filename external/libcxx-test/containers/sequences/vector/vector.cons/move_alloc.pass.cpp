...
TEST_CONSTEXPR_CXX20 bool tests()
{
    ...
    }
    {
        ...
        TC_ASSERT_EXPR(l2 == lo);
        TC_ASSERT_EXPR(l.empty());
        TC_ASSERT_EXPR(l2.get_allocator() == min_allocator<MoveOnly>());
        TC_ASSERT_EXPR(is_contiguous_container_asan_correct(l2));
    }
    {
      ...
      TC_ASSERT_EXPR(l2 == lo);
      TC_ASSERT_EXPR(l.empty());
      TC_ASSERT_EXPR(l2.get_allocator() == safe_allocator<MoveOnly>());
      TC_ASSERT_EXPR(is_contiguous_container_asan_correct(l2));
    }
...
int tc_containers_sequences_vector_vector_cons_move_alloc(void) {
    tests();
#if TEST_STD_VER > 17
    static_assert(tests());
#endif
