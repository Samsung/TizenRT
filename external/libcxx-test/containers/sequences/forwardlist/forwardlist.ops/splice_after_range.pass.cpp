...
testd(const C& c, std::ptrdiff_t p, ptrdiff_t f, ptrdiff_t l)
{
    ...
    for (; n1 < size_t1; ++n1, ++i)
        TC_ASSERT_EXPR(*i == t1[n1]);
    TC_ASSERT_EXPR(std::distance(c.begin(), c.end()) == size_t1 + (l > f+1 ? l-1-f : 0));
}
...
tests(const C& c, std::ptrdiff_t p, ptrdiff_t f, ptrdiff_t l)
{
    ...
    TC_ASSERT_EXPR(std::distance(c.begin(), c.end()) == size_t1);
}

int tc_containers_sequences_forwardlist_forwardlist_ops_splice_after_range(void) {
    {
