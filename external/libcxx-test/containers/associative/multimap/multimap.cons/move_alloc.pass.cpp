...
int tc_containers_associative_multimap_multimap_cons_move_alloc(void) {
    {
    ...
    }
    {
        ...
        Counter_base::gConstructed = 0;
        {
            ...
            TC_ASSERT_EXPR(Counter_base::gConstructed <= (int)(4*num));

            {
            M m4(std::move(m2), A(5));
            TC_ASSERT_EXPR(Counter_base::gConstructed >= (int)(3*num));
            TC_ASSERT_EXPR(Counter_base::gConstructed <= (int)(5*num));
            TC_ASSERT_EXPR(m4 == m3);
            LIBCPP_ASSERT(m2.empty());
            }
            TC_ASSERT_EXPR(Counter_base::gConstructed >= (int)(2*num));
            TC_ASSERT_EXPR(Counter_base::gConstructed <= (int)(4*num));
        }
