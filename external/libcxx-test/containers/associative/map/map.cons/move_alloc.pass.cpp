...
int tc_containers_associative_map_map_cons_move_alloc(void) {
    {
    ...
    }
    {
        ...
        Counter_base::gConstructed = 0;
        {
            ...
            TC_ASSERT_EXPR(m3 == m2);
            LIBCPP_ASSERT(m1.empty());
            TC_ASSERT_EXPR(Counter_base::gConstructed >= (int)(num+6));
            TC_ASSERT_EXPR(Counter_base::gConstructed <= (int)(num+6+m1.size()));

            {
            M m4(std::move(m2), A(5));
            TC_ASSERT_EXPR(Counter_base::gConstructed >= (int)(num+6));
            TC_ASSERT_EXPR(Counter_base::gConstructed <= (int)(num+6+m1.size()+m2.size()));
            TC_ASSERT_EXPR(m4 == m3);
            LIBCPP_ASSERT(m2.empty());
            }
            TC_ASSERT_EXPR(Counter_base::gConstructed >= (int)(num+3));
            TC_ASSERT_EXPR(Counter_base::gConstructed <= (int)(num+3+m1.size()+m2.size()));
        }
    ...
    }
    {
        ...
        M m3(std::move(m1), A());
        TC_ASSERT_EXPR(m3 == m2);
        TC_ASSERT_EXPR(m3.get_allocator() == A());
        TC_ASSERT_EXPR(m3.key_comp() == C(5));
        LIBCPP_ASSERT(m1.empty());
    }
    {
