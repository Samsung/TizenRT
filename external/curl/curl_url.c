...
static const struct Curl_handler * const protocols[] = {
...
#if defined(USE_SSL) && !defined(CONFIG_DISABLE_HTTP)
  &Curl_handler_https,
#endif
...
#if defined(USE_SSL) && !defined(CONFIG_DISABLE_FTP)
  &Curl_handler_ftps,
#endif
  ...
  &Curl_handler_ldap,
    ...
    ((defined(CONFIG_USE_OPENLDAP) && defined(USE_SSL)) || \
     (!defined(CONFIG_USE_OPENLDAP) && defined(HAVE_LDAP_SSL)))
  &Curl_handler_ldaps,
#endif
#endif
...
#if !defined(CONFIG_DISABLE_SMB) && defined(USE_NTLM) && \
...
#if defined(CONFIG_CA_BUNDLE)
  result = Curl_setstropt(&set->str[STRING_SSL_CAFILE_ORIG], CONFIG_CA_BUNDLE);
  if(result)
    return result;

  result = Curl_setstropt(&set->str[STRING_SSL_CAFILE_PROXY], CONFIG_CA_BUNDLE);
  if(result)
    return result;
#endif
#if defined(CONFIG_CA_PATH)
  result = Curl_setstropt(&set->str[STRING_SSL_CAPATH_ORIG], CONFIG_CA_PATH);
  if(result)
    return result;

  result = Curl_setstropt(&set->str[STRING_SSL_CAPATH_PROXY], CONFIG_CA_PATH);
  if(result)
    return result;
#endif
...
#ifdef CONFIG_CURL_DEBUG /* debug-only code */
...
#if defined(USE_RECV_BEFORE_SEND_WORKAROUND) && defined(CONFIG_CURL_DEBUG)
  conn->postponed[0].bindsock = CURL_SOCKET_BAD; /* no file descriptor */
  conn->postponed[1].bindsock = CURL_SOCKET_BAD; /* no file descriptor */
#endif /* USE_RECV_BEFORE_SEND_WORKAROUND && CONFIG_CURL_DEBUG */
