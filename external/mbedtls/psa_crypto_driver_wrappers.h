...
/* TizenRT Samsung SE opaque + transparent cipher driver */
#if defined(TIZENRT_PSA_DRIVER_PRESENT)
#include "alt/tizenrt_psa_driver.h"
#endif
...
#endif /* PSA_CRYPTO_ACCELERATOR_DRIVER_PRESENT */
#if defined(TIZENRT_PSA_DRIVER_PRESENT)
            /* TizenRT AES hardware transparent driver */
            status = tizenrt_transparent_cipher_encrypt_setup(
                &operation->ctx.tizenrt_ctx,
                attributes,
                key_buffer,
                key_buffer_size,
                alg );
            if( status == PSA_SUCCESS )
                operation->id = TIZENRT_TRANSPARENT_CIPHER_DRIVER_ID;
...
#endif /* PSA_CRYPTO_ACCELERATOR_DRIVER_PRESENT */
#if defined(TIZENRT_PSA_DRIVER_PRESENT)
        /* TizenRT transparent cipher driver – AES at LOCAL_STORAGE is already
         * handled above; this location case is intentionally not used. */
#endif /* TIZENRT_PSA_DRIVER_PRESENT */
        default:
            /* Key is declared with a lifetime not known to us */
            (void)status;
...
#if defined(TIZENRT_PSA_DRIVER_PRESENT)
            /* TizenRT AES hardware transparent driver */
            status = tizenrt_transparent_cipher_decrypt_setup(
                &operation->ctx.tizenrt_ctx,
                attributes,
                key_buffer,
                key_buffer_size,
...
#endif /* PSA_CRYPTO_ACCELERATOR_DRIVER_PRESENT */
#if defined(TIZENRT_PSA_DRIVER_PRESENT)
        case TIZENRT_TRANSPARENT_CIPHER_DRIVER_ID:
            return( tizenrt_transparent_cipher_set_iv(
                        &operation->ctx.tizenrt_ctx,
                        iv, iv_length ) );
#endif /* TIZENRT_PSA_DRIVER_PRESENT */
    }
...
#if defined(TIZENRT_PSA_DRIVER_PRESENT)
        case TIZENRT_TRANSPARENT_CIPHER_DRIVER_ID:
            return( tizenrt_transparent_cipher_update(
                        &operation->ctx.tizenrt_ctx,
                        input, input_length,
                        output, output_size, output_length ) );
#endif /* TIZENRT_PSA_DRIVER_PRESENT */
...
#if defined(TIZENRT_PSA_DRIVER_PRESENT)
        case TIZENRT_TRANSPARENT_CIPHER_DRIVER_ID:
            status = tizenrt_transparent_cipher_abort(
                         &operation->ctx.tizenrt_ctx );
            return( status );
#endif /* TIZENRT_PSA_DRIVER_PRESENT */
    }
