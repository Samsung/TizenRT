    ...
    size_t *key_buffer_size)
{
    ...
    *key_buffer_size = sizeof(tizenrt_se_key_ctx_t);
    return PSA_SUCCESS;
}
    ...
    size_t *key_buffer_length)
{
    ...
    if (key_buffer_size < sizeof(tizenrt_se_key_ctx_t)) {
    ...
    tizenrt_se_key_ctx_t *ctx = (tizenrt_se_key_ctx_t *)key_buffer;
    ctx->key_index = (uint32_t)key_idx;
    ctx->key_type  = (uint32_t)hal_key;

    *key_buffer_length = sizeof(tizenrt_se_key_ctx_t);
    return PSA_SUCCESS;
}
    ...
    size_t *data_length)
{
    ...
    const tizenrt_se_key_ctx_t *ctx =
        (const tizenrt_se_key_ctx_t *)key_buffer;
