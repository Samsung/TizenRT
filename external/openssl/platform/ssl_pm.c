...
static void ssl_pm_debug(void *ctx, int level, const char *file, int line,const char *str)
{
	dbg("%s:%04d: %s", file, line, str);
}
/*********************************************************************************************/
/************************************ SSL arch interface *************************************/
...
int ssl_pm_new(SSL *ssl)
{
	...
	mbedtls_ssl_conf_dbg(&(ssl_pm->conf), ssl_pm_debug, stdout);
#endif
