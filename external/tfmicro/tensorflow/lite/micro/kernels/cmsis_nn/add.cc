...
void EvalAddInt32(TfLiteContext* context, TfLiteNode* node, TfLiteAddParams* params,
             const OpData* data, const TfLiteEvalTensor* input1,
             const TfLiteEvalTensor* input2, TfLiteEvalTensor* output) {
  tflite::ArithmeticParams op_params;
  SetActivationParams(std::numeric_limits<int32_t>::min(),
...
TfLiteStatus EvalAdd(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (output->type == kTfLiteFloat32) {
    ...
    EvalAddInt32(context, node, params, data, input1, input2, output);
